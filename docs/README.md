...
This folder holds the vaults you create with a plain name, the key that protects saved cloud logins, your settings, the bundled engine, and a `logs` folder of diagnostic logs. Keeping it out of the program folder means upgrading or deleting the program never affects your vaults or keys.

Vaultonaut writes a diagnostic log so a problem on a long-running or headless service can be looked into after the fact, even when nothing was watching the screen. The logs live in the `logs` folder inside your data directory, one file per day named by the date, such as `2026-09-13.log`. Files older than 30 days are removed automatically, so the folder never grows without bound. Every line is scrubbed of anything that looks like a password, key, token, or recovery phrase before it is written, so a log is always safe to read or share.
...
- **I want to see what happened.** Vaultonaut keeps a diagnostic log in the `logs` folder inside your data directory (see "Where your data lives"), one file per day. It records what the background service did — mounts, unmounts, self-heal updates, backups, and any warnings. Every line is scrubbed of passwords, keys, and recovery phrases, so a log is safe to read or share when you need help.
