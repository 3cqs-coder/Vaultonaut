...
If you are running a long-lived service and want a trail to diagnose a problem, turn on **Keep diagnostic logs** in Settings, or start the service with `--diagnostic-log`. The logs then live in a `logs` folder inside your data directory, one file per day named by the date, such as `2026-09-13.log`, and files older than 30 days are removed automatically. Even then, a log never records a file name or any file contents from inside a vault, and every line is scrubbed of anything that looks like a password, key, token, or recovery phrase, so a log is always safe to read or share. It does note which vault was opened and when, which is why it stays off unless you ask for it.
