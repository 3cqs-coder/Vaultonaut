...
**First launch on macOS.** The Mac app is signed for local use rather than through a paid developer account, and macOS quarantines anything downloaded from the web, so the first time you open it you may see a warning that it "cannot be opened because the developer cannot be verified," or that the app "is damaged and can't be opened." The app is not damaged. To open it, right-click (or Control-click) the app in your Applications folder and choose **Open**, then **Open** again in the dialog. You can also go to **System Settings**, then **Privacy & Security**, and choose **Open Anyway**. If you prefer the command line, run `xattr -dr com.apple.quarantine /Applications/Vaultonaut.app`. You do this once per install, and again after an update, because each download is quarantined the same way.
