...
If a loss ever exceeds what the recovery data can rebuild, `vdisk heal` repairs everything it can and names the files it could not fully recover, so you can restore just those from a backup or a saved version. It never guesses, and it never reports a still-damaged file as fixed. A tamper check names those files too, and reports one unreadable file as damaged rather than stopping the whole check. And if a rebuilt file cannot be written back — the disk is full, the vault is read-only, or a file is locked — heal reports how many repairs it could not save and still fixes everything else, so one stuck file never stops the rest.
