...
If you are running a long-lived service and want a trail to diagnose a problem, turn on **Keep diagnostic logs** in Settings, or start the service with `--diagnostic-log`. The logs then live in a `logs` folder inside your data directory, one file per day named by the date, such as `2026-09-13.log`, and files older than 30 days are removed automatically. Even then, a log never records a file name or any file contents from inside a vault. Every line is scrubbed of anything that looks like a password, key, token, or recovery phrase, so a log is always safe to read or share. It does note which vault was opened and when, which is why it stays off unless you ask for it.
...
**Better still: the relay can step out of the way.** When both machines can reach a relay, Vaultonaut also tries to turn that relayed session into a **direct** one — even when *neither* machine has an open port and *neither* router will map one. Using the relay only to introduce the two sides, the machines open connections to each other at the same instant so each router, having just seen an outbound connection, lets the other's in — a "hole punch." If it works, the vault data flows **straight between the two machines** and the hub is left carrying nothing; if it cannot (some stricter networks make a direct path impossible), the sync simply keeps using the relay. It is automatic and safe by construction: the direct path runs the *same* pinned-certificate connection as the relay, so it only ever changes which route the encrypted data takes, never who is trusted or what is exposed. You do not have to do anything for this — it happens on its own.

You can choose how much of this to use under **Peer connections** in Settings (or `vdisk peer-mode`): **Automatic** — the default — connects directly when it can and falls back to the relay otherwise; **Relay only** always goes through the relay; and **Direct only** insists on a direct connection and never uses a relay.
