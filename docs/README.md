...
The one exception is specific to macOS. Because FUSE-T presents a drive as a local network mount, an engine that has *fully* hung — not merely crashed — can occasionally be cleared only by a reboot. That is a limitation of the mount type, not of the tool. Crucially, Vaultonaut never makes that worse: it will not force-kill a hung engine while its drive is still mounted (which is what would turn a recoverable hang into a reboot), so its own teardown can never be the cause. The check is careful to distinguish a wedged drive from a merely busy one, so a drive doing real work is never disturbed. It also never tears a drive down on its own; that stays a deliberate click. On the command line, `vdisk status` shows the same information, listing a wedged drive as `stuck` with the exact command to recover it. If you would rather a hung drive be stopped and restarted for you, turn on **Restart a wedged drive** in **Settings** (off by default, and `vdisk wedge-restart on`); until you do, a wedged drive stays locked and secured, never restarted, and nothing in the vault is lost either way.
