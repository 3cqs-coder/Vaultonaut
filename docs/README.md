...
- **I want to see what happened.** By default Vaultonaut keeps no log on disk, for privacy. Turn on **Keep diagnostic logs** in Settings (or start the service with `--diagnostic-log`) and it writes a daily log in the `logs` folder inside your data directory (see "Where your data lives"). It records what the background service did — mounts, unmounts, self-heal updates, backups, and any warnings — but never a file name or contents, and passwords, keys, and recovery phrases are removed, so a log is safe to read or share when you need help. Turn it back off when you are done to stop keeping a record.
