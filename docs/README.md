...
A vault holds more than files. The same vault is also a built-in password manager and secure-notes keeper, so your logins, two-factor codes, cards, keys, and private notes live in the same encrypted place as your documents. It runs on your own computer, with no account and no cloud required.
...
- [Verifying your download](#verifying-your-download)
  - [Reproducible builds — verify it yourself](#reproducible-builds--verify-it-yourself)
...
- [For developers and power users](#for-developers-and-power-users)
  ...
  - [Run a fleet with Docker](#run-a-fleet-with-docker)
...
- [Splitting a vault across places](#splitting-a-vault-across-places)
  - [Offering storage to someone else (a storage node)](#offering-storage-to-someone-else-a-storage-node)
...
- [Tamper detection](#tamper-detection)
  ...
  - [Recording every unlock (approval on use)](#recording-every-unlock-approval-on-use)
...
- macOS — FUSE-T (needs no kernel extension and no reboot). It is strongly recommended and, for Finder copies, effectively required. It is safe to keep alongside an older kernel-extension FUSE driver if you already use one for other apps.
...
Vaultonaut runs well on small, always-on machines such as a single-board computer or a home NAS. Two things are worth setting up correctly.

The first is the runtime. The command-line and from-source install need Node.js 24.7 or newer on a 64-bit system. A modern 64-bit single-board computer with at least 1 GB of RAM, running a 64-bit operating system, meets this. A very old or 32-bit board cannot run a current Node.js and is not supported. On Linux the mount driver is simply FUSE, which nearly every distribution already includes.
...
Each release also carries a second signature made with a post-quantum algorithm (ML-DSA, a NIST standard), so its authenticity holds even against a future quantum computer. The verifier checks both signatures automatically and accepts a release only if both pass. There is nothing extra to do; the post-quantum key travels with the copy and is published below for anyone who wants to anchor it independently.

<details><summary>Post-quantum signing public key</summary>

```

```
...
- Watch your other machines — the **Fleet** tile opens a view of every other Vaultonaut machine you have added, each with a live status (online, needs attention, or unreachable) and a short health summary. The app checks each one in the background, so the fleet view is instant. To add a machine, create a monitoring token on it with `vdisk api-token create`, then enter that machine's address and the token; the hub verifies it is reachable before saving. From a machine's row you can also **Refresh** its health check or **Lock** it — unmount every vault on it now — without leaving the hub. The hub only ever reads each machine's health — counts and status, never a vault's contents — and every machine stays fully in control of its own vaults. If a machine goes unreachable or reports a problem, the hub sends the notification you have configured.
...
Nearly everything the command line can do is also here. Each vault has a single **Manage** button that opens a panel grouping its actions:
...
**First launch on macOS.** The Mac app is signed for local use rather than through a paid developer account, and macOS quarantines anything downloaded from the web, so the first time you open it you may see a warning that it "cannot be opened because the developer cannot be verified," or that the app "is damaged and can't be opened." The app is not damaged. To open it, try to open Vaultonaut once and dismiss the warning, then go to **System Settings**, then **Privacy & Security**, scroll to the Security section, and choose **Open Anyway**. If you prefer the command line, run `xattr -dr com.apple.quarantine /Applications/Vaultonaut.app`. You do this once per install, and again after an update, because each download is quarantined the same way. (On older versions of macOS you could instead right-click the app and choose **Open**; recent versions have removed that shortcut for apps signed this way, so use **Open Anyway**.)
...
Closing the window behaves differently depending on whether the background service is running. If it is (you turned on Start at login), closing the window just puts the app away and leaves the service running, so your vaults and scheduled work are untouched. If it is not, closing the window also stops the app and locks any vault you have open, flushing saved work first. In that case, when a vault is unlocked it asks you to confirm first, so one stray click does not tear down your session; with nothing open, it closes right away.
...
This folder holds the vaults you create (vaults made in the app use an opaque folder name — see "What a vault protects"), the key that protects saved cloud logins, your settings, and the bundled engine. Keeping it out of the program folder means upgrading or deleting the program never affects your vaults or keys.

Vaultonaut keeps *no* diagnostic log on disk by default, on purpose. A dated log of which vault you opened and when it changed would be a record of your activity that anyone with access to the computer could later read, which is exactly what an encrypted vault is meant to avoid. So nothing is written to disk unless you turn it on.

If you are running a long-lived service and want a trail to diagnose a problem, turn on **Keep diagnostic logs** in Settings, or start the service with `--diagnostic-log`. The logs then live in a `logs` folder inside your data directory, one file per day named by the date (such as `2026-09-13.log`). Files older than 30 days are removed automatically. Even then, a log never records a file name or any file contents from inside a vault. Every line is scrubbed of anything that looks like a password, key, token, or recovery phrase, so a log is always safe to read or share. It does note which vault was opened and when, which is why it stays off unless you ask for it.
...
The drive's own format matters for one thing: the largest single file. Vaultonaut encrypts each file into its own store file of about the same size, so a file in the vault cannot be larger than the drive's format allows. On a drive formatted as exFAT, APFS, NTFS, or a Linux file system, there is no practical limit. On an older FAT32 drive, no single file can reach 4 GB, so a large video or disk image in the vault will not fit there. For a portable drive you will move between Windows, macOS, and Linux, exFAT is the best choice: every system reads it, and it has no such size limit.
...
vdisk name     <path> ["title"]  Show or set a vault's encrypted name and description (--description "text", or --anonymize to hide the folder name; needs the password)
...
vdisk serve    <name|path>       Serve a vault as a node another machine can mirror to (encrypted only). --lan serves it directly on your local network, discovered automatically (no setup); --wan also opens a router port automatically (NAT-PMP/PCP) to reach it from anywhere, degrading to LAN-only if the router refuses; --onion serves it over Tor with no exposed address or forwarded port, reachable only by holding the connect code (Tor is set up automatically — used if already running, downloaded on demand otherwise); add --single-hop for a faster onion when this machine's location need not be hidden, or --stable-address to reuse the same onion and connect code across serves (--new-address rotates it). See "Reaching a vault over Tor"
...
vdisk tor-bridge [mode]          Show or set a Tor bridge for a network that blocks Tor (off | snowflake | obfs4 | meek | custom "<line>" ...)
...
vdisk ui       [--port 7420]     Launch the local web interface (if it is already running, says so instead of failing; add --bind <address> to reach it from another machine — a password is then required and it is served over HTTPS; --allow-ip / --deny-ip restrict callers; --diagnostic-log keeps a daily log file for this run, off by default; on Windows --supervised auto-restarts it if it ever crashes)
...
- **any command** — `--json` prints a single JSON object on standard output instead of human text, for scripting and other tools: `{ "ok": true, "data": … }` on success, or `{ "ok": false, "error": { "code", "message" } }` on failure, with the exit code preserved and human messages sent to standard error. Read commands such as `status`, `keys`, `fingerprint`, `doctor`, `list`, and `update-check` fill in `data`; other commands still return `{ "ok": true }` so a script can branch on success. `--data-dir <folder>` uses a custom data directory instead of the per-user default (see "Where your data lives"); pass it consistently to every command. `--dns-order <ipv4first|ipv6first|verbatim>` sets how outbound connections resolve host names. The default, `ipv4first`, prefers IPv4 but still falls back to IPv6 when that is all there is. Preferring IPv4 keeps downloads, cloud vaults, timestamps, and relays working on a machine whose IPv6 is broken or flaky (common on some servers). Change it only if you specifically need a different order.
...
**Upgrading.** Set `VAULTONAUT_VERSION` in `docker/.env` to the new release, then pull and bring the fleet back up:
...
Compose recreates each container in place. Every vault, key, and setting lives in the named data volumes, so nothing is lost, and the new image verifies its own signed bundle before it serves. To roll back, set the previous release and run the same two commands.

**Building from source.** To build the image locally instead of pulling the published one, uncomment the `build` blocks in the Compose file and add `--build` to the `up` command. Building on its own is `docker build -f docker/Dockerfile -t vaultonaut:latest .` from the repository root.
...
When a router will not cooperate, you run your own `vdisk relay` hub on any public host. The hub is a token-gated, TCP-only bridge with per-node identity binding, brute-force throttling, and registration caps, so nothing about your network depends on someone else's server. When both the node and the hub are recent, the hub carries all of a node's connections over a single multiplexed link, which avoids a fresh connection to the hub per request. It falls back automatically when either side is older, and you can turn it off with `VAULTONAUT_RELAY_MUX=0`. When only a relay is reachable, the two peers still try to upgrade to a direct connection by hole-punching. That upgrade is coordinated over an authenticated signaling channel on the same hub, scoped by a per-node credential that can never register or impersonate a node. On success the hub drops out of the data path, and the direct hop carries the identical pinned-TLS stream, so the upgrade changes only the route.
...
New vaults created in the app go one step further and hide their own name. The folder on disk is named by the vault's own opaque identifier, so browsing the disk (or a cloud provider that holds it) reveals nothing about what a vault is; the name and description you give it are encrypted with the vault and readable only once you unlock it. Each device you unlock a vault on remembers its name locally so the list shows it right away. Until then, or on a device that has never opened it, the list shows the vault's opaque identifier instead of a readable name, which still gives nothing away. The folder name never changes, so renaming or describing a vault never affects where its backups or mirrors go. And if you do move or rename the folder itself on disk, re-add it from its new location, and its settings — mirrors, backups, schedules, favorites — come with it, because they follow the vault by its identity, not its path. On the command line, `vdisk create <name> --opaque` does the same, and `vdisk name <vault>` shows or changes the encrypted name and description.

An existing vault that still has a visible folder name is converted automatically the next time you unlock it, because **Hide folder names** in **Settings** is on by default. You can also convert one at any time from **Manage → Name & description → Hide the folder name**, or with `vdisk name <vault> --anonymize`. Turn the setting off to leave older folder names as they are; vaults created in the app stay opaque either way. The vault must be unmounted. Its files, keys, and identity are untouched — only the folder is renamed — and its settings come along. If the vault is paired with a decoy, pair it again afterward.
...
You can also search inside your files. In the Search window, switch to "Inside files"; on the command line, add `--in`. This searches the words in your text files — notes, code, data files, and similar — and inside common documents: PDFs, word-processor documents (`.docx`), spreadsheets (`.xlsx` and `.xlsm`), and saved web pages. A scanned PDF or a photo has no text layer to read on its own; turn on text recognition, described below, to read the words in those too.

It works from a small search index that lives inside the vault. The vault must be open, because the index is built by reading your files as you can read them and is written back into the vault encrypted, so the words never touch your disk in the clear. Indexing is incremental: after the first build, only files you have changed are read again. Because the index is a file inside the vault, it is protected and checked for tampering like everything else — so after it updates, take a fresh tamper snapshot. The index reveals nothing that opening the vault would not: anyone who could read it could already read your files.
...
You choose how long the link lasts, from an hour up to 30 days, and how many times it can be opened (once, by default). You can also add a password the recipient must enter — and that password becomes part of the decryption key itself, not just a gate on the download. So the item cannot be opened without both the link and the password: someone who somehow captured the whole link still could not read it without the password, and the app never can, because it holds neither the key from the link nor a way to derive it. Send the password by a different channel than the link. The recipient opens the link in any browser, and the item is decrypted right there in their page. Only encrypted data is stored, and the key that unlocks it rides in the link itself and never reaches the server. If someone guesses the link password too many times the link destroys itself, and once its time or open-count runs out it stops working for good.
...
Your phone needs a way to reach this computer. On the same network that can be a local address; from elsewhere it is a VPN or tunnel you already run. For the viewer to install to the home screen and work fully offline, the phone must reach it over a secure `https` address (a reverse proxy, a VPN, or a tunnel with a real certificate). A browser only grants offline storage to a secure page, so a plain address will not install the viewer. Over a plain address you can still view the vault while connected. When you do save a copy for offline use, only the encrypted files are stored on the phone; the decrypted content is never written to the phone's storage. The viewer shows a small badge that tells you which state you are in: **No-trace session** while nothing is stored on the device, and **Encrypted copy on this device** once you save one for offline use — that copy stays unreadable without your key (held only in memory) and is erased when you Lock. The badge describes only what the viewer itself writes, not what the phone's own operating system may keep.
...
If owner access is ever lost, the trustees each reveal their share (in the same **Owner recovery** dialog, or `vdisk recovery-share`), and any threshold-many of them together restore owner access under a new password (there too, or `vdisk owner-recover`). Recovery is self-checking: it confirms the reconstructed keys actually match this vault, so a wrong or corrupted share is caught and refused rather than quietly producing the wrong key.
...
The one exception is specific to macOS. Because FUSE-T presents a drive as a local network mount, an engine that has *fully* hung — not merely crashed — can occasionally be cleared only by a reboot. That is a limitation of the mount type, not of the tool. Crucially, Vaultonaut never makes that worse: it will not force-kill a hung engine while its drive is still mounted (which is what would turn a recoverable hang into a reboot), so its own teardown can never be the cause. The check is careful to distinguish a wedged drive from a merely busy one, so a drive doing real work is never disturbed. It also never tears a drive down on its own; that stays a deliberate click. On the command line, `vdisk status` shows the same information, listing a wedged drive as `stuck` with the exact command to recover it. If you would rather have the hung background service restarted for you, turn on **Restart a wedged drive** in **Settings** (off by default, and `vdisk wedge-restart on`). Your vaults are unmounted and stay locked until you unlock them again. Until you turn it on, a wedged drive stays locked and secured, never restarted, and nothing in the vault is lost either way.
...
Beyond detecting damage, a vault can repair it. `vdisk protect` (or **Self-heal** in the web interface) adds recovery data inside the vault, alongside your encrypted files, and `vdisk heal` later uses it to rebuild what has been lost. It recreates a file, or a whole folder, that was deleted outright, and it repairs recovery blocks that were themselves damaged — as long as the loss stays within the recovery budget you chose (below). Recovery data is built for the everyday risk of bit-rot: a run of bad blocks scattered through the vault. A very large single loss — a whole big file deleted — can be a bigger share of the vault than that budget, and no amount of recovery data replaces a backup for a loss that size. Self-healing scales to very large vaults. Vaultonaut sizes its recovery blocks to the vault, so it covers vaults into the multi-terabyte range while keeping memory use bounded. A vault beyond that range is skipped, with any earlier recovery data left in place untouched. The vault's own contents stay fully usable at any size, so protect a vault that large with backups and mirroring, which have no size limit.
...
A vault does not have to live on this computer. Its encrypted store can sit directly on cloud storage — a personal account (Google Drive, Dropbox, Microsoft OneDrive) or object storage (Amazon S3, Backblaze B2, a WebDAV server). It still mounts here as a normal drive and stays zero-knowledge. The cloud only ever holds ciphertext; your keys and the vault's settings stay on this machine, so the provider learns nothing, not even a file name.

For a personal account, sign in once from **Cloud storage** in the app, or run `vdisk cloud connect drive` (or `dropbox`, or `onedrive`). This opens the provider's sign-in page in your browser; when you approve, the account is saved for making vaults. Only a sign-in token is kept, encrypted on this machine — never your account password. OneDrive rotates its sign-in token as it is used. The tool captures each refreshed token automatically when you unmount, so the account keeps working across sessions without a reconnect. If a vault is ever left idle so long that even the refreshed token expires, simply run the connect step again.

For object storage with an access key, save it once with `vdisk cloud add --type <s3|b2|webdav> --label <name> key=value …` (the keys are the usual fields for that backend, for example `provider`, `region`, `access_key_id`, and `secret_access_key` for S3). Secrets are encrypted on this machine before they are stored. On a shared computer, keep the secret off the command line (where other users could read it) by putting the `key=value` lines in a file and passing `--secrets-file <path>` instead. The file takes one `key=value` per line; blank lines and lines starting with `#` are ignored.
...
**On a schedule.** In the **Back up** window you can also set a vault to back up automatically — every few hours, or once a day at a time you choose. Scheduled backups run while the app is open and only when the vault is unmounted, so they never interrupt what you are doing; a run that finds the vault mounted simply waits for the next opportunity. You pick daily times in your computer's own local time, and they are shown with your time zone. A daily time is remembered together with that time zone, so a "09:00" backup runs at 09:00 by your clock every day of the year — it stays put across a daylight-saving change instead of sliding an hour. Behind the scenes the exact moment to run is worked out fresh for each day in universal time (UTC), so the internal clock is always unambiguous while the time you set stays the time it runs.
...
**On the same network — direct, discovered automatically, no setup.** When both devices are on the same network (the same Wi-Fi or LAN), choose **This network** in the Serve window — it is the default — or run `vdisk serve <vault> --lan`. The vault is served over an encrypted connection bound to your local network, and your other device *finds it on its own*: there is nothing to configure, no port forwarding, and no server in the middle. Pair it once with the printed connect code, exactly as below. After that the two devices connect directly, and if the serving machine's address changes, discovery keeps it reachable. The discovery is a tiny broadcast that reveals only *where* an already-paired node is, never anything about your vaults. Your other device still pins the node's certificate before transferring anything, so it only ever talks to the exact node you authorized. If the two devices are ever on different networks, the same peer falls back to a relay (below) automatically.

**Across the internet, if your router allows — still no port forwarding.** Choose **This network + the internet** in the Serve window, or run `vdisk serve <vault> --wan`. This does everything *This network* does, and it also asks your router to open a port automatically (using the standard NAT-PMP and PCP protocols). If the router obliges and the address it hands back is a real public one, the node is reachable directly from anywhere, and the connect code carries that internet address. The port is released the moment you stop serving. If the router does not support automatic mapping, or hands back an address that your internet provider shares among many customers and so is not publicly reachable, the node simply stays reachable on your local network only. Nothing breaks. Either way the connection is encrypted and no data about your vaults is exposed. When you need to reach the node from a network whose router will not open a port at all, use a relay (below).

By default (`127.0.0.1`) the node listens on the serving machine only. To reach it from another computer off your network, put it behind a tunnel or VPN you already run — an SSH tunnel, a VPN, or a self-hosted relay — and use that address. The tunnel carries only ciphertext and also encrypts the hop, so nothing is exposed by reaching across the network. (You can instead bind a specific address with `--bind`, but *This network* / `--lan`, a tunnel, or a private network are the safer, easier defaults.) Add `--port <n>` to pin the local port, or `--read-only` to serve a vault that a peer can copy from but never change.
...
The token itself is sent in the clear to the hub. Your vault data and the serve login stay encrypted end to end (see the next paragraph), so the hub only ever handles scrambled data — but the hub, and anyone on the network path to it, can read the token. So run the hub on a host you trust, or reach it over a VPN or an HTTPS proxy you control, on an untrusted network. The hub defends the token on its side too. It refuses to start with a token short enough to guess (let it generate a strong one), rate-limits repeated wrong-token attempts from one source, and caps how many nodes a single host may register. It also binds each node's identity to a private per-node secret, so no one who merely knows the token can evict or impersonate a running node.
...
**What you need.** Nothing to install. Both the machine that *serves* and the machine that *connects* need Tor, and Vaultonaut sets it up for you: if it finds a Tor already running — for example the **Tor Browser** open on your desktop, or the `tor` package on a server — it uses that; otherwise it downloads a verified copy of Tor on demand and runs it quietly in the background, the same way it fetches its storage engine. Either way there is no configuration, and nothing is left running once you stop.

**How to use it.**

1. On the machine with the vault, serve it over Tor: `vdisk serve <vault> --onion`, or open the **Serve** window and choose **An anonymizing overlay network (Tor)**. Vaultonaut asks Tor for a fresh `.onion` address, serves the encrypted vault behind it, and shows you the usual one-paste connect code (now carrying the `.onion` address and a private access key — see below).
2. On the other machine, add that code as a peer and mirror or receive the vault exactly as you would for any node. Vaultonaut automatically routes the connection through Tor to reach the `.onion` address; you do nothing extra.

**Only the code can connect.** The `.onion` address alone is not enough to reach your node. Each time you serve, Vaultonaut generates a one-time private key, tells Tor to accept *only* that key, and folds the key into the connect code. Tor will not so much as acknowledge the service to anyone who does not hold it — so even if the `.onion` address is seen, logged, or leaked, it is useless without the code. The machine you gave the code to authorizes itself automatically; you never handle the key yourself. This is Tor's own *client authorization*, and it is always on for onion serves.

**Faster, if you don't need to hide this machine (single-hop).** A normal onion connection bounces through three relays at each end, which is what hides *both* machines — but it also makes transfers slower. If you don't need to hide the location of the machine that is *serving* (for example it is your own always-on box, or a server whose address is no secret), turn on **Faster onion (single-hop)** in the Serve window (or `--single-hop` on the command line, alongside `--onion`). It uses a single hop from your node to the meeting point instead of three, so a large sync goes noticeably faster. The trade-off is only about *location*: this machine's whereabouts are no longer hidden from the network. Nothing else changes — the machine that connects is still fully anonymous, and the unguessable address plus the private access key still decide who can reach it, so it never lets anyone new in. It is off by default and entirely opt-in; leave it off to keep both ends hidden.

**Keep the same address (stable).** By default each serve gets a brand-new throwaway `.onion` and a new connect code, so nothing lingers once you stop. If you would rather share the code **once** and reuse it — say you serve the same vault to the same machine regularly — turn on **Keep the same address every time** in the Serve window (or `--stable-address` with `--onion`). Vaultonaut then remembers this vault's onion keys (stored encrypted on your computer) and re-publishes the exact same address and connect code on every serve. It is opt-in because a stable address is a longer-lived identifier; leave it off for a fresh address each time. To change a stable address later, serve once with `--new-address` (or the equivalent), which forgets the old keys and mints new ones.

**If your network blocks Tor (bridges).** Some networks block Tor itself. When that happens, turn on a **bridge**: an unlisted way into the Tor network that disguises the traffic so it still gets through. In the Serve window, under **If your network blocks Tor**, pick **Snowflake** (a good general choice), **obfs4**, or **meek** — Vaultonaut ships the built-in bridge details, so there is nothing to look up or paste. The setting applies to all of Vaultonaut's Tor connections and takes effect right away. Leave it off unless plain Tor will not connect. If someone gave you a specific bridge line, set it from the command line with `vdisk tor-bridge custom "<line>"`; `vdisk tor-bridge off` returns to a direct connection.

**What it protects, and its honest limits.** As with every Vaultonaut connection, only encrypted data ever travels — Tor adds *anonymity and reachability*, not secrecy, because the vault is already encrypted end to end before it leaves. Unless you choose a stable address, the `.onion` exists only while you are serving: stop the node and it is gone, so nothing stays published. The private access key above, and the vault's serve login, both still gate access on top of the address. Two honest limits: a Tor connection is slower than a direct one (traffic takes a longer path), so a first large sync can take a while; and this is for connecting **nodes and desktop apps** — the phone's in-browser viewer would need the Tor Browser on the phone, so use one of the other reach options for that. It works the same on macOS, Windows, and Linux, with nothing platform-specific to install beyond Tor itself.
...
### Offering storage to someone else (a storage node)

You can turn any machine into an encrypted off-site store for **other people's** vaults, without ever being able to read them. In **Settings → Storage node**, choose whether it should be reachable on your local network or through a relay hub, and start it. Then, for each person you want to host, name them and get a one-paste invite code. Hand that code to them once; on their machine they add it as a peer and mirror a vault to it, and from then on their encrypted backup lives on yours.

It is zero-knowledge by construction, in both directions. Only encrypted data ever reaches the storage machine — the keys never leave the owner's computer — so the person offering the space can never read what they are holding, and each guest gets their own private, isolated area addressed by an unguessable code, so no guest can ever see another's data or anything else on the host. The host never *opens* any of it either: the stored vaults are held only as opaque files and are never added to the host's own vault list. You can see how much space each person is using and remove anyone's copy at any time, and you can set a size limit that warns you as the store fills. It runs the same on macOS, Windows, and Linux, over the same pinned-certificate encrypted connection as a served vault.

## Emergency and inheritance access

A threshold key also gives you a safe way to make sure trusted people can reach a vault if something happens to you. Run `vdisk emergency-access <vault> --shares <n> --threshold <k>` (or tick **Read-only** on the **Threshold key** button in the web interface), then give one share to each trusted contact. Any `k` of them, working together, can open the vault — but this access is read-only, so they can read what is there and can never change it, rotate your keys, or lock you out. No one contact can act alone, so keep the shares with different people. This is an honest, self-contained approach to emergency access: there is no company server holding a key, and no single person you have to trust — the vault opens only when enough of your chosen people agree to open it.
...
Two gates protect the sealed access, and both must pass to open it. The first is a real time lock: the access cannot be opened before its moment, not by the beneficiary and not by changing a computer's clock. The unlock key is published by a public time-beacon network only when the moment arrives. The second is the beneficiary's own key. So a copy that leaks, or that a beneficiary keeps, is useless until its time, and useless to anyone but that beneficiary even then.

For the dead-man's switch, the moment is your inactivity window plus a grace period. Checking in resets the inactivity timer (**Check in**, `vdisk emergency check-in`, or just using the app). You can also turn on a notification that reminds you as a release approaches, so an accidental release is much less likely (see below). Re-routing the vault pushes the time lock further out, so while you are active the access never becomes openable. Stop, and the access is released for the beneficiary to open with their private key (`vdisk emergency-open <file> --key <their private key>`). For a scheduled unlock, the moment is the date you picked. If a beneficiary has no internet when it is time to open, they can fetch the unlock key on another device and pass it in with `--signature`. Removing a beneficiary later drops the grants routed to them, since those could never be opened by anyone else anyway.

**Notifications for the events that matter.** A few events are worth hearing about even when the app is closed: the check-in reminder above, an emergency release actually firing, and a scheduled backup failing. In **Settings → Notifications** you can turn these on and point them at a webhook you control (your own push service, a chat channel's incoming webhook, or a small endpoint on your own network). They are off by default. Each alert carries only the event type and a short vault label, never file names or contents, and a generic mode drops even those and sends only "an event needs your attention." Notifications are set up and stored on this computer only, never over a network connection, and a failed send is retried a few times and then dropped rather than allowed to pile up.
...
**Panic key.** In the web interface, press **Ctrl/Cmd+Shift+L** to lock everything at once, with no confirmation. It closes anything open on screen, clears the clipboard and re-hides any revealed names so a just-copied password or an on-screen detail is wiped, then does the same gentle flush-then-unmount as Lock all — so it is instant and discreet, yet still never forces a vault that has a file open and never risks unsaved work.

**Disguise key.** Press **Ctrl/Cmd+Shift+H** to instantly drop a plain, innocuous full-screen cover over the whole interface — a dim idle clock with no name or hint of what the app is — for when someone walks up or you step away. It is not just a screen cover: it runs the same panic lock first, so every vault is already locked and the clipboard and on-screen details wiped before the cover even appears. Press the same keys again to bring the interface back, which reveals only the locked app.
...
Every detection — and every seal, acceptance, and unseal — is written to a local, persistent log, newest first. This trail is complete and reviewable, and it survives even if the vault, or its in-vault baseline, is later deleted or rolled back. Each entry carries a severity, so the serious findings stand out at a glance. Losing or altering a file, or a forged or removed baseline, ranks highest. A merely added file ranks below that, and the routine record of a seal you performed yourself ranks lowest. `vdisk tamper-log` (no password needed; the log holds no secrets) shows what happened and when, and the same history is available from **Tamper check** in the web interface.
...
- **macOS: prefer FUSE-T over an older kernel-extension FUSE driver.** FUSE-T needs no kernel extension and no reboot. It is also what makes Finder copies work: Finder writes macOS extended attributes that only FUSE-T handles, so with a kernel-extension driver alone a Finder copy fails with "error code -8062". You can still mount, read, and add files that other way (the **Add files** button, or `cp -X`), so it is not a hard block, just a limit. If you already have a kernel-extension FUSE driver for other apps, leave it installed: Vaultonaut points only its own engine at FUSE-T and never disturbs it, so both coexist. The doctor tells you what is installed and warns if FUSE-T is missing.
...
- **I want to see what happened.** By default Vaultonaut keeps no log on disk, for privacy. Turn on **Keep diagnostic logs** in Settings (or start the service with `--diagnostic-log`) and it writes a daily log in the `logs` folder inside your data directory (see "Where your data lives"). It records what the background service did: mounts, unmounts, self-heal updates, backups, and warnings. It never records a file name or contents, and it scrubs any password, key, or recovery phrase. So a log is always safe to read or share when you need help. Turn it back off when you are done to stop keeping a record.
