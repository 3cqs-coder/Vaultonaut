...
async function cmdEmergency(pos, flags) {
	...
	if (sub === 'arm') {
		...
		if (r.mode === 'date') console.log('Armed "' + vdisk.displayName(target) + '" for "' + r.contactLabel + '". It is delivered if you go inactive, and then cannot be opened before ' + new Date(r.opensAt).toLocaleString() + ' (' + r.opensAt + ').');
		else console.log('Armed "' + vdisk.displayName(target) + '" for "' + r.contactLabel + '". Re-arm periodically to stay protected:  ' + Brand.cli + ' emergency arm ' + target);
		return;
	}
