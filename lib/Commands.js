...
async function confirmVaultName(target, buildPrompt, cancelMsg) {
	// Prompt with the friendly title an opaque vault shows (folder name otherwise), and accept either that title or the
	// opaque folder name — so the confirmation matches what the user sees, however this machine labels the vault.
	const name = await vdisk.friendlyName(target);
	const folder = vdisk.displayName(target);
	const ans = (await prompt.line(buildPrompt(name))).trim();
	if (ans === name || ans === folder) return true;
...
// Flags that take NO value — simple on/off switches. The parser must know these so a switch placed BEFORE a
// positional (for example `unmount --force /path`) does not swallow the path as its value and leave the command with
// no target. Value-taking flags are deliberately NOT listed and keep the `--flag value` and `--flag=value` forms;
// listing a value flag here by mistake would break its space-separated form, so a flag is added only once it is
// confirmed value-less. Kept in sync with the handlers by lib/test/cliparse.js.
const BOOLEAN_FLAGS = new Set([
	'force', 'yes', 'read-only', 'panic', 'thorough', 'soft', 'latest', 'supervised', 'child', 'quick', 'heal',
	'recover', 'streaming', 'working-disk', 'allow-other', 'no-keep', 'no-key', 'no-password', 'no-shortcut',
	'keep-cache', 'off', 'list', 'content', 'in', 'no-refresh', 'worm', 'write', 'desktop', 'diagnostic-log',
	'dry-run', 'skip-duplicates', 'json', 'lan', 'wan', 'onion', 'single-hop', 'stable-address', 'new-address', 'opaque', 'anonymize',
]);
// Value-taking flags (the `--flag value` / `--flag=value` forms). Kept complete so the unknown-flag warning below
// never fires on a real flag; lib/test/cliparse.js pins that every flag a handler reads appears here or in
// BOOLEAN_FLAGS. `data-dir` and `dns-order` are consumed upstream (vaultonaut.js / DnsOrder) before dispatch, so they
// are listed here too — otherwise the warning would flag them as unknown.
const VALUE_FLAGS = new Set([
	'allow-ip', 'bind', 'cache-dir', 'cache-size', 'client-id', 'client-secret', 'cloud', 'contact', 'contact-key',
	'daily', 'data-dir', 'date', 'deny-ip', 'dest', 'dns-order', 'every', 'expect', 'expires', 'folders', 'from',
	'fuse-backend', 'grace-days', 'inactive-days', 'k', 'kdf', 'keep', 'key', 'key-shares', 'keyfile', 'keys',
	'label', 'member-key', 'mountpoint', 'name', 'need', 'out', 'port', 'ports', 'read-cap', 'ready-token', 'reason',
	'redundancy', 'relay', 'remote-path', 'retain-days', 'secrets-file', 'shards', 'shares', 'signature', 'threshold',
	'description', 'title',
	'to', 'token', 'token-file', 'trustees', 'trustees-file', 'tsa', 'type', 'url', 'user', 'vfs-cache-mode', 'volname',
	'worm-mode', 'format', 'host', 'relay-code',
]);
// Every flag this tool understands. A `--flag` NOT in here is a typo (or from a newer version) — warned about, not
// silently ignored. Silent-ignore was a real footgun: a misspelled security switch (e.g. "--reed-only") dropped the
// user's intent and mounted read-WRITE. The warning is non-fatal so a forward-compatible flag from a newer build
// still runs; it just tells the user their option was not applied.
const KNOWN_FLAGS = new Set([...BOOLEAN_FLAGS, ...VALUE_FLAGS]);

// ---- --json output mode ----
// A global --json flag makes the CLI print exactly ONE JSON object on stdout instead of human text, so scripts and
// other tools can consume it. The envelope is universal: { ok: true, ... } on success (with a command's structured
// data when it provides some via jsonData), or { ok: false, error: { code, message } } on failure, with the exit code
// preserved. Human console.log output is routed to stderr in this mode (see vaultonaut.js), keeping stdout clean.
// Additive and future-proof: a command that has not opted into structured data still returns a valid { ok: true }, and
// any command can attach data later by calling jsonData(). All of this is a tiny synchronous string op — non-blocking.
let _jsonData;
function jsonData(obj) { _jsonData = obj; } // a handler attaches its structured result; consumed only when --json is set
function takeJsonData() { const d = _jsonData; _jsonData = undefined; return d; }
// A STABLE error code so a script can branch without parsing prose. Prefer an explicit e.code (many Vault errors set
// one, e.g. MIRROR_IN_USE / VAULT_LOCKED); otherwise map a few common message shapes; else a generic ERROR.
function jsonErrorCode(e) {
	const code = e && e.code;
	if (typeof code === 'string' && /^[A-Z][A-Z0-9_]*$/.test(code)) return code;
	const m = String((e && e.message) || '');
	if (/password is incorrect|wrong password/i.test(m)) return 'WRONG_PASSWORD';
	if (/read-only/i.test(m)) return 'READ_ONLY';
	if (/not a vault|no such vault|unknown vault|no vault found|could not find the vault/i.test(m)) return 'NOT_A_VAULT';
	if (/in use|another machine|is busy|is locked/i.test(m)) return 'BUSY';
	if (/^usage:/i.test(m)) return 'USAGE';
	return 'ERROR';
}
function warnUnknownFlags(flags) {
	for (const k of Object.keys(flags)) {
		if (!KNOWN_FLAGS.has(k)) Common.warn('Ignoring unknown option "--' + k + '" — check the spelling. Run "' + Brand.cli + ' help" to see the options.');
	}
}
...
function parse(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (a.startsWith('--')) {
			...
			const key = body;
			if (BOOLEAN_FLAGS.has(key)) { flags[key] = true; continue; } // a switch never consumes the next token, so it can precede a positional
			const next = argv[i + 1];
...
function parsePort(flag, dflt) {
	...
	// Require ALL digits: parseInt is lenient (it would turn "7420x" into 7420 or "74.9" into 74), which would
	// silently bind a surprising port on a typo. Reject anything that is not a plain whole number.
...
async function resolveToken(flags, { required = false, what = 'the relay token' } = {}) {
	// A bare --token (no value) parses to boolean true. Reject it outright rather than silently treating it as absent:
	// on the hub that would quietly MINT A NEW token (invalidating every node already configured with the real one), and
	// on a node it would fall to the generic "required" error. Either way the user meant to pass a value and mistyped.
	if (flags.token === true || flags.token === '') throw new Error('--token needs a value (' + what + '). To keep it off the command line, pass --token-file <path> instead.'); // a bare --token (true) OR an empty --token= must be rejected, not silently treated as absent — on the hub that would mint a NEW token and invalidate every configured node
...
function notice(key) {
	if (NOTICES_CACHE === null) {
		...
		try {
			...
			let cur = null, buf = [];
			for (const line of raw.split(/\r?\n/)) { // tolerate CRLF (a Windows checkout with autocrlf) so no line keeps a trailing carriage return
				const m = /^\[(.+)\]\s*$/.exec(line);
...
async function cmdCreate(pos, flags) {
	...
	if (flags.worm || flags['retain-days']) {
		...
		if (!(Number.isFinite(retainDays) && retainDays > 0)) throw new Error('--worm needs --retain-days <N> (how many days each version stays locked, at least 1).'); // Number.isFinite rejects a non-finite value like "1e999" -> Infinity up front
	...
	// --opaque stores the vault under a folder named by its own id (revealing nothing on disk); the name you typed is
	// kept encrypted and shown once you unlock it. Without the flag the folder keeps the name you gave (the default).
	const res = await vdisk.create(target, { password, level, cloud, worm, opaque: !!flags.opaque });
	console.log('Created ' + (worm ? 'tamper-proof ' : '') + (cloud ? 'cloud ' : '') + 'vault at ' + res.vault + (level && level !== 'standard' ? ' (security level: ' + level + ')' : ''));
	if (flags.opaque) console.log('The folder name is an opaque id; the name you typed is stored encrypted and appears once the vault is unlocked. View or change it with:  ' + Brand.cli + ' name "' + res.vault + '"');
...
async function cmdCloud(pos, flags) {
	...
	if (sub === 'connect') {
		// A bare `--type` (no value) parses to boolean true; guard so it yields the clear usage line below, not a raw
		// "true.toLowerCase is not a function" crash. pos[1] is a positional, always a string or undefined.
		const type = (pos[1] || (typeof flags.type === 'string' ? flags.type : '') || '').toLowerCase();
	...
	if (sub === 'add') {
		const type = (typeof flags.type === 'string') ? flags.type : ''; // a bare `--type` parses to boolean true; treat it as missing so the usage line fires
...
async function cmdImport(pos, flags) {
	...
	const password = await prompt.newPassword('Password');
	const r = await vdisk.importFolder(dest, { password, sourceDir: source, level, opaque: !!flags.opaque });
...
async function cmdNotesImport(pos, flags) {
	const [file, target] = pos;
	if (!file || !target) throw new Error('Usage: ' + Brand.cli + ' notes-import <file> <vault>   (import logins and notes from a .csv, a Bitwarden .json, or a 1Password .1pux export into an OPEN vault; add --format csv|bitwarden|1pux|auto, --dry-run, or --skip-duplicates)');
	let text; // read as a Buffer: a .1pux is a binary ZIP, and NotesImport handles a Buffer or a text file the same way
	try { text = fs.readFileSync(file); } catch (e) { throw new Error('Could not read the import file "' + file + '": ' + (e && e.message || e)); }
	const r = await vdisk.notesImport(target, { text, format: flags.format, filename: file, skipDuplicates: !!flags['skip-duplicates'], dryRun: !!flags['dry-run'] });
	const types = Object.keys(r.byType || {}).sort().map((t) => r.byType[t] + ' ' + t).join(', ');
	if (r.dryRun) {
		console.log('Dry run — nothing was written. Detected ' + r.total + ' item' + (r.total === 1 ? '' : 's') + ' (' + r.format + ' export)' + (types ? ': ' + types : '') + '.');
		console.log('Run the same command without --dry-run to import them into the open vault.');
		return;
	}
	console.log('Imported ' + r.imported + ' item' + (r.imported === 1 ? '' : 's') + ' from a ' + r.format + ' export' + (types ? ' (' + types + ')' : '') + '.');
	if (r.skipped) console.log('Skipped ' + r.skipped + ' item' + (r.skipped === 1 ? '' : 's') + ' that already existed (matched by title and type).');
	console.log('Your export file at "' + file + '" still holds those secrets in the clear — delete it securely now that the import is done.');
}
...
// Show or change a vault's encrypted name (title) and description. The metadata is sealed with the vault key, so it
// needs the password: viewing needs any password that opens the vault; changing needs the read-write password. With
// no new value it prints the current metadata; a positional value or --title sets the title, --description the note.
async function cmdName(pos, flags) {
	const target = pos[0];
	if (!target) throw new Error('Usage: ' + Brand.cli + ' name <path> ["New title"] [--description "text"]');
	const password = await prompt.hidden('Password: ');
	const newTitle = (typeof flags.title === 'string') ? flags.title : (pos[1] != null ? String(pos[1]) : undefined);
	const newDesc = (typeof flags.description === 'string') ? flags.description : undefined;
	if (flags.anonymize) {
		// Save any name/description given first (so it is kept once the folder loses its name), then rename the folder
		// to an opaque id. The vault must be unmounted.
		if (newTitle !== undefined || newDesc !== undefined) {
			const f = {}; if (newTitle !== undefined) f.title = newTitle; if (newDesc !== undefined) f.description = newDesc;
			await vdisk.setVaultMeta(target, { password, fields: f });
		}
		const a = await vdisk.anonymizeVault(target, { password });
		console.log(a.alreadyOpaque ? 'This vault\'s folder name is already hidden.' : ('The folder name is now hidden. The vault is at ' + a.vault + ' and shows as "' + (a.title || '(no name)') + '" in the app.'));
		return;
	}
	if (newTitle === undefined && newDesc === undefined) {
		const r = await vdisk.getVaultMeta(target, password);
		console.log('Title:       ' + (r.meta.title || '(none — the list shows the folder name)'));
		console.log('Description: ' + (r.meta.description || '(none)'));
		return;
	}
	const fields = {};
	if (newTitle !== undefined) fields.title = newTitle;
	if (newDesc !== undefined) fields.description = newDesc;
	const r = await vdisk.setVaultMeta(target, { password, fields });
	console.log('Updated. Title: ' + (r.meta.title || '(none)') + (r.meta.description ? ' — Description: ' + r.meta.description : ''));
}
...
async function cmdShareSeal(pos, flags) {
	requireStringFlag(flags, 'out', '--out share.json'); // a bare --out would otherwise write to a file literally named "true"
	const target = pos[0];
...
async function cmdDecoy(pos) {
	...
	if (sub === 'list') {
		...
		for (const m of mappings) {
			// Show the friendly titles an opaque vault carries, not the hex folder ids; resolve both together (I/O-bound, bounded pair).
			const [realName, decoyName] = await Promise.all([vdisk.friendlyName(m.realVault), vdisk.friendlyName(m.decoyVault)]);
			console.log('  ' + realName + '  →  decoy: ' + decoyName);
		}
		return;
	}
...
async function cmdTravel(pos) {
	...
	if (sub === 'on' || sub === 'enable') {
		...
		// Route through the running service if there is one, so it ALSO stops any live serve, mobile, and lease
		// sessions it is holding for these vaults before hiding them. Those sessions live only in the service's
		// memory, so a separate CLI process cannot stop them — doing the hide directly would leave a "hidden" vault
		// still reachable over the network, breaking travel mode's guarantee. Fall back to a direct hide only when no
		// service is running, where there are no such live sessions to stop.
		let r = null;
		const ownerOn = await OwnerClient.find();
		if (ownerOn) {
			const resp = await OwnerClient.post(ownerOn.url, '/api/travel-start', { password }).catch(() => null);
			if (resp && OwnerClient.isUsable(resp)) { if (!resp.ok) throw new Error((resp.body && resp.body.error) || 'Travel mode could not be turned on.'); r = resp.body; }
		}
		if (!r) r = await vdisk.travelEnable({ travelPassword: password });
	...
	if (sub === 'off' || sub === 'restore') {
		...
		// Route restore through the running service too, so it re-applies the vaults' settings and resumes its
		// scheduled work with the correct in-memory state; fall back to a direct restore when no service is running.
		let r = null;
		const ownerOff = await OwnerClient.find();
		if (ownerOff) {
			const resp = await OwnerClient.post(ownerOff.url, '/api/travel-restore', { password }).catch(() => null);
			if (resp && OwnerClient.isUsable(resp)) { if (!resp.ok) throw new Error((resp.body && resp.body.error) || 'Travel mode could not be turned off.'); r = resp.body; }
		}
		if (!r) r = await vdisk.travelRestore({ travelPassword: password });
...
function readKeyArg(v) { // a base64 key, or a path to a file containing it
	// A value-flag passed with NO value parses to boolean `true`; treat that as absent so the caller's `if (!key)`
	// guard prints its "provide the key" usage line, rather than falling through to the literal string 'true' (which
	// would slip past that guard, waste a password prompt, and fail later with a cryptic key-format error).
	if (!v || v === true) return '';
...
async function cmdEmergency(pos, flags) {
	...
	if (sub === 'enroll') {
		...
		console.log('Enrolled "' + (flags.label || 'Trusted contact') + '" (id ' + r.contactId + '). Add more beneficiaries with:  ' + Brand.cli + ' emergency add-contact --contact-key <key> --label <name>');
		console.log('Then route a vault to one with:  ' + Brand.cli + ' emergency arm <vault> --contact <id-or-label>');
		return;
	}
	if (sub === 'add-contact') {
		const pubKey = readKeyArg(flags['contact-key']);
		if (!pubKey) throw new Error('Usage: ' + Brand.cli + ' emergency add-contact --contact-key <public-key|file> [--label <name>]');
		const r = await vdisk.emergencyAddContact({ label: flags.label, pubKey });
		console.log('Added beneficiary "' + (flags.label || 'Trusted contact') + '" (id ' + r.contactId + '). Route a vault to them:  ' + Brand.cli + ' emergency arm <vault> --contact ' + r.contactId);
		return;
	}
	if (sub === 'remove-contact') {
		const who = pos[1]; if (!who) throw new Error('Usage: ' + Brand.cli + ' emergency remove-contact <id-or-label>');
		const id = await resolveContactId(who);
		const r = await vdisk.emergencyRemoveContact(id);
		console.log(r.removed ? 'Removed. ' + r.droppedGrants + ' sealed grant' + (r.droppedGrants === 1 ? '' : 's') + ' for that beneficiary ' + (r.droppedGrants === 1 ? 'was' : 'were') + ' dropped.' : 'No such beneficiary.');
		return;
	}
	if (sub === 'contacts') {
		const s = await vdisk.emergencyStatus();
		if (!s.enrolled || !s.contacts.length) { console.log('No beneficiaries are enrolled.'); return; }
		console.log('Beneficiaries:'); for (const c of s.contacts) { const vaults = s.armed.filter(a => a.contactId === c.id).map(a => a.name); console.log('  ' + c.id + '  "' + c.label + '"' + (vaults.length ? '  → ' + vaults.join(', ') : '  (no vaults routed yet)')); }
		return;
	}
	if (sub === 'arm') {
		const target = pos[1]; if (!target) throw new Error('Usage: ' + Brand.cli + ' emergency arm <vault> [--contact <id-or-label>] [--date <when>]');
		const contactId = flags.contact ? await resolveContactId(flags.contact) : undefined;
		// Optional scheduled-unlock date: a grant that opens on an absolute moment rather than on inactivity. Parsed
		// strictly and refused if it is not a real, future time, so an invalid value can never create a past-dated
		// (already-open) or malformed grant.
		let mode, dateMs;
		if (flags.date != null) { dateMs = parseUnlockDate(flags.date); mode = 'date'; }
		const password = await prompt.hidden('Vault password: ');
		const r = await vdisk.emergencyArm(target, { password, contactId, mode, dateMs });
			const armedName = await vdisk.friendlyName(target); // match the title the arm record and status listing show, not the opaque folder
		if (r.mode === 'date') console.log('Armed "' + armedName + '" for "' + r.contactLabel + '". It is delivered if you go inactive, and then cannot be opened before ' + new Date(r.opensAt).toLocaleString() + ' (' + r.opensAt + ').');
		else console.log('Armed "' + armedName + '" for "' + r.contactLabel + '". Re-arm periodically to stay protected:  ' + Brand.cli + ' emergency arm ' + target);
		return;
	}
	...
	if (sub === 'status') {
		...
		console.log('Emergency access: ' + s.phase.toUpperCase() + ' — ' + s.contacts.length + ' beneficiar' + (s.contacts.length === 1 ? 'y' : 'ies') + ', ' + s.inactivityDays + '-day inactivity + ' + s.graceDays + '-day grace.');
		console.log('  Last check-in: ' + s.lastCheckIn + (s.phase === 'released' ? '' : '  (about ' + s.daysUntilRelease + ' day(s) until release without a check-in)'));
		for (const c of s.contacts) { const vaults = s.armed.filter(a => a.contactId === c.id).map(a => a.name); console.log('  ' + c.label + ' (' + c.id + '): ' + (vaults.length ? vaults.join(', ') : 'no vaults routed')); }
	...
	// The OPTIONAL trustee-quorum backstop: a beacon-free recovery path (a quorum of trustees can reconstruct a
	// released read cap if the time-lock beacon is ever unreachable). Off unless set up.
	if (sub === 'backstop') {
		const action = pos[1] || 'status';
		if (action === 'status') {
			const s = await vdisk.emergencyStatus();
			if (!s.enrolled) { console.log('Emergency access is not set up.'); return; }
			if (!s.backstop) { console.log('No recovery backstop is set. It is optional: a quorum of trustees who can recover access if the time-lock beacon is ever unreachable.'); return; }
			console.log('Recovery backstop: any ' + s.backstop.k + ' of ' + s.backstop.n + ' trustees can recover access. Trustees:');
			for (const t of s.backstop.trustees) console.log('  ' + t.label);
			return;
		}
		if (action === 'off' || action === 'clear') { await vdisk.emergencyClearBackstop(); console.log('Recovery backstop turned off. Already-armed vaults keep the backstop they were sealed with — re-arm them to drop it.'); return; }
		if (action === 'set') {
			// Reject a bare "--trustees-file" (parses to boolean true) with a usage line, instead of letting
			// fs.readFileSync(true) throw a cryptic Node type error; and validate --need at the edge like the other
			// numeric flags, so a bare/missing/invalid value gets a clear example rather than a downstream message.
			if (typeof flags['trustees-file'] !== 'string') throw new Error('Usage: ' + Brand.cli + ' emergency backstop set --trustees-file <file.json> --need <k>\nThe file is a JSON array of { "label": "...", "pub": "<their public key>" }.');
			const need = Number(flags.need);
			if (!Number.isInteger(need) || need < 2) throw new Error('--need must be a whole number of at least 2 (how many trustees must cooperate to recover), for example --need 3.');
			let list; try { list = JSON.parse(await Common.readFileCapped(flags['trustees-file'], 1024 * 1024, 'utf8')); } catch (e) { throw new Error('Could not read the trustees file: ' + e.message); } // capped + async: never block the loop or spike memory on a huge/wrong file
			const r = await vdisk.emergencySetBackstop({ trustees: list, k: need });
			console.log('Recovery backstop set: any ' + r.k + ' of ' + r.n + ' trustees can recover access if the beacon is unreachable. Re-arm a vault to apply it.');
			return;
		}
		throw new Error('Usage: ' + Brand.cli + ' emergency backstop <status|set|off>');
	}
	// A trustee opens their sealed backstop share; whoever coordinates recovery then combines a quorum of them.
	if (sub === 'trustee-open') {
		const file = pos[1]; const priv = readKeyArg(flags.key);
		if (!file || !priv) throw new Error('Usage: ' + Brand.cli + ' emergency trustee-open <backstop-file> --key <your-private-key|file>');
		let doc; try { doc = fs.readFileSync(file, 'utf8').trim(); } catch (_) { throw new Error('Could not read the backstop file "' + file + '".'); }
		let sealedShares; try { const j = JSON.parse(doc); sealedShares = Array.isArray(j.shares) ? j.shares.map(s => s.sealed) : [doc]; } catch (_) { sealedShares = [doc]; }
		let share = null; for (const s of sealedShares) { share = vdisk.emergencyTrusteeShare(s, priv); if (share) break; }
		if (!share) throw new Error('Your key did not open any share in this file. Check that you are using the right private key for this backstop.');
		console.log('Your recovery share (give this to whoever is reconstructing access):\n\n' + share + '\n');
		return;
	}
	if (sub === 'recover') {
		const shares = pos.slice(1);
		if (shares.length < 2) throw new Error('Usage: ' + Brand.cli + ' emergency recover <share1> <share2> [...]\nCollect each trustee\'s share (each runs "' + Brand.cli + ' emergency trustee-open").');
		const link = vdisk.emergencyRecoverToken({ shares });
		console.log('Read link recovered from the trustee shares. Open a copy of the vault with it:\n');
		console.log('  ' + Brand.cli + ' mount <vault-folder> --read-cap ' + link);
		return;
	}
	throw new Error('Usage: ' + Brand.cli + ' emergency <keypair|enroll|add-contact|remove-contact|contacts|arm|check-in|status|disarm|backstop|trustee-open|recover>');
}
// Resolve a beneficiary reference (an id, or a case-insensitive label) to a contact id, erroring helpfully if it is
// ambiguous or unknown — so a CLI user never seals a vault to the wrong person.
async function resolveContactId(ref) {
	const s = await vdisk.emergencyStatus();
	const contacts = (s && s.contacts) || [];
	const r = String(ref || '').trim();
	if (contacts.some(c => c.id === r)) return r;
	const byLabel = contacts.filter(c => c.label.toLowerCase() === r.toLowerCase());
	if (byLabel.length === 1) return byLabel[0].id;
	if (byLabel.length > 1) throw new Error('More than one beneficiary is labeled "' + r + '". Use the id instead (see "' + Brand.cli + ' emergency contacts").');
	throw new Error('No beneficiary "' + r + '". See "' + Brand.cli + ' emergency contacts" for the enrolled ones.');
}
// Parse a scheduled-unlock time strictly, returning an absolute epoch (ms). Accepts a relative form (+30d, +12h,
// +45m) or any date/date-time Date can parse (an ISO string with a Z or offset is unambiguous; a bare date-time is
// read in this machine's local time). Refuses anything unparseable or not strictly in the future, so a scheduled
// grant can never be created for an invalid or past moment.
function parseUnlockDate(s) {
	const v = String(s == null ? '' : s).trim();
	if (!v) throw new Error('An unlock date is required (for example --date 2035-01-01T09:00, or --date +30d).');
	const rel = /^\+(\d+)\s*([dhm])$/i.exec(v);
	let ms;
	if (rel) { const per = rel[2].toLowerCase() === 'd' ? 86400000 : rel[2].toLowerCase() === 'h' ? 3600000 : 60000; ms = Date.now() + Number(rel[1]) * per; }
	else { ms = Date.parse(v); if (!Number.isFinite(ms)) throw new Error('That unlock date is not a valid date or time. Use for example 2035-01-01T09:00, an ISO time with a zone, or a relative +30d / +12h.'); }
	if (ms <= Date.now()) throw new Error('The unlock date must be in the future.');
	return ms;
}
...
async function cmdEmergencyOpen(pos, flags) {
	...
	// A grant is time-locked: opening it fetches the beacon signature for its round. On a machine with no network,
	// pass --signature <hex> to open offline (the value is verified against the pinned key before use).
	const signature = typeof flags.signature === 'string' ? flags.signature.trim() : undefined;
	const link = await vdisk.emergencyOpen(priv, sealed, { signature });
...
// Turn "approval required on unlock" on or off for a vault (or show its state). When on, each unlock asks for
// approval and is recorded in the vault's tamper-evident ledger, so the ledger becomes a signed history of every use.
async function cmdConsentOnUse(pos) {
	const target = pos[0];
	if (!target) throw new Error('Usage: ' + Brand.cli + ' consent-on-use <vault> [on|off]');
	const arg = String(pos[1] || '').toLowerCase();
	if (arg === 'on' || arg === 'off') {
		const r = await vdisk.setConsentOnUse(target, arg === 'on');
		console.log('Approval on unlock is now ' + (r.consentOnUse ? 'ON — each unlock asks for approval and is recorded in the ledger.' : 'OFF.'));
	} else if (!arg) {
		console.log('Approval on unlock is ' + ((await vdisk.isConsentOnUse(target)) ? 'ON.' : 'OFF.') + ' Change it with "' + Brand.cli + ' consent-on-use ' + target + ' on|off".');
	} else throw new Error('Usage: ' + Brand.cli + ' consent-on-use <vault> [on|off]');
}
...
async function cmdMount(pos, flags) {
	...
	// Consent-on-use: if this vault requires explicit approval on each unlock, confirm before proceeding. The unlock is
	// recorded in the vault's tamper-evident ledger by the mount itself. --yes approves non-interactively (automation).
	if (!flags.yes && await vdisk.isConsentOnUse(target).catch(() => false)) {
		const ans = (await prompt.line('This vault requires approval to unlock, and each unlock is recorded in its ledger. Continue? [y/N] ')).trim();
	...
	if (!res.viaOwner && opts.rememberPrefs) {
		try { await vdisk.recordMountPrefs(res.vault || target, { readOnly: !!flags['read-only'], workingDisk: !!flags['working-disk'], streaming: !!flags.streaming, fuseBackend: flags['fuse-backend'] }); } catch (_) {}
	}
...
// Show or set how a peer node is reached: auto (direct when possible, relay fallback — the default), relay (relay
// only), or direct (require a direct hop, no relay fallback). The same preference the web interface exposes.
async function cmdPeerMode(pos) {
	const arg = String(pos[0] || '').trim().toLowerCase();
	if (arg === '') { const s = await vdisk.getSettings(); const m = (s.p2pMode === 'relay' || s.p2pMode === 'direct') ? s.p2pMode : 'auto'; console.log('Peer connections: ' + (m === 'relay' ? 'relay only' : m === 'direct' ? 'direct only (no relay fallback)' : 'automatic (direct when possible, relay otherwise)')); return; }
	if (!/^(auto|relay|direct)$/.test(arg)) throw new Error('Usage: ' + Brand.cli + ' peer-mode [auto|relay|direct]');
	await vdisk.setSettings({ p2pMode: arg });
	console.log('Peer connections: ' + (arg === 'relay' ? 'relay only.' : arg === 'direct' ? 'direct only — a sync fails if no direct connection can be made.' : 'automatic — connects directly when possible and falls back to a relay otherwise.'));
}
...
async function cmdStatus() {
	...
	if (!rows.length) { jsonData({ mounts: [] }); console.log('No vaults are mounted.'); return; }
	const health = await vdisk.mountHealth(rows); // { <mountpoint>: 'healthy'|'unresponsive'|'dead' }
	jsonData({ mounts: rows, health });
...
async function cmdSearch(pos, flags = {}) {
	...
	if (flags.in || flags.content) {
		// Refresh the index first (incremental — only files added or changed since the last index are read, with OCR when
		// it is on) so a file added since the last index is found without a separate "reindex". --no-refresh skips it to
		// search the existing index as-is. Best-effort: an unmounted/read-only/wedged vault falls through to the search,
		// which then reports the honest noIndex/stale state below.
		if (!flags['no-refresh']) {
			const prog = cliProgress();
			try { await vdisk.contentReindex(target, { onProgress: prog, allowOcr: true }); } catch (_) {} finally { prog.done(); }
		}
...
async function cmdReindex(pos) {
	...
	const prog = cliProgress();
	const r = await vdisk.contentReindex(target, { onProgress: prog, allowOcr: true });
	prog.done();
...
async function cmdProtect(pos, flags) {
	...
	if (r.deferred) { console.log(r.message); return; } // signed vault, no unlocked session to re-sign — deferred rather than silently stripping the signature
...
async function cmdHeal(pos, flags = {}) {
	...
	// preserveInPlaceEdits: a SAME-SIZE mismatch cannot be told apart from a same-length edit without the key, so
	// never silently revert it — report it and leave the file as-is unless the user explicitly forces the repair.
	// --force (already passed below) overrides this and rebuilds the block from parity for genuine same-size bit-rot.
	const h = await vdisk.heal(target, { allowUnverified: !!flags.force, force: !!flags.force, preserveInPlaceEdits: true, onProgress: prog2 }); prog2.done();
	...
	if (h.inPlaceDeferred) console.log(h.inPlaceDeferred + ' file(s) changed at the SAME size since protection and were left exactly as they are — a same-size change cannot be told apart from bit-rot without opening the vault, so it is preserved by default rather than risk reverting an edit. Run "protect" again to record them if they are your edits, or re-run with --force to repair them from the recovery data if you are certain they are corruption.');
	...
	if (h.unrecoverable) {
		// A genuine DATA loss larger than the recovery data can rebuild (e.g. a whole large file was deleted). Recovery
		// parity only ever recovers up to its redundancy budget, so this cannot be healed — say so plainly and point at
		// the real fix. A tamper check (which can decrypt names) will name the files; heal only sees encrypted names.
		console.log(h.unrecoverable + ' file(s) could not be fully recovered — the loss is larger than the recovery data can rebuild, so self-healing cannot restore them. Restore them from a backup or a saved version. Run "' + Brand.cli + ' audit ' + target + '" to see which files are affected.');
		process.exitCode = 1;
	} else if (h.unrecoverableStripes) {
		// No data file was lost, only some recovery (parity) blocks beyond the budget — protection is reduced but the
		// files themselves are intact. Refresh the recovery data rather than alarming about a restore.
		console.log('Your files are intact, but some recovery data could not be rebuilt, so protection is reduced — run "' + Brand.cli + ' protect ' + target + '" again to refresh it.');
...
async function cmdScrub(pos, flags) {
	...
	if (r.clean) console.log('Integrity scrub: clean — the recovery data verifies and no damage was found.');
		...
		// Report every outcome honestly, not just the repaired count: a deferred possible-edit, a loss beyond the recovery
		// budget, and a block that rebuilt but could not be written back all mean the vault is NOT fully clean. The last two
		// are real unrepaired damage, so set a non-zero exit code (matching the "damage found" branch below) so a scripted
		// or scheduled scrub never reads remaining corruption as success.
		const notes = ['repaired ' + r.repaired + ' damaged area(s) from the recovery data'];
		if (r.deferredEdits) notes.push('left ' + r.deferredEdits + ' possibly-edited file(s) unchanged for review — run "' + Brand.cli + ' protect ' + vault + '" if the changes are yours, or scrub --heal again if not');
		if (r.unrecoverable) notes.push(r.unrecoverable + ' file(s) could NOT be fully recovered (the loss is larger than the recovery data can rebuild) — restore them from a backup or a saved version');
		if (r.writeErrors) notes.push(r.writeErrors + ' rebuilt block(s) could NOT be written back (a read-only file, a full disk, or a lock) — free space or clear the lock and run scrub --heal again');
		console.log('Integrity scrub: ' + notes.join('; ') + '.');
		if (r.unrecoverable || r.writeErrors) process.exitCode = 1; // real damage remains, so a scripted scrub can detect it
	}
...
// A daily schedule's hour/minute are stored in UTC on the backend, so a daily job fires at the same absolute moment
// on every machine (uniform for sync and mirror, stable across a timezone move). The CLI takes and shows this
// machine's LOCAL time and converts at these edges, matching the web interface. Uses today's offset (current DST).
...
function scheduleTimingFromFlags(flags) {
	if (flags.daily) {
		...
		const ut = localHmToUtc(hour, minute); // local input -> UTC storage (kept for older clients)
		// Also store this machine's zone and the intended local time, so the job fires at the same wall-clock time every
		// day and does not drift across a daylight-saving change; the backend prefers these when present.
		return { mode: 'daily', hour: ut.hour, minute: ut.minute, localHour: hour, localMinute: minute, tz: schedTz() };
	}
...
function describeScheduleWhen(s) {
	...
	if (s.mode === 'daily') { const lt = Number.isFinite(s.localHour) ? { hour: s.localHour, minute: s.localMinute } : utcHmToLocal(s.hour, s.minute); return 'daily at ' + String(lt.hour).padStart(2, '0') + ':' + String(lt.minute).padStart(2, '0') + (s.tz ? ' ' + s.tz : ' local'); }
	return 'off';
}
...
async function cmdUnprotect(pos, flags) {
	...
	if (!flags || !flags.yes) {
		console.log('This removes the recovery data from "' + (await vdisk.friendlyName(target)) + '". The vault will still open, but it can no longer self-heal from corruption or bit-rot until you run "' + Brand.cli + ' protect" again.');
...
async function cmdSecureRemove(pos, flags) {
	...
	const name = await vdisk.friendlyName(target); // the friendly title an opaque vault shows (folder name otherwise), so messages match the confirmation prompt
		const fileBase = String(name).replace(/[^\p{L}\p{N} ._-]/gu, '_').trim() || 'vault'; // filesystem-safe stem for the --keep example (same sanitize as the pack/recovery-kit filenames)
	...
	if (panic) {
		...
		if (flags.keep === true) throw new Error('--keep needs a filename, for example --keep "' + fileBase + Brand.packExt + '". Use --no-keep to erase without a safety copy. (Nothing was destroyed.)');
...
async function cmdMakeBundle(pos, flags) {
	requireStringFlag(flags, 'out', '--out ./bundle-dir'); // fail fast on a bare --out instead of passing `true` down as a path
	const target = pos[0];
...
async function cmdVerifyBundle(pos, flags) {
	...
	if (r.verdict === 'GENUINE' && !r.anchored) console.log('Note: no recorded origin identity was provided, so "GENUINE" means this bundle is internally self-consistent — it does not prove it is the same vault you recorded. Pass --expect <origin-identity> to anchor the check to a known genesis.');
	process.exitCode = r.verdict === 'GENUINE' ? 0 : 1;
}

// Prove ONE file is in the vault's signed state — a small, shareable proof anyone can verify offline.
async function cmdProveFile(pos, flags) {
	requireStringFlag(flags, 'out', '--out proof.json'); // fail fast on a bare --out, before the password prompt and the proof work, not with a path.resolve(true) crash after
	const target = pos[0], rel = pos[1];
	if (!target || !rel) throw new Error('Usage: ' + Brand.cli + ' prove-file <path> <file-in-vault> [--out <file>]');
	const password = await prompt.hidden('Password: ');
	const proof = await vdisk.fileProof(target, { password, path: rel });
	const out = flags.out ? path.resolve(flags.out) : path.resolve(path.basename(String(rel)).replace(/[^A-Za-z0-9._-]/g, '_') + '.proof.json');
	await require('fs').promises.writeFile(out, JSON.stringify(proof, null, 2));
	console.log('File proof written to ' + out);
	console.log('File ' + proof.file.path + (proof.hasContentHash ? '' : '  (size-only — take a deep snapshot to bind the content)') + ', origin ' + proof.originIdentity + '.');
	console.log('Anyone can verify it offline with:  ' + Brand.cli + ' verify-file "' + out + '"   (or: node verify.js "' + out + '")');
}
// Verify a single-file proof offline — no vault, no password.
async function cmdVerifyFile(pos, flags) {
	const target = pos[0];
	if (!target) throw new Error('Usage: ' + Brand.cli + ' verify-file <proof.json> [--expect <recorded-origin-identity>]');
	let proof; try { proof = JSON.parse(await require('fs').promises.readFile(path.resolve(target), 'utf8')); } catch (_) { throw new Error('That proof file could not be read.'); }
	const r = vdisk.verifyFileProof(proof, { expectIdentity: flags.expect });
	console.log('Verdict:   ' + r.verdict);
	if (r.identity) console.log('Identity:  ' + r.identity + (r.seq != null ? '   (version ' + r.seq + ')' : ''));
	if (r.originIdentity) console.log('Origin:    ' + r.originIdentity + '   (the vault\'s ORIGINAL identity)');
	...
	console.log('\nChecks:');
	for (const c of r.checks) console.log('  ' + (c.ok ? 'ok  ' : 'FAIL') + '  ' + c.name + (c.detail ? '  — ' + c.detail : ''));
	if (!flags.expect && r.originIdentity) console.log('\nConfirm the Origin identity matches a value the owner gave you separately, then pass it with --expect to have this check it.');
	if (r.verdict === 'GENUINE' && !r.hasContentHash) console.log('WARNING: this proof binds only the file NAME and SIZE, not its contents (it was made from a size-only snapshot). A different file of the same size would also verify. Ask for a proof made from a deep snapshot to bind the content.');
	if (r.verdict === 'GENUINE') console.log('Note: a file proof shows the file is in a signed set; with no timestamp it does not prove that set is the latest.');
	if (r.verdict === 'GENUINE' && !r.anchored) console.log('Note: no recorded origin identity was provided, so "GENUINE" means this proof is internally self-consistent — it does not prove it is from the vault you recorded. Pass --expect <origin-identity> to anchor it.');
	process.exitCode = r.verdict === 'GENUINE' ? 0 : 1;
}

// Verify this installation's own files against the maintainer-signed release manifest — the same check the app runs
// at boot, on demand. Needs no vault and no password. GENUINE / ALTERED / UNSIGNED / NEWER, mirroring the bundle verdicts.
async function cmdVerifySelf() {
	const r = await vdisk.verifySelf();
	if (!r.present) { console.log('UNSIGNED — this build has no signed release manifest (normally a from-source checkout).'); console.log('Verify your download against the maintainer\'s public key instead — see "Verifying your download" in the README.'); return; }
	if (r.pubkey === false) { console.log('UNSIGNED — no verification key is embedded in this build, so its files cannot be checked here.'); return; }
	if (r.signatureValid === false) { console.log('ALTERED — the signed manifest\'s signature does not verify: the manifest or the embedded key was changed. Re-download ' + Brand.name + ' from the official source.'); process.exitCode = 1; return; }
	if (r.unknownFormat) { console.log('NEWER — this build has a signed manifest in a newer format; update ' + Brand.name + ' to verify it.'); return; }
	const modified = [...(r.mismatches || []), ...(r.missing || [])];
	if (modified.length) {
		// On the maintainer's own checkout (the private signing key is present), a mismatch is just their own edits not
		// yet re-signed — never a tamper alarm. This mirrors the boot self-check, which skips the alarm on that machine.
		if (r.maintainer) { console.log('You have local, not-yet-signed changes to ' + modified.length + ' file(s). Run "npm run sign" to sign the current files.'); return; }
		console.log('ALTERED — ' + modified.length + ' application file(s) do not match the signed release:');
		for (const f of modified.slice(0, 20)) console.log('  ' + f);
		if (modified.length > 20) console.log('  … and ' + (modified.length - 20) + ' more');
		console.log('Re-download ' + Brand.name + ' from the official source and verify it before using it with real vaults.');
		process.exitCode = 1; return;
	}
	console.log('GENUINE — all ' + r.count + ' signed application file(s) match the release signed by the maintainer\'s key' + (r.version ? ' (version ' + r.version + ')' : '') + '.');
	if (r.extraneous && r.extraneous.length) console.log('Note: ' + r.extraneous.length + ' extra file(s) are present that are not part of the signed set — normal for a from-source checkout; a downloaded release has none.');
}
...
async function cmdRecoveryKit(pos, flags) {
	requireStringFlag(flags, 'out', '--out kit.html'); // fail fast on a bare --out, before the password prompt, not with a path.resolve(true) crash after
	const target = pos[0];
	...
	// Name the file by the vault's readable name (an opaque vault would otherwise name it by its hex id). The title can
	// hold characters a folder name never would, so sanitize it to a safe, cross-platform file name.
	const kitName = String(await vdisk.friendlyName(r.vault)).replace(/[^\p{L}\p{N} ._-]/gu, '_').trim() || 'vault';
	const outFile = path.resolve(outArg || (kitName + '-recovery-kit.html'));
...
async function cmdAudit(pos) {
	...
	list('Damaged (present, but their content could not be read — restore from a backup or a saved version)', rep.damaged);
	printSyncIssues(rep.syncIssues);
...
async function cmdServe(pos, flags) {
	const target = pos[0];
	if (!target) throw new Error('Usage: ' + Brand.cli + ' serve <path> [--lan | --wan | --bind <ip> | --relay <hub> --token <t>] [--port <n>] [--read-only]');
	...
	let relay = null;
	if (flags['relay-code'] !== undefined && flags['relay-code'] !== true) {
		// One paste carries the address AND the token (see "relay --host"), so a node needs neither --relay nor --token.
		const rc = vdisk.parseRelayCode(flags['relay-code']);
		if (!rc) throw new Error('That relay code is not valid. Copy the exact "relay invite code" printed by "' + Brand.cli + ' relay --host <public-host>".');
		const { host: rh, port: rp } = Common.splitHostPort(rc.host, Common.DEFAULT_RELAY_PORT);
		relay = { host: rh, port: rp, token: rc.token };
	} else if (flags.relay) {
		const { host: rh, port: rp } = Common.splitHostPort(flags.relay, Common.DEFAULT_RELAY_PORT); // bracket-aware, so an IPv6 hub literal ([::1]:7443) is parsed correctly
	...
	// --lan: a direct, same-network serve — binds all interfaces, serves over TLS, and announces itself so an
	// authorized peer on this network finds it automatically (no relay, no port forwarding, nothing to configure).
	// --wan: the same direct same-network serve, but also asks the router to open a port automatically (NAT-PMP/PCP).
	// If the router obliges the vault is reachable directly from anywhere; otherwise it degrades to LAN-only. The port
	// is released when the node stops.
	// --onion: publish an ephemeral onion through a running Tor and serve behind it — an anonymizing overlay reach with
	// no exposed IP and no forwarded port. Needs a Tor already running (a system tor, or the Tor Browser); if none is
	// found it fails with a clear message and nothing else is affected.
	if ((flags.lan || flags.wan || flags.onion) && flags.bind) throw new Error('Use --lan (this network), --wan (this network + the internet), --onion (an anonymizing overlay network), or --bind <ip> — not more than one.');
	if ([flags.lan, flags.wan, flags.onion].filter(Boolean).length > 1) throw new Error('Choose only one of --lan, --wan, or --onion.');
	const wan = !!flags.wan && !relay;
	const lan = !!flags.lan && !relay && !wan;
	const onion = !!flags.onion && !relay && !lan && !wan;
	// --single-hop (only with --onion): publish a faster SINGLE onion service — one hop instead of three. This machine's
	// location is no longer hidden from the network, but the client still connects anonymously and the address + connect
	// code still gate access. Opt-in; ignored without --onion.
	const singleHop = onion && !!flags['single-hop'];
	// --stable-address (only with --onion): keep the SAME .onion and connect code across serves (persisted, encrypted, on
	// this machine). --new-address forgets any saved stable identity first, so this serve mints a fresh one to hand out.
	const stableOnion = onion && !!flags['stable-address'];
	if (onion && flags['new-address']) { try { await vdisk.clearOnionIdentity(target); } catch (_) {} }
	const h = await vdisk.serveVault(target, { bind: (relay || lan || wan || onion) ? undefined : flags.bind, lan, wan, onion, singleHop, stableOnion, port: parsePort(flags.port, undefined), readOnly: !!flags['read-only'], onEvent, relay });
	...
	const bind = relay ? 'relay' : (wan ? 'wan' : (lan ? 'lan' : (onion ? 'onion' : (flags.bind || '127.0.0.1'))));
	console.log('\n  Address:   ' + h.url);
	console.log('  Username:  ' + h.user);
	console.log('  Password:  ' + h.pass + '\n');
	if (bind === 'relay' || bind === 'lan' || bind === 'wan' || bind === 'onion') {
		console.log('One-paste connection code for the other machine:\n  ' + vdisk.makePeerCode({ url: h.url, user: h.user, pass: h.pass, ca: h.ca, cand: h.candidates, sig: h.sig, oca: h.onionAuth }) + '\n');
		console.log('On the other machine: "' + Brand.cli + ' peer-add <that-code>", then "' + Brand.cli + ' mirror <vault> webdav:<id>".');
		if (bind === 'onion') {
			console.log('Reachable over Tor with no exposed address or forwarded port. The connect code carries a client-authorization key, so the onion is invisible to anyone without the code — a leaked address alone cannot connect. The other machine needs Tor too (it sets itself up automatically).');
			if (singleHop) console.log('Single-hop mode: faster (one hop instead of three), but this machine\'s location is no longer hidden from the network. The other machine still connects anonymously, and the connect code still gates access.');
			if (stableOnion) console.log('Stable address: this .onion and connect code stay the same every time you serve this vault, so you can share the code once. The keys are stored encrypted on this machine; run again with --new-address to change them.');
		} else if (bind === 'wan') {
			console.log(h.secure
				? (h.wan
					? 'Your router opened a port, so this node is reachable directly from anywhere over an encrypted connection — the other device pins this node\'s certificate. The port is released when you stop.'
					: 'Your router would not open a port, so this stays reachable on your local network only, over an encrypted connection — the other device discovers it automatically. Add "--relay <hub-host> --token <token>" to reach it from anywhere with no port forwarding.')
				: 'Serving. NOTE: plain HTTP (a TLS certificate could not be made), so the connection is not encrypted. The vault contents are encrypted either way; serve again to retry the certificate.');
		} else if (bind === 'lan') {
			console.log(h.secure
				? 'Served on your local network over an encrypted connection — the other device pins this node\'s certificate and discovers this node automatically, with no server involved. If its address changes, discovery keeps it reachable.'
				: 'Served on your local network. NOTE: plain HTTP (a TLS certificate could not be made), so the connection is not encrypted. The vault contents are encrypted either way; serve again to retry the certificate.');
		} else {
			console.log(h.secure
...
async function cmdRelay(pos, flags) {
	...
	const port = parsePort(flags.port, Common.DEFAULT_RELAY_PORT);
	let range = [...Common.DEFAULT_RELAY_DATA_PORT_RANGE];
	...
	// The relay TOKEN travels in the clear on the node<->hub control link (the vault data and the serve login are
	// TLS-encrypted end to end when a certificate is in use, which is the default). So the hub itself, or anyone on
	// the network path to it, can read the token. Say so plainly here, not only in the docs, so an operator putting a
	// bare hub on the public internet knows to run it on a trusted host or behind a VPN/TLS terminator.
	console.log('\nNote: the token is sent in the clear to this hub (your vault data and serve login stay encrypted end to end).');
	console.log('      Run the hub on a host you trust, or reach it over a VPN/TLS terminator, on an untrusted network.');
	console.log('\nOn each node:  ' + Brand.cli + ' serve <vault> --relay <this-host>:' + port + ' --token ' + (provided ? '<your-token>' : token));
	// Effortless setup: with --host <public-host>, print a single "relay invite code" that carries the address AND the
	// token, so a node joins with one paste instead of typing both. The code holds the token, so it is as secret as the
	// token — share it privately. The host is combined with the control port when it carries no explicit port.
	if (flags.host !== undefined && flags.host !== true) {
		const h = String(flags.host).trim();
		const hostPort = h.includes(':') ? h : h + ':' + port; // a bare host (no port) gets this hub's control port
		console.log('\nRelay invite code — one paste sets up each node (it carries the address AND the token, so share it privately):');
		console.log('  ' + vdisk.makeRelayCode({ host: hostPort, token }));
		console.log('On each node:  ' + Brand.cli + ' serve <vault> --relay-code <code>');
	} else {
		console.log('\nTip: pass --host <public-host> to also print a single "relay invite code" a node joins with one paste ("' + Brand.cli + ' serve <vault> --relay-code <code>").');
	}
...
async function cmdPeerAdd(pos, flags) {
	...
	const { id } = code
		? await vdisk.savePeer({ label: flags.label, url: code.url, user: code.user, password: code.password, ca: code.ca, cand: code.cand, sig: code.sig })
...
// Receive (pull) a vault from another machine by its connect code — the destination side of moving a vault between
// machines. Get the code from "<cli> serve <vault>" on the other machine; only the encrypted store is copied, and the
// same password opens the received vault here (the key derives from the password + the copied manifest's salt).
async function cmdReceiveVault(pos, flags) {
	const code = pos[0];
	if (!code) throw new Error('Usage: ' + Brand.cli + ' receive-vault <connect-code> [--name <name>]  (get the code from "' + Brand.cli + ' serve <vault>" on the other machine)');
	console.log('Receiving the vault (copying the encrypted store, then verifying it is complete)…');
	const r = await vdisk.receiveVaultFromCode(code, { toRoot: vdisk.vaultsDir(), name: flags.name });
	console.log('Received to ' + r.vault + '. Open it with your password:  ' + Brand.cli + ' mount "' + r.vault + '"');
}

async function cmdDoctor() {
	const d = await vdisk.doctor();
	jsonData({ platform: d.platform, arch: d.arch, engine: d.engine, driver: d.driver, ready: !!(d.engine.ok && d.driver.ok) });
...
// A monitoring-friendly health check: run the integrity self-check and exit 0 (OK), 1 (warnings), or 2 (errors), so a
// monitoring plugin, a cron job, a container health check, or an orchestration probe can gate on the exit code. With
// --json it prints one machine-readable object; otherwise a short human summary. Mirrors the web /health endpoint's verdict
// (both use Metrics.healthVerdict) so the CLI and the endpoint always agree.
async function cmdHealthcheck() {
	const findings = await vdisk.selfCheck({ label: 'healthcheck', quiet: true });
	const Metrics = require('./Metrics');
	const v = Metrics.healthVerdict(findings);
	const checks = require('./SelfCheck').list().length; // total checks run, matching the /health endpoint's "checks"
	jsonData({ status: v.status, checks: checks, warnings: v.warn, errors: v.error, findings: findings.map(f => ({ check: f.check, level: f.level, message: f.message })) });
	console.log('Status: ' + v.status.toUpperCase() + (findings.length ? '  (' + v.error + ' error(s), ' + v.warn + ' warning(s))' : '  (all checks passed)'));
	for (const f of findings) console.log('  [' + f.level + '] ' + f.message + (f.fix ? '\n         ' + f.fix : ''));
	process.exitCode = v.code; // 0 ok, 1 warnings, 2 errors
}
...
async function cmdUi(pos, flags) {
	...
	// Persistent diagnostic log FILES stay OFF unless the operator opts in — the default leaves no on-disk record of
	// which vault opened or changed and when. Enable them for this long-running service from a saved setting or a
	// one-run --diagnostic-log flag; short one-shot CLI commands never persist a trail regardless.
	try { if (flags['diagnostic-log'] || (await vdisk.getSettings()).keepDiagnosticLog) Common.setDiagnosticLogging(true); } catch (_) {}
	...
	if (!bind) {
		const runningOwner = await OwnerClient.find();
		// The packaged desktop app launches this same `ui` backend on the shared loopback port. When a TRUSTED
		// instance is already running (the login/background service), the app must show THAT instance rather than
		// fail on the busy port. Signal the shell with a distinct exit code so it attaches instead of showing its
		// startup-error screen. Only a trusted owner qualifies — an owner-only pidfile with a live process, the same
		// record the CLI already trusts to route the vault password. A process that merely answers the port is NOT
		// trusted: the app falls through to start(), fails to bind, and shows the error rather than ever navigating to
		// a server it cannot vouch for. So a squatter can never be shown in the trusted window.
		if (runningOwner && Common.isDesktopApp() && await OwnerClient.verifyOwner(runningOwner.url, runningOwner.token)) {
			console.log(Brand.name + ' is already running; the app will show that instance.');
			process.exit(Common.DESKTOP_ATTACH_EXIT_CODE);
		}
		if (runningOwner || await serviceResponding(port)) {
	...
	// A per-launch readiness token, passed only by the packaged desktop shell. The shell confirms the backend echoes
	// it at /__ready before navigating the trusted window, so it never shows a foreign process that grabbed the port
	// first. A bare (value-less) flag is ignored. Absent for the headless service.
	const readyToken = (typeof flags['ready-token'] === 'string' && flags['ready-token']) ? flags['ready-token'] : undefined;
	await start(port, { bind, allowIp: flags['allow-ip'], denyIp: flags['deny-ip'], readyToken });
...
function runWindowsSupervisor(port, bind, { allowIp, denyIp } = {}) {
	...
	const spawnChild = () => {
		...
		const child = spawn(node, args, { stdio: 'inherit', windowsHide: true }); // windowsHide for consistency with every other background spawn: never pop a console window if this is ever launched console-less
		// Both a spawn 'error' (bad path / permissions / locked image) and an unclean 'exit' are crashes and MUST go
		// through the SAME backoff + crash-loop ceiling — a spawn that keeps erroring (e.g. the branded launcher was
		// renamed) never reaches 'exit', so handling only 'exit' would leave 'error' respawning every second forever
		// with no escalation. A per-child `settled` guard makes exactly one of error/exit drive the next attempt, so
		// a child that emits both cannot double-respawn or double-count.
		let settled = false;
		const onDone = (cleanStop) => {
			if (settled) return; settled = true;
			if (stopping || cleanStop) return process.exit(0); // clean stop -> do not respawn
		...
		child.on('error', () => onDone(false)); // could not spawn — a crash, not a clean stop
		child.on('exit', (code, signal) => onDone(code === 0 || signal === 'SIGINT' || signal === 'SIGTERM'));
	};
	spawnChild();
...
async function cmdApiToken(pos, flags) {
	const sub = (pos[0] || '').toLowerCase();
	if (sub === 'create' || sub === 'new' || sub === 'add') {
		const r = await vdisk.createApiToken(typeof flags.label === 'string' ? flags.label : undefined);
		jsonData(r);
		console.log('API token created (' + r.label + '). It is shown ONCE — copy it now:\n');
		console.log('  ' + r.token + '\n');
		console.log('Use it as a bearer header on the local API:  Authorization: Bearer ' + r.token);
		console.log('It authenticates without a browser session, so a script or a third-party front-end can reach the API when the interface is network-exposed. Revoke it with "' + Brand.cli + ' api-token revoke ' + r.id + '".');
		return;
	}
	if (sub === 'list' || sub === 'ls' || sub === '') {
		const list = await vdisk.listApiTokens();
		jsonData({ tokens: list });
		if (!list.length) { console.log('No API tokens. Create one with "' + Brand.cli + ' api-token create".'); return; }
		console.log('API tokens (' + list.length + '):');
		for (const t of list) console.log('  ' + t.id + '  ' + (t.label || '') + (t.createdAt ? '  — created ' + t.createdAt : ''));
		return;
	}
	if (sub === 'revoke' || sub === 'remove' || sub === 'rm') {
		const id = pos[1];
		if (!id) throw new Error('Usage: ' + Brand.cli + ' api-token revoke <id>   (see "' + Brand.cli + ' api-token list")');
		const r = await vdisk.revokeApiToken(id);
		jsonData(r);
		console.log(r.removed ? 'Revoked API token ' + id + '.' : 'No API token with id ' + id + '.');
		return;
	}
	throw new Error('Usage: ' + Brand.cli + ' api-token <create|list|revoke <id>>   (add --label <name> when creating)');
}
...
// Show or set the global Tor bridge (for reaching Tor from a network that blocks it). No argument shows the current
// setting; `off` disables it; `snowflake`/`obfs4`/`meek` use built-in bridges; `custom <line>…` takes your own.
async function cmdTorBridge(pos, _flags) {
	const mode = String(pos[0] || '').toLowerCase();
	if (!mode) {
		const cur = await vdisk.getTorBridges();
		console.log('Tor bridge: ' + cur.mode + (cur.lines && cur.lines.length ? ' (' + cur.lines.length + ' custom line(s))' : ''));
		console.log('A bridge helps reach Tor from a network that blocks it. Set one with:');
		console.log('  ' + Brand.cli + ' tor-bridge <off | snowflake | obfs4 | meek | custom "<bridge line>" ...>');
		return;
	}
	const lines = mode === 'custom' ? pos.slice(1) : [];
	const saved = await vdisk.setTorBridges({ mode: mode, lines: lines });
	if (saved.mode === 'off') console.log('Tor bridges are off (connecting to Tor directly).');
	else console.log('Tor bridge set to "' + saved.mode + '"' + (saved.lines.length ? ' with ' + saved.lines.length + ' custom line(s)' : '') + '. It applies to Vaultonaut\'s own Tor for onion connections.');
}
...
async function dispatch(cmd, positionals, flags) {
	let keepAlive = false;
	warnUnknownFlags(flags); // surface a misspelled option (e.g. "--reed-only") instead of silently dropping the user's intent
	switch (cmd) {
		case '_guard': await require('./Guardian').watch(parseInt(positionals[0], 10), positionals[1] || 'ui'); process.exit(0); break; // watch() only resolves once the owner is gone and its vaults are locked — the guardian's whole job — so exit deterministically here rather than relying on natural event-loop drain (defense-in-depth against a future stray open handle keeping the guardian process alive)
		case 'create': await cmdCreate(positionals, flags); break;
		case 'name': case 'describe': await cmdName(positionals, flags); break;
		case 'cloud': await cmdCloud(positionals, flags); break;
		case 'import': await cmdImport(positionals, flags); break;
		case 'notes-import': case 'notesimport': await cmdNotesImport(positionals, flags); break;
		...
		case 'consent-on-use': case 'consent': await cmdConsentOnUse(positionals, flags); break;
		...
		case 'peer-mode': await cmdPeerMode(positionals); break;
		...
		case 'prove-file': await cmdProveFile(positionals, flags); break;
		case 'verify-file': await cmdVerifyFile(positionals, flags); break;
		case 'verify-self': await cmdVerifySelf(); break;
		...
		case 'tor-bridge': case 'tor-bridges': await cmdTorBridge(positionals, flags); break;
		case 'restore': await cmdRestore(positionals, flags); break;
		case 'receive-vault': case 'receive': await cmdReceiveVault(positionals, flags); break;
		case 'doctor': await cmdDoctor(); break;
		case 'healthcheck': case 'health': await cmdHealthcheck(); break;
		...
		case 'api-token': case 'apitoken': await cmdApiToken(positionals, flags); break;
...
module.exports = { parse, dispatch, help, takeJsonData, jsonErrorCode, _serviceLaunchArgs: serviceLaunchArgs };
module.exports._flagSets = { BOOLEAN_FLAGS, VALUE_FLAGS, KNOWN_FLAGS }; // test-only: cliparse.js pins KNOWN_FLAGS complete (so the unknown-flag warning never fires on a real flag) and BOOLEAN/VALUE disjoint
