...
async function cmdServe(pos, flags) {
	...
	if (bind === 'relay' || bind === 'lan' || bind === 'wan') {
		console.log('One-paste connection code for the other machine:\n  ' + vdisk.makePeerCode({ url: h.url, user: h.user, pass: h.pass, ca: h.ca, cand: h.candidates }) + '\n');
...
async function cmdPeerAdd(pos, flags) {
	...
	const { id } = code
		? await vdisk.savePeer({ label: flags.label, url: code.url, user: code.user, password: code.password, ca: code.ca, cand: code.cand })
