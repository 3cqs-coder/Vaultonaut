...
async function cmdUnmount(pos, flags) {
	...
	// --force overrides a stuck/stale mount (skips the flush; may discard unflushed writes).
	const res = await unmountVia(target, { wipeCache: !flags['keep-cache'], force: !!flags.force });
	if (!res.ok) {
		...
		else console.log('Could not fully unmount ' + res.mountpoint + ' (it may be busy). Try again with --force.');
...
async function cmdServe(pos, flags) {
	...
	const onEvent = (e) => {
		...
		else if (e.type === 'gaveup') { console.error('The server could not be kept running after repeated attempts. Exiting.'); process.exit(1); }
	};
...
async function cmdStop(pos, flags) {
	...
	try { process.kill(pid, 'SIGTERM'); } // POSIX: runs the graceful drain-and-lock shutdown. Windows: terminates (safe — see above).
