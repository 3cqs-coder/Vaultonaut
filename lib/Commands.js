...
async function cmdNotesImport(pos, flags) {
	...
	if (!file || !target) throw new Error('Usage: ' + Brand.cli + ' notes-import <file> <vault>   (import logins and notes from a .csv or a Bitwarden .json export into an OPEN vault; add --format csv|bitwarden|auto, --dry-run, or --skip-duplicates)');
	let text;
	try { text = fs.readFileSync(file, 'utf8'); } catch (e) { throw new Error('Could not read the import file "' + file + '": ' + (e && e.message || e)); }
