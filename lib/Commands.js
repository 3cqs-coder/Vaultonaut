...
async function cmdEmergency(pos, flags) {
	...
	if (sub === 'arm') {
		const target = pos[1]; if (!target) throw new Error('Usage: ' + Brand.cli + ' emergency arm <vault> [--contact <id-or-label>]');
		...
		const r = await vdisk.emergencyArm(target, { password, contactId });
		console.log('Armed "' + vdisk.displayName(target) + '" for "' + r.contactLabel + '". Remember to check in with:  ' + Brand.cli + ' emergency check-in');
		return;
	}
