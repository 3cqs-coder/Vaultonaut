...
const BOOLEAN_FLAGS = new Set([
	...
	'dry-run', 'skip-duplicates', 'json', 'lan', 'wan', 'opaque', 'anonymize',
]);
...
async function cmdServe(pos, flags) {
	...
	if ((flags.lan || flags.wan) && flags.bind) throw new Error('Use --lan (this network), --wan (this network + the internet), or --bind <ip> — not more than one.');
	if (flags.lan && flags.wan) throw new Error('Use either --lan (this network) or --wan (this network + the internet), not both.');
	const wan = !!flags.wan && !relay;
	const lan = !!flags.lan && !relay && !wan;
	const h = await vdisk.serveVault(target, { bind: (relay || lan || wan) ? undefined : flags.bind, lan, wan, port: parsePort(flags.port, undefined), readOnly: !!flags['read-only'], onEvent, relay });
	...
	const bind = relay ? 'relay' : (wan ? 'wan' : (lan ? 'lan' : (flags.bind || '127.0.0.1')));
	console.log('\n  Address:   ' + h.url);
