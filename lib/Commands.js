...
const BOOLEAN_FLAGS = new Set([
	...
	'dry-run', 'skip-duplicates', 'json', 'lan',
]);
...
async function cmdServe(pos, flags) {
	const target = pos[0];
	if (!target) throw new Error('Usage: ' + Brand.cli + ' serve <path> [--lan | --bind <ip> | --relay <hub> --token <t>] [--port <n>] [--read-only]');
	...
	if (flags.lan && flags.bind) throw new Error('Use either --lan (this network) or --bind <ip>, not both.');
	const lan = !!flags.lan && !relay;
	const h = await vdisk.serveVault(target, { bind: (relay || lan) ? undefined : flags.bind, lan, port: parsePort(flags.port, undefined), readOnly: !!flags['read-only'], onEvent, relay });
	...
	const bind = relay ? 'relay' : (lan ? 'lan' : (flags.bind || '127.0.0.1'));
	console.log('\n  Address:   ' + h.url);
	console.log('  Username:  ' + h.user);
	console.log('  Password:  ' + h.pass + '\n');
	if (bind === 'relay' || bind === 'lan') {
