...
function scheduleTimingFromFlags(flags) {
	if (flags.daily) {
		...
		const ut = localHmToUtc(hour, minute); // local input -> UTC storage
		return { mode: 'daily', hour: ut.hour, minute: ut.minute };
	}
...
function describeScheduleWhen(s) {
	...
	if (s.mode === 'daily') { const lt = utcHmToLocal(s.hour, s.minute); return 'daily at ' + String(lt.hour).padStart(2, '0') + ':' + String(lt.minute).padStart(2, '0') + ' local'; }
	return 'off';
}
