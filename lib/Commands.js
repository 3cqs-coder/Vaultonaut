...
const BOOLEAN_FLAGS = new Set([
	...
	'keep-cache', 'off', 'list', 'content', 'worm', 'write', 'desktop', 'diagnostic-log',
]);
...
const VALUE_FLAGS = new Set([
	...
	'fuse-backend', 'grace-days', 'in', 'inactive-days', 'k', 'kdf', 'keep', 'key', 'key-shares', 'keyfile', 'keys',
...
async function cmdEmergency(pos, flags) {
	...
	if (sub === 'arm') {
		...
		if (r.mode === 'date') console.log('Armed "' + vdisk.displayName(target) + '" for "' + r.contactLabel + '". It unlocks on ' + new Date(r.opensAt).toLocaleString() + ' (' + r.opensAt + ').');
