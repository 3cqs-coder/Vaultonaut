...
async function cmdEmergency(pos, flags) {
	...
	if (sub === 'backstop') {
		...
		if (action === 'set') {
			if (!flags['trustees-file']) throw new Error('Usage: ' + Brand.cli + ' emergency backstop set --trustees-file <file.json> --need <k>\nThe file is a JSON array of { "label": "...", "pub": "<their public key>" }.');
			...
			const r = await vdisk.emergencySetBackstop({ trustees: list, k: Number(flags.need) });
