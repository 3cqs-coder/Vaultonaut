...
// Validate advertised candidates hard (untrusted, from a pasted code): keep only well-formed {host, port} entries, cap
// the count, and never let it grow unbounded — a candidate only ever becomes a TCP connect target, never a shell/arg.
function sanitizeCandidates(cand) {
	if (!Array.isArray(cand)) return [];
	const out = [];
	for (const c of cand) {
		if (out.length >= 8) break;
		if (c && typeof c.host === 'string' && c.host.length <= 64 && /^[0-9A-Za-z.:_-]+$/.test(c.host) && Number.isInteger(c.port) && c.port >= 1 && c.port <= 65535) out.push({ host: c.host, port: c.port });
	}
	return out;
}
