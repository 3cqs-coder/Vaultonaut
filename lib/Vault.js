...
async function healWork(abs, { allowUnverified, force, preserveInPlaceEdits, onProgress } = {}) {
	...
	const ctx = await recoveryAuthContext(abs);
	const res = await Recovery.heal(abs, { cipherDir: cipherDirOf(abs), pubkey: ctx.pubkey, vaultId: ctx.vaultId, allowUnverified, force, preserveInPlaceEdits, onProgress });
	return annotateRecoveryDowngrade(res, ctx.vaultId);
}
