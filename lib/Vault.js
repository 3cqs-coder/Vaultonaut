...
async function noteRecordRaw(vaultDir, id) {
	...
	const cfg = await Rclone.writeEphemeralConfig(cfgText);
	let raw;
	try { raw = await readNoteObject(bin, cfg, id + '.json'); } finally { await Rclone.removeConfig(cfg); }
...
async function noteRecordRawWrite(vaultDir, id, record) {
	...
	return serializeNoteWrite(vaultDir, async () => {
		const cfg = await Rclone.writeEphemeralConfig(cfgText);
		try { await writeVaultObjectAtomic(bin, cfg, NOTES_DIR + '/' + id + '.json', JSON.stringify(record), { timeoutMs: MOUNT_FS_TIMEOUT_MS }); }
		finally { await Rclone.removeConfig(cfg); }
	});
}
...
async function noteDelete(vaultDir, id) {
	...
	return serializeNoteWrite(vaultDir, async () => {
		...
		try { await Common.withTimeout(Rclone.run(bin, ['deletefile', 'vault:' + NOTES_DIR + '/' + id + '.json'], { configPath: cfg, timeoutMs: MOUNT_FS_TIMEOUT_MS }), MOUNT_FS_TIMEOUT_MS); }
		finally { await Rclone.removeConfig(cfg); }
		return { deleted: true };
	});
}
