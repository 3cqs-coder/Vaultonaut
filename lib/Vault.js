...
function parsePeerCode(code) { const s = String(code || '').trim(); if (s.length > 8192) return null; try { const o = JSON.parse(Buffer.from(s, 'base64').toString('utf8')); if (o && o.url) return { url: o.url, user: o.user || 'vd', password: o.pass || '', ca: o.ca || '' }; } catch (_) {} return null; } // cap the pasted connect code before decoding: a real one is a few hundred bytes (address + login + cert), so a local bound makes it independent of any caller's own limit
...
// Prefer a peer's live LAN address over its stored (relay / last-known) one when the node is on this network — a
// direct, relay-free hop over the SAME pinned-TLS channel and credentials. Returns the peer object unchanged when the
// node is not discoverable, so the stored address is used and the relay stays the guaranteed fallback.
async function preferLanUrl(peer) {
	try { const lanUrl = await findPeerOnLan(peer, { timeoutMs: 2000 }); if (lanUrl) return { ...peer, url: lanUrl, viaLan: true }; } catch (_) {}
	return peer;
}
