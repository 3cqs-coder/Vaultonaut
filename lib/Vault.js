...
function cleanMetaText(v, max) {
	if (v == null) return '';
	return String(v).replace(/[\p{Cc}\p{Cf}]/gu, ' ').replace(/\s+/g, ' ').trim().slice(0, max);
}
...
async function anonymizeVault(vaultDir, { password } = {}) {
	...
	const salt = manifest.crypt && manifest.crypt.salt;
	const existingTitle = (openMetaObj(manifest.meta, cred.master, salt).title || '').trim();
	...
	try {
		...
		await rememberVaultName(vid, { title });
	} catch (e) {
		try { await Common.renameWithRetry(newAbs, oldAbs); } catch (_) {} // roll the rename back if the bookkeeping failed, so the vault is never left stranded
		throw e;
	}
...
module.exports = {
	...
	readManifest, readManifestForPoll, checkManifestSeal, updateManifestFields, displayName, vaultTitle, vaultIdOf, cachedVaultName, getVaultMeta, setVaultMeta, anonymizeVault, defaultMountRoot, normalizeMountpoint, resolveVaultDir, assertReadable, checkOnMount, isIgnoredVaultPath, INTERNAL_VAULT_OBJECTS, OS_JUNK_COMMON, RECOVERY_EXCLUDE_NAMES,
