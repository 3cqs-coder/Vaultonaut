...
function startIndexWatch(mountpoint, vault, readOnly) {
	...
	// Only watch a vault that already has a content index — the same gate as the on-mount refresh, so we never build an
	// index for a vault whose owner never searches inside files.
	fsp.access(searchIndexPathFor(mountpoint)).then(() => {
		let watcher;
		try {
			// Recursive watch is supported on macOS, Windows, and (Node 20+) Linux; on a mount that cannot deliver events
			// it simply stays quiet, and if the platform/mount refuses a recursive watch outright the throw is caught and
			// we fall back to the search-time refresh. persistent:false keeps the watcher from holding the process open.
			watcher = fs.watch(mountpoint, { recursive: true, persistent: false }, (_evt, name) => {
				...
				rec.timer = setTimeout(() => { contentReindex(rec.vault, { allowOcr: true }).catch(() => {}); }, INDEX_WATCH_DEBOUNCE_MS);
			});
	...
	}, () => {}); // no index yet: nothing to keep fresh (a first build is still an explicit or search-time action)
}
...
async function contentReindex(vaultDir, { onProgress, allowOcr = false } = {}) {
	...
	try {
		...
		return await WorkerRun.runWorker(path.join(__dirname, 'SearchWorker.js'), { op: 'index', args: { mountpoint, ocr } }, onProgress, { idleMs: 180000, idleMessage: 'Indexing the vault', resourceLimits: { maxOldGenerationSizeMb: 2560 } });
