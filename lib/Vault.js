...
async function rotate(vaultDir, { password, reason, onProgress } = {}) {
	...
	return withVaultLock(abs, () => withVaultBusy(abs, 'This vault is busy with another operation — wait for it to finish, then rotate.', async () => {
		...
		// Free-space precondition: rotation copies the store to a new location before swapping, so the copy roughly
		// doubles the on-disk footprint. That copy needs at least about 1.1x the store's current size FREE (the new
		// copy plus a small margin), which is what the guard below checks.
		...
		let journalState = { v: REKEY_JOURNAL_VERSION, phase: 'COPY', oldIdentity, newIdentity, hadRecovery, reason: reason || 'rotation', at: new Date().toISOString() };
