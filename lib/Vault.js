...
async function savePeer(peer = {}) {
	...
	await mutateSettings(cur => {
		...
		const prev = peers[id] || {};
		peers[id] = { label, url, user, password: (password != null && password !== '') ? encCred(password, key) : (prev.password || ''), ca: ca !== undefined ? ca : (prev.ca || ''), cand: cand !== undefined ? cand : (prev.cand || []), sig: sig !== undefined ? (sig || undefined) : (prev.sig || undefined) };
		return { ...cur, peers };
	});
	return { id };
}
...
function makePeerCode({ url, user, pass, ca, cand, sig } = {}) { return Buffer.from(JSON.stringify({ v: 1, url, user, pass, ca: ca || undefined, cand: (Array.isArray(cand) && cand.length) ? cand : undefined, sig: sanitizeSig(sig) })).toString('base64'); }
function parsePeerCode(code) { const s = String(code || '').trim(); if (s.length > 8192) return null; try { const o = JSON.parse(Buffer.from(s, 'base64').toString('utf8')); if (o && o.url) return { url: o.url, user: o.user || 'vd', password: o.pass || '', ca: o.ca || '', cand: sanitizeCandidates(o.cand), sig: sanitizeSig(o.sig) }; } catch (_) {} return null; } // cap the pasted connect code before decoding: a real one is a few hundred bytes (address + login + cert), so a local bound makes it independent of any caller's own limit
...
async function buildWebdavConfig(bin, d) {
	...
	let proxyEnv = null;
	if (Onion.isOnionUrl(d.url)) { try { const t = await Onion.provideTor(); const su = 'socks5h://127.0.0.1:' + t.socksPort; proxyEnv = { ALL_PROXY: su, HTTPS_PROXY: su, HTTP_PROXY: su, all_proxy: su }; } catch (_) { proxyEnv = null; } }
...
async function serveDirTransport(abs, { user, pass, bind, port, readOnly, onLine, onEvent, relay, lan, wan, onion, isVault } = {}) {
	...
	if (onion && !(relay && relay.host)) {
		...
		try { const tor = await Onion.provideTor(); onionSvc = await Onion.publishOnion(served0.port, { virtualPort: 80, controlPort: tor.controlPort }); }
		catch (e) { try { await served0.stop(); } catch (_) {} throw e; }
		const stop = async () => { try { onionSvc.stop(); } catch (_) {} try { await served0.stop(); } catch (_) {} };
		return { ...served0, url: 'http://' + onionSvc.onion + '/', ca: null, secure: true, onion: onionSvc.onion, bindMode: 'onion', stop, user, pass, isVault };
	}
...
async function storeInvite(served, label) {
	...
	const code = makePeerCode({ url: url, user: served.user, pass: served.pass, ca: served.ca || '' });
