...
async function readNoteObject(bin, cfg, filename) {
	const r = await Common.withTimeout(Rclone.run(bin, ['cat', 'vault:' + NOTES_DIR + '/' + filename], { configPath: cfg, maxOutBytes: MAX_MANIFEST_BYTES }), MOUNT_FS_TIMEOUT_MS).catch(() => null);
...
async function listNoteFiles(bin, cfg) {
	const r = await Common.withTimeout(Rclone.run(bin, ['lsf', 'vault:' + NOTES_DIR + '/', '--files-only'], { configPath: cfg, maxOutBytes: MAX_LISTING_BYTES }), MOUNT_FS_TIMEOUT_MS).catch(() => null);
...
async function noteDelete(vaultDir, id) {
	...
	try { await Common.withTimeout(Rclone.run(bin, ['deletefile', 'vault:' + NOTES_DIR + '/' + id + '.json'], { configPath: cfg }), MOUNT_FS_TIMEOUT_MS); }
	finally { await Rclone.removeConfig(cfg); }
	return { deleted: true };
}
