...
// wrong password throws. Returns the public, stable-shape view (every known field defined, empty when unset).
async function getVaultMeta(vaultDir, password) {
	...
	return { vault: abs, meta: publicMeta(openMetaObj(manifest.meta, cred.master, salt)) };
}
...
async function setVaultMeta(vaultDir, { password, fields } = {}) {
	...
	return { vault: abs, meta: publicMeta(merged) };
}
...
async function travelEnable({ travelPassword, owner } = {}) {
	...
	await mutateSettings(cur => { const next = { ...cur }; for (const k of VAULT_PATH_KEYED) if (next[k]) next[k] = {}; delete next.emergency; delete next.repairSchedules; return next; });
...
async function travelRestore({ travelPassword } = {}) {
	...
	if (Object.keys(maps).length) await mutateSettings(cur => { const next = { ...cur }; for (const k of Object.keys(maps)) { if (!VAULT_PATH_KEYED.includes(k)) continue; next[k] = { ...(next[k] || {}), ...maps[k] }; } return next; });
...
module.exports = {
	...
	readManifest, readManifestForPoll, checkManifestSeal, updateManifestFields, displayName, vaultTitle, getVaultMeta, setVaultMeta, defaultMountRoot, normalizeMountpoint, resolveVaultDir, assertReadable, checkOnMount, isIgnoredVaultPath, INTERNAL_VAULT_OBJECTS, OS_JUNK_COMMON, RECOVERY_EXCLUDE_NAMES,
