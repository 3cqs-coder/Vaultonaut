...
async function receiveVault(sourceKey, { toRoot, name, overwrite } = {}) {
	...
	if (!name || /[\\/]/.test(String(name))) throw new Error('A vault folder name (with no path separators) is required.');
	const dest = path.join(path.resolve(toRoot), name);
	// Never overwrite an existing vault at the destination unless explicitly asked — a move targets a fresh folder, so
	// a collision means a different vault is already there and must not be silently clobbered (same rule as restore).
	if ((await looksLikeVault(dest)) && !overwrite) throw new Error('A vault already exists at ' + dest + '. Receive into a new, empty folder.');
