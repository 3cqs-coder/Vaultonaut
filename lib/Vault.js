...
async function readSignedBaseline(abs, password, manifest, cred) {
	...
	return withVaultConfig(bin, abs, password, manifest, cred.master, async (cfg) => {
		...
		try { return JSON.parse(r.stdout); } catch (_) { throw new Error('This vault\'s snapshot could not be read (it is unreadable or corrupt) — take a fresh snapshot, then try again.'); } // a clear message instead of a raw SyntaxError, matching the other snapshot readers
	});
}
