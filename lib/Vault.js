...
async function recoveryAuthContext(abs) {
	try { const m = await readManifest(abs); return { pubkey: Integrity.pubkeyOf(m), vaultId: Integrity.vaultId(m), recoveryVersion: Number(m.recoveryVersion) || 0 }; }
	catch (_) { return { pubkey: null, vaultId: null, recoveryVersion: 0 }; }
}
...
async function annotateRecoveryDowngrade(res, vaultId, manifestRecoveryVersion = 0) {
...
async function verifyRecoveryWork(abs, { onProgress } = {}) {
	...
	return annotateRecoveryDowngrade(res, ctx.vaultId, ctx.recoveryVersion);
}
...
module.exports._sealParity = { manifestSealInput, manifestSealVerifies, successionInput, attestGenesis, attestChainHash, attestDigest, tokenHashOf };
