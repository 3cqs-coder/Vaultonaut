...
async function notesList(vaultDir) {
	...
	return Rclone.withEphemeralConfig(cfgText, async (cfg) => {
		...
		// The notes key is fetched at most ONCE for the whole list, then only the small encrypted TITLE of each note
		// is decrypted — never the body — so listing many notes cannot load every body into memory (no OOM).
		let nck = null, nckTried = false;
		const notes = [];
		let seen = 0;
		for (const f of files) {
			const raw = await readNoteObject(bin, cfg, f); // skip an unreadable/corrupt note file
			let j = null; try { j = JSON.parse(raw); } catch (_) { j = null; }
			if (!j || !isNoteId(j.id) || !j.t) continue; // a record without a sealed metadata field is not one this build writes
			if (!nckTried) { nckTried = true; try { nck = await notesKeyForMounted(vaultDir); } catch (_) { nck = null; } }
			...
			notes.push({ id: j.id, title: newer ? '(needs a newer version)' : (meta ? (meta.title || '(untitled)') : (nck ? '(unreadable)' : '(locked)')), type: meta ? meta.type : 'note', createdAt: meta && meta.createdAt, updatedAt: meta && meta.updatedAt });
			if (++seen % 50 === 0) await Common.yieldToLoop(); // cooperative: a vault with very many notes never blocks the event loop
		}
...
async function notesHealth(vaultDir, { breach = false } = {}) {
	...
	const pwItems = []; // { id, title, values: [..] }
	const cfg = await Rclone.writeEphemeralConfig(cfgText);
	try {
		const files = await listNoteFiles(bin, cfg);
		let seen = 0;
		for (const f of files) {
			const raw = await readNoteObject(bin, cfg, f);
			let j = null; try { j = JSON.parse(raw); } catch (_) { j = null; }
			if (!j || !isNoteId(j.id) || !j.t || !j.b) continue;
			let meta, content;
			try { meta = openNoteMeta(j.t, nck); content = JSON.parse(openNoteField(j.b, nck)); } catch (_) { continue; }
			const values = (Array.isArray(content.fields) ? content.fields : []).filter(x => x && x.kind === 'password' && x.value).map(x => String(x.value));
			if (values.length) pwItems.push({ id: j.id, title: meta.title || '(untitled)', values });
			if (++seen % 25 === 0) await Common.yieldToLoop();
		}
	} finally { await Rclone.removeConfig(cfg); }
