...
const VAULT_PATH_KEYED = ['mountPrefs', 'backupDests', 'backupSchedules', 'scrubSchedules', 'syncDests', 'syncLast', 'favorites', 'vaultColors', 'serveCreds', 'serveNodeIds', 'serveNodeAuth', 'consentOnUse'];
...
async function serveDirTransport(abs, { user, pass, bind, port, readOnly, onLine, onEvent, relay, lan, wan, onion, singleHop, isVault } = {}) {
	const bin = await ensureEngine();
	...
	if (onion && !(relay && relay.host)) {
		...
		try { const tor = await Onion.provideTor({ singleHop: !!singleHop }); onionSvc = await Onion.publishOnion(served0.port, { virtualPort: 80, controlPort: tor.controlPort, clientAuth: true, singleHop: !!singleHop }); }
		catch (e) { try { await served0.stop(); } catch (_) {} throw e; }
		const stop = async () => { try { onionSvc.stop(); } catch (_) {} try { await served0.stop(); } catch (_) {} };
		return { ...served0, url: 'http://' + onionSvc.onion + '/', ca: null, secure: true, onion: onionSvc.onion, onionAuth: onionSvc.clientAuthPriv || null, singleHop: !!singleHop, bindMode: 'onion', stop, user, pass, isVault };
	}
