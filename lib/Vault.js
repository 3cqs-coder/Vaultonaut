...
// NAMES), these have a VALID decrypted name but unreadable content, so the engine prints a per-file
// "ERROR : <path>: failed to open file …" line and exits non-zero while STILL hashing every OTHER file. Parsing them
// lets the scan report the damaged files and hash the rest, instead of aborting the whole check on one bad blob. The
// path is captured non-greedily up to the stable failure phrase, so a name containing ": " is still kept whole.
function hashsumDamagedFiles(stderr) {
	const names = [];
	for (const line of String(stderr || '').split(/\r?\n/)) {
		const m = line.match(/ ERROR : (.+?): (?:failed to open file|failed to authenticate|not an encrypted file)/i);
...
function hashsumErrorsAllPerFile(stderr) {
	const errs = String(stderr || '').split(/\r?\n/).filter(l => / ERROR : /.test(l));
	return errs.length > 0 && errs.every(l => /failed to open file|failed to authenticate|not an encrypted file/i.test(l));
}
...
async function captureFiles(bin, cfg, { deep, timeoutMs }) {
	...
	const damaged = hashsumDamagedFiles(h.stderr).filter(n => !isIgnoredVaultPath(n));
...
async function writeSnapshot(bin, cfg, abs, manifest, keys, { deep, auto, sealed, timeoutMs }) {
	...
	if (deep && !auto && cap.damaged && cap.damaged.length) {
		const e = new Error('Snapshot refused: ' + cap.damaged.length + ' file(s) are damaged and their content cannot be read (' + cap.damaged.slice(0, 5).join(', ') + (cap.damaged.length > 5 ? ', and more' : '') + '). Repair them (Check & repair) or restore them from a backup or a saved version first, so the baseline records a good state — not the damage.');
...
async function scrubScheduleTick() {
	return runScheduleTick({
		...
		handle: async (abs, sched, now) => {
			...
			await patchScrubSchedule(abs, { lastRunAt: now.toISOString(), lastResult: result, lastErrorAt: (r.checked && !r.clean && (!r.healed || r.deferredEdits)) ? now.toISOString() : null });
			return { abs, result };
		},
