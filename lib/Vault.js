...
async function removeMember(vaultDir, { password, memberKey, memberId, rotate: doRotate = true, onProgress } = {}) {
	...
	await withVaultLock(abs, async () => {
		...
		// while the creator's password-owner slot persists (ownerHasPasswordSlot errs toward yes for it), but a local
		// guard keeps it safe if a future leave-team / ownership-transfer path, or looser key-slot rules, ever change that.
...
const SEAL_VERSION = 3; // the manifest-seal format this build writes and verifies; a newer one is refused forward (see checkManifestSeal)
...
function manifestSealInput(manifest) {
	...
	// left unsealed. The signature binds to this exact string. There is one current format (v3); nothing has shipped
	// an older one, so manifestSealVerifies accepts only this — no legacy fallback to weaken it.
	...
	return ['vault-manifest-seal-v3', Integrity.SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', String(Number(manifest.recoveryVersion) || 0), slots.join(',')].join('\n');
}
