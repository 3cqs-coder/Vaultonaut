...
async function create(vaultDir, { password, level, cloud, worm } = {}) {
	...
	Common.assertPortableVaultName(stripVaultExt(path.basename(resolveVaultDir(vaultDir)))); // reject a name that would not port to Windows (reserved/illegal), so a new vault opens on every platform
