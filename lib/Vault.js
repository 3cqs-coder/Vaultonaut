...
const indexWatchers = new Map(); // mountpoint -> { watcher, timer, vault }
...
function startIndexWatch(mountpoint, vault, readOnly) {
	...
	try {
		...
		watcher = fs.watch(mountpoint, { recursive: true, persistent: false }, (_evt, name) => {
			...
			rec.timer = setTimeout(() => {
				// Only MAINTAIN an index that already exists — never build one unprompted for a vault whose owner does not
				// search inside files. This is checked at FIRE time (not when the watcher started), so a vault whose first
				// index is built after it was mounted still gets kept fresh from that point on.
				fsp.access(searchIndexPathFor(mountpoint)).then(() => contentReindex(rec.vault, { allowOcr: true }).catch(() => {}), () => {});
	...
	indexWatchers.set(mountpoint, { watcher, timer: null, vault });
}
...
// Open a file that lives INSIDE a mounted vault, by its path relative to the mount — used to open a search result in
// its default app (an image in the previewer, a PDF in the reader). The vault must be open, since the file only exists
// through the mount. The relative path is resolved against the mountpoint and confined to it (a leading slash is
// stripped and any `..` that would escape is rejected), so a crafted path can never point the opener outside the vault.
async function revealInVault(vaultDir, rel) {
	const mountpoint = await mountpointFor(vaultDir, 'Open (mount) this vault first to open a file inside it.');
	const abs = path.resolve(mountpoint, String(rel == null ? '' : rel).replace(/^[\\/]+/, ''));
	if (!(abs === normalizeMountpoint(mountpoint) || Common.pathWithin(abs, mountpoint))) throw new Error('That file is not inside the vault, so it was not opened.');
	return await reveal(abs);
}
