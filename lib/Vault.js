...
async function notesList(vaultDir) {
	...
	let entries = [];
	// Bound each mount read: notes live inside the mount, so a wedged mount must fail fast here, never hang the
	// request or pin a threadpool thread (the never-freeze invariant).
	try { entries = await Common.withTimeout(fsp.readdir(dir), MOUNT_FS_TIMEOUT_MS); } catch (_) { return { notes: [] }; } // no folder yet (or a wedged mount) = no notes
	// The notes key is fetched at most ONCE for the whole list, then only the small encrypted TITLE of each note is
	// decrypted — never the body — so listing many notes cannot load every body into memory (no OOM) and stays cheap.
	let nck = null, nckTried = false;
	const notes = [];
	let seen = 0;
	for (const f of entries) {
		if (!f.endsWith('.json')) continue;
		const j = await Common.withTimeout(Common.readJsonCorruptAside(path.join(dir, f)), MOUNT_FS_TIMEOUT_MS).catch(() => null); // skip an unreadable/corrupt/wedged note file
...
async function noteGet(vaultDir, id) {
	...
	const dir = notesDirFor(await noteMountpoint(vaultDir));
	const j = JSON.parse(await Common.withTimeout(Common.readFileCapped(path.join(dir, id + '.json'), MAX_MANIFEST_BYTES, 'utf8'), MOUNT_FS_TIMEOUT_MS)); // bounded: the note lives inside the mount
...
async function noteSave(vaultDir, { id, title, type, fields, note, body } = {}) {
	...
	const dir = notesDirFor(await noteMountpoint(vaultDir));
	await Common.withTimeout(fsp.mkdir(dir, { recursive: true }), MOUNT_FS_TIMEOUT_MS); // bounded: the notes dir is inside the mount
	let theId = id;
	...
	let existing = {};
	try { existing = JSON.parse(await Common.withTimeout(Common.readFileCapped(path.join(dir, theId + '.json'), MAX_MANIFEST_BYTES, 'utf8'), MOUNT_FS_TIMEOUT_MS)) || {}; } catch (_) {}
	...
	// createdAt is set once and preserved across edits (it lives inside the existing note's sealed metadata, so reading
	// it needs the notes key we already hold); updatedAt is stamped every save. Both are ENCRYPTED, not plaintext.
	let createdAt = null;
	...
	// Atomic write through the mount so a reader never sees a half-written note. Bounded so a wedged mount fails
	// fast; the write is atomic (temp + rename), so a timeout leaves either the old note or the new one, never a
	// partial file. fsync so the note is DURABLE the moment it is saved rather than lingering in the mount's
	// write-back cache: a note can hold passwords, and every other important record in the vault is fsync'd the same
	// way. Without it, a note saved shortly before the vault is unmounted (or the process exits) could be lost —
	// which also broke a key rotation right after an edit, since the rotation re-encrypts what has actually reached
	// the store. The fsync is bounded by the timeout above, so a wedged mount still fails fast instead of hanging.
	await Common.withTimeout(Common.writeJsonAtomic(path.join(dir, theId + '.json'), rec, { fsync: true }), MOUNT_FS_TIMEOUT_MS);
	return { id: theId, type: noteType(type), createdAt, updatedAt: now };
}
async function noteDelete(vaultDir, id) {
	if (!isNoteId(id)) throw new Error('Unknown note.');
	const dir = notesDirFor(await noteMountpoint(vaultDir));
	await Common.withTimeout(fsp.rm(path.join(dir, id + '.json'), { force: true }), MOUNT_FS_TIMEOUT_MS); // bounded: the note lives inside the mount
	return { deleted: true };
}
...
async function notesHealth(vaultDir, { breach = false } = {}) {
	...
	let entries = [];
	try { entries = await Common.withTimeout(fsp.readdir(dir), MOUNT_FS_TIMEOUT_MS); } catch (_) { return empty; }
	...
	for (const f of entries) {
		if (!f.endsWith('.json')) continue;
		const j = await Common.withTimeout(Common.readJsonCorruptAside(path.join(dir, f)), MOUNT_FS_TIMEOUT_MS).catch(() => null);
