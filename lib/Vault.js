...
async function emergencyRemoveContact(contactId) {
	...
	return { removed: true, droppedGrants: dropped };
}
...
async function notesList(vaultDir) {
	...
	for (const f of entries) {
		...
		let meta = null;
		if (nck) { try { meta = openNoteMeta(j.t, nck); } catch (_) { meta = null; } }
		notes.push({ id: j.id, title: meta ? (meta.title || '(untitled)') : (nck ? '(unreadable)' : '(locked)'), type: meta ? meta.type : 'note', createdAt: meta && meta.createdAt, updatedAt: meta && meta.updatedAt });
...
// NIST discourages nagging on password age, so there is deliberately no "old" category. OOM-safe and cooperative:
// it decrypts one item at a time (only their metadata + fields, never all at once) and yields across many items.
async function notesHealth(vaultDir, { breach = false } = {}) {
	const dir = notesDirFor(await noteMountpoint(vaultDir));
	const empty = { items: [], counts: { reused: 0, weak: 0, exposed: 0 }, scanned: 0, breachChecked: false };
	let entries = [];
	...
	const prefixCache = breach ? new Map() : null; // one HIBP request per distinct prefix across the whole scan
	for (const it of pwItems) {
		...
		for (const v of it.values) {
			...
			if (breach && !issues.exposed) { try { if (await PwnedCheck.count(v, { cache: prefixCache }) > 0) issues.exposed = true; } catch (_) {} }
		}
	...
	return { items, counts, scanned: pwItems.length, breachChecked: !!breach };
}
...
async function sealItemForSend(vaultDir, { id } = {}) {
	...
	const payload = JSON.stringify({ kind: 'item', title: note.title, type: note.type, fields: note.fields, note: note.note });
