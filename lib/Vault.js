...
async function unmountImpl(opts, entry, mountpoint) {
	...
	return { ok: !stillMounted, mountpoint, vault: entry && entry.vault ? path.resolve(entry.vault) : null, forced: !!opts.force, stuck: !!(opts.recover && stillMounted), recovered: !!(opts.recover && !stillMounted) };
}
...
// Best-effort — the ledger is local and off the data path, so a hiccup must never fail a check-in.
function noteEmergencyCheckIn() { try { return Integrity.noteCheckInAt(EMERGENCY_CHECKIN_KEY, Date.now()); } catch (_) { return Promise.resolve(); } }
...
function emergencyPhase(cfg, now, anchorMs = 0) {
	...
	if (Number.isFinite(anchorMs) && anchorMs > t) t = anchorMs; // anti-rollback: never accept a check-in earlier than the anchored latest
...
async function emergencyCheckIn() {
	return emergencyQueue(async () => {
		...
		await noteEmergencyCheckIn(); // anchor the check-in so a later restored-older settings file cannot move it backward
...
async function emergencyStatus() {
	...
	const t = (stored !== null && Number.isFinite(anchorMs) && anchorMs > stored) ? anchorMs : stored;
