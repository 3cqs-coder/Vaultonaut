...
async function contentSearch(vaultDir, { query } = {}) {
	...
	// Inflate off the event loop, and materialize the index STRING only when it is small enough to load and search on
	// the main thread cheaply (SEARCH_INLINE_MAX_BYTES). A larger index (a huge document set, or an attacker-chosen
	// index in a shared vault, up to 512 MiB) comes back with its byte length but WITHOUT the string, so the big
	// toString never runs on the loop and neither does MiniSearch's search below — the query is handed to a worker.
	...
	if (!env.indexJSON) {
		// LARGE index: run the whole search off the event loop in a one-shot worker. It is not cached (only small
		// indexes are cached on the main thread), and the worker holds nothing after it returns, so a locked vault
		// still leaves no plaintext-derived index in memory. The worker reads the index itself; the heap cap matches
		// the indexing worker so it can hold and parse an index up to the ceiling.
		searchIndexCache.delete(indexPath);
		...
		catch (_) { return { query: q, results: [], noIndex: true }; } // a worker failure reads as "no index" (offer to reindex), never a hang
...
async function protectWork(abs, { tier, thorough, onProgress } = {}) {
	...
	if (!signPriv) {
		...
		if (floor > 0) return { vault: abs, protected: true, deferred: true, reason: 'signed-needs-unlock', message: 'This vault\'s self-healing data is signed. Updating it now would remove that signature (there is no unlocked session to re-sign it) and pause automatic repair until the vault is opened again. Open (unlock) this vault once, then update protection.' };
	}
