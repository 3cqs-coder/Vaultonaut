...
async function mountImplInner(vaultDir, opts = {}) {
	...
	if (keys) sessionKeys.set(res.mountpoint, { cfgText, keys, trusted: baselineTrusted, backing: abs, master, readOnly: readOnlyCred });
...
async function unmountImpl(opts, entry, mountpoint) {
	...
	return { ok: !stillMounted, mountpoint, vault: entry && entry.vault ? path.resolve(entry.vault) : null, redirected: !!(sk && sk.backing), forced: !!opts.force, stuck: !!(opts.recover && stillMounted), recovered: !!(lastResortRan && !stillMounted) };
}
