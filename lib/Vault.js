...
async function contentIndexStatus(vaultDir) {
	...
	try { const env = await SearchDefs.unpackEnvelopeAsync(await Common.withTimeout(Common.readFileCapped(indexPath, SearchDefs.MAX_SEARCH_INDEX_BYTES), MOUNT_FS_TIMEOUT_MS)); return { mounted: true, hasIndex: !!env, indexed: env ? env.fileCount : 0 }; } // bounded read (index is inside the mount); fileCount from the small meta
