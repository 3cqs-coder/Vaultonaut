...
async function vaultSize(vaultDir) {
	const abs = resolveVaultDir(vaultDir);
	await readManifest(abs); // confirm this is a real vault (throws otherwise)
	...
	// Count only the user's own files. The self-healing recovery data (.recovery/) already sits outside the mounted
	// store, so it is not counted; the vault's own small bookkeeping objects (the tamper snapshot, the canary, the
	// session marker) DO live in the store, so subtract them here.
...
// Open a path in the platform's file manager (best-effort; used by the UI's
// "Reveal" button). Never throws into the caller.
async function reveal(target) {
	// On Windows a bare drive-letter mountpoint ("X:") is drive-RELATIVE, so explorer silently opens nothing —
	// normalize it to the drive root ("X:\") first. normalizeMountpoint is a no-op for ordinary paths and on other
	// platforms, and it is the SAME normalizer the mount path uses, so a drive letter is treated identically here.
	const mp = normalizeMountpoint(String(target || ''));
	const opener = process.platform === 'darwin' ? 'open' : process.platform === 'win32' ? 'explorer' : 'xdg-open';
	// A missing opener (e.g. xdg-open absent on a minimal Linux install) makes spawn
	// throw; report that so the UI can show the path instead of silently doing nothing.
	// explorer exits non-zero even on success, so only a spawn failure counts as an error.
	try { await Rclone.exec(opener, [mp]); return { ok: true }; }
	catch (_) { return { ok: false, error: 'No file manager is available to open ' + mp }; }
}
