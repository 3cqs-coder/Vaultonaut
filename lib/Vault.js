...
async function savePeer(peer = {}) {
	...
	// here. It lets the client later upgrade a relayed sync to a direct one; it holds no registration power.
	const sig = peer.sig !== undefined ? sanitizeSig(peer.sig) : undefined;
	await mutateSettings(cur => {
		const peers = { ...(cur.peers || {}) };
		const prev = peers[id] || {};
		peers[id] = { label, url, user, password: (password != null && password !== '') ? encCred(password, key) : (prev.password || ''), ca: ca !== undefined ? ca : (prev.ca || ''), cand: cand !== undefined ? cand : (prev.cand || []), sig: sig !== undefined ? sig : (prev.sig || undefined) };
		return { ...cur, peers };
	});
	return { id };
}
...
async function preferReachableUrl(peer, opts = {}) {
	...
	if (sig && sig.hub && sig.node && sig.cred && !punchOnCooldown(sig.node)) {
		try {
			...
			const direct = await HolePunch.connectDirect({ hubHost: host, hubPort: port, credential: sig.cred, nodeId: sig.node, budgetMs: 5000 });
