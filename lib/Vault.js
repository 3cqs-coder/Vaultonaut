...
async function syncMirror(vaultDir, { onProgress, prime } = {}) {
	...
	return withVaultBusy(abs, 'This vault is busy with another operation (a mount or a mirror sync). Wait for it to finish, then try again.', async () => {
		...
		try {
			...
			const r = await Sync.runBisync(bin, { path1: abs, path2, configPath, workdir, resync: needsPrime, onProgress, caPath, backupDir1, backupDir2, primeArchive2, bwlimit: remoteDest ? bwlimitOf(await getSettings()) : '' });
			if (!r.ok) {
...
async function buildWebdavConfig(bin, d) {
	...
	// A .onion peer is reached over an anonymizing overlay: route this transfer through the local SOCKS proxy (never
	// globally — only this run). If no Tor SOCKS is up, proxyEnv is null and the transfer fails with a clear error.
	let proxyEnv = null;
	if (Onion.isOnionUrl(d.url)) { const su = await Onion.socksUrl(); if (su) proxyEnv = { ALL_PROXY: su, HTTPS_PROXY: su, HTTP_PROXY: su, all_proxy: su }; }
...
async function serveDirTransport(abs, { user, pass, bind, port, readOnly, onLine, onEvent, relay, lan, wan, onion, isVault } = {}) {
	...
	if (onion && !(relay && relay.host)) {
		...
		let onionSvc;
		try { onionSvc = await Onion.publishOnion(served0.port, { virtualPort: 80 }); }
