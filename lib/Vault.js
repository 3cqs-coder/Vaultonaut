...
async function reveal(target) {
	...
	// The OS file manager per platform, given the path as a single argv entry so a space in it needs no quoting.
	const [cmd, args] = process.platform === 'darwin' ? ['open', [mp]]
		: process.platform === 'win32' ? ['explorer', [mp]]
		: ['xdg-open', [mp]];
