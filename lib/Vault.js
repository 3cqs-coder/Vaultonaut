...
async function importFolder(vaultDir, { password, sourceDir, level } = {}) {
	...
	assertKdfLevel(level);
	Common.assertPortableVaultName(stripVaultExt(path.basename(resolveVaultDir(vaultDir)))); // reject a name that would not port to Windows, so the new vault opens on every platform
...
async function mobilePrepare(vaultDir, { password, expiryDays, record = true } = {}) {
	...
	let notesKey = null;
	try { notesKey = unwrapNotesKey(manifest, JSON.parse(Buffer.from(web.token.split('.')[1], 'base64url').toString('utf8')).key); }
	catch (_) { notesKey = null; }
	return {
...
async function emergencyArm(vaultDir, { password, contactId, mode, dateMs } = {}) {
	...
	const rec = { path: abs, name: displayName(abs), sealed, round, condition, at: new Date().toISOString(), contactId: cid };
...
async function backupScheduleTick() {
	return runScheduleTick({
		...
		onError: async (abs, now, e) => {
			...
			try { require('./Notify').emit({ type: 'backup-failed', label: path.basename(String(abs)).replace(/\.vault$/i, ''), message: 'A scheduled backup failed: ' + (e.message || 'unknown error') }, await getSettings()).catch(() => {}); } catch (_) {}
		},
	});
}
