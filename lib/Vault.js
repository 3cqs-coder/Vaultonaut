...
function runWatchedReindex(mountpoint) {
	...
	fsp.access(searchIndexPathFor(mountpoint))
		.then(() => contentReindex(rec.vault, { allowOcr: true }).catch(() => {}), () => {})
		.finally(() => {
			const r = indexWatchers.get(mountpoint);
			if (!r) return; // the mount was unmounted while indexing — nothing more to do
			r.running = false;
			if (r.dirty) { r.dirty = false; runWatchedReindex(mountpoint); } // changes arrived mid-run: index them now
		});
}
...
// `allowOcr` opts a reindex into OCR — passed by the explicit user-triggered reindex (the CLI `reindex` and the web
// "Update index") AND by the automatic refresh on mount, so a user who has turned OCR on gets newly added scans read
// automatically without pressing a button. It stays safe on the mount path because that refresh is detached (never
// delays the mount) and indexing is incremental (only new/changed files are read). Whether OCR actually runs is still
// gated on the setting being on (ocrOptions()); with OCR off, allowOcr changes nothing.
async function contentReindex(vaultDir, { onProgress, allowOcr = false } = {}) {
	...
	try {
		...
		// automatically, with no search or manual step and no client involvement.
		try { startIndexWatch(mountpoint, abs, false); } catch (_) {}
		return result;
