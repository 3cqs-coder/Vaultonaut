...
async function resolveMirrorTarget(bin, destKey, name) {
	...
	if (destKey.startsWith('webdav:')) {
		...
		return { path2: remote, configPath: cfg, cleanup: cleanupAll, localDir: null, caPath };
	}
...
async function serveDirTransport(abs, { user, pass, bind, port, readOnly, onLine, onEvent, relay, lan, wan, isVault } = {}) {
	const bin = await ensureEngine();
...
async function receiveVault(sourceKey, { toRoot, name, overwrite } = {}) {
	...
	return withVaultBusy(dest, 'The destination vault is busy with another operation. Wait for it to finish, then receive again.', async () => {
		...
		try {
			...
			const copy = await Rclone.run(bin, ['copy', target.path2, dest, '--transfers', '8', '--checkers', '8', ...caArgs(target.caPath)], { configPath: target.configPath, timeoutMs: 12 * 60 * 60 * 1000 });
			...
			const chk = await Rclone.run(bin, ['check', target.path2, dest, '--one-way', ...caArgs(target.caPath)], { configPath: target.configPath, timeoutMs: 60 * 60 * 1000 });
