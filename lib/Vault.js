...
// or reflexive public address the node put in its connect code, raced with Happy-Eyeballs pacing and used only if
// one is found reachable; else (3) the stored address (relay fallback), returning the peer unchanged. Best-effort
// and bounded.
async function preferReachableUrl(peer) {
...
async function serveVault(vaultDir, { bind, port, readOnly, onLine, onEvent, relay, lan, wan } = {}) {
	...
	const registration = Relay.registerNode({ hubHost: relay.host, hubPort: clampInt(relay.port, 1, 65535, Common.DEFAULT_RELAY_PORT), token: relay.token, nodeId: nid, nodeAuth, localPort: served.port, onEvent });
	try {
