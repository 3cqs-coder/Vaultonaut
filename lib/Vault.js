...
// Dispersal's Reed-Solomon encode/decode runs in a worker that buffers the WHOLE packed archive in memory and builds
// all the shard buffers from it — roughly two to three times the archive at peak. On a very large vault that would
// OOM-crash the worker mid-operation. Refuse UPFRONT with a clear, recoverable message when the archive is too large
// to hold in RAM safely on this machine, mirroring assertStagingSpace's upfront disk check (streaming the codec would
// lift this ceiling, but that is a larger change). Uses total RAM as the stable reference, not the fluctuating free
// figure. Best-effort: if memory cannot be read, skip the check and let the operation proceed as before.
function assertDispersalMemory(bytes, action) {
	try {
		const total = os.totalmem();
		if (total > 0 && bytes > 0 && bytes * 3 > total * 0.7) {
			throw new Error('This vault is too large to ' + action + ' on this computer: doing so needs to hold about ' + Math.ceil(bytes / 1e9 * 3) + ' GB in memory at once, more than is safely available here. Use a computer with more memory.');
		}
	} catch (e) {
...
async function disperse(vaultDir, { n, k, dests, force, onProgress } = {}) {
	...
	assertDispersalMemory(treeBytes, 'split into shards'); // refuse upfront if the archive is too large to encode in RAM, rather than OOM-crash the worker
...
async function reconstructFromShards(shardPaths, destDir, { onProgress } = {}) {
	...
	assertDispersalMemory(shardBytes, 'rebuild from shards'); // refuse upfront if the archive is too large to decode in RAM, rather than OOM-crash the worker
