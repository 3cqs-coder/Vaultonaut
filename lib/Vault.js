...
async function serveVault(vaultDir, { bind, port, readOnly, onLine, onEvent, relay, lan, wan } = {}) {
	...
	let punchAnswers = 0;
	const MAX_PUNCH_ANSWERS = 4;
	const onSignal = certInfo ? (({ blob, reply }) => {
		if (punchAnswers >= MAX_PUNCH_ANSWERS) return; // shed load rather than fan out unbounded punches
		punchAnswers++;
		Promise.resolve(HolePunch.answerSignal({ hubHost: relay.host, hubPort: hubPortForNode, credential: relay.token, nodeId: nid, offerBlob: blob, reply, localServePort: served.port }))
