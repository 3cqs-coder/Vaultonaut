...
async function runScrub(vaultDir, { autoHeal, onProgress } = {}) {
	...
	return withVaultBusy(abs, RECOVERY_BUSY, async () => {
		...
		if (!clean && autoHeal) { const h = await healWork(abs, { preserveInPlaceEdits: true, onProgress }); healed = true; repaired = (h && h.healed) || 0; deferredEdits = (h && h.inPlaceDeferred) || 0; unrecoverable = (h && h.unrecoverable) || 0; }
