...
async function runScrub(vaultDir, { autoHeal, onProgress } = {}) {
	...
	return withVaultBusy(abs, RECOVERY_BUSY, async () => {
		...
		let repaired = 0, healed = false, deferredEdits = 0;
		...
		if (!clean && autoHeal) { const h = await healWork(abs, { preserveInPlaceEdits: true, onProgress }); healed = true; repaired = (h && h.healed) || 0; deferredEdits = (h && h.inPlaceDeferred) || 0; }
		return { vault: abs, checked: true, clean, healed, repaired, deferredEdits };
	});
}
async function scrubScheduleTick() {
	return runScheduleTick({
		...
		handle: async (abs, sched, now) => {
			...
			const result = r.skipped ? ('skipped: ' + r.skipped) : r.clean ? 'ok' : (r.healed ? ('repaired ' + r.repaired + deferredNote) : 'damage found (auto-repair off)');
