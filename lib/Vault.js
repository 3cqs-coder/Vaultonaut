...
async function emergencyRelease(cfg, now) {
	...
	try {
		await Common.withTimeout((async () => {
			...
			await fsp.writeFile(path.join(dir, 'HOW-TO-OPEN.txt'), emergencyReadme());
		})(), MOUNT_FS_TIMEOUT_MS);
	} catch (e) {
