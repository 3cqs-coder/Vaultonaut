...
async function listShares(vaultDir) {
	...
	// Refuse forward ONLY for a genuinely newer roster: the version field is part of the signed input, so a roster
	// that is BOTH newer-versioned AND validly signed was written by a newer build and cannot be verified under this
	// build's rules — report unknownFormat (not a tamper). A FORGED version bump does not verify, so it falls through
	// to the tamper path below (sigOk false). Ordering the signature check first is what keeps forgery detectable.
	if (sigOk && sidecarNewer(store, SHARES_VERSION)) return { vault: abs, shares: [], sigOk: true, rolledBack: false, unknownFormat: true };
...
async function verifySuccession(vaultDir) {
	...
	// Refuse forward: a newer-format succession chain would recompute its links under rules this build does not know
	// and read as broken. Report it as a newer format (ok true, not a break) instead of a false verification failure.
	if (sidecarNewer(store, SUCCESSION_VERSION)) return { vault: abs, ok: true, chain: [], current: null, unknownFormat: true };
...
async function attestations(vaultDir) {
	...
	const store = await readAttestations(abs);
	// Refuse forward: a newer-format attestation log would have its hash chain recomputed under rules this build does
	// not know and read as broken. Report it as a newer format (chainOk true, not a break) instead of a false alarm.
	if (sidecarNewer(store, ATTEST_VERSION)) return { vault: abs, identity: curIdentity, currentRoot: curRoot || null, currentSeq: (curSeq == null ? null : curSeq), items: [], chainOk: true, head: null, maxAttestedSeq: null, rolledBack: false, unknownFormat: true };
...
async function verifyBundle(bundleDir, { expectIdentity } = {}) {
	...
	// Refuse forward, exactly like readSnapshot: a bundle whose baseline was written by a NEWER version binds fields
	// this verifier does not know into its signing input, so the signature check below would fail and wrongly brand a
	// legitimate newer bundle as TAMPERED. Report UNVERIFIED ("made by a newer version — update to verify") instead.
	if ((typeof baseline.version === 'number' && baseline.version > BASELINE_VERSION) || (baseline.scheme && baseline.scheme !== Integrity.SCHEME)) {
		add('bundle-format', false, 'This bundle was made by a newer version of ' + Brand.name + ' — update to verify it.');
		return { verdict: 'UNVERIFIED', checks, identity: Integrity.pubkeyOf(manifest) ? Integrity.identity(Integrity.pubkeyOf(manifest)) : null };
	}
