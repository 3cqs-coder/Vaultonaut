...
async function assertBackupDeletionsSafe(bin, source, dest, { configPath, caPath } = {}) {
	...
	if (destCount === 0) return { verified: true }; // nothing at the destination yet — a first backup deletes nothing, no risk to guard
	...
	return { verified: true }; // both sides listed and compared, the deletion count is within bounds — the sync needs no extra backstop
}
