		...
		for (const oldAbs of known) {
			...
			await mutateSettings(cur => {
				...
				for (const store of VAULT_PATH_KEYED) {
					...
					next[store] = { ...s };
					if (next[store][newAbs] === undefined) next[store][newAbs] = next[store][oldAbs]; // never clobber a setting already at the new path
...
module.exports = {
	...
	readManifest, readManifestForPoll, checkManifestSeal, updateManifestFields, displayName, vaultTitle, vaultIdOf, cachedVaultName, getVaultMeta, setVaultMeta, defaultMountRoot, normalizeMountpoint, resolveVaultDir, assertReadable, checkOnMount, isIgnoredVaultPath, INTERNAL_VAULT_OBJECTS, OS_JUNK_COMMON, RECOVERY_EXCLUDE_NAMES,
