...
async function savePeer(peer = {}) {
	...
	await mutateSettings(cur => {
		...
		const prev = peers[id] || {};
		peers[id] = { label, url, user, password: (password != null && password !== '') ? encCred(password, key) : (prev.password || ''), ca: ca !== undefined ? ca : (prev.ca || ''), cand: cand !== undefined ? cand : (prev.cand || []) };
		return { ...cur, peers };
	});
	return { id };
}
...
// It is additive and ignored by a 1.0.0 client (which reads only url/user/pass/ca), so older codes/clients still work.
function makePeerCode({ url, user, pass, ca, cand } = {}) { return Buffer.from(JSON.stringify({ v: 1, url, user, pass, ca: ca || undefined, cand: (Array.isArray(cand) && cand.length) ? cand : undefined })).toString('base64'); }
function parsePeerCode(code) { const s = String(code || '').trim(); if (s.length > 8192) return null; try { const o = JSON.parse(Buffer.from(s, 'base64').toString('utf8')); if (o && o.url) return { url: o.url, user: o.user || 'vd', password: o.pass || '', ca: o.ca || '', cand: sanitizeCandidates(o.cand) }; } catch (_) {} return null; } // cap the pasted connect code before decoding: a real one is a few hundred bytes (address + login + cert), so a local bound makes it independent of any caller's own limit
