...
async function serveVault(vaultDir, { bind, port, readOnly, onLine, onEvent, relay, lan } = {}) {
	const abs = resolveVaultDir(vaultDir);
	...
	// LAN direct mode: bind every interface so a peer on the same network reaches it, and serve over TLS with a cert
	// that covers this machine's LAN IPs (so hostname verification passes and the writable-share downgrade below does
	// not force read-only). A relay request always wins over lan.
	const lanMode = !!lan && !(relay && relay.host);
	// With a relay the serve stays on loopback and the hub exposes it; a LAN serve binds all interfaces; otherwise
	// it binds where asked.
	const host = relay ? '127.0.0.1' : (lanMode ? '0.0.0.0' : (bind || '127.0.0.1'));
	// For a relay OR a LAN serve, encrypt the hop end to end: the node serves over TLS with a self-signed cert the
	// client pins, so the hub/network only ever splice opaque bytes. Certificate generation is pure Node
	// (cross-platform, no external tool), so this normally always succeeds; it fails soft to plain HTTP only if a
	// certificate could not be produced at all.
	const certInfo = (relay && relay.host) ? await Cert.ensureCert(relay.host) : (lanMode ? await Cert.ensureLanCert() : null);
	...
	if (lanMode) {
		...
		const stop = async () => { try { if (disc) disc.stop(); } catch (_) {} try { await served.stop(); } catch (_) {} };
		// A sensible default address for the connect code (the first LAN IPv4); discovery keeps it current afterwards.
		// Every LAN IPv4 is advertised as a direct-connect candidate, so a peer on a routable-but-non-broadcast subnet
		// (where discovery's broadcast does not reach) can still probe and reach it directly — the cert covers them all.
		const ips = Cert.lanIPv4s();
		const candidates = ips.map((ip) => ({ host: ip, port: served.port }));
		const lanUrl = (certInfo ? 'https://' : 'http://') + (ips[0] || '127.0.0.1') + ':' + served.port + '/';
		return { ...served, url: lanUrl, ca: certInfo ? certInfo.certPem : null, secure: !!certInfo, lan: true, nodeId: nid, candidates, stop, user, pass, isVault };
	}
