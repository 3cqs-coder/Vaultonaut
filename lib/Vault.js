...
async function disperse(vaultDir, { n, k, dests, force, onProgress } = {}) {
	...
	assertDispersalMemory(treeBytes * 3, 'split into shards'); // encode peak ~= the archive plus all the shards built from it; refuse upfront rather than OOM-crash the worker
...
async function reconstructFromShards(shardPaths, destDir, { onProgress } = {}) {
	...
	assertDispersalMemory(shardBytes * 2, 'rebuild from shards'); // decode peak ~= the shards in memory plus the rebuilt archive (which is no larger than the shards); ~2x the shard bytes, so do not over-count and refuse a recoverable vault
