...
async function emergencyArm(vaultDir, { password, contactId } = {}) {
	...
	const sealed = Emergency.seal(contact.pubKey, cap.token);
	const vaultId = (manifest.integrity || {}).pubkey || abs;
	const rec = { path: abs, name: displayName(abs), sealed, at: new Date().toISOString(), contactId: cid };
	await mutateSettings(cur => ({ ...cur, emergency: { ...cur.emergency, armed: { ...(cur.emergency.armed || {}), [vaultId]: rec } } }));
	return { armed: true, vault: abs, contactId: cid, contactLabel: contact.label };
}
...
async function emergencyStatus() {
	...
	return {
		...
		armed: Object.values(cfg.armed || {}).map(a => { const id = grantContactId(a); return { name: a.name, at: a.at, contactId: id, contactLabel: (contacts[id] && contacts[id].label) || 'a former beneficiary' }; }),
	};
}
