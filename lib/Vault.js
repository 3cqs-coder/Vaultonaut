...
async function notesHealth(vaultDir, { breach = false } = {}) {
	...
	for (const it of pwItems) {
		...
		for (const v of it.values) {
			...
			if (breach && !issues.exposed) { try { if (await PwnedCheck.count(v, { cache: prefixCache, onFail: () => { breachFailed = true; } }) > 0) issues.exposed = true; } catch (_) { breachFailed = true; } }
		}
...
async function sealItemForSend(vaultDir, { id } = {}) {
	const note = await noteGet(vaultDir, id); // decrypts the item using the open vault's session key
	...
	return { ciphertext, key: key.toString('base64url'), meta: { kind: 'item' } };
}
...
module.exports = {
	...
	emergencyKeypair, emergencyEnroll, emergencyAddContact, emergencyRemoveContact, emergencyArm, emergencyCheckIn, emergencyStatus, emergencyTick, emergencyNoteClockDrift, emergencyOpen, emergencyDisarm,
