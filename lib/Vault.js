...
async function maybeSendCheckinReminder(cfg, now) {
	...
	// Record the attempt (delivered OR failed) once per window, so a flaky endpoint cannot cause a reminder storm; a
	// pure skip (notifications off or no channel yet) leaves it re-armable so enabling a channel still reminds.
	if (r && (r.delivered || r.error)) await mutateSettings(cur => (cur.emergency ? { ...cur, emergency: { ...cur.emergency, reminderSentFor: key } } : cur)).catch(() => {});
}
async function emergencyTick(now = Date.now()) {
	...
	try {
		return await emergencyQueue(async () => {
			...
			await maybeSendCheckinReminder(cfg, now).catch(() => {}); // opt-in, best-effort heads-up before release; never affects the release decision
