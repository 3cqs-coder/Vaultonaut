...
async function contentIndexStatus(vaultDir) {
	...
	try { const env = await SearchDefs.unpackEnvelopeAsync(await Common.readFileCapped(indexPath, SearchDefs.MAX_SEARCH_INDEX_BYTES)); return { mounted: true, hasIndex: !!env, indexed: env ? env.fileCount : 0 }; } // fileCount comes from the small meta — no need to parse the whole change manifest just to count files
