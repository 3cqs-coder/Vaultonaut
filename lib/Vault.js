...
// Files whose encrypted CONTENT could not be read during a `hashsum --download` scan — a damaged or corrupt blob (a
// "bad magic" header, a failed block authentication, a truncated file). Unlike foreignCipherNames (undecryptable
// NAMES), these have a VALID decrypted name but unreadable content, so the engine prints a per-file error line and
// exits non-zero while STILL hashing every OTHER file. Parsing them lets the scan report the damaged files and hash
// the rest, instead of aborting the whole check on one bad blob. Every per-file failure is "ERROR : <path>: failed to
// <open file … | copy file to hasher: failed to authenticate …>", so anchor on ": failed to" (the message ALWAYS
// begins there) and capture the path non-greedily up to it. That keeps a path containing ": " whole and stops before
// the message even when the message itself repeats ": " (the block-auth form). The caller cross-references each result
// against the real vault listing (the `sizes` map), so a stray misparse is dropped rather than shown.
...
// True only when EVERY engine ERROR line is one of those per-file content failures, so a non-zero exit is FULLY
// explained by damaged files and the rest of the scan is trustworthy. If any ERROR line is something else (a fatal or
// global failure), the scan must still abort rather than treat a broken run as clean-but-for-N-files.
function hashsumErrorsAllPerFile(stderr) {
	const s = String(stderr || '');
	const errs = s.split(/\r?\n/).filter(l => / ERROR : /.test(l));
	// Two conditions must BOTH hold to trust that a non-zero exit is only per-file damage: every ERROR line has the
	// per-file "<path>: failed to …" shape, AND the engine printed its end-of-run summary, which comes LAST. rclone
	// prints "Failed to hashsum: …" for a single error and "Failed to hashsum with N errors" for two or more, so accept
	// BOTH forms. If the stderr buffer was truncated (a vault with tens of thousands of damaged files fills the cap and
	// the tail summary is dropped) or a FATAL/global error replaced the normal completion, the summary is absent — so
	// we fall back to aborting rather than treating a possibly-truncated or fatal run as clean-but-for-N. Fail-closed.
	const completed = /Failed to hashsum(:| with \d+ error)/i.test(s);
	return completed && errs.length > 0 && errs.every(l => / ERROR : .+?: failed to /i.test(l));
}
// A self-explanatory finding for files present with a valid name but whose encrypted CONTENT is damaged/unreadable
// (distinct from a foreign file, whose NAME does not decrypt). Names a few, capped, and points at the real fix —
// restoring them — because self-healing cannot rebuild a loss larger than its recovery budget.
function damagedNote(names) {
	const n = names.length;
	const shown = names.slice(0, 5).join(', ');
	const more = n > 5 ? ', and ' + (n - 5) + ' more' : '';
	return n + ' file' + (n === 1 ? '' : 's') + ' could not be read — present in the vault, but the encrypted content is '
		+ 'damaged (' + shown + more + '). If self-healing could not fully rebuild them, restore them from a backup or a '
		+ 'saved version.';
}
...
async function writeSnapshot(bin, cfg, abs, manifest, keys, { deep, auto, sealed, timeoutMs }) {
	...
	// message (repair or restore first). The AUTO baseline (unmount) is structural, never deep, so it is unaffected;
	// gating on `!auto` also guarantees the never-wedge unmount path can never be blocked by this check.
...
async function audit(vaultDir, { password, deep = true } = {}) {
	...
	return withVaultConfig(bin, abs, password, manifest, cred.master, async (cfg) => {
		...
		if (report.foreign.length) report.tamper.push(foreignNote(report.foreign));
		// Files present with a valid name but unreadable encrypted content — a damaged/corrupt blob, or the residue of
		// a loss too large for self-healing to fully rebuild. Invisible to the size-based diff above (their size is
		// unchanged), so surface them explicitly, exactly like foreign files.
		report.damaged = cur.damaged || [];
		if (report.damaged.length) report.tamper.push(damagedNote(report.damaged));
...
module.exports._hashsumParse = { hashsumDamagedFiles, hashsumErrorsAllPerFile, damagedNote }; // test-only: the tamper-scan stderr parser that decides report-damaged vs abort
