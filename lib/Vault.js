...
function buildBackstop(cfg, token) {
	...
	const shares = Shamir.split(Buffer.from(String(token), 'utf8'), n, k);
	return { v: 1, k, n, createdAt: new Date().toISOString(),
		shares: trustees.map((t, i) => ({ label: t.label || ('Trustee ' + (i + 1)), fp: memberFingerprint(t.pubKey), sealed: Emergency.seal(String(t.pubKey).trim(), shares[i], SEAL_CTX.backstop) })) };
}
