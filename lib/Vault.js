...
async function backupScheduleTick() {
	return runScheduleTick({
		...
		onError: async (abs, now, e) => {
			...
			try { require('./Notify').emit({ type: 'backup-failed', label: await friendlyName(abs), message: 'A scheduled backup failed: ' + (e.message || 'unknown error') }, await getSettings()).catch(() => {}); } catch (_) {}
		},
	});
}
