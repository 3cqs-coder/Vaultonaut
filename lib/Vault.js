...
async function runScrub(vaultDir, { autoHeal, onProgress } = {}) {
	...
	return withVaultBusy(abs, RECOVERY_BUSY, async () => {
		...
		let repaired = 0, healed = false, deferredEdits = 0, unrecoverable = 0;
		...
		if (!clean && autoHeal) { const h = await healWork(abs, { preserveInPlaceEdits: true, onProgress }); healed = true; repaired = (h && h.healed) || 0; deferredEdits = ((h && h.inPlaceDeferred) || 0) + ((h && h.changedSkipped) || 0); unrecoverable = (h && h.unrecoverable) || 0; }
		return { vault: abs, checked: true, clean, healed, repaired, deferredEdits, unrecoverable };
	});
}
async function scrubScheduleTick() {
	return runScheduleTick({
		...
		handle: async (abs, sched, now) => {
			...
			const result = r.skipped ? ('skipped: ' + r.skipped) : r.clean ? 'ok' : (r.healed ? ('repaired ' + r.repaired + deferredNote + lostNote) : 'damage found (auto-repair off)');
			// A damage-found-but-not-repaired outcome is recorded as an error so the back-off applies and it stands out.
			// A deferred possible-edit is flagged the same way so it surfaces and backs off, rather than reading as a clean "ok".
			await patchScrubSchedule(abs, { lastRunAt: now.toISOString(), lastResult: result, lastErrorAt: (r.checked && !r.clean && (!r.healed || r.deferredEdits || r.unrecoverable)) ? now.toISOString() : null });
			return { abs, result };
		},
