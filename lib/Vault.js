...
async function emergencyArm(vaultDir, { password, contactId, mode, dateMs } = {}) {
	...
	return { armed: true, vault: abs, contactId: cid, contactLabel: contact.label, mode: condition.mode, round, opensAt: new Date(Timelock().timeOfRoundMs(round)).toISOString() };
}
...
async function emergencyStatus() {
	...
	return {
		...
		armed: Object.values(cfg.armed || {}).map(a => { const id = grantContactId(a); const mode = (a.condition && a.condition.mode) || 'inactivity'; return { name: a.name, at: a.at, contactId: id, contactLabel: (contacts[id] && contacts[id].label) || 'a former beneficiary', mode, opensAt: a.round ? new Date(Timelock().timeOfRoundMs(a.round)).toISOString() : null }; }),
	};
}
