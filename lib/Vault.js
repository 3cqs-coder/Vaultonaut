...
async function unwrapMasterOneForm(secret, crypt) {
	...
	for (const slot of keySlotsOf(crypt).filter(s => s.kind !== 'member').slice(0, 64)) {
		let wrapKey;
...
async function unlockByMemberKey(memberPrivB64, manifest) {
	...
	for (const slot of keySlotsOf((manifest && manifest.crypt) || {}).filter(s => s.kind === 'member').slice(0, 256)) {
