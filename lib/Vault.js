...
// A quick TCP-reachability probe of a candidate direct address, so the connecting side can prefer a reachable direct
// hop over the relay. Resolves true/false; never throws, never hangs (unref'd timeout), and only ever opens a TCP
// connection (a candidate is never used as a shell/arg). The socket is torn down immediately either way.
function probeReachable(host, port, timeoutMs = 1500) {
	return new Promise((resolve) => {
		let done = false;
		const s = net.connect(port, host);
		const finish = (v) => { if (done) return; done = true; clearTimeout(t); try { s.destroy(); } catch (_) {} resolve(v); };
		const t = setTimeout(() => finish(false), timeoutMs); if (t.unref) t.unref();
...
// or reflexive public address the node put in its connect code, used only if a quick probe finds it reachable; else
// (3) the stored address (relay fallback), returning the peer unchanged. Fully best-effort and bounded.
async function preferReachableUrl(peer) {
	...
	const cands = Array.isArray(peer && peer.cand) ? peer.cand : [];
	for (const c of cands) {
		if (!c || typeof c.host !== 'string' || !Number.isInteger(c.port)) continue;
		try { if (await probeReachable(c.host, c.port, 1500)) return { ...peer, url: 'https://' + c.host + ':' + c.port + '/', viaDirect: true }; } catch (_) {}
	}
	return peer;
}
