...
async function sealItemForSend(vaultDir, { id } = {}) {
	const abs = resolveVaultDir(vaultDir);
	...
	const ciphertext = Kdf.wrapSecret(payload, key); // base64 of iv|ciphertext|tag — the layout the recipient's Web Crypto expects
	return { ciphertext, key: key.toString('base64url'), meta: { kind: 'item' }, vault: abs };
}
