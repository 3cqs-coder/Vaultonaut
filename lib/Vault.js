...
// `allowOcr` is passed only by an EXPLICIT, user-triggered reindex (the CLI `reindex` and the web "Update index"),
// never by the automatic refresh on mount — OCR is expensive, so it must never slow down opening a vault. When
// allowed, it is still gated on the OCR setting being on (ocrOptions()).
