...
async function noteRecoverySignedResult(r, vaultId, abs, signPriv) {
	...
	if (abs && signPriv) { try { await bumpManifestRecoveryVersion(abs, r.signedVersion, signPriv); } catch (_) {} }
}
...
async function bumpManifestRecoveryVersion(abs, version, signPriv) {
	...
	await updateManifestFields(abs, async (m) => {
		...
		const sig = Integrity.sign(signPriv, manifestSealInput(candidate));
		// Commit BOTH the new floor and its seal together, and ONLY if the seal verifies against the manifest's OWN
		// published key. If the in-hand signer is stale (a key rotation completed after this session captured it),
		...
		if (!manifestSealVerifies(candidate, m.integrity.pubkey, sig)) return;
...
async function protectWork(abs, { tier, thorough, onProgress } = {}) {
	...
	await noteRecoverySignedResult(r, vaultId, abs, signPriv);
	return r;
}
	...
	try {
		...
		if (!stale && !excludeStale) {
			...
			if (signPriv && vaultId != null) {
				try { const s = await Recovery.signIndex(abs, { signPriv, vaultId }); await noteRecoverySignedResult(s, vaultId, abs, signPriv); } catch (_) {}
			}
...
const SEAL_VERSION = 4; // the manifest-seal format this build writes and verifies; a newer one is refused forward (see checkManifestSeal)
...
function manifestSealInput(manifest) {
	...
	return ['vault-manifest-seal-v4', Integrity.SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', String(Number(manifest.recoveryVersion) || 0), slots.join(',')].join('\n');
}
...
function manifestSealVerifies(manifest, pub, sig) {
	try { return Integrity.verify(pub, manifestSealInput(manifest), sig); } catch (_) { return false; }
}
...
async function writeSnapshot(bin, cfg, abs, manifest, keys, { deep, auto, sealed, timeoutMs }) {
	...
	const fresh = await updateManifestFields(abs, (m) => {
		...
		try { m.integrity.manifestSig = Integrity.sign(keys.signPriv, manifestSealInput(m)); m.integrity.sealVersion = SEAL_VERSION; } catch (_) {} // sealVersion is a readable format marker so an older build refuse-forwards instead of false-alarming (see checkManifestSeal)
	});
