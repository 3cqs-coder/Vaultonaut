...
function sharesSigVerifiesVersioned(store, pub) {
	if (!(pub && store && store.sig)) return false;
	try { return Integrity.verify(pub, sharesSigInput(store), store.sig); } catch (_) { return false; }
}
...
async function writeSharesSigned(abs, store, writeSeed, vid) {
	...
	if (claimed > SHARES_VERSION) {
		...
		if (sharesSigVerifiesVersioned(store, pub)) throw new Error('This vault\'s share roster was written by a newer version of ' + Brand.name + ' (format v' + claimed + '; this build understands v' + SHARES_VERSION + '). Update ' + Brand.name + ' to change it.');
	}
...
async function listShares(vaultDir) {
	...
	if (sidecarNewer(store, SHARES_VERSION) && sharesSigVerifiesVersioned(store, pub)) return { vault: abs, shares: [], sigOk: true, rolledBack: false, unknownFormat: true };
...
async function protectWork(abs, { tier, thorough, onProgress } = {}) {
	...
	const r = await Recovery.protect(abs, { tier, thorough, cipherDir: cipherDirOf(abs), exclude: await recoveryExcludeFor(abs), signPriv, vaultId, manifestFloor: Number(manifest.recoveryVersion) || 0, onProgress });
	...
	try {
		...
		if (!stale && !excludeStale) {
			...
			if (signPriv && vaultId != null) {
				try { const s = await Recovery.signIndex(abs, { signPriv, vaultId }); await noteRecoverySignedResult(s, vaultId, abs, signPriv, signPqPriv); } catch (_) {}
			}
		...
		const r = await Recovery.protect(abs, { tier: idx.tier, thorough: idx.thorough, cipherDir, exclude, signPriv, vaultId, manifestFloor, onProgress });
...
async function recoveryAuthContext(abs) {
	try {
		...
		return { pubkey: Integrity.pubkeyOf(m), vaultId: Integrity.vaultId(m), recoveryVersion: Number(m.recoveryVersion) || 0, manifestSealBroken: !!(seal.sealed && !seal.ok && !seal.unknownFormat) };
	}
	catch (_) { return { pubkey: null, vaultId: null, recoveryVersion: 0, manifestSealBroken: false }; }
}
...
async function healWork(abs, { allowUnverified, force, preserveInPlaceEdits, onProgress } = {}) {
	...
	const res = await Recovery.heal(abs, { cipherDir: cipherDirOf(abs), pubkey: ctx.pubkey, vaultId: ctx.vaultId, expectedFloor: ctx.recoveryVersion, allowUnverified, force, preserveInPlaceEdits, onProgress });
	return annotateRecoveryDowngrade(res, ctx.vaultId, ctx.recoveryVersion, ctx.manifestSealBroken);
}
...
const ROSTER_SEAL_VERSION = 1; // the roster-seal format; folded into the signed input AND written as a readable marker so a newer roster is refused forward (UNVERIFIED), not falsely reported as TAMPERED (mirrors SEAL_VERSION)
...
function rosterSealInput(manifest) {
	...
	return ['vault-roster-seal-v' + ROSTER_SEAL_VERSION, Integrity.SCHEME, String(m.epoch || 0), String(m.keyGeneration || 0), String(m.ownerPubKey || ''), memberSlots.join(','), rec].join('\n');
}
