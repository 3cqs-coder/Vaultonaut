...
async function travelRestore({ travelPassword } = {}) {
	...
	if (blob.emergency) { await mutateSettings(cur => ({ ...cur, emergency: { ...blob.emergency, lastCheckIn: new Date().toISOString(), releasedAt: null } })); await noteEmergencyCheckIn(); }
	await Travel.clear();
...
//   'inactivity' (default): now + the inactivity + grace window. Re-arming (which re-derives the read cap) rolls it
//                           forward, so while the owner is active the grant never becomes openable.
//   'date':                 a fixed absolute moment (a scheduled unlock / time capsule).
//   'both':                 whichever comes first — released on inactivity OR by the date.
...
async function emergencyCheckIn() {
	return emergencyQueue(async () => {
		...
		try { await Common.withTimeout(fsp.rm(path.join(Common.dataDir(), EMERGENCY_RELEASE_DIR), { recursive: true, force: true }), MOUNT_FS_TIMEOUT_MS); } catch (_) {} // a check-in vetoes a release; bounded so a wedged data dir can't stall the veto
...
async function emergencyRelease(cfg, now) {
	...
	let vetoed = false;
	await mutateSettings(cur => {
		const cin = cur && cur.emergency && cur.emergency.lastCheckIn;
...
async function emergencyOpen(privateKey, sealed, { signature } = {}) {
	...
	if (signature) {
		...
		if (!T.isRoundDue(round)) throw new Error('This access is time-locked and cannot be opened until ' + new Date(T.timeOfRoundMs(round)).toISOString() + '. Try again then.');
		sig = await T.fetchSignature(round);
		if (!sig) throw new Error('This access is due to open, but the time-lock beacon could not be reached right now. Check the connection and try again.');
	}
...
async function emergencyDisarm() {
	return emergencyQueue(async () => {
		...
		try { await Common.withTimeout(fsp.rm(path.join(Common.dataDir(), EMERGENCY_RELEASE_DIR), { recursive: true, force: true }), MOUNT_FS_TIMEOUT_MS); } catch (_) {} // bounded so a wedged data dir can't stall a disarm // serialized with the release tick, so a disarm can't be overwritten by a release that read settings just before it
