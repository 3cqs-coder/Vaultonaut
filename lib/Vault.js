...
function openNoteMeta(sealed, nckB64) { const o = JSON.parse(openNoteField(sealed, nckB64)); return { title: String(o.title == null ? '' : o.title), createdAt: o.createdAt || null, updatedAt: o.updatedAt || null }; }
...
async function notesList(vaultDir) {
	...
	for (const f of entries) {
		...
		notes.push({ id: j.id, title: meta ? (meta.title || '(untitled)') : (nck ? '(unreadable)' : '(locked)'), createdAt: meta && meta.createdAt, updatedAt: meta && meta.updatedAt });
...
async function noteGet(vaultDir, id) {
	...
	const meta = j.t ? openNoteMeta(j.t, nck) : { title: '', createdAt: null, updatedAt: null };
	return { id: j.id, title: meta.title, body: j.b ? openNoteField(j.b, nck) : '', createdAt: meta.createdAt, updatedAt: meta.updatedAt };
}
const NOTE_VERSION = 2; // the note-record format: v2 encrypts the title and body. There is no released v1 to read.
// A note is a small text record. Cap its body well under the 16 MB cap the reader (noteGet/notesList) enforces, so a
// note can never be written that those capped reads would later reject or silently hide, and so encryption never has
// to hold a large plaintext in memory. The web route already caps its whole request body; this is a defensive limit.
const MAX_NOTE_BODY_BYTES = 1024 * 1024;
async function noteSave(vaultDir, { id, title, body } = {}) {
	if (body != null && Buffer.byteLength(String(body), 'utf8') > MAX_NOTE_BODY_BYTES) throw new Error('This note is too long to save (the limit is 1 MB of text). Shorten it, or store large content as a file in the vault instead.');
	...
	const rec = { v: NOTE_VERSION, id: theId, t: sealNoteMeta({ title: cleanTitle, createdAt, updatedAt: now }, nck), b: sealNoteField(String(body == null ? '' : body), nck) };
	...
	return { id: theId, createdAt, updatedAt: now };
}
