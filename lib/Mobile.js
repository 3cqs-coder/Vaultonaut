...
function sweep() {
	const now = Date.now();
	// A session keeps the plaintext bearer (_bearer) only until its pairing code is redeemed. If the code is never
	// redeemed it expires in 10 minutes while the session lives 12 hours, so the plaintext would otherwise linger in
	// memory for that whole window. Drop it once the pairing is gone, so an unredeemed session keeps only the hash.
	const livePairing = new Set([...pairings.values()].filter((v) => v.expiresAt > now).map((v) => v.sessionId));
	for (const [k, v] of pairings) if (v.expiresAt <= now) pairings.delete(k);
	for (const [k, v] of sessions) {
		if (v.expiresAt <= now) { sessions.delete(k); continue; }
		if (v._bearer && !livePairing.has(k)) delete v._bearer; // pairing expired without redemption — only the hash remains
	}
}
...
async function start(vaultDir, { password, ttlMs, local, useMountedKey } = {}) {
	sweep();
	...
	// The effective session lifetime, computed up front so a RECORDED grant carries a MATCHING expiry and self-clears
	// from the roster when the session ends. Without it the grant was stamped "never expires", so a phone session that
	// has since ended (or a process restart that dropped it entirely) left a "Mobile access" row that overstated live
	// access until the owner manually revoked it. A `local` session records nothing, so the expiry there is a harmless no-op.
	const ttlEffectiveMs = (Number(ttlMs) > 0 ? Number(ttlMs) : (local ? LOCAL_TTL_MS : DEFAULT_TTL_MS));
	const prep = await Vault.mobilePrepare(vaultDir, { password, record: !local, expiryDays: ttlEffectiveMs / 86400000, useMountedKey: !!useMountedKey });
	...
	const now = Date.now();
	const expiresAt = now + ttlEffectiveMs;
	sessions.set(sessionId, { vaultId: prep.vaultId, sid: prep.sid, cipherRoot: prep.cipherRoot, cipherRootReal, bearerHash: Common.sha256(String(bearer)), webCap: prep.webCap, name: prep.name, notesKey: prep.notesKey || null, notesDir: prep.notesDir || null, local: !!local, createdAt: now, expiresAt, _bearer: bearer });
...
function redeem(code) {
	...
	return { sessionId: hit.sessionId, bearer, cap: s.webCap, name: s.name, local: !!s.local, notesKey: s.notesKey || null, notesDir: s.notesDir || null, base: '/m/files/' + hit.sessionId + '/', list: '/m/list/' + hit.sessionId };
}
...
// unmounted/locked, so the server stops honoring that session: any further live fetch from a phone or in-app viewer
// gets a 401. NOTE this cannot reach data the phone already pulled — a phone that used "Save offline" holds the
// ciphertext in its own IndexedDB and the read key in page memory, so it keeps opening those saved files until its own
// Lock, its session TTL, or the viewer heartbeat (which calls forget() on the next 401) revokes them. An online phone
// therefore loses access within the heartbeat window; a phone that is genuinely offline keeps its saved copies until
// it reconnects. Path-based so it needs no manifest read.
function stopByVaultPath(vaultDir) {
	...
	try {
		...
		for (const [id, s] of sessions) {
			...
			// Case- and boundary-aware compare (Windows and macOS are case-insensitive): a differently-cased vault
			// path must still tear down its in-process viewer session on lock, so use the shared path helpers rather
			// than a raw === / startsWith, which would miss a case difference and leave the read key in memory.
			if (Common.samePath(root, target) || Common.pathWithin(root, target)) { if (stop(id)) n++; }
		}
	} catch (_) {}
	return n;
}
