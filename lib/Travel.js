...
async function restore(travelPassword) {
	...
	const found = SR.findSlot(K, slots, (o) => o.kind === 'travel'); // constant-work scan (no early break)
	return found ? (found.blob || {}) : null;
}

async function clear() { await reg.remove(); }

module.exports = { hasHidden, enable, restore, clear, registryPath, SLOT_COUNT };
