...
function main() {
	...
	// The visible label span carries an id and the input points at it with aria-labelledby, so the input's accessible
	// name is exactly the label — not polluted by the strength meter that also lives inside the <label> (which would
	// otherwise make the name mutate on every keystroke). Lock both the visible span and the programmatic link.
	ok('the autostart password field has a visible label linked by aria-labelledby', /<span id="autostartPassLabel">Web login password[^<]*<\/span>\s*<input[^>]*id="autostartPass"[^>]*aria-labelledby="autostartPassLabel"/.test(indexHtml));
	// Every password field that carries a live strength meter inside its <label> must name itself via aria-labelledby
	// (pointing at its own label span), so the meter's per-keystroke text never becomes part of the field's name. Lock
	// each of the four so a future markup edit can't drop the association and reintroduce the name-mutation bug.
	for (const [input, label] of [['createPass', 'createPassLabel'], ['passwdNew', 'passwdNewLabel'], ['confirmPw2', 'confirmPw2Label']]) {
		ok('the ' + input + ' field is named by aria-labelledby (meter does not pollute the name)', new RegExp('id="' + input + '"[^>]*aria-labelledby="' + label + '"').test(indexHtml) && new RegExp('id="' + label + '"').test(indexHtml));
	}
	...
	// The app is a single view with a top bar (no nav rail), so the icon-only top-bar controls must carry accessible
	// names — a screen reader gets "Settings" / "Change theme" / "System notifications", not an unlabeled glyph.
	ok('the top-bar icon controls have accessible names', /id="settingsBtn"[^>]*aria-label=/.test(indexHtml) && /id="themeToggle"[^>]*aria-label=/.test(indexHtml) && /id="notifBtn"[^>]*aria-label=/.test(indexHtml) && /id="lockAllBtn"[^>]*aria-label=/.test(indexHtml));
	...
	// Search hits are now openable in the built-in viewer whether or not the vault is mounted, so they legitimately show
	// a pointer — but a pointer must never be misleading, so a clickable hit is a real keyboard-reachable control
	// (role="button" tabindex="0"), activated by Enter/Space as well as a click. This replaces the old "read-only hits
	// are not clickable" rule, which predated opening results in the mount-free viewer.
	ok('clickable search hits are real keyboard-accessible controls (role=button, tabindex — pointer is not misleading)', /class="search-hit search-hit-open" role="button" tabindex="0"/.test(app) && /addEventListener\('keydown'[\s\S]{0,160}hit\.click\(\)/.test(app));
	...
	// Match the dir="auto" on the name span and the path element while tolerating an optional class attribute (the
	// privacy screen adds a "blurred" class conditionally), so the guarantee is pinned without pinning exact markup.
	...
	// A keyboard user can jump past the whole nav rail to the content: a skip link that targets the focusable <main>.
	ok('a skip-to-content link targets the main region', /class="skip-link" href="#main"/.test(indexHtml) && /<main[^>]*id="main"[^>]*tabindex="-1"/.test(indexHtml));
	// One startup helper gives every labelled field a clean name (aria-labelledby to its label span) and a separate
	// description (aria-describedby to its hint), so a long hint is announced as a description, not baked into the name.
	ok('a startup helper links every field to its label and hint (aria-labelledby / aria-describedby)', /function describeFieldHints\(\)[\s\S]{0,700}aria-labelledby[\s\S]{0,300}aria-describedby/.test(app) && /\bdescribeFieldHints\(\);/.test(app));
	// Purely-decorative inline SVG icons are hidden from assistive tech, so a screen reader names each control once.
	ok('decorative SVG icons are hidden from assistive tech', /svg:not\(\[aria-hidden\]\)[\s\S]{0,80}setAttribute\('aria-hidden'/.test(app));
	...
	ok('the mobile viewer moves focus into itself on open', /function showViewer[\s\S]{0,700}el\('viewerClose'\)\.focus\(\)/.test(mApp));
	...
	ok('the mobile viewer restores focus to the opener on close', /function closeViewer[\s\S]{0,1000}lastFocusBeforeViewer[\s\S]{0,80}\.focus\(\)/.test(mApp));
