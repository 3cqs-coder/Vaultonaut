...
function main() {
	...
	// The OCR engine ships as pure JavaScript and WebAssembly, with no native addon. The desktop bundle copies
	// node_modules verbatim except a filter that drops ONLY native .node addons, so the OCR WebAssembly (the recognizer
	// core and the PDF engine) rides along. Guard that the filter never grows to strip .wasm or .js, which would
	// silently break OCR in the packaged app, and that the WebAssembly the bundle copies from is actually present.
	const prepSrc = fs.readFileSync(prep, 'utf8');
	ok('the sidecar copies node_modules into the bundle', /fs\.cpSync\([^)]*node_modules[\s\S]{0,160}filter:/.test(prepSrc));
	ok('the sidecar node_modules filter excludes only native .node addons (OCR wasm and js survive)', /endsWith\('\.node'\)/.test(prepSrc) && !/endsWith\('\.wasm'\)/.test(prepSrc) && !/endsWith\('\.js'\)/.test(prepSrc));
	const repoRoot = path.join(__dirname, '..', '..');
	if (fs.existsSync(path.join(repoRoot, 'node_modules'))) {
		for (const w of ['node_modules/tesseract.js-core/tesseract-core-lstm.wasm', 'node_modules/@hyzyla/pdfium/dist/pdfium.wasm']) {
			ok('OCR WebAssembly present to bundle: ' + w, fs.existsSync(path.join(repoRoot, w)));
		}
	} else { console.log('  skip  (node_modules not installed — cannot check OCR WebAssembly presence)'); }
