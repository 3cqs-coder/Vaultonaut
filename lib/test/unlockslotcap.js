...
async function main() {
	...
	ok('the passphrase unlock filters out member slots before the 64-slot cap', /filter\(s => s\.kind !== 'member'\)\.slice\(0, 64\)/.test(src2));
	ok('the member unlock filters to member slots before the 256-slot cap', /filter\(s => s\.kind === 'member'\)\.slice\(0, 256\)/.test(src2));
