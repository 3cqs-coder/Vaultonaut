...
const Serve = require('../Serve'); // freePort() — pick an ephemeral port for the spawned service, so a busy fixed port cannot make this flake
...
function runCli(args, input) {
	return new Promise((resolve) => {
		// process.execPath, not a bare 'node': the child must be the SAME Node this suite runs under (the ML-DSA
		// signature check needs 24.7+), and a bare 'node' on PATH could be an older, wrong version — or absent.
		const c = cp.spawn(process.execPath, ['-r', SETUP, path.join(ROOT, 'vaultonaut.js'), ...args], { cwd: ROOT });
		let out = '';
		c.stdout.on('data', (d) => out += d); c.stderr.on('data', (d) => out += d);
		c.on('error', (e) => resolve({ code: -1, out: out + '\n[spawn error] ' + (e && e.message || e) })); // never leave the promise pending on a spawn failure
		if (input != null) { c.stdin.on('error', () => {}); c.stdin.write(input); c.stdin.end(); }
...
(async () => {
	...
	// This test creates a vault and mounts it THROUGH a running service, so it needs the bundled engine AND a
	// FUSE/WinFsp driver. On a host without a driver (a CI runner) the delegated mount produces no files and the
	// read-back would fail with ENOENT, so skip cleanly rather than fail — matching every other mount test.
	const d = await vdisk.doctor();
	if (!d.engine.ok || !d.driver.ok) { console.log('  skip  (needs the bundled engine and a mount driver)'); process.exit(0); }
	...
	try {
		...
		const PORT = await Serve.freePort();
		// Spawn the real service as a SUPERVISED CHILD: pipe its stdin and set VDISK_SUPERVISED so it arms the dead-man's
		// switch. Then, even if THIS test process is hard-killed (an interrupted suite) before the teardown below runs, the
		// OS closes the child's stdin and the service exits on its own — so a spawned service can never be leaked.
		svc = cp.spawn(process.execPath, ['-r', SETUP, '-e', "require('" + ROOT.replace(/\\/g, '\\\\') + "/lib/webserver').start(" + PORT + ").catch(e=>{console.error(e);process.exit(1)})"], { cwd: ROOT, stdio: ['pipe', 'ignore', 'ignore'], env: Common.supervisedChildEnv() });
		svc.on('error', () => {}); // a spawn failure surfaces as the url-advertised check below timing out, not an unhandled event
		let url = null;
