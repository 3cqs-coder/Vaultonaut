...
// The only fs rename in Vault.js is the corrupt-settings quarantine (a local file OUTSIDE any vault). Any other rename
// would be a durable vault write sneaking back onto the filesystem.
{
	const renameLines = V.split('\n').filter(l => /\.rename\(/.test(l) && !/^\s*\/\//.test(l));
	ok('the only fs rename in Vault.js is the corrupt-settings quarantine, not a vault write', renameLines.length === 1 && /corrupt-/.test(renameLines[0]));
}
