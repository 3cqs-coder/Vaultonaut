...
(async () => {
	// This test creates, mounts, and re-mounts a vault to plant and detect an uninterpretable baseline, so it needs
	// the bundled engine AND a FUSE/WinFsp driver. Skip cleanly where either is missing (a CI runner, or any host
	// without a driver) instead of failing on the mount.
