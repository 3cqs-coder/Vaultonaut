...
function main() {
	...
	const stripsBidi = (src) => /\\u202A-\\u202E/.test(src) && /\\u2066-\\u2069/.test(src) && /\\uFFFD/.test(src);
	ok('the desktop esc() neutralizes bidi/control characters (anti-spoofing)', stripsBidi(app));
	ok('the mobile escapeHtml() neutralizes bidi/control characters (anti-spoofing)', stripsBidi(mobileApp));
