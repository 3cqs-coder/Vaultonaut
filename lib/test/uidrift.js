...
function main() {
	...
	// 7. Cloud vaults do not support key rotation, packing, or a shared folder, so the Keys and Share dialogs must be
	//    cloud-aware or they present dead paths that only error server-side. Pin that: the Keys dialog hides its rotate
	//    action for a cloud vault, and the Share dialog swaps its file/folder delivery for cloud guidance. A regression
	//    that drops this awareness reintroduces the dead click, so guard it statically.
	ok('the Keys dialog hides the rotate action for a cloud vault', /#keysRotate'\)\.hidden = keysCloud/.test(app) && /vaultsByPath\[path\] \|\| \{\}\)\.cloud/.test(app));
