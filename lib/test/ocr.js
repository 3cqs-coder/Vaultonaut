...
async function main() {
	...
	try {
		...
		// OCR must never run on the automatic index refresh (it is expensive) — only on an explicit, user-triggered
		// reindex. contentReindex defaults allowOcr to false, and the auto-refresh-on-mount path never turns it on.
		const vault = read('lib/Vault.js');
		ok('contentReindex defaults OCR off (allowOcr = false) so the auto-refresh never OCRs', /allowOcr\s*=\s*false/.test(vault));
