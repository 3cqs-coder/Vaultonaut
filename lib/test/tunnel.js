...
async function main() {
	...
	// 6. Cross-platform / non-blocking source contract.
	{
		...
		// The keepalive must NOT advance the missed-pong count while inbound is deliberately paused for aggregate
		// backpressure: sock.pause() stops 'data', so PONGs are not read and a healthy peer would otherwise be declared
		// dead, tearing down the whole shared link and every stream on it (a one-slow-client DoS on the relay mux path).
		ok('keepalive skips the missed-pong penalty while paused for aggregate backpressure', /if \(aggPaused\)[^\n]*\breturn\b/.test(src) && src.indexOf('if (aggPaused)') < src.indexOf('missedPongs++'));
		// armStreamIdle must baseline the byte counter at arm time (not -1), so a silent stream is reaped at ~streamIdleMs
		// rather than ~2x it (a -1 seed wastes the first sample just recording the count).
		ok('armStreamIdle baselines bytes at arm time (reaps at ~ms, not 2x ms)', /let last = -1; try \{ last = \(sock\.bytesRead/.test(src));
	}
