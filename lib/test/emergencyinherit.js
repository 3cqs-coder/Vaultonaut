...
// Grants are time-locked. To exercise the OPEN path offline, route them to a scheduled unlock at drand round
// 1,000,000 (already mature) and open with that round's real published signature (the same interop vector the
// time-lock test pins). The identity gate — which key opens which grant — is unaffected by using one shared round.
...
async function main() {
	...
	const capsuleDate = Timelock.timeOfRoundMs(REAL_ROUND); // a scheduled-unlock instant that maps to the mature round
	...
	// Route the bank vault to the spouse and the business vault to the partner (scheduled to the mature round so the
	// isolation check below can actually open each grant offline).
	await vdisk.emergencyArm(vBank, { password: 'pw', contactId: spouse, mode: 'date', dateMs: capsuleDate });
	await vdisk.emergencyArm(vBiz, { password: 'pw', contactId: partner, mode: 'date', dateMs: capsuleDate });
	...
	const tryOpen = async (priv, sealed) => { try { return await vdisk.emergencyOpen(priv, sealed, { signature: REAL_SIG }); } catch (_) { return null; } };
	// Open the grants ONE AT A TIME, the way a real beneficiary does — never several at once. Each open runs the
	// time-locked pairing core in a worker thread, so opening them concurrently (Promise.all) would spawn several
	// worker threads at the same instant, and concurrent worker spawn and teardown is a known crash trigger on some
	// platforms. Production always opens sequentially (one person opening one grant), so a sequential loop both matches
	// real use and keeps this test to a single worker thread at a time.
	const openAll = async (priv) => { const out = []; for (const s of sealedFiles) { const r = await tryOpen(priv, s); if (r) out.push(r); } return out; };
	const aOpens = await openAll(A.privateKey); // the read-cap TOKENS A can recover
	const bOpens = await openAll(B.privateKey);
	...
	ok('the legacy beneficiary key opens the released legacy grant', !!vdisk.parseReadCap(await vdisk.emergencyOpen(A.privateKey, lsealed, { signature: REAL_SIG })));
