...
async function main() {
	...
	hub = Relay.runHub({ controlPort: 17451, token: 'secret', host: '127.0.0.1', portRange: [21200, 21210] });
	await sleep(250);
	{ const r = Relay.registerNode({ hubHost: '127.0.0.1', hubPort: 17451, token: 'secret', nodeId: 'node-alpha', localPort }); const info = await r.ready; node = { publicPort: info.publicPort, stop: r.stop }; }
	ok('the node is assigned a public port on the hub', Number.isInteger(node.publicPort) && node.publicPort >= 21200 && node.publicPort <= 21210);
	...
	const bad = await new Promise((resolve) => { const s = net.connect(17451, '127.0.0.1', () => s.write('HELLO wrongtoken n2\n')); let b = ''; s.on('data', d => b += d); s.on('close', () => resolve(b.trim())); setTimeout(() => { s.destroy(); resolve(b.trim()); }, 600); });
	...
	const r2 = Relay.registerNode({ hubHost: '127.0.0.1', hubPort: 17451, token: 'secret', nodeId: 'node-alpha', localPort });
