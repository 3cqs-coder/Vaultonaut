...
const TOKEN = 'relay-secret-token-0123456789'; // >= MIN_TOKEN_LEN (the hub refuses a guessably short token)
...
async function main() {
	...
	hub = Relay.runHub({ controlPort, token: TOKEN, host: '127.0.0.1', portRange: [pub, pub] });
	await sleep(250);
	{ const r = Relay.registerNode({ hubHost: '127.0.0.1', hubPort: controlPort, token: TOKEN, nodeId: 'node-alpha', localPort }); const info = await r.ready; node = { publicPort: info.publicPort, stop: r.stop }; }
	...
	const r2 = Relay.registerNode({ hubHost: '127.0.0.1', hubPort: controlPort, token: TOKEN, nodeId: 'node-alpha', localPort });
