...
async function main() {
	...
	// The child self-destructs after 20s so that even if THIS test is hard-killed before killAll runs, it cannot be
	// left running — a test must never leak a process, not even a bare node child.
	const child = spawn(process.execPath, ['-e', 'setInterval(()=>{},1000);setTimeout(()=>process.exit(0),20000)'], { stdio: 'ignore' });
	P.track(child);
	await wait(200);
