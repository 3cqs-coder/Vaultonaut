...
//   #2f2f33 — the undercover-disguise clock glyph, a dim grey on the fixed black cover. The disguise MUST look identical
//   and innocuous under every theme (it deliberately does not use theme tokens, which would reveal the app), so its
//   colors are intentionally fixed literals, not tokens.
// The mobile PWA viewer ships a SEPARATE, dependency-free stylesheet that cannot use the full token system, so a couple
// of its per-theme literals (verified to clear WCAG AA in every theme) are allowed there:
//   #845400 — the "stored offline" trace tag on the light/sepia grounds;
//   #e0a94a — the same tag on the dark ground.
const ALLOW = new Set([
	'#fff', '#ffffff', '#000', '#000000',
	'#ffe066', '#1a1400', '#ffb300', // search highlight (bg / text / active bg)
	'#2f2f33', // undercover disguise: dim clock glyph on the fixed black cover (theme-neutral by design)
	'#845400', '#e0a94a', // mobile viewer: "stored offline" trace tag (light/sepia and dark; AA-verified in every theme)
	'rgba(0, 0, 0, .22)', 'rgba(0,0,0,.12)', 'rgba(0,0,0,.25)', 'rgba(6, 10, 18, .45)', 'rgba(8, 12, 24, .4)', 'rgba(8, 12, 24, .5)',
]);

// Scan BOTH stylesheets: the desktop interface AND the mobile viewer, so a hardcoded color can never slip into either.
const cssFiles = [
	path.join(__dirname, '..', 'webserver', 'public', 'css', 'app.css'),
	path.join(__dirname, '..', 'webserver', 'public', 'mobile', 'app.css'),
];
const offenders = [];
for (const cssFile of cssFiles) {
	let css; try { css = fs.readFileSync(cssFile, 'utf8'); } catch (_) { continue; }
	...
	for (const c of new Set(literals)) if (!ALLOW.has(c)) offenders.push(path.basename(path.dirname(cssFile)) + '/' + path.basename(cssFile) + ': ' + c);
}
ok('no un-reviewed hardcoded color literal appears in a CSS rule body (desktop or mobile — use a theme token)', offenders.length === 0);
