...
const EXPECTED_PASSPHRASE = new Set(['id', 'kind', 'label', 'createdAt', 'kdf', 'wrappedKey', 'owner', 'webauthn', 'keyfileName']);
...
// 1b. MIN_FORMAT (the oldest format this build opens) is FROZEN at 5 and must never be raised in lock-step with a
//     SUPPORTED_FORMAT bump: doing so would make every existing vault at the previous format (including every vault
//     the shipped 1.0.0 client created) permanently unopenable, and every round-trip test would still pass because it
//     writes and reads with the same new code. It moves forward ONLY when a real migration branch for the old layout
//     lands in parseManifest — a deliberate, reviewed change to this frozen value, not an incidental bump. Also pin
//     the sanity relation MIN_FORMAT <= SUPPORTED_FORMAT.
const FROZEN_MIN_FORMAT = 5;
const minMatch = src.match(/const\s+MIN_FORMAT\s*=\s*(\d+)/);
ok('found MIN_FORMAT in Vault.js', !!minMatch);
const liveMin = minMatch ? Number(minMatch[1]) : NaN;
ok('MIN_FORMAT (' + liveMin + ') is still frozen at ' + FROZEN_MIN_FORMAT + ' — raise it only with a real parseManifest migration for the old layout, never in lock-step with a SUPPORTED_FORMAT bump', liveMin === FROZEN_MIN_FORMAT);
ok('MIN_FORMAT (' + liveMin + ') is not above SUPPORTED_FORMAT (' + liveFormat + ')', liveMin <= liveFormat);
...
function fieldsOfLiteralContaining(marker) {
	...
	// Collect `name:` at the start of each field. Handles bare `id: s.id` and `owner: !!s.owner` (a leading `!!`).
	const names = new Set();
	for (const m of braced.matchAll(/([A-Za-z_][A-Za-z0-9_]*)\s*:\s*(?:!!)?s\./g)) names.add(m[1]);
	// Also capture spread-guarded optional fields `...(s.name ? { name: <value> } : {})` by their guard variable, since
	// such a field's stored value may be a literal (e.g. `owner: true`) rather than a read of `s.name`.
	for (const m of braced.matchAll(/\.\.\.\(s\.([A-Za-z_][A-Za-z0-9_]*)\s*\?/g)) names.add(m[1]);
	return names;
}
