...
async function main() {
	...
	// POST-QUANTUM: stripping the baseline's ML-DSA signature (classical half untouched) must be caught as TAMPERED by
	// BOTH verifiers — the main-tool verifyBundle once checked only the classical half, so this pins that it now checks
	// the post-quantum half too, in lockstep with the standalone verifier.
	const noPq = JSON.parse(await fsp.readFile(path.join(out, 'baseline.json'), 'utf8'));
	ok('the genuine baseline actually carries a post-quantum signature', typeof noPq.sigPq === 'string' && noPq.sigPq.length > 100);
	delete noPq.sigPq;
	await fsp.writeFile(path.join(out, 'baseline.json'), JSON.stringify(noPq));
	ok('a baseline with its post-quantum signature stripped is TAMPERED (main verifier)', (await vdisk.verifyBundle(out)).checks.some(c => c.name === 'baseline-signature' && !c.ok));
	ok('the standalone verifier also flags the stripped post-quantum baseline signature', require('../verify-bundle').verifyBundle(out).checks.some(c => c.name === 'baseline-signature' && !c.ok));
	await fsp.writeFile(path.join(out, 'baseline.json'), orig); // restore
	...
	// A FORGED membership roster is TAMPERED. Member slots are excluded from the manifest seal (the owner key signs
	// them separately), so a bundle that gains a members section with an owner key but no valid owner signature must
	// not read GENUINE — otherwise added/edited member slots would ride along undetected in a team vault's proof.
	const withMembers = JSON.parse(mOrig);
	withMembers.members = { epoch: 1, keyGeneration: 0, ownerPubKey: require('crypto').randomBytes(32).toString('hex'), sig: Buffer.from(require('crypto').randomBytes(64)).toString('base64') };
	await fsp.writeFile(path.join(out, 'manifest.json'), JSON.stringify(withMembers));
	ok('a bundle whose membership roster fails its owner signature is TAMPERED (main verifier)', (await vdisk.verifyBundle(out)).checks.some(c => c.name === 'roster-signature' && !c.ok));
	ok('the standalone verifier also flags a forged roster', require('../verify-bundle').verifyBundle(out).checks.some(c => c.name === 'roster-signature' && !c.ok));
	await fsp.writeFile(path.join(out, 'manifest.json'), mOrig); // restore
	ok('the restored bundle verifies GENUINE again after the roster check', (await vdisk.verifyBundle(out)).verdict === 'GENUINE');
