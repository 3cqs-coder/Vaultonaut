...
async function main() {
	...
	// A network error yields 0 (best-effort), never a throw that could wedge a health scan, and calls onFail so the
	// caller can report the scan as incomplete.
	let calls = 0, fails = 0;
	global.fetch = async () => { calls++; throw new Error('offline'); };
	const fcache = new Map();
	let threw = false, val = null;
	try { val = await PwnedCheck.count(PW, { cache: fcache, onFail: () => { fails++; } }); } catch (_) { threw = true; }
	ok('a network failure yields 0 rather than throwing, and calls onFail', !threw && val === 0 && fails === 1);

	// Within ONE scan (the same cache) a failed prefix is remembered so an outage is not re-hammered: the repeat
	// does NOT re-fetch, yet it still returns "unknown" (0) and reports the scan incomplete via onFail — a failed
	// lookup is never cached as a definitive "not breached".
	await PwnedCheck.count(PW, { cache: fcache, onFail: () => { fails++; } });
	ok('within a scan a failed prefix is remembered (no re-fetch) but still reports incomplete', calls === 1 && fails === 2 && fcache.size === 1);

	// A FRESH scan (fresh cache) retries the same prefix rather than inheriting the earlier failure.
	await PwnedCheck.count(PW, { cache: new Map(), onFail: () => { fails++; } });
	ok('a fresh scan retries a previously-failed prefix', calls === 2 && fails === 3);
