...
async function main() {
	...
	// A network error yields 0 (best-effort), never a throw that could wedge a health scan; it calls onFail so the
	// caller can report the scan as incomplete; and it is NOT cached, so a later lookup of the same prefix retries
	// (a transient failure never poisons the whole scan into a false "not breached").
	let calls = 0, fails = 0;
	...
	ok('a failed lookup is NOT cached — the same prefix retries next time', calls === 2 && fails === 2 && fcache.size === 0);
