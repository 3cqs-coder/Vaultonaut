...
async function main() {
	...
	await signInto();
	res = await signBundle.embedSignedManifest({ appDir, repoDir, pubHex: pub, pqPubHex: pqPub });
	ok('a matching committed manifest embeds and verifies', res && res.signed === true && res.count === list.length);
	ok('the manifest and BOTH signatures were copied into the staged bundle', fs.existsSync(path.join(appDir, RI.MANIFEST_NAME)) && fs.existsSync(path.join(appDir, RI.SIG_NAME)) && fs.existsSync(path.join(appDir, RI.SIG_PQ_NAME)));
	...
	let threw = false; try { await signBundle.embedSignedManifest({ appDir, repoDir, pubHex: pub, pqPubHex: pqPub }); } catch (_) { threw = true; }
	...
	threw = false; try { await signBundle.embedSignedManifest({ appDir, repoDir, pubHex: pub, pqPubHex: pqPub }); } catch (_) { threw = true; }
