...
async function main() {
	...
	const leaks = [path.resolve(v1), path.resolve(v2)].some(p => stateText.includes(p) || settingsText.includes(p));
	ok('no hidden vault path appears in the app\'s plaintext state', !leaks);
