...
function main() {
	...
	// 3. Every setting the SAVE handler accepts must also be emitted by publicSettings(). This is the other half of the
	//    original "didn't stick" bug: a key that POST /api/settings writes to disk but publicSettings() never returns is
	//    saved, yet the next state poll never sends it back, so the control silently reverts on reload. Scope the scan to
	//    the /api/settings handler body (from its route to the next route) so `body.X` reads elsewhere are not counted.
	const settingsStart = server.indexOf("app.post('/api/settings'");
	const settingsEnd = server.indexOf('app.post(', settingsStart + 1);
	const handlerBody = server.slice(settingsStart, settingsEnd > 0 ? settingsEnd : undefined);
	ok('the /api/settings handler body was located for scanning', settingsStart >= 0 && handlerBody.length > 0);
	const accepted = new Set();
	for (const m of handlerBody.matchAll(/body\.([a-zA-Z][a-zA-Z0-9]*)\s*!=\s*null/g)) accepted.add(m[1]);
	ok('the save handler accepts a non-trivial set of settings', accepted.size >= 4);
	for (const key of accepted) ok('saved setting "' + key + '" is emitted back by publicSettings() (so it restores on reload)', emitted.has(key));

	// 4. Explicit presence pins for the peer-connections transport control. The symmetry checks above catch a field
	//    dropped from ONE side, but not from BOTH at once; pin p2pMode on both sides and in the UI select so the whole
	//    control cannot be silently removed. (This is a real user-facing setting: Auto / Relay only / Direct only.)
	ok('p2pMode is emitted by publicSettings()', emitted.has('p2pMode'));
	ok('p2pMode is accepted by the /api/settings save handler', accepted.has('p2pMode'));
	ok('the interface wires a #p2pMode control that saves p2pMode', /getElementById\('p2pMode'\)|\$\('#p2pMode'\)/.test(app) && /p2pMode:\s*v/.test(app));
