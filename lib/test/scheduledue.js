...
function main() {
	...
	// --- daily with an IANA zone: fires at the same LOCAL wall-clock time year-round (daylight-saving-correct) ---
	// The whole point: "9:00 local" maps to a DIFFERENT UTC instant in winter vs summer, so it never slides an hour
	// across a clock change. The stored hour/minute are deliberately bogus (0,0) here to prove the zone path wins.
	const NY = 'America/New_York';
	const dailyTz = (lh, lm, tz, lastRunAt) => ({ mode: 'daily', hour: 0, minute: 0, localHour: lh, localMinute: lm, tz, dest: '/backup', lastRunAt: lastRunAt ? lastRunAt.toISOString() : null });
	// Winter: 9:00 EST = 14:00 UTC.
	ok('zoned daily is due at its local time in winter (09:00 EST = 14:00 UTC)', due(dailyTz(9, 0, NY, null), D(2026, 0, 15, 14, 0)) === true);
	ok('zoned daily is NOT due one hour before its local time in winter', due(dailyTz(9, 0, NY, null), D(2026, 0, 15, 13, 0)) === false);
	// Summer: 9:00 EDT = 13:00 UTC — one hour earlier in UTC than winter, proving the zone tracks daylight saving.
	ok('zoned daily is due at its local time in summer (09:00 EDT = 13:00 UTC)', due(dailyTz(9, 0, NY, null), D(2026, 6, 15, 13, 0)) === true);
	ok('zoned daily is NOT due one hour before its local time in summer', due(dailyTz(9, 0, NY, null), D(2026, 6, 15, 12, 0)) === false);
	// Run-once and catch-up semantics still hold on the zone path.
	ok('zoned daily already run today is not due again', due(dailyTz(9, 0, NY, D(2026, 0, 15, 14, 0)), D(2026, 0, 15, 15, 0)) === false);
	ok('zoned daily that ran on time yesterday is not due before today\'s local time', due(dailyTz(9, 0, NY, D(2026, 0, 14, 14, 0)), D(2026, 0, 15, 13, 0)) === false);
	ok('zoned daily catches up a missed local-yesterday run', due(dailyTz(9, 0, NY, D(2026, 0, 13, 14, 0)), D(2026, 0, 15, 13, 0)) === true);
	// An unknown/invalid zone degrades gracefully to the legacy UTC hour/minute path (never silently stops firing).
	ok('zoned daily with an unknown zone falls back to the UTC hour/minute path', due({ mode: 'daily', hour: 0, minute: 0, localHour: 9, localMinute: 0, tz: 'Not/AZone', dest: '/backup', lastRunAt: null }, D(2026, 0, 15, 1, 0)) === true);
