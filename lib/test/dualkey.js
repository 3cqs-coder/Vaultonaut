...
async function main() {
	...
	// The capability-model checks below need only the engine; the read-ONLY vs read-write MOUNT checks also need a
	// FUSE/WinFsp driver, which a CI runner (or any host without one installed) lacks. Gate just the mount parts on it
	// so this test still verifies the key model everywhere and skips only what genuinely cannot run.
	const hasDriver = (await vdisk.doctor()).driver.ok;
	...
	} else { console.log('  skip  read-only mount checks (no mount driver on this host)'); }
	...
	} else { console.log('  skip  read-capability mount checks (no mount driver on this host)'); }
	ok('a garbage token is rejected', vdisk.parseReadCap('vdrc1.not-valid') === null);
	// Forward-compat: a read cap whose inner version is beyond what this build understands must fail closed, not be
	// misread as the current format. A current-version cap with a master still parses.
	const mkCap = (obj) => 'vdrc1.' + Buffer.from(JSON.stringify(obj)).toString('base64url');
	ok('a read cap from a newer format (v > 1) is rejected (fails closed)', vdisk.parseReadCap(mkCap({ v: 2, master: 'x' })) === null);
	ok('a current-version read cap (v: 1) still parses', !!vdisk.parseReadCap(mkCap({ v: 1, master: 'x' })));
