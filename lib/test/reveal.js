...
ok('the viewer routes a deep-linked Back to the search (close its own tab, else back to the app)', /deepLinked = true;/.test(viewerSrc) && /if \(deepLinked && wasOpen\) \{ deepLinked = false; returnToSearch\(\); return; \}/.test(viewerSrc) && /window\.opener && !window\.opener\.closed\) \{ window\.close\(\); return; \}/.test(viewerSrc));
