...
async function main() {
	...
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, vaultId: fmId }); // current index validly signed at version V
	const vFm = (await Recovery.readIndex(v)).version;
	let floorRefused = false; try { await Recovery.heal(v, { pubkey: kp.pub, vaultId: fmId, expectedFloor: vFm + 5 }); } catch (e) { floorRefused = e.code === 'AUTH_REFUSED' && !!e.downgrade; }
	ok('heal refuses a rolled-back index via the manifest floor alone, no ledger anchor (fresh machine)', floorRefused);
	const hFm = await Recovery.heal(v, { pubkey: kp.pub, vaultId: fmId, expectedFloor: vFm }); // floor == current version: not a downgrade
