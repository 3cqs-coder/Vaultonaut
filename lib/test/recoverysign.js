...
async function main() {
	...
	let a = (await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId })).authenticity;
	ok('an unsigned index reads as weaker-trust (signed:false)', !!a && a.signed === false);

	const p = await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId }); // signed build
	ok('a signed protect reports signed + a version', p.signed === true && Number.isInteger(p.signedVersion));
	a = (await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId })).authenticity;
	ok('a valid signature verifies against the published key', !!a && a.verified === true);
	a = (await Recovery.verify(v, { pubkey: wrong.pub, pqPubkey: wrong.pqPub, vaultId })).authenticity;
	ok('a signature checked against the WRONG key is a tamper signal', !!a && a.tampered === true);
	a = (await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: 'a-different-vault' })).authenticity;
	...
	await Recovery.signIndex(v, { signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId }); // sign the current index in place (no rebuild)
	a = (await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId })).authenticity;
	ok('sign-in-place makes an existing index verify', !!a && a.verified === true);

	// The signature is HYBRID: the sidecar carries a post-quantum signature at the current version.
	const sig0 = path.join(v, '.recovery', 'index.sig.json');
	const s0shape = JSON.parse(await fsp.readFile(sig0, 'utf8'));
	ok('the recovery signature is hybrid (a post-quantum signature at v2)', s0shape.v === 2 && typeof s0shape.sigPq === 'string' && s0shape.sigPq.length > 100);
	...
	// The post-quantum half is REQUIRED (AND-combiner): forging it in every replica, while the classical half stays
	// valid, must read as tampered — a stripped or forged post-quantum signature can never be accepted.
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId });
	for (const n of ['index.sig.json', 'index.sig.bak.json', 'index.sig.bak2.json']) { try { const pth = path.join(v, '.recovery', n); const s = JSON.parse(await fsp.readFile(pth, 'utf8')); s.sigPq = (s.sigPq || 'AA').slice(0, -6) + 'AAAAAA'; await fsp.writeFile(pth, JSON.stringify(s)); } catch (_) {} }
	ok('a forged post-quantum half (classical still valid) reads as tampered', (((await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId })).authenticity) || {}).tampered === true);
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId }); // re-sign clean for the checks below

	let refused = false;
	try { await Recovery.heal(v, { pubkey: wrong.pub, pqPubkey: wrong.pqPub, vaultId }); } catch (e) { refused = !!(e.authenticity && e.authenticity.tampered); }
	ok('heal REFUSES a present-but-invalid signature (possible forgery)', refused);
	const h = await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId });
	...
	ok('an all-replica bad signature reads as tampered', (((await Recovery.verify(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId })).authenticity) || {}).tampered === true);
	let denied = false; try { await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId }); } catch (e) { denied = e.code === 'AUTH_REFUSED'; }
	ok('heal refuses a tampered signature by default', denied);
	const hf = await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId, allowUnverified: true });
	...
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId: dgId });          // sign, then anchor it locally
	...
	let dgRefused = false; try { await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: dgId }); } catch (e) { dgRefused = e.code === 'AUTH_REFUSED' && !!e.downgrade; }
	ok('heal refuses a signature-strip downgrade by default', dgRefused);
	const hdg = await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: dgId, allowUnverified: true });
	...
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId: rpId });            // current index: validly signed at version V
	...
	let replayRefused = false; try { await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: rpId }); } catch (e) { replayRefused = e.code === 'AUTH_REFUSED' && !!e.downgrade; }
	ok('heal refuses an older-but-validly-signed index below the anchored version', replayRefused);

	// FRESH MACHINE (no ledger anchor): the same replay must be refused using ONLY the manifest's signed recovery-
	// version floor (expectedFloor), which travels with the vault. A brand-new vaultId means no local ledger record,
	// so the ledger gate is silent; the floor alone must still catch the rolled-back index.
	const fmId = crypto.randomBytes(16).toString('hex');
	await Recovery.protect(v, { tier: 'low', signPriv: kp.priv, signPqPriv: kp.pqPriv, vaultId: fmId }); // current index validly signed at version V
	const vFm = (await Recovery.readIndex(v)).version;
	let floorRefused = false; try { await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: fmId, expectedFloor: vFm + 5 }); } catch (e) { floorRefused = e.code === 'AUTH_REFUSED' && !!e.downgrade; }
	ok('heal refuses a rolled-back index via the manifest floor alone, no ledger anchor (fresh machine)', floorRefused);
	const hFm = await Recovery.heal(v, { pubkey: kp.pub, pqPubkey: kp.pqPub, vaultId: fmId, expectedFloor: vFm }); // floor == current version: not a downgrade
	ok('heal proceeds when the index meets the manifest floor', !!hFm && hFm.protected === true);
