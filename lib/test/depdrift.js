...
for (const [name, range] of Object.entries(deps)) {
	...
	if (inst) { const s = inst.scripts || {}; const life = ['preinstall', 'install', 'postinstall'].filter(x => s[x]); ok('dependency "' + name + '" declares no install lifecycle script', life.length === 0); if (life.length) console.log('        scripts: ' + life.join(', ')); }
}
