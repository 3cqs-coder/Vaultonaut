...
const Sync = require('../Sync');     // workdirFor — the mirror's on-disk bisync baseline location, keyed by vault path
...
async function main() {
	...
	// A mirror's on-disk bisync BASELINE is keyed by the vault path, not by settings, so it must follow an anonymize —
	// otherwise the mirror would read as "not primed" and force a destructive re-prime. Simulate a primed mirror with a
	// destination and a baseline directory, then anonymize and confirm the baseline moved to the new opaque path.
	const mv = await vdisk.create(path.join(tmp, 'Mirrored.vault'), { password: 'pw7' });
	const destKey = 'sftp:test-dest';
	await Vault.setSettings({ syncDests: { [mv.vault]: destKey } });
	const oldWd = Sync.workdirFor(mv.vault, destKey);
	await fsp.mkdir(oldWd, { recursive: true });
	await fsp.writeFile(path.join(oldWd, 'baseline.lst'), 'baseline');
	const mvAn = await vdisk.anonymizeVault(mv.vault, { password: 'pw7' });
	const newWd = Sync.workdirFor(mvAn.vault, destKey);
	ok('the mirror bisync baseline follows the vault when its folder is anonymized', !(await exists(oldWd)) && (await exists(path.join(newWd, 'baseline.lst'))));
	await Vault.setSettings({ syncDests: {} }); // tidy the shared test settings
