...
function main() {
	...
	// The Linux WebKitGTK first-paint fix: the webview misses the first paint of content loaded at window creation, so
	// the shell re-navigates the visible window to the splash once, after the surface is mapped, to force that paint.
	// (The splash may be animated — proven on a virtio_gpu VM — because animation was never the cause; the missed first
	// paint was.) Pin that the fix exists, is called, and is gated to Linux (a runtime cfg! so the body still
	// type-checks on every platform, never a Linux-only #[cfg] that goes unverified).
