...
function main() {
	...
	// The window starts HIDDEN and is revealed only after it navigates to the running interface, so the user never sees
	// WebKitGTK's unpainted first surface — a black window for the whole startup wait on Linux. Pin both halves so this
	// can't silently regress to a visible-at-launch (black-on-Linux) window.
	ok('the main window starts hidden (revealed only after it navigates to the ready interface)', win.visible === false);
	ok('the shell reveals the window after navigating (show_outcome calls window.show)', /fn show_outcome[\s\S]{0,1200}window\.show\(\)/.test(mainRs));
