...
function main() {
	...
	if (fs.existsSync(wf)) {
		...
		// the unmount ladder get real execution rather than only source pins. All three now GATE a release — there is
		// no continue-on-error — so a broken cross-platform path blocks the build and release rather than shipping. If
		// a continue-on-error is ever reintroduced to the test job, this catches it.
		...
		ok('all three test legs gate a release (no continue-on-error)', !/continue-on-error/.test(testJob));
	}
