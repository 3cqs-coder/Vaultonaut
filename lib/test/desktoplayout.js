...
function main() {
	...
	if (fs.existsSync(wf)) {
		...
		// the unmount ladder get real execution rather than only source pins. macOS and Windows are non-blocking for
		// now (continue-on-error), so a not-yet-green cross-platform path cannot block a release; Linux stays the gate.
		...
		ok('the non-Linux test legs are non-blocking for now (Linux gates)', /continue-on-error:\s*\$\{\{\s*matrix\.os\s*!=\s*'ubuntu-22\.04'\s*\}\}/.test(testJob));
	}
