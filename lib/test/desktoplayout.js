...
function main() {
	...
	// The splash MUST stay fully static. An animation, transition, or transform pushes WebKitGTK onto its
	// accelerated-compositing path on the window's first surface, which is unreliable on Linux and left the whole
	// splash black until the app navigated away seconds later. Pin that no such property creeps back into the splash.
	{
		// Strip HTML and CSS comments first: the file legitimately NAMES these words in a comment explaining why they are
		// banned, so scan only for real usage (the at-rule, or a property with its colon/hyphen).
		const css = shellHtml.replace(/<!--[\s\S]*?-->/g, '').replace(/\/\*[\s\S]*?\*\//g, '');
		const offenders = [/@keyframes/, /animation\s*:/, /animation-/, /transition\s*:/, /transition-/, /transform\s*:/].filter((re) => re.test(css)).map((re) => re.source);
		ok('the startup splash is fully static (no animation/transition/transform that breaks Linux rendering)' + (offenders.length ? ': ' + offenders.join(', ') : ''), offenders.length === 0);
	}
	// The Linux WebKitGTK repaint nudge: on affected setups the surface does not paint until a resize forces it, so the
	// shell resizes the window a pixel and back at startup. Pin that the nudge exists, is called, and is gated to Linux
	// (a runtime cfg! so the body still type-checks on every platform, never a Linux-only #[cfg] that goes unverified).
	ok('the shell forces the WebKitGTK first paint on Linux by re-navigating the splash at startup', /fn nudge_repaint[\s\S]{0,1600}win\.navigate\(/.test(mainRs) && /nudge_repaint\(app\.handle\(\)\)/.test(mainRs));
	ok('the first-paint fix is gated to Linux at runtime (stays compiled and type-checked on every platform)', /fn nudge_repaint[\s\S]{0,600}cfg!\(target_os = "linux"\)/.test(mainRs));
