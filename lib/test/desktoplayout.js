...
function main() {
	...
	ok('the shell forces a WebKitGTK repaint on Linux by nudging the window size at startup', /fn nudge_repaint[\s\S]{0,1400}set_size\(/.test(mainRs) && /nudge_repaint\(app\.handle\(\)\)/.test(mainRs));
	ok('the repaint nudge is gated to Linux at runtime (stays compiled and type-checked on every platform)', /fn nudge_repaint[\s\S]{0,500}cfg!\(target_os = "linux"\)/.test(mainRs));
