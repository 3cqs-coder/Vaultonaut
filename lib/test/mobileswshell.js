...
// 4. The app shell is served NETWORK-FIRST, with the cache only as an offline fallback, so a viewer code change is
//    picked up on the very next open instead of a load behind. A regression to cache-first (returning the cached hit
//    before the network) reintroduces the stale-code lag, so pin that the fetch handler tries the network first and
//    falls back to caches.match only on failure.
ok('the app shell is served network-first (cache is only the offline fallback)', /addEventListener\('fetch'[\s\S]*e\.respondWith\(\s*fetch\(req\)/.test(sw) && /catch\(function \(\) \{ return caches\.match\(req\)/.test(sw) && !/return hit \|\| net;/.test(sw));
