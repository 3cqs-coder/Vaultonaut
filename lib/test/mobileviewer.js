...
function main() {
	...
	// The no-trace indicator must be honest and STATE-AWARE: it reads "no-trace" only when nothing is written to the
	// device, and flips to an "encrypted copy on this device" state once the user saves an offline copy. Guard that the
	// element exists, that setTrace is driven by the real presence of a cipher copy at connect (idbCount) and by a
	// successful Save offline, and that the wording never overclaims (it says the copy is unreadable without the
	// in-memory key and is erased on Lock, rather than promising the phone's OS keeps nothing).
	ok('the mobile page has a trace indicator element', /id="traceTag"/.test(html));
	ok('setTrace has both a no-trace and an encrypted-copy state', /function setTrace\(stored\)[\s\S]{0,400}'tracetag stored'[\s\S]{0,400}'tracetag notrace'/.test(app));
	ok('the indicator reflects the real presence of an offline copy at connect', /idbCount\('cipher'\)\.then\(function \(n\) \{ setTrace\(n > 0\)/.test(app));
	ok('a successful Save offline flips the indicator to the stored state', /if \(okc > 0\) setTrace\(true\)/.test(app));
	ok('the no-trace wording is scoped to the app and does not overclaim about the OS', /decrypted only in memory[\s\S]{0,80}Nothing is saved to this device by the viewer/.test(app));
