...
function main() {
	...
	// Cross-browser: a PDF renders in the sandboxed frame in Firefox (pdf.js is JavaScript) as well as Chromium. That
	// needs BOTH allow-scripts (or Firefox shows a blank page) and allow-same-origin (or allow-scripts alone gives the
	// frame an opaque origin that blocks the parent-minted blob: URL). A regression to only one of the two breaks a
	// whole browser family, so pin both.
	ok('a PDF frame is sandboxed with BOTH allow-scripts and allow-same-origin (renders in Firefox and Chromium)', /e === 'pdf'[\s\S]{0,900}sandbox="allow-scripts allow-same-origin"[\s\S]{0,120}application\/pdf/.test(app) || /application\/pdf'\);[\s\S]{0,200}sandbox="allow-scripts allow-same-origin"/.test(app));
