...
async function main() {
	...
	// ---- forward-compat: a roster sealed by a NEWER format is UNVERIFIED (update to verify), never a false TAMPERED ----
	// Simulate a future build: bump the readable sealVersion beyond this build AND make the v1 signature not verify
	// (a real newer-format roster carries a signature this build cannot reconstruct). The roster is not tampered — it
	// is just newer — so the audit must not flag it and the bundle must read UNVERIFIED, not TAMPERED. This is the
	// no-false-tamper-alarm guarantee every other signed format already has (manifest seal, baseline, succession).
	const cleanManifest = await manifestOf(v);
	const future = JSON.parse(JSON.stringify(cleanManifest));
	future.members.sealVersion = 999;
	future.members.sig = 'AAAA' + String(future.members.sig || '').slice(4); // a signature this (older) build cannot verify, like a genuinely newer roster format
	await fsp.writeFile(path.join(v, 'vault.json'), JSON.stringify(future, null, 2));
	await fsp.writeFile(path.join(v, '.vault.bak'), JSON.stringify(future, null, 2));
	const futureMembers = await vdisk.listMembers(v);
	ok('a newer-format roster is flagged unknown-format, not invalid', futureMembers.rosterUnknownFormat === true && futureMembers.rosterValid === true);
	const futureAudit = await vdisk.audit(v, { password: 'owner-pw' });
	ok('the audit does NOT report a newer-format roster as tampering', !(Array.isArray(futureAudit.tamper) && futureAudit.tamper.some(t => /roster/i.test(t))));
	const futureBundleOut = path.join(tmp, 'future-roster-bundle');
	await vdisk.makeBundle(v, { password: 'owner-pw', outDir: futureBundleOut });
	ok('a bundle from a newer-format roster verifies UNVERIFIED (not GENUINE, not TAMPERED)', (await vdisk.verifyBundle(futureBundleOut)).verdict === 'UNVERIFIED');
	await fsp.writeFile(path.join(v, 'vault.json'), JSON.stringify(cleanManifest, null, 2)); // restore the clean roster for the tamper test below
	await fsp.writeFile(path.join(v, '.vault.bak'), JSON.stringify(cleanManifest, null, 2));
