...
// UTC date — the daily log file is named by UTC (so rotation is timezone-stable), so the test must expect the UTC name.
const today = (() => { const d = new Date(), p = (n) => String(n).padStart(2, '0'); return d.getUTCFullYear() + '-' + p(d.getUTCMonth() + 1) + '-' + p(d.getUTCDate()); })();
...
async function main() {
	...
	ok('a daily log file named by the UTC date is created once enabled', fs.existsSync(file));
	const content = fs.existsSync(file) ? fs.readFileSync(file, 'utf8') : '';
	ok('the marker line reached the file', content.includes('logger-test marker line'));
	// Log lines are stamped in UTC (trailing "Z"), so a line reads the same on any host regardless of timezone.
	ok('the log line carries a UTC timestamp prefix', /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}Z /m.test(content));
