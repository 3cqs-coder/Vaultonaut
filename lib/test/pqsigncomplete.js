...
// The standalone verifier and the root verify.js only VERIFY, never sign — assert they hold no signer at all.
ok('verify.js contains no signing call (verify-only)', !/\.sign\s*\(/.test(stripComments(read(path.join(repo, 'verify.js')))));

// --- 2. signHybrid is the single choke point and always emits BOTH halves (so a caller cannot get one without the other).
const integ = stripComments(read(path.join(lib, 'Integrity.js')));
ok('Integrity.signHybrid returns both sig and sigPq', /function signHybrid\([^)]*\)\s*\{\s*return\s*\{\s*sig:\s*sign\([^)]*\),\s*sigPq:\s*signPq\([^)]*\)\s*\}/.test(integ));
ok('Integrity.verifyHybrid ANDs both checks (fails if either half fails)', /function verifyHybrid\([^)]*\)\s*\{\s*return\s*verify\([^)]*\)\s*&&\s*verifyPq\([^)]*\)/.test(integ));
