...
function main() {
	...
	// A NON-MINIMAL sub-identifier (a redundant leading 0x80 continuation group) must NOT decode to the same dotted
	// string as its canonical form — otherwise two byte encodings satisfy a content-type/algorithm gate.
	ok('decodeOid refuses a non-minimal leading 0x80 on the first sub-identifier', Der.decodeOid(Buffer.from([0x80, 0x2a])) === '');
	ok('decodeOid refuses a non-minimal leading 0x80 on a later arc', Der.decodeOid(Buffer.from([0x2a, 0x80, 0x01])) === '');
	// The first sub-identifier is a FULL base-128 value (not just one byte) and its arc split is correct past 0x80:
	// z=127 (single byte 0x7f) is arc 2.47, and a multi-byte first sub-identifier decodes rather than being mis-split.
	ok('decodeOid splits a first-arc value >= 120 correctly (2.47, not 3.7)', Der.decodeOid(Buffer.from([0x7f])) === '2.47');
	ok('decodeOid decodes a multi-byte first sub-identifier', Der.decodeOid(Buffer.from([0x81, 0x00])) === '2.48'); // 0x81 0x00 = 128 = 80 + 48 -> "2.48"
	{
