...
function main() {
	...
	// Build wiring: the scan only guards a release if the build actually runs it. Pin that the Tauri `build` script
	// still ends with scan-leaks, so a future refactor can't silently drop the identity-leak gate from the build.
	const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'src-tauri', 'package.json'), 'utf8'));
	const buildScript = String((pkg.scripts || {}).build || '');
	ok('the Tauri build script still runs scan-leaks as its final step', /&&\s*npm run scan-leaks\s*$/.test(buildScript));
