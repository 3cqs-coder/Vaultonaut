...
async function main() {
	...
	for (const fn of ['changePassword', 'addKeyInternal', 'removeKey']) {
		...
		// Bound the slice to the START OF THE NEXT function declaration, not a fixed character count, so the check
		// stays exact as a function grows (a fixed window could either truncate a longer body or bleed into the next
		// function and falsely pass) and never needs re-tuning when one of these gains a guard.
		const next = vault.slice(at + 10).search(/\n(?:async )?function /);
		const body = at >= 0 ? vault.slice(at, next >= 0 ? at + 10 + next : undefined) : '';
