...
async function main() {
	...
	// ISOLATION GUARD: unpack parses an archive that arrives from OUTSIDE (untrusted input), so the extract must run
	// in the worker — where a parser fault is contained as a clean rejection and a hang is bounded by the watchdog —
	// not on the main thread/process. Pin that Vault.unpack routes the extraction through the PackWorker 'unpack' op
	// (and no longer calls Net.unzip directly on the main thread), and that the worker defines that op.
	const vaultSrc = fs.readFileSync(path.join(__dirname, '..', 'Vault.js'), 'utf8');
	const workerSrc = fs.readFileSync(path.join(__dirname, '..', 'PackWorker.js'), 'utf8');
	const i = vaultSrc.indexOf('async function unpack('), j = vaultSrc.indexOf('\n}', i);
	const unpackBody = i < 0 ? '' : vaultSrc.slice(i, j);
	ok('Vault.unpack extracts via the PackWorker (untrusted parse isolated off the main process)', /PackWorker\.js/.test(unpackBody) && /op: 'unpack'/.test(unpackBody) && !/Net\.unzip\(/.test(unpackBody));
	ok('PackWorker defines the unpack op wiring Net.unzip', /unpack:\s*async/.test(workerSrc) && /Net\.unzip\(/.test(workerSrc));
