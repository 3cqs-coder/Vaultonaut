...
async function main() {
	...
	ok('manifestSealInput (v4, incl. recoveryVersion + owner/keyfile/webauthn attributes) is byte-identical across both implementations', mseOk);

	// verify (Ed25519) + manifestSealVerifies end-to-end: a genuine seal verifies on both, a tampered one fails on both.
	{
		const manifest = { integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub }, crypt: { salt: 'S', keySlots: [{ id: 'primary', kind: 'password', owner: true, keyfileName: 'k.bin', webauthn: { credentialId: 'c1', prfSalt: 'p1' }, wrappedKey: 'AA', kdf: { algo: 'argon2id', v: 1, memKiB: 65536, iterations: 3, parallelism: 4, hashLen: 32, salt: 's' } }] } };
