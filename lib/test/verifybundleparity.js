...
async function main() {
	...
	// sealTag governs the KDF portion of the current (v4) manifest-seal input; it must be byte-identical to Kdf.sealTag over all fields.
	let stOk = true;
	for (let t = 0; t < 60; t++) {
	...
	// manifestSealInput (the single current v4 form) over representative manifests, including a MEMBER slot (which must
	// be excluded from the seal) alongside passphrase-family slots with varied KDF params, recoveryVersion values, AND
	// the v4 non-secret governance/auth attributes (the `owner` marker, keyfile name, and WebAuthn binding) so the
	// attribute folding is byte-identical on both sides too.
	...
	for (let t = 0; t < 20; t++) {
		...
		const manifest = { recoveryVersion: t % 3 === 0 ? undefined : t, integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub, pqPubkey: keypair.pqPub }, crypt: { salt: crypto.randomBytes(16).toString('base64'), keySlots: [
			{ id: 'primary', kind: 'password', wrappedKey: crypto.randomBytes(48).toString('base64'), kdf: kdf(), owner: t % 2 === 0 },
			{ id: 'reader', kind: 'readonly', wrappedKey: crypto.randomBytes(48).toString('base64'), kdf: kdf(), keyfileName: t % 2 ? 'key.bin' : undefined },
			{ id: 'pw2', kind: 'password', wrappedKey: crypto.randomBytes(48).toString('base64'), kdf: kdf(), webauthn: t % 2 ? { credentialId: 'c' + t, prfSalt: 's' + t } : undefined },
			{ id: 'dev1', kind: 'member', wrappedKey: crypto.randomBytes(48).toString('base64'), kdf: kdf(), owner: true } ] } };
		if (V.manifestSealInput(manifest) !== vp.manifestSealInput(manifest)) { mseOk = false; break; }
	}
	ok('manifestSealInput (v5, incl. the ML-DSA key, recoveryVersion + owner/keyfile/webauthn attributes) is byte-identical across both implementations', mseOk);

	// verify (Ed25519 + ML-DSA AND-combiner) + manifestSealVerifies end-to-end: a genuine hybrid seal verifies on both,
	// a tampered one fails on both.
	{
		const manifest = { integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub, pqPubkey: keypair.pqPub }, crypt: { salt: 'S', keySlots: [{ id: 'primary', kind: 'password', owner: true, keyfileName: 'k.bin', webauthn: { credentialId: 'c1', prfSalt: 'p1' }, wrappedKey: 'AA', kdf: { algo: 'argon2id', v: 1, memKiB: 65536, iterations: 3, parallelism: 4, hashLen: 32, salt: 's' } }] } };
		const input = V.manifestSealInput(manifest);
		const sig = Integrity.sign(keypair.priv, input);
		manifest.integrity.manifestSig = sig;
		manifest.integrity.manifestSigPq = Integrity.signPq(keypair.pqPriv, input); // the post-quantum half both verifiers now require
		...
		// v4 attribute coverage: flipping any folded governance/auth attribute must break the seal on BOTH implementations
		// — this is what stops a folder-level attacker from forging/stripping the owner marker, promoting a slot's kind,
		// or rebinding a keyfile/WebAuthn credential without the write seed.
		const flip = (label, mutate) => {
			const m = JSON.parse(JSON.stringify(manifest)); mutate(m.crypt.keySlots[0]);
			ok('a v4 seal fails on both when ' + label, V.manifestSealVerifies(m, keypair.pub, sig) === false && vp.manifestSealVerifies(m, keypair.pub, sig) === false);
		};
		flip('the owner marker is stripped', s => { delete s.owner; });
		flip('the slot kind is promoted', s => { s.kind = 'owner'; });
		flip('the keyfile name is rebound', s => { s.keyfileName = 'evil.bin'; });
		flip('the WebAuthn credential is rebound', s => { s.webauthn = { credentialId: 'evil', prfSalt: 'p1' }; });
	}
	...
	// team vault's bundle could read GENUINE on one and TAMPERED on the other.
	{
		...
		const roster = { members: { epoch: 4, keyGeneration: 1, ownerPubKey: keypair.pub, ownerPqPubKey: keypair.pqPub }, crypt: { keySlots: [{ id: 'm1', kind: 'member', role: 'read', owner: true, pubFp: 'p', sealed: 'S' }] } };
		{ const rs = Integrity.signHybrid(keypair.priv, keypair.pqPriv, V.rosterSealInput(roster)); roster.members.sig = rs.sig; roster.members.sigPq = rs.sigPq; }
		...
		// rosterSealNewer must agree on both sides: a roster whose members.sealVersion is beyond what the verifier knows
		// is refused FORWARD (surfaced as UNVERIFIED by the caller), never falsely reported TAMPERED. This mirrors the
		// manifest-seal sealVersion gate. If one side treated a newer roster as current, it would reconstruct the wrong
		// input bytes and false-alarm. Cover current, newer, and absent markers.
		const rosterAt = (sv) => ({ members: { epoch: 1, keyGeneration: 0, ownerPubKey: keypair.pub, sealVersion: sv } });
		ok('rosterSealNewer agrees on a current-version roster (not newer, on both)', V.rosterSealNewer(rosterAt(1)) === false && vp.rosterSealNewer(rosterAt(1)) === false);
		ok('rosterSealNewer flags a newer-version roster on both', V.rosterSealNewer(rosterAt(999)) === true && vp.rosterSealNewer(rosterAt(999)) === true);
		ok('rosterSealNewer treats an absent marker as not-newer on both', V.rosterSealNewer({ members: { ownerPubKey: keypair.pub } }) === false && vp.rosterSealNewer({ members: { ownerPubKey: keypair.pub } }) === false);
		// End-to-end refuse-forward: a fully-signed roster whose sealVersion is bumped beyond the verifier's knowledge must
		// come back UNVERIFIED (not TAMPERED) from the standalone verifyBundle. Build a minimal genuine bundle in a temp
		// dir, then bump only members.sealVersion and confirm the verdict is UNVERIFIED with the bundle-format reason.
		{
			const os = require('os'), fs = require('fs'), path = require('path');
			const { verifyBundle } = require('../verify-bundle');
			const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vdisk-vbparity-'));
			try {
				const roster2 = { members: { epoch: 2, keyGeneration: 1, ownerPubKey: keypair.pub, ownerPqPubKey: keypair.pqPub }, crypt: { salt: 'S', keySlots: [{ id: 'primary', kind: 'password', wrappedKey: 'AA', kdf: { algo: 'argon2id', v: 1, memKiB: 65536, iterations: 3, parallelism: 4, hashLen: 32, salt: 's' } }, { id: 'm1', kind: 'member', role: 'read', owner: true, pubFp: 'p', sealed: 'S' }] }, integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub, pqPubkey: keypair.pqPub } };
				{ const rs = Integrity.signHybrid(keypair.priv, keypair.pqPriv, V.rosterSealInput(roster2)); roster2.members.sig = rs.sig; roster2.members.sigPq = rs.sigPq; }
				// A genuine, current baseline over an empty file set (root === empty-set root), signed (hybrid) with the same key.
				const emptyRoot = await Integrity.merkleRoot([]);
				const header = { scheme: Integrity.SCHEME, root: emptyRoot, seq: 1, prevRoot: null, count: 0, createdAt: new Date().toISOString(), version: require('../Vault')._bundleVersions.BASELINE_VERSION, sealed: false, deep: true };
				const bsig = Integrity.signHybrid(keypair.priv, keypair.pqPriv, Integrity.signingInput(header));
				const baseline = Object.assign({ sig: bsig.sig, sigPq: bsig.sigPq, files: [], merkleRoot: emptyRoot }, header);
				const write = (f, o) => fs.writeFileSync(path.join(dir, f), JSON.stringify(o));
				write('manifest.json', roster2); write('baseline.json', baseline);
				const genuine = verifyBundle(dir, null);
				ok('a genuine current roster bundle verifies (roster-signature ok, not refused forward)', genuine.checks.find(c => c.name === 'roster-signature').ok === true && genuine.verdict !== 'UNVERIFIED');
				// Bump ONLY the readable marker beyond the verifier's knowledge: the roster signature no longer reconstructs,
				// but the verdict must be UNVERIFIED (update the verifier), not TAMPERED.
				const bumped = JSON.parse(JSON.stringify(roster2)); bumped.members.sealVersion = 999; write('manifest.json', bumped);
				const forward = verifyBundle(dir, null);
				ok('a newer-format roster bundle is UNVERIFIED, not TAMPERED (refuse-forward)', forward.verdict === 'UNVERIFIED' && forward.checks.find(c => c.name === 'roster-signature').ok === true && forward.checks.some(c => c.name === 'bundle-format' && !c.ok));
	...
	// a genuine current bundle. This is the fast static guard the end-to-end bundle test (engine-gated) cannot be.
	{
		...
		let vOk = true; const keys = ['BASELINE_VERSION', 'SEAL_VERSION', 'ROSTER_SEAL_VERSION', 'ATTEST_VERSION', 'SUCCESSION_VERSION', 'GENESIS_VERSION', 'FILE_PROOF_VERSION', 'SCHEME'];
	...
	// (UNVERIFIED — the refuse-forward gate), and a wrong expected origin (TAMPERED).
	{
		...
		// The `anchored` flag lets the caller qualify a GENUINE verdict: false means "self-consistent only" (no recorded
		// origin identity was supplied to anchor the lineage), true means it was checked against a known genesis. Both
		// verifiers must agree, or the CLI caveat would appear on one path and not the other.
		ok('anchored is false when no origin identity is supplied (both verifiers agree)', Vf(proof).anchored === false && Bf(proof).anchored === false);
		{
			const rv = Vf(proof, { expectIdentity: id }), rb = Bf(proof, id);
			ok('anchored is true and the verdict stays GENUINE when the matching origin identity is supplied (both verifiers)', rv.anchored === true && rb.anchored === true && rv.verdict === 'GENUINE' && rb.verdict === 'GENUINE');
		}
	}

	// --- The STANDALONE verifier's post-quantum half must itself reject a downgrade. The rest of this suite tampers the
	//     CLASSICAL half (a salt/root change), so a regression that reduced verify-bundle's verifyHybrid to "return true
	//     after the Ed25519 check" (dropping the ML-DSA verify) would pass every other assertion. Exercise the PQ half
	//     in isolation on the offline verifier: a genuine pair verifies, but a stripped or byte-flipped ML-DSA half with
	//     an intact classical half must FAIL — otherwise a future Ed25519 forger could strip the post-quantum signature
	//     and read GENUINE on the portable verifier. On a runtime that genuinely lacks ML-DSA this classical-only accept
	//     is correct (there is nothing to strip), so gate the strict assertions on the verifier's own pqAvailable(). ---
	{
		const seed = require('crypto').randomBytes(32).toString('base64');
		const k = Integrity.signKeysFromSeed(seed);
		const payload = 'pq-downgrade-probe\n7\nroot';
		const h = Integrity.signHybrid(k.priv, k.pqPriv, payload); // a genuine hybrid pair
		ok('standalone verifyHybrid accepts a genuine pair', vb.verifyHybrid(k.pub, k.pqPub, payload, h.sig, h.sigPq) === true);
		if (vb.pqAvailable()) {
			// classical half intact, PQ half stripped -> must be rejected (the downgrade the classical-only tampers miss)
			ok('standalone verifyHybrid rejects a STRIPPED post-quantum half (classical intact)', vb.verifyHybrid(k.pub, k.pqPub, payload, h.sig, null) === false);
			const flipped = Buffer.from(h.sigPq, 'base64'); flipped[0] ^= 0xff; // byte-flip the ML-DSA signature
			ok('standalone verifyHybrid rejects a FORGED post-quantum half (classical intact)', vb.verifyHybrid(k.pub, k.pqPub, payload, h.sig, flipped.toString('base64')) === false);
			ok('standalone verifyPq itself rejects a wrong-key signature', vb.verifyPq(Integrity.signKeysFromSeed(require('crypto').randomBytes(32).toString('base64')).pqPub, payload, h.sigPq) === false);
		} else {
			console.log('  skip  standalone PQ-downgrade strict checks (this runtime lacks ML-DSA)');
		}
		// The classical half is always required, on every runtime — a stripped/garbage Ed25519 half never passes.
		ok('standalone verifyHybrid always rejects a bad classical half', vb.verifyHybrid(k.pub, k.pqPub, payload, 'AAAA', h.sigPq) === false);
	}
