...
async function main() {
	...
	// sealTag governs the current (v2) manifest-seal input; it must be byte-identical to Kdf.sealTag over all fields.
	let stOk = true;
	for (let t = 0; t < 60; t++) {
	...
	// manifestSealInput (v2) and its legacy v1 form over representative manifests, including a MEMBER slot (which must
	// be excluded from the seal) alongside passphrase-family slots with varied KDF params.
	...
	for (let t = 0; t < 20; t++) {
		...
		const manifest = { integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub }, crypt: { salt: crypto.randomBytes(16).toString('base64'), keySlots: [
		...
		if (V.manifestSealInput(manifest) !== vp.manifestSealInput(manifest) || V.manifestSealInputLegacy(manifest) !== vp.manifestSealInputLegacy(manifest)) { mseOk = false; break; }
	}
	ok('manifestSealInput (v2) and manifestSealInputLegacy (v1) are byte-identical', mseOk);
