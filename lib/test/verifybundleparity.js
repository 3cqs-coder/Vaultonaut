...
async function main() {
	...
	// team vault's bundle could read GENUINE on one and TAMPERED on the other.
	{
		...
		// dir, then bump only members.sealVersion and confirm the verdict is UNVERIFIED with the bundle-format reason.
		{
			...
			try {
				const roster2 = { members: { epoch: 2, keyGeneration: 1, ownerPubKey: keypair.pub }, crypt: { salt: 'S', keySlots: [{ id: 'primary', kind: 'password', wrappedKey: 'AA', kdf: { algo: 'argon2id', v: 1, memKiB: 65536, iterations: 3, parallelism: 4, hashLen: 32, salt: 's' } }, { id: 'm1', kind: 'member', role: 'read', owner: true, pubFp: 'p', sealed: 'S' }] }, integrity: { scheme: Integrity.SCHEME, pubkey: keypair.pub } };
				roster2.members.sig = Integrity.sign(keypair.priv, V.rosterSealInput(roster2));
				// A genuine, current baseline over an empty file set (root === empty-set root), signed with the same key.
				...
				const baseline = Object.assign({ sig: Integrity.sign(keypair.priv, Integrity.signingInput(header)), files: [], merkleRoot: emptyRoot }, header);
