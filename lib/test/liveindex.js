...
// It must not write on a read-only mount (nothing to keep fresh, and a reader must never write), and must only start
// once an index already exists (never build one behind the user's back).
ok('the watcher does nothing on a read-only mount', /function startIndexWatch\([^)]*\)\s*\{\s*if\s*\(readOnly\)\s*return;/.test(vault));
ok('the watcher only starts when a content index already exists', /startIndexWatch[\s\S]{0,600}fsp\.access\(searchIndexPathFor\(mountpoint\)\)/.test(vault));
...
// loop), not inline.
ok('the watcher debounces bursts before reindexing', /INDEX_WATCH_DEBOUNCE_MS/.test(vault) && /setTimeout\([^,]*contentReindex\(rec\.vault,\s*\{\s*allowOcr:\s*true\s*\}\)/.test(vault));
...
ok('the web refresh runs once per dialog session (guarded by a freshness marker)', /contentIndexFreshFor === searchTarget\)\s*return;/.test(app) && /contentIndexFreshFor = null;/.test(app));
