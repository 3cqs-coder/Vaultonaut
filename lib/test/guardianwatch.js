...
async function main() {
	// The REAL isOwnerAlive (the guardian mocks it below) must BOUND its heartbeat read: the data dir can be a per-user
	// home on a network/NFS mount that wedges, and this runs in the guardian's watch loop — an unbounded stat would hang
	// the loop and stop it ever reaching the FS-free isProcessAlive fast path, silently voiding the crash-safety net if
	// the service then died. Lock the bounding at the source (fast, deterministic) plus the FS-free verdict.
	const ioaBody = (require('fs').readFileSync(require('path').join(__dirname, '..', 'Vault.js'), 'utf8').match(/async function isOwnerAlive\([\s\S]*?\n}/) || [''])[0];
	ok('isOwnerAlive bounds the heartbeat read with a timeout race', /Promise\.race\(/.test(ioaBody) && /setTimeout\(/.test(ioaBody) && /r\(TIMED_OUT\)/.test(ioaBody));
	ok('a timeout reads as ALIVE (defer, never hang); an absent heartbeat reads as an orphan', /TIMED_OUT\) return true/.test(ioaBody) && /if \(!st\) return false/.test(ioaBody));
	ok('a non-service (cli) owner is left alone (reported alive)', (await Vault.isOwnerAlive('cli')) === true);
	...
	// --- FAIL-SAFE DEFAULT: the setting is bound as `wedgeRestart === true` (strict), so an ABSENT setting (a fresh
	// install, or a settings read that returned no such key) and any non-`true` value must default to lock, NEVER to
	// kill-and-relaunch. A regression to a truthy check (`!== false`, or `!!x`) would make the dangerous restart the
	// default. Cover both an absent setting and a truthy-but-not-true value; both must lock and leave the pid alone. ---
	for (const [desc, settings] of [['absent (default)', {}], ['a non-true truthy value', { wedgeRestart: 1 }]]) {
		fired = null; killed = null;
		process.kill = (pid, sig) => { killed = { pid, sig }; };
		Common.isProcessAlive = () => true;
		Vault.isOwnerAlive = async () => false;
		Vault.getSettings = async () => settings;
		Phase.readPhaseSync = () => null;
		try { await Guardian.watch(646464, 'owner-D'); } finally { process.kill = realKill; }
		ok('a wedged service with wedgeRestart ' + desc + ' defaults to lock, never kill', killed === null && fired && fired.owner === 'owner-D' && fired.wipeCache === true);
	}
