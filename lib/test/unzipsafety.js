...
async function main() {
	...
	ok('the cumulative inflation cap is keyed off the archive real on-disk size (fs.stat), not declared compressed sizes',
		/archiveBytes = await fsp\.stat\(zipPath\)/.test(netSrc) && /totalCap = Math\.max\([^,]*,\s*archiveBytes \* MAX_UNZIP_RATIO/.test(netSrc) && !/sumCsz/.test(netSrc));
	ok('each entry inflation ceiling is clamped to the total cap', /entryCap = Math\.min\(totalCap,/.test(netSrc));
