...
async function main() {
	// Source-level pin for the inflation-cap DESIGN. The behavioral bomb cases below exercise the per-entry ratio and
	// the floor, but the CUMULATIVE cap only bites above the 512 MiB floor — too large to build in a fast test. So lock
	// at the source that the cap is keyed off the archive's REAL on-disk size (a stat of the file), NEVER a sum of the
	// per-entry compressed sizes an attacker declares in the central directory, and that each entry's ceiling is clamped
	// to that total. A regression to the old `sumCsz`-keyed cap (which an over-declared entry could inflate) fails here.
	// The unzip implementation now lives in the shared lib/Zip.js (Net.unzip re-exports it), so pin the guard there.
	const zipSrc = fs.readFileSync(path.join(__dirname, '..', 'Zip.js'), 'utf8');
	ok('the cumulative inflation cap is keyed off the archive real on-disk size (fs.stat), not declared compressed sizes',
		/archiveBytes = await fsp\.stat\(zipPath\)/.test(zipSrc) && /totalCap = Math\.max\([^,]*,\s*archiveBytes \* MAX_UNZIP_RATIO/.test(zipSrc) && !/sumCsz/.test(zipSrc));
	ok('each entry inflation ceiling is clamped to the total cap', /entryCap = Math\.min\(totalCap,/.test(zipSrc));
	...
	// 9. An entry that OVER-declares its COMPRESSED size (a central-directory field an attacker controls) must not
	//    extract. It is refused — the declared size no longer buys a raised inflation ceiling (see the source pin above),
	//    and the over-declared size makes the reader run past the real body's end, so the archive is rejected rather than
	//    written out. Either way, nothing lands on disk. (The cumulative-cap CLAMP itself is pinned at the source above,
	//    since exercising it behaviorally would need an archive that inflates past the 512 MiB floor.)
	...
	ok('an entry that OVER-declares its compressed size does not extract (nothing written)', overRejected && !exists(path.join(tmp, 'out8', 'over.bin')));
