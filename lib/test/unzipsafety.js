...
// entries: [{ name, data, method: 'store'|'deflate', fakeUsz }]. `fakeUsz`, if set, is written as the DECLARED
// uncompressed size in place of the real one — so a test can forge an entry that lies about (or zeroes) its size to
// probe the inflation backstop. CRC-32 uses Node's native zlib.crc32 (fast on large data).
function buildZip(entries, opts = {}) {
	...
	for (const e of entries) {
		...
		lh.writeUInt32LE(crc, 14); lh.writeUInt32LE(body.length, 18); lh.writeUInt32LE(usz, 22);
		...
		cd.writeUInt32LE(crc, 16); cd.writeUInt32LE(body.length, 20); cd.writeUInt32LE(usz, 24);
