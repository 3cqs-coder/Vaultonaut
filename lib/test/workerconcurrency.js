'use strict';
// lib/test/workerconcurrency.js — a DRIFT GUARD against a subtle cross-platform hazard: opening several time-locked
// emergency grants CONCURRENTLY from a test. Each emergencyOpen runs the pairing-heavy time-lock core in a worker
// thread (see lib/Timelock.js -> lib/WorkerRun.js), so wrapping several opens in Promise.all spawns several worker
// threads at the same instant. Concurrent worker-thread spawn and teardown is a known native-crash trigger on some
// platforms (it surfaced as a Windows CI segfault). Production never does this — a real beneficiary opens one grant at
// a time — so a test that parallelizes opens is both unrealistic and a regression risk. This guard keeps every test's
// emergency opens sequential. A test that genuinely needs the concurrent pattern can opt out with a
// `worker-concurrency-ok` marker in the same call.
//
