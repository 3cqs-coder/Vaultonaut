...
async function main() {
	...
	// 2b. Hard attempt cap: the count of outbound SYNs per punch is bounded independently of the time budget, so a
	//     hostile hub cannot turn a client into an unbounded SYN source by relaying a large budget or a tiny interval.
	//     A HUGE budget with a tiny interval against a closed port must still give up quickly — the attempt cap, not the
	//     deadline, stops it — proven by resolving null in far less than the budget.
	{
		const t0 = Date.now();
		const r = await HolePunch.punch({ localPort: await freePort(), peerHost: '127.0.0.1', peerPort: await freePort(), budgetMs: 60000, intervalMs: 1, permitHost: anyHost });
		ok('a punch is bounded by a hard attempt cap, not just the time budget (stops well before a 60s budget)', r === null && (Date.now() - t0) < 5000);
	}
