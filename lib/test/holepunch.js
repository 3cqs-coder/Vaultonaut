...
async function main() {
	...
	//    direct connection with no listener on either side. This is the core hole-punch mechanism.
	{
		...
		const [ra, rb] = await Promise.all([
			HolePunch.punch({ localPort: A, peerHost: '127.0.0.1', peerPort: B, budgetMs: 4000, intervalMs: 40, permitHost: anyHost }),
			HolePunch.punch({ localPort: B, peerHost: '127.0.0.1', peerPort: A, budgetMs: 4000, intervalMs: 40, permitHost: anyHost }),
		]);
	...
	// 6. Cross-platform / non-blocking source contract: Node built-ins only (no native module, no spawned binary), no
	//    non-portable socket options (SO_REUSEPORT / reusePort throw on Windows and macOS), and no *Sync I/O.
	{
		...
		ok('HolePunch requires only net and ./PortMap (no native module)', !/require\('(?!net'|\.\/PortMap')[^']+'\)/.test(src));
