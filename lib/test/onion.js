...
async function main() {
	...
	// a custom line whose transport has no plugin is dropped rather than producing a half-configured Tor.
	{
		...
		try {
			...
			// Fallback branches: when the bundle's pt_config is missing/unreadable (a real user machine before a full
			// bundle), bridge handling must degrade SAFELY — no bridge args (run direct, never half-configured), and the
			// allowed modes fall back to the known list so the UI/CLI still work.
			TorSetup.readPtConfig = async () => { throw new Error('no pt_config'); };
			ok('bridgeArgsFor returns [] when pt_config is unreadable (safe degrade to direct)', (await Onion.bridgeArgsFor({ mode: 'obfs4' })).length === 0);
			const fb = await Onion.allowedBridgeModes();
			ok('allowedBridgeModes falls back to the known modes when pt_config is unavailable', fb.includes('off') && fb.includes('custom') && fb.includes('obfs4') && fb.includes('snowflake'));
			TorSetup.readPtConfig = async () => null;
			ok('bridgeArgsFor returns [] when pt_config is null', (await Onion.bridgeArgsFor({ mode: 'snowflake' })).length === 0);
