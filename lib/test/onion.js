...
async function main() {
	...
	ok('a changed bridge config restarts the managed Tor to match', /_managed\.bridgeSig === sig/.test(onionSrc) && /_managedServer\.bridgeSig === sig/.test(onionSrc));
	...
	ok('the bridge setting is validated through Onion sanitizers and defaults to off', /Onion\.KNOWN_BRIDGE_MODES\.includes\(b\.mode\)/.test(vault) && /Onion\.sanitizeBridgeLines/.test(vault));
	ok('the server exposes a tor-bridges endpoint that restarts the managed Tor on a change', /app\.post\('\/api\/tor-bridges'/.test(idx) && /setTorBridges\([\s\S]{0,120}stopManagedTor\(\)/.test(idx));
