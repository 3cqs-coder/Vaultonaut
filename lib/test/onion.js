...
async function main() {
	...
	ok('the serve ensures a Tor and publishes an onion on loopback (no TLS — the overlay encrypts)', /if \(onion && !\(relay && relay\.host\)\)[\s\S]{0,700}Onion\.provideTor\(\)[\s\S]{0,120}Onion\.publishOnion\(served0\.port/.test(vault));
