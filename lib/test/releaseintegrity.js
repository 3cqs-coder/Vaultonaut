...
async function main() {
	...
	// A wrong key must fail the signature — nobody can re-sign without the private key.
	{
		...
		const r = await RI.verifyInstall(tmp, { pubHex: otherPub, pqPubHex: null });
	...
	// Editing a covered file is detected (its hash no longer matches; the signature still verifies).
	{
		...
		await fsp.writeFile(path.join(tmp, 'lib', 'a.js'), 'module.exports = 1\n'); // restore
		ok('restoring the file clears the mismatch', (await RI.verifyInstall(tmp, { pubHex: pub, pqPubHex: null })).ok === true);
	}
	...
	// ADDING a covered file (not in the manifest) is detected as extraneous — an attacker dropping a file in.
	{
		...
		ok('removing the added file clears the extraneous finding', (await RI.verifyInstall(tmp, { pubHex: pub, pqPubHex: null })).ok === true);
	}
	...
	// The standalone verifier's exclude sets MUST match the library's, or extraneous detection would drift.
	{
		...
		// The standalone verifier's manifest schema + hash algo must ALSO match the library's, or a schema bump would
		// make verify.js silently report genuine releases as UNVERIFIED (fail-closed, but a silent false alarm).
		ok('verify.js KNOWN_SCHEMA matches lib/ReleaseIntegrity.js SCHEMA', verify.KNOWN_SCHEMA === RI.SCHEMA);
		ok('verify.js KNOWN_ALGO matches the library hash algorithm', verify.KNOWN_ALGO === 'sha256');
	}
	...
	// A future manifest FORMAT (new schema), correctly signed, must read as "newer version — update the tool", NOT as
	// TAMPERED. Otherwise the trust tool would raise a false alarm on a genuine future release.
	{
		const future = JSON.parse(buf.toString('utf8'));
		future.schema = 'vaultonaut-release-2';
		const futureBuf = RI.serialize(future);
		await fsp.writeFile(path.join(tmp, RI.MANIFEST_NAME), futureBuf);
		await fsp.writeFile(path.join(tmp, RI.SIG_NAME), RI.signManifest(futureBuf, seed) + '\n'); // validly signed with the REAL key — only the FORMAT is unknown
		const r = await RI.verifyInstall(tmp, { pubHex: pub, pqPubHex: null });
		ok('verifyInstall reports a newer manifest format as unknown, not a mismatch', r.present && r.signatureValid === true && r.unknownFormat === true);
		ok('verify.js returns UNVERIFIED (not TAMPERED) for a newer manifest format', verifyRelease(tmp, pub).verdict === 'UNVERIFIED');
		await fsp.writeFile(path.join(tmp, RI.MANIFEST_NAME), buf); // restore the genuine manifest
		await fsp.writeFile(path.join(tmp, RI.SIG_NAME), sig + '\n');
	}
	...
	// An UNSIGNED tree (no manifest) is inert — the boot check must say nothing (present:false).
	{
		...
		const r = await RI.verifyInstall(bare, { pubHex: pub, pqPubHex: null });
