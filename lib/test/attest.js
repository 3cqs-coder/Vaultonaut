...
async function main() {
	...
	// GeneralizedTime strictness: RFC 3161 / RFC 5280 require UTC. The trailing 'Z' is mandatory (an unqualified
	// time is LOCAL in ASN.1 — trusting it as UTC could shift the certified instant by hours), and out-of-range
	// calendar fields that Date.UTC would silently roll into a different instant must be refused, not accepted.
	const pgt = D.parseGeneralizedTime;
	ok('a proper UTC GeneralizedTime parses', pgt('20260901120000Z') instanceof Date && pgt('20260901120000Z').toISOString() === '2026-09-01T12:00:00.000Z');
	ok('a fractional-second UTC time parses (and cannot roll into the next second)', pgt('20260901120000.999Z') instanceof Date && pgt('20260901120059.99999Z').getUTCSeconds() === 59);
	ok('a time WITHOUT the mandatory Z is refused (not read as UTC)', pgt('20260901120000') === null);
	ok('a numeric zone offset is refused', pgt('20260901120000+0100') === null && pgt('20260901120000-05') === null);
	ok('surrounding whitespace is refused', pgt(' 20260901120000Z') === null && pgt('20260901120000Z ') === null);
	ok('an out-of-range month is refused (no silent rollover)', pgt('20261301120000Z') === null);
	ok('an out-of-range day is refused', pgt('20260932120000Z') === null);
	ok('an out-of-range hour is refused', pgt('20260901250000Z') === null);
