...
async function main() {
	...
	// OPAQUE vault: the folder on disk is named by the vaultId (reveals nothing) and the human name is stored encrypted
	// and seeded into the local cache at creation, so it shows immediately.
	const created = await vdisk.create(path.join(tmp, 'Photos.vault'), { password: 'pw2', opaque: true });
	const base = created.vault.split(/[\\/]/).pop().replace(/\.vault$/i, '');
	ok('an opaque vault\'s folder is named by a 32-hex vaultId, not the typed name', /^[0-9a-f]{32}$/.test(base) && !/Photos/.test(created.vault));
	const om = await Vault.readManifest(created.vault);
	ok('the opaque folder name equals the vault\'s own vaultId', base === Vault.vaultIdOf(om));
	ok('the typed name is stored as the encrypted title', (await vdisk.getVaultMeta(created.vault, 'pw2')).meta.title === 'Photos');
	ok('the opaque folder holds no plaintext of the name', !(await fsp.readFile(path.join(created.vault, 'vault.json'), 'utf8')).includes('Photos'));
	ok('the name is seeded into the local cache at creation (shows immediately, before any unlock)', ((await Vault.getSettings()).vaultNames[Vault.vaultIdOf(om)] || {}).title === 'Photos');
	ok('the display name resolves to the typed name from the cache', vdisk.vaultTitle(created.vault, ((await Vault.getSettings()).vaultNames[Vault.vaultIdOf(om)] || {}).title) === 'Photos');
	ok('the opaque vault still opens with its password', !!(await vdisk.getVaultMeta(created.vault, 'pw2')));
