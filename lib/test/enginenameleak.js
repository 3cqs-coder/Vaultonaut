...
function main() {
	...
	// The README is public and user-facing, so it must never name the engine either — it is always "the engine".
	const readme = (() => { try { return fs.readFileSync(path.join(__dirname, '..', '..', 'docs', 'README.md'), 'utf8'); } catch (_) { return ''; } })();
	ok('the README never names the engine (it is always "the engine")', readme.length > 0 && !NAME.test(readme));

	// No COMPETITOR product may be named to explain a technique, in the README or anywhere in lib/ (source or comments).
	// These are not integration targets or required drivers (which stay) — they are other tools whose name would only be
	// used to stand in for how something works, exactly what the standing rule forbids. Word-boundary matched so a
	// substring inside an ordinary word never trips it.
	// Clear competitor tools only. A functional integration or endpoint the code actually connects to (a cloud
	// provider, a drand randomness beacon such as drand.cloudflare.com, a required OS driver) is allowed and is NOT
	// listed here — this catches a name used to stand in for a technique, not a real integration target.
	const DENY = /\b(syncthing|resilio|tailscale|wireguard|cryptomator|veracrypt|truecrypt|gocryptfs|encfs|ngrok)\b/i;
	const libFiles = walk(path.join(__dirname, '..'), []).filter((f) => f.endsWith('.js') && !f.includes(path.sep + 'test' + path.sep));
	const named = [];
	for (const f of [path.join(__dirname, '..', '..', 'docs', 'README.md'), ...libFiles]) {
		let s; try { s = fs.readFileSync(f, 'utf8'); } catch (_) { continue; }
		if (DENY.test(s)) named.push(path.basename(f) + ' → ' + (s.match(DENY) || [''])[0]);
	}
	ok('no competitor product is named in the README or lib/ code and comments' + (named.length ? ': ' + named.join(', ') : ''), named.length === 0);
