...
// floor. Bind the README's human-facing "Node.js X.Y or newer" line to the enforced range's major.minor.
{
	...
	// The hardcoded "Node X.Y" strings in user-facing runtime messages (the post-quantum capability notices) must also
	// track the enforced floor, so a future bump can never leave a stale version in an error the user reads.
	const wrong = [];
	for (const rel of ['Emergency.js', 'Integrity.js', 'SelfCheck.js', 'verify-bundle.js']) {
		const s = fs.readFileSync(path.join(root, 'lib', rel), 'utf8');
		for (const m of s.matchAll(/Node(?:\.js)?\s+(\d+\.\d+)\+?/g)) if (m[1] !== min) wrong.push(rel + ': ' + m[1]);
	}
	ok('hardcoded "Node X.Y" version strings in user-facing messages match the enforced floor' + (wrong.length ? ' — stale: ' + wrong.join(', ') : ''), wrong.length === 0);
}
