...
for (const [name, blk] of Object.entries(THEMES)) {
	...
	// --muted is the token for READABLE secondary text (tile details, counts, timestamps, device notes, fine print).
	// It must clear AA (>=4.5) not only on the plain panels but also on the TINTED tile backgrounds it sits on (the
	// amber warn tile, the red error tile), in every theme — otherwise secondary text goes hard to read (which it did
	// when this text used the decorative --faint token). --faint stays for non-text/decorative use only.
	const panel2 = tok(blk, 'panel-2'), warnbg = tok(blk, 'warn-bg'), dangersoft = tok(blk, 'danger-soft'), muted = tok(blk, 'muted');
	ok(name + ' defines --muted, --panel-2, --warn-bg, --danger-soft', !!(muted && panel2 && warnbg && dangersoft));
	if (muted && panel2 && warnbg && dangersoft) {
		for (const [surf, bgc] of [['--panel', panel], ['--panel-2', panel2], ['--warn-bg', warnbg], ['--danger-soft', dangersoft]]) {
			const rm = ratio(muted, bgc);
			ok(name + ': --muted text clears AA (>=4.5) on ' + surf, rm >= 4.5, 'got ' + rm.toFixed(2));
		}
	}
}
