...
async function main() {
	console.log('[post-quantum capability check]');
	// The pq_signing check must be registered and must fire an ERROR finding when this runtime cannot do ML-DSA, so a
	// user on a Node built against pre-3.5 OpenSSL is told clearly instead of hitting raw crypto errors. Force the
	// capability off to exercise the failure path without such a runtime; restore it no matter what.
	ok('the pq_signing self-check is registered', SelfCheck.list().includes('pq_signing'));
	try {
		Integrity._forcePqAvailable(false);
		const f = await SelfCheck.run({ quiet: true, label: 'pqtest' });
		const pq = (f || []).find(x => x.check === 'pq_signing');
		ok('pq_signing fires an error finding when ML-DSA is unavailable', !!pq && pq.level === 'error' && /ML-DSA|post-quantum/i.test(pq.message || ''));
	} finally { Integrity._forcePqAvailable(null); }
	ok('pq_signing is silent when ML-DSA is available (this runtime)', !((await SelfCheck.run({ quiet: true, label: 'pqtest2' })) || []).some(x => x.check === 'pq_signing'));
	...
	// A normal (non-throwing) finding is collected with its EXACT message — register a probe that returns a warn, run,
	// and assert the message reaches the findings (not just a hard-coded true, which would pass even if collection broke).
	SelfCheck.register('_test_msg', () => ({ level: 'warn', message: 'collected-probe-message-xyz', fix: 'x' }));
	const withMsg = await SelfCheck.run({ quiet: true, label: 'msg' });
	ok('a normal finding is collected with its exact message', withMsg.some(f => f.check === '_test_msg' && f.message === 'collected-probe-message-xyz'));
	SelfCheck.register('_test_msg', () => null); // retire the probe so it can't affect the runs below
	...
	try {
		...
		// preserveCorruptFile is the shared aside primitive behind the corrupt-file handling in the mount state, the
		// settings store, and the integrity ledger. Its return value is a promise the callers rely on: settings REFUSE
		// to overwrite (and instead surface an error) when it reports the corrupt file could not be preserved, so a
		// lock or I/O fault can never clobber the only copy of the encrypted off-site logins it holds. Pin both outcomes.
		const src = path.join(tmp, 'c.json');
		await fsp.writeFile(src, 'payload');
		const dst = path.join(tmp, 'c.json.corrupt-x');
		ok('preserveCorruptFile moves an existing file aside and reports true', (await Common.preserveCorruptFile(src, dst)) === true && !fs.existsSync(src) && fs.readFileSync(dst, 'utf8') === 'payload');
		ok('preserveCorruptFile reports false when it cannot preserve (source absent, both move and copy fail)', (await Common.preserveCorruptFile(path.join(tmp, 'missing.json'), path.join(tmp, 'missing.corrupt'))) === false);
	...
	// This is the drift a vault deleted OUTSIDE the app leaves behind (secure-remove prunes its own arming).
	{
		...
		try {
			...
			if ((await vdisk.doctor()).engine.ok) {
				...
				const armRes = await Vault2.emergencyArm(present, { password: 'pw' });
				...
				// Contact drift: the grant is armed to a beneficiary who is no longer enrolled, while the vault is still
				// present. Removing a contact through the app drops its own grants, but a legacy grant or a hand-edited
				// settings file can leave this dangling — a release no one could open. Enroll a SECOND contact (so the
				// roster stays non-empty) then drop the armed grant's contact from the roster by hand, keeping the grant.
				// The watchdog must flag it even though the vault itself is present.
				const contact2 = require('../Emergency').generateContactKeypair();
				await Vault2.emergencyAddContact({ label: 'D', pubKey: contact2.publicKey });
				const sNow = await Vault2.getSettings();
				const trimmed = { ...(sNow.emergency.contacts || {}) };
				delete trimmed[armRes.contactId];
				await Vault2.setSettings({ emergency: { ...sNow.emergency, contacts: trimmed } });
				const orphanContact = (await SelfCheck.run({ quiet: true, label: 'emg-contact' })).filter(f => f.check === 'orphaned_emergency_arming');
				ok('the watchdog flags an arming whose beneficiary is no longer enrolled', orphanContact.length === 1 && orphanContact[0].level === 'warn' && /no longer enrolled/.test(orphanContact[0].message));
				...
				ok('the watchdog flags an emergency arming whose vault is gone', orphaned.length === 1 && orphaned[0].level === 'warn' && /no longer present/.test(orphaned[0].message));
	...
	console.log('[schema-version watchdog warns (never crashes) on a newer-schema security record]');
	{
		const svOrigDataDir = Common.dataDir, svOrigStatePath = Common.statePath;
		try {
			const svDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'vdisk-sv-'));
			Common.dataDir = () => svDir; Common.statePath = () => path.join(svDir, 'state.json');
			// The tamper/rollback ledger written by a NEWER build (schemaVersion above this build's) must produce a plain
			// WARN telling the user to update — NOT a swallowed "could not run". The schema_versions callback once used
			// Brand.name without requiring Brand in its scope, so it threw a ReferenceError at exactly this moment and the
			// per-check guard turned the security warning into a generic failure. Pin the warn, and that it is not swallowed.
			await fsp.writeFile(path.join(svDir, 'integrity.json'), JSON.stringify({ schemaVersion: Common.SCHEMA_VERSION + 100 }));
			const sv = await SelfCheck.run({ quiet: true, label: 'sv' });
			const svFinding = sv.filter(f => f.check === 'schema_versions');
			ok('schema_versions warns (does not crash) on a newer-schema security record', svFinding.length >= 1 && svFinding.every(f => f.level === 'warn'));
			ok('the schema_versions warning is not swallowed as a generic "could not run"', !sv.some(f => f.check === 'schema_versions' && /could not run/.test(f.message)));
			await fsp.rm(svDir, { recursive: true, force: true }).catch(() => {});
		} finally { Common.dataDir = svOrigDataDir; Common.statePath = svOrigStatePath; }
	}
	...
	console.log('[forward-clock-jump watchdog surfaces a recent jump]');
	{
		...
		try {
			...
			// The clear of guardianDownAt must be UNCONDITIONAL on a successful (re)launch, not gated on THIS process
			// having warned — guardianWarned resets per process, so a healthy new process must still clear a stale flag a
			// prior process left after a transient failure, or the alarm falsely persists up to a day. Source-pin it so a
			// refactor cannot move the clear back inside the `if (guardianWarned)` block.
			const idxSrc = fs.readFileSync(path2.join(__dirname, '..', 'webserver', 'index.js'), 'utf8');
			const egBody = (idxSrc.match(/function ensureGuardian\(\)[\s\S]*?\n}/) || [''])[0];
			ok('ensureGuardian clears guardianDownAt unconditionally on a successful launch (self-clearing at boot)', /guardianPid = Guardian\.launch[\s\S]*?setSettings\(\{ guardianDownAt: null \}\)[\s\S]*?if \(guardianWarned\)/.test(egBody));
			// A boot sweep (resumeRekey via Vault.sweep) must run BEFORE the startup self-check, so a cleanly-interrupted
			// key rotation gets its automatic resume a chance before stuck_rekey reports it — otherwise it false-alarms at
			// boot. Pin that the startup self-check is chained after the sweep.
			ok('the startup self-check runs after a boot sweep (so stuck_rekey does not false-alarm before resume)', /Vault\.sweep\(\)[\s\S]{0,200}SelfCheck\.run\(\{ label: 'startup' \}\)/.test(idxSrc));
	...
	console.log('[runtime-health watchdogs: event-loop lag + heap headroom]');
	{
		// Both act on a live runtime signal through an overridable seam, so the firing path is driven deterministically
		// here rather than by really stalling the loop or exhausting the heap (which would be slow and flaky across
		// platforms). Pin for each: registered, fires past its threshold, silent under it, and silent when the signal is
		// unavailable — plus that neither false-alarms on this idle runtime, a guard against a threshold set too low.
		const has = async (check, label) => (await SelfCheck.run({ quiet: true, label })).some(f => f.check === check);

		ok('event_loop_lag is registered', SelfCheck.list().includes('event_loop_lag'));
		try {
			SelfCheck._forceLoopLagReading(() => ({ maxMs: 1500, meanMs: 30 }));   // a 1.5s worst stall
			ok('event_loop_lag fires on a long main-thread stall', await has('event_loop_lag', 'lag-on'));
			SelfCheck._forceLoopLagReading(() => ({ maxMs: 200, meanMs: 25 }));     // ordinary GC-scale jitter
			ok('event_loop_lag is silent on jitter below the threshold', !(await has('event_loop_lag', 'lag-lo')));
			SelfCheck._forceLoopLagReading(() => null);                            // monitor unavailable on this runtime
			ok('event_loop_lag is silent when the loop monitor is unavailable', !(await has('event_loop_lag', 'lag-na')));
		} finally { SelfCheck._forceLoopLagReading(null); }

		ok('memory_headroom is registered', SelfCheck.list().includes('memory_headroom'));
		try {
			SelfCheck._forceHeapReading(() => ({ used_heap_size: 95, heap_size_limit: 100 }));  // 95% of the ceiling
			ok('memory_headroom fires when the heap nears its ceiling', await has('memory_headroom', 'heap-hi'));
			SelfCheck._forceHeapReading(() => ({ used_heap_size: 50, heap_size_limit: 100 }));  // ample headroom
			ok('memory_headroom is silent with ample heap headroom', !(await has('memory_headroom', 'heap-lo')));
			SelfCheck._forceHeapReading(() => null);                                            // stats unavailable
			ok('memory_headroom is silent when heap statistics are unavailable', !(await has('memory_headroom', 'heap-na')));
		} finally { SelfCheck._forceHeapReading(null); }

		// At rest on this runtime, with the real readers restored, neither may fire. Reset the loop-delay window through a
		// deliberately quiet pause first, so a stall earlier in this test run (or a busy CI box) cannot bleed in and make
		// this deterministic on every platform.
		await SelfCheck.run({ quiet: true, label: 'lag-reset' });       // the real event_loop_lag read resets its window
		await new Promise(r => setTimeout(r, 60));                       // a quiet window with no synchronous work
		const atRest = (await SelfCheck.run({ quiet: true, label: 'rest' })).map(f => f.check);
		ok('neither runtime-health check false-alarms at rest on this runtime', !atRest.includes('event_loop_lag') && !atRest.includes('memory_headroom'));
	}
