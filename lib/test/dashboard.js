...
async function main() {
	...
	console.log('[model: tiles that manage a subsystem carry an action (launchpad)]');
	{
		...
		ok('the connectivity tile links to the devices view', t.connectivity.action === 'devices');
	...
	console.log('[wiring: renderer stays on the quiet signature-diff path; state feeds the gauge from the cache]');
	{
		...
		ok('renderDashboard writes only when the tiles change (signature-diff)', /function renderDashboard\([\s\S]{0,1200}renderDashboard\._sig === sig/.test(app) && /renderDashboard\._sig = sig/.test(app));
		...
		ok('the shared model is loaded before app.js', /shared\/dashboard-model\.js[\s\S]{0,400}js\/app\.js/.test(ejs));
	...
	console.log('[model: performance tile + sparkline data]');
	{
		const perf = [];
		for (let i = 0; i < 8; i++) perf.push({ at: Date.now() + i, loopLagMs: 20 + i, heapPct: 40 + i, rssBytes: 5e7 });
		const t = tilesById({ perf, vaults: [], doctor: {} });
		ok('the performance tile summarizes the newest sample', t.perf && t.perf.value === '47% heap' && /Event-loop lag 27 ms/.test(t.perf.detail));
		ok('it carries both sparkline series', t.perf.spark && t.perf.spark.heap.length === 8 && t.perf.spark.lag.length === 8);
		ok('it is good under the thresholds', t.perf.level === 'good');
		const hot = tilesById({ perf: [{ at: 1, loopLagMs: 1500, heapPct: 95, rssBytes: 1 }], vaults: [], doctor: {} });
		ok('it warns when heap or lag crosses the self-check thresholds', hot.perf.level === 'warn');
		ok('it is omitted before any sample exists', !tilesById({ vaults: [], doctor: {} }).perf);
	}

	console.log('[model: overall() drives the header status pill]');
	{
		ok('overall is ok when nothing is wrong', Model.overall([{ level: 'good' }, { level: 'info' }, { level: 'neutral' }]) === 'ok');
		ok('overall is warn when any tile warns (and none errors)', Model.overall([{ level: 'good' }, { level: 'warn' }, { level: 'info' }]) === 'warn');
		ok('overall is error when any tile errors (error beats warn)', Model.overall([{ level: 'warn' }, { level: 'error' }]) === 'error');
		ok('an available update alone does not raise the pill (info is not trouble)', Model.overall([{ level: 'good' }, { level: 'info' }]) === 'ok');
	}
