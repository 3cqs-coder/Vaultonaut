...
async function main() {
	...
	// RACE / CLOBBER GUARD: two instances that pick the SAME free drive letter must get DIFFERENT marker files, so
	// the loser (whose `imdisk -a` fails because the letter is already taken) deletes only its OWN marker — never the
	// winner's, which would leave the winner's live RAM disk unswept if it later crashed. Force both provisions onto
	// 'R:' and make the second allocation fail, then confirm the winner's marker survives.
	await fsp.rm(markerDir, { recursive: true, force: true });
	await fsp.mkdir(markerDir, { recursive: true });
	const realAccess = fsp.access;
	fsp.access = (p) => (String(p).startsWith('R:') ? Promise.reject(Object.assign(new Error('ENOENT'), { code: 'ENOENT' })) : Promise.resolve()); // only R: reads as free -> both provisions pick it
	let allocs = 0;
	Rclone.exec = async (cmd, args) => { const line = cmd + ' ' + args.join(' '); calls.push(line); if (cmd === 'imdisk' && args.includes('-a')) { allocs++; return { status: allocs === 1 ? 0 : 1, stdout: '', stderr: allocs === 1 ? '' : 'drive letter in use' }; } return { status: 0, stdout: '', stderr: '' }; };
	try {
		const winner = await RamCache.provision('race1', { cacheSizeMB: 256 }); // first provision wins R:
		ok('the winning provision returns a handle for the raced letter', winner && winner.device === 'R:' && !!winner.markerToken);
		const loser = await RamCache.provision('race2', { cacheSizeMB: 256 }); // second picks R:, imdisk -a fails -> best-effort provision returns null (falls back to streaming)
		ok('the losing provision (letter already taken) returns null rather than a handle', loser === null);
		const left = (await fsp.readdir(markerDir)).filter(n => n.endsWith('.json'));
		ok('exactly one marker survives the race (the loser did not clobber the winner)', left.length === 1);
		let survivor = null; try { survivor = JSON.parse(await fsp.readFile(path.join(markerDir, left[0]), 'utf8')); } catch (_) {}
		ok('the surviving marker is the winner\'s (its device is still recorded for the sweep)', survivor && survivor.device === 'R:');
		ok('the surviving marker filename carries the winner\'s per-allocation token', left[0].includes(winner.markerToken));
	} finally { fsp.access = realAccess; }
