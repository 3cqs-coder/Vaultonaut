...
async function main() {
	...
	// Install the bundled engine up front. doctor() is a PASSIVE probe by design — it never triggers a download, so a
	// short-lived command can't hang on one — and on a fresh machine or CI runner with no cached engine the engine is
	// otherwise installed lazily on first real use (create/mount). Install it here so this end-to-end test does not
	// depend on ambient cache state and matches the documented standalone run. A failure here (e.g. offline) surfaces
	// as the "engine available" check below.
	try { await vdisk.setup(); } catch (_) {}
	...
	const dueDaily = vdisk.isBackupDue({ mode: 'daily', hour: 9, minute: 0, dest: bdest }, new Date(Date.UTC(2026, 2, 8, 9, 0)));   // at the scheduled UTC time
	const notYet = vdisk.isBackupDue({ mode: 'daily', hour: 9, minute: 0, dest: bdest }, new Date(Date.UTC(2026, 2, 8, 8, 59)));    // before the scheduled UTC time
	ok('a daily schedule is due at its scheduled UTC time and not before', dueDaily === true && notYet === false);
