...
function main() {
	...
	for (const name of ['localHmToUtc', 'utcHmToLocal', 'schedTz']) {
	...
	// Daylight-saving-correct schedules: a daily schedule also carries the machine's IANA zone and the intended LOCAL
	// time, so the backend fires it at the same wall-clock time year-round instead of sliding an hour across a clock
	// change. These are stored ADDITIVELY — the UTC hour/minute stay, so an older client keeps reading the schedule.
	ok('the web backup save also stores the zone and intended local time', /localHour: h, localMinute: m, tz: schedTz\(\)/.test(app));
	ok('the web scrub save also stores the zone and intended local time', /localHour: 3, localMinute: 0, tz: schedTz\(\)/.test(app));
	ok('the CLI also stores the zone and intended local time', /localHour: hour, localMinute: minute, tz: schedTz\(\)/.test(cmds));
	// Backend: the daily due-gate has a zone-aware path (evaluates the local wall-clock in the stored IANA zone) and it
	// only kicks in when the zone is valid, otherwise it falls back to the UTC hour/minute — so old schedules are safe.
	ok('the backend evaluates the daily time in the stored zone when present', /validTz\(sched\.tz\)[\s\S]{0,600}zonedWallToUtc\(/.test(vault));
	ok('the backend still keeps the daily hour/minute in UTC for backward compatibility', /hour: mode === 'daily' \? clampInt\(src\.hour/.test(vault));
	// The zone-aware helpers never throw on a bad zone (they return null/false and the caller falls back to UTC).
	ok('the zone helpers fail closed on an unknown zone', /function validTz\([\s\S]{0,220}catch \(_\) \{ return false; \}/.test(vault) && /function tzOffsetMsAt\([\s\S]{0,900}catch \(_\) \{ return null; \}/.test(vault));
