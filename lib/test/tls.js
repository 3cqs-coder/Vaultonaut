...
async function main() {
	...
	hub = Relay.runHub({ controlPort: 17480, token: 'tok', host: '127.0.0.1', portRange: [21500, 21510] });
	await sleep(300);
	const peerDir = path.join(tmp, 'nodeB');
	handle = await vdisk.serveVault(peerDir, { relay: { host: '127.0.0.1', port: 17480, token: 'tok' } });
