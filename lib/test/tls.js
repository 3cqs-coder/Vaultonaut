...
async function main() {
	...
	hub = Relay.runHub({ controlPort, token: 'relay-tls-token-0123456789', host: '127.0.0.1', portRange: [pub, pub] });
	await sleep(300);
	const peerDir = path.join(tmp, 'nodeB');
	handle = await vdisk.serveVault(peerDir, { relay: { host: '127.0.0.1', port: controlPort, token: 'relay-tls-token-0123456789' } });
