...
// it REFUSES for an unmounted vault (so the UI never broadcasts an unmounted folder's on-disk footprint, which
// would reveal a large real vault hidden behind a small decoy), and when mounted it measures the DECRYPTED contents
// and EXCLUDES the self-healing recovery data (whose parity size scales with the real data). This pins both the
// refusal (no mount driver needed) and, when a driver is present, the content measurement and healing exclusion.
//
...
async function main() {
	...
	try {
		...
		ok('a mounted vault reports its decrypted content size (about the files it holds)', r.bytes >= CONTENT && r.bytes < CONTENT + 15000);
