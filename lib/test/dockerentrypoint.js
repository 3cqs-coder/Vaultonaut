...
async function main() {
	...
	// ── Health probe ──
	// No port configured means this role has nothing to probe (e.g. a relay-joined node, or a direct `docker run`
	// without HEALTHCHECK_PORT): report HEALTHY so the container is not marked unhealthy and restart-looped. A port
	// that IS set but invalid is a real misconfiguration and still fails.
	ok('healthcheck exits 0 (nothing to probe) when HEALTHCHECK_PORT is unset', (await runHealth({ HEALTHCHECK_PORT: '' })) === 0);
	ok('healthcheck exits non-zero on an out-of-range port', (await runHealth({ HEALTHCHECK_PORT: '999999' })) === 1);
	// Swallow errors on accepted sockets: the health probe connects then closes at once, and on Windows that abrupt
	// close arrives as an RST, so the server's accepted socket emits ECONNRESET — which would be an unhandled 'error'
	// (crashing the test) without this handler.
