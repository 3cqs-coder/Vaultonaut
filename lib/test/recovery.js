...
async function main() {
	...
	// The DESTRUCTIVE path: an older build must not rebuild OVER a newer-format index (that would overwrite recovery
	// data it cannot understand). protect must refuse with RECOVERY_NEWER and leave the on-disk index untouched — this
	// is the one acting path whose failure loses data, so it is guarded separately from the read-only verify/heal above.
	const anIndex = path.join(recDir, (await fsp.readdir(recDir)).find(n => /^index/.test(n)));
	const idxBefore = await fsp.readFile(anIndex);
	let protectRefused = false;
	// The error crosses the recovery worker boundary, which preserves the message but not a custom .code — so assert on
	// the message, exactly as the verify/heal refusals above do.
	try { await Recovery.protect(v, { tier: 'low' }); } catch (e) { protectRefused = /newer version/i.test(e.message); }
	ok('protect refuses to rebuild over a newer recovery format instead of overwriting it', protectRefused);
	ok('the newer-format index is left untouched after the refused protect', (await fsp.readFile(anIndex)).equals(idxBefore));
