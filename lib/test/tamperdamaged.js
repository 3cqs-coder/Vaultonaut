...
async function main() {
	...
	// Corrupt ONE file's ciphertext header in the encrypted store (bad magic), the same shape self-healing leaves when
	// a loss is larger than its recovery budget. The blob keeps its size and its valid encrypted NAME, so it still
	// lists and decodes its name — only its content can no longer be read.
	const cipherDir = path.join(v, 'data');
	const biggest = (await blobs(cipherDir)).sort((a, b) => b.size - a.size)[0];
	const fh = await fsp.open(biggest.p, 'r+'); await fh.write(Buffer.from('XXXXXXXX'), 0, 8, 0); await fh.close();
	...
	if (rep) {
		...
		ok('the damaged file is called out as tampering/damage, not left silent', rep.tamper.some(t => /damaged/i.test(t)) && rep.clean === false);
