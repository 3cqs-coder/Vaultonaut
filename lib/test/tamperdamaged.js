...
async function main() {
	...
	// Now the OTHER damage shape, verified against the LIVE pinned engine (not just the hand-written fixtures in
	// hashsumparse.js): a corrupt HEADER. Overwrite the first bytes of a SECOND blob (the crypt magic "RCLONE\x00\x00"),
	// so the engine cannot even recognize it as an encrypted file and prints the "not an encrypted file - bad magic
	// string" line plus the counted "with N errors" summary. If a future engine re-pin changed that wording, HASHSUM_DAMAGE
	// would silently stop matching and damage would flip to a fatal abort — this catches that, which the string fixtures
	// alone cannot. The blob keeps its size, so it must still read as damaged (not removed), now alongside big.bin.
	const second = (await blobs(cipherDir)).sort((a, b) => b.size - a.size).find(x => x.p !== biggest.p);
	const fh2 = await fsp.open(second.p, 'r+'); await fh2.write(Buffer.from('ZZZZZZZZ'), 0, 8, 0); await fh2.close();
	let rep2, threw2 = null;
	try { rep2 = await vdisk.audit(v, { password: 'pw', deep: true }); } catch (e) { threw2 = e.message; }
	ok('a header-corrupt (bad-magic) file also does not abort the scan', threw2 === null);
	ok('the header-corrupt case did not surface "Could not scan the vault"', !(threw2 && /could not scan the vault/i.test(threw2)));
	if (rep2) {
		ok('both the body-corrupt and header-corrupt files are reported as damaged', Array.isArray(rep2.damaged) && rep2.damaged.length === 2 && rep2.damaged.includes('big.bin'));
		ok('two damaged files still make the result not-clean with no false tamper alarm', rep2.clean === false && rep2.tamper.length === 0);
		ok('neither damaged file is mis-reported as removed (sizes unchanged)', rep2.removed.length === 0 && rep2.added.length === 0);
	}
