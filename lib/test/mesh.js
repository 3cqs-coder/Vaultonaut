...
let workspace = null;
async function main() {
	const doc = await vdisk.doctor();
	if (!doc.engine.ok) { console.log('Engine missing — skipping.'); return; }
	// This test mounts to verify a secret reconstructed from key shares, so it needs a FUSE/WinFsp driver; skip
	// cleanly where none is installed (a CI runner, or any host without one) rather than fail on the mount.
	if (!doc.driver.ok) { console.log('No mount driver — skipping.'); return; }
