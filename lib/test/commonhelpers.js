...
async function main() {
	...
	for (const fn of ['mapLimit', 'fsyncPath', 'sha256', 'sha256Hex', 'windowsHiddenRunVbs', 'pathExistsBounded', 'writeJsonAtomic', 'renameWithRetry', 'uniqueTempPath', 'timingSafeEqualHashed', 'pathWithin', 'resolveThroughSymlinks', 'readFileCapped', 'foldPath', 'samePath', 'splitHostPort', 'isLoopbackHost', 'redactPaths']) {
	...
	const loopbackYes = ['127.0.0.1', '127.0.0.2', '127.1.2.3', '127.255.255.254', 'localhost', 'LOCALHOST', '::1', '', '  127.0.0.1  '];
	const loopbackNo = ['0.0.0.0', '128.0.0.1', '126.255.255.255', '192.168.1.10', '10.0.0.1', '::2', '227.0.0.1', '1127.0.0.1', '127.0.0.1.evil.com', '127.example.com', 'localhost.attacker.com', 'notlocalhost', '127.0.0.256', 'example.com'];
	ok('isLoopbackHost accepts every genuine loopback form (127/8, localhost, ::1, empty, trimmed)', loopbackYes.every(h => Common.isLoopbackHost(h) === true));
	ok('isLoopbackHost rejects non-loopback addresses and DNS-name spoofs', loopbackNo.every(h => Common.isLoopbackHost(h) === false));
