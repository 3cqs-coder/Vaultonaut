...
async function main() {
	...
	try {
		...
		ok('sameFile bounds its stat with withTimeout (never hangs on a wedged path)', /async function sameFile[\s\S]{0,900}withTimeout\(Promise\.all\(\[fsp\.stat/.test(commonSrc));
		ok('sameFile trusts inode identity only for non-zero inodes (no exFAT false-merge)', /async function sameFile[\s\S]{0,1400}sa\.ino && sb\.ino/.test(commonSrc));
