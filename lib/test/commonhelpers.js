...
async function main() {
	...
	for (const fn of ['mapLimit', 'fsyncPath', 'sha256', 'sha256Hex', 'windowsHiddenRunVbs', 'pathExistsBounded', 'writeJsonAtomic', 'renameWithRetry', 'uniqueTempPath', 'timingSafeEqualHashed', 'pathWithin', 'resolveThroughSymlinks', 'readFileCapped', 'foldPath', 'samePath', 'sameFile', 'splitHostPort', 'isLoopbackHost', 'redactPaths', 'openExternal']) {
	...
	try {
		...
		// sameFile decides identity by device+inode, so it is correct on any volume regardless of case behavior (unlike
		// the per-OS case-fold in samePath). Pin: a path equals itself; a symlink equals its target; two DISTINCT files
		// are never equal (this is what protects a case-sensitive volume from merging two genuinely different vaults);
		// and a missing path falls back to the lexical compare instead of throwing.
		const fA = path.join(tmp, 'A.txt'); await fsp.writeFile(fA, 'a');
		const fB = path.join(tmp, 'B.txt'); await fsp.writeFile(fB, 'b');
		const linkA = path.join(tmp, 'A-link.txt'); await fsp.symlink(fA, linkA);
		ok('sameFile: a path equals itself', (await Common.sameFile(fA, fA)) === true);
		ok('sameFile: a symlink resolves to the same file as its target', (await Common.sameFile(fA, linkA)) === true);
		ok('sameFile: two distinct files are not the same (protects a case-sensitive volume from merging vaults)', (await Common.sameFile(fA, fB)) === false);
		ok('sameFile: a null argument is never equal', (await Common.sameFile(fA, null)) === false);
		ok('sameFile: two missing paths fall back to the lexical compare (equal when identical)', (await Common.sameFile(path.join(tmp, 'gone'), path.join(tmp, 'gone'))) === true);
		ok('sameFile: two DIFFERENT missing paths are not equal (lexical fallback)', (await Common.sameFile(path.join(tmp, 'gone1'), path.join(tmp, 'gone2'))) === false);
		// Wedge-safety and exFAT-safety are hard to exercise live (they need a hung mount / an inode-0 volume), so pin
		// them at the source: the identity stat MUST be bounded (so a wedged mount/network path can never hang a
		// caller — a regression here would re-freeze the mount-state lock and the schedule tick), and inode identity
		// must be trusted only for non-zero inodes (so distinct files on exFAT/FAT, which report inode 0, are not merged).
		const commonSrc = fs.readFileSync(path.join(__dirname, '..', 'Common.js'), 'utf8');
		ok('sameFile bounds its stat with withTimeout (never hangs on a wedged path)', /async function sameFile[\s\S]{0,1400}withTimeout\(pair, SAME_FILE_STAT_TIMEOUT_MS\)/.test(commonSrc));
		ok('sameFile trusts inode identity only for non-zero inodes (no exFAT false-merge)', /async function sameFile[\s\S]{0,2000}sa\.ino && sb\.ino/.test(commonSrc));
		// The bound above only limits the WAIT — it cannot cancel the stat, which keeps running on a libuv threadpool
		// thread until the backend answers. Without dedup, each probe of the SAME wedged path would pin a fresh thread
		// and could starve the small default pool, so sameFile must share ONE in-flight stat per resolved path (an
		// inflight map that clears when the syscall settles), mirroring Rclone.statDev. Pin the dedup at the source.
		ok('sameFile shares one in-flight stat per path via an inflight map (no threadpool starvation)', /sameFileStatInflight = new Map\(\)/.test(commonSrc) && /function sameFileSharedStat\([\s\S]{0,300}sameFileStatInflight\.set\([\s\S]{0,120}sameFileStatInflight\.delete\(/.test(commonSrc));
		ok('sameFile probes both paths through the shared-stat helper', /Promise\.all\(\[sameFileSharedStat\(ra\), sameFileSharedStat\(rb\)\]\)/.test(commonSrc));
	...
	// tildePath — collapses the user's home directory to "~" for display only, so the UI never shows the account name in
	// a full path. Exported, and correct on the real home dir plus synthetic cases across separators.
	{
		ok('tildePath is exported', typeof Common.tildePath === 'function');
		const home = os.homedir();
		ok('a path under home collapses to ~', Common.tildePath(path.join(home, 'Documents', 'x.vault')) === '~' + path.sep + path.join('Documents', 'x.vault'));
		ok('the home directory itself collapses to ~', Common.tildePath(home) === '~');
		ok('a path NOT under home is returned unchanged', Common.tildePath(path.join(path.sep + 'somewhere', 'else')) === path.join(path.sep + 'somewhere', 'else'));
		ok('a sibling directory that merely shares the home prefix is NOT collapsed', Common.tildePath(home + 'extra' + path.sep + 'x') === home + 'extra' + path.sep + 'x');
		ok('the tail keeps its real casing (only the home prefix is matched)', /Camera Uploads/.test(Common.tildePath(path.join(home, 'Camera Uploads'))));
		ok('an empty or nullish input is returned as-is', Common.tildePath('') === '' && Common.tildePath(null) === '');
	}
