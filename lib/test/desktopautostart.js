...
function main() {
	...
	// The handoff must also PROVE the server answering the port is that owner (proof of possession of the owner token),
	// not just trust the pidfile — otherwise a stale pidfile with a reused pid, or a squatter, could be shown. Pin that
	// the attach path awaits OwnerClient.verifyOwner before exiting with the attach code.
	ok('the handoff verifies the owner identity (proof of possession) before attaching', /runningOwner && Common\.isDesktopApp\(\) && await OwnerClient\.verifyOwner\(runningOwner\.url, runningOwner\.token\)/.test(cmds));
