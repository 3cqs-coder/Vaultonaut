...
const { hashsumDamagedFiles, hashsumErrorsAllPerFile, damagedNote } = require('../Vault')._hashsumParse;
...
// The note names a few offenders and caps the list.
{ const note = damagedNote(['a.bin', 'b.bin', 'c.bin', 'd.bin', 'e.bin', 'f.bin']); ok('the damaged note names up to five and says "and 1 more"', /a\.bin/.test(note) && /and 1 more/.test(note) && /damaged/i.test(note)); }
ok('the damaged note is singular for one file', /^1 file could not be read/.test(damagedNote(['only.bin'])));
