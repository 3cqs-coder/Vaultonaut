...
function main() {
	...
	// A few flags belong to the separate `verify.js` authenticity tool (documented in the README but not part of the
	// `vdisk` command surface), so they are excluded here rather than forced into the CLI's flag sets.
	const EXTERNAL_TOOL_FLAGS = new Set(['pubkey']); // flags of the standalone verify.js, not vdisk
	...
	ok('cmdServe rejects combining --lan/--wan with --bind', /\(flags\.lan \|\| flags\.wan\) && flags\.bind/.test(cmds));
	ok('cmdServe rejects combining --lan with --wan', /flags\.lan && flags\.wan/.test(cmds));
