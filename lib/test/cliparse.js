...
function main() {
	...
	// Strip Markdown link anchors (`](#...)`) before scanning: a heading that contains an em-dash produces an anchor with
	// a double hyphen (e.g. `#reproducible-builds--verify-it-yourself`), which is NOT a CLI flag but would otherwise match
	// the `--flag` pattern below and read as phantom doc drift.
	const readmeTxt = fs.readFileSync(path.join(__dirname, '..', '..', 'docs', 'README.md'), 'utf8').replace(/\]\(#[^)]*\)/g, '');
