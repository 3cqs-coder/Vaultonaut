...
async function main() {
	...
	// Pin the EXACT guard shape, scoped to the function body, so neither an inversion (if (sk && !sk.readOnly) throw —
	// which would refuse a WRITABLE session and allow a read-only one) nor a dropped throw slips through as a pass.
	const notesGuardBody = (vaultSrc.match(/async function assertNotesWritable\(vaultDir\)\s*\{[\s\S]*?\n\}/) || [''])[0];
	ok('the note-write guard refuses a read-only session (exact positive condition, throws, not inverted)', /if \(sk && sk\.readOnly\) throw new Error/.test(notesGuardBody) && !/!sk\.readOnly/.test(notesGuardBody));
	...
	try {
		...
		// BOUNDED-CONCURRENCY read: the list and the health scan now read notes with a small concurrency cap
		// instead of strictly one at a time. Create MORE notes than that cap so the read runs several waves, then confirm
		// every note appears exactly once — a concurrency bug (a dropped or duplicated slot) would surface here where a
		// tiny set would not.
		const bulkIds = new Set();
		for (let i = 0; i < 15; i++) { const r = await vdisk.noteSave(v, { title: "Bulk-" + i, note: "n" + i }); bulkIds.add(r.id); }
		const bulkList = (await vdisk.notesList(v)).notes.map(n => n.id);
		ok("every bulk-created note is listed exactly once (bounded-concurrency read is complete, no drop or dupe)", [...bulkIds].every(id => bulkList.filter(x => x === id).length === 1) && new Set(bulkList).size === bulkList.length);
	...
	// A READ-ONLY key holder may READ notes but must NOT write or delete them. The engine uses one data key for read
	// and write, so a store-direct note write would otherwise succeed and bypass the read-only promise the UI makes.
	// This is the behavioral counterpart to the source pin above: add a read-only key, mount WITH it, and confirm a save
	// and a delete are refused with the read-only message while the list still reads.
	await vdisk.addReadOnlyKey(v, { password: 'pw1', readOnlyPassword: 'ro-pw' });
	await vdisk.mount(v, { password: 'ro-pw', cacheDir });
	try {
		ok('a read-only session can still LIST notes', Array.isArray((await vdisk.notesList(v)).notes));
		let roSave = false, roDelete = false;
		try { await vdisk.noteSave(v, { title: 'nope', body: 'no' }); } catch (e) { roSave = /read-only/i.test(e.message); }
		try { await vdisk.noteDelete(v, 'a'.repeat(24)); } catch (e) { roDelete = /read-only/i.test(e.message); }
		ok('a read-only session cannot SAVE a note (the read-only promise holds)', roSave);
		ok('a read-only session cannot DELETE a note', roDelete);
	} finally { await vdisk.unmount(v, {}).catch(() => {}); }
