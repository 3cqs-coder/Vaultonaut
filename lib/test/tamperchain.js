...
//
// Pure unit test over Integrity — no engine needed. It uses throwaway vault ids in the real data files and
// removes them afterward.  Run:  node lib/test/tamperchain.js
...
async function cleanup() {
	try { const l = await readJson(LOG); for (const v of vids) delete l[v]; await writeJson(LOG, l); } catch (_) {}
	try { const g = await readJson(LEDGER); for (const v of vids) { delete g[v]; if (g.tamperHeads) delete g.tamperHeads[v]; } await writeJson(LEDGER, g); } catch (_) {}
}
