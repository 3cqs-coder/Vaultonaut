...
async function reason(vid, pub, pqPub) { return (await Integrity.verifyTamperLog(vid, pub, pqPub)).reason; }
...
async function main() {
	...
	let v = await Integrity.verifyTamperLog(vid, kp.pub, kp.pqPub);
	ok('a clean 3-entry chain verifies', v.ok === true);
	ok('an unsigned chain reports signed:false', v.signed !== true);
	await Integrity.signTamperHead(vid, kp.priv, kp.pqPriv);
	v = await Integrity.verifyTamperLog(vid, kp.pub, kp.pqPub);
	...
	vid = freshVid();
	for (const k of ['a', 'b', 'c']) await Integrity.logTamper(vid, { kind: k });
	await Integrity.signTamperHead(vid, kp.priv, kp.pqPriv); // checkpoint at n=3
	...
	ok('a rollback below a signed checkpoint is detected', (await reason(vid, kp.pub, kp.pqPub)) === 'rolled-back');
	...
	await Integrity.signTamperHead(vid, kp.priv, kp.pqPriv); // checkpoint at n=1
