...
async function main() {
	...
	//    no further snapshot is taken after it.
	{
		...
		// Corrupt EVERY ciphertext blob so the snapshot record is certainly among them. The record's name is encrypted,
		// so it cannot be singled out by name, and it is no longer reliably the smallest blob — it carries a full file
		// list plus BOTH signatures (the 3309-byte ML-DSA one makes it one of the LARGER blobs), so an old "smallest few"
		// heuristic would miss it. A byte flip in an authenticated-encryption blob fails to decrypt, so the reader sees
		// no readable record; audit must report that, never silently clear it.
		for (const p of blobs) { const buf = await fsp.readFile(p); if (buf.length) { buf[0] ^= 0xff; await fsp.writeFile(p, buf); } }
		const a = await vdisk.audit(v, { password: 'pw1' });
