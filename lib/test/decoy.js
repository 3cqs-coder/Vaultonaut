...
async function main() {
	...
	// must return a CURATED object, never the raw result. This pins the fix against a future refactor re-adding `return r`.
	{
		...
		ok('the /api/unmount handler does not return the raw unmount result (would leak `redirected`)', unmountBody && !/\n\t\t\treturn r;/.test(unmountBody));
		const curated = (unmountBody.match(/return r \? (\{[^}]*\}) : r;/) || [])[1] || '';
		ok('the /api/unmount response is a curated object that omits the `redirected` flag', !!curated && /ok: r\.ok/.test(curated) && !/redirected/.test(curated));
	}
