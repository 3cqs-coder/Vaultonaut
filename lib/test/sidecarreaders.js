...
async function main() {
	...
	// Self-isolate: put the known-vaults STATE inside this throwaway workspace (keeping the engine wherever it already
	// is, so nothing re-downloads), so `create` registers the vault in workspace/state.json — never the real or shared
	// data directory. Then even if this process is killed before the cleanup below runs (for example the suite's
	// per-test timeout SIGKILL, or a Ctrl+C on a direct run), the only leftover is a temp folder the OS reclaims, and
	// the user's real vault list is never touched. This holds whether the file is run through the suite (with the
	// _setup preload) or directly as its header shows.
	const engineDir = Common.binDir();
	Common.setDataDir(tmp);
	Common.setBinDir(engineDir);
...
main().catch(e => { console.error(e); process.exit(1); }).finally(async () => {
	// Remove the throwaway workspace. Because the data directory was pointed inside it, this also takes the
	// known-vaults state with it, so no phantom entry is left for a later test or the app's UI to see.
