...
async function main() {
	...
	// A bare --dest on backup-schedule used to SLIP the `!flags.dest` check (since `!true` is false) and persist a
	// recurring background backup to a folder literally named "true"; it must now be rejected outright. And a bare
	// --shards on reconstruct is rejected like its sibling --to. Regression guards for the whole value-flag class.
	ok('a bare --dest on backup-schedule is rejected (never a folder-named-"true" schedule)',
		await throwsMatching(() => dispatch('backup-schedule', ['/tmp/x.vault'], { dest: true }), /--dest needs a value/));
	ok('a bare --shards on reconstruct is rejected with a clear message',
		await throwsMatching(() => dispatch('reconstruct', [], { shards: true }), /--shards needs/));
