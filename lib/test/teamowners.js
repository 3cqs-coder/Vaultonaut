...
async function main() {
	...
	const pristine = await mfOf(v); // the authentic, correctly-signed manifest — restored after the tamper check below
	const tam = JSON.parse(JSON.stringify(pristine));
	...
	// Restore the authentic roster: every owner mutation now refuses to act on a roster that fails its owner signature
	// (a member could otherwise be re-sealed to a swapped key), so the regression checks below must run against the
	// valid vault, not the deliberately-corrupted one the tamper check left on disk.
	await fsp.writeFile(path.join(v, 'vault.json'), JSON.stringify(pristine));
	await fsp.writeFile(path.join(v, '.vault.bak'), JSON.stringify(pristine));
