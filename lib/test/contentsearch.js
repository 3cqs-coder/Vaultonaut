...
async function main() {
	...
	const attrBomb = denseElementZip(Buffer.from('<w:t ' + 'a="1" '.repeat(14 * 1024 * 1024) + '/>')); // ~14M attributes (over the node cap), ~84 MiB inflated (under the byte budget), only one '<'
	...
	const largeDoc = denseElementZip(Buffer.from('<w:t ' + 'a="1" '.repeat(5 * 1024 * 1024) + '/>')); // ~5M nodes: comfortably under the node cap and the byte budget
	...
	const storedDenseDD = (() => {
		const raw = Buffer.from('<w:t ' + 'a="1" '.repeat(13 * 1024 * 1024) + '/>'); // ~78 MiB stored, ~13M attributes (over the node cap)
		const name = Buffer.from('word/document.xml');
