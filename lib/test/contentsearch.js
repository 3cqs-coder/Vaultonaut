...
async function main() {
	...
	// `ratio` picks the content: a bomb (default) fills with one repeated byte so it compresses near DEFLATE's ~1032x
	// ceiling (tiny on disk, huge inflated); a realistic document (ratio: 'real') uses varied text that compresses only
	// a few times, so it stays under the per-entry inflation-ratio limit and models an honest file.
	const craftDeflateZip = (realBytes, declaredSize, eocdEntries = 1, ratio = 'bomb') => {
		const raw = ratio === 'real'
	...
	// The per-entry inflation-ratio limit (best-practice layer): a single entry expanding far beyond a real document's
	// ratio is refused even when its absolute size is under the total budget, so a bomb is caught early. An honest entry
	// with the same real size but a realistic ratio is accepted.
	ok('an entry that expands beyond the inflation-ratio limit is rejected even under the total budget', Extract.zipWithinBudget(craftDeflateZip(8 * 1024 * 1024, 12, 1, 'bomb')) === false);
	ok('an entry of the same real size with a realistic ratio is accepted', Extract.zipWithinBudget(craftDeflateZip(8 * 1024 * 1024, 12, 1, 'real')) === true);
	// The element-count (DOM-amplification) layer: the readers build one heavy in-memory node per XML tag, so a
	// tag-dense part can exhaust the worker heap even while its INFLATED BYTES sit under the total budget and its ratio
	// under the limit. Bounding the byte size is not enough; the tag count must be bounded too. Craft an entry that
	// inflates to ~80 MiB (under the budget) but is nothing but tiny elements (tens of millions of tags), and an honest
	// entry of the same inflated size that is mostly text (few tags).
	...
	const denseBomb = denseElementZip(Buffer.from('<a/>'.repeat(20 * 1024 * 1024))); // ~80 MiB inflated (under budget), ~20M tags (over the element cap)
	...
	// so counting '=' too must catch it. Build one <w:t> element with millions of a="1" attributes (~90 MiB inflated,
	// under the byte budget, only one '<').
	const attrParts = ['<w:t ']; let attrLen = 5; for (let k = 0; attrLen < 90 * 1024 * 1024; k++) { const a = 'a' + k + '="1" '; attrParts.push(a); attrLen += a.length; } attrParts.push('/>');
	const attrBomb = denseElementZip(Buffer.from(attrParts.join('')));
	...
	// directory's size), not skipped — otherwise a dense stored part reaches the reader unvetted.
	const storedDenseDD = (() => {
		const raw = Buffer.from('<w:t ' + 'a="1" '.repeat(3 * 1024 * 1024) + '/>'); // ~18 MiB stored, ~3M attributes
		const name = Buffer.from('word/document.xml');
