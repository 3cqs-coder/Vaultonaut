...
async function main() {
	...
	ok('the deletion guard reports whether it verified, with the approved deletion count', /return \{ verified: false/.test(vaultSrc) && /return \{ verified: true, would, destCount \}/.test(vaultSrc));
	...
	// Both backup syncs carry an engine-level --max-delete on EVERY run: at ~half the destination when the guard could
	// not verify (an over-cap vault), and pinned at exactly the approved deletion count when it could. The verified cap
	// closes the window between the guard's listing and the sync, during which a source that remounts empty could
	// otherwise mass-delete the destination with no cap.
	ok('both backup syncs cap deletions when the guard could not verify (over-cap fallback)', (vaultSrc.match(/verified === false && delChk\.destCount > 0\) syncArgs\.push\('--max-delete',/g) || []).length >= 2);
	ok('both backup syncs also cap deletions at the approved count when the guard verified', (vaultSrc.match(/verified === true\) syncArgs\.push\('--max-delete', String\(Math\.max\(0, delChk\.would/g) || []).length >= 2);
