...
//    or a fatal error (both drop the summary) can never be mistaken for "all per-file".
const ape = fnBody(V, 'function hashsumErrorsAllPerFile(');
ok('the per-file decision requires the engine completion summary (fail-closed on truncation)', /completed\s*&&/.test(ape) && /Failed to hashsum/.test(ape) && /\.every\(/.test(ape));

// 3. audit() must surface `damaged` as its own report field and as a finding, so a damaged file is never silent.
...
ok('audit turns damaged files into a tamper finding (damagedNote)', /damagedNote\(report\.damaged\)/.test(au));
