...
async function main() {
	...
	// The blob format version and scheme id are baked into every stored grant's header and AEAD, so a silent change
	// would make every previously-armed grant unopenable. Pin their exact values so a change stays deliberate.
	ok('the blob format version is pinned to 1', T.BLOB_VERSION === 1);
	ok('the scheme id is pinned', T.SCHEME_ID === 'drand-quicknet-g1');
	...
	// ── beacon fetch: gateways are raced, but only a verified signature is trusted (offline, mocked Net) ──────────
	// fetchSignature races all pinned gateways so one slow or dead gateway can't hold up the caller, yet it returns
	// ONLY a signature that verifies against the pinned key. Drive it with a mock Net (no network): a gateway is
	// "healthy" when the map gives it a JSON body string, and "dead" otherwise. The committed real vector is the one
	// answer that verifies for REAL_ROUND.
	const eps = T.ENDPOINTS;
	const goodBody = JSON.stringify({ round: REAL_ROUND, signature: REAL_SIG });
	const wrongBody = JSON.stringify({ round: REAL_ROUND, signature: 'ab'.repeat(48) }); // right shape (96 hex), will not verify
	const mockNet = (map) => ({ getText: async (url) => { const base = eps.find(e => url.startsWith(e)); const v = map[base]; if (typeof v === 'string') return v; throw new Error('gateway unavailable'); } });
	// A verified signature wins even when earlier-listed gateways are dead or serve a wrong one.
	const raced = await T.fetchSignature(REAL_ROUND, { Net: mockNet({ [eps[1]]: wrongBody, [eps[2]]: goodBody }) });
	ok('fetchSignature returns a verified signature even when other gateways are dead or serve a wrong one', Buffer.isBuffer(raced) && raced.equals(Buffer.from(REAL_SIG, 'hex')));
	// No gateway serves a valid signature -> null (fail-closed, never a throw).
	const noneOk = await T.fetchSignature(REAL_ROUND, { Net: mockNet({ [eps[0]]: wrongBody }) });
	ok('fetchSignature returns null when no gateway serves a valid signature', noneOk === null);
	// A well-formed signature that does not verify for the requested round is never trusted.
	const wrongRound = await T.fetchSignature(REAL_ROUND + 1, { Net: mockNet({ [eps[0]]: goodBody, [eps[1]]: goodBody }) });
	ok('fetchSignature rejects a signature that does not verify for the requested round', wrongRound === null);
