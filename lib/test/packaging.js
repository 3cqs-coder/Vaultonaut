...
function main() {
	...
	// NEVER ship: a secret, the test tree, runtime data, internal docs, or dev-only material.
	const forbidden = [
		/(^|\/)release-signing-key\.json$/, /(^|\/)release-manifest\.(json|sig)$/, /(^|\/)SHA256SUMS(\.sig)?$/,
		/(^|\/)\.test-data(\/|$)/, /(^|\/)data(\/|$)/,
	...
	// MUST ship: the app cannot run (or be verified) without these.
	const required = ['vaultonaut.js', 'verify.js', 'docs/README.md', 'LICENSE', 'CONTRIBUTING.md', 'lib/index.js', 'lib/Vault.js', 'lib/templates/help.txt'];
