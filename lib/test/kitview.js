'use strict';
// lib/test/kitview.js — the short-lived Recovery Kit view endpoint (lib/webserver/kitView.js). A kit can carry a
// one-time recovery key, so it is never persisted to disk and is served only over a random token, under its own
// kit-appropriate CSP (so the self-contained kit's inline styles render, while scripts stay blocked). The token is
// deliberately viewable more than once within its short TTL, so the user can print or re-save the kit if the first
// attempt is cancelled. This drives the real handler over a real socket: a fresh token serves the kit, a second view
// within the TTL still serves it, an unknown token is refused, and an expired token is refused — and the security
// headers are exactly what the kit needs and nothing more.
//
