...
async function main() {
	...
	try {
		...
		//     ISOLATED hub so blocking loopback does not poison the other tests on the main hub. ---
		{
			...
			const hubRL = Relay.runHub({ controlPort: cpRL, token: TOKEN, host: '127.0.0.1', portRange: [await Serve.freePort(), await Serve.freePort()] });
			await sleep(120);
			try {
				let lastErr = '';
		...
		//     Test the total cap and the per-IP cap on isolated hubs with tiny injected limits. ---
		{
			...
			const hubTot = Relay.runHub({ controlPort: cpTot, token: TOKEN, host: '127.0.0.1', portRange: [await Serve.freePort(), await Serve.freePort()], maxNodes: 2 });
			await sleep(120);
			try {
			...
			const hubIp = Relay.runHub({ controlPort: cpIp, token: TOKEN, host: '127.0.0.1', portRange: [await Serve.freePort(), await Serve.freePort()], maxNodes: 50, maxNodesPerIp: 2 });
			await sleep(120);
			try {
