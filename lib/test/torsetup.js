...
ok('provideTor prefers a Tor the user already runs before downloading one', /async function provideTor\([\s\S]{0,600}connectControl\(CONTROL_PORTS\[i\][\s\S]{0,200}startManagedTor\(bridge\)/.test(onion));
ok('the managed Tor binds control and the client SOCKS on loopback only', /--ControlPort', '127\.0\.0\.1:' \+ controlPort/.test(onion) && /--SocksPort', '127\.0\.0\.1:' \+ socksPort/.test(onion));
