...
async function main() {
	...
	for (const route of ['/api/import', '/api/import-files', '/api/reconstruct', '/api/unpack']) {
	...
	ok('the API guard requires the anti-CSRF custom header', /req\.get\('x-vdisk'\) !== '1'[\s\S]{0,40}status\(403\)/.test(server));
