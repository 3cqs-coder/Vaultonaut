...
async function main() {
	...
	ok('the /__mounted probe is registered only when a launch token is provided', /if \(readyToken\) \{[\s\S]{0,900}app\.get\('\/__mounted'/.test(server));
	...
	ok('the /__mounted probe reports the count from the shared active-mounts list', /app\.get\('\/__mounted'[\s\S]{0,520}Vault\.listMounts\(\)/.test(server));
