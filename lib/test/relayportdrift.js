...
// relay registration can never disagree. A zero-config relay-and-node connect depends on them matching. This guards
// against a literal 7443 (or the 20000-20099 range) creeping back into the relay modules and silently re-drifting.
// (The CLI command is `vdisk relay`; there is no `vdisk hub`.)
//
...
function main() {
	...
	// The default UI port is single-sourced in Common too, and it is printed verbatim in the user-facing docs. Pin the
	// value AND that both the built-in help and the README show the SAME number, so changing the port (e.g. to dodge a
	// conflict) fails here until the docs are updated — otherwise `vdisk ui` would open a port the docs do not mention.
	ok('Common single-sources the UI port', Common.DEFAULT_UI_PORT === 7420);
	const help = read('templates/help.txt'), readme = read('../docs/README.md');
	ok('help.txt documents the current UI port', help.includes('localhost:' + Common.DEFAULT_UI_PORT));
	ok('the README documents the current UI port', readme.includes(String(Common.DEFAULT_UI_PORT)));
	// No code module may carry the bare UI-port literal (comments allowed); every reader uses Common.DEFAULT_UI_PORT.
	for (const rel of ['webserver/index.js', 'Commands.js', 'Autostart.js']) {
		ok('no literal 7420 in ' + rel + ' code (use Common.DEFAULT_UI_PORT)', !/\b7420\b/.test(codeOnly(read(rel))));
	}
