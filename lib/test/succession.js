...
async function main() {
	...
	// REFUSE FORWARD: a succession log written by a NEWER build would have its per-block links recomputed under rules
	// this build does not know and read as a broken chain (a false alarm). verifySuccession must instead report it as
	// a newer format (ok true, empty chain, unknownFormat) so the UI prompts to update rather than crying tampering.
	{
		...
		const newer = await vdisk.verifySuccession(v);
		ok('a newer-format succession log is reported as unknownFormat, not a broken chain', newer.unknownFormat === true && newer.ok === true && newer.chain.length === 0);
		await fsp.writeFile(sp, orig); // restore so the checks below still see the real chain
