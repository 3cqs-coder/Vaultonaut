...
async function main() {
	...
	const sealedFiles = (await fsp.readdir(relDir)).filter(f => f.endsWith('.sealed'));
	ok('a sealed blob and a how-to file are written to the release folder', sealedFiles.length === 1 && fs.existsSync(path.join(relDir, 'HOW-TO-OPEN.txt')));

	// The CONTACT opens the sealed blob with their PRIVATE key and recovers a real read link.
	const sealed = await fsp.readFile(path.join(relDir, sealedFiles[0]), 'utf8');
