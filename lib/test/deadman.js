...
async function main() {
	...
	// Boundary tightness — the switch must never fire EARLY (an early release hands someone's vault to a beneficiary
	// before the owner has actually gone silent). The window is inactivity (30d) + grace (14d) = 44d. The effective
	// reference is the anti-rollback anchor, which sits a few milliseconds AFTER the recorded check-in, so the true
	// boundary is never earlier than the nominal one — which means asserting "still armed at exactly 30d" and "not yet
	// released at exactly 44d" catches any shaved window or a `>`→`>=` early-fire, while staying robust to that gap.
	ok('at exactly the inactivity boundary (30d) grace has NOT started yet (still armed)', (await vdisk.emergencyTick(start + 30 * DAY)).phase === 'armed');
	const atEnd = await vdisk.emergencyTick(start + 44 * DAY);
	ok('at exactly the end of the grace window (44d) it has NOT released yet (still grace)', atEnd.released !== true && atEnd.phase === 'grace');
	ok('no release directory exists right up to the window end', !fs.existsSync(path.join(dataDir, 'emergency-release')));
	...
	// Re-enrolling ALSO withdraws a release: it clears releasedAt, so the on-disk release material (which names vaults
	// and beneficiaries in plaintext and would otherwise linger with no later tick to clean it) must go with it.
	await fsp.mkdir(path.join(relDir, 'stale-beneficiary'), { recursive: true });
	await fsp.writeFile(path.join(relDir, 'stale-beneficiary', 'grant.sealed'), 'x');
	await vdisk.emergencyEnroll({ contactPubKey: contact.publicKey, contactLabel: 'My sister', inactivityDays: 30, graceDays: 14 });
	ok('re-enrolling removes any leftover on-disk release material', !fs.existsSync(relDir));
