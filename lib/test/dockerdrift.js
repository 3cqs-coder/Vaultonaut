...
function main() {
	...
	// The OCR engine is pure JavaScript and WebAssembly living in node_modules. The runtime image copies node_modules
	// whole from the builder, and its cleanup prunes only the test and scripts trees. Guard that node_modules is copied
	// and that nothing prunes node_modules or WebAssembly, so text recognition keeps working inside the container.
	ok('the Dockerfile copies node_modules whole into the runtime image (OCR WebAssembly included)', /COPY[^\n]*--from=builder[^\n]*\/app\/node_modules[^\n]*node_modules/.test(dockerfile));
	ok('the Dockerfile cleanup never prunes node_modules or WebAssembly (OCR survives)', !/rm\s+-rf[^\n]*node_modules/.test(dockerfile) && !/\.wasm/.test(dockerfile));
