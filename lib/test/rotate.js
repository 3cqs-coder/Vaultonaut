...
async function main() {
	...
	// --- A journal written by a NEWER build is left UNTOUCHED, never clobbered ---
	// An interrupted rotation a future version wrote must be completed or rolled back by THAT version. This build must
	// not guess at a layout it does not understand: it leaves the journal and the staged state exactly as they are and
	// reports unknownVersion, so a newer build can finish the rotation. Clobbering it could destroy an in-progress
	// key change.
	{
		const jp = path.join(v, '.rekey-journal.json');
		const future = JSON.stringify({ v: 999, phase: 'COMMIT', at: new Date().toISOString() });
		await fsp.writeFile(jp, future);
		const rr = await vdisk.resumeRekey(v); // unforced, as the mount path and startup sweep call it
		ok('a rekey journal from a newer version is not resumed (unknownVersion)', rr.resumed === false && rr.unknownVersion === true);
		ok('the newer-version journal is left exactly as it was (not clobbered)', (await fsp.readFile(jp, 'utf8')) === future);
		await fsp.rm(jp, { force: true }); // clean up so the next block starts from a settled vault
		ok('the vault still opens after the untouched newer-version journal', !(await vdisk.audit(v, { password: 'pw1' })).errors.length);
	}
	...
	// --- Resume-decision coverage: the commit anchor, the busy deferral, and sidecar cleanup ---
	// These pin the exact rules resumeRekey uses to decide complete-vs-defer-vs-rollback, on a FRESH vault so they
	// cannot perturb the carefully ordered state the blocks above depend on. The staged store is a copy of the live
	// data, so completing is idempotent and the vault stays openable — the same technique the synthetic block above uses.
	{
		const vR = path.join(tmp, 'RotResume.vault');
		await vdisk.importFolder(vR, { password: 'pw1', sourceDir: src });
		await vdisk.snapshot(vR, { password: 'pw1' });
		const stageCommitted = async (journal) => {
			await fsp.cp(path.join(vR, 'data'), path.join(vR, 'data.new.tmp'), { recursive: true });
			await fsp.rename(path.join(vR, 'data'), path.join(vR, 'data.old.tmp'));
			await fsp.copyFile(path.join(vR, 'vault.json'), path.join(vR, 'vault.json.new'));
			await fsp.copyFile(path.join(vR, 'vault.json'), path.join(vR, '.vault.bak.new'));
			if (journal) await fsp.writeFile(path.join(vR, '.rekey-journal.json'), JSON.stringify(journal));
		};
		const cleanTemps = async () => { for (const f of ['data.new.tmp', 'data.old.tmp', 'vault.json.new', '.vault.bak.new', 'succession.json.new', 'shares.json.new', '.rekey-journal.json']) await fsp.rm(path.join(vR, f), { recursive: true, force: true }); };

		// A1a: vault.json.new present but the journal is PRE-COMMIT (VERIFY) — the crash window between the commit
		// anchor write and setPhase('COMMIT'). Since `committed = stagedManifest || phase ∈ {COMMIT,CLEANUP}`, the
		// staged manifest ALONE must drive completion; rolling back here would discard a committed rotation = data loss.
		await stageCommitted({ phase: 'VERIFY', heartbeatAt: new Date().toISOString(), at: new Date().toISOString() });
		let rr = await vdisk.resumeRekey(vR);
		ok('a staged manifest with a pre-COMMIT (VERIFY) journal is COMPLETED, not rolled back', rr.resumed === true && rr.completed === true);
		ok('the store is restored after completing via the staged-manifest anchor', (await exists(path.join(vR, 'data'))) && !(await exists(path.join(vR, 'data.new.tmp'))) && !(await exists(path.join(vR, 'data.old.tmp'))));
		ok('the vault opens after completing via the staged-manifest anchor (VERIFY journal)', !(await vdisk.audit(vR, { password: 'pw1' })).errors.length);
		await cleanTemps();

		// A1b: vault.json.new present with NO journal at all — the same anchor, nothing to consult. Must also complete.
		await stageCommitted(null);
		rr = await vdisk.resumeRekey(vR);
		ok('a staged manifest with NO journal is COMPLETED via the anchor', rr.resumed === true && rr.completed === true);
		ok('the vault opens after completing with no journal present', !(await vdisk.audit(vR, { password: 'pw1' })).errors.length);
		await cleanTemps();

		// A2: a PRE-COMMIT journal (COPY, no staged manifest) with a FRESH heartbeat means a rotation is actively
		// running in ANOTHER process — an unforced resume (the sweep) must DEFER as busy, never roll it back and delete
		// its staged copy. A STALE heartbeat means that process is gone, so it may be rolled back.
		await fsp.mkdir(path.join(vR, 'data.new.tmp'), { recursive: true });
		await fsp.writeFile(path.join(vR, 'data.new.tmp', 'junk'), 'x');
		await fsp.writeFile(path.join(vR, '.rekey-journal.json'), JSON.stringify({ phase: 'COPY', heartbeatAt: new Date().toISOString(), at: new Date().toISOString() }));
		rr = await vdisk.resumeRekey(vR); // UNFORCED, exactly as the recovery sweep calls it
		ok('a pre-commit rotation with a FRESH heartbeat is deferred as busy (not rolled back)', rr.resumed === false && rr.busy === true);
		ok('the deferred rotation keeps its staged copy and journal intact', (await exists(path.join(vR, 'data.new.tmp'))) && (await exists(path.join(vR, '.rekey-journal.json'))));
		await fsp.writeFile(path.join(vR, '.rekey-journal.json'), JSON.stringify({ phase: 'COPY', heartbeatAt: new Date(Date.now() - 3600000).toISOString(), at: new Date().toISOString() }));
		rr = await vdisk.resumeRekey(vR);
		ok('a pre-commit rotation with a STALE heartbeat is rolled back', rr.resumed === true && rr.completed === false);
		ok('the rolled-back staged copy and journal are gone (stale-heartbeat path)', !(await exists(path.join(vR, 'data.new.tmp'))) && !(await exists(path.join(vR, '.rekey-journal.json'))));

		// A3: a pre-commit rollback must also remove the sidecar *.new temps (succession/shares/.vault.bak), not only
		// the staged store — a leftover *.new could confuse a later rotation. Stage them WITHOUT vault.json.new (whose
		// presence would flip the state to committed), then roll back and assert every sidecar temp is gone.
		await fsp.mkdir(path.join(vR, 'data.new.tmp'), { recursive: true });
		await fsp.writeFile(path.join(vR, 'succession.json.new'), 'x');
		await fsp.writeFile(path.join(vR, 'shares.json.new'), 'x');
		await fsp.writeFile(path.join(vR, '.vault.bak.new'), 'x');
		await fsp.writeFile(path.join(vR, '.rekey-journal.json'), JSON.stringify({ phase: 'COPY', at: new Date().toISOString() })); // no heartbeat -> rolls back
		rr = await vdisk.resumeRekey(vR);
		ok('a pre-commit rollback discards the staged store and journal', rr.resumed === true && rr.completed === false && !(await exists(path.join(vR, 'data.new.tmp'))));
		ok('a pre-commit rollback also removes the sidecar *.new temps', !(await exists(path.join(vR, 'succession.json.new'))) && !(await exists(path.join(vR, 'shares.json.new'))) && !(await exists(path.join(vR, '.vault.bak.new'))));
		ok('the vault still opens after the resume-decision coverage block', !(await vdisk.audit(vR, { password: 'pw1' })).errors.length);
		await vdisk.removeKnownVault(vR).catch(() => {});
	}
	...
	// Source pin: the LOCAL-MOUNT FENCE. The first assertUnmounted runs before the rekey journal exists, so in a
	// narrow cross-process window a mount could slip past its own "rotation pending?" check and mount the OLD store
	// that is about to be swapped away — writes through it would be lost. rotate() re-checks assertUnmounted RIGHT
	// BEFORE the commit anchor so such a mount aborts the rotation and it rolls back (the old store, with those writes,
	// is preserved); a mount that starts after the journal is written is already refused through the lock. Forcing a
	// real cross-process mount mid-rotation is impractical here, and the rollback-on-abort path is exercised by the
	// failed-rotation case above, so pin that the fence exists and sits between the lease fence and the commit anchor.
	const vsrc = await fsp.readFile(path.join(__dirname, '..', 'Vault.js'), 'utf8');
	const leaseIdx = vsrc.indexOf('await assertStillHoldLease(abs);\n\t\t\t// LOCAL-MOUNT FENCE');
	const fenceIdx = vsrc.indexOf("await assertUnmounted(abs, 'before switching to the new keys')");
	const anchorIdx = vsrc.indexOf('path.join(abs, MANIFEST_NEW), newManifest');
	ok('rotate re-checks assertUnmounted at the commit edge, between the lease fence and the commit anchor', leaseIdx > 0 && fenceIdx > leaseIdx && anchorIdx > fenceIdx);
