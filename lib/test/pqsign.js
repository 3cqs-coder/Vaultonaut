...
// --- pqAvailable() is true on this (real) runtime, and the memoized probe works ---
ok('ML-DSA is available on this runtime (pqAvailable is true)', Integrity.pqAvailable() === true);

// --- capability fallback: a runtime that CANNOT do ML-DSA must accept a valid classical signature alone rather than
// false-reporting a genuine record as forged (which would block self-heal and raise a false tamper alarm) — but it must
// STILL reject a bad classical signature, and must return to requiring the PQ half once ML-DSA is available again ---
try {
	Integrity._forcePqAvailable(false);
	ok('without ML-DSA, verifyHybrid accepts a good classical signature even with a bad post-quantum half', Integrity.verifyHybrid(a.pub, a.pqPub, msg, edSig, 'AAAA') === true);
	ok('without ML-DSA, verifyHybrid still rejects a bad classical signature', Integrity.verifyHybrid(a.pub, a.pqPub, msg, 'AAAA', pqSig) === false);
	// SIGN side must degrade the same way: on a runtime that lacks ML-DSA, the pqPriv getter returns null (not a throw)
	// and signHybrid emits the classical half with sigPq:null — never throwing deep inside a signer, and never leaving
	// a caller (e.g. a manifest reseal) to swallow the throw and keep a stale signature. The classical half still
	// verifies on that same runtime. This is the symmetry that makes create/seal/roster/reseal work end to end there.
	const noPqKeys = Integrity.signKeysFromSeed(seed);
	ok('without ML-DSA, the pqPriv getter returns null instead of throwing', noPqKeys.pqPriv === null && noPqKeys.pqPub === null);
	let signThrew = false, hNoPq = null;
	try { hNoPq = Integrity.signHybrid(noPqKeys.priv, noPqKeys.pqPriv, msg); } catch (_) { signThrew = true; }
	ok('without ML-DSA, signHybrid does not throw and omits the post-quantum half', !signThrew && hNoPq && typeof hNoPq.sig === 'string' && hNoPq.sigPq === null);
	ok('without ML-DSA, the classical-only hybrid signature verifies', Integrity.verifyHybrid(noPqKeys.pub, noPqKeys.pqPub, msg, hNoPq.sig, hNoPq.sigPq) === true);
	Integrity._forcePqAvailable(true);
	ok('with ML-DSA available, verifyHybrid again requires the post-quantum half', Integrity.verifyHybrid(a.pub, a.pqPub, msg, edSig, 'AAAA') === false);
} finally { Integrity._forcePqAvailable(null); } // restore the real probe no matter what
