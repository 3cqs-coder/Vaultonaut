...
function main() {
	...
	console.log('[tamper + malformed]');
	{
		...
		const corrupted = 'vds1.3.' + bad.x + '.' + Buffer.from(bad.y).toString('base64');
	...
	console.log('[same-shape cross-split does not yield either true secret]');
	{
		...
		const mixed = Shamir.combine([a[0], a[1], b[2]]); // 3 shares but not all from one split
		ok('mixing shares from two same-shape splits gives neither secret (caught downstream by AEAD)', !eqBuf(mixed, s1) && !eqBuf(mixed, s2));
	}
