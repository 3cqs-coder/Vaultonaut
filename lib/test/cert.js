...
async function main() {
	...
	// --- ensureLanCert always covers loopback, so a direct hole-punched tunnel (which reaches this node through a
	//     LOCAL proxy at 127.0.0.1 and pins this cert) verifies its hostname. Even alongside a public "extra" host. ---
	const lan = await Cert.ensureLanCert('203.0.113.5');
	const xlan = new crypto.X509Certificate(lan.certPem);
	ok('ensureLanCert always covers 127.0.0.1 (for the direct-tunnel local proxy) plus the extra host', /IP Address:127\.0\.0\.1/.test(xlan.subjectAltName) && /IP Address:203\.0\.113\.5/.test(xlan.subjectAltName));
