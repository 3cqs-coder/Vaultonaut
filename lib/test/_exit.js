...
module.exports = async function closeServersThenExit(code, ...servers) {
	...
	// Close undici's global connection pool (fetch's client sockets) and each server first, so as little as possible
	// is still tearing down at exit. The symbol is undici's own stable handle for its global dispatcher.
	...
	for (const s of servers.filter(Boolean)) {
		...
		try { await new Promise((r) => s.close(r)); } catch (_) {} // wait for the listener to fully close before exiting
	}
	// THE Windows fix. Node 24 has a libuv bug (nodejs/node#56645, unfixed as of this writing): calling process.exit()
	// soon after fetch()/undici activity aborts the process with "Assertion failed: !(handle->flags &
	// UV_HANDLE_CLOSING), src\\win\\async.c" because undici's TLS/socket teardown is still running in worker threads
	// and races libuv's shutdown. Closing the dispatcher is not enough — the teardown needs a moment to finish. A short
	// yield before exit lets it complete. Windows only (POSIX is unaffected), so it never slows the macOS/Linux suite.
