...
async function main() {
	...
	// REFUSE FORWARD vs. FORGERY for the roster: the version field is part of the SIGNED input, so a bare version bump
	// (no re-sign) must still be caught as tampering, NOT masked as "newer format" — listShares only treats a newer
	// version as unknownFormat when the signature also verifies (a genuinely newer, signed roster). The dedicated
	// sharesversion.js test proves the forgery-detection direction; here we just confirm an unsigned bump is not masked.
	const spV = path.join(v, 'shares.json');
	const stV = JSON.parse(await fsp.readFile(spV, 'utf8'));
	await fsp.writeFile(spV, JSON.stringify({ ...stV, version: (Number(stV.version) || 1) + 1 })); // bump WITHOUT re-signing
	const lNewer = await vdisk.listShares(v);
	ok('an unsigned version bump is NOT masked as newer-format (stays a detectable tamper)', !lNewer.unknownFormat && lNewer.sigOk === false);
