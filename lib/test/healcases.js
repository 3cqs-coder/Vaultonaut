...
async function main() {
	...
	console.log('[1b] same-size in-place change under preserveInPlaceEdits — the UNATTENDED path must DEFER, not revert');
	{
		// A same-length in-place change is ambiguous: silent bit-rot, or a coherent edit written at the same length.
		// The unattended scheduled path passes preserveInPlaceEdits so it never silently reverts a possible edit — it
		// leaves the file exactly as-is and reports it. The attended default heal (no flag) still restores it.
		const blobs = await walk(dataDir); const t = blobs[0].path; const want = await sha(t);
		const changed = await fsp.readFile(t); changed[Math.floor(changed.length / 2)] ^= 0xff; await fsp.writeFile(t, changed);
		const deferred = await vdisk.heal(v, { preserveInPlaceEdits: true });
		ok('in-place (preserve): the block was left exactly as written, not reverted', Buffer.compare(await fsp.readFile(t), changed) === 0);
		ok('in-place (preserve): heal repaired no data and reported the deferral', (deferred.repairedData || 0) === 0 && deferred.inPlaceDeferred >= 1);
		await healOk('in-place (attended)'); // the explicit attended heal (no flag) restores it from parity
		ok('in-place (attended): content restored byte-for-byte', await sha(t) === want);
	}
