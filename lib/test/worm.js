...
async function main() {
	...
	// A lock mode that is present but not one of the two valid values must be REFUSED, never silently coerced to
	// governance (the bypassable mode). A typo/malformed field asking for compliance-grade immutability that quietly
	// became governance would hand the caller the weak guarantee while they believe they have the strong one — the one
	// failure a tamper-proof mode must never have. This throws at create time, before any network call.
	let e3 = ''; try { await vdisk.create(path.join(tmp, 'c.vault'), { password: 'pw', cloud: { remoteId: 'nope' }, worm: { mode: 'complaince', retainDays: 30 } }); } catch (e) { e3 = e.message; }
	ok('a misspelled WORM mode is refused, not coerced to governance', /must be either "governance".*or "compliance"/i.test(e3));
	let e4 = ''; try { await vdisk.create(path.join(tmp, 'd.vault'), { password: 'pw', cloud: { remoteId: 'nope' }, worm: { mode: 'Compliance', retainDays: 30 } }); } catch (e) { e4 = e.message; }
	ok('a valid mode with odd casing/spacing is accepted (reaches the cloud step, not a mode error)', !/must be either/i.test(e4));
