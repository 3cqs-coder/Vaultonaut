...
const DEFAULT_REPO = Brand.name; // the public repo is named after the product; derive it so a rename can't leave a stale literal here
...
// The bounded response reader is shared (Common.readCappedText) so the update check and the breach check use one
// implementation; kept as a thin local alias to minimize the diff at the call sites below.
const readCappedText = (res, cap) => Common.readCappedText(res, cap);
...
async function checkForUpdate({ url, timeoutMs = FETCH_TIMEOUT_MS } = {}) {
	...
	// Constrain the source to an http(s) web address. The default is safe; a custom --url is operator-supplied, so
	// reject anything that is not a valid http/https URL — a file:, data:, or other scheme could read a local file or
	// reach a non-web endpoint. Host is left to the operator on purpose (a private release mirror is legitimate).
	const target = url || DEFAULT_TAGS_URL;
	try { const u = new URL(target); if (u.protocol !== 'https:' && u.protocol !== 'http:') { out.error = 'The update source must be an http or https address.'; return out; } }
	catch (_) { out.error = 'The update source is not a valid web address.'; return out; }
	try {
		// Bounded via the shared Common.fetchWithTimeout. The body read happens INSIDE the consume callback so the
		// abort deadline covers it too — a source that sends headers quickly but then drips the body slowly (staying
		// under the size cap) is aborted rather than hanging the check forever.
		const r = await Common.fetchWithTimeout(target, { headers: { 'Accept': 'application/vnd.github+json', 'User-Agent': Brand.slug + '-update-check' } }, Math.max(1000, timeoutMs), async (res) => {
			if (!res || !res.ok) return { httpStatus: res ? res.status : '—' };
			return { text: await readCappedText(res, MAX_RESPONSE_BYTES) };
		});
		if (r.httpStatus !== undefined) { out.error = 'The update source could not be reached (HTTP ' + r.httpStatus + ').'; return out; }
		if (r.text == null) { out.error = 'The update source returned an unreadable or oversized response.'; return out; }
		let body; try { body = JSON.parse(r.text); } catch (_) { out.error = 'The update source returned an unreadable response.'; return out; }
