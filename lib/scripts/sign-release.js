...
function writeEmbeddedPubKey(pubHex, pqPub) {
	const p = path.join(ROOT, 'lib', 'releasePubKey.js');
	let s = fs.readFileSync(p, 'utf8');
	const line = "module.exports = { pubkey: '" + pubHex + "', pqPubkey: '" + (pqPub || '') + "' };";
...
async function doInit() {
	...
	fs.writeFileSync(keyPath, JSON.stringify({ alg: 'ed25519+ml-dsa-65', seed, pub, pqPub, createdAt: new Date().toISOString() }, null, 2) + '\n', { mode: 0o600 });
	try { fs.chmodSync(keyPath, 0o600); } catch (_) {}
	writeEmbeddedPubKey(pub, pqPub);
...
async function doSign() {
	const seed = loadSeed();
	// Pin the maintainer's post-quantum public key BEFORE building the manifest (releasePubKey.js is a signed file, so
	// it must be final before it is hashed). Backfill it from the seed the first time a hybrid sign runs on a key that
	// predates post-quantum signing; thereafter it is already present and this is a no-op.
	if (!RI.embeddedPqPubKey()) { delete require.cache[require.resolve(path.join(ROOT, 'lib', 'releasePubKey.js'))]; writeEmbeddedPubKey(RI.publicKeyForSeed(seed), RI.pqPublicKeyForSeed(seed)); delete require.cache[require.resolve(path.join(ROOT, 'lib', 'releasePubKey.js'))]; }
	...
	fs.writeFileSync(path.join(ROOT, RI.SIG_PQ_NAME), RI.signManifestPq(buf, seed) + '\n'); // the ML-DSA co-signature over the same manifest bytes
	...
	console.log('  ' + RI.MANIFEST_NAME + ' + ' + RI.SIG_NAME + ' + ' + RI.SIG_PQ_NAME + ' (authenticity — verify.js; committed so every build, including CI, embeds them)');
	console.log('  SHA256SUMS (plain corruption check)');
	// The reproducible digest is a deterministic function of the source at this tag: publish it (release notes / site) so
	// anyone can regenerate it with "npm run verify:build" and confirm the signed release is exactly the public source.
	console.log('Reproducible digest (publish this — verify with "npm run verify:build"): ' + RI.reproducibleDigest(manifest));
	console.log('Public key (must match the README): ' + RI.publicKeyForSeed(seed));
	console.log('Post-quantum public key: ' + RI.pqPublicKeyForSeed(seed));
	console.log('Commit ' + RI.MANIFEST_NAME + ', ' + RI.SIG_NAME + ', and ' + RI.SIG_PQ_NAME + ' with your changes so the desktop bundle build can embed them.');
}
