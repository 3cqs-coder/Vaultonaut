...
const FAILED = Symbol('hibp-failed'); // a per-scan marker cached for a prefix whose lookup could not complete
async function count(password, { cache = null, timeoutMs = 6000, base = HIBP_BASE, onFail = null } = {}) {
	...
	// A prefix whose earlier lookup FAILED is remembered for THIS scan (the cache is per-scan) so the same prefix is
	// not re-fetched again and again during an outage — but it is never treated as a definitive "not breached": it
	// still reports the scan incomplete and returns "unknown" (0). A fresh scan gets a fresh cache and retries.
	if (body === FAILED) { if (onFail) { try { onFail(); } catch (_) {} } return 0; }
	if (body == null) {
		try {
		...
		if (body == null) { if (cache) cache.set(prefix, FAILED); if (onFail) { try { onFail(); } catch (_) {} } return 0; } // remember the failure for this scan; report incomplete; treat as "unknown"
