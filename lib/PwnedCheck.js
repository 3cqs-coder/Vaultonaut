...
// Read a response body as text, bounded, so a hostile or broken endpoint cannot stream unbounded data into memory.
// (Kept local to this module rather than shared, so the breach lookup carries its own ceiling and has no cross-module
// dependency; it mirrors the same guard the update checker uses.)
...
// per-prefix response for a scan so many items sharing a prefix cost one request. `base` is overridable for tests.
async function count(password, { cache = null, timeoutMs = 6000, base = HIBP_BASE } = {}) {
	...
	if (body == null) {
		try {
			body = await Common.fetchWithTimeout(base + prefix, { headers: { 'Add-Padding': 'true', 'User-Agent': 'vaultonaut-health' } }, Math.max(1000, timeoutMs), (res) => readCappedText(res, MAX_RESPONSE_BYTES));
		} catch (_) { body = null; }
		if (body == null) body = '';
...
module.exports = { count, readCappedText, HIBP_BASE };
