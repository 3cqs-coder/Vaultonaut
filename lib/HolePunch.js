...
// A hard ceiling on outbound connection attempts (SYNs) per punch, independent of the time budget. The budget/interval
// above already bound a normal punch to roughly this many attempts, so a legitimate punch is never truncated; the cap
// exists so a client can never be turned into an unbounded SYN source aimed at an attacker-named address — the target
// host/port in a punch comes from a hub-relayed answer, and a hostile hub is in the threat model, so the count of
// packets it can direct at a third party is bounded here no matter what timing values are in play.
const PUNCH_MAX_ATTEMPTS = 64;
...
function reflexiveEndpoint({ hubHost, hubPort, token, nodeId, localPort, localAddress, timeoutMs = REFLEX_TIMEOUT_MS, permitHost = isPunchableHost }) {
	if (!token || !nodeId || !Number.isInteger(localPort)) return Promise.resolve(null);
	// Shares the hub's one-shot query with the WHOAMI/SIGNAL probes. It binds the exact local port it will punch from,
	// and RST-closes so that port is immediately reusable (a FIN would strand it in TIME_WAIT). The parse validates the
	// reported endpoint is a punchable public address in range before it is trusted.
	return Relay.hubQuery({
		hubHost, hubPort, timeoutMs, localPort, localAddress, rst: true,
		request: 'WHERE ' + token + ' ' + nodeId,
		parse: (line) => { const m = /^WHERE (\S+) (\d{1,5})$/.exec(line); const ip = m && m[1], port = m && parseInt(m[2], 10); return (m && permitHost(ip) && port > 0 && port < 65536) ? { ip, port } : null; },
	});
}
...
function punch({ localPort, localAddress, peerHost, peerPort, budgetMs = PUNCH_BUDGET_MS, intervalMs = PUNCH_INTERVAL_MS, permitHost = isPunchableHost }) {
	return new Promise((resolve) => {
		...
		let settled = false, cur = null, attemptTimer = null, attempts = 0;
		...
		function attempt() {
			if (settled) return;
			if (Date.now() >= deadline || ++attempts > PUNCH_MAX_ATTEMPTS) return fail();
