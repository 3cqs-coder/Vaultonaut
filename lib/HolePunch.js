...
async function connectDirect({ hubHost, hubPort, credential, nodeId, budgetMs = PUNCH_BUDGET_MS, intervalMs = PUNCH_INTERVAL_MS, timeoutMs, permitHost = isPunchableHost }) {
	...
	const sock = await punch({ localPort, peerHost: ans.ip, peerPort: ans.port, budgetMs, intervalMs, permitHost });
	if (!sock) return null;
...
async function answerSignal({ hubHost, hubPort, credential, nodeId, offerBlob, reply, localServePort, budgetMs = PUNCH_BUDGET_MS, intervalMs = PUNCH_INTERVAL_MS, permitHost = isPunchableHost }) {
	...
	const sock = await punch({ localPort, peerHost: offer.ip, peerPort: offer.port, budgetMs, intervalMs, permitHost });
	if (!sock) return;
	Tunnel.nodeProxy(sock, { localServePort });
}
