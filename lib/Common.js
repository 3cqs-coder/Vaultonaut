...
const WINDOWS_RESERVED_BASE = /^(con|prn|aux|nul|com[0-9]|lpt[0-9])$/i; // reserved device names, case-insensitive, reserved even WITH an extension (CON.vault)
...
function assertPortableVaultName(displayName) {
	...
	if (name.length > 200) throw new Error('That vault name is too long. Keep it under 200 characters so the folder name works on every system.');
