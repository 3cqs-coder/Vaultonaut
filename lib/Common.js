...
async function sameFile(a, b) {
	...
	try {
		const [sa, sb] = await Promise.all([fsp.stat(ra), fsp.stat(rb)]);
		return sa.dev === sb.dev && sa.ino === sb.ino;
	} catch (_) { return samePath(a, b); } // one side missing/unreadable — fall back to the conservative lexical compare
}
