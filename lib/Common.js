...
function launchedFromTest() {
	if (_launchedFromTest === undefined) {
		let m = '';
		try { m = (require.main && require.main.filename) || ''; } catch (_) { m = ''; }
		...
		const ownTest = path.resolve(__dirname, 'test') + path.sep;
		_launchedFromTest = !!m && path.resolve(m).startsWith(ownTest);
	}
	return _launchedFromTest;
}
function dataDir() {
	if (dataDirOverride) return dataDirOverride;
	if (launchedFromTest()) return path.join(__dirname, '..', '.test-data'); // never the real user dir from a test entry
...
function binDir() { return binDirOverride || process.env.VAULTONAUT_ENGINE_DIR || path.join(dataDir(), 'bin'); }
...
module.exports = {
	root, appScriptPath, setDesktopApp, isDesktopApp, dataDir, setDataDir, dataDirOverridePath, ensureDataDir, binDir, setBinDir, vaultsDir, runDir, statePath, samePath, sameFile, pathWithin, resolveThroughSymlinks, pathExistsBounded, isLoopbackHost, redactPaths, sanitizeBwlimit, validateBwlimit, foldPath,
