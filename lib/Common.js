...
// is unref'd). Returns the Response so the caller reads the body itself (capped where the response is untrusted).
// Requires the runtime's built-in fetch; callers guard for its absence and treat a throw (including the AbortError a
// timeout raises) as a failed request. Single-sourced so every outbound request shares one abort + timer discipline.
...
async function pathExistsBounded(p, ms = 2000) {
	try { await withTimeout(fsp.stat(p), ms); return true; } catch (_) { return false; }
}
...
function isLoopbackHost(host) {
	const a = String(host == null ? '' : host).trim().toLowerCase();
...
module.exports = {
	...
	sleep, yieldToLoop, withTimeout, fetchWithTimeout, readFileCapped, readJsonCorruptAside, freshCache, isProcessAlive, pollUntil, writeJsonAtomic, fsyncPath, windowsHiddenRunVbs, uniqueTempPath, renameWithRetry,
	serialQueue, serialQueueByKey, splitHostPort, diskFree, runCmd, runCmdOrThrow, suspiciousMassDeletion, failureBackoff,
	SCHEMA_VERSION, schemaVersionFor, DEFAULT_UI_PORT
};
