...
const Logger = require('./Logger'); // diagnostic file logging + secret redaction; log()/warn() route through it (no load cycle: Logger requires Common lazily)
...
// True when this process was launched as a SUPERVISED CHILD of a parent that holds our stdin open as a dead-man's
// switch — the packaged desktop shell, a test harness, or any future supervisor. Such a parent pipes our stdin and
// sets VDISK_SUPERVISED=1; the service then watches stdin (ProcRegistry.watchStdinForQuit) and exits the instant the
// parent dies by ANY means (crash, SIGKILL, Force-Quit, an interrupted test run), so it can never be orphaned. This is
// OPT-IN by the spawner: a headless service under launchd/systemd (stdin /dev/null, no flag) is unaffected and keeps
// running as intended. One env flag, read here once, so every entry point agrees. See SUPERVISED_ENV/childEnv below.
const SUPERVISED_ENV = 'VDISK_SUPERVISED';
function isSupervisedChild() { return process.env[SUPERVISED_ENV] === '1'; }
// Build the environment for spawning a Vaultonaut service as a supervised child: the parent MUST also give the child a
// piped stdin (stdio: ['pipe', ...]) so closing it (or the parent dying) delivers the EOF the switch watches for.
function supervisedChildEnv(base) { return Object.assign({}, base || process.env, { [SUPERVISED_ENV]: '1' }); }
...
const SAME_FILE_STAT_TIMEOUT_MS = 3000; // bound the identity stat so a wedged mount/network path can never hang a caller
// One shared, in-flight stat per resolved path. The paths sameFile compares are exactly the ones designed to wedge
// (FUSE mountpoints, removable and network drives), and a caller's timeout below bounds the WAIT but cannot cancel the
// underlying stat — the syscall keeps running on a libuv threadpool thread until the backend answers. Without dedup,
// the mount-state lock and the schedule tick could each fire a fresh stat at the same wedged path and pin a separate
// thread every time, eventually starving the small default pool. Sharing the raw promise while it is pending means a
// wedged path holds at most ONE thread no matter how often it is probed; the entry clears when the syscall settles.
// Mirrors the identical discipline in Rclone.statDev.
const sameFileStatInflight = new Map(); // resolved path -> the raw fsp.stat promise still pending (the real slot holder)
function sameFileSharedStat(resolved) {
	let raw = sameFileStatInflight.get(resolved);
	if (!raw) {
		raw = fsp.stat(resolved);
		sameFileStatInflight.set(resolved, raw);
		raw.then(() => {}, () => {}).finally(() => { sameFileStatInflight.delete(resolved); }); // clear only when the SYSCALL itself settles
	}
	return raw;
}
// Do two paths point to the SAME existing location, by filesystem identity (device id + inode)? Unlike samePath,
// this makes NO assumption about a volume's case behavior, so it is correct even on a case-SENSITIVE APFS volume or
// network share (where foldPath would over-merge two genuinely different names) AND on the case-INSENSITIVE default
// (where two case-variant names are one file, which the inode confirms). It also sees through a symlink to the same
// target. It stats, so it is async — use it for IDENTITY decisions (is this the same vault? does this record point
// at the same mount?); samePath / pathWithin stay the sync, conservative choice for lexical and containment guards.
// Exact-equal resolved paths short-circuit without a stat; if either path cannot be stat'd (it does not exist, or is
// unreadable) it falls back to the lexical samePath, so a stale record (a mount whose engine died) still matches.
async function sameFile(a, b) {
	if (a == null || b == null) return false;
	const ra = path.resolve(String(a)).normalize('NFC'), rb = path.resolve(String(b)).normalize('NFC');
	if (ra === rb) return true; // lexically identical — the same path, no stat needed
	try {
		// BOUND the stat: these paths are exactly the ones designed to wedge (FUSE mountpoints, vaults on removable or
		// network drives). An unbounded stat on a hung backend would freeze the mount-state lock and the schedule tick,
		// so a timeout falls back to the lexical compare — the pre-inode behavior — rather than ever hanging a caller.
		// Share one in-flight stat per path (see sameFileSharedStat) so repeated probes of a wedged path never pin more
		// than one threadpool slot. The pair promise gets its own swallow handler because, if the timeout wins the race
		// below, a later rejection from the stats themselves would otherwise be an unhandled rejection.
		const pair = Promise.all([sameFileSharedStat(ra), sameFileSharedStat(rb)]);
		pair.catch(() => {});
		const [sa, sb] = await withTimeout(pair, SAME_FILE_STAT_TIMEOUT_MS);
		// Trust inode identity ONLY when the volume reports real, non-zero inodes and devices. Some filesystems
		// (exFAT/FAT, a few network shares) report ino 0 for every file, which would FALSE-MERGE two distinct paths;
		// fall back to the conservative lexical compare there. External media is often exFAT, and vaults run in place.
		if (sa.ino && sb.ino && sa.dev != null && sb.dev != null) return sa.dev === sb.dev && sa.ino === sb.ino;
		return samePath(a, b);
	} catch (_) { return samePath(a, b); } // missing / unreadable / timed out — fall back to the conservative lexical compare
}
...
// Hash a FILE by streaming it through the digest one chunk at a time, so a file of ANY size never loads whole into
// memory (the no-OOM invariant) — for a shipped-asset checksum or a keyfile's identity, which must still cover every
// byte. One seam so the identical createReadStream + createHash bodies do not drift. Defaults to hex SHA-256; pass
// `encoding: 'base64'` (or another) or a different `algo` when a caller needs it.
function hashFileStream(filePath, { algo = 'sha256', encoding = 'hex' } = {}) {
	return new Promise((resolve, reject) => {
...
function defaultDataDir() {
	...
	// Prefer %LOCALAPPDATA%, then %APPDATA%. If a stripped environment (a bare service or SYSTEM account) leaves both
	// unset, fall back to the conventional per-user AppData\Local under the home directory, not the home root itself, so
	// the data folder always lands where a Windows user expects it and never clutters the profile root.
	if (process.platform === 'win32') return path.join(process.env.LOCALAPPDATA || process.env.APPDATA || path.join(home, 'AppData', 'Local'), Brand.name);
...
// Fail-safe test isolation. If this process was launched with a test file (…/lib/test/…) as its entry point, NEVER
// fall back to the user's real data directory — isolate to the project-local, git-ignored .test-data. This makes even
// a direct `node lib/test/foo.js` run (without the _setup preload) safe, so a test can never scatter state — the
// known-vaults list, locks, ledgers, the credential key — into the user's real vault data. An explicit setDataDir
// (the _setup preload, or a self-isolating test) still wins; production is unaffected because its entry point is
// vaultonaut.js, never a test file. Computed once (the entry point does not change during a run).
let _launchedFromTest;
function launchedFromTest() {
	if (_launchedFromTest === undefined) {
		// The entry script Node was launched with. Consider BOTH signals: require.main is the usual one, but it is
		// bound to process.mainModule when this module's `require` is created — and when Common is loaded via an `-r`
		// PRELOAD (how the test suite runs: `node -r ./lib/test/_setup.js <test>`) that binding is still undefined, so
		// require.main is permanently undefined here. process.argv[1] is the actual entry script path and is reliable
		// under a preload, a direct run, and the packaged app alike, so it is the primary signal; require.main is a
		// secondary confirm for any exotic launcher that does not populate argv[1].
		const candidates = [];
		try { if (require.main && require.main.filename) candidates.push(require.main.filename); } catch (_) {}
		try { if (process.argv && process.argv[1]) candidates.push(process.argv[1]); } catch (_) {}
		// Anchor to THIS project's own lib/test directory (via __dirname), not a bare "…/lib/test/…" substring, so a
		// production install whose path happens to contain "lib/test" cannot false-positive into test isolation.
		// Compare through pathWithin so the match is boundary-aware AND case-folded: macOS and Windows are
		// case-insensitive, so a test entry reached by a differently-cased path (e.g. a symlink or a shell that
		// lower-cases the drive letter) must still engage the fail-safe — a raw case-sensitive startsWith would miss
		// it and hand back the real user data directory.
		const ownTest = path.resolve(__dirname, 'test');
		_launchedFromTest = candidates.some((c) => pathWithin(c, ownTest));
	}
	return _launchedFromTest;
}
function dataDir() {
	if (dataDirOverride) return dataDirOverride;
	if (launchedFromTest()) return path.resolve(__dirname, '..', '.test-data'); // never the real user dir from a test entry
	return defaultDataDir();
}
// The per-user directory for daily-rotated diagnostic log files, under the data dir so it moves with --data-dir and
// stays per-user on every platform. The Logger writes here; nothing else should.
function logsDir() { return path.join(dataDir(), 'logs'); }
// Turn persistent diagnostic log FILES on or off. OFF by default so no vault-activity trail is written to disk — an
// encryption tool must not leave a forensic record of which vault opened and when it changed. A long-running service
// opts in (a setting or --diagnostic-log) only when the operator wants a trail for troubleshooting. The console and the
// in-memory live view are unaffected; this controls only what is persisted to disk.
function setDiagnosticLogging(on) { Logger.setFileLogging(on); }
...
// binDir can be pinned INDEPENDENTLY of dataDir. The test suite sets it (setBinDir) to share one already-downloaded
// engine across tests that each isolate their own data directory, so a test never re-downloads or needs a system
// rclone. Production may point it at a PRE-INSTALLED engine with the VAULTONAUT_ENGINE_DIR environment variable — the
// container image bakes the pinned, checksum-verified engine there so an air-gapped or egress-restricted node needs no
// first-serve download. Precedence: an explicit setBinDir() wins, then the environment variable, then the default
// under the data directory (which stays writable and is where a normal install downloads the engine).
...
function binDir() {
	...
	// Honor the environment override only in a real (non-test) process. A stray VAULTONAUT_ENGINE_DIR left in a dev or
	// CI shell must never steer a test run at some other engine directory; a test that has not called setBinDir falls
	// through to the isolated data directory's bin so it stays fully self-contained.
	if (!launchedFromTest() && process.env.VAULTONAUT_ENGINE_DIR) return process.env.VAULTONAUT_ENGINE_DIR;
	return path.join(dataDir(), 'bin');
}
// Is binDir() a PRE-INSTALLED engine directory supplied by the operator (the VAULTONAUT_ENGINE_DIR environment
// variable) rather than the writable default under the data dir? The container bakes a checksum-verified engine at a
// read-only /engine and points here at it, so a caller must NOT try to create or chmod this directory — those writes
// fail against a read-only root filesystem, and the directory is already present and correctly owned. An explicit
// setBinDir() (used by the test suite to share one downloaded engine) is NOT treated as pre-installed: that directory
// is the normal writable download location and is fine to manage.
function binDirIsPreinstalled() { return !binDirOverride && !launchedFromTest() && !!process.env.VAULTONAUT_ENGINE_DIR; }
function vaultsDir() { return path.join(dataDir(), 'vaults'); } // default home for vault storage
function storeDir() { return path.join(dataDir(), 'store'); }   // storage-node role: other people's vault ciphertext, in per-client slots (never adopted, never decryptable here)
...
// A compact UTC timestamp, e.g. "2026-08-29 08:57:14Z". Logs are written in UTC (not the host's local time) so a line
// reads the same whichever machine or timezone produced it — important for a headless service and for comparing logs
// across fleet nodes in different zones. The trailing "Z" marks it as UTC; the frontend converts to local for display.
function logStamp() {
	const d = new Date();
...
// Console PLUS a daily-rotated file (for a long-running/headless service), with secrets scrubbed once at this choke
// point so nothing reaches the terminal, a log file, or a viewer. File writes are non-blocking and best-effort.
function log(msg) { const line = logStamp() + ' ' + Logger.redact(String(msg == null ? '' : msg)); try { console.log(line); } catch (_) {} try { Logger.write(line); } catch (_) {} }
function warn(msg) { const line = logStamp() + ' ' + Logger.redact(String(msg == null ? '' : msg)); try { console.warn(line); } catch (_) {} try { Logger.write(line); } catch (_) {} }
...
// The default port the self-hosted relay hub listens on, and the default public data-port range it hands out. One
// source so the hub (`vdisk hub`), the serve-through-relay client (`vdisk serve --relay host`), and the stored relay
// registration can never disagree — a zero-config hub-and-node connect depends on them matching. A drift-guard test
// keeps the value honest.
const DEFAULT_RELAY_PORT = 7443;
const DEFAULT_RELAY_DATA_PORT_RANGE = [20000, 20099];
...
// The header carrying the owner-attach token on the /__owner-check probe (the packaged desktop shell asking a
// loopback instance to prove it is the trusted owner instance). Single-sourced for the same reason as the CSRF pair
// above: the sender (OwnerClient) and the checker (the web server) must use the identical name or the probe silently
// 404s and the shell cannot attach to an already-running instance.
const OWNER_TOKEN_HEADER = 'x-vaultonaut-owner';
// The process exit code the `ui` backend uses to tell the packaged desktop shell that a trusted instance is ALREADY
// running on the shared loopback port (the login/background service), so the shell should show that instance instead
// of failing on the busy port. One source so the backend (Commands.cmdUi) and the Rust shell can never disagree; a
// drift-guard test pins the shell's copy to this. Chosen to avoid the common/reserved codes (0, 1, 2, 126-128, 130).
const DESKTOP_ATTACH_EXIT_CODE = 97;
...
function withTimeout(promise, ms) {
	let timer;
	// The rejection carries `timedOut = true` so a caller can tell a genuine deadline breach (a wedged drive) apart
	// from the underlying operation's own failure, and react differently (surface a "not responding" error) instead of
	// lumping every rejection together. The message is unchanged, so existing callers that only log it are unaffected.
	const timeout = new Promise((_, reject) => { timer = setTimeout(() => { const e = new Error('timed out after ' + ms + 'ms'); e.timedOut = true; reject(e); }, ms); if (timer && timer.unref) timer.unref(); });
...
const MAX_JSON_BYTES = 16 * 1024 * 1024; // the shared byte ceiling for small JSON control files (manifest, state, sidecars); single-sourced and exported so every control-file reader uses the same bound and they cannot drift
// Move a corrupt or unreadable control file aside to `saved` BEFORE its store is rewritten, so a caller's fresh write
// can never clobber the only copy. Prefer a rename (retried, to ride out a transient Windows AV/indexer lock); if that
// still fails, or the paths are on different devices, fall back to a COPY. Returns whether a copy was preserved — a
// caller holding NON-rebuildable data (settings) can then refuse to overwrite, while a caller of a rebuildable store
// can proceed. The caller emits the user-facing message, since only it knows what the file held and what to advise.
async function preserveCorruptFile(p, saved) {
	try { await renameWithRetry(p, saved); return true; }
	catch (_) { try { await fsp.copyFile(p, saved); return true; } catch (_) { return false; } }
}
...
async function readJsonCorruptAside(p, { repair = false, label = 'data' } = {}) {
	...
	try { txt = await readFileCapped(p, MAX_JSON_BYTES, 'utf8'); }
		...
		if (repair) { const saved = p + '.corrupt-' + Date.now(); const kept = await preserveCorruptFile(p, saved); warn('A ' + label + ' file was unreadable (too large) and ' + (kept ? 'was moved aside (kept as a .corrupt-* copy); starting fresh.' : 'could not be preserved; starting fresh.')); }
		return null;
	}
	try { return JSON.parse(txt); }
	catch (_) {
		if (repair) { const saved = p + '.corrupt-' + Date.now(); const kept = await preserveCorruptFile(p, saved); warn('A ' + label + ' file was corrupt and ' + (kept ? 'was moved aside (kept as a .corrupt-* copy); starting fresh.' : 'could not be preserved; starting fresh.')); }
		return null;
	}
}
...
// NOTE: like fs.rename, this is a same-volume move — it does NOT handle a cross-device EXDEV rename. A caller that
// may move across volumes must catch EXDEV and fall back to copy+unlink itself (see the vault-unpack path).
...
// Candidate GUI "open this" launchers for Linux and other Unix desktops, in preference order. `xdg-open` (freedesktop)
// is on virtually every desktop, but a minimal or headless box may lack xdg-utils while still having one of the
// others, so we fall through the list. `unixOpener()` resolves — once, memoized, since this only runs on a rare user
// action (open a page / reveal a folder), not a hot path — which of these actually exists on PATH, returning null when
// NONE do so a caller can tell the user the path instead of silently failing. `openerCommand()` maps a resolved opener
// to its argv: `gio` needs an "open" subcommand; the rest take the target directly.
const UNIX_OPENERS = ['xdg-open', 'gio', 'kde-open5', 'kde-open', 'gnome-open', 'wslview'];
let _unixOpenerResolved = false, _unixOpener = null;
function unixOpener() {
	if (_unixOpenerResolved) return _unixOpener;
	_unixOpenerResolved = true;
	const dirs = String(process.env.PATH || '').split(path.delimiter).filter(Boolean);
	for (const name of UNIX_OPENERS) {
		for (const d of dirs) {
			try { if (fs.existsSync(path.join(d, name))) { _unixOpener = name; return name; } } catch (_) {}
		}
	}
	return null;
}
function openerCommand(name, target) { return name === 'gio' ? ['gio', ['open', target]] : [name, [target]]; }
...
// commands are the platform standards — `open` (macOS) and `start` via cmd (Windows) — plus a freedesktop-opener
// fallback chain on Linux/Unix, so no extra dependency or WebView plugin is needed, and the same call works from the
// CLI service and the desktop app. On Linux/Unix, returns false when no opener exists at all, rather than claiming
// success it cannot deliver.
function openExternal(url) {
	const plat = process.platform;
	// On Windows the launcher runs through cmd's `start`, whose arguments are hand-quoted below and passed verbatim
	// (see the spawn flag). A double quote or a control character in the URL could break out of that quoting, so refuse
	// one — the URLs opened here are app-generated (the loopback Recovery Kit page and similar), so this never rejects a
	// legitimate target. macOS and Linux pass the URL as a single argv entry with no shell, so they need no such guard.
	if (plat === 'win32' && /["\x00-\x1f\x7f]/.test(String(url))) return false;
	let cmd, args;
	if (plat === 'darwin') { [cmd, args] = ['open', [url]]; }
	else if (plat === 'win32') { [cmd, args] = ['cmd', ['/c', 'start', '""', '"' + url + '"']]; } // quoted empty title, then the quoted target, so a URL/path with a space or a cmd metacharacter can't mis-split
	else { const op = unixOpener(); if (!op) return false; [cmd, args] = openerCommand(op, url); } // Linux/Unix: use whichever freedesktop opener exists; if none, report failure so the caller can fall back
	// windowsVerbatimArguments: the args above are already quoted for cmd's `start`, so Node must pass the command line
	// through unchanged. Without it, Node re-escapes the embedded quotes ("" becomes "\"\""), and `start` — which does
	// not read \" as an escape — receives a mangled line, so the URL fails to open while cmd itself still launches: the
	// error callback never fires and this would wrongly return true. The flag is Windows-only and ignored elsewhere.
	try { const c = spawn(cmd, args, { detached: true, stdio: 'ignore', windowsHide: true, windowsVerbatimArguments: true }); c.on('error', () => {}); c.unref(); return true; }
	catch (_) { return false; }
}
...
// A vault is a folder on disk, so its name must be a valid, PORTABLE folder name — Windows is the strictest, so a name
// that works there works on macOS and Linux too. Enforced on EVERY platform at creation (not when opening an existing
// vault), so a vault made on one system still opens and mounts when it travels to another. Pure and synchronous — a
// string check — so it never blocks. `displayName` is the chosen name WITHOUT the vault extension (the caller adds that).
const WINDOWS_RESERVED_BASE = /^(con|prn|aux|nul|clock\$|com[1-9¹²³]|lpt[1-9¹²³])$/i; // reserved device names, case-insensitive, reserved even WITH an extension (CON.vault). COM0/LPT0 are NOT reserved (only 1-9, plus the superscript ¹²³ forms Win32 also maps); CLOCK$ is a legacy reserved DOS device.
const ILLEGAL_FILENAME_CHARS = /[<>:"/\\|?*\x00-\x1f]/;             // reserved on Windows and/or unportable; control chars break every system
function assertPortableVaultName(displayName) {
	const name = String(displayName == null ? '' : displayName);
	if (!name.trim()) throw new Error('A vault name is required.');
	// Cap the NAME well below a raw filesystem component limit, because on Windows the whole PATH — not just the name —
	// must fit MAX_PATH (260) on a default (non-long-path) system. A vault lives at
	// %LOCALAPPDATA%\Vaultonaut\vaults\<name>.vault\data\<encrypted-name>, so a 200-char name alone could push the
	// folder past 260 before any file is written. 64 leaves generous room for the data-dir prefix, the .vault\data
	// segment, and the (longer, base-encoded) encrypted file names inside — while staying ample for a real vault name.
	// Enforced at creation only, so an older vault with a longer name still opens.
	if (name.length > 64) throw new Error('That vault name is too long. Keep it to 64 characters or fewer so the whole vault path stays within Windows limits (a vault name is short by design; put longer descriptions elsewhere).');
	if (ILLEGAL_FILENAME_CHARS.test(name)) throw new Error('A vault name cannot contain any of these characters, which are not allowed in file names on some systems: < > : " / \\ | ? *');
	if (/^[ .]/.test(name) || /[ .]$/.test(name)) throw new Error('A vault name cannot begin or end with a space or a period, because some systems silently drop it. Trim the name and try again.');
	const base = name.split('.')[0]; // Windows reserves these even with an extension, so check the part before the first dot
	if (WINDOWS_RESERVED_BASE.test(base)) throw new Error('"' + base + '" is a reserved device name on Windows and cannot be used as a vault name. Choose a different name.');
}

// Collapse the current user's home directory at the START of a path to "~", for DISPLAY only — so the UI never shows
// the account name in a full path (a needless privacy leak in a screenshot or a screen share), while the real path is
// still used for every action. Cross-platform: it keeps the platform's own separator (so a Windows path stays
// "~\vaults\X"), and folds case only where the filesystem is case-insensitive by default (Windows and macOS), never on
// Linux. The tail is sliced from the ORIGINAL string, so the path's real casing after the home prefix is preserved.
// Pure and synchronous — a string check — so it never blocks. Returns the input unchanged when it is not under home.
function tildePath(p) {
	const s = String(p == null ? '' : p);
	const home = os.homedir();
	if (!s || !home) return s;
	const sep = path.sep;
	const h = home.endsWith(sep) ? home.slice(0, -1) : home; // tolerate a trailing separator on the home dir
	const ci = process.platform === 'win32' || process.platform === 'darwin'; // case-insensitive filesystems by default
	const fold = (x) => (ci ? x.toLowerCase() : x);
	if (fold(s) === fold(h)) return '~';
	if (fold(s).startsWith(fold(h + sep))) return '~' + s.slice(h.length); // s.slice keeps the leading separator + real tail casing
	return s;
}

module.exports = {
	root, appScriptPath, setDesktopApp, isDesktopApp, isSupervisedChild, supervisedChildEnv, SUPERVISED_ENV, dataDir, setDataDir, dataDirOverridePath, ensureDataDir, binDir, setBinDir, binDirIsPreinstalled, vaultsDir, storeDir, runDir, statePath, samePath, sameFile, pathWithin, resolveThroughSymlinks, pathExistsBounded, isLoopbackHost, redactPaths, unixOpener, openerCommand, sanitizeBwlimit, validateBwlimit, foldPath,
	timingSafeEqual, timingSafeEqualHashed, sha256, sha256Hex, hashFileStream, mapLimit, semaphore,
	log, warn, ensureDir, exeName, hardenDir, assertPortableVaultName, logsDir, setDiagnosticLogging,
	sleep, yieldToLoop, withTimeout, fetchWithTimeout, readCappedText, readFileCapped, readJsonCorruptAside, preserveCorruptFile, freshCache, isProcessAlive, pollUntil, writeJsonAtomic, fsyncPath, windowsHiddenRunVbs, uniqueTempPath, renameWithRetry,
	serialQueue, serialQueueByKey, splitHostPort, diskFree, openExternal, runCmd, runCmdOrThrow, suspiciousMassDeletion, failureBackoff, tildePath,
	SCHEMA_VERSION, schemaVersionFor, DEFAULT_UI_PORT, DEFAULT_RELAY_PORT, DEFAULT_RELAY_DATA_PORT_RANGE, OWNER_CSRF_HEADER, OWNER_CSRF_VALUE, OWNER_TOKEN_HEADER, DESKTOP_ATTACH_EXIT_CODE, MAX_JSON_BYTES
};
