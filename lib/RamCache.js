...
//
// The marker filename carries a PER-ALLOCATION token, not just the drive letter, so two instances that race for the
// same free letter get DIFFERENT marker files. The loser's allocation fails and it deletes only its OWN marker; it
// can no longer delete the winner's marker out from under a still-live RAM disk (which would leave that locked
// physical memory unswept if the winner later crashed). The sweep frees by the recorded device and removes by the
// actual filename it read, so it still reclaims markers from any older build regardless of the naming.
function winMarkerDir() { return path.join(Common.dataDir(), 'ramdisk-pending'); }
function winMarkerPath(device, token) { return path.join(winMarkerDir(), String(device).replace(/[^A-Za-z]/g, '') + (token ? '.' + String(token).replace(/[^a-f0-9]/gi, '') : '') + '.json'); }
async function writeWinMarker(handle) {
	try { await fsp.mkdir(winMarkerDir(), { recursive: true }); await Common.writeJsonAtomic(winMarkerPath(handle.device, handle.markerToken), { v: 1, device: handle.device, dir: handle.dir, at: Date.now() }); } catch (_) {}
}
async function removeWinMarker(handle) { try { await fsp.rm(winMarkerPath(handle.device, handle.markerToken), { force: true }); } catch (_) {} }
...
//
// On macOS and Windows the whole size is WIRED in physical memory the moment the RAM disk is created (an hdiutil
// `ram://` device / an ImDisk `awe` unit), and one is created PER mount — unlike a Linux tmpfs, which pages
// lazily. So the per-mount clamp alone cannot stop several open vaults from summing past the machine's memory and
// pressuring or exhausting it. Track how much this process's live RAM caches have reserved and bound the AGGREGATE
// to a fraction of physical memory: the generous default still applies to the first mount on a well-provisioned
// machine, but later mounts shrink and, once too little headroom remains, fall back to streaming (which writes
// nothing to disk, so the no-plaintext-on-disk guarantee still holds). A smaller-than-ideal cache at worst makes a
// single very large in-place write fail with an out-of-space error — never a system-wide out-of-memory.
const AGGREGATE_FRACTION = 0.5; // never reserve more than half of physical RAM across all of this process's caches
let reservedMB = 0;             // RAM (MB) currently reserved by this process's live RAM caches (see provision/release)
function aggregateCeilingMB() { try { const t = Math.floor(os.totalmem() / (1024 * 1024)); return t > 0 ? Math.floor(t * AGGREGATE_FRACTION) : 8192; } catch (_) { return 8192; } }
// Pure, testable: the size to give a NEW cache given the request, what is already reserved, and the ceiling.
// Returns 0 when too little headroom remains to wire a usable cache (the caller then streams). Never exceeds the
// per-mount [256, 8192] MB clamp, and never the remaining aggregate headroom.
function planSizeMB(opts, reserved, ceiling) {
	const want = opts && Number(opts.cacheSizeMB) > 0 ? Number(opts.cacheSizeMB) : 2048;
	const perMount = Math.max(256, Math.min(Math.round(want), 8192));
	const headroom = ceiling - reserved;
	if (headroom < 256) return 0;               // not enough left for even a minimum cache -> stream instead
	return Math.min(perMount, headroom);
}
function sizeMB(opts) { return planSizeMB(opts, reservedMB, aggregateCeilingMB()); }
...
async function provisionDarwin(id, opts) {
	...
	if (!mb) return null; // no aggregate headroom left -> stream (writes nothing to disk) rather than wire more RAM
...
async function provisionLinux(id, opts) {
	...
	// Probe with a bounded async access, not existsSync, so this mount-path step never does synchronous I/O on the
	// single-threaded service (a local tmpfs stat is microseconds, but keep the whole path async for consistency).
	try { await fsp.access('/dev/shm'); } catch (_) { return null; } // no /dev/shm -> stream instead (never fall back to a disk-backed /tmp)
	...
	let mb = sizeMB(opts);
	if (!mb) return null; // no aggregate headroom left -> stream instead of growing tmpfs further
...
async function provisionWin(id, opts) {
	...
	const mb = sizeMB(opts);
	if (!mb) return null; // no aggregate headroom left -> stream rather than wire more locked physical memory
	const letter = await freeDriveLetterWin();
	if (!letter) throw new Error('no free drive letter for the RAM cache');
	const handle = { dir: letter + '\\', device: letter, kind: 'imdisk', sizeMB: mb, markerToken: crypto.randomBytes(6).toString('hex') };
	...
	// Any failure of the allocation must remove the marker we just wrote. `Rclone.exec` REJECTS (does not return a
	// non-zero status) when imdisk is not on PATH — a spawn ENOENT — so a bare `if (r.status !== 0)` check would be
	// skipped on that path and the marker would leak, accumulating one stale pending-marker file per mount attempt on
	// every ImDisk-less PC. Handle both the non-zero-exit and the rejected-spawn cases in one place.
	let r;
	try {
		r = await Rclone.exec('imdisk', ['-a', '-s', mb + 'M', '-m', letter, '-o', 'awe', '-p', '/fs:ntfs /q /y'], { timeoutMs: 30000 });
	} catch (e) {
		await removeWinMarker(handle);
		throw e; // preserve the original error (e.g. spawn ENOENT) so provision() can render the "driver not installed" guidance
	}
	if (r.status !== 0) {
		await removeWinMarker(handle);
		// `-o awe` allocates AWE physical memory, which ImDisk can only do with the "Lock pages in memory"
		// (SeLockMemoryPrivilege) right — not granted to a standard Windows account by default. Detect that specific
		// denial and say how to fix it, rather than folding it into a generic failure. We deliberately do NOT retry
		// without `-o awe`: a non-AWE ImDisk disk is pagefile-backed, which could page decrypted data to disk and
		// break the never-on-disk guarantee this module exists to keep — streaming (writing nothing to disk) is the
		// correct, safe fallback instead.
		const err = String(r.stderr || r.status);
		if (/privilege|access is denied|0xc0000061|not held|SeLockMemory/i.test(err)) {
			throw new Error('imdisk could not lock physical memory for the RAM cache: this Windows account is missing the "Lock pages in memory" privilege (SeLockMemoryPrivilege). Grant it to your account (secpol.msc -> Local Policies -> User Rights Assignment -> "Lock pages in memory"), sign out and back in, then remount. Raw error: ' + err);
		}
		throw new Error('imdisk failed (is the RAM-disk driver installed?): ' + err);
	}
	return handle;
}
...
async function provision(id, opts) {
	try {
		let h = null;
		if (process.platform === 'darwin') h = await provisionDarwin(id, opts);
		else if (process.platform === 'linux') h = await provisionLinux(id, opts);
		else if (process.platform === 'win32') h = await provisionWin(id, opts);
		// Reserve the ACTUAL sized amount (the Linux path may have lowered it to fit /dev/shm) against the aggregate,
		// so the next mount's plan sees it. This counter tracks only THIS process's live caches, so it is decremented
		// solely in release() (on unmount and on every provision-failure path below); the cross-process sweep reclaims
		// other/dead processes' cache dirs, which never contributed to this process's reservedMB, so it leaves it alone.
		if (h && h.sizeMB > 0) reservedMB += h.sizeMB;
		return h;
	} catch (e) {
...
async function release(handle) {
	...
	try {
		...
		else if (handle.kind === 'imdisk' && handle.device) { await Rclone.exec('imdisk', ['-D', '-m', handle.device], { timeoutMs: 15000 }); await removeWinMarker(handle); }
	} catch (_) {}
	// Return this cache's reservation to the aggregate budget, exactly once (guard a double-release), so a later
	// mount can use the freed headroom. Kept in `finally`-style after the best-effort frees so the counter always
	// tracks provision/release pairs even if a detach threw.
	finally { if (handle.sizeMB > 0) { reservedMB = Math.max(0, reservedMB - handle.sizeMB); handle.sizeMB = 0; } }
}
	...
	try {
		if (process.platform === 'darwin') {
			...
			for (const n of markers) {
				...
				try { await fsp.rm(path.join(winMarkerDir(), n), { force: true }); } catch (_) {} // remove by the real filename read (handles any naming, old or new)
			}
		}
	} catch (_) {}
}

module.exports = { provision, release, sweep, planSizeMB, aggregateCeilingMB };
