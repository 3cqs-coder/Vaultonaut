...
async function healCore({ vaultDir, cipherDir, expectIndexHash, force, preserveInPlaceEdits }, onProgress) {
	...
	// it, rather than silently overwriting what could be a real edit with stale parity. Genuine same-size bit-rot is
	// still detected and is one explicit "Check & repair" away (that attended path does not set this flag). Requires
	...
	for (let s = 0; s < S; s++) {
		...
		for (let slot = 0; slot < k; slot++) {
			...
			// so skip and report it — the vault stays exactly as the user left it, and an explicit "Check & repair"
			// (which does not set this flag) still restores it if it truly is damage. `cur` is truthy here, so the
