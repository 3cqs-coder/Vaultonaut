...
async function readIndexMeta(vaultDir) {
	...
	const idx = await readIndex(vaultDir); // full read + hash-verify only when the index actually changed
	const meta = idx
		? { protected: true, tier: idx.tier, redundancyPercent: idx.redundancyPercent, createdAt: idx.createdAt, dataBlocks: idx.dataBlocks, thorough: !!idx.thorough }
		: { protected: false };
...
async function indexMetaCore({ vaultDir }) {
	const idx = await readIndex(vaultDir);
	return idx ? { protected: true, version: idx.version, indexHash: idx.indexHash } : { protected: false };
}
