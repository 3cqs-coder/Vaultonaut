...
async function listCipherFiles(cipherDir, exclude, tick) {
	...
	async function walk(dir, rel) {
		...
		for (const e of entries) {
			...
			else if (e.isFile() && !skip.has(e.name)) { const st = await fsp.stat(path.join(dir, e.name)); out.push({ path: relPath, size: st.size, mtimeMs: Math.floor(st.mtimeMs), mode: st.mode, uid: st.uid, gid: st.gid }); }
		}
	}
