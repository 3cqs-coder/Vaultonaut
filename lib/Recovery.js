...
// Ceiling on the number of 64 KiB data blocks a single vault's recovery data may cover. The index holds one small
// CRC record per block, so both the index's on-disk size AND the working memory the encode builds (the flat block
// list plus the per-block CRC array) grow in proportion to the vault's data. Left unbounded, a very large vault
// (hundreds of GB and up) would hold well over a gigabyte of live objects during protect — risking an out-of-memory
// in the worker — and could write an index larger than MAX_RECOVERY_INDEX_BYTES that readIndex then silently refuses
// to load, leaving the vault "protected" on disk yet unreadable. So above this ceiling, protect DECLINES and keeps
// any existing recovery data untouched (keep-last-good), the same discipline the content-search indexer uses for an
// oversized index. This bounds peak encode memory to a few hundred MB and keeps the index far under the read cap.
// The vault's own data is unaffected and fully usable at any size — this bounds only the supplementary parity
// self-heal. At 64 KiB per block, this covers vaults up to 256 GB; larger vaults rely on backups and mirroring.
...
async function protectCore({ vaultDir, tier = 'medium', cipherDir, thorough = false, exclude, versionFloor = 0, maxDataBlocks = MAX_DATA_BLOCKS }, onProgress) {
	...
	let blockCount = 0;
	for (const f of files) blockCount += (f.size === 0 ? 0 : Math.ceil(f.size / BLOCK_SIZE));
	...
	if (blockCount > maxDataBlocks) {
		return { vault: path.resolve(vaultDir), skipped: true, reason: 'too-large', dataBlocks: blockCount, maxDataBlocks,
			message: 'This vault is too large for parity self-heal (about ' + blockCount + ' blocks, over the ' + maxDataBlocks + '-block limit). Its data is fully usable at any size; protect against loss with backups and mirroring instead. Any existing recovery data was left in place.' };
	}
...
async function healCore({ vaultDir, cipherDir, expectIndexHash, force, preserveInPlaceEdits }, onProgress) {
	...
	for (let s = 0; s < S; s++) {
		...
		// The erasure solve. loss <= m guarantees enough survivors, every shard is BLOCK_SIZE, and readIndex pins the
		...
		for (let p = 0; p < m; p++) {
			...
			let fh;
			try { fh = await fsp.open(parityPath(vaultDir), 'r+'); const { bytesWritten } = await fh.write(shards[k + p], 0, BLOCK_SIZE, gp * BLOCK_SIZE); if (bytesWritten !== BLOCK_SIZE) throw new Error('short write'); repairedParity++; } // a short write is counted as an error, never a certified repair
			catch (_) { writeErrors++; }
			finally { if (fh) await fh.close().catch(() => {}); }
		}
	}
...
module.exports = { protect, signIndex, verify, heal, readIndex, readIndexMeta, hasRecovery, removeRecovery, isStale, staleCheck, missingProtectedCount, recoveryDir, CORES, TIERS, BLOCK_SIZE, MAX_DATA_BLOCKS };
