...
async function protectCore({ vaultDir, tier = 'medium', cipherDir, thorough = false, exclude, versionFloor = 0 }, onProgress) {
...
async function protect(vaultDir, { tier, cipherDir, thorough, exclude, signPriv, vaultId, manifestFloor = 0, onProgress } = {}) {
	...
	return serialize(abs, async () => {
		...
		const res = await dispatch('protect', { vaultDir: abs, tier, cipherDir, thorough, exclude, versionFloor }, onProgress);
		try {
