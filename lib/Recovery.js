...
const BLOCK_SIZE = 65536;   // 64 KiB: the SMALLEST (and default) data/parity block, used as-is for vaults that fit the
                            // block-count ceiling at this granularity. Larger vaults step the block size up (chooseBlockSize).
const MAX_BLOCK_SIZE = 1024 * 1024; // 1 MiB: the largest adaptive block. It bounds the per-stripe encode buffer — (k + m)
                            // shards of one block, with k <= K_MAX (128) — to a safe few hundred MB, and stays far under any
                            // large-single-read limit. With the block-count ceiling below, this lifts self-heal coverage to
                            // the multi-terabyte range (MAX_DATA_BLOCKS * MAX_BLOCK_SIZE) while keeping memory bounded.
...
// Ceiling on the number of data BLOCKS (not bytes) a single vault's recovery data may cover. The index holds one small
// CRC record per block, so both the index's on-disk size AND the working memory the encode builds (the flat block
// list plus the per-block CRC array) grow with the block COUNT, not the byte size. Left unbounded, a very large vault
// would hold well over a gigabyte of live objects during protect — risking an out-of-memory in the worker — and could
// write an index larger than MAX_RECOVERY_INDEX_BYTES that readIndex then silently refuses to load, leaving the vault
// "protected" on disk yet unreadable. Bounding the block COUNT bounds both. The BLOCK SIZE is what adapts to the vault
// (chooseBlockSize): a large vault uses a bigger block (up to MAX_BLOCK_SIZE) so its block count still fits here — the
// standard parity-archive approach of choosing a slice size that keeps the block count under the format's cap. So
// self-heal coverage is MAX_DATA_BLOCKS * MAX_BLOCK_SIZE (multiple terabytes), not a fixed byte limit, and peak encode
// memory stays a few hundred MB at any vault size. Only past that block*size product does protect DECLINE and keep any
// existing recovery data untouched (keep-last-good) — an honest, non-destructive limit, with backups/mirroring for the
// rare vault beyond it. The vault's own data is unaffected and fully usable at any size regardless.
const MAX_DATA_BLOCKS = 4_000_000;
...
const SIG_ALG = 'ed25519+ml-dsa-65', SIG_VERSION = 2; // hybrid AND-signature (classical + post-quantum); the read side branches on these so an older/newer scheme is clean
...
async function writeIndexSig(vaultDir, { vaultId, version, indexHash }, signPriv, signPqPriv) {
	const { sig, sigPq } = Integrity.signHybrid(signPriv, signPqPriv, sigMessage(vaultId, version, indexHash)); // signed with BOTH the classical and post-quantum keys
	const rec = { v: SIG_VERSION, alg: SIG_ALG, vaultId, version, indexHash, sig, sigPq };
...
async function authenticity(vaultDir, { vaultId, version, indexHash }, pubkey, pqPubkey) {
	...
	// Both published keys are needed to verify the hybrid signature. Missing either (e.g. a context without the
	// post-quantum key) is reported as unverified, never as tampering, so it is backstopped by the signed baseline.
	if (!pubkey || !pqPubkey) return { signed: true, verified: false, reason: 'no-pubkey', version };
	const msg = sigMessage(vaultId, version, indexHash);
	// AND-combiner: a replica is authentic only if BOTH its signatures verify. A single intact replica still rescues
	// a bit-rotten one; a genuine forgery (all replicas fail one or both signatures) is caught.
	return matching.some(s => Integrity.verifyHybrid(pubkey, pqPubkey, msg, s.sig, s.sigPq))
...
async function signIndexInPlace(vaultDir, { signPriv, signPqPriv, vaultId }) {
	if (!signPriv || !signPqPriv || vaultId == null) return null;
	...
	if (existing && existing.indexHash === idx.indexHash && existing.vaultId === vaultId && existing.v === SIG_VERSION) return { signed: true, signedVersion: idx.version, already: true };
	await writeIndexSig(vaultDir, { vaultId, version: idx.version, indexHash: idx.indexHash }, signPriv, signPqPriv);
...
async function listCipherFiles(cipherDir, exclude, tick, maxFiles = 0) {
	...
	async function walk(dir, rel) {
		...
		for (const e of entries) {
			...
			// mtimeMs is quantized to a whole millisecond (Math.floor): an integer serializes identically on every
			// platform and Node version, so the index self-hash (hashIndex) can never shift with a float-formatting
			// difference. Whole-millisecond precision is more than enough to restore a repaired file's timestamp.
			else if (e.isFile() && !skip.has(e.name)) {
				// Bound the working set by FILE COUNT, not just by block count. A vault with millions of empty or
				// sub-block files has a tiny block count (so the block ceiling never trips) yet one array entry per
				// file, which alone could exhaust the worker. Stop and signal BEFORE the array can grow past the
				// ceiling, so the caller declines gracefully (keep-last-good) instead of the worker being OOM-killed.
				// Only protect passes maxFiles; the read/verify paths (bounded already by the index read cap) do not.
				if (maxFiles && out.length >= maxFiles) throw Object.assign(new Error('The vault has too many files to protect.'), { code: 'RECOVERY_TOO_MANY_FILES', count: out.length });
				const st = await fsp.stat(path.join(dir, e.name)); out.push({ path: relPath, size: st.size, mtimeMs: Math.floor(st.mtimeMs), mode: st.mode, uid: st.uid, gid: st.gid });
			}
		}
	}
...
async function readBlock(absPath, offset, len, pool, bs = BLOCK_SIZE) {
...
async function readDataBlock(cipherDir, flat, i, pool, bs = BLOCK_SIZE) { const info = flat[i]; return readBlock(path.join(cipherDir, info.file), info.offset, info.len, pool, bs); }
async function readParityBlock(vaultDir, gp, pool, bs = BLOCK_SIZE) { return readBlock(parityPath(vaultDir), gp * bs, bs, pool, bs); }

// Choose the block size for a vault so its block COUNT fits under maxDataBlocks (which bounds index size and encode
// memory) at ANY byte size. Start at the fine-grained 64 KiB default and step up in powers of two until the count fits,
// capped at MAX_BLOCK_SIZE. Small vaults keep 64 KiB (the tightest repair granularity); only a large vault needs a
// bigger block. Powers of two keep the geometry simple and every non-final block a fixed size. This is the canonical
// parity-archive rule: pick a slice size that keeps the total block count under the format's ceiling.
function chooseBlockSize(files, maxDataBlocks) {
	const countAt = (bs) => { let c = 0; for (const f of files) c += (f.size === 0 ? 0 : Math.ceil(f.size / bs)); return c; };
	let bs = BLOCK_SIZE;
	while (bs < MAX_BLOCK_SIZE && countAt(bs) > maxDataBlocks) bs *= 2;
	return bs;
}
// A stored block size is valid if it is one of the powers of two chooseBlockSize can produce (64 KiB .. MAX_BLOCK_SIZE).
// readIndex uses this instead of pinning a single constant, so an index built at any adaptive size still loads, while a
// crafted size (huge, tiny, or non-power-of-two — which would drive wrong offsets or huge allocations) is still refused.
function isValidBlockSize(x) { return Number.isInteger(x) && x >= BLOCK_SIZE && x <= MAX_BLOCK_SIZE && (x & (x - 1)) === 0; }

// Build the flat block list (one entry per data block, in global order) from the file list, at block size `bs`.
function flattenBlocks(files, bs = BLOCK_SIZE) {
	const flat = [];
	for (const f of files) {
		const nBlocks = f.size === 0 ? 0 : Math.ceil(f.size / bs);
		for (let b = 0; b < nBlocks; b++) {
			const offset = b * bs;
			const len = Math.min(bs, f.size - offset);
...
// would change every existing indexHash. Every numeric field in the index is an integer (mtimeMs is quantized to a
// whole millisecond at capture), so JSON.stringify produces identical bytes on every platform and Node version —
// the self-hash cannot drift with float-to-string formatting.
...
async function protectCore({ vaultDir, tier = 'medium', cipherDir, thorough = false, exclude, versionFloor = 0, maxDataBlocks = MAX_DATA_BLOCKS }, onProgress) {
	const r = TIERS[tier] || TIERS.medium;
	const cdir = cipherOf(vaultDir, cipherDir);
	// Cap the enumeration by file count too (not only by block count below): a vault with millions of empty or tiny
	// files would otherwise build a files[] array proportional to the file count and exhaust the worker before the
	// block-count check could run. listCipherFiles stops and throws at the ceiling, and we decline (keep-last-good) —
	// the same honest, non-destructive outcome as the block-count decline. The ceiling is maxDataBlocks: a protectable
	// vault cannot have more than that many single-block files anyway, so this never rejects a vault protect could handle.
	let files;
	try { files = await listCipherFiles(cdir, exclude, heartbeat(onProgress, 'Scanning files'), maxDataBlocks); }
	catch (e) {
		if (e && e.code === 'RECOVERY_TOO_MANY_FILES') return { vault: path.resolve(vaultDir), skipped: true, reason: 'too-many-files', files: e.count, maxDataBlocks,
			message: 'This vault has too many files for parity self-heal (over the ' + maxDataBlocks + '-file limit). Its data is fully usable at any size; protect against loss with backups and mirroring instead. Any existing recovery data was left in place.' };
		throw e;
	}
	// Decline (keep-last-good) BEFORE building the O(D) block list if the vault has too many blocks to protect. The
	// count is derived cheaply from the file sizes here, so an over-ceiling vault never allocates memory proportional
	// to its size and never reads a single block — see MAX_DATA_BLOCKS. Leaving any existing recovery data untouched
	// is deliberate: a decline must never strip protection a smaller earlier version already earned.
	// Pick the block size FIRST so the block count is measured at the size we will actually use — a large vault gets a
	// bigger block, keeping its count under the ceiling instead of tripping the decline below.
	const bs = chooseBlockSize(files, maxDataBlocks);
	let blockCount = 0;
	for (const f of files) blockCount += (f.size === 0 ? 0 : Math.ceil(f.size / bs));
	if (blockCount === 0) throw new Error('The vault has no data to protect yet.');
	if (blockCount > maxDataBlocks) {
		// Only reachable once the block size is already at MAX_BLOCK_SIZE and the vault still overflows the count ceiling
		// (i.e. past MAX_DATA_BLOCKS * MAX_BLOCK_SIZE — multiple terabytes). Decline honestly and keep any existing data.
		return { vault: path.resolve(vaultDir), skipped: true, reason: 'too-large', dataBlocks: blockCount, maxDataBlocks,
			message: 'This vault is beyond the size parity self-heal can cover (about ' + blockCount + ' blocks at the largest block size). Its data is fully usable at any size; protect against loss with backups and mirroring instead. Any existing recovery data was left in place.' };
	}
	const flat = flattenBlocks(files, bs);
	const D = flat.length;
		...
		try {
			for (let s = 0; s < S; s++) {
				...
				for (let slot = 0; slot < k; slot++) {
					...
					if (gi < D) {
						const b = await readDataBlock(cdir, flat, gi, pool, bs);
					...
					} else shards[slot] = Buffer.alloc(bs); // zero-pad short stripes
				}
				const parity = rs.encode(shards);
				for (let p = 0; p < m; p++) {
					await parityFh.write(parity[p], 0, bs, (s * m + p) * bs);
	...
	// A monotonic version so a signature over an OLD index can never be replayed as the current one — the signed
	// payload binds it (see sigMessage). It steps past any prior index's version and the caller-supplied floor below;
	// if the prior index is unreadable it would otherwise restart at 1, but the floor (the highest signed version,
	// carried both in the local ledger and — crucially for a fresh machine — inside the signed manifest) prevents that
	// regression, and the downgrade gate refuses an index presenting below it.
	let priorVersion = 0;
	...
	// Step past the higher of the prior on-disk version and the caller-supplied anti-rollback floor (the highest
	// version this machine has anchored as signed), so a rebuild after total index loss cannot regress the version.
	const baseVersion = Math.max(priorVersion, Number.isInteger(versionFloor) ? versionFloor : 0);
	const index = {
		schemaVersion: RECOVERY_SCHEMA, tool: Brand.slug, createdAt: new Date().toISOString(), version: baseVersion + 1,
		tier, redundancyPercent: r, blockSize: bs, rs: { k, m }, stripes: S, dataBlocks: D,
...
async function readIndex(vaultDir) {
	...
	if (found) {
		// Validate the block size up front: the per-block length checks below use it, and verify/heal read and write at
		// its granularity, so a crafted value (huge, tiny, or non-power-of-two) must be refused before anything trusts it.
		if (!isValidBlockSize(found.blockSize)) throw new Error('The recovery data has an invalid block size and was refused — the vault\'s recovery folder may have been tampered with.');
		...
		for (const f of files) {
			...
			for (let bi = 0; bi < nb; bi++) {
				const b = f.blocks[bi];
				// Every block is a fixed full-size chunk (the index's blockSize) except the LAST, which holds the
				// 1..blockSize-byte remainder (an empty file has no blocks). Enforcing this — no zero-length or undersized
				// interior blocks — pins the block COUNT to ceil(size / blockSize), so a crafted index can't declare
				// millions of tiny blocks to drive verify/heal into an effectively unbounded loop while progress keeps the
				// worker watchdog from tripping. The existing f.size === sum(len) check then bounds the whole geometry.
				// The blockSize itself is validated once (isValidBlockSize) in the shape check below.
				const bsz = found.blockSize;
				const minLen = bi === nb - 1 ? 1 : bsz;
				if (!b || !Number.isInteger(b.len) || b.len < minLen || b.len > bsz || !Number.isInteger(b.crc)) { wellFormed = false; break; }
				fileBytes += b.len;
			}
			if (!wellFormed) break;
			...
			// crafted mode/uid/gid must not be applied. Keep only in-range non-negative integers, masked to the
			// permission bits (0o777) — the setuid/setgid/sticky bits are deliberately dropped, so a repair never
			// restores them onto a ciphertext store blob; anything else is skipped.
			f.mode = Number.isInteger(f.mode) && f.mode >= 0 ? (f.mode & 0o777) : undefined;
		...
		let ok = wellFormed && D >= 1 && isValidBlockSize(found.blockSize)
...
async function readIndexMeta(vaultDir) {
	...
	const meta = (m && m.protected)
		? { protected: true, tier: m.tier, redundancyPercent: m.redundancyPercent, createdAt: m.createdAt, dataBlocks: m.dataBlocks, thorough: !!m.thorough, exclude: Array.isArray(m.exclude) ? m.exclude : [] }
		: { protected: false };
...
async function verifyCore({ vaultDir, cipherDir }, onProgress) {
	...
	const bs = index.blockSize; // read/verify at the block size this index was built with (validated in readIndex)
	...
	try {
		for (let i = 0; i < D; i++) {
			const buf = await readDataBlock(cdir, flat, i, pool, bs);
		...
		for (let s = 0; s < S; s++) {
			for (let p = 0; p < m; p++) {
				const gp = s * m + p;
				const buf = await readParityBlock(vaultDir, gp, pool, bs);
...
async function healCore({ vaultDir, cipherDir, expectIndexHash, force, preserveInPlaceEdits }, onProgress) {
	const index = await readIndex(vaultDir);
	if (!index) return { protected: false };
	...
	await assertContained(cdir, index.files, heartbeat(onProgress, 'Checking vault safety')); // refuse a hostile vault that symlinks a blob path out of the vault
	const flat = flattenFromIndex(index);
	const bs = index.blockSize; // reconstruct at the block size this index was built with (validated in readIndex)
	...
	// Files with at least one block lost in a stripe we could NOT repair (a loss larger than this stripe's parity
	// budget — the case a whole-file deletion of a file bigger than the redundancy hits). Recorded here as raw
	// candidates, then classified after the pass into a genuine unrecoverable loss vs. a possible out-of-band edit, so
	// heal can report honestly WHICH files it could not fully rebuild instead of counting a still-broken file as fixed.
	const unrecoverableCandidates = new Set();
	...
	const recreatedFromMissing = new Set(); // files this run recreated from a MISSING blob (openForRepair fell to 'w')
	...
	// The residual ambiguity the size check above cannot resolve: a SAME-LENGTH in-place change. It is either silent
	// bit-rot (damage to repair) or a coherent edit written since protect at the same byte length (data to preserve).
	// With no vault key here, the two are indistinguishable at rest — the same conclusion mature parity-based integrity
	// tools reach for content that changed without being "blessed": they do NOT auto-fix modified blocks whose parity
	// was never updated to match; they report them and leave the fix to an explicit command. So on the UNATTENDED path
	// (preserveInPlaceEdits) this predicate marks such a block "possibly edited" and heal SKIPS the revert and reports
	// it, rather than silently overwriting what could be a real edit with stale parity. Every real heal entry point (the
	// CLI, the web "Check & repair", and the scheduled scrub) sets this flag; genuine same-size bit-rot is still detected
	// and is one explicit `--force` (the web "Check & repair" force option) away, which clears this flag. Requires
	// the file to still EXIST at exactly its protect-time size; a missing file (a loss to rebuild) or a resized file
	// (already handled by editedSinceProtect) is NOT this case and falls through to the normal repair.
	const inPlaceSkipped = new Set();
	async function presentAtProtectSize(encPath) {
		const meta = idxByFile.get(encPath); if (!meta) return false;
		try { return (await fsp.stat(path.join(cdir, encPath))).size === meta.size; } catch (_) { return false; }
	}
	...
	async function openForRepair(abs, rel) {
		return fsp.open(abs, 'r+').catch(async (e) => {
			...
			// The blob was missing, so heal is recreating this file from parity. Record it: if the same file also has
			// a stripe beyond the repair budget, the loss classification below must treat it as a genuine unrecoverable
			// loss (heal created its current partial on-disk size), not misread that size as a user edit and hide it.
			if (rel != null) recreatedFromMissing.add(rel);
	...
	for (let s = 0; s < S; s++) {
		...
		for (let slot = 0; slot < k; slot++) {
			...
			const gi = stripeSlotToBlock(s, slot, S);
			if (gi >= D) { shards[slot] = Buffer.alloc(bs); present[slot] = true; continue; } // virtual zero
			dataGlobal[slot] = gi;
			const buf = await readDataBlock(cdir, flat, gi, pool, bs);
		...
		if (loss === 0) continue;
		if (loss > m) { // beyond this stripe's repair budget — record which files lost a block here so heal can report them
			unrecoverableStripes++;
			for (let slot = 0; slot < k; slot++) if (!present[slot] && dataGlobal[slot] >= 0) unrecoverableCandidates.add(flat[dataGlobal[slot]].file);
			continue;
		}

		// The erasure solve. loss <= m guarantees enough survivors, every shard is one block (index.blockSize), and readIndex pins the
		// geometry, so this should never throw. But if an unexpected field error ever escaped, an uncaught throw here
		// would abort the WHOLE heal mid-run and leave every other repairable stripe unfixed, breaking the invariant
		// that one bad stripe never stops the run. Treat a throw exactly like an over-budget stripe: record the affected
		// files as an unrecoverable loss and press on to the next stripe.
		try { rs.reconstruct(shards, present); }
		catch (_) {
			unrecoverableStripes++;
			for (let slot = 0; slot < k; slot++) if (!present[slot] && dataGlobal[slot] >= 0) unrecoverableCandidates.add(flat[dataGlobal[slot]].file);
			continue;
		...
		for (let slot = 0; slot < k; slot++) {
			...
			const cur = await readDataBlock(cdir, flat, gi, pool, bs);
			...
			// UNATTENDED FAIL-SAFE: a same-length in-place mismatch on a still-present file is ambiguous (bit-rot vs. a
			// coherent out-of-band edit). On the scheduled path we must never silently revert what could be a real edit,
			// so skip and report it — the vault stays exactly as the user left it, and an explicit `--force` (the web
			// "Check & repair" force option, which clears this flag) still restores it if it truly is damage. `cur` is truthy here, so the
			// file's block is present (a trimmed/short file was already caught by editedSinceProtect above).
			if (preserveInPlaceEdits && !force && await presentAtProtectSize(flat[gi].file)) { inPlaceSkipped.add(flat[gi].file); continue; }
			...
			try { fh = await openForRepair(path.join(cdir, flat[gi].file), flat[gi].file); const { bytesWritten } = await fh.write(good, 0, flat[gi].len, flat[gi].offset); if (bytesWritten !== flat[gi].len) throw new Error('short write'); repairedData++; repairedFiles.add(flat[gi].file); } // a short write must not be certified as a repair — treat it as a write error and leave the block for the next verify
			catch (_) { writeErrors++; }
		...
		for (let p = 0; p < m; p++) {
			...
			const gp = s * m + p;
			const cur = await readParityBlock(vaultDir, gp, pool, bs);
			...
			let fh;
			try { fh = await fsp.open(parityPath(vaultDir), 'r+'); const { bytesWritten } = await fh.write(shards[k + p], 0, bs, gp * bs); if (bytesWritten !== bs) throw new Error('short write'); repairedParity++; } // a short write is counted as an error, never a certified repair
			catch (_) { writeErrors++; }
			finally { if (fh) await fh.close().catch(() => {}); }
		}
	}
	...
	// Classify the files that lost a block in a stripe beyond its repair budget: a size change means a user EDIT (not
	// damage), a same-size still-present file on the unattended path is a possible edit (deferred, never a false
	// "damaged"), and the rest are a genuine loss heal could NOT fully rebuild. Reuses the same size-based edit/damage
	// split the repair path uses, so the honest report of what could not be recovered never false-alarms on an edit.
	const unrecoverableFiles = [];
	for (const f of unrecoverableCandidates) {
		// A file heal RECREATED from a missing blob during THIS run, yet still lost a block beyond a stripe's repair
		// budget, is a GENUINE unrecoverable loss, not a user edit: heal itself wrote its current (partial) on-disk
		// size, so the size-based edit and same-size checks below would misread heal's own partial reconstruction as
		// an edit and hide the loss from the honest report. Skip those checks for it and report the loss directly. A
		// file that still existed (a possible user edit or a same-size change) falls through to the normal checks.
		if (!recreatedFromMissing.has(f)) {
			if (!force && await editedSinceProtect(f)) { editedFiles.add(f); continue; } // grew/shrank => a user edit, not damage
			if (preserveInPlaceEdits && !force && await presentAtProtectSize(f)) { inPlaceSkipped.add(f); continue; } // same size => possible edit, defer
		}
		unrecoverableFiles.push(f);
		repairedFiles.delete(f); // only partly rebuilt => NOT whole; don't count it as repaired or restore its metadata as if pristine
	}
	...
	for (const f of index.files) {
		...
		if (samePlatform && !isWin && f.mode != null) { try { await fsp.chmod(p, f.mode); } catch (_) {} } // POSIX mode bits are meaningless on Windows (chmod there only toggles read-only), so skip it like chown
		if (f.mtimeMs != null) { const t = new Date(f.mtimeMs); try { await fsp.utimes(p, t, t); } catch (_) {} }
	}
	if (onProgress) onProgress({ percent: 100, label: 'Done' });
	return { protected: true, repairedData, repairedParity, repairedSize, writeErrors, unrecoverableStripes, unrecoverable: unrecoverableFiles.length, unrecoverableFiles, healed: repairedData + repairedParity + repairedSize, repairedFiles: [...repairedFiles], changedSkipped: editedFiles.size, inPlaceDeferred: inPlaceSkipped.size, inPlaceDeferredFiles: [...inPlaceSkipped], indexHash: index.indexHash, version: index.version };
}
...
async function isStale(vaultDir, { cipherDir, deep = false, onProgress } = {}) {
	...
	const flat = flattenFromIndex(index);
	const bs = index.blockSize; // re-read at the index's block size (validated in readIndex)
...
// Worker core: count protected files now MISSING from the store. Runs in the worker so a large recovery-index parse
// never blocks the caller's event loop (same reason as the staleness/heal cores). Throws `indeterminate` when the
// index can't be read; the message propagates across the worker boundary, and both callers treat ANY throw here as
// fail-safe (defer the refresh / refuse the backup), so the flag not surviving the boundary changes nothing.
async function missingCore({ vaultDir, cipherDir }) {
	let index;
...
// Worker core for the staleness check — kept off the event loop for BOTH the cheap set+size check and the deep
// content check, so a large recovery index (hundreds of MB on a multi-terabyte vault) is never parsed and hashed on
// the caller's event loop. `deep` selects the thorough content comparison (see isStale's deep path).
async function staleCore({ vaultDir, cipherDir, deep = true }, onProgress) { return { stale: await isStale(vaultDir, { cipherDir, deep: !!deep, onProgress }) }; }
// Public staleness check: runs the comparison in the worker and returns a boolean. `deep` defaults to true so the
// existing thorough callers are unchanged; the unmount-time refresh passes deep:false for the cheap check, which now
// also runs in the worker instead of parsing the index on the main thread. Serialized per vault like the other
// recovery operations, so it never reads a half-written index mid-refresh.
async function staleCheck(vaultDir, { cipherDir, deep = true } = {}) {
	const abs = path.resolve(vaultDir);
	const r = await serialize(abs, () => dispatch('stale', { vaultDir: abs, cipherDir, deep: !!deep }));
	return !!(r && r.stale);
}
// Public loss check: runs the count in the worker, serialized per vault. A malformed result (no numeric count) is
// treated as indeterminate — the callers fail safe on it — so a worker hiccup can never read as "nothing missing".
async function missingProtectedCount(vaultDir, { cipherDir } = {}) {
	const abs = path.resolve(vaultDir);
	const r = await serialize(abs, () => dispatch('missing', { vaultDir: abs, cipherDir }));
	if (!r || typeof r.missing !== 'number') throw indeterminate('the loss check did not return a count.');
	return r.missing;
}
...
async function indexMetaCore({ vaultDir }) {
	...
	const idx = await readIndex(vaultDir);
	return idx
		// `exclude` (the small list of encrypted metadata-blob names this parity was built to skip) is surfaced too, so
		// the auto-refresh can tell — without re-reading the whole index — whether the index was built with an
		// out-of-date exclusion (e.g. after a key rotation re-encrypted every name) and rebuild it once.
		? { protected: true, version: idx.version, indexHash: idx.indexHash, tier: idx.tier, redundancyPercent: idx.redundancyPercent, createdAt: idx.createdAt, dataBlocks: idx.dataBlocks, thorough: !!idx.thorough, exclude: Array.isArray(idx.exclude) ? idx.exclude : [] }
		: { protected: false };
}
const CORES = { protect: protectCore, verify: verifyCore, heal: healCore, stale: staleCore, indexmeta: indexMetaCore, missing: missingCore };
...
function dispatch(op, args, onProgress) {
	// A heap ceiling as a backstop: protect declines a too-large vault gracefully before it can exhaust memory, but the
	// read/verify/heal/stale cores walk the current files (bounded by the index read cap in normal use). Should a vault
	// that was protected while small later grow enormous, this turns a would-be unbounded process crash into a clean,
	// bounded worker rejection the caller degrades from (best-effort). Matches the content-search worker's cap.
	return isMainThread
		? WorkerRun.runWorker(path.join(__dirname, 'RecoveryWorker.js'), { op, args }, onProgress, { idleMs: WORKER_IDLE_MS, idleMessage: 'A recovery operation', resourceLimits: { maxOldGenerationSizeMb: 2560 } })
...
async function protect(vaultDir, { tier, cipherDir, thorough, exclude, signPriv, signPqPriv, vaultId, manifestFloor = 0, maxDataBlocks, onProgress } = {}) {
	...
	return serialize(abs, async () => {
		// Anti-rollback floor for the monotonic index version: if the prior index is unreadable (all replicas lost or
		// corrupted), the rebuild would otherwise restart at version 1 and a later signature could not be told apart by
		// version from an older one. Seed the floor from the highest version this machine has anchored as SIGNED, so a
		// rebuild never regresses the version on a machine that has seen this vault before. (A brand-new machine has no
		// anchor; the in-vault signed index remains the cross-machine record.)
		let versionFloor = Number(manifestFloor) || 0; // the version signed into the manifest travels with the vault (a fresh machine has no ledger)
		if (vaultId != null) { try { versionFloor = Math.max(versionFloor, await Integrity.recoverySignedVersion(vaultId)); } catch (_) {} }
		const res = await dispatch('protect', { vaultDir: abs, tier, cipherDir, thorough, exclude, versionFloor, maxDataBlocks }, onProgress);
		// A declined (too-large) protect wrote nothing and must leave the existing index AND its signature untouched —
		// so return before the sign/remove branch below, which would otherwise strip the signature off good recovery data.
		if (res && res.skipped) return res;
		try {
			if (signPriv && signPqPriv && vaultId != null && res && res.indexHash) { await writeIndexSig(abs, { vaultId, version: res.version, indexHash: res.indexHash }, signPriv, signPqPriv); res.signed = true; res.signedVersion = res.version; }
...
async function signIndex(vaultDir, { signPriv, signPqPriv, vaultId } = {}) {
	const abs = path.resolve(vaultDir);
	return serialize(abs, () => signIndexInPlace(abs, { signPriv, signPqPriv, vaultId }).catch(() => null));
}
async function verify(vaultDir, { cipherDir, pubkey, pqPubkey, vaultId, onProgress } = {}) {
	const abs = path.resolve(vaultDir);
	const res = await serialize(abs, () => dispatch('verify', { vaultDir: abs, cipherDir }, onProgress));
	if (res && res.protected && res.indexHash) res.authenticity = await authenticity(abs, { vaultId, version: res.version, indexHash: res.indexHash }, pubkey, pqPubkey);
	return res;
}
async function heal(vaultDir, { cipherDir, pubkey, pqPubkey, vaultId, expectedFloor = 0, allowUnverified, force, preserveInPlaceEdits, onProgress } = {}) {
	const abs = path.resolve(vaultDir);
	return serialize(abs, async () => {
		...
		try { const m = await dispatch('indexmeta', { vaultDir: abs }); if (m && m.indexHash) { expectIndexHash = m.indexHash; auth = await authenticity(abs, { vaultId, version: m.version, indexHash: m.indexHash }, pubkey, pqPubkey); } } catch (_) {}
		...
		// The expected floor is the HIGHER of the local ledger anchor and the version signed into the manifest
		// (expectedFloor, supplied by the caller). The manifest floor is what protects a FRESH machine that has no
		// ledger record: the signed manifest carries the highest version, so a rolled-back index below it is refused.
		let expectedVer = Number(expectedFloor) || 0; try { expectedVer = Math.max(expectedVer, await Integrity.recoverySignedVersion(vaultId)); } catch (_) {}
		...
		const res = await dispatch('heal', { vaultDir: abs, cipherDir, expectIndexHash, force: !!force, preserveInPlaceEdits: !!preserveInPlaceEdits }, onProgress);
		if (res && res.protected && res.indexHash) { res.authenticity = await authenticity(abs, { vaultId, version: res.version, indexHash: res.indexHash }, pubkey, pqPubkey); if (auth.tampered && allowUnverified) res.repairedUnverified = true; }
		return res;
	});
}

module.exports = { protect, signIndex, verify, heal, readIndex, readIndexMeta, hasRecovery, removeRecovery, isStale, staleCheck, missingProtectedCount, recoveryDir, CORES, TIERS, BLOCK_SIZE, MAX_BLOCK_SIZE, MAX_DATA_BLOCKS };
module.exports._geom = { shapeFor, stripeSlotToBlock, K_MAX }; // test-only: the RS geometry algebra the whole self-heal rests on (recoverygeometry.js locks its invariants)
