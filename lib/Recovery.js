...
async function protect(vaultDir, { tier, cipherDir, thorough, exclude, signPriv, vaultId, onProgress } = {}) {
	...
	return serialize(abs, async () => {
		...
		let versionFloor = 0;
		if (vaultId != null) { try { versionFloor = await Integrity.recoverySignedVersion(vaultId); } catch (_) {} }
...
async function heal(vaultDir, { cipherDir, pubkey, vaultId, allowUnverified, force, preserveInPlaceEdits, onProgress } = {}) {
	const abs = path.resolve(vaultDir);
	return serialize(abs, async () => {
