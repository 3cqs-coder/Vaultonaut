...
async function deriveSecret(passphrase, params) {
	...
	try {
		...
		// A DETERMINISTIC parameter rejection (unsupported/out-of-range params) is not a worker problem — re-running it
		// in this thread would only re-throw the same error after a misleading "worker unavailable" warning. Surface it
		// as-is, without the warning or the fallback.
...
async function deriveSecretRaw(passphrase, params) {
	if (!params || params.algo !== 'argon2id' || (params.v != null && params.v !== 1)) {
		...
		e.code = 'KDF_PARAMS'; throw e; // deterministic: deriveSecret must NOT retry this in-thread or warn "worker unavailable"
	}
	...
	// hashLen is pinned to 32: the derived key feeds AES-256-GCM (wrapSecret/unwrapSecret), which needs exactly a
	// 32-byte key, so any other length is not something this build can consume — refuse it with the module's clean
	// message here rather than let it reach the cipher as a raw "invalid key length" throw. Argon2 also requires
	// memory of at least 8x the parallelism, so enforce that relation, not just the independent bounds, so a
	// self-inconsistent (memKiB, parallelism) pair is refused cleanly instead of throwing inside the hasher.
	if (!(okInt(p.memKiB, 8, 524288) && okInt(p.iterations, 1, 6) && okInt(p.parallelism, 1, 8) && p.hashLen === 32 && p.memKiB >= 8 * p.parallelism && typeof p.salt === 'string' && p.salt.length <= 4096)) {
		...
		e.code = 'KDF_PARAMS'; throw e; // deterministic: a re-run in the main thread would just re-throw, so don't warn/fall back
	}
