...
// Upper bounds for the UNTRUSTED, vault-supplied cost parameters, derived from the shipped LEVELS so the validator
// can never drift from the presets it is meant to bound: a vault may present at most the strongest preset's cost
// (512 MiB / 4 passes / 4 lanes), never more. Every legitimately-created slot uses a preset (via defaultParams), so
// it always falls within these bounds; only a hand-crafted or hostile manifest exceeds them, and is refused before
// any hasher or worker runs. Deriving from LEVELS keeps the bound in lockstep with the presets — a future release
// that adds a costlier preset widens the bound automatically, with no second number to update and drift from.
const KDF_MAX = {
	memKiB: Math.max(...Object.values(LEVELS).map(l => l.memKiB)),
	iterations: Math.max(...Object.values(LEVELS).map(l => l.iterations)),
	parallelism: Math.max(...Object.values(LEVELS).map(l => l.parallelism)),
};
...
// Far above any legitimate Argon2id run (even 'max' on a slow machine is seconds), so this absolute cap only ever
// fires for a worker that started but never settled — converting a would-be forever-hang on unlock into the
// in-thread fallback below.
const KDF_WORKER_MAX_MS = 120000;
let kdfFallbackWarnedAt = 0;
async function deriveSecret(passphrase, params) {
	// Validate the (untrusted, vault-supplied) parameters HERE, before offloading — a deterministic rejection must
	// fail fast in this thread. Validating only inside the worker would surface the rejection as a worker error whose
	// `.code` does not survive the worker boundary, so it would be misread as "worker unavailable" and pointlessly
	// re-tried (and re-rejected) in-thread with a misleading warning. deriveSecretRaw re-checks too, for its direct/
	// worker-body callers.
	assertKdfParams(params);
	await acquireKdfSlot();
	try {
		return await WorkerRun.runWorker(path.join(__dirname, 'KdfWorker.js'), { op: 'derive', args: { passphrase, params } }, null, { maxMs: KDF_WORKER_MAX_MS });
	} catch (e) {
		// A DETERMINISTIC parameter rejection is not a worker problem — surface it as-is (it was already thrown before
		// the worker ran, but guard here too for the worker-body path), without the warning or the fallback.
		if (e && e.code === 'KDF_PARAMS') throw e;
		// The worker offload is unavailable (unresolvable in a build, a crash, or the cap above). Deriving in this
		// thread is still correct, but it blocks the event loop for the memory-hard work — so a SYSTEMATICALLY broken
		// offload (e.g. a packaging regression) must be visible, not silently janky under concurrent unlocks. Log it,
		// rate-limited, then fall back (still bounded by the KDF slot we hold).
		const now = Date.now();
		if (now - kdfFallbackWarnedAt > 60000) { kdfFallbackWarnedAt = now; try { require('./Common').warn('Key derivation ran in the main thread because the worker was unavailable (' + (e && e.message || e) + '); unlocks may briefly pause. This is expected only if the packaged worker is missing.'); } catch (_) {} }
		return await deriveSecretRaw(passphrase, params);
	} finally {
		releaseKdfSlot();
	}
}

// Validate the (untrusted, vault-supplied) key-derivation parameters, failing closed with a clean, coded error. The
// parameters travel INSIDE a shareable vault, so a crafted memKiB or iterations would let a hostile vault OOM or
// CPU-hang the victim the instant they type a password. They are bounded TIGHTLY to the tool's own levels (max 512
// MiB / 4 passes) so a hostile manifest cannot amplify the unlock cost. hashLen is pinned to 32 — the derived key
// feeds AES-256-GCM (wrapSecret/unwrapSecret), which needs exactly 32 bytes — and memKiB must be at least 8x the
// parallelism, Argon2's own requirement, so a self-inconsistent pair is refused cleanly here rather than throwing
// inside the hasher. The `KDF_PARAMS` code marks the rejection DETERMINISTIC, so deriveSecret never mistakes it for a
// worker failure and re-runs it. Shared by deriveSecret (pre-offload) and deriveSecretRaw (direct / worker body).
function assertKdfParams(params) {
	if (!params || params.algo !== 'argon2id' || (params.v != null && params.v !== 1)) {
		const e = new Error('Unsupported key-derivation settings in this vault (algo=' + (params && params.algo) + ', v=' + (params && params.v) + '). A newer version of the tool may be required.');
		e.code = 'KDF_PARAMS'; throw e;
	}
	const p = params, okInt = (n, lo, hi) => Number.isInteger(n) && n >= lo && n <= hi;
	// The salt must also DECODE to at least a few bytes: `Buffer.from(x, 'base64')` silently drops non-base64
	// characters, so without a decoded-length floor a hostile or corrupt manifest could present a salt string that
	// decodes to zero or a couple of bytes. That is already fail-closed downstream (the hasher rejects an under-length
	// salt), but rejecting it HERE — deterministically, coded KDF_PARAMS, before the worker offload — gives a clear
	// "invalid parameters" error instead of a misleading "worker unavailable, falling back in-thread" path that just
	// re-fails. The genuine salt is 16 random bytes; 8 is a generous floor.
	const saltBytes = (typeof p.salt === 'string' && p.salt.length <= 4096) ? Buffer.from(p.salt, 'base64').length : 0;
	if (!(okInt(p.memKiB, 8, KDF_MAX.memKiB) && okInt(p.iterations, 1, KDF_MAX.iterations) && okInt(p.parallelism, 1, KDF_MAX.parallelism) && p.hashLen === 32 && p.memKiB >= 8 * p.parallelism && saltBytes >= 8)) {
		const e = new Error('This vault has invalid or out-of-range password-protection parameters and was refused.');
		e.code = 'KDF_PARAMS'; throw e;
	}
}
...
module.exports = { LEVELS, KDF_MAX, defaultParams, levelOf, sealTag, deriveSecret, deriveSecretRaw, deriveKey, wrapSecret, unwrapSecret };
