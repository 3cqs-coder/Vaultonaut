...
// Start, or reuse, a single bounded probe for a mount. Returns 'ok' (answered), 'fail' (answered
// with an error — e.g. the mount is gone/stale), 'timeout' (did not answer in time), or 'skip'
// (the outstanding-probe cap was reached — do not launch a new probe; keep the last known health).
// Uses Rclone.mountProbeCall (a bare access(), drive-letter aware) so this watch and the mount-detection
// check share ONE primitive: a WinFsp directory mount answers access() but can fail the heavier stat(),
// which was making a perfectly healthy Windows drive read as "not responding".
function probe(mp) {
	if (!pending.has(mp)) {
		if (pending.size >= MAX_OUTSTANDING) return Promise.resolve('skip'); // protect the worker-thread pool
		const p = Rclone.mountProbeCall(mp).then(() => 'ok', () => 'fail');
		pending.set(mp, p);
		// Clear the slot only when THIS probe settles, so a still-stuck probe keeps blocking new ones.
