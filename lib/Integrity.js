...
// The leaf hash for one file entry, as hex — so a verifier can recompute a leaf from the claimed {path,size,hash}
// and confirm the proof binds THAT exact file, never a leaf value handed to it.
function fileLeafHex(f) { return fileLeaf(f).toString('hex'); }

// A Merkle INCLUSION PROOF that one file is part of the signed set whose root merkleRoot() produced. Returns the
// target's canonical fields plus an ordered list of sibling hashes ({ side, hash }) from the leaf up to the root —
// so a third party can recompute the root from just this file and the (log N) siblings, without the whole set.
// Byte-identical tree to merkleRoot (same sort, same LEAF/NODE tags, same odd-node-promoted shape), and cooperative
// the same way, so building a proof over a huge vault never blocks the event loop. Returns null if the file is absent.
async function merkleProof(files, targetPath) {
	if (!files || !files.length) return null;
	const sorted = await sortFilesByPath(files);
	let idx = -1;
	for (let i = 0; i < sorted.length; i++) { if (sorted[i].path === targetPath) { idx = i; break; } }
	...
	const proof = [];
	while (level.length > 1) {
		// Record the sibling of the tracked node at THIS level. An odd last node has no sibling (it is promoted
		// unchanged), so nothing is recorded for that step — exactly mirroring merkleRoot's promotion.
		if (idx % 2 === 0) { if (idx + 1 < level.length) proof.push({ side: 'right', hash: level[idx + 1].toString('hex') }); }
		else { proof.push({ side: 'left', hash: level[idx - 1].toString('hex') }); }
		const next = [];
		for (let i = 0; i < level.length; i += 2) { next.push(i + 1 < level.length ? h(NODE, level[i], level[i + 1]) : level[i]); if (i > 0 && i % (YIELD_EVERY * 2) === 0) await yieldToLoop(); }
		idx = Math.floor(idx / 2);
		level = next;
	}
	return { path: target.path, size: target.size == null ? null : target.size, hash: target.hash || '', proof, root: level[0].toString('hex') };
}

// Recompute the root from a leaf and its inclusion path, and check it equals `rootHex`. Synchronous: the path is
// only ~log2(N) long, so this is a handful of hashes and cannot block. Fail-closed on any malformed input.
function verifyMerkleProof(leafHex, proof, rootHex) {
	try {
		...
		for (const step of (proof || [])) {
			...
			acc = step.side === 'left' ? h(NODE, sib, acc) : h(NODE, acc, sib);
		}
...
// Post-quantum companion signature: ML-DSA-65 (FIPS 204), derived DETERMINISTICALLY from the SAME write seed as the
// Ed25519 key, so the write-cap alone reproduces both — no extra key material is stored and the read/write capability
// split is unchanged. The two are combined as an AND-signature (verify requires BOTH), so a baseline stays authentic
// as long as EITHER scheme is unbroken: Ed25519 guards today, ML-DSA guards against a future quantum forger. This is
// the same NIST security level (3) as the ML-KEM-768 already used for sealing, so the whole suite is consistent.
// ML-DSA is provided natively by OpenSSL 3.5 (Node 24.7+), so no dependency is added. The private key is imported from
// a 32-byte SEED: MLDSA_PKCS8_SEED_PREFIX is the fixed DER header of an ml-dsa-65 PKCS#8 OneAsymmetricKey whose
// privateKey is the `[0]` seed choice, so `prefix || seed` is a complete seed-format key OpenSSL expands the same way
// every time. The seed is a domain-separated HKDF subkey of the write seed, independent of the Ed25519 key.
const MLDSA_PKCS8_SEED_PREFIX = Buffer.from('3034020100300b060960864801650304031204228020', 'hex');
function mldsaSeedFrom(seed32) {
	return Buffer.from(crypto.hkdfSync('sha256', seed32, Buffer.alloc(0), Buffer.from('vdisk-mldsa-sign-v1'), 32));
}
function mldsaKeysFromSeed(seed32) {
	const priv = crypto.createPrivateKey({ key: Buffer.concat([MLDSA_PKCS8_SEED_PREFIX, mldsaSeedFrom(seed32)]), format: 'der', type: 'pkcs8' });
	const pub = crypto.createPublicKey(priv).export({ format: 'der', type: 'spki' }).toString('base64'); // SPKI DER, base64 — the compact form stored/pinned next to the Ed25519 public key
	return { priv, pub };
}
// Sign a payload with the ML-DSA private key (base64 signature). Pure, native, cross-platform.
function signPq(pqPriv, payload) { return crypto.sign(null, Buffer.from(payload, 'utf8'), pqPriv).toString('base64'); }
// Verify an ML-DSA signature against the base64 SPKI public key. Never throws — a malformed key or signature is a
// clean `false`, so a hostile or corrupt record can only fail the check, never crash the verifier.
function verifyPq(pqPubB64, payload, sigB64) {
	try {
		if (!pqPubB64 || !sigB64) return false;
		const pub = crypto.createPublicKey({ key: Buffer.from(String(pqPubB64), 'base64'), format: 'der', type: 'spki' });
		return crypto.verify(null, Buffer.from(payload, 'utf8'), pub, Buffer.from(String(sigB64), 'base64'));
	} catch (_) { return false; }
}
// Hybrid AND-signature helpers — one place that signs (and one that verifies) with BOTH schemes over the SAME bytes,
// so no caller can accidentally produce or accept one half without the other. Every signed artifact (baseline, manifest
// seal, and any future site) routes through these, which is what makes the two signatures impossible to get out of step.
// Produce a hybrid signature: always the Ed25519 half, plus the ML-DSA half WHEN this runtime can do ML-DSA. On a
// runtime that genuinely lacks ML-DSA (a Node built against pre-3.5 OpenSSL), pqPriv is null (see signKeysFromSeed's
// capability-gated getter), so the post-quantum half is omitted rather than throwing — symmetric with verifyHybrid,
// which accepts the classical half alone on that same runtime. This keeps every signer (create, seal, roster, reseal,
// recovery, succession, checkpoint, session) consistent instead of some throwing and some silently swallowing the
// throw and leaving a stale signature. It is NOT a downgrade on a capable machine: pqAvailable() is a once-per-process
// probe an attacker cannot flip per record, so a PQ-capable runtime always signs both halves.
function signHybrid(edPriv, pqPriv, payload) { return { sig: sign(edPriv, payload), sigPq: pqPriv ? signPq(pqPriv, payload) : null }; }
function verifyHybrid(edPub, pqPub, payload, sig, sigPq) {
	if (!verify(edPub, payload, sig)) return false; // the classical half must always verify
	// If this runtime CANNOT do ML-DSA at all (a Node built against pre-3.5 OpenSSL — see pqAvailable), the post-quantum
	// half is unverifiable here, so accept the classical signature alone rather than reading a genuine, untampered record
	// as forged. A runtime's lack of a capability is not evidence of tampering: without this, a healthy vault opened on
	// such a Node would false-alarm as tampered and self-heal would be blocked. This is NOT a downgrade an attacker can
	// exploit — the capability is probed once per process (an attacker cannot flip it per record), and on a runtime that
	// truly lacks ML-DSA there is no post-quantum protection to strip in the first place. The boot self-check
	// (pq_signing) tells the user to move to a runtime that has ML-DSA.
	if (!pqAvailable()) return true;
	return verifyPq(pqPub, payload, sigPq);
}
// Is ML-DSA (the post-quantum half) actually usable on this runtime? Node 24.7+ ships it via OpenSSL 3.5, but a Node
// built against an OLDER system OpenSSL (some distro/self-built binaries) passes the version gate yet lacks ml-dsa-65.
// The whole integrity suite signs and verifies with it, so this is probed ONCE (a throwaway derive+sign+verify) and
// cached, letting the boot self-check warn the user with an actionable message instead of every signing op throwing a
// raw OpenSSL error and every release check false-alarming as "altered". Mirrors Emergency.js's ML-KEM capability probe.
let _pqAvailable = null;
let _pqForced = null; // test-only override (see _forcePqAvailable); null = use the real probe
function pqAvailable() {
	if (_pqForced !== null) return _pqForced;
	if (_pqAvailable !== null) return _pqAvailable;
	try {
		const k = mldsaKeysFromSeed(Buffer.alloc(32, 1));
		_pqAvailable = verifyPq(k.pub, 'probe', signPq(k.priv, 'probe'));
	} catch (_) { _pqAvailable = false; }
	return _pqAvailable;
}
// Test-only: force pqAvailable() to a value (or null to restore the real probe), so a test can exercise the classical
// fallback and the self-check's error path without a runtime that actually lacks ML-DSA. Never called in production.
function _forcePqAvailable(v) { _pqForced = v; }
...
function signKeysFromSeed(seedB64) {
	...
	// The ML-DSA companion is derived LAZILY: the many callers that only read the Ed25519 `pub` (identity checks, key
	// comparisons) pay nothing, while a signer or verifier that touches pqPriv/pqPub computes it once, on first access.
	let pq = null;
	const lazy = () => (pq || (pq = mldsaKeysFromSeed(seed)));
	// The ML-DSA getters are capability-gated: on a runtime that cannot do ML-DSA at all they return null instead of
	// throwing a raw OpenSSL error deep inside a signer. signHybrid then omits the post-quantum half (and verifyHybrid
	// already accepts the classical half alone on that runtime), so the whole suite degrades gracefully rather than
	// having some call sites throw and others swallow the throw into a stale signature. A capable runtime is unchanged.
	return { priv, pub: spki.subarray(spki.length - 32).toString('hex'), get pqPriv() { return pqAvailable() ? lazy().priv : null; }, get pqPub() { return pqAvailable() ? lazy().pub : null; } };
}
...
//   'chain-break' — the record does not connect to the previous one (a spliced or reordered history).
...
function signTamperHead(vid, signPriv, signPqPriv) {
	return withLedgerLock(async () => {
		const head = ((await readTamperLog())[vid] || [])[0];
		if (!head || !head.hash || !signPriv || !signPqPriv) return;
		const led = await readLedger();
		led.tamperHeads = led.tamperHeads || {};
		const cur = led.tamperHeads[vid] || { n: head.n, hash: head.hash, evicted: 0 };
		const s = signHybrid(signPriv, signPqPriv, tamperHeadMessage(vid, head.n, head.hash)); // the checkpoint is co-signed with both schemes
		cur.checkpoint = { n: head.n, hash: head.hash, sig: s.sig, sigPq: s.sigPq };
...
async function verifyTamperLog(vid, pub, pqPub) {
	...
	if (cp && cp.sig) { // a signed checkpoint must verify (both schemes) and cannot sit ahead of the current head
		if (!pub || !verifyHybrid(pub, pqPub, tamperHeadMessage(vid, cp.n, cp.hash), cp.sig, cp.sigPq)) return { ok: false, reason: 'checkpoint-forged', verdict: 'altered' };
...
module.exports = {
	SCHEME, HASH,
	merkleRoot, merkleProof, verifyMerkleProof, fileLeafHex, sortFilesByPath, fingerprint, identity, groupB32,
	signKeysFromSeed, readKeyFromSeed, sign, verify, signPq, verifyPq, signHybrid, verifyHybrid, pqAvailable, _forcePqAvailable, signingInput, equalHex,
