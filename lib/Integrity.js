...
function observe(vid, seq, root, prevRoot, identity, identityAuthorized = false, sealed = undefined) {
	return withLedgerLock(async () => {
		...
		if (!cur || seq > cur.seq) {
			// Re-read immediately before writing and take the per-vault max. The CLI and the web
			// server are separate processes and the in-process lock does not span them, so this
			// merge NARROWS the lost-update window — it does not close it: two processes that read,
			// then write, interleaved can still clobber each other's newest entry. The intended
			// deployment is a single long-running service, where the in-process lock is authoritative;
			// this best-effort merge keeps the common CLI-alongside-service case moving forward. (A
			// full guarantee would need a cross-process lock or per-vault sidecar files.)
...
// The highest recovery-index version this app has seen SIGNED for a vault — a sticky rollback anchor kept in the
// local ledger. It lets the password-less recovery verify/heal path flag a DOWNGRADE: a recovery index that
// arrives unsigned, or at a lower version than one already seen signed, is exactly what an attacker who STRIPS
// the signature (to force the weaker "unsigned" path) would leave behind. Local-only; the in-vault signed
// baseline remains the cross-machine backstop. Advances monotonically, never backward.
function noteRecoverySigned(vid, version) {
	if (vid == null || !Number.isInteger(version)) return Promise.resolve();
	return withLedgerLock(async () => {
		const led = await readLedger();
		led.recoverySigned = led.recoverySigned || {}; // a named key (not 32-hex) never collides with a vault-id entry
		if (!(led.recoverySigned[vid] >= version)) {
			led.recoverySigned[vid] = version;
			led.schemaVersion = Common.schemaVersionFor('rollback ledger', led);
			await Common.writeJsonAtomic(ledgerPath(), led, { fsync: true });
		}
	});
}
async function recoverySignedVersion(vid) { return ((await readLedger()).recoverySigned || {})[vid] || 0; }
...
// The highest share-roster EPOCH this app has seen for a vault — the same sticky, local rollback anchor, applied
// to the signed list of who has access. The roster is signed, but a signature alone cannot stop someone who can
// write the vault folder from restoring an OLDER, still-validly-signed roster (from before a revocation). Anchor
// the highest epoch here so a restored older roster is detected as a rollback. Local-only, monotonic, never backward.
function noteSharesEpoch(vid, epoch) {
	if (vid == null || !Number.isInteger(epoch)) return Promise.resolve();
	return withLedgerLock(async () => {
		const led = await readLedger();
		led.sharesEpoch = led.sharesEpoch || {}; // a named key (not 32-hex) never collides with a vault-id entry
		if (!(led.sharesEpoch[vid] >= epoch)) {
			led.sharesEpoch[vid] = epoch;
			led.schemaVersion = Common.schemaVersionFor('rollback ledger', led);
			await Common.writeJsonAtomic(ledgerPath(), led, { fsync: true });
		}
	});
}
async function sharesEpochSeen(vid) { return ((await readLedger()).sharesEpoch || {})[vid] || 0; }

// The same anti-rollback anchor for a TEAM vault's MEMBER roster. The roster (and its epoch) live inside the
// signed manifest, so a valid signature alone can't stop someone who can write the vault folder from restoring an
// OLDER, still-validly-signed manifest from before a member was removed — which would re-list that member's slot.
// Anchor the highest members-epoch this machine has seen so a restored older manifest is detected as a rollback.
// Local-only, monotonic, never backward — exactly like the shares-epoch anchor above.
function noteMembersEpoch(vid, epoch) {
	if (vid == null || !Number.isInteger(epoch)) return Promise.resolve();
	return withLedgerLock(async () => {
		const led = await readLedger();
		led.membersEpoch = led.membersEpoch || {}; // a named key (not 32-hex) never collides with a vault-id entry
		if (!(led.membersEpoch[vid] >= epoch)) {
			led.membersEpoch[vid] = epoch;
			led.schemaVersion = Common.schemaVersionFor('rollback ledger', led);
			await Common.writeJsonAtomic(ledgerPath(), led, { fsync: true });
		}
	});
}
async function membersEpochSeen(vid) { return ((await readLedger()).membersEpoch || {})[vid] || 0; }

// The same anti-rollback anchor for a vault's KEY-SLOT set (crypt.slotEpoch). Restoring an older, still-validly-
// signed manifest could re-list a removed extra password, keyfile, or read-only slot — the manifest seal alone
// cannot stop it, because the old manifest was legitimately signed. Anchor the highest slot epoch this machine has
// seen so a restored older manifest is detected. Local-only, monotonic, never backward — like the members anchor.
function noteSlotEpoch(vid, epoch) {
	if (vid == null || !Number.isInteger(epoch)) return Promise.resolve();
	return withLedgerLock(async () => {
		const led = await readLedger();
		led.slotEpoch = led.slotEpoch || {}; // a named key (not 32-hex) never collides with a vault-id entry
		if (!(led.slotEpoch[vid] >= epoch)) {
			led.slotEpoch[vid] = epoch;
			led.schemaVersion = Common.schemaVersionFor('rollback ledger', led);
			await Common.writeJsonAtomic(ledgerPath(), led, { fsync: true });
		}
	});
}
async function slotEpochSeen(vid) { return ((await readLedger()).slotEpoch || {})[vid] || 0; }
...
module.exports = {
	...
	vaultId, pubkeyOf, observe, lastSeen, noteRecoverySigned, recoverySignedVersion, clearRecoverySigned, noteSharesEpoch, sharesEpochSeen, noteMembersEpoch, membersEpochSeen, noteSlotEpoch, slotEpochSeen, noteRecoveryDropped, clearRecoveryDropped, recoveryDroppedAt, logTamper, tamperLog, verifyTamperLog, signTamperHead
};
