...
<dict>
  ...
  <key>ThrottleInterval</key><integer>10</integer>
	...
	if (platform === 'linux') {
		...
		// No `After=/Wants=network-online.target`: this is a systemd *user* unit (installed under ~/.config/systemd/user
			// and started by `systemctl --user`), and network-online.target is a system target that generally does not
			// exist in the per-user manager — so the ordering would silently no-op and only mislead. The UI binds
			// loopback by default (no network needed at all), and even a `--bind` to a network address is handled by the
			// listen path, so no network ordering is required here.
			return `[Unit]
...
// Read the installed service file ONCE (async) and extract its bind/script/node together, instead of re-reading the
// same file three times with blocking *Sync on the shared service's status poll. Returns nulls when it is absent or
// unreadable (the .vbs is UTF-16; the plist/unit are UTF-8). `access` is used for the existence check for the same
// non-blocking reason.
async function fileExists(p) { try { await fsp.access(p); return true; } catch (_) { return false; } }
async function readServiceFacts(servicePath, windows) {
	if (!servicePath) return { bind: null, script: null, node: null };
	try {
		const text = await fsp.readFile(servicePath, windows ? 'utf16le' : 'utf8');
		return { bind: bindFromServiceText(text), script: scriptFromServiceText(text), node: nodeFromServiceText(text) };
	} catch (_) { return { bind: null, script: null, node: null }; }
}
...
async function status() {
	...
	if (info.platform === 'windows') {
		const { bind, script, node } = await readServiceFacts(info.vbsPath, true); // one read, not three
		...
		catch (_) { return { platform: 'windows', installed: await fileExists(info.vbsPath), supported: true, servicePath: info.vbsPath, bind, script, node }; }
	}
	const installed = await fileExists(info.servicePath);
	const { bind, script, node } = installed ? await readServiceFacts(info.servicePath, false) : { bind: null, script: null, node: null };
	return { platform: info.platform, installed, supported: true, servicePath: info.servicePath, bind, script, node };
}
