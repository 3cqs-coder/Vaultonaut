...
async function status() {
	...
	if (info.platform === 'windows') {
		...
		try { await sh('reg', ['query', 'HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run', '/v', info.taskName]); return { platform: 'windows', installed: true, supported: true, servicePath: info.vbsPath, bind, script }; }
		catch (_) { return { platform: 'windows', installed: fs.existsSync(info.vbsPath), supported: true, servicePath: info.vbsPath, bind, script }; }
	}
	const installed = fs.existsSync(info.servicePath);
	return { platform: info.platform, installed, supported: true, servicePath: info.servicePath, bind: installed ? installedBind(info.servicePath, false) : null, script: installed ? installedScript(info.servicePath, false) : null };
}

module.exports = { install, uninstall, status, bindFromServiceText, scriptFromServiceText, buildServiceText, resolveBindArg };
