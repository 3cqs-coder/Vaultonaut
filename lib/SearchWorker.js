...
async function readText(mountpoint, rel, size) {
	...
	try {
		if (D.isExtractable(rel)) {
			if (size > D.EXTRACT_MAX_BYTES) return ''; // too large to parse safely — index by name only (no read at all)
			const buf = await fsp.readFile(abs);
			if (buf.length > D.EXTRACT_MAX_BYTES) return ''; // backstop if the file grew since the walk stat
			const t = await Extract.extractText(buf, D.extOf(rel), D.PER_FILE_TEXT_CAP);
	...
	index: async ({ mountpoint }, progress) => {
		...
		async function walk(rel) {
			...
			for (const e of ents) {
				...
				let st; try { st = await fsp.stat(path.join(mountpoint, r)); } catch (_) { continue; }
				if (st.size > D.HARD_FILE_CEIL) continue;
				current[r] = { size: st.size, mtimeMs: Math.round(st.mtimeMs) };
