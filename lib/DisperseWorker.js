...
async function encode({ archivePath, files, n, k }, progress) {
	...
	// packed archive is bounded" invariant is local to the worker rather than resting on the orchestrator alone. RS
	// coding holds the whole blob (plus the shard set) in memory, so an over-budget archive would balloon this
	// worker's heap; failing fast with a clear message beats a generic ERR_FS_FILE_TOO_LARGE or an out-of-memory kill.
