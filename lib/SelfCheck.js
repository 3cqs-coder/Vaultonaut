...
// Passive event-loop-delay monitor, started once when this module loads so it is always accumulating. Each sweep of
// the event_loop_lag check reads the worst stall since the previous sweep and resets, which turns the tool's standing
// "never block the event loop" rule into a check that would actually CATCH a synchronous crypto, compression, or OCR
// step regressing on the shared web and background service. monitorEventLoopDelay's own sampling timer is unref'd, so
// it never keeps the process alive; the whole thing is feature-probed and wrapped so a runtime without it (or a future
// removal of it) simply leaves the check disabled rather than throwing. Available on every platform Node runs on.
let loopDelayHist = null;
try {
	if (typeof perfHooks.monitorEventLoopDelay === 'function') { loopDelayHist = perfHooks.monitorEventLoopDelay({ resolution: 20 }); loopDelayHist.enable(); }
} catch (_) { loopDelayHist = null; }

const LOOP_LAG_MAX_MS = 1000; // a single main-thread stall this long on the shared service is a real block, not GC jitter
const HEAP_HEADROOM_PCT = 90; // warn when the V8 heap reaches this % of its hard ceiling — the next big allocation may OOM

// The raw readings the two runtime-health checks act on, behind small seams. Each returns null when its source is
// unavailable, so the check simply goes quiet on a runtime that lacks it. The seams are overridable (see the _force*
// exports) purely so a guard can drive the firing path deterministically — without having to actually stall the loop
// or exhaust the heap — the same way the other checks are tested by swapping the module function they read from.
const defaultLoopLagReader = () => {
	if (!loopDelayHist) return null;
	try { const r = { maxMs: loopDelayHist.max / 1e6, meanMs: loopDelayHist.mean / 1e6 }; loopDelayHist.reset(); return r; } catch (_) { return null; }
};
const defaultHeapReader = () => { try { return v8.getHeapStatistics(); } catch (_) { return null; } };
let readLoopLag = defaultLoopLagReader;
let readHeap = defaultHeapReader;
...
register('engine_integrity', async () => {
	const Setup = require('./RcloneSetup');
	let status = 'ok';
	try { status = await Setup.integrityStatus(); } catch (_) { return null; }
	if (status !== 'mismatch') return null; // 'ok'/'unrecorded'/'absent' are not tampering signals
	// Clear the once-per-process verification memo so the remediation below is real: without this, a long-running
	// service that verified the engine at startup would keep re-using the now-mismatched binary from the memo, and the
	// "re-verified on the next mount" promise would only come true after a restart. Now the next mount re-hashes it.
	try { Setup.invalidateVerification(); } catch (_) {}
...
// 1c-pq. Post-quantum signing support. Every integrity record (baseline, manifest seal, recovery index, roster,
// succession, checkpoint, release) is co-signed with ML-DSA-65, which Node exposes through OpenSSL 3.5 (Node 24.7+).
// The numeric Node-version gate at boot does not prove this runtime's OpenSSL actually carries ML-DSA: a Node built
// against an older system OpenSSL passes the version check yet cannot sign or verify it, which would otherwise surface
// as a raw crypto error on the first vault operation and a false "altered" on the release check. Detect it up front and
// say exactly how to fix it. The official Node builds and the bundled desktop runtime always have it.
register('pq_signing', () => {
	if (Integrity.pqAvailable()) return null;
	return { level: 'error', message: 'This Node build cannot perform post-quantum (ML-DSA) signatures, which Vaultonaut uses to protect every integrity record. Its OpenSSL is older than 3.5, even though the Node version is recent enough.', fix: 'Use an official Node.js 24.7 or newer build (nodejs.org), or the Vaultonaut desktop app, which bundles a compatible runtime.' };
});
...
// Advisory storage-node quota. When this machine offers space to OTHER people (the storage-node role) and the operator
// set a size cap, warn as it fills up. A served folder has no hard rclone cap, so this is the honest guard — a warning,
// not enforcement. It only walks the store when a cap is set, so it costs nothing on a machine not offering storage.
// A lazy require avoids any load-time cycle; both modules are fully loaded by the time a self-check runs.
register('storage_node_quota', async () => {
	let settings; try { settings = await require('./Vault').getSettings(); } catch (_) { return null; }
	const quotaMb = Number(settings && settings.storeQuotaMB) || 0;
	if (quotaMb <= 0) return null; // no cap set → not quota-managed, nothing to warn about
	let usage; try { usage = await require('./Vault').storeUsage(); } catch (_) { return null; }
	const usedMb = Math.round((usage.bytes || 0) / (1024 * 1024));
	if (usedMb < quotaMb) return null;
	return { level: 'warn', message: 'Your storage node is holding ' + usedMb + ' MB across ' + usage.slots + ' client slot(s), at or over the ' + quotaMb + ' MB limit you set.', fix: 'Raise the limit, or remove a client slot in the storage-node settings, to free space.' };
});
...
register('schema_versions', async (ctx) => {
	const out = [];
	const Brand = require('./Brand'); // required inline like every other Brand user in this file — without it the warning branches below throw ReferenceError and the check is swallowed exactly when it should fire
...
// The most recent per-volume free-space reading from the disk_space sweep. It is captured here — on the periodic
// self-check, OFF the request hot path — so the web state can surface a live disk gauge by reading this cached
// snapshot instead of re-stat-ing the disks on every 5-second poll (which would put file I/O on the hot path). Each
// entry is { where, freeBytes, totalBytes, pctFree }; the disk gauge shows the tightest volume (least free).
let lastDiskSnapshot = [];
function diskSnapshot() { return lastDiskSnapshot.slice(); }
register('disk_space', async (ctx) => {
	...
	const consider = async (p, where) => {
		...
		seen.add(key);
		// Record every volume for the dashboard gauge — not only the ones low enough to warn.
		snap.push({ where, freeBytes: info.freeBytes, totalBytes: info.totalBytes, pctFree: info.pctFree });
	...
	lastDiskSnapshot = snap; // publish the snapshot for the web state's disk gauge (read-only, no I/O on the poll)
	return out;
});

// Event-loop health. The tool's standing rule is to never block the Node event loop on the shared web and background
// service, and this is the check that would CATCH a regression of that rule. It reads the worst delay the loop has
// suffered since the previous sweep and resets, so each finding describes one window rather than an all-time figure.
// The read is O(1) and synchronous; it does nothing on a runtime without the monitor. Warn-only, cross-platform.
register('event_loop_lag', () => {
	const r = readLoopLag();
	if (!r || !isFinite(r.maxMs) || r.maxMs < LOOP_LAG_MAX_MS) return null;
	return {
		level: 'warn',
		message: 'The app paused for about ' + (r.maxMs / 1000).toFixed(1) + 's at once recently (its average delay was ' + r.meanMs.toFixed(0) + ' ms). A pause this long means a slow step ran on the main thread and briefly froze the web interface for everyone viewing.',
		fix: 'If this keeps happening, note what you were doing when it did — opening a very large file, building the search index, or recognizing text in scans — and report it, because that work is meant to run off the main thread.'
	};
});

// Heap headroom. A large in-memory operation — opening a very large file in the built-in viewer, building the content
// index — can push the V8 heap toward its hard ceiling, and the allocation PAST that ceiling is an immediate crash,
// not a slowdown. Surface the pressure while there is still room to act. This is the memory counterpart to disk_space,
// the tool's other resource ceiling. The read is O(1) and synchronous. Warn-only, cross-platform.
register('memory_headroom', () => {
	const stats = readHeap();
	const limit = stats && stats.heap_size_limit, used = stats && stats.used_heap_size;
	if (!limit || !used) return null;
	const pct = used / limit * 100;
	if (!isFinite(pct) || pct < HEAP_HEADROOM_PCT) return null;
	const mib = (n) => (n / 1048576).toFixed(0);
	return {
		level: 'warn',
		message: 'The app is using ' + pct.toFixed(0) + '% of its memory ceiling (' + mib(used) + ' of ' + mib(limit) + ' MiB). A large operation from here — opening a very large file, or building the search index — could run it out of memory and stop it.',
		fix: 'Restart the app to reclaim memory. If it keeps happening, avoid opening extremely large files in the built-in viewer, or give the process a higher memory limit.'
	};
});
...
register('autostart_command_stale', async () => {
	...
	// The launch target is TWO paths: the runtime (node) and the launcher script. Either one going missing makes the
	// service silently fail to come up at the next login. Check both; report whichever is definitively absent. A path
	// this parser could not read is skipped (null), so an unparsed entry is never a false alarm.
	const fix = 'Reinstall autostart from the current install (run "' + require('./Brand').cli + ' autostart install", or use the "Start at login" toggle) so the entry points at the right location.';
	if (st.script && !(await existsBounded(st.script))) return { level: 'warn', message: 'The start-at-login entry points at a launcher that no longer exists (' + st.script + '), so ' + require('./Brand').name + ' would fail to start automatically at the next login.', fix };
	if (st.node && !(await existsBounded(st.node))) return { level: 'warn', message: 'The start-at-login entry runs a program file that no longer exists (' + st.node + '), so ' + require('./Brand').name + ' would fail to start automatically at the next login.', fix };
	return null;
});
...
// Test-only seams (mirroring Integrity._forcePqAvailable): pass a reader to drive the firing path, or null to restore
// the real reading. Not used in production code.
function _forceLoopLagReading(fn) { readLoopLag = fn || defaultLoopLagReader; }
function _forceHeapReading(fn) { readHeap = fn || defaultHeapReader; }

module.exports = { register, list, run, last, diskSnapshot, _forceLoopLagReading, _forceHeapReading };
