...
register('engine_version', async () => {
	...
	const cmp = (a, b) => { const pa = parts(a), pb = parts(b); for (let i = 0; i < 3; i++) { if ((pa[i] || 0) !== (pb[i] || 0)) return (pa[i] || 0) > (pb[i] || 0) ? 1 : -1; } return 0; };
...
register('release_integrity', async () => {
	...
	if (!r.ok) {
		const changed = [...(r.mismatches || []), ...(r.missing || []), ...(r.extraneous || [])];
		const n = changed.length;
		return { level: 'warn', message: n + ' application file(s) do not match the signed release (' + changed.slice(0, 5).join(', ') + (n > 5 ? ', …' : '') + ') — this copy of ' + Brand.name + ' may have been modified.', fix: 'Re-download ' + Brand.name + ' from the official source and verify it before using it with real vaults.' };
	}
	return null;
});
...
register('schema_versions', async (ctx) => {
	const out = [];
	for (const [file, label] of [[path.join(ctx.dataDir, 'settings.json'), 'settings'], [Common.statePath(), 'mount state'], [path.join(ctx.dataDir, 'integrity.json'), 'tamper/rollback ledger'], [path.join(ctx.dataDir, 'tamper-log.json'), 'tamper log']]) {
		...
		if (obj && Number(obj.schemaVersion) > Common.SCHEMA_VERSION) {
			out.push({ level: 'warn', message: 'The ' + label + ' file was written by a newer version of the tool (schema v' + obj.schemaVersion + '; this build understands v' + Common.SCHEMA_VERSION + ').', fix: 'Update to the newer version to avoid losing settings it added.' });
		}
	}
	return out;
});
...
// Emergency (dead-man) access is armed per vault and holds a sealed read capability for a trusted contact. If the
// vault it points at is no longer present — securely removed, or deleted outside the app — the arming is orphaned: a
// later release would hand the contact a grant for a vault that may be gone. Warn-only, and it never auto-disarms,
// because the arming is security-sensitive; it surfaces the drift and asks the user to re-check or disarm. A vault
// is "present" when its folder still holds a readable manifest, so a folder whose keys were destroyed counts as gone.
register('orphaned_emergency_arming', async (ctx) => {
	let settings; try { settings = await ctx.Vault.getSettings(); } catch (_) { return null; }
	const armed = settings && settings.emergency && settings.emergency.armed;
