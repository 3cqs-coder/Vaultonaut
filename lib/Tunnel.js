...
function armStreamIdle(sock, ms) {
	if (!ms || !sock) return;
	// Baseline the byte counter at ARM time, not -1: with -1 the first interval merely records the count and only the
	// SECOND equal sample reaps, so a silent stream lived up to ~2x ms (e.g. ~10 min at SPLICE_IDLE_MS instead of ~5).
	// Seeding `last` to the current total makes the first idle interval with no delta reap at ~ms, matching the splice path.
	let last = -1; try { last = (sock.bytesRead || 0) + (sock.bytesWritten || 0); } catch (_) {}
...
function makeLink(sock, { onFrame, onDown }) {
	...
	ka = setInterval(() => {
		if (down) return;
		// While inbound is DELIBERATELY paused for aggregate backpressure, sock.pause() stops 'data', so incoming PONG
		// frames are not read and missedPongs would climb on a perfectly healthy peer — tearing down the WHOLE shared link
		// (and every stream multiplexed on it) within ~MAX_MISSED_PONGS * KEEPALIVE_MS. On the relay mux path that turns one
		// slow/stalled client into a denial of service for every other client on the node. Keep pinging to hold the NAT
		// mapping warm, but do NOT penalize while paused: a stalled stream is reaped by armStreamIdle, which drains the
		// aggregate and resumes reads, at which point the buffered PONGs flush and reset the count.
		if (aggPaused) { write(T.PING, 0, null); return; }
		if (missedPongs >= MAX_MISSED_PONGS) return die();
		missedPongs++; write(T.PING, 0, null);
	}, KEEPALIVE_MS);
