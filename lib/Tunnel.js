...
function makeLink(sock, { onFrame, onDown }) {
	...
	const die = () => { if (down) return; down = true; for (const s of paused) { try { s.resume(); } catch (_) {} } paused.clear(); try { sock.destroy(); } catch (_) {} try { onDown(); } catch (_) {} };
	sock.on('data', (chunk) => {
		if (down) return;
	...
	return new Promise((resolve, reject) => {
		...
		const link = makeLink(punchSocket, {
			onFrame: (type, sid, payload) => {
				...
				if (type === T.DATA && payload) { const ok = s.write(payload); if (ok === false) { /* the OS socket buffers; loopback rarely stalls */ } }
...
function nodeProxy(punchSocket, { localServePort, localServeHost = '127.0.0.1', onDown } = {}) {
	...
	const link = makeLink(punchSocket, {
		onFrame: (type, sid, payload) => {
			...
			const s = streams.get(sid); if (!s) return;
			if (type === T.DATA && payload) s.write(payload);
			else if (type === T.CLOSE) { streams.delete(sid); try { s.end(); } catch (_) {} }
		},
		onDown: shutdown,
	});
