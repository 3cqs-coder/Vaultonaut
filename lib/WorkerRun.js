...
function runProcess(childFile, job, opts = {}) {
	...
	// ORPHANED child (parent gone, so the maxMs kill can't fire) still bounds itself. Only for a plain-object job.
	if (job && typeof job === 'object' && !Buffer.isBuffer(job) && !Array.isArray(job)) {
		job = Object.assign({ rssLimitMb: DEFAULT_PROC_RSS_MB, selfTimeoutMs: maxMs || DEFAULT_PROC_MAX_MS }, job);
	}
	return new Promise((resolve, reject) => {
		let child;
