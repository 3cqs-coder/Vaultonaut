...
// Conservative DEFAULT resource bounds, so an isolated child is bounded even when a caller forgets to set its own —
// the safe path is the default, not opt-in. A caller may pass a larger value for a legitimately long/heavy job, or an
// explicit 0 to deliberately opt out (e.g. maxMs: 0 for no hard deadline). These only take effect when the caller
// omits them; the sole current caller (SearchWorker) sets all three, so its behavior is unchanged.
const DEFAULT_PROC_MAX_MS = 10 * 60 * 1000; // parent hard-kill deadline: bounds any runaway child while the parent lives
const DEFAULT_PROC_RSS_MB = 1024;           // child self-RSS budget: ends a memory bomb cleanly before an OS OOM kill

// Build a MINIMAL environment for a child that parses UNTRUSTED input (the document extractor). The child inherits
// nothing from the parent's environment by default; instead it gets only the handful of variables Node needs to
// start and that a reader library may legitimately consult (locale, temp dir, and the Windows runtime essentials).
// Everything else — cloud credentials, API tokens, SSH agent sockets, anything the user's shell exported into the
// app's process — is withheld, so a memory-corruption exploit in a document parser cannot read the ambient secrets of
// the environment it runs in. An ALLOWLIST (not a denylist) is used so a newly-invented secret variable is withheld
// by default rather than leaking until someone remembers to add it. Matched case-insensitively for Windows, whose
// variable names vary in case. This is defense-in-depth alongside the process's own memory/time caps.
const ENV_KEEP = new Set(['path', 'home', 'tmpdir', 'temp', 'tmp', 'lang', 'lc_all', 'lc_ctype', 'lc_messages', 'lc_numeric', 'tz',
	// Windows runtime essentials — Node will not start, or cannot resolve its own paths, without these.
	'systemroot', 'systemdrive', 'windir', 'userprofile', 'comspec', 'pathext', 'number_of_processors', 'processor_architecture',
	'appdata', 'localappdata', 'programdata', 'programfiles', 'programfiles(x86)', 'commonprogramfiles']);
function minimalEnv() {
	const out = {};
	for (const k of Object.keys(process.env)) { if (ENV_KEEP.has(k.toLowerCase())) out[k] = process.env[k]; }
	return out;
}

function runProcess(childFile, job, opts = {}) {
	const maxMs = opts.maxMs === 0 ? 0 : (opts.maxMs || DEFAULT_PROC_MAX_MS);
	// Inject the child-side caps (RSS budget + self wall-clock) into the job unless the caller already set them, so an
	// ORPHANED child (parent gone, so the maxMs kill can't fire) still bounds itself. The self wall-clock tracks the
	// parent deadline: `selfTimeoutMs: maxMs` means an explicit maxMs:0 (no deadline) also leaves the child without a
	// self-timeout — consistent with the opt-out — while the RSS budget still bounds its memory. Only a plain-object job
	// can carry these fields; a Buffer/Array job (none today) can't, so it relies on the parent maxMs kill while alive.
	if (job && typeof job === 'object' && !Buffer.isBuffer(job) && !Array.isArray(job)) {
		job = Object.assign({ rssLimitMb: DEFAULT_PROC_RSS_MB, selfTimeoutMs: maxMs }, job);
	}
	return new Promise((resolve, reject) => {
		let child;
		...
		try { child = require('child_process').fork(childFile, [], { execArgv: opts.execArgv || [], serialization: 'advanced', stdio: ['ignore', 'ignore', 'ignore', 'ipc'], windowsHide: true, ...(opts.scrubEnv ? { env: minimalEnv() } : {}) }); }
		catch (e) { reject(e); return; }
