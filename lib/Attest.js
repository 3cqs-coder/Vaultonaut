...
// Parse a GeneralizedTime string (YYYYMMDDHHMMSS[.fff]Z) into a Date. RFC 3161 / RFC 5280 require this time to be
// UTC: the trailing 'Z' is MANDATORY, and any numeric zone offset or surrounding whitespace is refused rather than
// silently read as UTC (an unqualified GeneralizedTime is a LOCAL time in ASN.1 — trusting it as UTC could shift the
// certified instant by hours). Out-of-range calendar fields (month 13, day 32) that Date.UTC would silently roll
// into a different instant are rejected too, so a malformed genTime fails closed instead of certifying a wrong time.
function parseGeneralizedTime(s) {
	const m = /^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(?:\.(\d+))?Z$/.exec(String(s));
	if (!m) return null;
	const [, Y, Mo, D, H, Mi, S, frac] = m;
	const ms = frac ? Math.min(999, Math.floor(Number('0.' + frac) * 1000)) : 0; // floor + cap so a long fraction near 1 cannot roll into the next second
	const base = Date.UTC(+Y, +Mo - 1, +D, +H, +Mi, +S, 0);
	const chk = new Date(base);
	if (chk.getUTCFullYear() !== +Y || chk.getUTCMonth() !== +Mo - 1 || chk.getUTCDate() !== +D || chk.getUTCHours() !== +H || chk.getUTCMinutes() !== +Mi || chk.getUTCSeconds() !== +S) return null;
	return new Date(base + ms);
}

// Parse a TimeStampResp: SEQUENCE { PKIStatusInfo, TimeStampToken OPTIONAL }. Returns the numeric PKIStatus and the raw
// token DER (a CMS ContentInfo) when a TimeStampToken element is present. This does NOT gate on the status itself — it
// reports the status so the CALLER can accept only a granted one (0 or 1) and reject the rest.
function parseResponse(respDer) {
	...
	try {
		...
		// PKIStatus is a small single-byte INTEGER (0 granted … 5 revocationNotification). Read it length-aware and
		// reject an empty or multi-byte encoding, so a malformed status can't read a byte outside the element or be
		// silently truncated into a "granted" value.
		const st = statusKids[0];
		if (st.end - st.start !== 1) throw new Error(MALFORMED);
		const statusVal = respDer[st.start];
...
// A real RFC 3161 timestamp token — even with a full signer-certificate chain — is a few kilobytes. Refuse a
// token far larger than that before parsing, independent of whatever cap the CALLER applied: parsing an enormous
// DER blob would make Der.children allocate one descriptor per TLV (millions of tiny elements → gigabytes). The
// network path already caps the TSA response at MAX_TSA_RESPONSE; this also bounds a token embedded in a
// user-chosen proof bundle, so no path can turn a hostile token into a huge allocation.
const MAX_TOKEN_BYTES = 1024 * 1024;
function readToken(tokenDer) {
	try {
		if (!Buffer.isBuffer(tokenDer) || tokenDer.length > MAX_TOKEN_BYTES) return null;
...
function certExtension(certDer, wantOid) {
	try {
		...
		for (const ext of children(certDer, extsSeq.start, extsSeq.end)) {
			...
			// DER encodes a TRUE BOOLEAN as exactly one 0xFF byte. Require that, rather than "any nonzero," so the read
			// is strict; this is also fail-closed, because a non-DER critical flag then reads as false and the caller's
			// "the timestamping EKU must be marked critical" check rejects the token rather than trusting a loose byte.
			if (parts.length === 3) { critical = (parts[1].end - parts[1].start === 1) && certDer[parts[1].start] === 0xFF; valTlv = parts[2]; }
...
function verifyToken(tokenDer, { digest, extraRoots = [] } = {}) {
	try {
		if (!Buffer.isBuffer(tokenDer) || tokenDer.length > MAX_TOKEN_BYTES) return { ok: false, reason: 'oversized' }; // bound the DER parse (see readToken) before any children() allocation
