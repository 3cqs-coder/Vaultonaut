...
async function parseCsvRows(text) {
	...
	for (let i = 0; i < str.length; i++) {
		// Yield to the event loop roughly every 128 KB so importing a very large file never blocks the shared web/
		// background service (the parse is a per-character scan, so a multi-megabyte input would otherwise stall it).
		if ((i & 0x1ffff) === 0 && i !== 0) await new Promise((r) => setImmediate(r));
		const c = str[i];
		if (inQuotes) {
			if (c === '"') {
...
async function parse(input, { format, filename } = {}) {
	...
	else if (fmt === 'csv') items = await parseCsv(isBuf ? input.toString('utf8') : s(input));
