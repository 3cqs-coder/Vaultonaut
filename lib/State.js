...
async function readState(repair = false) {
	...
	try {
		...
		if (repair) {
			...
			// Preserve the corrupt original BEFORE the caller's fresh write overwrites it. Prefer a move; if that
			// fails (a lock, a cross-device link), fall back to a COPY — never let the write clobber it with no
			// surviving copy, which would break the promise of "preserved for recovery".
			let preserved = false;
			try { await Common.renameWithRetry(Common.statePath(), saved); preserved = true; }
			catch (_) { try { await fsp.copyFile(Common.statePath(), saved); preserved = true; } catch (_) {} }
