...
const STATE_MAX_BYTES = Common.MAX_JSON_BYTES; // shared control-file size cap (single-sourced in Common, so it can't drift from the manifest/sidecar readers)
...
async function readAllOrNull() {
	let raw;
	// Bounded read, matching every other read in this module: an absent file is genuinely empty, but an unreadable
	// OR over-cap file is untrusted (null), so the destructive sweep that calls this fails safe and tears nothing down.
	try { raw = await Common.readFileCapped(Common.statePath(), STATE_MAX_BYTES, 'utf8'); }
	catch (e) { return (e && e.code === 'ENOENT') ? [] : null; } // absent -> genuinely empty; unreadable/over-cap -> untrusted
...
async function rejectSame(list, key, target) {
	const keep = [];
	for (const item of list) { if (!(await Common.sameFile(item[key], target))) keep.push(item); }
	return keep;
}
...
function add(entry) {
	return withLock(async () => {
		...
		s.mounts = await rejectSame(s.mounts, 'mountpoint', entry.mountpoint);
		s.mounts.push(entry);
		await writeState(s);
	});
}
...
function remove(mountpoint) {
	return withLock(async () => {
		const s = await readState(true);
		s.mounts = await rejectSame(s.mounts, 'mountpoint', mountpoint);
		await writeState(s);
	});
}
...
async function find(target) {
	...
	// Exact-path matches first (by filesystem identity, so a case-variant path on a case-insensitive volume matches
	// and two genuinely distinct vaults on a case-sensitive volume do not), then the bare-name fallback.
	for (const m of s.mounts) { if ((await Common.sameFile(m.mountpoint, target)) || (await Common.sameFile(m.vault, target))) return m; }
	if (name) { for (const m of s.mounts) { if (Common.foldPath(path.basename(m.mountpoint)) === foldedName || Common.foldPath(stripVault(m.vault)) === foldedName) return m; } }
	return null;
}
...
function addVault(vaultDir) {
	...
	return withLock(async () => {
		const s = await readState(true);
		let present = false;
		for (const v of s.vaults) { if (await Common.sameFile(v, abs)) { present = true; break; } }
		if (!present) { s.vaults.push(abs); await writeState(s); } // identity, not case-fold, so two case-variant vaults on a case-sensitive volume are not merged into one entry
	});
}
...
function removeVault(vaultDir) {
	return withLock(async () => {
		const s = await readState(true);
		const keep = [];
		for (const v of s.vaults) { if (!(await Common.sameFile(v, vaultDir))) keep.push(v); }
		s.vaults = keep;
		await writeState(s);
	});
}
