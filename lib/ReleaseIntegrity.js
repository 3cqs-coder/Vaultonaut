...
// the maintainer's pinned public keys. When a post-quantum key/signature is not present (a source checkout signed by an
// older maintainer setup), fall back to the classical check alone so the app is not falsely reported as tampered.
function verifyManifestSigHybrid(manifestBuf, sigB64, sigPqB64, pubHex, pqPubB64) {
	...
	if (!pqPubB64 || !sigPqB64) return true;
	return Integrity.verifyPq(pqPubB64, manifestBuf.toString('utf8'), sigPqB64);
}
...
async function verifyInstall(root, { pubHex, pqPubHex } = {}) {
	...
	pqPubHex = pqPubHex || embeddedPqPubKey();
	let manifestBuf;
