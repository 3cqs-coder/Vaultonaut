...
// descends at the ROOT only into these, then recurses FULLY within each one — so a file added anywhere inside the
// application's own directory tree is caught, including in a brand-new nested subdirectory. Directories that hold no
// listed file — the bundled runtime, node_modules, and a source checkout's development-only trees (lib/test,
// lib/scripts) — are top-level entries with no listed file, so they are never descended and stay cheap and unflagged.
// That single rule lets one manifest verify a source download, an npm install, and a desktop bundle alike (the bundle
// contains only the published files plus those out-of-scope top-level trees, so it never has a stray file to miss).
