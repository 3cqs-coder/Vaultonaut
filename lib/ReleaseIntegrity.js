...
async function buildManifestFromList(root, relPaths, { version } = {}) {
	...
	for (const rel of relPaths) {
		...
		let st; try { st = await fsp.stat(abs); } catch (_) { continue; } // a listed path that is not a regular file is skipped
...
// The directories (POSIX, '' = the root) that CONTAIN a listed file at any depth — every ancestor directory of every
// manifest entry. Extraneous-file detection is limited to these: a file that appears in a covered directory but is
// not in the manifest was added after signing. Directories that hold no listed file — the bundled runtime,
// node_modules, and development-only trees in a source checkout — sit outside the signed scope, so their contents are
// neither hashed nor flagged. That single rule lets the same manifest verify a source download, an npm install, and a
// desktop bundle without special-casing any of them. A brand-new top-level directory an attacker might drop in is not
// an ancestor of any listed file, so it is not scanned — but it is inert unless a signed (hash-checked) file requires
// it, and that file cannot be changed to do so without failing its hash.
function coveredDirs(listedPaths) {
	const dirs = new Set(['']);
	for (const p of listedPaths) {
		const parts = String(p).split('/'); parts.pop(); // drop the file name, keep its directory chain
		let cur = '';
		for (const seg of parts) { cur = cur ? cur + '/' + seg : seg; dirs.add(cur); }
	}
	return dirs;
}

// Files present directly inside a covered directory but absent from the manifest (an addition after signing). Only
// the covered directories are read — never a full-tree walk — so this stays cheap even when a large node_modules or a
// bundled runtime sits alongside. Shared by verifyInstall here and mirrored byte-for-byte in the standalone verify.js.
async function extraneousFiles(root, listedSet) {
	const out = [];
	for (const d of coveredDirs(listedSet)) {
		const abs = d ? resolveWithin(root, d) : root;
		if (!abs) continue;
		let entries; try { entries = await fsp.readdir(abs, { withFileTypes: true }); } catch (_) { continue; }
		for (const e of entries) {
...
module.exports = {
	...
	coveredDirs, extraneousFiles, signManifest, verifyManifestSig, publicKeyForSeed, verifyInstall, verifyInstallCached, signingPending, releaseSigningStatus,
};
