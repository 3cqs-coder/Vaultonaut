...
const SIG_PQ_NAME = 'release-manifest.sig.pq'; // the ML-DSA (post-quantum) co-signature over the same manifest bytes
...
function embeddedPqPubKey() { try { return require('./releasePubKey').pqPubkey || null; } catch (_) { return null; } }
...
function hashFile(abs) { return Common.hashFileStream(abs); } // streamed hex SHA-256 (shared seam — see Common.hashFileStream)
...
// The manifest's covered CONTENT with the one non-reproducible field (generatedAt, a wall-clock timestamp) removed, in
// a fixed key order. Everything left is a deterministic function of the source at a tag: the schema, product, version,
// hash algorithm, and the path-sorted list of {path, sha256, size}. `path` is already POSIX-normalized by the packager
// and the list is already sorted, so two independent builds from the same source produce identical content on any OS.
function reproducibleContent(manifest) {
	return {
		schema: manifest.schema, product: manifest.product, version: manifest.version, algo: manifest.algo,
		files: (manifest.files || []).map((f) => ({ path: f.path, sha256: f.sha256, size: f.size })),
	};
}
// A single SHA-256 over that canonical content: the REPRODUCIBLE DIGEST. It is the one number an independent party can
// regenerate from the public source at a tag and compare against the value the maintainer publishes — proving the
// signed manifest describes exactly the public source, with no hidden code. Unaffected by the signing timestamp.
function reproducibleDigest(manifest) {
	return crypto.createHash('sha256').update(Buffer.from(JSON.stringify(reproducibleContent(manifest)), 'utf8')).digest('hex');
}
...
function signManifestPq(manifestBuf, seedB64) { const { pqPriv } = Integrity.signKeysFromSeed(seedB64); return Integrity.signPq(pqPriv, manifestBuf.toString('utf8')); }
function verifyManifestSig(manifestBuf, sigB64, pubHex) { return Integrity.verify(pubHex, manifestBuf.toString('utf8'), sigB64); }
// AND-combiner: the release manifest is authentic only if BOTH the Ed25519 and the ML-DSA signatures verify against
// the maintainer's pinned public keys. Once a post-quantum key is pinned, the post-quantum signature is REQUIRED — a
// stripped `.sig.pq` then fails, so a quantum forger cannot downgrade to a classical-only check by deleting it. The
// classical-only fallback applies only when NO post-quantum key is pinned (a maintainer setup that predates it).
function verifyManifestSigHybrid(manifestBuf, sigB64, sigPqB64, pubHex, pqPubB64) {
	if (!Integrity.verify(pubHex, manifestBuf.toString('utf8'), sigB64)) return false;
	if (!pqPubB64) return true; // no post-quantum key pinned -> classical alone
	if (!Integrity.pqAvailable()) return true; // this runtime cannot do ML-DSA -> classical alone, not a false "altered"
	return Integrity.verifyPq(pqPubB64, manifestBuf.toString('utf8'), sigPqB64); // pinned -> the PQ half must verify (missing/forged fails closed)
}
function publicKeyForSeed(seedB64) { return Integrity.signKeysFromSeed(seedB64).pub; }
function pqPublicKeyForSeed(seedB64) { return Integrity.signKeysFromSeed(seedB64).pqPub; }
...
async function verifyInstall(root, { pubHex, pqPubHex } = {}) {
	root = path.resolve(root || Common.root());
	pubHex = pubHex || embeddedPubKey();
	if (pqPubHex === undefined) pqPubHex = embeddedPqPubKey(); // an explicit null forces the classical-only path (no pinned PQ key); undefined uses the embedded key
	let manifestBuf;
	...
	let sigPqB64 = null; try { sigPqB64 = (await fsp.readFile(path.join(root, SIG_PQ_NAME), 'utf8')).trim(); } catch (_) {}
	...
	let signatureValid = false; try { signatureValid = !!sigB64 && verifyManifestSigHybrid(manifestBuf, sigB64, sigPqB64, pubHex, pqPubHex); } catch (_) { signatureValid = false; }
	if (!signatureValid) return { present: true, signatureValid: false };
	// Authentic but a newer FORMAT this build cannot compare (unknown schema/algorithm): report it as such rather than
	// letting the file-hash loop below read every file as a mismatch. The boot self-check surfaces this as "update the
	// tool", never as a tamper alarm — mirroring the in-vault baseline reader.
	if (manifest.schema !== SCHEMA || (manifest.algo && manifest.algo !== 'sha256')) return { present: true, signatureValid: true, unknownFormat: true, schema: manifest.schema || null };
...
async function releaseSigningStatus(root, publishedList, { pubHex, pqPubHex } = {}) {
	root = path.resolve(root || Common.root());
	...
	const pqPub = pqPubHex !== undefined ? pqPubHex : embeddedPqPubKey();
	// Verify BOTH signatures (hybrid), so the committed post-quantum release signature is checked here too — a stale or
	// corrupt .sig.pq is caught by the same gate the suite runs, not silently accepted on the classical signature alone.
	let sigValid = false; try { sigValid = !!pub && !!sigB64 && verifyManifestSigHybrid(committedBuf, sigB64, sigPqB64, pub, pqPub); } catch (_) {}
...
module.exports = {
	MANIFEST_NAME, SIG_NAME, SIG_PQ_NAME, KEY_NAME, SCHEMA, EXCLUDE_TOP, EXCLUDE_NAME,
	embeddedPubKey, embeddedPqPubKey, appVersion, hashFile, buildManifest, buildManifestFromList, serialize, reproducibleContent, reproducibleDigest, walkCovered, resolveWithin,
	coveredTopEntries, extraneousFiles, signManifest, signManifestPq, verifyManifestSig, verifyManifestSigHybrid, publicKeyForSeed, pqPublicKeyForSeed, verifyInstall, verifyInstallCached, signingPending, releaseSigningStatus,
};
