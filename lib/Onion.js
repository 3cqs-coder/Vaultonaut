...
async function publishOnion(localPort, opts) {
	...
	try {
		...
		// non-anonymous mode (see startManagedServerTor). Client authorization is independent of the anonymity mode, so it
		// still applies. A plain (anonymous, 3-hop) onion omits the flag.
		const flags = 'DiscardPK' + (opts.singleHop ? ',NonAnonymous' : '');
		const r = await sendCmd(sock, 'ADD_ONION NEW:ED25519-V3 Flags=' + flags + ' Port=' + virtualPort + ',127.0.0.1:' + localPort + authFlag);
...
async function _spawnTor(extraArgs) {
	...
	// path it already uses for a system Tor. Control port is loopback-only. Profile args (SOCKS/client, or single-hop) follow.
	const args = ['--ControlPort', '127.0.0.1:' + controlPort, '--CookieAuthentication', '1',
		'--GeoIPFile', paths.geoip, '--GeoIPv6File', paths.geoip6, '--DataDirectory', dataDir, '--Log', 'notice stdout', '--AvoidDiskWrites', '1'].concat(extraArgs || []);
...
async function startManagedTor() {
	...
	const h = await _spawnTor(['--SocksPort', '127.0.0.1:' + socksPort, '--ClientOnly', '1']);
	const rec = { controlPort: h.controlPort, socksPort: socksPort, proc: h.proc, dataDir: h.dataDir, stop: () => { h.stop(); if (_managed && _managed.proc === h.proc) _managed = null; } };
	_managed = rec;
...
async function startManagedServerTor() {
	if (_managedServer) return _managedServer;
	const h = await _spawnTor(['--SocksPort', '0', '--HiddenServiceNonAnonymousMode', '1', '--HiddenServiceSingleHopMode', '1']);
	const rec = { controlPort: h.controlPort, proc: h.proc, dataDir: h.dataDir, singleHop: true, stop: () => { h.stop(); if (_managedServer && _managedServer.proc === h.proc) _managedServer = null; } };
	_managedServer = rec;
	try { await h.ready; } catch (e) { rec.stop(); throw e; }
...
// service, so a dedicated managed server Tor is always started. Non-blocking.
async function provideTor(opts) {
	opts = opts || {};
	if (opts.singleHop) { const s = await startManagedServerTor(); return { controlPort: s.controlPort, managed: true, singleHop: true }; }
...
module.exports = { publishOnion, registerClientAuth, makeClientAuthKeypair, base32, torAvailable, socksUrl, isOnionUrl, authenticate, sendCmd, connectControl, provideTor, startManagedTor, startManagedServerTor, stopManagedTor, onionAvailable, CONTROL_PORTS, SOCKS_PORTS };
