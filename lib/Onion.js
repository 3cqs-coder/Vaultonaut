...
async function publishOnion(localPort, opts) {
	...
	if (!sock) throw Object.assign(new Error('No running Tor was found (looked for a control port on ' + ports.join(' and ') + '). Start Tor — or open the Tor Browser — to use onion mode.' + (lastErr && lastErr.message ? ' (' + lastErr.message + ')' : '')), { code: 'NO_TOR' });
	try {
		...
		// Count this as a live onion so a managed Tor is never restarted out from under it (see _ensureManaged). stop() is
		// idempotent — it decrements at most once — so a double stop cannot corrupt the count.
		_liveOnions++;
		let stopped = false;
		return { onion: id + '.onion', port: virtualPort, clientAuthPriv: clientAuthPriv, serviceKey: serviceKey, stop: () => { if (!stopped) { stopped = true; _liveOnions = Math.max(0, _liveOnions - 1); } try { sock.destroy(); } catch (_) {} } };
...
// The bridge modes this install actually supports: always 'off' and 'custom', plus every built-in mode the DOWNLOADED
// bundle's pt_config.json advertises (obfs4/meek/snowflake today, but whatever the bundle ships tomorrow). Deriving the
// list from pt_config instead of a hardcoded constant means a future bundle that renames or adds a transport can never
// silently reject a mode the UI offered. Falls back to KNOWN_BRIDGE_MODES only when pt_config cannot be read.
async function allowedBridgeModes() {
	const modes = new Set(['off', 'custom']);
	try { const cfg = await require('./TorSetup').readPtConfig(); if (cfg && cfg.bridges) for (const k of Object.keys(cfg.bridges)) modes.add(k); } catch (_) {}
	if (modes.size <= 2) for (const m of KNOWN_BRIDGE_MODES) modes.add(m); // pt_config unavailable → fall back to the known list
	return [...modes];
}
...
let _managedStarting = null;       // in-flight start promise for the client Tor (single-flight: no double-spawn)
let _managedServerStarting = null; // in-flight start promise for the server Tor
let _liveOnions = 0;               // onion services currently published; while > 0 a managed Tor is never restarted out from under a live serve
...
// Start (or reuse) one managed Tor. `isServer` picks the profile: the client profile has a loopback SOCKS port for
// connecting OUT plus ClientOnly (never a relay); the server profile is a SEPARATE instance with SocksPort off and
// single-hop non-anonymous mode (Tor forbids a single-hop non-anonymous service on the same instance that acts as a
// SOCKS client), used only to publish fast single onion services while the client Tor still does the anonymous
// connecting. Two guarantees that fix real races: (1) SINGLE-FLIGHTED — concurrent callers share one in-flight start,
// so two operations at once never spawn two Tors (one of which would leak) and both get the same bootstrapped instance;
// (2) it never restarts a running Tor out from under a LIVE serve — while any onion is published (_liveOnions > 0) the
// existing instance is reused even if the bridge config changed, so a bridge-setting change can never kill an active
// serve; the new config takes effect the next time a managed Tor starts with nothing published. Returns the LOCAL
// record, so a stopManagedTor() firing during the bootstrap await can never turn the result into null.
async function _ensureManaged(isServer, bridge) {
	const sig = bridgeSig(bridge);
	const existing = isServer ? _managedServer : _managed;
	if (existing && (existing.bridgeSig === sig || _liveOnions > 0)) return existing;
	if (existing) { try { existing.stop(); } catch (_) {} if (isServer) _managedServer = null; else _managed = null; }
	const inflight = isServer ? _managedServerStarting : _managedStarting;
	if (inflight) return inflight;
	const p = (async () => {
		const socksPort = isServer ? 0 : await freePort();
		const extra = isServer
			? ['--SocksPort', '0', '--HiddenServiceNonAnonymousMode', '1', '--HiddenServiceSingleHopMode', '1']
			: ['--SocksPort', '127.0.0.1:' + socksPort, '--ClientOnly', '1'];
		const h = await _spawnTor(extra, bridge);
		const rec = { controlPort: h.controlPort, socksPort: isServer ? undefined : socksPort, proc: h.proc, dataDir: h.dataDir, singleHop: !!isServer, bridgeSig: sig,
			stop: () => { h.stop(); if (isServer) { if (_managedServer && _managedServer.proc === h.proc) _managedServer = null; } else if (_managed && _managed.proc === h.proc) _managed = null; } };
		if (isServer) _managedServer = rec; else _managed = rec;
		try { await h.ready; } catch (e) { rec.stop(); throw e; }
		return rec;
	})();
	if (isServer) _managedServerStarting = p; else _managedStarting = p;
	try { return await p; } finally { if (isServer) _managedServerStarting = null; else _managedStarting = null; }
}
async function startManagedTor(bridge) { return _ensureManaged(false, bridge); }
async function startManagedServerTor(bridge) { return _ensureManaged(true, bridge); }
