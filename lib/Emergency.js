...
// ── time-locked seal (two independent gates) ────────────────────────────────────────────────────────────────
// Layer a TIME gate over the identity seal, so a released grant cannot be opened before a chosen moment even by the
// beneficiary. The message is first sealed to the beneficiary's key exactly as above (the identity gate: only their
// private key opens it, post-quantum-safe), then that sealed blob is time-locked to a drand round (the delay gate:
// the decryption key does not exist until the beacon reaches that round — see Timelock.js). Opening therefore needs
// BOTH the round's beacon signature AND the beneficiary's private key: a copy grabbed early is inert, and even after
// the round matures it is useless to anyone but the beneficiary. Timelock is lazy-required so plain seals never pull
// the curve library in. `publicKey` overrides the pinned beacon key for the offline test.
async function timelockSeal(recipientPubB64, message, round, { context = '', publicKey } = {}) {
	const inner = seal(recipientPubB64, message, context); // identity gate (post-quantum hybrid), a base64 string
	return require('./Timelock').seal(round, Buffer.from(inner, 'utf8'), { publicKey }); // delay gate, returns the combined blob
}
// Open a time-locked seal: unwrap the time gate with the round's beacon signature, then the identity gate with the
// beneficiary's private key. Throws (fail-closed) if the round is not mature (no signature), the signature is wrong,
// the private key is wrong, or anything was altered.
async function timelockOpen(contactPrivB64, blob, sigBytes, { context = '' } = {}) {
	const inner = await require('./Timelock').open(blob, sigBytes); // -> the inner sealed blob's bytes
	return open(contactPrivB64, inner.toString('utf8'), context);
}

module.exports = { generateContactKeypair, seal, open, isPostQuantum, timelockSeal, timelockOpen };
