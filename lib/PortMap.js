		...
		try {
			if (process.platform === 'win32') {
				...
				execFile('ip', ['route'], { timeout: 1500 }, (err, out) => {
					...
					// /proc/net/route: the row whose Destination is 00000000 is the default route, and its Gateway
					// column is a little-endian hex IPv4 (byte order reversed). Read-only, async, and fail-closed.
					fsp.readFile('/proc/net/route', 'utf8').then((txt) => {
						for (const line of String(txt).split('\n').slice(1)) {
							const c = line.trim().split(/\s+/);
							if (c.length > 3 && c[1] === '00000000' && (parseInt(c[3], 16) & 0x2) && /^[0-9A-Fa-f]{8}$/.test(c[2])) {
								const h = c[2];
								return finish([6, 4, 2, 0].map((i) => parseInt(h.slice(i, i + 2), 16)).join('.'));
							}
						}
						finish(null);
					}).catch(() => finish(null));
