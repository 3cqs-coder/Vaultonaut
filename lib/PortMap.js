...
// (richer, returns the external IP), then NAT-PMP. Never throws; a router that speaks neither simply yields null.
async function mapPort(internalPort, { lifetime = DEFAULT_LIFETIME, tcp = true, gateway: gwOverride } = {}) {
	...
	if (np && np.externalPort) return { externalPort: np.externalPort, externalIP: null, lifetime: np.lifetime, protocol: tcp ? 'tcp' : 'udp', gateway, internalIP, internalPort };
	return null;
}
...
module.exports = { mapPort, unmapPort, keepMapped, defaultGatewayV4, localIPv4For, NATPMP_PORT, DEFAULT_LIFETIME };
...
module.exports._proto = { natpmpMapRequest, natpmpParse, pcpMapRequest, pcpParse };
