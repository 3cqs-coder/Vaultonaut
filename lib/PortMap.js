...
// Parse the Linux kernel route table (the text of /proc/net/route) for the default-route gateway. The row whose
// Destination is 00000000 is the default route; its Flags must have both RTF_UP (0x1) and RTF_GATEWAY (0x2), and its
// Gateway column is a little-endian hex IPv4 (byte order reversed). Returns the dotted-quad gateway, or null when
// there is no usable default route. Pure and synchronous so it can be unit-tested against fixtures; the caller does
// the async, fail-closed file read.
function parseProcNetRoute(text) {
	for (const line of String(text || '').split('\n').slice(1)) {
		const c = line.trim().split(/\s+/);
		if (c.length > 3 && c[1] === '00000000' && (parseInt(c[3], 16) & 0x3) === 0x3 && /^[0-9A-Fa-f]{8}$/.test(c[2])) {
			const h = c[2];
	...
	return new Promise((resolve) => {
		...
		try {
			if (process.platform === 'win32') {
				...
				// Linux: the `ip route` default line; if the `ip` binary is missing (a minimal install), fall back to
				// reading the kernel route table at /proc/net/route directly.
				execFile('ip', ['route'], { timeout: 1500 }, (err, out) => {
					...
					// Read-only, async, and fail-closed; the parse itself is a pure helper (parseProcNetRoute) so it can
					// be unit-tested against fixtures.
					fsp.readFile('/proc/net/route', 'utf8').then((txt) => finish(parseProcNetRoute(txt))).catch(() => finish(null));
				});
			}
		} catch (_) { finish(null); }
	});
...
module.exports._proto = { natpmpMapRequest, natpmpParse, natpmpExtRequest, natpmpExtParse, pcpMapRequest, pcpParse, parseProcNetRoute };
