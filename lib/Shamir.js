...
// A string that is clearly one of our shares but written by a NEWER format than this build understands. The share
// family prefix is "vds<version>", so a higher version number — or the current family carrying more fields than we
// read — is a forward format. Detected so combine can say "update the app" instead of the misleading "unrecognized
// format," which would otherwise send a user hunting for a corrupt share that is actually just newer than this build.
function looksNewerFormat(s) {
	const parts = String(s || '').trim().split('.');
	const m = /^vds(\d+)$/.exec(parts[0] || '');
	if (!m) return false;
	if (parseInt(m[1], 10) > 1) return true; // vds2+, a bumped share family
	return parts.length > 5;                 // vds1 carrying fields beyond the id — a forward extension of this family
}
...
function combine(shareStrings) {
	...
	for (const s of shareStrings || []) {
		...
		if (!p) {
			if (looksNewerFormat(s)) throw new Error('A share was made by a newer version of the tool. Update the app to reconstruct it.');
			throw new Error('A share is malformed or from an unrecognized format.');
		}
...
module.exports = { split, combine, parseShare, looksNewerFormat, PREFIX };
