...
const { StringDecoder } = require('string_decoder'); // decode child output across chunk boundaries so a multibyte UTF-8 sequence split between two 'data' chunks is never turned into replacement characters
...
// A generous DEFAULT cap on buffered stdout, so a future caller that forgets to pass its own maxOutBytes still cannot
// grow memory without bound on a weeks-up service (e.g. a recursive listing of a huge vault). It never bites a legitimate
// buffered read — those are small, and the few large ones pass their own, higher, explicit cap — and any output meant to
// be large streams line-by-line (onLine) instead of accumulating here. A streaming/discardStdout read ignores it.
const MAX_STDOUT_DEFAULT = 256 * 1024 * 1024;
function spawnP(bin, args, { input, stdio, timeoutMs = 60000, discardStdout = false, onLine, dropLine, maxOutBytes = MAX_STDOUT_DEFAULT } = {}) {
	return new Promise((resolve, reject) => {
		...
		const feed = (chunk, isErr) => {
			...
			for (const ln of parts) {
				...
				// Bound the retained diagnostic tail exactly like the non-streaming branch below: a long-running streaming
				// op (a multi-hour bisync) against a chatty or hostile remote can emit an unbounded number of distinct
				// non-progress lines, so keep only the head (up to MAX_STDERR_BYTES) and stop appending past it. The
				// progress itself already went to onLine; this only caps what is kept for the result, so memory can't grow
				// without bound. No kill — a flood of notices is not a reason to abort a legitimate transfer.
				if (!dropLine || !dropLine(ln)) { if (isErr) { if (err.length < MAX_STDERR_BYTES) err += ln + '\n'; } else if (out.length < MAX_STDERR_BYTES) out += ln + '\n'; }
			}
		};
		const timer = setTimeout(() => { timedOut = true; try { child.kill('SIGKILL'); } catch (_) {} }, timeoutMs);
		// Decode each stream through a StringDecoder so a multibyte UTF-8 character split across two 'data' chunks is
		// completed with the next chunk instead of becoming replacement characters on both sides — the captured output
		// is surfaced to the user (engine error tails often echo a non-ASCII filename), so mojibake there is a real bug.
		const outDec = new StringDecoder('utf8'), errDec = new StringDecoder('utf8');
		if (streaming) {
			if (child.stdout) child.stdout.on('data', d => feed(outDec.write(d), false));
			if (child.stderr) child.stderr.on('data', d => feed(errDec.write(d), true));
		} else {
			if (child.stdout) child.stdout.on('data', d => { out += outDec.write(d); if (maxOutBytes && out.length > maxOutBytes) { try { child.kill('SIGKILL'); } catch (_) {} } }); // bound the accumulator so a hostile/huge output (e.g. a peer-controlled lease file) can't grow memory unbounded — the truncated result then fails to parse and is treated as absent
			if (child.stderr) child.stderr.on('data', d => { if (err.length < MAX_STDERR_BYTES) err += errDec.write(d); }); // bound stderr like stdout above, so a chatty or hostile error stream can't grow memory unbounded; the head is kept for diagnostics (no kill: a stderr flood is not itself a reason to abort a legitimate long-running op)
			// Swallow a stdio-stream 'error' (a rare EIO on a child pipe), which `child.on('error')` does NOT catch, so
			// it can never surface as an uncaughtException that would kill a non-resilient one-shot CLI command. This is
			// the same guard `authorize` already applies; keep them consistent.
		...
		child.on('close', (status) => {
			clearTimeout(timer);
			if (streaming) { // flush any bytes the decoder held for an incomplete trailing sequence, then the trailing partial line
				feed(outDec.end(), false); feed(errDec.end(), true);
				if (obuf && (!dropLine || !dropLine(obuf))) out += obuf;
				if (ebuf && (!dropLine || !dropLine(ebuf))) err += ebuf;
			} else { out += outDec.end(); if (err.length < MAX_STDERR_BYTES) err += errDec.end(); }
...
// Run `fn` with a freshly written ephemeral config, then delete it — no matter how `fn` ends. This is the
// write/try/finally/remove idiom in one place, so a caller can never forget the cleanup and leave the obscured
// engine secret on disk. Use it wherever a config is written for the span of one operation; the few callers that
// hand the config path OFF to a longer-lived owner keep calling writeEphemeralConfig/removeConfig directly.
async function withEphemeralConfig(text, fn) {
	const cfg = await writeEphemeralConfig(text);
	try { return await fn(cfg); } finally { await removeConfig(cfg); }
}
...
async function spawnMount(bin, opts) {
	...
	// Argument-injection guard: the mountpoint is a bare positional argv token, and cacheDir/volname are flag VALUES,
	// all derived from vault config. A value that began with "-" would be read by the engine as a flag ("option
	// smuggling"). These are always real paths or a volume label, so a leading "-" is never legitimate — reject it up
	// front rather than hand the engine an attacker-shaped flag. (Remote specs carry a "vault:"/"backend:" prefix, so
	// they can never start with "-"; this covers the config-controlled positionals that could.)
	for (const [label, v] of [['mount point', mountpoint], ['cache directory', cacheDir], ['volume name', volname]]) {
		if (v != null && String(v).startsWith('-')) throw new Error('Refusing to mount: the ' + label + ' may not start with "-" (it would be read as a command-line option).');
	}
	...
	// Open the per-mount log ASYNCHRONOUSLY and bounded. A synchronous open blocks the whole event loop, and the
	// data dir can be a per-user home on a network/removable mount that wedges — exactly the case every other
	// data-dir touch on this path avoids with bounded async I/O. If the open cannot complete promptly the mount
	// fails fast (the caller retries/falls back) rather than freezing every concurrent request.
	let out;
	const openP = fsp.open(logFile, 'w');
	try { out = await Common.withTimeout(openP, 5000); }
	catch (e) {
		// If the open TIMED OUT but the underlying open later resolves, close that late handle so it is not orphaned
		// (an fd leak). If it rejects, ignore. Either way the mount fails fast here rather than freezing the loop.
		openP.then((h) => h.close().catch(() => {}), () => {});
		throw new Error('Could not open the mount log — the data directory may be on an unresponsive drive: ' + (e && e.message ? e.message : e));
	}
	const spawnOpts = { detached: true, stdio: ['ignore', out.fd, out.fd], windowsHide: true }; // windowsHide so the long-lived detached mount engine never leaves a console window open on Windows
	...
	catch (e) { try { await out.close(); } catch (_) {} throw e; } // spawn can throw SYNCHRONOUSLY (bad opts/args) before the child dups the fd; close the parent's copy so a repeated failure can't leak fds on the long-running service
	// A detached child with no 'error' listener would throw an uncaughtException on a
	// spawn failure (bad path, permissions), which would tear the host process down. Append the note
	// asynchronously (best-effort) so a wedged data dir can never block the event loop from this handler.
	child.on('error', (e) => { fsp.appendFile(logFile, '\nspawn error: ' + (e && e.message) + '\n').catch(() => {}); });
	child.unref();
	try { await out.close(); } catch (_) {} // the child dup'd the fd; close the parent's copy so the long-running UI never leaks fds
...
async function waitForMountOrExit(mountpoint, pid, timeoutMs = 15000) {
	const step = 250;
	// Bound on the MONOTONIC clock, not on accumulated sleep: each isMounted() can itself spend seconds inside a
	// bounded stat on a slow/half-wedged mount, so counting only the sleep intervals would let the effective wait
	// run several times past timeoutMs and make a failing mount feel wedged (delaying the caller's retry/fallback).
	// performance.now() also can't be defeated by a wall-clock jump — the same reason drain() uses it.
...
async function drain(bin, endpoint, entry, timeoutMs = 300000, onProgress) {
	...
	const step = 200; let stableIdle = 0, rcDownSince = 0, drainErrCount = 0; const start = performance.now();
	while (performance.now() - start < timeoutMs) {
		...
		// the `transferring` key entirely, so an absent key correctly means zero active transfers — not busy. Also
		// note the running error count, so a drain that reaches idle WITH errors recorded can be surfaced (below).
		if (!cs || cs.status !== 0) busy = true;
		else { try { const j = JSON.parse(cs.stdout); if (Array.isArray(j.transferring)) pending += j.transferring.length; if (typeof j.errors === 'number' && j.errors > drainErrCount) drainErrCount = j.errors; } catch (_) { busy = true; } }
		if (busy || pending > 0) { stableIdle = 0; if (onProgress && pending > 0) onProgress(pending); }
		else if (++stableIdle >= 6) {
			// Both channels have read idle across ~1.2s of polls — past the close hand-off, so every write handed to the
			// engine has been accounted for and the write-back is flushed. If the engine also recorded upload errors this
			// session, a genuine cloud failure (an expired token, a full quota, a provider rejection) may have kept some
			// handoffs from actually landing. The engine retries write-back indefinitely, so a write still pending would
			// have kept us busy above rather than reaching here — but a nonzero error count still deserves a warning so
			// such a failure is never silent. We do NOT block teardown on it: rclone latches its session error state, so
			// blocking would risk wedging the unmount on an unrelated earlier error. A clear warning plus a loss-free
			// deferral path (the caller already defers on `false`) is the right balance.
			if (drainErrCount > 0) Common.warn('The storage provider reported ' + drainErrCount + ' error(s) while saving buffered changes. Your data is safe locally, but if a cloud sign-in expired or the account is full, some changes may not have finished uploading — check the connection and re-mount to retry.');
			return 'drained';
		}
		await Common.sleep(step);
	}
...
module.exports = {
	obscure, reveal, authorize, harvestToken, buildConfig, cryptRemoteSection, writeEphemeralConfig, removeConfig, withEphemeralConfig, sweepStaleConfigs, run, exec, cryptEncodeNames, rcSocketPath, rcTcpEndpoint, rcClientArgs, parseRcPort, drain, rcQuit, rcConfigGet,
