...
function spawnP(bin, args, { input, stdio, timeoutMs = 60000, discardStdout = false, onLine, dropLine, maxOutBytes = MAX_STDOUT_DEFAULT, env } = {}) {
	return new Promise((resolve, reject) => {
		let child;
		const useStdio = stdio || (discardStdout ? ['pipe', 'ignore', 'pipe'] : ['pipe', 'pipe', 'pipe']);
		// `env`, when given, is MERGED over the process env (e.g. a SOCKS proxy for a .onion transfer) — never global, so
		// only the run that opts in is affected. Omitted for every ordinary run, which inherits the parent env unchanged.
		const spawnEnv = env ? { stdio: useStdio, windowsHide: true, env: Object.assign({}, process.env, env) } : { stdio: useStdio, windowsHide: true };
		try {
			child = spawn(bin, args, spawnEnv); // windowsHide: the app runs windowless, so a console-subsystem child would otherwise pop a console window on Windows (stdio redirection does not suppress it)
		...
		if (streaming) {
			if (child.stdout) { child.stdout.on('data', d => feed(outDec.write(d), false)); child.stdout.on('error', () => {}); }
			if (child.stderr) { child.stderr.on('data', d => feed(errDec.write(d), true)); child.stderr.on('error', () => {}); }
		} else {
...
async function run(bin, args, { configPath, input, discardStdout, timeoutMs, onLine, dropLine, maxOutBytes, env } = {}) {
	const full = [];
	if (configPath) full.push('--config', configPath);
	full.push(...args);
	return spawnP(bin, full, { input, discardStdout, timeoutMs, onLine, dropLine, maxOutBytes, env });
}
...
async function drain(bin, endpoint, entry, timeoutMs = 300000, onProgress) {
	...
	while (performance.now() - start < timeoutMs) {
		...
		// Both channels idle across ~1.2s of polls — past the close hand-off, so the write-back is flushed. We do NOT
		// warn on a nonzero core/stats `errors` here: that counter is cumulative and latched and also counts low-level
		// errors that were retried and ultimately SUCCEEDED (the mount runs with --low-level-retries/--retries), and a
		// write that had genuinely NOT landed would have kept this loop busy (rclone retries write-back indefinitely)
		// rather than reaching this idle state — so a nonzero count here is a recovered hiccup, not lost data, and
		// surfacing it would cry wolf on every transient blip. A genuinely unflushed state shows up as the loop never
		// going idle, which returns `false` below and the caller defers with a clear "still saving" message.
		else if (++stableIdle >= 6) return 'drained';
		await Common.sleep(step);
	}
	return false;
}
