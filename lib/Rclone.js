...
function authorize(bin, backend, { clientId, clientSecret, onUrl, timeoutMs = 300000 } = {}) {
	return new Promise((resolve, reject) => {
		...
		if (child.stdout) { child.stdout.on('data', d => { out += d.toString(); scan(); }); child.stdout.on('error', () => {}); }
		if (child.stderr) { child.stderr.on('data', d => { err += d.toString(); scan(); }); child.stderr.on('error', () => {}); }
...
async function isMounted(mountpoint, timeoutMs = 4000) {
	if (process.platform === 'win32') {
		...
		// the caller tears it down, exactly as the POSIX branch treats a stale/timed-out mount.
		const root = mountpoint.endsWith(':') ? mountpoint + '\\' : mountpoint;
		const probe = fsp.access(root).then(() => true, (e) => !(e && e.code === 'ENOENT'));
...
// A bounded stat of a mountpoint: true only if it answers promptly (the drive works). withTimeout bounds the
// RESULT, not the underlying stat — a wedged mount's stat(2) never returns and holds a libuv threadpool slot
// (only 4 by default) until the OS gives up. So DEDUPE by mountpoint on the RAW syscall: while a prior stat on
...
const statInflight = new Map(); // mountpoint -> the raw fsp.stat promise still pending (the real slot holder)
async function mountResponds(mp) {
	if (!mp) return false;
	if (statInflight.has(mp)) return 'pending'; // a prior probe's syscall hasn't returned yet — DISTINCT from a definite failure, so a concurrent caller doesn't misread it as "dead" and force-release a healthy mount
	const raw = fsp.stat(mp);
	statInflight.set(mp, raw);
	raw.then(() => {}, () => {}).finally(() => { statInflight.delete(mp); }); // clear only when the SYSCALL itself settles
...
async function engineAlive(entry, mountProbe) {
	...
	if (process.platform === 'win32' && entry.image) {
		const img = await windowsImageName(entry.pid);
		if (img) return img === String(entry.image).toLowerCase(); // decisive: matches our engine or not
		// tasklist gave no answer — fall back to pid-alive rather than block recovery of our own engine.
	}
	return true;
}
...
module.exports = {
	obscure, reveal, authorize, harvestToken, buildConfig, cryptRemoteSection, writeEphemeralConfig, removeConfig, sweepStaleConfigs, run, exec, cryptEncodeNames, rcSocketPath, rcTcpEndpoint, parseRcPort, drain, rcQuit, rcConfigGet,
	spawnMount, isMounted, engineAlive, touchSockets, waitForMountOrExit, unmount
};
