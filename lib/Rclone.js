...
async function spawnMount(bin, opts) {
	...
	let out;
	try { out = await Common.withTimeout(fsp.open(logFile, 'w'), 5000); }
	catch (e) { throw new Error('Could not open the mount log — the data directory may be on an unresponsive drive: ' + (e && e.message ? e.message : e)); }
