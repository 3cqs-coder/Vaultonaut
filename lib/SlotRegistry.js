...
function store(fileName) {
	...
	return {
		...
		// A corrupt/truncated primary falls back to the backup; a genuinely missing file (ENOENT) is rethrown.
		async read() {
			try { return JSON.parse(await fsp.readFile(filePath(), 'utf8')); }
			catch (e) { if (e.code === 'ENOENT') throw e; try { return JSON.parse(await fsp.readFile(bakPath(), 'utf8')); } catch (_) { throw e; } }
		},
		...
		record(slots, count, extra) {
			...
			shuffle(out);
			return Object.assign({ v: 1, aead: AEAD, slotCount: count, slotSize: SLOT_LEN, slots: out.map(b => b.toString('base64')) }, extra || {});
		},
	};
}

module.exports = { newKdfParams, deriveK, encryptSlot, tryDecryptSlot, randomSlot, shuffle, store, PLAINTEXT_SIZE, SLOT_LEN };
