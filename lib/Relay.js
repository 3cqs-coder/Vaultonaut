...
const FIRST_BYTE_MS = 20 * 1000; // a freshly accepted public connection must send its first byte within this window or be dropped — a real WebDAV client sends its request immediately, so this reaps a silent-connection flood in seconds instead of holding a node at its connection ceiling for the full idle window; once any byte arrives the idle reaper above governs
...
const MAX_NODES = 256;             // cap on TOTAL registered nodes, so one token holder cannot register enough distinct node ids to exhaust the listening sockets/file descriptors. With the default data-port range (~100 ports) the range runs out first, so that is the effective limit there; this cap binds only when a larger port range is configured.
...
function runHub({ controlPort = Common.DEFAULT_RELAY_PORT, token, host = '0.0.0.0', portRange = Common.DEFAULT_RELAY_DATA_PORT_RANGE, onLog, maxActiveSplices = MAX_ACTIVE_SPLICES, maxNodes = MAX_NODES, maxNodesPerIp = MAX_NODES_PER_IP, mux } = {}) {
	...
	// A mux-negotiated node's single data link. Refuse it unless this node ACTUALLY negotiated mux (so a mux link can
	// never be attached to — and thus capture the clients of — a node serving via the worker pool) and proves the same
	// per-node tag as a WORKER. Requiring a NON-EMPTY tag also closes the tagless-node case: tokenMatch('','') is true,
	// so without this a bare token holder could attach a mux link to a legacy tagless node and capture its clients (the
	// WORKER path shares that legacy property, but every current node sends a tag, so this hardens the mux path). Then
	// carry every subsequent client for the node as a stream over the link — with the SAME idle reaping (streamIdleMs)
	// and concurrent-connection ceiling the splice path enforces. Drain any workers-waiting clients onto the mux now.
	function attachMux(sock, nodeId, tag) {
		const n = nodes.get(nodeId);
		if (!n) { try { sock.destroy(); } catch (_) {} return; }
		if (!n.muxWanted || !n.authTag || !tokenMatch(String(tag || ''), String(n.authTag))) { try { sock.destroy(); } catch (_) {} return; }
		if (n.muxLink) { try { n.muxLink.stop(); } catch (_) {} n.muxLink = null; }
		const mux = Tunnel.muxClient(sock, { onDown: () => { if (n.muxLink === mux) n.muxLink = null; }, streamIdleMs: SPLICE_IDLE_MS });
		n.muxLink = mux;
		while (n.waiters.length) {
			if (n.active + mux.count() >= maxActiveSplices) break;
			mux.openStream(n.waiters.shift().client);
		}
	}

	function registerNode(control, nodeId, tag, ip, signalCred, wantMux, initialBytes) {
		...
		const publicServer = net.createServer((client) => {
			// First-byte deadline: a freshly accepted connection that has read NO bytes by FIRST_BYTE_MS is dropped, so an
			// unauthenticated flood of silent connections cannot pin this node at its connection ceiling for the whole
			// idle window. Checked via client.bytesRead (set by whichever path reads the socket — splice pipe, mux, or the
			// waiter), so it neither adds a data listener, changes flow mode, nor consumes bytes. A one-shot timer: once any
			// byte has arrived the idle reaper (SPLICE_IDLE_MS) governs from there.
			const firstByte = setTimeout(() => { if (client.bytesRead === 0) { try { client.destroy(); } catch (_) {} } }, FIRST_BYTE_MS);
			if (firstByte.unref) firstByte.unref();
			client.once('close', () => clearTimeout(firstByte));
			...
			// Prefer the mux link: carry this client as one stream over the single node link. The mux link reaps a stream
			// that goes silent both ways (streamIdleMs = SPLICE_IDLE_MS), matching the splice path's idle reaper, so an
			// unauthenticated client that opens a stream and then sits silent cannot tie one up.
			if (n.muxLink && !n.muxLink.down()) { n.muxLink.openStream(client); return; }
		...
		}, { max: SIGNAL_MAX, initial: initialBytes || null }); // forward any bytes that arrived in the same segment as HELLO, so a node that pipelines a SIGNALREPLY after its greeting is not silently dropped (mirrors the node side)
	...
	const server = net.createServer((sock) => {
		...
		readGreeting(sock, (err, line, rest) => {
			...
			if (verb === 'HELLO') registerNode(sock, nodeId, tag, ip, parts[4], muxEnabled && parts.includes('MUX'), rest);
...
function registerNode({ hubHost, hubPort, token, nodeId, nodeAuth, localPort, pool = DEFAULT_POOL, onEvent, onSignal = null, maxActiveSplices = MAX_ACTIVE_SPLICES, mux }) {
	...
	function openMux() {
		...
		const proxy = Tunnel.nodeProxy(m, { localServePort: localPort, streamIdleMs: SPLICE_IDLE_MS, maxStreams: maxActiveSplices });
