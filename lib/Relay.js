...
function splice(a, b, idleMs = SPLICE_IDLE_MS) {
	let dead = false, idle = null;
	const kill = () => { if (dead) return; dead = true; if (idle) clearTimeout(idle); try { a.destroy(); } catch (_) {} try { b.destroy(); } catch (_) {} };
...
function runHub({ controlPort = Common.DEFAULT_RELAY_PORT, token, host = '0.0.0.0', portRange = Common.DEFAULT_RELAY_DATA_PORT_RANGE, onLog } = {}) {
