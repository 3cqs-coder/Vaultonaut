...
function runHub({ controlPort = Common.DEFAULT_RELAY_PORT, token, host = '0.0.0.0', portRange = Common.DEFAULT_RELAY_DATA_PORT_RANGE, onLog, maxActiveSplices = MAX_ACTIVE_SPLICES, maxNodes = MAX_NODES, maxNodesPerIp = MAX_NODES_PER_IP } = {}) {
	...
	function registerNode(control, nodeId, tag, ip, signalCred) {
		...
		const n = { publicPort, workers: [], control, waiters: [], active: 0, authTag: String(tag || ''), signalCred: String(signalCred || ''), ip: ip || null };
	...
	const server = net.createServer((sock) => {
		...
		readGreeting(sock, (err, line) => {
			...
			const scopedOk = !!nodeId && (verb === 'SIGNAL' || verb === 'WHERE') && (() => { const n = nodes.get(nodeId); return !!(n && n.signalCred && tokenMatch(tok, n.signalCred)); })();
			if (!mainOk && !scopedOk) { noteAuthFail(ip); try { sock.end('ERR auth\n'); } catch (_) {} return; }
			...
			if ((verb === 'HELLO' || verb === 'WORKER') && !mainOk) { try { sock.end('ERR auth\n'); } catch (_) {} return; }
			if (verb === 'HELLO') registerNode(sock, nodeId, tag, ip, parts[4]);
...
function registerNode({ hubHost, hubPort, token, nodeId, nodeAuth, localPort, pool = DEFAULT_POOL, onEvent, onSignal = null, maxActiveSplices = MAX_ACTIVE_SPLICES }) {
	...
	const greet = (verb) => verb === 'HELLO'
		? 'HELLO ' + token + ' ' + nodeId + (tag ? ' ' + tag + ' ' + sigCred : '') + '\n'
...
module.exports = { runHub, registerNode, reflexiveAddress, signalExchange, nodeId, nodeTag, signalCredential, splice, readLines, reconnectDelay, SPLICE_IDLE_MS, RECONNECT_BASE_MS, RECONNECT_MAX_MS, MIN_TOKEN_LEN, MAX_NODES, MAX_NODES_PER_IP, AUTH_FAIL_MAX, SIGNAL_MAX, SIGNAL_MAX_PENDING };
