...
function runHub({ controlPort = Common.DEFAULT_RELAY_PORT, token, host = '0.0.0.0', portRange = Common.DEFAULT_RELAY_DATA_PORT_RANGE, onLog, maxActiveSplices = MAX_ACTIVE_SPLICES } = {}) {
	...
	function registerNode(control, nodeId) {
		...
		const n = { publicPort, workers: [], control, waiters: [], active: 0 };
...
function registerNode({ hubHost, hubPort, token, nodeId, localPort, pool = DEFAULT_POOL, onEvent, maxActiveSplices = MAX_ACTIVE_SPLICES }) {
...
module.exports = { runHub, registerNode, nodeId, splice, SPLICE_IDLE_MS };
