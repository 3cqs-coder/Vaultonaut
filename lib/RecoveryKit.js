...
// The template is a fixed bundled file, so read it once and reuse it — a kit generated from a web route should not
// re-read disk on the shared service every time (the same one-time-cache pattern as the emergency README template).
let TEMPLATE_CACHE = null;
function templateHtml() { if (TEMPLATE_CACHE === null) TEMPLATE_CACHE = fs.readFileSync(TEMPLATE_PATH, 'utf8'); return TEMPLATE_CACHE; }
...
function render(data = {}) {
	...
	const values = {
		...
		GENERATED_AT: d.generatedAt || new Date().toISOString().replace('T', ' ').replace(/\..*$/, ' UTC'),
	};
