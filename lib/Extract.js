...
//
// The limits mirror the layered defense the established OOXML libraries use (for example Apache POI's ZipSecureFile):
//   * a per-entry INFLATION-RATIO ceiling — a deflate stream that expands more than ZIP_MAX_INFLATE_RATIO times its
//     compressed size is treated as a bomb. It aborts EARLY, so a tiny compressed stream can never materialize much;
//   * a total UNCOMPRESSED budget across the whole archive — the hard memory backstop for OUR OWN inflation here;
//   * a NODE-count cap — the crucial one. The readers do not just hold the inflated bytes: they build a full in-memory
//     DOM (mammoth via jszip) or row/string arrays (read-excel-file), and every XML element AND every attribute becomes
//     a heavy node object. That node amplification, not the byte count, is what exhausts the worker heap — an archive
//     can sit under the byte budget yet still expand into tens of millions of nodes and crash the reader. Every element
//     carries a '<' and every attribute carries a '=', so counting both markers upper-bounds the node count regardless
//     of whether the bomb is element-dense or attribute-dense, and the total is capped to what the worker heap can hold;
...
// output would exceed the smaller of the ratio limit and the remaining budget. An over-budget, over-ratio, over-dense,
// or malformed file simply indexes by name (returns ''). Pure Node (zlib), cross-platform.
const ZIP_MAX_UNCOMPRESSED = 128 * 1024 * 1024; // total REAL inflated bytes across all entries (both enumerations, deduped); the hard memory backstop for this guard's own inflation
const ZIP_MAX_INFLATE_RATIO = 500;              // a single entry may expand at most this many times its compressed size; real OOXML parts stay well below this (even whitespace-heavy XML is ~100x), while a classic single-stream bomb approaches DEFLATE's ~1032x ceiling
const ZIP_MAX_NODES = 3000000;                  // total XML nodes ('<' elements + '=' attributes) across the archive. The reader builds one heavy object (~100+ bytes) per node, so this bounds its peak memory (a few hundred MB of nodes) below the worker heap even alongside a large index. Real documents hold far fewer; an element- or attribute-dense bomb hits this long before the byte budget
...
function zipWithinBudget(buf) {
	try {
		...
		const vet = (lho, cdComp) => {
			...
			if (method === 8) {                                     // deflate: inflate to the stream's own end, bounded by ratio and budget
				// The trusted compressed size for the ratio ceiling: the local header's, else the central directory's
				// (a data-descriptor local header carries 0). If neither is known, the total budget alone bounds it.
				const comp = (!hasDataDescriptor && compSize > 0) ? compSize : (cdComp > 0 ? cdComp : 0);
				const ratioCap = comp > 0 ? Math.max(4096, comp * ZIP_MAX_INFLATE_RATIO) : state.budget; // an over-ratio entry throws early; the 4 KiB floor lets a tiny entry hold a small header
				const cap = Math.max(1, Math.min(state.budget, ratioCap));
				// maxOutputLength makes zlib throw the moment the output would exceed the cap, so an over-ratio or
				// over-budget stream can never materialize more than the cap before it is refused.
				try { content = zlib.inflateRawSync(buf.subarray(dataStart), { maxOutputLength: cap }); }
				catch (_) { return false; }                         // over the cap, or a corrupt/truncated stream -> refuse (index by name)
