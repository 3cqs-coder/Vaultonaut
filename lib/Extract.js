...
function zipWithinBudget(buf) {
	try {
		...
		// anomaly, size, or node-count breach.
...
// A PDF whose text layer holds fewer than this many characters is treated as scanned/image-only, so when OCR is
// enabled its pages are recognized. Not zero: a scan often carries a stray page number or header in a real text layer.
const OCR_PDF_TEXT_THRESHOLD = 100;
// `ocr`, when present and enabled, turns on optical character recognition for scanned PDFs and image files:
// { enabled, lang, langDir, engine }. It is null (off) unless the caller passes it, so nothing changes for a build
// with OCR disabled. Recognition itself lives in lib/Ocr.js and is required lazily only when it actually runs.
async function extractText(buffer, ext, cap = DEFAULT_CAP, ocr = null) {
	try {
		switch (String(ext || '').toLowerCase()) {
			case 'pdf': {
				...
				try {
					...
					layerText = sliceText(parts.join('\n'), cap);
				} finally { try { await doc.destroy(); } catch (_) {} }
				// A scanned/image-only PDF carries little or no text layer, so the walk above yields almost nothing. When
				// OCR is enabled, recognize the rendered pages and use that text (appending any stray text-layer text).
				// This runs on the original bytes AFTER the reader is torn down, and is bounded by the same cap.
				if (ocr && ocr.enabled && layerText.length < OCR_PDF_TEXT_THRESHOLD) {
					const recognized = await require('./Ocr').ocrPdf(buffer, { cap, lang: ocr.lang, langDir: ocr.langDir, engine: ocr.engine });
					if (recognized) return sliceText([layerText, recognized].filter(Boolean).join('\n'), cap);
				}
				return layerText;
			}
			case 'docx':
			...
			case 'png': case 'jpg': case 'jpeg': case 'tif': case 'tiff': case 'bmp': case 'webp':
				// An image file has no text layer at all, so it is content-indexable only through OCR. Index it by name
				// only (return null) unless OCR is enabled, so nothing changes for a build with OCR off.
				if (!(ocr && ocr.enabled)) return null;
				return sliceText(await require('./Ocr').ocrImage(buffer, { cap, lang: ocr.lang, langDir: ocr.langDir, engine: ocr.engine }), cap);
			default:
				return null;
		}
...
// Image formats that carry no text layer and are therefore extractable ONLY when OCR is enabled. Kept separate from
// EXTRACTABLE so the caller can gate them on the OCR setting (an off build never spawns an extractor for an image).
const OCR_IMAGE_EXTS = new Set(['png', 'jpg', 'jpeg', 'tif', 'tiff', 'bmp', 'webp']);

module.exports = { extractText, EXTRACTABLE, OCR_IMAGE_EXTS, stripHtml, sliceText, zipWithinBudget, ZIP_MAX_UNCOMPRESSED, ZIP_MAX_NODES, ZIP_MAX_ENTRIES };
