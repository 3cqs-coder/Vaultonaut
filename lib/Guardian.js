	...
	for (;;) {
		...
		if (stale) {
			...
			if (stillStale) {
				...
				// Trade-off (accepted): this kills the bare pid. A stale nonce cannot tell a wedged instance of OUR
				// service apart from a dead one whose pid the OS recycled onto an unrelated process, so in the narrow
				// window where the pid was reused before we noticed, the wrong process could be signaled. Confirming the
				// pid's process identity would need its start time, which has no cross-platform read in Node without
				// spawning OS tools; given this path is opt-in, the window is small, and no data is at risk either way
				// (the lock/unmount below still runs), the pid-only kill is the deliberate choice. wedgeRestart therefore
				// assumes a host without aggressive immediate pid reuse.
