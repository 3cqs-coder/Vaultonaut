...
async function emit(ev, settings) {
	try {
		...
		try { Common.warn('A notification for "' + ev.type + '" could not be delivered after ' + RETRY_BACKOFFS_MS.length + ' attempts' + (lastStatus ? ' (last HTTP ' + lastStatus + ')' : (lastErr ? ': ' + (lastErr.message || lastErr) : '')) + '.'); } catch (_) {}
		return { delivered: false, status: lastStatus || undefined, error: lastErr && lastErr.message, attempts: RETRY_BACKOFFS_MS.length };
	} catch (e) {
		try { Common.warn('A notification could not be sent (' + (e && e.message || e) + ').'); } catch (_) {}
		return { delivered: false, error: e && e.message };
	}
}
