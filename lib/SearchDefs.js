...
const EXTRACT_TIME_BUDGET_MS = 90 * 1000;            // per-file parse ceiling. Memory is already bounded (the bomb guard and the byte cap), but a pathological document could still burn a CPU core for a long time; past this the file indexes by name so one bad file never stalls the whole reindex
...
// Image formats that are extractable ONLY through OCR (they carry no text layer). Gated on the OCR setting, so an
// OCR-off build never treats an image as content-indexable and never spawns an extractor for one.
const OCR_IMAGE_EXTS = require('./Extract').OCR_IMAGE_EXTS;
...
function isExtractable(name, ocrOn) { const e = extOf(name); return EXTRACT_EXTS.has(e) || (ocrOn && OCR_IMAGE_EXTS.has(e)); }
function isIndexable(name, ocrOn) { return isTextLike(name) || isExtractable(name, ocrOn); }
...
module.exports = {
	SCHEMA_VERSION, MS_OPTIONS, MS_SEARCH_OPTS, MAX_SEARCH_HITS, queryIndex, SEARCH_INLINE_MAX_BYTES, OPTIONS_HASH, SEARCH_DIR, NOTES_DIR, INDEX_NAME, INTERNAL_DIRS, inInternalDir, PER_FILE_TEXT_CAP, HARD_FILE_CEIL, EXTRACT_MAX_BYTES, EXTRACT_TIME_BUDGET_MS,
