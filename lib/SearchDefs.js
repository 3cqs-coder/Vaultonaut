...
// buffer, or a meta larger than MAX_META_BYTES — which is also what an old (differently-framed) envelope decodes to
...
function parseEnvelope(inflated, opts) {
	...
	if (!(opts && opts.metaOnly)) out.indexJSON = inflated.subarray(idxStart).toString('utf8');
