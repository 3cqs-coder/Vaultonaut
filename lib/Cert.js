...
// discovered peer can pin it and connect over HTTPS with hostname verification intact. Cached by the address set
...
async function ensureLanCert() {
	const ips = lanIPv4s();
	return ensureCert(ips.length ? ips : '127.0.0.1'); // no LAN interface → a loopback-only cert (not LAN-reachable, but valid)
}
