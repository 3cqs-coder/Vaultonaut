...
function torBinPath() { return path.join(torRoot(), 'tor', Common.exeName('tor')); }
...
async function ensureImpl() {
	...
	try {
		...
		if (process.platform !== 'win32') { try { await fsp.chmod(torBinPath(), 0o755); } catch (_) {} }
...
function paths() { return { tor: torBinPath(), geoip: geoipPath(), geoip6: geoip6Path(), tag: PINNED_TAG }; }
...
module.exports = { ensureTor, isInstalled, torSuffix, pinnedSha256, torBinPath, geoipPath, geoip6Path, PINNED_TAG, PINNED_SHA256 };
