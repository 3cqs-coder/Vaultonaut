...
// A hard timeout (SIGKILL) bounds it, so a wedged `tar`/`codesign` can never hang the one-time Tor setup forever —
// it resolves with a non-zero code and the caller surfaces a clear "could not extract/sign" error instead of stalling.
function runTool(cmd, args, opts) {
	return new Promise((resolve) => {
		let out = '', done = false; let child;
		...
		const to = setTimeout(() => { try { child.kill('SIGKILL'); } catch (_) {} finish({ code: -1, out: (cmd + ' timed out') }); }, 5 * 60 * 1000);
		...
		child.on('close', (code) => finish({ code: code, out: out }));
	});
}
