...
function launch(cmd, args) {
	try {
		const c = spawn(cmd, args, { detached: true, stdio: 'ignore', windowsHide: true }); // match every other detached spawn: never flash a console window on Windows
...
// Exposed ONLY so the driver-install test can exercise the "refuse an unverified installer" guard and the
// checksum-forwarding directly, without downloading or launching a real privileged installer. Not part of the API.
module.exports._test = { fetchAndVerify };
