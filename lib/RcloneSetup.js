...
// unavailable). Keyed by archiveSuffix(), so it is cross-platform by construction — it covers every platform and
// architecture this tool SUPPORTS (macOS, Linux, and Windows across the 64-bit and Arm builds Node itself supports),
// which the enginepin drift-guard test enforces. A niche target the installer could name but that this tool does not
// support (for example 32-bit x86 Linux, which the minimum Node dropped) is intentionally absent from the pin and, on
// that platform, verifies against the source-published checksum instead. Updating the engine is a deliberate,
// reviewable change: bump PINNED_TAG AND
...
// safely to the release's published checksum. These are the archive (.zip) checksums straight from the
// SHA256SUMS file published with PINNED_TAG at downloads.rclone.org (the exact release named above).
const PINNED_SHA256 = {
...
async function writeTagSidecar(tag) { try { const p = tagSidecar(), tmp = Common.uniqueTempPath(p); await fsp.writeFile(tmp, String(tag || '')); await Common.fsyncPath(tmp); await Common.renameWithRetry(tmp, p); } catch (_) {} }
...
async function writeSidecar(hex) {
	const p = shaSidecar();
	const tmp = Common.uniqueTempPath(p);
...
async function resolveRelease({ latest = false } = {}) {
	...
	// Fast, API-FREE path for a pinned install with a committed checksum (every shipped platform). The archive name is
	// deterministic (rclone-<tag>-<suffix>.zip) and the SHA-256 is shipped in this file, so the download can go
	// straight to the rclone.org mirror (and a direct GitHub asset URL as a fallback) and be verified against the
	// committed hash — with NO call to the GitHub releases API. That API rate-limits unauthenticated callers and
	// returns HTTP 403 from shared CI runner IPs, which was breaking a fresh pinned install (and the smoke test) there.
	if (!latest) {
		const committed = pinnedSha256();
		if (committed) {
			const fileName = 'rclone-' + PINNED_TAG + '-' + suffix + '.zip';
			return {
				tag: PINNED_TAG,
				sha256: committed,
				fileName,
				primaryUrl: 'https://downloads.rclone.org/' + PINNED_TAG + '/' + fileName,       // rclone.org mirror
				fallbackUrl: 'https://github.com/rclone/rclone/releases/download/' + PINNED_TAG + '/' + fileName // direct GitHub asset (same file/checksum) — NOT the rate-limited API
			};
		}
	}
	// Otherwise (a --latest install, or a platform with no committed checksum) resolve the asset via the GitHub API.
...
async function liftBinary(dir) {
	...
	let wantHash = null; try { wantHash = await Net.sha256File(src); } catch (_) {} // the NEW binary's hash, taken before the move consumes src
	// renameWithRetry rides out the transient Windows EPERM/EBUSY an antivirus or Search indexer takes on the
	// just-extracted binary (a bare rename would fail the first attempt and report a spurious fresh-install failure);
	// the copyFile fallback covers a cross-volume (EXDEV) staging dir. A genuine lock by a RUNNING engine is still
	// caught by the hash check below, so a silently failed overwrite is reported as a failed lift, not a success.
	try { await Common.renameWithRetry(src, dst); } catch (_) { try { await fsp.copyFile(src, dst); } catch (_) {} }
	...
	// Confirm the NEW binary is actually in place, not just that SOMETHING is. On Windows a running engine (a mounted
	// vault) locks rclone.exe, so both the rename and the copy fail and the OLD binary is left behind — existsSync(dst)
	// would still be true and the caller would wrongly record the update as done. Compare the destination to the
	// extracted binary's hash so a silently failed overwrite is reported as a failed lift, not a success.
	if (!fs.existsSync(dst)) return false;
	if (wantHash) { try { return (await Net.sha256File(dst)) === wantHash; } catch (_) { return false; } }
	return true; // could not hash the source (unexpected) — fall back to existence, matching the prior behavior
}
...
let _ranOk = false;
// Clear the once-per-process verification memo, so the NEXT ensure() re-hashes the on-disk engine (and, if it no
// longer matches, re-downloads it) instead of trusting that a binary verified earlier is still the one on disk. The
// periodic self-check calls this when it detects that the cached engine no longer matches its recorded checksum — a
// swap or corruption while the service is running — so its "re-verified, and if needed re-downloaded, on the next
// mount" remediation actually happens in a long-running process rather than only after a restart.
function invalidateVerification() { verifiedThisProcess = false; _ranOk = false; }
// Ensure data/bin has a working, verified rclone. Returns { rclone, ok }.
// Coalesce concurrent ensure() calls into ONE download-and-verify. The background auto-installer (startEngineSetup)
// and a real operation (ensureEngine) each call ensure() independently, so without this they launched TWO full ~30 MB
// downloads that raced — wasting bandwidth and, if the process was interrupted mid-download, leaving an orphaned temp.
// While a run is in flight, every other caller AWAITS the same result (it wants the engine ready either way). Keyed on
// `latest` so a deliberate `--latest` install never shares a pinned run's result. On settle the slot clears, so the
// next call re-checks the cache (fast when already installed) or retries a failed one.
const _ensureInFlight = new Map();
function ensure(opts = {}) {
	const key = opts && opts.latest ? 'latest' : 'pinned';
	let p = _ensureInFlight.get(key);
	if (!p) { p = ensureImpl(opts).finally(() => { _ensureInFlight.delete(key); }); _ensureInFlight.set(key, p); }
	return p;
}
// Remove download/extract temps left in the bin dir by a PRIOR run that was hard-killed mid-download (the normal path
// cleans its own temps on both success and failure). Age-gated well past the 20-minute download deadline, so it can
// never remove a temp still being written — single-flight rules out same-process concurrency, and a separate `--latest`
// CLI process on a slow link is the only other possible writer.
async function sweepDownloadTemps(dir) {
	try {
		const now = Date.now();
		for (const name of await fsp.readdir(dir)) {
			if (!/^_rclone_(download|extract)\./.test(name)) continue; // matches the zip, its Net.download `.part-*`, and the extract dir
			const p = path.join(dir, name);
			try { const st = await fsp.stat(p); if (now - st.mtimeMs > 30 * 60 * 1000) await fsp.rm(p, { recursive: true, force: true }); } catch (_) {}
		}
	} catch (_) {}
}
async function ensureImpl(opts = {}) {
	const dir = Common.binDir();
	// A pre-installed engine directory (VAULTONAUT_ENGINE_DIR, e.g. the container's baked, read-only /engine) is already
	// present and correctly owned; skip creating and chmod-ing it, which would fail against a read-only root filesystem.
	if (!Common.binDirIsPreinstalled()) {
	...
	await sweepDownloadTemps(dir); // clear any temp orphaned by a prior hard-killed download before starting a fresh one
	// Download to a PER-PROCESS temp zip and extract into a PER-PROCESS temp folder, then lift the binary into place.
	// ensure() coalesces concurrent callers into one run (see above), so a second download no longer races this one; the
	// unique per-process temp paths remain as belt-and-suspenders (e.g. a separate `--latest` CLI process), and the final
	// rename into data/bin is atomic and idempotent (identical verified bytes either way).
	...
	try {
		...
		const lifted = await liftBinary(exdir);
		await cleanup();
		if (lifted && await works(bin)) {
		...
		if (!lifted) {
			// The freshly downloaded engine could not be moved into place. The usual cause is Windows: a running engine
			// (a mounted vault) holds rclone.exe open, so it cannot be overwritten and the OLD binary stays. Do NOT
			// record the new tag or hash — that would falsely mark the update as applied and stop the next run from
			// retrying, leaving the on-disk binary and the recorded pin permanently out of step. Keep using the existing
			// verified engine for now (fallback), so nothing breaks; the update completes on the next setup once no vault
			// is mounted. On a FIRST install there is no old engine to fall back to, so fallback fails closed as before.
			log('The engine update could not be applied while the current engine is in use (a mounted vault is holding it open). The existing engine is still used; unmount all vaults, then run setup again to finish the update.');
...
module.exports = { ensure, probe, resolve, integrityStatus, installedVersion, invalidateVerification, PINNED_TAG, PINNED_SHA256, pinnedSha256, archiveSuffix, rcloneArch };
module.exports._test = { sweepDownloadTemps }; // test-only: enginedownload.js exercises the orphan-temp sweep
module.exports._resolveRelease = resolveRelease; // test-only: verify a pinned install resolves without the GitHub API
