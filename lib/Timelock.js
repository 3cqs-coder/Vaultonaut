...
async function seal(round, payload, { publicKey } = {}) {
	const data = Buffer.isBuffer(payload) ? payload : Buffer.from(String(payload), 'utf8');
	try {
		const { blob } = await require('./WorkerRun').runWorker(path.join(__dirname, 'TimelockWorker.js'),
			{ op: 'seal', args: { round, payloadB64: data.toString('base64'), publicKey } }, null, { maxMs: 120000, idleMessage: 'Time-locking the grant' });
		return blob;
	} catch (_) { return sealToRound(round, data, { publicKey }); } // worker unavailable — do it inline
}
async function open(blob, sigBytes) {
	const sigHex = Buffer.from(sigBytes).toString('hex');
	try {
		const { payloadB64 } = await require('./WorkerRun').runWorker(path.join(__dirname, 'TimelockWorker.js'),
			{ op: 'open', args: { blob, sigHex } }, null, { maxMs: 120000, idleMessage: 'Opening the time-locked grant' });
		return Buffer.from(payloadB64, 'base64');
	} catch (_) { return openWithSignature(blob, sigBytes); } // worker unavailable, or surface the crypto error the inline path throws
}
...
module.exports = {
	...
	ibeEncrypt, ibeDecrypt, verifyBeacon, sealToRound, openWithSignature, fetchSignature, seal, open, _test,
};
