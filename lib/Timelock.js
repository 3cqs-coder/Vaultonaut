...
// ── beacon fetch ─────────────────────────────────────────────────────────────────────────────────────────────
// Fetch the signature for a round from the public gateways and VERIFY it against the pinned key before returning, so
// a wrong or hostile gateway is rejected rather than trusted. Returns the 48-byte signature, or null if the round is
// not available yet or no gateway served a valid one. Each request is bounded and non-blocking (Net.getText caps size
// and time). The gateways are RACED, not tried one-by-one: a single slow or dead gateway would otherwise make the
// caller wait out its whole timeout before the next is even attempted, so with several gateways the worst case became
// the sum of the dead ones' timeouts. Racing makes the latency that of the fastest healthy gateway. Racing changes
// only the timing, never the trust decision — each racer resolves ONLY with a signature that passes verifyBeacon, so
// a wrong or hostile gateway simply rejects and loses the race; if every gateway fails, Promise.any rejects and we
// return null.
async function fetchSignature(round, { Net } = {}) {
	const net = Net || require('./Net');
	const fromGateway = async (base) => {
		const body = await net.getText(base + '/' + QUICKNET.chainHash + '/public/' + round);
		const j = JSON.parse(body);
		const sigHex = j && typeof j.signature === 'string' ? j.signature.trim() : '';
		if (!/^[0-9a-f]{96}$/i.test(sigHex)) throw new Error('no signature'); // a 48-byte compressed G1 point is 96 hex chars
		const sig = Buffer.from(sigHex, 'hex');
		if (!verifyBeacon(round, sig)) throw new Error('signature did not verify against the pinned key');
		return sig; // only a signature that verifies against the pinned key is trusted
	};
	try { return await Promise.any(ENDPOINTS.map(fromGateway)); }
	catch (_) { return null; } // every gateway failed, served nothing, or the round is not available yet
}
...
// On Windows, run the pairing SYNCHRONOUSLY rather than in a worker thread. Node 24.x on Windows can segfault during a
// worker thread's TEARDOWN once that worker has loaded the elliptic-curve library and done pairing math — observed as a
// process-killing crash (exit 139) in the time-locked emergency tests, while the NON-time-locked emergency paths, which
// spawn no worker, run clean on the same runner. The synchronous core is byte-for-byte the same crypto (see the shared
// seal/open below), only briefly on the main thread; the emergency seal/open are rare, user-initiated operations, never
// a per-request hot path, so the short pause is an acceptable trade to avoid a crash. Elsewhere the worker still keeps
// the shared service's event loop responsive during the pairing.
const PAIRING_IN_WORKER = process.platform !== 'win32';
async function seal(round, payload, { publicKey } = {}) {
	const data = Buffer.isBuffer(payload) ? payload : Buffer.from(String(payload), 'utf8');
	if (!PAIRING_IN_WORKER) return sealToRound(round, data, { publicKey });
	let result;
...
async function open(blob, sigBytes) {
	...
	if (!PAIRING_IN_WORKER) return openWithSignature(blob, sigBytes);
	let result;
	try { result = await require('./WorkerRun').runWorker(path.join(__dirname, 'TimelockWorker.js'),
