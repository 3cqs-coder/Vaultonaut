...
async function removeDecoy({ realVault, managerPassword } = {}) {
	return regQueue(async () => {
		// Deniability: "no registry" must be indistinguishable from "wrong manager password" — the same result AND the
		// same timing — or this endpoint becomes an oracle for whether any decoy is configured on this machine (the
		// exact hole listMappings deliberately closes). With no registry, still burn an equivalent Argon2id derivation
		// (default params, a fresh salt) and fail exactly like a wrong password, instead of returning {removed:false}
		// instantly. A legitimate manager who removes a decoy that is not configured simply sees the wrong-password
		// error, which is the correct deniable outcome.
		if (!hasRegistry()) { try { await SR.deriveK(String(managerPassword || ''), SR.newKdfParams()); } catch (_) {} throw new Error(MANAGER_PW_WRONG); }
		const m = await loadManager(managerPassword);
		if (!m) throw new Error(MANAGER_PW_WRONG);
...
async function resolveDecoy(realVault, password) {
	// Deniability on the MOUNT path too: "no decoy registry" must take the same time as "registry present but this
	// password matches nothing". Returning instantly when no registry exists would make a wrong-password unlock
	// measurably faster on a machine with no decoy than on one with a decoy configured — a timing oracle for whether
	// ANY decoy exists. So with no registry, burn an equivalent Argon2id derivation (default params, fresh salt) before
	// returning null, exactly as listMappings/removeDecoy do. The caller therefore consults this on EVERY failed unlock.
	if (!hasRegistry()) { try { await SR.deriveK(String(password || ''), SR.newKdfParams()); } catch (_) {} return null; }
