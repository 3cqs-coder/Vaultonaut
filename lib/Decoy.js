...
async function loadManager(managerPassword) {
	...
	let manager = null;
	for (let i = 0; i < slots.length; i++) { const o = SR.tryDecryptSlot(Km, slots[i]); if (o && o.kind === 'manager' && manager === null) manager = o; } // no break: constant work
...
async function resolveDecoy(realVault, password) {
	...
	let decoy = null;
	for (let i = 0; i < slots.length; i++) { const o = SR.tryDecryptSlot(K, slots[i]); if (o && o.kind === 'duress' && decoy === null && Common.samePath(o.realVault, want)) decoy = o.decoyVault; } // no break
	return decoy;
}
