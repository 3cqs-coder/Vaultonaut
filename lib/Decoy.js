...
async function loadManager(managerPassword) {
	...
	for (let i = 0; i < slots.length; i++) { const o = SR.tryDecryptSlot(Km, slots[i]); if (o && o.kind === 'manager' && Number(o.v) === 1 && manager === null) manager = o; } // no break: constant work
...
async function resolveDecoy(realVault, password) {
	...
	for (let i = 0; i < slots.length; i++) { const o = SR.tryDecryptSlot(K, slots[i]); if (o && o.kind === 'duress' && Number(o.v) === 1 && decoy === null && Common.samePath(o.realVault, want)) decoy = o.decoyVault; } // no break
	return decoy;
}
