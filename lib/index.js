...
module.exports = {
	...
	emergencyEnroll: Vault.emergencyEnroll,     // set up the dead-man's switch (windows + first beneficiary)
	emergencyAddContact: Vault.emergencyAddContact,   // add another beneficiary
	emergencyRemoveContact: Vault.emergencyRemoveContact, // remove a beneficiary and drop their sealed grants
	...
	emergencySetBackstop: Vault.emergencySetBackstop,   // set up the optional trustee-quorum recovery backstop
	emergencyClearBackstop: Vault.emergencyClearBackstop, // turn the backstop off
	emergencyTrusteeShare: Vault.emergencyTrusteeShare, // a trustee opens their sealed backstop share
	emergencyRecoverToken: Vault.emergencyRecoverToken, // reconstruct a read cap from a quorum of trustee shares
	...
	setConsentOnUse: Vault.setConsentOnUse,   // require explicit approval on each unlock, recorded in the tamper ledger
	isConsentOnUse: Vault.isConsentOnUse,
	...
	clearOnionIdentity: Vault.clearOnionIdentity,
	getTorBridges: Vault.getTorBridges,
	setTorBridges: Vault.setTorBridges,
	setRelay: Vault.setRelay,
	getRelay: Vault.getRelay,
	makePeerCode: Vault.makePeerCode,
	parsePeerCode: Vault.parsePeerCode,
	makeRelayCode: Vault.makeRelayCode,     // bundle a relay hub's address + token into one paste-able invite code
	parseRelayCode: Vault.parseRelayCode,
	...
	createApiToken: Vault.createApiToken,           // bearer tokens for headless / third-party API clients
	listApiTokens: Vault.listApiTokens,
	revokeApiToken: Vault.revokeApiToken,
	...
	// Verify THIS installation's own files against the maintainer-signed release manifest, on demand — the same
	// check the app runs at boot, exposed so a user can ask for it explicitly. Reuses ReleaseIntegrity (one verifier).
	verifySelf: async () => { const RI = require('./ReleaseIntegrity'); const r = await RI.verifyInstall(); let pending = null; try { pending = await RI.signingPending(); } catch (_) {} return Object.assign({ maintainer: pending !== null, signingPending: !!(pending && pending.pending) }, r); },
	...
	notesImport: Vault.notesImport,         // import logins/notes from a .csv or Bitwarden .json export into an open vault
	...
	receiveVault: Vault.receiveVault,               // pull a vault's ciphertext from another node into a new local vault
	receiveVaultFromCode: Vault.receiveVaultFromCode, // the same, addressed by a peer connect code (keys never cross the wire)
	vaultsDir: Vault.vaultsDir,                     // the default home for vault storage (where a received vault lands)
	...
	vaultTitle: Vault.vaultTitle,     // display title (encrypted, once known) else the folder name
	friendlyName: Vault.friendlyName, // best readable name without a password (cached title, else folder name) — async
	getVaultMeta: Vault.getVaultMeta, // read the encrypted title/description (needs a credential that opens the vault)
	setVaultMeta: Vault.setVaultMeta, // set the encrypted title/description (needs the read-write password)
	anonymizeVault: Vault.anonymizeVault, // rename an existing named vault's folder to an opaque id (needs the read-write password; must be unmounted)
