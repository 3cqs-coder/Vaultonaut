...
// folds each slot's full KDF via sealTag; the legacy v1 form folds only levelOf. Both are kept byte-identical to
// the main tool so accepting either lets a v2-sealed AND an older v1-sealed bundle verify GENUINE here.
function manifestSealInput(manifest) {
	...
	return ['vault-manifest-seal-v2', SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', slots.join(',')].join('\n');
}
function manifestSealInputLegacy(manifest) {
	const c = (manifest && manifest.crypt) || {};
	const slots = keySlotsOf(c).filter(s => s.kind !== 'member').map(s => s.id + ':' + crypto.createHash('sha256').update(String(s.wrappedKey || '')).digest('hex') + ':' + levelOf(s.kdf)).sort();
	return ['vault-manifest-seal-v1', SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', slots.join(',')].join('\n');
}
function manifestSealVerifies(manifest, pub, sig) {
	try { if (verify(pub, manifestSealInput(manifest), sig)) return true; } catch (_) {}
	try { if (verify(pub, manifestSealInputLegacy(manifest), sig)) return true; } catch (_) {}
	return false;
}
...
function verifyBundle(dir, expectIdentity) {
	...
	// Refuse forward: a bundle whose baseline was written by a NEWER version binds fields this vendored verifier does
	// not know into its signing input, so the check below would fail and wrongly print TAMPERED. Report UNVERIFIED and
	// tell the reader to update the verifier — a legitimate newer bundle must never look tampered.
	if ((typeof baseline.version === 'number' && baseline.version > BASELINE_VERSION) || (baseline.scheme && baseline.scheme !== SCHEME)) {
		add('bundle-format', false, 'This bundle was made by a newer version of the tool — update the verifier to check it.');
		const pk = manifest.integrity && manifest.integrity.pubkey;
		return { verdict: 'UNVERIFIED', checks, identity: pk ? identity(pk) : null };
	}
...
module.exports = { verifyBundle, _primitives: { merkleRoot, identity, fingerprint, signingInput, levelOf, sealTag, keySlotsOf }, _sealParity: { verify, manifestSealInput, manifestSealInputLegacy, manifestSealVerifies, successionInput, attestGenesis, attestChainHash, attestDigest, tokenHashOf } };
