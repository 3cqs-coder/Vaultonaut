...
const SEAL_VERSION = 4; // manifest-seal format
...
function manifestSealInput(manifest) {
	...
	return ['vault-manifest-seal-v4', SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', String(Number(manifest.recoveryVersion) || 0), slots.join(',')].join('\n');
}
