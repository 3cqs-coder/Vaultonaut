...
const ROSTER_SEAL_VERSION = 1; // roster (owner) seal format; a newer roster is refused forward (UNVERIFIED), never falsely reported TAMPERED — mirrors the main tool
...
function rosterSealInput(manifest) {
	...
	return ['vault-roster-seal-v' + ROSTER_SEAL_VERSION, SCHEME, String(m.epoch || 0), String(m.keyGeneration || 0), String(m.ownerPubKey || ''), memberSlots.join(','), rec].join('\n'); // ROSTER_SEAL_VERSION === 1 reproduces the original 'vault-roster-seal-v1' bytes
}
