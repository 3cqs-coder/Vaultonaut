...
const BASELINE_VERSION = 5; // the baseline record format this verifier understands (v5 adds the ML-DSA post-quantum co-signature); a newer one is refused forward (see verifyBundle)
...
const SEAL_VERSION = 5; // manifest-seal format (v5 adds the ML-DSA post-quantum co-signature)
const ROSTER_SEAL_VERSION = 2; // roster (owner) seal format (v2 adds the owner ML-DSA post-quantum co-signature); a newer roster is refused forward (UNVERIFIED), never falsely reported TAMPERED — mirrors the main tool
...
// Post-quantum companion verify (ML-DSA-65, FIPS 204), mirroring Integrity.verifyPq. The public key is the base64 SPKI
// exactly as published in the manifest/baseline. Fails closed on any malformed input, so a corrupt or missing
// signature only fails the check. Kept self-contained here so the standalone verifier needs no project imports.
...
// Does THIS runtime carry ML-DSA (OpenSSL 3.5, Node 24.7+)? Probed once and cached. A Node built against an older
// OpenSSL passes the version gate but cannot verify the post-quantum half; treating that inability as tampering would
// false-report a genuine release. Mirrors Integrity.pqAvailable, self-contained so this verifier needs no imports.
let _pqAvail = null;
function pqAvailable() {
	if (_pqAvail !== null) return _pqAvail;
	try { const kp = crypto.generateKeyPairSync('ml-dsa-65'); const sig = crypto.sign(null, Buffer.from('probe'), kp.privateKey); _pqAvail = crypto.verify(null, Buffer.from('probe'), kp.publicKey, sig); }
	catch (_) { _pqAvail = false; }
	return _pqAvail;
}
// AND-combiner: both signatures must verify over the same bytes (mirrors Integrity.verifyHybrid). One place so the
// baseline and the manifest seal check the pair identically. If the runtime cannot do ML-DSA at all, accept the
// classical signature alone rather than false-reporting a genuine record — a capability the runtime lacks is not proof
// of tampering, and there is no post-quantum protection to strip on such a runtime.
function verifyHybrid(edPub, pqPub, payload, sig, sigPq) { if (!verify(edPub, payload, sig)) return false; if (!pqAvailable()) return true; return verifyPq(pqPub, payload, sigPq); }
...
// The current (v4) manifest-seal input: SCHEME, salt, published key, the signed recovery-version floor, and each
// non-member slot folded with its full KDF via sealTag PLUS its non-secret governance/auth attributes — kind, the
// `owner` marker, the keyfile name, and the WebAuthn binding (credential id + PRF salt). Kept byte-identical to the
// main tool so a bundle it sealed verifies GENUINE here.
function manifestSealInput(manifest) {
	...
	const slots = keySlotsOf(c).filter(s => s.kind !== 'member').map(s => {
		...
		return s.id + ':' + wrapHash + ':' + sealTag(s.kdf) + ':' + attrs;
	}).sort();
	// v5 folds the ML-DSA public key in alongside the Ed25519 one; kept byte-identical to the main tool.
	return ['vault-manifest-seal-v5', SCHEME, String(c.salt || ''), (manifest.integrity && manifest.integrity.pubkey) || '', (manifest.integrity && manifest.integrity.pqPubkey) || '', String(Number(manifest.recoveryVersion) || 0), slots.join(',')].join('\n');
}
function manifestSealVerifies(manifest, pub, sig, pqPub, sigPq) {
	try {
		// AND-combiner: both the Ed25519 and the ML-DSA signatures must verify. The post-quantum half is required
		// unconditionally (the main tool always writes it), so a stripped or forged one fails closed here too.
		return verifyHybrid(pub, pqPub || (manifest.integrity && manifest.integrity.pqPubkey), manifestSealInput(manifest), sig, sigPq || (manifest.integrity && manifest.integrity.manifestSigPq));
	} catch (_) { return false; }
}
...
function rosterSealInput(manifest) {
	...
	return ['vault-roster-seal-v' + ROSTER_SEAL_VERSION, SCHEME, String(m.epoch || 0), String(m.keyGeneration || 0), String(m.ownerPubKey || ''), String(m.ownerPqPubKey || ''), memberSlots.join(','), rec].join('\n'); // v2 folds in the owner ML-DSA key
}
...
function rosterVerifies(manifest) {
	...
	try { return verifyHybrid(m.ownerPubKey, m.ownerPqPubKey, rosterSealInput(manifest), m.sig, m.sigPq); } catch (_) { return false; }
}
// A roster sealed by a NEWER build (members.sealVersion above what this verifier knows) has a shape we cannot
// reconstruct, so verifying it would raise a false tamper alarm. The refuse-forward gate turns such a bundle into
// UNVERIFIED instead. The marker is unsigned, so a forged bump only downgrades to UNVERIFIED, never up to GENUINE.
...
function verifyBundle(dir, expectIdentity) {
	...
	if (manifest.members && manifest.members.ownerPubKey && manifest.members.sig) {
		...
		rosterOk = rosterNewer ? true : rosterVerifies(manifest);
	}
	...
	let sigOk = false;
	try {
		const baseInput = signingInput({ scheme: baseline.scheme, root: baseline.merkleRoot, seq: baseline.seq, prevRoot: baseline.prevRoot, count: baseline.count, createdAt: baseline.createdAt, version: baseline.version, sealed: baseline.sealed, deep: baseline.deep });
		// AND-combiner: the baseline is authentic only if BOTH its Ed25519 and its ML-DSA signatures verify over the same
		// input. The post-quantum key is the one published in the manifest (pqPubkey); it is required unconditionally.
		sigOk = verifyHybrid(pub, manifest.integrity && manifest.integrity.pqPubkey, baseInput, baseline.sig, baseline.sigPq);
	} catch (_) { sigOk = false; }
	...
	if (genesis && id && genesis.identity !== id) {
		...
		for (const b of (succession.items || [])) {
			...
			try { const input = successionInput(b); vfd = (b.alg || 'ed25519') === 'ed25519+ml-dsa-65' && verifyHybrid(b.oldPubkey, b.oldPqPubkey, input, b.sigOld, b.sigOldPq) && verifyHybrid(b.newPubkey, b.newPqPubkey, input, b.sigNew, b.sigNewPq) && identity(b.oldPubkey) === b.oldIdentity && identity(b.newPubkey) === b.newIdentity && (b.prev || null) === (prevChain || null); } catch (_) { vfd = false; }
	...
	// `anchored` mirrors the full tool: true only when an origin identity was supplied to check the lineage against a
	// known genesis. Without it, GENUINE means internally self-consistent, not proven to be the vault you recorded.
	return { verdict, checks, identity: id, anchored: !!expectIdentity, originIdentity: genesis ? genesis.identity : id, fingerprint: baseline.merkleRoot ? fingerprint(baseline.merkleRoot) : null, seq: baseline.seq, timestamps: (attest.items || []).length };
}
...
function verifyFileProof(p, expectIdentity) {
	...
	const anchored = !!expectIdentity;
	// Whether the proof binds the file's CONTENT (a deep-snapshot hash) or only its name and size. A size-only proof
	// is honest but weak — a different file padded to the same size would also satisfy it — so the caller must say so.
	const hasContentHash = !!(p.file && p.file.hash);
	return { verdict, checks, identity: id, anchored, originIdentity: p.originIdentity || null, file: p.file, at: p.at || null, seq: p.baseline.seq, hasContentHash };
}
...
if (require.main === module) {
	...
	// Refuse an oversized file BEFORE reading it into memory, the same guard loadJSON applies to every bundle file —
	// a real single-file proof is a few kilobytes, so a multi-gigabyte .json handed to this verifier is hostile and
	// must not be slurped and parsed (which would exhaust the verifier's heap before any check runs).
	try { const fp = path.resolve(dir); const st = fs.statSync(fp); if (st.isFile() && st.size <= MAX_BUNDLE_FILE_BYTES) asFileProof = JSON.parse(fs.readFileSync(fp, 'utf8')); } catch (_) {}
	if (asFileProof && asFileProof.kind === 'file-proof') {
		...
		if (r.verdict === 'GENUINE' && !r.anchored) console.log('Note: no recorded origin identity was provided, so GENUINE means internally self-consistent — not proof this is from the vault you recorded.');
	...
	if (r.verdict === 'GENUINE' && !r.anchored) console.log('Note: no recorded origin identity was provided (--expect <identity>), so GENUINE means internally self-consistent — not proof this is the vault you recorded. Pass --expect to anchor it.');
	process.exit(r.verdict === 'GENUINE' ? 0 : 1);
}
...
module.exports = { verifyBundle, verifyFileProof, _primitives: { merkleRoot, verifyMerkleProof, fileLeafHex, identity, fingerprint, signingInput, levelOf, sealTag, keySlotsOf, verify, verifyPq, verifyHybrid, pqAvailable }, _sealParity: { verify, manifestSealInput, manifestSealVerifies, rosterSealInput, rosterVerifies, rosterSealNewer, successionInput, attestGenesis, attestChainHash, attestDigest, tokenHashOf }, _bundleVersions: { BASELINE_VERSION, SEAL_VERSION, ROSTER_SEAL_VERSION, ATTEST_VERSION, SUCCESSION_VERSION, GENESIS_VERSION, FILE_PROOF_VERSION, SCHEME } };
