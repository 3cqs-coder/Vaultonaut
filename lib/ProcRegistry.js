...
//
// Lifetime model (why nothing here can leak). A process the tool spawns is one of two kinds:
//   • SHORT-lived — a quick engine call (obscure, cat, list). Tracked in the set below and killed by killAll on every
//     exit path, so a stopped CLI or service never leaves one behind.
//   • LONG-lived — a detached service, guardian, served node, or mount, MEANT to outlive its launcher. These are NOT
//     killed here (that would defeat their purpose). They are kept from leaking two ways that need no per-process kill:
//       1. The dead-man's switch (watchStdinForQuit): a SUPERVISED long child — the desktop shell's backend, a
//          test-spawned service, any future supervisor — is given a piped stdin; the instant its parent dies by any
//          means (crash, SIGKILL, Force-Quit, an interrupted test run) the OS closes that stdin and the child drains,
//          locks, and exits on its own. Cross-platform, no PID poll, no PID-reuse race.
//       2. A genuinely user-started long service (no supervising parent) is left running until an explicit stop. It is
//          deliberately NEVER auto-reaped: a detached service legitimately has no live owner to test against, so
//          "the owner is gone" cannot tell a running service the user wants from a leaked one — killing on that signal
//          would kill the very service they are using. The crash-guardian still guarantees its vaults are locked.
...
async function gracefulExit(code, sig) {
	...
	//
	// The DESKTOP app is neither: its native shell hides the window and then WAITS for this backend to drain and lock,
	// for a background-drain grace of ~5 minutes, before it force-kills. If the backend force-killed ITSELF at the tight
	// 15s launchd cap, it would abandon a large cloud vault's write-back mid-upload — orphaning a ".partial" — long
	// before the shell's grace even runs. So the desktop gets the same generous room, held a touch under the shell's
	// grace so the backend finishes and exits first. (The 15s cap still applies to a plain non-systemd headless run,
	// which may be under launchd.)
	const systemd = !!process.env.NOTIFY_SOCKET;
	let isDesktop = false; try { isDesktop = require('./Common').isDesktopApp(); } catch (_) {}
	const hardMs = systemd ? 9 * 60 * 1000 : (isDesktop ? 4.5 * 60 * 1000 : 15 * 1000);
...
function watchStdinForQuit() {
	try {
		...
		// The shell died without sending "quit". When the parent goes away its end of this pipe closes and our stdin
		// hits EOF — treat that exactly like a quit, so a force-terminated shell (Task Manager "End Task" on Windows,
		// where the "quit" line is the ONLY clean-stop channel, or a SIGKILL'd shell on Unix) still drains and locks
		// vaults instead of orphaning this backend holding the loopback port and any mounted drives.
