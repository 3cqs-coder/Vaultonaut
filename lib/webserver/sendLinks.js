...
function createSendStore({ now = () => Date.now() } = {}) {
	...
	function create({ ciphertext, meta = {}, ttlMs, maxViews, password = null } = {}) {
		sweep();
		...
		sends.set(id, { ciphertext: ct, meta: meta && typeof meta === 'object' ? meta : {}, exp: now() + ttl, maxViews: views, views: 0, pwHash: password ? hashPassword(password, pwSalt) : null, pwSalt, tries: 0 });
