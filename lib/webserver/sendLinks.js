...
function createSendStore({ now = () => Date.now() } = {}) {
	const sends = new Map(); // id -> { ciphertext, meta, exp, maxViews, views, pwHash, pwSalt, tries }

	function sweep() { const t = now(); for (const [k, v] of sends) if (v.exp <= t) sends.delete(k); }

	// Hash a link password with a per-send random salt, so the stored verifier is unique per link (a memory capture
	// cannot be matched against a precomputed table, and two links with the same password store different hashes).
	function hashPassword(password, salt) { return crypto.createHash('sha256').update(salt + ':' + String(password == null ? '' : password), 'utf8').digest('hex'); }

	// Stash one sealed item and return its id. `ciphertext` is base64 (iv|ct|tag) sealed under the caller's random
	// key; `meta` is small, non-secret display info shown before decrypting. `ttlMs` and `maxViews` bound the link;
	// `password` (optional) is a link password the recipient must supply — salted and hashed here, never stored plain.
	function create({ ciphertext, meta = {}, ttlMs, maxViews, password = null, vaultPath = null } = {}) {
		sweep();
		...
		const pwSalt = password ? crypto.randomBytes(16).toString('hex') : null;
		sends.set(id, { ciphertext: ct, meta: meta && typeof meta === 'object' ? meta : {}, exp: now() + ttl, maxViews: views, views: 0, pwHash: password ? hashPassword(password, pwSalt) : null, pwSalt, tries: 0, vaultPath: vaultPath || null });
	...
	// The most wrong password guesses a single protected link tolerates before it self-destructs — so obtaining the
	// link URL never lets someone brute-force the link password at leisure (the /s route adds a per-IP throttle too).
	...
	function redeem(id, password) {
		...
		if (e.pwHash) {
			const got = hashPassword(password, e.pwSalt);
			// constant-time compare so a wrong link password cannot be guessed by timing
			if (got.length !== e.pwHash.length || !crypto.timingSafeEqual(Buffer.from(got), Buffer.from(e.pwHash))) {
				// Count the failed guess and destroy the link once too many pile up, so the link itself bounds a
				// brute-force even if the per-IP throttle is somehow bypassed (a botnet spreading the guesses).
				e.tries = (e.tries || 0) + 1;
				if (e.tries >= MAX_PASSWORD_TRIES) { sends.delete(String(id)); return { gone: true }; }
	...
	// Drop every link that sealed an item from a given vault — called when that vault is crypto-erased or destroyed,
	// so a resealed one-item ciphertext can never outlive the vault it came from. Path-compared with the shared,
	// case-aware helper so a differently-cased path still matches. Returns how many links were dropped.
	function revokeByVault(vaultPath, samePath) {
		if (!vaultPath) return 0;
		let n = 0;
		for (const [id, e] of sends) { if (e.vaultPath && (samePath ? samePath(e.vaultPath, vaultPath) : e.vaultPath === vaultPath)) { sends.delete(id); n++; } }
		return n;
	}
	function clearAll() { sends.clear(); }

	return { create, info, redeem, revoke, revokeByVault, clearAll, sweep, _size: () => sends.size, MAX_CIPHERTEXT_BYTES };
}
