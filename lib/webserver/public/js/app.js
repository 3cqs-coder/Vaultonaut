...
async function runHeal(allowUnverified, force) {
	try {
		...
		protectShow((h.unrecoverableStripes || h.writeErrors) ? 'bad' : 'ok', 'Repaired ' + h.repairedData + ' data block' + (h.repairedData === 1 ? '' : 's') + (h.repairedParity ? ' and ' + h.repairedParity + ' recovery block' + (h.repairedParity === 1 ? '' : 's') : '') + sizeNote + '.' + filesNote + writeNote + changedNote + inPlaceNote + tail + authNote + unverifiedNote);
