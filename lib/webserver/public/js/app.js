...
async function keyfileDigest(file) {
	...
	// undefined". (The UI also hides the keyfile controls on such an origin; this is the belt-and-suspenders.)
	if (!(window.isSecureContext && window.crypto && crypto.subtle)) throw new Error('Keyfiles need a secure address (https, or localhost on this computer). This page is on a plain http address, so it cannot read a keyfile here — open it on the computer itself, or use a password.');
