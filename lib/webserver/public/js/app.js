...
// Bring the content index up to date before EVERY inside-files search, so a file you just added is searchable without a
// manual "Update index" — even when you add it while the search window is already open. This is cheap: indexing is
// incremental, so when nothing changed the refresh reads no files and returns almost at once (only genuinely new or
// changed files are read, and only new scans are OCR'd). It streams the same progress bar as the manual button, and it
// is best-effort — a failure (a read-only or wedged mount) is swallowed and the search proceeds against whatever index
// exists, so the refresh can never block or break a search. A concurrent refresh is deduped on the server, so a rapid
// second search never starts a second index run.
async function ensureContentIndexFresh() {
	$('#searchProgress').hidden = false; searchProgress({ indeterminate: true, label: 'Updating index' });
	try { await withBusy(() => apiStream('/api/content-reindex', { path: searchTarget }, searchProgress)); }
