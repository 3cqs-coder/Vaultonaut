...
function updateNotifBadge() {
	...
	// Announce new findings to assistive tech: #integrity sits inside the hidden panel, so mirror the count into
	// this always-present live region. Only write on change so polling does not re-announce the same message.
	const announce = $('#notifAnnounce');
	if (announce) {
		const msg = unseen ? ('System check found ' + n + ' item' + (n === 1 ? '' : 's') + ' to review. Open notifications from the bell.') : '';
...
if ($('#renameSave')) $('#renameSave').addEventListener('click', guarded(async () => {
	const password = $('#renamePassword').value;
