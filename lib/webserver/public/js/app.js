...
async function openServe(path, name) {
	...
	if (v.serving && v.serving.serving) {
		...
		// Reflect the current global Tor bridge setting (a built-in mode selects itself; a custom line, set from the CLI,
		// leaves the select at its neutral position without disturbing the saved setting).
		try { const tb = await api('/api/tor-bridges', {}); if ($('#serveBridge')) $('#serveBridge').value = (tb && ['snowflake', 'obfs4', 'meek'].indexOf(tb.mode) >= 0) ? tb.mode : 'off'; } catch (_) {}
