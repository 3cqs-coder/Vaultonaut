...
// Reflect the integrity status in the notification bell: a badge with the finding count while the integrity section
// (rendered above, and now living inside the bell dropdown) is showing something, and the "all clear" line when it
// is not. The environment banner stays a prominent banner, so it is deliberately not counted here.
function updateNotifBadge() {
	...
	const badge = $('#notifBadge'); if (badge) { badge.hidden = n === 0; badge.textContent = n ? String(n) : ''; }
	const clear = $('#notifClear'); if (clear) clear.hidden = n > 0;
}
// The bell toggles its dropdown; a click outside or Escape closes it. Kept simple and self-contained.
(function () {
