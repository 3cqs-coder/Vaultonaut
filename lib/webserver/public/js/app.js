...
function saveScrubSchedule() {
	...
	if (m === 'daily') { const st = localHmToUtc(3, 0); schedule = { mode: 'daily', hour: st.hour, minute: st.minute, autoHeal: $('#scrubHeal').checked }; } // 3am local, stored as UTC
...
function openBackup(path, name) {
	...
	if (sch.mode === 'daily') { const lt = utcHmToLocal(sch.hour, sch.minute); $('#backupTime').value = pad2(lt.hour) + ':' + pad2(lt.minute); } else { $('#backupTime').value = '02:00'; }
...
$('#backupSave').addEventListener('click', async () => {
	...
	if (f === 'off') schedule = { mode: 'off' };
		...
		const dest = backupDestValue();
		if (!dest) return toast('Choose a destination first', true);
		if (f === 'daily') { const [h, m] = ($('#backupTime').value || '02:00').split(':').map(Number); const ut = localHmToUtc(h, m); schedule = { mode: 'daily', hour: ut.hour, minute: ut.minute, dest }; }
