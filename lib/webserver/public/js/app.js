...
async function refresh() {
	...
	try {
		...
		// exists; a null status leaves the default prompt (or a just-shown manual result) in place.
		if (state.settings && state.settings.updateStatus && $('#updateStatusText') && $('#updateStatusText').textContent !== 'Checking…') renderUpdateStatus(state.settings.updateStatus);
...
async function recoverWedged(target, name, btn) {
	const okd = await uiConfirm({ title: 'Recover wedged drive', message: 'The drive "' + esc(name) + '" is stuck and a normal force could not release it. Recovery will stop its file-server process and release it, usually without a restart. Nothing in the vault is lost. Continue?', confirmLabel: 'Recover', danger: true });
	if (!okd) return 'declined';
	try {
		if (btn) btn.disabled = true;
...
function renderUpdateStatus(st, err) {
	...
	if (st.updateAvailable) {
		out.textContent = 'Version ' + st.latest + ' is available (you have ' + st.current + '). Get it from ' + st.releasesUrl + ' — downloading is manual, and the download is verified before you trust it.';
	} else if (st.ahead) {
