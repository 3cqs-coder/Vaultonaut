...
// Reflect a saved numeric setting into a preset <select> without disturbing a control the user is mid-change,
// and without silently mis-showing a value the select has no option for. If the saved value has no matching
// option (a power-user's custom number set outside these presets), fall back to `fallback` so the control never
// displays a wrong preset as if it were selected. Returns nothing; a missing element is a no-op.
function setSelect(sel, value, fallback) {
	const el = $(sel);
	if (!el || document.activeElement === el) return;
	const want = String(value == null ? fallback : value);
	const has = Array.prototype.some.call(el.options, (o) => o.value === want);
	el.value = has ? want : String(fallback);
}
...
const api = async (path, body) => {
	...
	if (!data.ok) {
		...
		throw Object.assign(new Error(data.error || 'Something went wrong — please try again.'), { install: data.install, locked: data.locked, holder: data.holder, flushing: data.flushing, busy: data.busy });
	}
	return data;
};

// Upload a file's raw bytes to an API endpoint (metadata rides in the query string). Same X-Vdisk CSRF header as
// api(); the octet-stream content-type keeps the file out of the JSON body parser and preserves binary (a .1pux ZIP).
const apiUpload = async (path, bytes, query) => {
	const qs = new URLSearchParams(query || {}).toString();
	const res = await fetch(path + (qs ? '?' + qs : ''), { method: 'POST', headers: { 'X-Vdisk': '1', 'Content-Type': 'application/octet-stream' }, body: bytes });
	const data = await res.json().catch(() => ({ ok: false, error: 'Bad response' }));
	if (data && data.login) { location.href = '/login'; throw new Error('Session expired'); }
...
const apiStream = async (path, body, onProgress) => {
	...
	if (!res.ok || !res.body) { // a non-streaming error response (e.g. blocked/forbidden) — surface it
		let msg = 'Something went wrong — please try again. (Error ' + res.status + ')'; // fallback only when the server sent no curated message; keep the code for support
...
let createPanelDefaulted = false; // the create panel's open/closed default is set once, on the first authoritative render (see renderVaults)
...
function toast(msg, isError, opts = {}) {
	if (undercoverActive) return; // the disguise must show nothing — suppress every status popover while it is up
	const t = $('#toast');
	...
	t.textContent = msg;
	t.title = 'Click or press Escape to dismiss';
	if (!toast._wired) {
		t.addEventListener('click', hideToast); // click anywhere on it to dismiss
		// Keyboard/switch users need a non-mouse way out, especially for a STICKY toast that stays until dismissed
		// (e.g. a shown-once recovery key). Escape, Enter, or Space clears it when it holds focus.
		t.addEventListener('keydown', (ev) => { if (ev.key === 'Escape' || ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); hideToast(); } });
		toast._wired = true;
	}
	...
	if (!opts.sticky) {
		...
		// A sticky toast stays until dismissed, so move focus to it — a keyboard/switch user can then press Escape,
		// Enter, or Space to clear it. (An auto-expiring toast never grabs focus, so typing is not interrupted.)
		try { setTimeout(() => { if (!t.hidden) t.focus(); }, 0); } catch (_) {}
	}
}
...
// HTML-escape from the shared sanitizer (public/shared/safe-text.js), loaded before this script — one copy for
// every client, so the desktop and mobile bundles can never drift to different escaping.
const esc = window.VaultSafe.escapeHtml;
...
function initTheme() {
	let saved = 'auto';
	try { saved = localStorage.getItem('vdisk-theme') || 'auto'; } catch (_) {} // private mode or blocked storage — use the default
	document.documentElement.setAttribute('data-theme', saved);
	// Keep the browser-chrome color (the phone address/status bar when this desktop interface is opened from a phone on
	// the network) in step with the resolved background. Reads the computed --bg token, so it never drifts from the
	// theme. No-op in the desktop app's own window, which has no browser chrome.
	const applyThemeColor = () => { try { const bg = getComputedStyle(document.body).backgroundColor, meta = document.querySelector('meta[name="theme-color"]'); if (bg && meta) meta.setAttribute('content', bg); } catch (e) {} };
	applyThemeColor();
	$('#themeToggle').addEventListener('click', () => {
		...
		try { localStorage.setItem('vdisk-theme', next); } catch (_) {} // persist is best-effort; a blocked store must not break the toggle
		applyThemeColor();
		const label = { auto: 'Theme: match system', light: 'Theme: light', dark: 'Theme: dark', sepia: 'Theme: sepia (calm)' };
		toast(label[next] || 'Theme: ' + next);
	});
	// Keep the browser-chrome color in step if the system flips light/dark while the page is open in "auto" mode. This
	// matters when a phone on the network views this interface: the body recolors via the CSS media query, but the
	// address/status-bar tint reads the theme-color meta, which otherwise stays stale until the theme button is cycled.
	try { window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', applyThemeColor); } catch (_) {}
...
// install shows nothing. Each finding pairs a plain-language message with how to fix it. The findings now live in
// the notification-bell dropdown, so there is no minimize/collapse — the dropdown itself keeps them out of the way.
let lastSelfCheck = null; // the latest self-check payload, so the notification badge can recount
...
function renderIntegrity(sc) {
	...
	if (!findings.length) { if (renderIntegrity._key !== 'empty') { el.hidden = true; el.innerHTML = ''; renderIntegrity._key = 'empty'; } return; }
	// This now lives in the notification dropdown, so there is no collapse/minimize (that made sense only for a
	// full-width banner). The list is always shown; the dropdown itself is the "minimize". Severity is carried by the
	// level class (a neutral card for a routine check, a red edge for an error) — never a brown fill.
	const level = findings.some(f => f.level === 'error') ? 'bad' : 'warn';
	const sig = integritySig(findings);
	// aria-live region: rewrite only when the findings actually change, so the 5-second poll does not make a screen
	// reader re-read the whole list every tick.
	const key = level + '|' + sig;
	...
	el.innerHTML = '<div class="banner-row"><span class="banner-msg">System check found ' + count + ' to look at:</span></div>'
		+ '<ul class="integrity-list">' + items + '</ul>';
}

// The at-a-glance dashboard. All logic that turns state into tiles lives in the shared, unit-tested model
// (shared/dashboard-model.js); this only renders the tiles it returns. It follows the same quiet-render rule as the
// other renderers: build the HTML, compare a signature, and touch the DOM only when the tiles actually change, so the
// 5-second poll does not thrash the panel or a screen reader. The model reads only fields safe to show and never a
// size or file count for a locked vault, so this cannot leak locked-vault contents.
var DASH_OPEN_KEY = 'vdisk-dashboard-open';
// A dependency-free inline-SVG sparkline: a filled area + line over the series, on a fixed `max` scale (100 for a
// percentage, so the shape is meaningful without renormalizing each poll). preserveAspectRatio="none" lets it stretch
// to the tile width; colors come from CSS tokens. No user data, so the markup is safe to inject as-is.
function sparkSvg(vals, max) {
	if (!vals || vals.length < 2) return '';
	var W = 100, H = 26, n = vals.length, m = max || Math.max.apply(null, vals) || 1;
	var pts = [];
	for (var i = 0; i < n; i++) { var x = (i / (n - 1)) * W; var y = H - (Math.min(Number(vals[i]) || 0, m) / m) * (H - 2) - 1; pts.push(x.toFixed(1) + ',' + y.toFixed(1)); }
	var line = pts.join(' ');
	return '<svg class="dspark" viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="none" aria-hidden="true">'
		+ '<polygon class="dspark-area" points="0,' + H + ' ' + line + ' ' + W + ',' + H + '"/>'
		+ '<polyline class="dspark-line" points="' + line + '"/></svg>';
}
function renderDashboard(state) {
	var host = $('#dashboard'), panel = $('#dashboardPanel');
	if (!host || !panel || !window.VaultDashboard) return;
	var tiles = window.VaultDashboard.computeDashboard(state, fmtBytes);
	// The fleet tile is computed by its own model and slotted in after connectivity, so the two models stay decoupled
	// and the fleet only appears once a node is enrolled. Its severity still feeds the overall verdict below.
	if (window.VaultFleet) {
		var ft = window.VaultFleet.fleetTile(state);
		if (ft) { var ci = -1; for (var i = 0; i < tiles.length; i++) if (tiles[i].id === 'connectivity') ci = i; if (ci >= 0) tiles.splice(ci + 1, 0, ft); else tiles.push(ft); }
	}
	updateStatusPill(window.VaultDashboard.overall(tiles));
	var sig = tiles.map(function (t) { return t.id + ':' + t.level + ':' + t.value + ':' + t.detail + ':' + (t.action || ''); }).join('|');
	if (renderDashboard._sig === sig) { if (panel.hidden) panel.hidden = false; return; }
	renderDashboard._sig = sig;
	host.innerHTML = tiles.map(function (t) {
		var inner = '<span class="dtile-title">' + esc(t.title) + '</span>'
			+ '<span class="dtile-value">' + esc(t.value) + '</span>'
			+ (t.detail ? '<span class="dtile-detail">' + esc(t.detail) + '</span>' : '')
			+ (t.spark && t.spark.heap ? sparkSvg(t.spark.heap, 100) : ''); // heap% trend on a fixed 0–100 scale
		// An actionable tile is a real <button> (native keyboard + focus), a static one a plain card. The label spells out
		// where the button goes so it is clear to a screen reader, not conveyed by color alone.
		if (t.action) return '<button type="button" class="dtile dtile-' + t.level + ' dtile-action" data-action="' + esc(t.action) + '" aria-label="' + esc(t.title + ': ' + t.value + (t.detail ? '. ' + t.detail : '')) + '">' + inner + '</button>';
		return '<div class="dtile dtile-' + t.level + '">' + inner + '</div>';
	}).join('');
	panel.hidden = false; // reveal on the first fill so it never flashes an empty frame
}
// Where each tile's action goes — a jump into the existing area that sets up or manages that subsystem, so the overview
// is a launchpad rather than a dead readout. One delegated listener (added once in initDashboardPanel) covers every
// tile across re-renders. Every target reuses an existing control, so this adds no new management surface of its own.
function dashboardAction(key) {
	if (key === 'notifications') { var nb = $('#notifBtn'); if (nb) nb.click(); return; } // open the system-check panel
	if (key === 'vaults') { var vs = $('#vaults'); if (vs) { vs.scrollIntoView({ behavior: 'smooth', block: 'start' }); var first = vs.querySelector('.card'); if (first && first.focus) try { first.focus(); } catch (e) {} } return; }
	if (key === 'engine') { var env = $('#env'); if (env && !env.hidden) { env.scrollIntoView({ behavior: 'smooth', block: 'start' }); var b = env.querySelector('button'); if (b && b.focus) try { b.focus(); } catch (e) {} } return; }
	if (key === 'update') { var u = lastState && lastState.settings && lastState.settings.updateStatus; var url = u && u.releasesUrl; if (url) window.open(url, '_blank', 'noopener'); return; }
	if (key === 'devices') { openDevices(); return; }
	if (key === 'fleet') { openFleet(); return; }
}
// The header status pill mirrors the dashboard's overall verdict, so the one-line state of the whole install is visible
// even with the Overview collapsed. Quiet-render: only rewritten when the level changes, so the aria-live pill does not
// re-announce on every poll. Clicking it opens the Overview.
var STATUS_PILL_TEXT = { ok: 'All systems normal', warn: 'Needs a look', error: 'Attention needed' };
function updateStatusPill(level) {
	var pill = $('#statusPill'); if (!pill) return;
	if (updateStatusPill._level === level) { if (pill.hidden) pill.hidden = false; return; }
	updateStatusPill._level = level;
	pill.className = 'status-pill status-' + level;
	var txt = pill.querySelector('.status-text'); if (txt) txt.textContent = STATUS_PILL_TEXT[level] || STATUS_PILL_TEXT.ok;
	pill.hidden = false;
}

// ---- Devices & connections ----
// One place for every paired device, cloud/backup destination, and served vault, with an on-demand reachability check.
// All list/verdict/concurrency logic lives in the shared, unit-tested model (shared/devices-model.js); this renders it
// and drives the probes. Reachability is NEVER on the poll — only when the user asks — so it adds no background load.
var DEVICE_KIND_LABEL = { peer: 'Paired device', cloud: 'Cloud storage', sftp: 'Backup destination', serve: 'Serving' };
var DEVICE_TEST_PATH = { peer: '/api/peer-test', cloud: '/api/cloud-test', sftp: '/api/sftp-test' };
// A reachability probe reads the endpoint's own {ok,...} body (which may be ok:false) — so, unlike api(), it must NOT
// throw on a not-ok result. It returns the parsed body for the model to interpret, and a soft failure on any error.
function probeApi(path, body) {
	var opts = { headers: { 'X-Vdisk': '1' } };
	if (body) { opts.method = 'POST'; opts.headers['Content-Type'] = 'application/json'; opts.body = JSON.stringify(body); }
	return fetch(path, opts).then(function (r) { return r.json(); }).catch(function (e) { return { ok: false, error: (e && e.message) || 'Network error' }; });
}
function relTime(ms) {
	var s = Math.max(0, Math.round((Date.now() - ms) / 1000));
	if (s < 45) return 'just now';
	var m = Math.round(s / 60); if (m < 60) return m + (m === 1 ? ' min ago' : ' mins ago');
...
function deviceRowHtml(d) {
	var actions = '';
	if (d.testable) actions += '<button type="button" class="ghost-btn dev-test" data-test>Test</button>';
	if (d.removable) actions += '<button type="button" class="link-btn danger dev-remove" data-remove>Remove</button>';
	return '<div class="dev-row" data-kind="' + esc(d.kind) + '" data-id="' + esc(String(d.id)) + '">'
		+ '<span class="dev-dot dev-dot-' + (d.live ? 'live' : 'unknown') + '" data-devdot title="' + (d.live ? 'Serving' : 'Not checked yet') + '"></span>'
		+ '<div class="dev-main"><div class="dev-label">' + esc(d.label) + '</div>'
		+ '<div class="dev-sub">' + esc(DEVICE_KIND_LABEL[d.kind] || d.kind) + (d.sub ? ' · ' + esc(d.sub) : '') + '</div>'
		+ '<div class="dev-note" data-note></div></div>'
		+ '<div class="dev-actions">' + actions + '</div></div>';
}
function renderDevices() {
	var state = lastState || {};
	var list = window.VaultDevices.deviceList(state);
	var host = $('#devicesList'); if (!host) return;
	host.innerHTML = list.map(deviceRowHtml).join('');
	var empty = $('#devicesEmpty'); if (empty) empty.hidden = list.length > 0;
	var check = $('#devicesCheckAll'); if (check) check.hidden = !list.some(function (d) { return d.testable; });
	var act = window.VaultDevices.recentActivity(state);
	var ah = $('#devicesActivity');
	if (ah) ah.innerHTML = act.length
		? act.map(function (a) { return '<div class="dev-evt dev-evt-' + esc(a.level) + '"><span class="dev-evt-text">' + esc(a.text) + '</span><span class="dev-evt-when">' + esc(relTime(a.atMs)) + '</span></div>'; }).join('')
		: '<div class="muted">No recent activity.</div>';
}
function testDeviceRow(row) {
	var kind = row.getAttribute('data-kind'), id = row.getAttribute('data-id');
	var dot = row.querySelector('[data-devdot]'); if (dot) dot.className = 'dev-dot dev-dot-testing';
	var body = kind === 'cloud' ? { id: id, remotePath: '' } : { id: id };
	return probeApi(DEVICE_TEST_PATH[kind], body).then(function (res) {
		var v = window.VaultDevices.reachabilityFrom(kind, res);
		if (dot) dot.className = 'dev-dot dev-dot-' + (v.reachable ? 'ok' : 'bad');
		var n = row.querySelector('[data-note]'); if (n) n.textContent = v.note || '';
		return v;
	});
}
function removeDeviceRow(row) {
	var id = row.getAttribute('data-id');
	uiConfirm({ title: 'Remove device', message: 'Stop trusting this paired device? You can pair it again later with a fresh code.', confirmLabel: 'Remove', danger: true }).then(function (yes) {
		if (!yes) return;
		withBusy(function () { return api('/api/peer-remove', { id: id }); }).then(function () { toast('Removed'); renderDevices(); refresh(); }).catch(function (e) { toast((e && e.message) || 'Could not remove', true); });
	});
}
function openDevices() {
	var d = $('#devicesDialog'); if (!d) return;
	renderDevices();
	var msg = $('#devicesCheckMsg'); if (msg) msg.textContent = '';
	if (d.showModal) d.showModal();
}
function initDevices() {
	var check = $('#devicesCheckAll');
	if (check) check.addEventListener('click', guarded(function () {
		var rows = Array.prototype.slice.call(document.querySelectorAll('#devicesList .dev-row')).filter(function (r) { return r.querySelector('[data-test]'); });
		if (!rows.length) return Promise.resolve();
		var msg = $('#devicesCheckMsg'); if (msg) msg.textContent = 'Checking ' + rows.length + '…';
		rows.forEach(function (r) { var dot = r.querySelector('[data-devdot]'); if (dot) dot.className = 'dev-dot dev-dot-testing'; });
		// Bounded fan-out (4 at a time) so many connections never open an unbounded burst of engine probes.
		return window.VaultDevices.mapLimit(rows, 4, function (row) { return testDeviceRow(row); }).then(function (results) {
			var okCount = results.filter(function (v) { return v && v.reachable; }).length;
			if (msg) msg.textContent = okCount + ' of ' + rows.length + ' reachable';
		});
	}));
	var listEl = $('#devicesList');
	if (listEl) listEl.addEventListener('click', function (e) {
		var row = e.target.closest && e.target.closest('.dev-row'); if (!row) return;
		if (e.target.closest('[data-test]')) { guarded(function () { return testDeviceRow(row); })(); return; }
		if (e.target.closest('[data-remove]')) { removeDeviceRow(row); }
	});
}

// ---- Fleet (multi-node hub) ----
// Watch other Vaultonaut nodes from one place. The server polls each node's /health in the background and puts the
// cached snapshot in state.fleet; this only renders it and drives add/remove. All list/rollup logic is in the shared,
// unit-tested fleet model. Enrolling a node reaches out once to verify it before saving (server-side).
var FLEET_DOT = { online: 'ok', degraded: 'bad', unreachable: 'warn', unknown: 'unknown' };
function fleetRowHtml(n) {
	return '<div class="dev-row" data-id="' + esc(String(n.id)) + '">'
		+ '<span class="dev-dot dev-dot-' + (FLEET_DOT[n.status] || 'unknown') + '" title="' + esc(n.status) + '"></span>'
		+ '<div class="dev-main"><div class="dev-label">' + esc(n.label) + '</div>'
		+ '<div class="dev-sub">' + esc(n.sub) + '</div>'
		+ '<div class="dev-note">' + esc(n.detail) + '</div></div>'
		+ '<div class="dev-actions">'
		+ '<button type="button" class="ghost-btn fleet-act" data-fleet-action="refresh" title="Re-run this node\'s health check">Refresh</button>'
		+ '<button type="button" class="ghost-btn fleet-act" data-fleet-action="lock-all" title="Lock (unmount) every vault on this node now">Lock</button>'
		+ '<button type="button" class="link-btn danger fleet-remove" data-remove>Remove</button></div></div>';
}
function renderFleet() {
	var list = (window.VaultFleet ? window.VaultFleet.nodeList(lastState || {}) : []);
	var host = $('#fleetList'); if (!host) return;
	host.innerHTML = list.map(fleetRowHtml).join('');
	var empty = $('#fleetEmpty'); if (empty) empty.hidden = list.length > 0;
}
function openFleet() {
	var d = $('#fleetDialog'); if (!d) return;
	renderFleet();
	var msg = $('#fleetAddMsg'); if (msg) msg.textContent = '';
	if (d.showModal) d.showModal();
}
function initFleet() {
	var add = $('#fleetAdd');
	if (add) add.addEventListener('click', guarded(function () {
		var label = ($('#fleetLabel') || {}).value, url = ($('#fleetUrl') || {}).value, token = ($('#fleetToken') || {}).value;
		var msg = $('#fleetAddMsg');
		if (!url || !token) { if (msg) msg.textContent = 'Enter the node address and a token.'; return Promise.resolve(); }
		if (msg) msg.textContent = 'Checking…';
		return withBusy(function () { return api('/api/fleet-add', { label: label, url: url, token: token }); }).then(function () {
			if (msg) msg.textContent = 'Added.';
			var l = $('#fleetLabel'), u = $('#fleetUrl'), t = $('#fleetToken'); if (l) l.value = ''; if (u) u.value = ''; if (t) t.value = '';
			refresh(); setTimeout(renderFleet, 300);
		}).catch(function (e) { if (msg) msg.textContent = (e && e.message) || 'Could not add that node.'; });
	}));
	var listEl = $('#fleetList');
	if (listEl) listEl.addEventListener('click', function (e) {
		var row = e.target.closest && e.target.closest('.dev-row'); if (!row) return;
		var id = row.getAttribute('data-id');
		// A safe cross-node action (refresh its health, or lock it). Lock asks first — it unmounts the node's vaults.
		var actBtn = e.target.closest && e.target.closest('[data-fleet-action]');
		if (actBtn) {
			var action = actBtn.getAttribute('data-fleet-action');
			var run = function () { withBusy(function () { return api('/api/fleet-action', { id: id, action: action }); }).then(function () { toast(action === 'lock-all' ? 'Locked that node' : 'Refreshed'); refresh(); setTimeout(renderFleet, 300); }).catch(function (err) { toast((err && err.message) || 'The action did not complete', true); }); };
			if (action === 'lock-all') { uiConfirm({ title: 'Lock node', message: 'Lock (unmount) every vault on this node now? It flushes first; a vault with a file open stays mounted.', confirmLabel: 'Lock node', danger: true }).then(function (yes) { if (yes) run(); }); }
			else { run(); }
			return;
		}
		if (!e.target.closest('[data-remove]')) return;
		uiConfirm({ title: 'Remove node', message: 'Stop watching this node? It keeps running and is unaffected; you can add it again anytime.', confirmLabel: 'Remove', danger: true }).then(function (yes) {
			if (!yes) return;
			withBusy(function () { return api('/api/fleet-remove', { id: id }); }).then(function () { toast('Removed'); renderFleet(); refresh(); }).catch(function (err) { toast((err && err.message) || 'Could not remove', true); });
		});
	});
}

// Remember whether the overview is expanded, per browser. Wrapped in try/catch so a private window or blocked storage
// never breaks the page; the panel just defaults to open.
function initDashboardPanel() {
	var panel = $('#dashboardPanel'); if (!panel) return;
	var open = true; try { var v = localStorage.getItem(DASH_OPEN_KEY); if (v === '0') open = false; } catch (e) {}
	panel.open = open;
	panel.addEventListener('toggle', function () { try { localStorage.setItem(DASH_OPEN_KEY, panel.open ? '1' : '0'); } catch (e) {} });
	// One delegated click handler for every tile action, so re-renders never need to rebind.
	var host = $('#dashboard');
	if (host) host.addEventListener('click', function (e) { var b = e.target.closest && e.target.closest('.dtile-action'); if (b) dashboardAction(b.getAttribute('data-action')); });
	// The header status pill opens (and scrolls to) the Overview.
	var pill = $('#statusPill');
	if (pill) pill.addEventListener('click', function () { panel.open = true; try { panel.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) {} });
}

// Reflect the integrity status in the notification bell. The panel always lists the current findings (the integrity
// section, rendered above, lives inside the dropdown), and the "all clear" line shows when there are none. The BADGE,
// though, is an "unseen" marker: it shows the count only while the current finding-set has not been dismissed. Opening
// the bell dismisses it (marks the current set seen), so the badge clears once you have looked — and returns on its own
// if the findings later CHANGE (a new signature). The environment banner is separate and deliberately not counted.
const NOTIF_SEEN_KEY = 'vdisk-notif-seen';
function currentNotifSig() {
	const integrityEl = $('#integrity');
	if (!integrityEl || integrityEl.hidden) return '';
	const findings = ((lastSelfCheck && lastSelfCheck.findings) || []).filter(f => f.check !== 'engine' && f.check !== 'driver');
	return findings.length ? integritySig(findings) : '';
}
function updateNotifBadge() {
	const integrityEl = $('#integrity');
	const showing = integrityEl && !integrityEl.hidden;
	const findings = ((lastSelfCheck && lastSelfCheck.findings) || []).filter(f => f.check !== 'engine' && f.check !== 'driver');
	const n = showing ? findings.length : 0;
	const sig = n ? integritySig(findings) : '';
	let seen = ''; try { seen = localStorage.getItem(NOTIF_SEEN_KEY) || ''; } catch (_) {}
	const unseen = n > 0 && sig !== seen;
	const badge = $('#notifBadge'); if (badge) { badge.hidden = !unseen; badge.textContent = unseen ? String(n) : ''; }
	const clear = $('#notifClear'); if (clear) clear.hidden = n > 0;
	// Announce new findings to assistive tech: #integrity sits inside the panel, so while the panel is closed it is out
	// of the a11y tree and never spoken. Mirror the count into this always-present live region instead — but only while
	// the panel is closed, so an open panel (whose #integrity announces the detail) is not read out twice. Write only on
	// change so polling does not re-announce the same message.
	const panel = $('#notifPanel');
	const panelOpen = !!(panel && !panel.hidden);
	const announce = $('#notifAnnounce');
	if (announce) {
		const msg = (unseen && !panelOpen) ? ('System check found ' + n + ' item' + (n === 1 ? '' : 's') + ' to review. Open notifications from the bell.') : '';
		if (announce.textContent !== msg) announce.textContent = msg;
	}
}
// Dismiss the current findings: remember their signature so the badge stays clear until the findings change.
function markNotifSeen() { try { localStorage.setItem(NOTIF_SEEN_KEY, currentNotifSig()); } catch (_) {} updateNotifBadge(); }
// The bell toggles its dropdown; a click outside or Escape closes it. Opening it dismisses the badge (you've looked).
(function () {
	const btn = $('#notifBtn'), panel = $('#notifPanel');
	if (!btn || !panel) return;
	const setOpen = (open) => { panel.hidden = !open; btn.setAttribute('aria-expanded', String(open)); if (open) markNotifSeen(); };
	...
	// Escape closes the dropdown and returns focus to the bell, so a keyboard user is not dropped onto <body>.
	document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && !panel.hidden) { setOpen(false); btn.focus(); } });
})();

// The small self-heal badge on a vault's name: a live "updating" state while recovery data is being rebuilt in the
// background; a brief "just updated" confirmation right after; otherwise a quiet marker that protection is present. The
// recovery index is rewritten on every refresh, so its createdAt is the last-updated time. Showing a short "just
// updated" window makes the signal durable: the live "updating" pill is easy to miss because the list polls every few
// seconds and only while the window is focused, so a background refresh often finishes before the user looks back.
function recoveryBadge(v) {
	const r = v.recovery;
	if (!r) return '';
	if (r.refreshing) return ` <span class="badge updating" title="Rebuilding recovery data after your changes">updating ${r.refreshPercent != null ? Number(r.refreshPercent) + '%' : '…'}</span>`;
	if (r.protected) {
		const when = r.createdAt ? Date.parse(r.createdAt) : NaN;
		const title = isFinite(when) ? 'Self-healing recovery data last updated ' + new Date(when).toLocaleString() : 'Self-healing recovery data is present';
		const justUpdated = isFinite(when) && (Date.now() - when) < 120000; // 2 minutes — long enough to survive the poll/focus gap
		return justUpdated
			? ` <span class="badge protected" title="${esc(title)}">self-heal ✓ updated</span>`
			: ` <span class="badge protected" title="${esc(title)}">self-heal</span>`;
	}
	return '';
}
...
// The vault's LIVE status badges — the ones with a moving state a background task drives: the self-heal "updating"
// pill, the mirror "syncing" pill, and the serving marker. Grouped so they can be refreshed in place (see renderVaults)
// even while a "More" menu or a dialog is open and the full card list must not be rebuilt — otherwise opening More
// would freeze the very self-heal signal the user opened it to act on.
function liveBadgesHtml(v) { return recoveryBadge(v) + mirrorBadge(v) + servingBadge(v); }
// ---- Manage surface: the grouped home for every advanced per-vault action (replaces the old "More" dropdown) ----
// Each row carries the same data-act as before, so the existing handlers run unchanged. The same gating the old menu
// used is applied here: an action tagged { needsUnmount } is shown DISABLED with an "unmount first" hint while the
// vault is mounted (the delegated handler turns a click into that hint; the server is the real guard via
// assertUnmounted), and a { needsLocal } action is omitted for a cloud vault (its data lives at the provider).
const MANAGE_ICON = {
	rename: '<path d="M20.6 13.4 12 22l-9-9V4a1 1 0 0 1 1-1h8l8.6 8.6a1 1 0 0 1 0 1.4Z"/><circle cx="7.5" cy="7.5" r="1.5"/>',
	keys: '<circle cx="8" cy="15" r="3"/><path d="M10 13l6-6 3 3-2 2"/>',
	passwd: '<rect x="4" y="10" width="16" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/>',
	consent: '<path d="M12 3l7 3v5c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3Z"/><path d="m9 12 2 2 4-4"/>',
	members: '<circle cx="9" cy="8" r="3"/><path d="M3 20c0-3 2.7-5 6-5s6 2 6 5"/><path d="M16 8a3 3 0 0 1 0 6"/>',
	tamper: '<path d="M12 3l7 3v5c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3Z"/><path d="m9 12 2 2 4-4"/>',
	protect: '<path d="M20 12a8 8 0 1 1-2.3-5.6M20 4v4h-4"/>',
	share: '<circle cx="7" cy="12" r="2.5"/><circle cx="17" cy="6" r="2.5"/><circle cx="17" cy="18" r="2.5"/><path d="M9.2 11 15 7M9.2 13 15 17"/>',
	emergency: '<circle cx="12" cy="12" r="9"/><path d="M12 8v4l3 2"/>',
	serve: '<rect x="3" y="4" width="18" height="7" rx="2"/><rect x="3" y="13" width="18" height="7" rx="2"/><path d="M7 7.5h.01M7 16.5h.01"/>',
	mirror: '<path d="M4 8h12l-3-3M20 16H8l3 3"/>',
	'move-to-node': '<rect x="3" y="4" width="7" height="16" rx="1.5"/><rect x="14" y="4" width="7" height="16" rx="1.5"/><path d="M10 12h4M13 10l2 2-2 2"/>',
	mobile: '<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>',
	backup: '<path d="M12 3v12M8 11l4 4 4-4"/><path d="M5 21h14"/>',
	versions: '<path d="M3 12a9 9 0 1 0 3-6.7M3 4v4h4"/><path d="M12 8v4l3 2"/>',
	disperse: '<circle cx="6" cy="6" r="2.5"/><circle cx="18" cy="6" r="2.5"/><circle cx="12" cy="18" r="2.5"/><path d="M7.6 7.6 11 15M16.4 7.6 13 15"/>',
	remove: '<path d="M5 7h14M9 7V5h6v2M6 7l1 13h10l1-13"/>',
	destroy: '<path d="M10.3 3.9 2.4 18a2 2 0 0 0 1.7 3h15.8a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"/><path d="M12 9v4M12 17h.01"/>'
};
// [act, label, description, opts] — opts mirrors the old moreBtn gating, plus notServing (hide "Serve" while serving).
const MANAGE_GROUPS = [
	['Security', [
		['rename', 'Name & description', 'Set the vault’s encrypted name and description', {}],
		['keys', 'Keys & rotation', 'Rotate encryption keys, manage key slots', {}],
		['passwd', 'Change password', 'Update the vault password', {}],
		['consent', 'Require approval on unlock', 'Ask before each unlock and record it in the ledger', {}],
		['members', 'Members', 'Who can open this shared vault', { needsUnmount: true }],
		['tamper', 'Tamper check', 'Verify nothing has been altered', { needsUnmount: true }],
		['protect', 'Self-healing', 'Auto-repair from built-in redundancy', { needsUnmount: true, needsLocal: true }]
	]],
	['Sharing & access', [
		['share', 'Share with a group', 'Give people read or write access', {}],
		['emergency', 'Emergency access', 'A trusted contact after inactivity', { needsUnmount: true, needsLocal: true }],
		['serve', 'Serve as a node', 'Offer this vault to your other devices', { needsUnmount: true, needsLocal: true, notServing: true }],
		['move-to-node', 'Move to another node', 'Copy it to a fleet node and retire it here', { needsUnmount: true, needsLocal: true, needsFleet: true }],
		['mirror', 'Mirror', 'Two-way sync to another location', { needsUnmount: true, needsLocal: true }],
		['mobile', 'View files & notes', 'Read the contents without mounting', { needsLocal: true }]
	]],
	['Storage & backups', [
		['backup', 'Back up', 'Copy the encrypted store off-site', { needsUnmount: true, needsLocal: true }],
		['versions', 'Version history', 'Restore an earlier snapshot', { needsUnmount: true }],
		['disperse', 'Split across places', 'Shard across multiple locations', { needsUnmount: true, needsLocal: true }]
	]],
	['Danger zone', [
		['remove', 'Remove from list', 'Forget it here — your files are kept', {}],
		['destroy', 'Delete permanently', 'Erase the keys and data for good', { needsUnmount: true, danger: true }]
	]]
];
function manageRow(v, spec) {
	let [act, label, desc, opts] = spec;
	// The consent toggle shows its current state in the row (it flips on tap rather than opening a dialog).
	if (act === 'consent') desc = v.consentOnUse ? 'On — every unlock asks for approval and is recorded. Tap to turn off.' : 'Off — tap to require approval on each unlock and record it in the ledger.';
	if (opts.needsLocal && v.cloud) return '';                          // local-only action: never valid for a cloud vault
	if (opts.notServing && v.serving && v.serving.serving) return '';   // "Serve" is hidden while the vault is already serving
	if (opts.needsFleet && !(lastState && lastState.fleet && lastState.fleet.length)) return ''; // "Move to a node" only shows once a fleet node is enrolled
	const blocked = !!opts.needsUnmount && v.mounted;
	const cls = 'arow' + (opts.danger ? ' danger' : '') + (blocked ? ' disabled' : '');
	const extra = blocked ? ' data-blocked="unmount" aria-disabled="true" title="Unmount this vault first to use this"' : '';
	return `<button type="button" class="${cls}" data-act="${act}" data-path="${esc(v.path)}" data-name="${esc(v.name)}"${extra}>`
		+ `<span class="ai"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">${MANAGE_ICON[act] || ''}</svg></span>`
		+ `<span class="arow-t"><b>${esc(label)}</b><span class="arow-d">${esc(desc)}</span></span>`
		+ `<svg class="chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 6 6 6-6 6"/></svg></button>`;
}
function openManage(path, name) {
	const v = vaultsByPath[path]; if (!v) return;
	const nameEl = $('#manageVaultName'); if (nameEl) nameEl.textContent = name || v.name || '';
	const wrap = $('#manageGroups');
	if (wrap) {
		wrap.innerHTML = MANAGE_GROUPS.map(([title, rows]) => {
			const html = rows.map((r) => manageRow(v, r)).filter(Boolean).join('');
			if (!html) return '';
			const danger = title === 'Danger zone' ? ' danger' : '';
			return `<div class="manage-group${danger}"><div class="manage-group-label">${esc(title)}</div><div class="alist">${html}</div></div>`;
		}).join('');
	}
	const d = $('#manageDialog'); if (d && d.showModal) d.showModal();
}
// The card's leading identity icon: a lock for a local vault (its ciphertext lives on this machine) and a cloud
// for a cloud vault (its ciphertext lives at the provider). Purely a visual cue for where the data sits — the
// authoritative local/cloud handling is elsewhere (v.cloud drives which actions are offered). Marked aria-hidden
// because the same distinction is already in the text ("Cloud" path / the cloud-only action set), so a screen
// reader is not read a redundant glyph. Inline SVG (currentColor) so it themes for free and loads nothing.
const VAULT_ICON_LOCAL = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="10" width="16" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>';
const VAULT_ICON_CLOUD = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M7 18a4 4 0 0 1-.5-8A6 6 0 0 1 18 9.5a3.5 3.5 0 0 1-.5 8.5Z"/><path d="M12 12v3.5M10.3 13.7 12 12l1.7 1.7"/></svg>';
function vaultIcon(v) {
	return `<div class="vault-icon ${v.cloud ? 'cloud' : 'local'}" aria-hidden="true">${v.cloud ? VAULT_ICON_CLOUD : VAULT_ICON_LOCAL}</div>`;
}
// Small inline icons for the per-card path tools (currentColor, so they theme for free and load nothing).
const COPY_ICON = '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg>';
const EYE_ICON = '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>';
const EYE_OFF_ICON = '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 3l18 18M10.6 10.6a3 3 0 0 0 4.2 4.2M9.9 5.2A9.8 9.8 0 0 1 12 5c6.5 0 10 7 10 7a17 17 0 0 1-3 3.7M6.1 6.1A17 17 0 0 0 2 12s3.5 7 10 7a9.7 9.7 0 0 0 3.9-.8"/></svg>';
// Privacy screen: when on, vault names and paths render blurred until the user reveals a card. `revealedPaths` holds
// the paths the user has deliberately shown; it is cleared when the window loses focus so a walk-away re-hides them.
let privacyScreen = false;
let undercoverActive = false; // true while the full-screen disguise is up; suppresses status popovers so nothing peeks through
const revealedPaths = new Set();
// The per-vault color tag picker: a small dot showing the current color that opens a swatch popover. A color is an
// at-a-glance trust / sensitivity signal (assign red to your most sensitive vault, and so on). The names are a fixed
// palette mapped to theme tokens in CSS, so a color is never a raw value and always fits every theme. Keyboard-native
// (real buttons); the popover closes on a click elsewhere (wired once in initColorPickers).
var VAULT_COLOR_NAMES = ['red', 'amber', 'green', 'blue', 'purple', 'slate'];
function colorPicker(v) {
	var cur = v.color || 'none';
	var swatches = VAULT_COLOR_NAMES.concat(['none']).map(function (c) {
		var on = (v.color === c || (!v.color && c === 'none')) ? ' on' : '';
		return '<button type="button" class="color-sw color-sw-' + c + on + '" data-act="set-color" data-color="' + c + '" data-path="' + esc(v.path) + '" title="' + c + '" aria-label="' + (c === 'none' ? 'No color' : c) + '"></button>';
	}).join('');
	return '<span class="color-wrap"><button type="button" class="color-btn" data-act="color-toggle" title="Set a color tag" aria-label="Set a color tag" aria-haspopup="true" aria-expanded="false"><span class="color-dot color-dot-' + esc(cur) + '"></span></button>'
		+ '<span class="color-pop" data-color-pop hidden>' + swatches + '</span></span>';
}
function initColorPickers() {
	// Close any open color popover on a click that is not inside one (or on its toggle). Wired once.
	document.addEventListener('click', function (e) {
		var inWrap = e.target.closest && e.target.closest('.color-wrap');
		document.querySelectorAll('.color-pop:not([hidden])').forEach(function (p) {
			if (!inWrap || !p.parentNode.contains(e.target) || e.target.closest('[data-act="set-color"]')) { p.hidden = true; var btn = p.parentNode.querySelector('.color-btn'); if (btn) btn.setAttribute('aria-expanded', 'false'); }
		});
	}, true);
}
function vaultCard(v) {
	...
	const hidden = privacyScreen && !revealedPaths.has(v.path); // blur this card's name + path until the user reveals it
	...
	// Two tiers keep the card uncluttered: the handful of actions people reach for daily stay in view, while
	// key management, sharing, storage, and the destructive actions live in the Manage surface (one grouped
	// dialog). Every button still routes through the same delegated click handler.
	const actions = [];
	if (v.valid && stuck) {
	...
	// One "Manage" button opens the grouped Manage surface (Keys, Password, Sharing, Storage, Remove, Delete…).
	// It is populated on open (openManage), reusing the same data-act values, with the "needs an unmounted vault"
	// gating applied there — so the card stays to the daily actions plus this single entry point.
	if (v.valid) actions.push(`<button class="ghost-btn" data-act="manage" data-path="${esc(v.path)}" data-name="${esc(v.name)}">Manage</button>`);
	// An unreachable vault (moved, renamed, or on a disconnected drive) cannot be mounted or managed, so it would
	// otherwise show no actions at all — leaving the user unable to act on the "Remove only forgets it here" note.
	// Offer Remove directly on the card so a stale entry is never stuck on the list; it forgets the entry only and
	// never deletes files, using the same confirm-guarded action the Manage surface exposes.
	else actions.push(`<button class="ghost-btn danger" data-act="remove" data-path="${esc(v.path)}" data-name="${esc(v.name)}">Remove</button>`);
	...
	return `<div class="card${stuck ? ' card-stuck' : ''}"${v.color ? ` data-color="${esc(v.color)}"` : ''} role="listitem">
			${vaultIcon(v)}
		<div class="info">
			<div class="name"><button class="star-btn${v.favorite ? ' on' : ''}" data-act="favorite" data-path="${esc(v.path)}" data-on="${v.favorite ? '0' : '1'}" aria-pressed="${v.favorite ? 'true' : 'false'}" title="${v.favorite ? 'Remove from favorites' : 'Add to favorites'}" aria-label="${v.favorite ? 'Remove from favorites' : 'Add to favorites'}">${v.favorite ? '★' : '☆'}</button>${colorPicker(v)} <span dir="auto"${hidden ? ' class="blurred"' : ''}>${esc(v.name)}</span> ${badge}${v.sealed ? ' <span class="badge sealed" title="Strict tamper tripwire is active">sealed</span>' : ''}${v.worm ? ` <span class="badge" title="Tamper-proof: every saved version is locked for ${esc(String(v.worm.retainDays))} days and cannot be deleted or overwritten (Object Lock)">tamper-proof</span>` : ''} <span class="live-badges" data-live-path="${esc(v.path)}">${liveBadgesHtml(v)}</span></div>
			<div class="path-line">
				<span class="path${hidden ? ' blurred' : ''}" dir="auto">${esc(v.mounted ? (v.mountpointDisplay || v.mountpoint) + '  ←  ' + (v.pathDisplay || v.path) : (v.pathDisplay || v.path))}</span>
				<span class="path-tools">
					<button type="button" class="path-tool" data-act="copy-path" data-path="${esc(v.path)}" title="Copy the full folder path" aria-label="Copy the full folder path">${COPY_ICON}</button>
					${privacyScreen ? `<button type="button" class="path-tool" data-act="toggle-priv" data-path="${esc(v.path)}" title="${hidden ? 'Show the name and path' : 'Hide the name and path'}" aria-label="${hidden ? 'Show the name and path' : 'Hide the name and path'}" aria-pressed="${hidden ? 'false' : 'true'}">${hidden ? EYE_ICON : EYE_OFF_ICON}</button>` : ''}
				</span>
			</div>
			${v.description ? `<div class="vault-desc${hidden ? ' blurred' : ''}" dir="auto">${esc(v.description)}</div>` : ''}
			${sizeCell(v)}
			${note}
		</div>
...
// `force` rebuilds the cards even while a dialog is open. The dialog guard below exists so a rebuild never detaches the
// card button a CARD-opened dialog (Manage, More) returns focus to. A global control that is not opened from a card —
// the Settings privacy-screen toggle — must instead update the cards immediately, so it passes force: the open Settings
// dialog keeps its own focus and is unaffected by the card list being redrawn beneath it.
function renderVaults(force) {
	const vaults = (lastState && lastState.vaults) || [];
	vaultsByPath = {};
	vaults.forEach(v => { vaultsByPath[v.path] = v; });
	// Privacy screen state, read fresh each render. When off, forget any reveals; and drop reveals for vaults that are
	// gone, so the set never grows or leaks a stale path.
	privacyScreen = !!(lastState && lastState.settings && lastState.settings.privacyScreen);
	if (!privacyScreen) revealedPaths.clear();
	else for (const p of [...revealedPaths]) if (!vaultsByPath[p]) revealedPaths.delete(p);
	...
	if (force || (!list.querySelector('details.more[open]') && !document.querySelector('dialog[open]'))) {
		const html = ordered.map(vaultCard).join('');
		if (html !== lastCardsHtml) { list.innerHTML = html; lastCardsHtml = html; }
	} else {
		// A More menu or dialog is open, so the full list must not be rebuilt (it would detach the open menu or the
		// element a dialog returns focus to). But the LIVE status badges must keep moving — otherwise opening More
		// freezes the self-heal "updating" pill and the mirror "syncing" pill mid-progress. Update just those per-vault
		// badge spans in place; they sit in the name line, away from the actions/menu, so nothing interactive is
		// disturbed. Matched by dataset (not a CSS selector) so an arbitrary vault path can never break the lookup, and
		// written only when the badge HTML actually changed. lastCardsHtml is left stale on purpose, so the next
		// unobstructed poll does a full, authoritative rebuild.
		list.querySelectorAll('.live-badges').forEach(span => {
			const v = vaultsByPath[span.dataset.livePath];
			if (!v) return;
			const h = liveBadgesHtml(v);
			if (span.innerHTML !== h) span.innerHTML = h;
		});
	}
	...
	// Bulk-remove affordance: when two or more vaults are unreachable, offer one control to forget them all at once.
	// Below two, the per-card Remove button is enough, so the bar stays hidden. Updated on every poll so it appears and
	// disappears as vaults come and go, and always outside the card-list rebuild guard so it stays current.
	const ubar = $('#unreachableBar');
	if (ubar) {
		const unreachable = vaults.filter(v => !v.valid).length;
		ubar.hidden = unreachable < 2;
		if (!ubar.hidden) { const um = $('#unreachableMsg'); if (um) um.textContent = unreachable + ' vaults can’t be read right now.'; }
	}
	// First authoritative render only: open the create panel for a first-run user (no vaults yet) so it doubles as
	// onboarding, and collapse it for a returning user so the vault list stays the focus. After this the user owns the
	// toggle — polls never fight a manual open/close, and the New-vault button re-opens it on demand.
	if (!createPanelDefaulted) {
		const cp = $('#createPanel');
		if (cp) { cp.open = vaults.length === 0; createPanelDefaulted = true; }
	}
}
...
async function refresh() {
	...
	try {
		...
		renderVaults();
		try { maybeRestoreSearchReturn(); } catch (_) {} // returning from a search-opened file: re-open the search on its results
		const al = $('#autoLock');
		...
		const aan = $('#autoAnonymize');
		if (aan && document.activeElement !== aan) aan.checked = (state.settings && state.settings.autoAnonymize) !== false; // on by default
		const psw = $('#privacyScreen');
		if (psw && document.activeElement !== psw) psw.checked = !!(state.settings && state.settings.privacyScreen);
		const auc = $('#autoUpdateCheck');
		if (auc && document.activeElement !== auc) auc.checked = !!(state.settings && state.settings.autoUpdateCheck);
		const ocrT = $('#ocrToggle');
		if (ocrT && document.activeElement !== ocrT) ocrT.checked = !!(state.settings && state.settings.ocr);
		const kdl = $('#keepDiagnosticLog');
		if (kdl && document.activeElement !== kdl) kdl.checked = !!(state.settings && state.settings.keepDiagnosticLog);
		...
		const p2p = $('#p2pMode');
		if (p2p && document.activeElement !== p2p) p2p.value = (state.settings && state.settings.p2pMode) || 'auto';
		// File-version retention and wedged-drive auto-restart: reflect the saved values, but never yank a control the
		// user is actively changing. A select whose saved value is not one of its presets falls back to its first option
		// via a guarded set (setSelect only assigns when the value exists), so a power-user's custom number is not misread.
		setSelect('#versionsKeep', state.settings && state.settings.versionsKeep, 10);
		setSelect('#versionsMaxAgeDays', state.settings && state.settings.versionsMaxAgeDays, 0);
		setSelect('#versionsMaxSizeMB', state.settings && state.settings.versionsMaxSizeMB, 0);
		const wr = $('#wedgeRestart');
		if (wr && document.activeElement !== wr) wr.checked = !!(state.settings && state.settings.wedgeRestart);
...
document.addEventListener('click', async (ev) => {
	...
	// Copy the REAL full folder path (not the ~-collapsed display) so it can be pasted straight into a file manager or a
	// terminal — a quiet way to reach a vault's folder even while the privacy screen keeps it blurred on screen.
	if (act === 'copy-path') {
		const p = btn.dataset.path || '';
		let done = false;
		try { if (navigator.clipboard && navigator.clipboard.writeText) { await navigator.clipboard.writeText(p); done = true; } } catch (_) {}
		if (!done) { try { const ta = document.createElement('textarea'); ta.value = p; ta.style.position = 'fixed'; ta.style.opacity = '0'; document.body.appendChild(ta); ta.select(); done = document.execCommand('copy'); ta.remove(); } catch (_) {} }
		toast(done ? 'Full path copied' : 'Could not copy the path', !done);
		return;
	}
	// Privacy screen: reveal or re-hide this one card's name and path. A deliberate click, so a glance never exposes it.
	if (act === 'toggle-priv') {
		const p = btn.dataset.path || '';
		if (revealedPaths.has(p)) revealedPaths.delete(p); else revealedPaths.add(p);
		renderVaults();
		return;
	}
	if (act === 'manage') { openManage(btn.dataset.path, btn.dataset.name); return; }
	// A Manage-surface row (the blocked ones already returned above) closes the Manage dialog first, so its target
	// dialog opens cleanly instead of stacking on top of Manage.
	if (btn.closest('#manageDialog')) { const md = $('#manageDialog'); if (md && md.open) md.close(); }
	...
	if (act === 'rename') { openRename(btn.dataset.path, btn.dataset.name); return; }
	...
	if (act === 'emergency') { openEmergency(btn.dataset.path); return; }
	if (act === 'favorite') { toggleFavorite(btn.dataset.path, btn.dataset.on === '1'); return; }
	// Color tag: the dot toggles the swatch popover; a swatch sets (or clears) this vault's color and refreshes.
	if (act === 'color-toggle') { const pop = btn.parentNode.querySelector('[data-color-pop]'); if (pop) { const show = pop.hidden; pop.hidden = !show; btn.setAttribute('aria-expanded', show ? 'true' : 'false'); } return; }
	if (act === 'set-color') {
		const c = btn.dataset.color === 'none' ? '' : btn.dataset.color;
		try { await withBusy(() => api('/api/vault-color', { path: btn.dataset.path, color: c })); refresh(); } catch (e) { toast((e && e.message) || 'Could not set the color', true); }
		return;
	}
	if (act === 'bio-mount') { quickBioMount(btn.dataset.path); return; }
	if (act === 'passwd') { openPasswd(btn.dataset.path, btn.dataset.name); return; }
	if (act === 'consent') { const cv = vaultsByPath[btn.dataset.path] || {}; const on = !cv.consentOnUse; try { await withBusy(() => api('/api/consent-on-use', { path: btn.dataset.path, on })); toast(on ? 'Approval now required on unlock — each unlock is recorded in the ledger' : 'Approval on unlock turned off'); refresh(); if ($('#manageDialog').open) openManage(btn.dataset.path, btn.dataset.name); } catch (e) { toast(e.message, true); } return; }
	...
	if (act === 'move-to-node') { openMove(btn.dataset.path, btn.dataset.name); return; }
	...
	if (act === 'remove') {
		...
		message += rv.cloud
			? ' Its encrypted data stays in the cloud, and the provider may keep versions or backups even after a later delete. To make it truly unrecoverable, use “Delete permanently” in this vault’s Manage panel instead — that destroys the keys, so the cloud data can never be decrypted again.'
			: ' To permanently destroy the keys so this copy can never be opened again, use “Delete permanently” in this vault’s Manage panel instead.';
...
function openMount(path, name) {
	...
	// "Full working disk" and "Streaming" are opposites (one caches everything, the other nothing), so ticking one
	// clears the other — the UI should never show both selected even though the backend resolves a conflict on its own.
	// onchange (not addEventListener) so re-opening the dialog never stacks duplicate handlers.
	if ($('#mountWorkingDisk') && $('#mountStreaming')) {
		$('#mountWorkingDisk').onchange = function () { if (this.checked) $('#mountStreaming').checked = false; };
		$('#mountStreaming').onchange = function () { if (this.checked) $('#mountWorkingDisk').checked = false; };
	}
...
async function doMount(password, over = {}) {
	// Consent-on-use: if this vault requires explicit approval on each unlock, confirm before proceeding. The server
	// then records this unlock in the vault's tamper-evident ledger, so the ledger becomes a signed history of every
	// use of the key. Skipped on a force re-attempt (the same unlock the user already approved).
	if (!over.force) {
		const cv = vaultsByPath[mountTarget] || {};
		if (cv.consentOnUse && !(await uiConfirm({ title: 'Approve this unlock', message: 'This vault requires your approval each time it is unlocked, and every unlock is recorded in its tamper-evident ledger. Unlock “' + (cv.name || 'this vault') + '” now?', confirmLabel: 'Approve & unlock', danger: false }))) return;
	}
...
let versionsTarget = null, versionsSnapshots = [];
function openVersions(path, name) {
	versionsTarget = path; versionsSnapshots = [];
	$('#versionsName').textContent = name;
	$('#versionsPw').value = '';
	$('#versionsFilterRow').hidden = true; $('#versionsFilter').value = '';
...
// A relative-time label ("2 hours ago") from a timestamp, computed against now. Server timestamps are UTC; the browser's
// Date does the local conversion, so the label is correct in the viewer's own timezone.
function relTime(then, now) {
	...
	const y = Math.round(mo / 12); return y + (y === 1 ? ' yr ago' : ' yr ago');
}
// A local day-bucket label: Today / Yesterday / a local date. Buckets are by the viewer's local calendar day.
function dayLabel(d, nowD) {
	const startOf = (x) => new Date(x.getFullYear(), x.getMonth(), x.getDate()).getTime();
	const days = Math.round((startOf(nowD) - startOf(d)) / 86400000);
	if (days === 0) return 'Today'; if (days === 1) return 'Yesterday';
	return d.toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
}
// Render the loaded snapshots as a chronological timeline: newest-first, grouped into local day buckets, each snapshot a
// node on a spine that expands to its files. A non-empty filter narrows to snapshots touching a matching file and shows
// only those files (a per-file history lens). Pure front-end regrouping of the data /api/versions already returned; a
// restore reuses the existing per-file handler (the data-restore-* attributes are unchanged).
function renderVersionsTimeline() {
	const list = $('#versionsList');
	const filter = ($('#versionsFilter') && $('#versionsFilter').value || '').trim().toLowerCase();
	const now = Date.now(), nowD = new Date();
	const buckets = []; let curKey = null, cur = null, shown = 0;
	for (const s of versionsSnapshots) {
		let files = s.files || [];
		if (filter) { files = files.filter((f) => f.toLowerCase().includes(filter)); if (!files.length) continue; }
		shown++;
		const at = s.at ? Date.parse(s.at) : 0, d = at ? new Date(at) : null;
		const key = d ? d.toDateString() : (s.timestamp || 'unknown');
		if (key !== curKey) { curKey = key; cur = { label: d ? dayLabel(d, nowD) : (s.timestamp || 'Unknown'), items: [] }; buckets.push(cur); }
		const time = d ? d.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' }) : (s.timestamp || '');
		const rel = d ? relTime(d.getTime(), now) : '';
		const badge = s.originLabel ? '<span class="badge">' + esc(s.originLabel) + '</span>' : '';
		const n = files.length;
		const fileItems = files.map((f) => '<li>' + esc(f) + ' <button type="button" class="link-btn" data-restore-ts="' + esc(s.timestamp) + '" data-restore-origin="' + esc(s.origin || '') + '" data-restore-file="' + esc(f) + '">Restore</button></li>').join('') || '<li class="muted">(no files)</li>';
		cur.items.push('<details class="tl-snap"' + (filter ? ' open' : '') + '><summary class="tl-snap-head"><span class="tl-time">' + esc(time) + '</span> ' + badge + ' <span class="tl-rel">' + esc(rel) + '</span><span class="tl-count">' + n + (n === 1 ? ' file' : ' files') + '</span></summary><ul class="tl-files">' + fileItems + '</ul></details>');
	}
	if (!shown) { list.innerHTML = '<div class="muted">' + (filter ? 'No versions match that file name.' : 'No prior versions yet.') + '</div>'; return; }
	list.innerHTML = buckets.map((b) => '<div class="tl-day"><div class="tl-day-head">' + esc(b.label) + '</div><div class="tl-snaps">' + b.items.join('') + '</div></div>').join('');
}
onDialogSubmit('#versionsForm', async () => {
	...
	try {
		...
		if (!rep.hasStore) { $('#versionsFilterRow').hidden = true; $('#versionsList').innerHTML = '<div class="muted">This vault has no backup or mirror yet, so it has no version history. Back it up or set up a mirror first.</div>'; return; }
		versionsSnapshots = rep.snapshots || [];
		if (!versionsSnapshots.length) { $('#versionsFilterRow').hidden = true; $('#versionsList').innerHTML = '<div class="muted">No prior versions yet — they are captured each time a backup or mirror runs after a file changes.</div>'; return; }
		$('#versionsFilterRow').hidden = false;
		renderVersionsTimeline();
	} catch (e) { $('#versionsFilterRow').hidden = true; $('#versionsList').innerHTML = '<div class="muted">' + esc(e.message) + '</div>'; }
});
if ($('#versionsFilter')) $('#versionsFilter').addEventListener('input', renderVersionsTimeline);
...
async function loadMembers() {
	...
	const recText = r.recovery ? ('Owner recovery: any ' + r.recovery.k + ' of ' + r.recovery.n + ' trustees can restore owner access.' + (r.recovery.stale ? ' <strong>It was set up before the last key rotation and no longer works — set it up again below.</strong>' : '')) : 'Owner recovery is not set up yet, so no trusted people can restore a lost owner.';
	const rec = recText + ' <button type="button" class="link-btn" id="ownerRecoveryOpen">' + (r.recovery ? 'Manage owner recovery…' : 'Set up owner recovery…') + '</button>';
	$('#membersBody').innerHTML =
...
document.addEventListener('click', async (e) => {
	...
	if (e.target.id === 'ownerRecoveryOpen') { openOwnerRecovery(membersTarget); return; }
...
function buildFieldRow(field) {
	...
	// The value control's accessible name mirrors the field's editable label (or its kind when the label is still
	// blank), so a screen reader announces "Username, edit text" rather than a bare "edit text" with no field identity.
	const fieldName = () => ((label.value || '').trim() || (KIND_LABELS[row.dataset.kind] || row.dataset.kind) || 'Value');
	...
	// Keep the value control's accessible name in step as the user types the label.
	label.addEventListener('input', () => { const cur = valWrap.querySelector('.sf-input'); if (cur) cur.setAttribute('aria-label', fieldName()); });
...
// ---- Name & description (the vault's encrypted metadata) ----
let renameTarget = null;
const renameShow = makeShow('#renameResult');
// The folder's own name (basename minus the .vault suffix), split on either separator so it is correct on every OS.
function vaultFolderName(p) { return String(p || '').split(/[\\/]/).pop().replace(/\.vault$/i, ''); }
function openRename(path, name) {
	renameTarget = path;
	const folder = vaultFolderName(path);
	$('#renameName').textContent = name;
	const v = vaultsByPath[path] || {};
	// Prefill from the local cache the list already shows: the title only when it differs from the folder name, and the
	// cached description. The password is needed only to save (it re-seals the metadata under the vault key).
	$('#renameTitle').value = (v.name && v.name !== folder) ? v.name : '';
	$('#renameTitle').placeholder = folder + ' (the folder name)';
	$('#renameDescription').value = v.description || '';
	$('#renamePassword').value = '';
	// Offer "hide the folder name" only when the folder is still a real name (not already an opaque id) and the vault
	// is unmounted (a rename needs the folder free).
	const isOpaque = /^[0-9a-f]{32}$/.test(folder);
	if ($('#anonymizeRow')) { $('#anonymizeRow').hidden = isOpaque || !!(v && v.mounted); $('#anonymizeRow').open = false; }
	const r = $('#renameResult'); r.hidden = true; r.textContent = '';
	$('#renameDialog').showModal();
	setTimeout(() => $('#renameTitle').focus(), 50);
}
if ($('#anonymizeBtn')) $('#anonymizeBtn').addEventListener('click', guarded(async () => {
	const password = $('#renamePassword').value;
	if (!password) return renameShow('bad', 'Enter the vault password first.');
	const okd = await uiConfirm({ title: 'Hide folder name', message: 'Rename this vault’s folder to an opaque id? Your files, keys, and password are unchanged — only the name on disk changes, and the name you set stays visible here. Important: if you have paired a decoy with this vault, the pairing will stop working and you must set it up again afterward.', confirmLabel: 'Hide it', danger: true });
	if (!okd) return;
	try {
		// Save the typed name/description first, so it is kept as the encrypted name once the folder loses its name.
		await withBusy(() => api('/api/vault-meta-set', { path: renameTarget, password, fields: { title: $('#renameTitle').value, description: $('#renameDescription').value } }));
		await withBusy(() => api('/api/vault-anonymize', { path: renameTarget, password }));
		await refresh();
		$('#renameDialog').close();
		toast('The folder name is now hidden on disk');
	} catch (e) { renameShow('bad', esc(e.message)); }
}));
// Save is a submit button (value="ok"), so Enter in the name or password field saves too — matching every other dialog.
onDialogSubmit('#renameForm', async () => {
	const password = $('#renamePassword').value;
	if (!password) return renameShow('bad', 'Enter the vault password to save the name and description.');
	const fields = { title: $('#renameTitle').value, description: $('#renameDescription').value };
	try {
		await withBusy(() => api('/api/vault-meta-set', { path: renameTarget, password, fields }));
		await refresh();
		$('#renameDialog').close();
		toast('Saved the vault name and description');
	} catch (e) { renameShow('bad', esc(e.message)); }
});
...
(function wireNotesIdle() {
	...
	// Import logins/notes from another password manager's export. Pick a file, preview the count with a dry run, then
	// import on confirm. The file is sent as raw bytes so a binary .1pux survives; parsing/writing happen server-side.
	if ($('#notesImportBtn') && $('#notesImportFile')) {
		$('#notesImportBtn').addEventListener('click', () => $('#notesImportFile').click());
		$('#notesImportFile').addEventListener('change', async (ev) => {
			const file = ev.target.files && ev.target.files[0];
			ev.target.value = ''; // reset so choosing the same file again re-fires change
			if (!file) return;
			const target = notesTarget, name = ($('#notesName') && $('#notesName').textContent) || 'this vault';
			try {
				notesShow('', 'Reading “' + file.name + '”…');
				const preview = await apiUpload('/api/notes-import', await file.arrayBuffer(), { path: target, filename: file.name, dry: '1' });
				if (!preview.total) return notesShow('bad', 'No items were found in that file. Make sure it is an unencrypted .csv, Bitwarden .json, or 1Password .1pux export.');
				const types = Object.keys(preview.byType || {}).sort().map((t) => preview.byType[t] + ' ' + t).join(', ');
				const go = await uiConfirm({ title: 'Import items', message: 'Found ' + preview.total + ' item' + (preview.total === 1 ? '' : 's') + ' in this ' + preview.format + ' export' + (types ? ' (' + types + ')' : '') + '. Import them into “' + name + '”? Items already present (matched by title and type) are skipped.', confirmLabel: 'Import' });
				if (!go) return notesShow('', 'Import canceled.');
				notesShow('', 'Importing ' + preview.total + ' item' + (preview.total === 1 ? '' : 's') + '…');
				const r = await apiUpload('/api/notes-import', await file.arrayBuffer(), { path: target, filename: file.name, dedupe: '1' });
				if (notesTarget === target) await loadNotesList();
				notesShow('ok', 'Imported ' + r.imported + ' item' + (r.imported === 1 ? '' : 's') + (r.skipped ? ', skipped ' + r.skipped + ' already present' : '') + '. Your export file still holds these secrets in the clear — delete it now.');
			} catch (err) { notesShow('bad', err.message); }
		});
	}
...
// The Send dialog has no submit button (its actions are all type="button"), so wire Enter in the optional
// link-password field to the primary "Create link" action — a keyboard user can complete the flow without tabbing.
if ($('#sendPassword')) $('#sendPassword').addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); $('#sendCreate').click(); } });
...
// Bring the content index up to date before an inside-files search, so a file you just added is searchable without a
// manual "Update index" — even when you add it while the search window is already open. This is cheap: indexing is
// incremental, so when nothing changed the refresh reads no files (only genuinely new or changed files are read, and
// only new scans are OCR'd). It streams the same progress bar as the manual button, and it is best-effort — a failure
// (a read-only or wedged mount) is swallowed and the search proceeds against whatever index exists, so the refresh can
// never block or break a search. A concurrent refresh is deduped on the server, so a rapid second search never starts a
// second index run.
let lastSearchRefreshAt = 0, lastSearchRefreshTarget = null;
async function ensureContentIndexFresh() {
	// Coalesce only RAPID repeat searches of the SAME vault — refining a query, or a double submit. When we refreshed
	// this vault a couple of seconds ago, skip the re-walk. The window is far shorter than the time it takes to add a
	// file and start a search, so it never hides a just-added file; a genuinely new file is still caught by the next
	// search after the window, when the vault is opened, and by the background watcher.
	if (lastSearchRefreshTarget === searchTarget && Date.now() - lastSearchRefreshAt < 2000) return;
	$('#searchProgress').hidden = false; searchProgress({ indeterminate: true, label: 'Updating index' });
	try { await withBusy(() => apiStream('/api/content-reindex', { path: searchTarget }, searchProgress)); lastSearchRefreshAt = Date.now(); lastSearchRefreshTarget = searchTarget; }
	catch (_) { /* keep the current index; the search below still runs */ }
	finally { $('#searchProgress').hidden = true; }
}
...
$('#searchIndexBtn').addEventListener('click', guarded(async (ev) => {
	// Confirm first: a manual rebuild reads every file in the vault (and runs text recognition when it is on), so it can
	// take a while on a large vault, and it rewrites the in-vault index, which then wants a fresh tamper snapshot.
	const go = await uiConfirm({ title: 'Update the search index', message: 'This reads every file in the vault to build the words-inside-files index, and can take a while on a large vault or with text recognition turned on. The index is stored inside the vault, so take a fresh tamper snapshot afterward. Update it now?', confirmLabel: 'Update index' });
	if (!go) return;
...
onDialogSubmit('#searchForm', async () => {
	...
	try {
		if (searchScope() === 'names') {
			...
			results.innerHTML = matches.map(m => '<div class="search-hit search-hit-open" role="button" tabindex="0" dir="auto" data-rel="' + esc(m) + '" title="Open ' + esc(m) + '">' + esc(m) + '</div>').join('');
			showResult($('#searchResult'), 'ok', matches.length + ' match' + (matches.length === 1 ? '' : 'es') + (mounted ? '' : ' (unmounted)') + '.');
		} else {
			await ensureContentIndexFresh(); // pick up files added since the last index before searching (once per dialog)
			results.innerHTML = '<div class="muted">Searching…</div>';
			...
			results.innerHTML = res.results.map(m => '<div class="search-hit search-hit-open" role="button" tabindex="0" dir="auto" data-rel="' + esc(m.path) + '" title="Open ' + esc(m.path) + '">' + esc(m.path) + '</div>').join('');
			showResult($('#searchResult'), 'ok', res.results.length + ' file' + (res.results.length === 1 ? '' : 's') + ' matched. Click a result to open it.');
		}
	} catch (e) { results.innerHTML = ''; showResult($('#searchResult'), 'bad', esc(e.message)); }
});
// Opening a search result in the viewer navigates away (the desktop app reuses its single window), so leave a short-
// lived note of the search — the vault, the query, and the "inside files" scope — in per-tab sessionStorage. When the
// viewer's Back returns to the app, maybeRestoreSearchReturn (called after each state render) re-opens the Search
// window on that vault and re-runs the query, so the user lands back on their results instead of the vault dashboard.
function rememberSearchReturn() {
	try { sessionStorage.setItem('vdisk-search-return', JSON.stringify({ path: searchTarget, name: $('#searchName').textContent || 'Vault', scope: searchScope(), query: $('#searchQuery').value || '', ts: Date.now() })); } catch (_) {}
}
let searchReturnChecked = false; // one-shot: consume the note once the target vault is in the rendered list
function maybeRestoreSearchReturn() {
	if (searchReturnChecked) return;
	let ctx = null;
	try { const raw = sessionStorage.getItem('vdisk-search-return'); ctx = raw ? JSON.parse(raw) : null; } catch (_) {}
	// No note, malformed, or stale (a minute is far longer than a look at one file) — discard and never check again.
	if (!ctx || !ctx.path || (Date.now() - (ctx.ts || 0)) > 60000) { try { sessionStorage.removeItem('vdisk-search-return'); } catch (_) {} searchReturnChecked = true; return; }
	if (!vaultsByPath[ctx.path]) return; // the vault is not in the list yet — wait for a later render, then restore
	try { sessionStorage.removeItem('vdisk-search-return'); } catch (_) {}
	searchReturnChecked = true;
	openSearch(ctx.path, ctx.name);
	if (ctx.scope === 'content') { const c = document.querySelector('input[name="searchScope"][value="content"]'); if (c) { c.checked = true; updateSearchScope(); } }
	$('#searchQuery').value = ctx.query;
	if (ctx.query) { try { $('#searchForm').requestSubmit(); } catch (_) {} }
}
// Open a search result in the built-in, zero-residue viewer — the file decrypts in THIS browser, with no external app
// and no OS preview cache, reusing the same "View files & notes" flow as the viewer button. It prompts for the vault
// password once, then opens straight to this one file. This works whether or not the vault is mounted (the viewer needs
// no drive), so a name search on a locked vault can open a result too.
$('#searchResults').addEventListener('click', async (e) => {
	const hit = e.target.closest('.search-hit-open'); if (!hit) return;
	const rel = hit.dataset.rel; if (rel == null) return;
	rememberSearchReturn(); // so "Back" from the opened file returns to these results (see maybeRestoreSearchReturn)
	const v = vaultsByPath[searchTarget];
	if (v && v.mounted) {
		// The vault is already unlocked on this machine, so open the viewer reusing the key the mounted session holds —
		// no password prompt. The blank tab opens synchronously on the click so a pop-up blocker cannot swallow it.
		viewerFileTarget = rel;
		const tab = window.open('', VIEWER_WINDOW);
		let r; try { r = await withBusy(() => api('/api/mobile-start', { path: searchTarget, local: true })); }
		catch (err) { if (tab && !tab.closed) tab.close(); viewerFileTarget = null; return toast(err.message, true); }
		if (!r) { if (tab && !tab.closed) tab.close(); viewerFileTarget = null; return; }
		launchViewer(r, tab);
		return;
	}
	// A locked vault: authorize once with the password (the viewer needs no drive), pre-set to open this file.
	$('#searchDialog').close();
	openMobile(searchTarget, $('#searchName').textContent || 'Vault', rel);
});
// Keyboard activation for a focused result (each is role="button" tabindex="0"): Enter or Space opens it, exactly as a
// click does, so results are reachable and usable without a mouse.
$('#searchResults').addEventListener('keydown', (e) => {
	if (e.key !== 'Enter' && e.key !== ' ') return;
	const hit = e.target.closest('.search-hit-open'); if (!hit) return;
	e.preventDefault(); hit.click();
});
...
async function loadKeys() {
	...
	const rows = slots.map((s) => {
		...
		const rm = slots.length > 1 ? `<button type="button" class="link-btn danger" aria-label="Remove ${esc((kind + ' ' + (label || '')).trim())}" data-remove-key="${esc(s.id)}">Remove</button>` : ''; // aria-label names WHICH key, so a screen reader hears "Remove recovery key …", not a row of identical "Remove"s
...
async function openKeys(path, name) {
	...
	// A cloud vault CAN rotate now — it re-encrypts and re-uploads the whole store — so the rotate action is shown; the
	// note below explains that for a cloud vault it re-uploads everything (slower) and the tamper-proof (WORM) caveat.
	const keysCloud = !!(vaultsByPath[path] || {}).cloud;
	if ($('#keysRotate')) $('#keysRotate').hidden = false;
	if ($('#keysRotateCloudNote')) $('#keysRotateCloudNote').hidden = !keysCloud;
...
async function loadShares() {
	...
	const rows = shares.map(s => {
		...
		const rm = (!s.revoked) ? '<button type="button" class="link-btn danger" aria-label="Revoke ' + esc(s.label || 'read link') + '" data-revoke-sid="' + esc(s.sid) + '">Revoke</button>' : ''; // name WHICH link, so a screen reader does not hear a row of identical "Revoke"s
...
let shareTarget = null, shareGroupSlotId = null, shareCloud = false;
...
let viewerFileTarget = null; // when set, "Open in this browser" deep-links straight to this file (a search-result open)
function openMobile(path, name, fileTarget) {
	mobileTarget = path;
	viewerFileTarget = fileTarget || null;
...
// Open the built-in viewer for a freshly minted session `r`, optionally deep-linked to one file. Shared by the password
// dialog and the mounted-vault search open so both handle the fragment, the desktop in-app WebView, and pop-up blockers
// the same way. The target file rides in the FRAGMENT, so its path is never sent to the server and is scrubbed from the
// viewer's address bar on arrival, exactly like the pairing code. `tab` is a blank tab opened synchronously on the click
// (so a pop-up blocker cannot swallow it); in the desktop app the single WebView has no second tab, so r.inApp navigates
// this window instead. Clears viewerFileTarget so it never leaks into a later open.
// A stable window name so a plain browser REUSES one viewer tab instead of piling up a new window per file opened.
// Reusing a named target needs no `noopener`, and the viewer is our own same-origin page, so nothing is exposed by the
// window.opener link. The desktop app never uses this — its r.inApp branch navigates the single WebView in place.
const VIEWER_WINDOW = 'vaultonaut-viewer';
function launchViewer(r, tab) {
	const url = location.origin + r.path + '#' + r.code + (viewerFileTarget ? '&f=' + encodeURIComponent(viewerFileTarget) : '');
	viewerFileTarget = null;
	if (r.inApp) { if (tab && !tab.closed) tab.close(); $('#mobileDialog').close(); location.href = url; return; }
	// Reuse the pre-opened named tab, or (if a pop-up blocker swallowed it) open/reuse the viewer window by name, so a
	// second file replaces the same tab rather than spawning another.
	if (tab && !tab.closed) tab.location = url; else if (!window.open(url, VIEWER_WINDOW)) location.href = url;
	$('#mobileDialog').close();
}
...
function openShare(path, name) {
	...
	shareCloud = !!(vaultsByPath[path] || {}).cloud; // a cloud vault can't be packed or shared as a folder — the delivery step below shows cloud-specific guidance instead
...
onDialogSubmit('#shareForm', async () => {
	...
	try {
		...
		$('#shareCreate').hidden = true; // password made — the next step is now delivery (pack/folder, or cloud guidance)
		// A cloud vault has no file to pack and no folder to share, so show the cloud guidance and skip the pack key list.
		if ($('#shareDeliverLocal')) $('#shareDeliverLocal').hidden = shareCloud;
		if ($('#shareDeliverCloud')) $('#shareDeliverCloud').hidden = !shareCloud;
		if (!shareCloud) await loadShareKeys();
...
function saveScrubSchedule() {
	...
	if (m === 'daily') { const st = localHmToUtc(3, 0); schedule = { mode: 'daily', hour: st.hour, minute: st.minute, localHour: 3, localMinute: 0, tz: schedTz(), autoHeal: $('#scrubHeal').checked }; } // 3am local, daylight-saving-correct via the stored zone (UTC hour/minute kept for older clients)
...
$('#createForm').addEventListener('submit', async (ev) => {
	...
	try {
		...
		// The created vault's REAL folder — for an opaque vault this is an opaque <id>.vault path the server chose, not
		// the name the user typed — so the follow-up recovery-kit offer acts on the right vault.
		...
		if (sourceDir) {
			...
			if (r && r.vault) createdPath = r.vault;
			resetCreateForm();
			...
			if (cr && cr.vault) createdPath = cr.vault;
			resetCreateForm();
		...
		await offerRecoveryKit(createdPath, newName);
	} catch (e) { toast(e.message, true); }
});
...
// ---- Restore a vault from a backup copy ----
// The backup is already encrypted, so this is a copy plus a list entry — no password. Reuses the in-page folder
// picker for both the source (a .vault folder) and the destination (any folder), and the same /api/restore the
// backend gates to this computer only.
function openRestore() {
	$('#restoreSource').value = ''; $('#restoreDest').value = '';
	$('#restoreOverwrite').checked = false;
	const res = $('#restoreResult'); if (res) res.hidden = true;
	$('#restoreDialog').showModal();
}
if ($('#restoreOpen')) $('#restoreOpen').addEventListener('click', openRestore);
if ($('#restoreSourceBrowse')) $('#restoreSourceBrowse').addEventListener('click', () => openBrowse({ title: 'Choose the backed-up vault folder', pickVaults: true, onChoose: (p) => { $('#restoreSource').value = p; reopen('#restoreDialog'); } }));
if ($('#restoreDestBrowse')) $('#restoreDestBrowse').addEventListener('click', () => openBrowse({ title: 'Choose where to restore', useLabel: 'Restore here', pickVaults: false, onChoose: (p) => { $('#restoreDest').value = p; reopen('#restoreDialog'); } }));
if ($('#restoreGo')) $('#restoreGo').addEventListener('click', async () => {
	const source = $('#restoreSource').value.trim(), dest = $('#restoreDest').value.trim();
	const res = $('#restoreResult');
	if (!source || !dest) { showResult(res, 'bad', 'Choose both the backup vault folder and where to restore it.'); return; }
	showResult(res, '', 'Restoring — this can take a moment for a large vault.');
	try {
		const r = await withBusy(() => api('/api/restore', { source, dest, overwrite: $('#restoreOverwrite').checked }));
		showResult(res, 'ok', 'Restored to <code>' + esc(r.vault) + '</code>. It is now in your list — unlock it with its own password.');
		refresh();
	} catch (e) { showResult(res, 'bad', esc(e.message)); }
});

// ---- Receive a vault from another machine (the destination side of moving a vault between machines) ----
// Paste the connect code the other machine's Serve window shows; only the encrypted store is copied here, and the
// received vault opens with its own password. Guarded against a double-click: receiving ADDS a vault, and a blank
// name would otherwise create a second copy on a fast second click.
function openReceive() {
	$('#receiveCode').value = ''; $('#receiveName').value = '';
	const res = $('#receiveResult'); if (res) res.hidden = true;
	$('#receiveDialog').showModal();
}
if ($('#receiveOpen')) $('#receiveOpen').addEventListener('click', openReceive);
if ($('#receiveGo')) $('#receiveGo').addEventListener('click', guarded(async () => {
	const code = $('#receiveCode').value.trim(), name = $('#receiveName').value.trim();
	const res = $('#receiveResult');
	if (!code) { showResult(res, 'bad', 'Paste the connect code from the other machine first.'); return; }
	showResult(res, '', 'Receiving — copying the encrypted vault, then checking it is complete. This can take a while for a large vault.');
	try {
		const r = await withBusy(() => api('/api/receive-vault', { code, name: name || undefined }));
		showResult(res, 'ok', 'Received to <code>' + esc(r.vault) + '</code>. It is now in your list — unlock it with its own password.');
		refresh();
	} catch (e) { showResult(res, 'bad', esc(e.message)); }
}));

// ---- Move a vault to another fleet node (the common "push my vault to a node" direction) ----
// This machine serves the vault read-only, the chosen node pulls and verifies it, and only then is the source retired
// here (forgotten from the list; its files stay on disk). Guarded against a double-click since it ends by retiring.
let moveTarget = null;
function openMove(path, name) {
	moveTarget = path;
	const v = vaultsByPath[path] || {};
	$('#moveVaultName').textContent = name || v.name || 'this vault';
	const sel = $('#moveDest');
	const nodes = (lastState && lastState.fleet) || [];
	sel.innerHTML = nodes.map((n) => '<option value="' + esc(n.id) + '">' + esc(n.label || n.url || n.id) + (n.status ? ' — ' + esc(n.status) : '') + '</option>').join('');
	$('#moveName').value = ''; $('#movePassword').value = '';
	const res = $('#moveResult'); if (res) res.hidden = true;
	$('#moveDialog').showModal();
}
if ($('#moveGo')) $('#moveGo').addEventListener('click', guarded(async () => {
	const res = $('#moveResult');
	const destId = $('#moveDest').value;
	if (!destId) { showResult(res, 'bad', 'Enrol a fleet node first, then choose it here.'); return; }
	const destLabel = $('#moveDest').selectedOptions[0] ? $('#moveDest').selectedOptions[0].textContent : 'the node';
	if (!(await uiConfirm({ title: 'Move this vault', message: 'Copy this vault to “' + destLabel + '” and then retire it from this machine? The copy is verified first, and your files stay on this disk until you delete them.', confirmLabel: 'Move', danger: false }))) return;
	showResult(res, '', 'Moving — serving the vault, copying it to the node, and verifying it is complete. This can take a while for a large vault.');
	try {
		const r = await withBusy(() => api('/api/fleet-push', { destId: destId, path: moveTarget, name: $('#moveName').value.trim() || undefined, password: $('#movePassword').value || undefined }));
		if (r.ok === false) { showResult(res, 'bad', 'The move did not complete' + (r.stage ? ' (' + esc(r.stage) + ')' : '') + ': ' + esc(r.error || 'unknown error') + '. Nothing was removed from this machine.'); return; }
		const note = r.retired ? 'This machine’s copy has been retired (its files remain on disk until you delete them).' : 'This machine’s copy could NOT be retired automatically' + (r.retireError ? ' (' + esc(r.retireError) + ')' : '') + ' — remove it from your list when ready.';
		showResult(res, 'ok', 'Moved to <code>' + esc(r.vault || 'the node') + '</code> on the destination. ' + note);
		refresh();
	} catch (e) { showResult(res, 'bad', esc(e.message) + ' — nothing was removed from this machine.'); }
}));

// ---- Owner recovery (team vaults): set up / get a trustee share / reconstruct a lost owner ----
// Opened from the Members dialog (per-vault, so it operates on that vault — no vault picker). Each action reuses the
// same Vault functions the CLI uses, and the credential gates each: setup needs the owner password, a share needs the
// trustee's private key, and reconstruction needs a quorum of shares plus a new password. parseTrusteeLines is the
// same "Name = public key" parser the emergency backstop uses.
let ownerRecoveryTarget = null;
function openOwnerRecovery(path) {
	ownerRecoveryTarget = path;
	const v = ((lastState && lastState.vaults) || []).find(x => x.path === path);
	$('#ownerRecoveryName').textContent = v ? (v.name || v.path) : '';
	$('#orTrustees').value = ''; $('#orK').value = '2'; $('#orPw').value = '';
	$('#orTrusteeKey').value = ''; $('#orShareOut').value = ''; $('#orShareOutRow').hidden = true;
	$('#orShares').value = ''; $('#orNewPw').value = '';
	$('#orResult').hidden = true;
	$('#ownerRecoveryDialog').showModal();
}
if ($('#orSetupBtn')) $('#orSetupBtn').addEventListener('click', async () => {
	const trustees = parseTrusteeLines($('#orTrustees').value);
	const k = Number($('#orK').value) || 0;
	const password = $('#orPw').value;
	const res = $('#orResult');
	if (trustees.length < 2) return showResult(res, 'bad', 'Enter at least two trustees, one per line, as “Name = public key”.');
	if (!(k >= 2 && k <= trustees.length)) return showResult(res, 'bad', 'The number needed must be between 2 and ' + trustees.length + '.');
	if (!password) return showResult(res, 'bad', 'Enter your owner password.');
	try { const r = await withBusy(() => api('/api/owner-recovery-setup', { path: ownerRecoveryTarget, password, trustees, k })); showResult(res, 'ok', 'Owner recovery set up: any ' + esc(String(r.k)) + ' of ' + esc(String(r.n)) + ' trustees can restore owner access. Each trustee can now reveal their share below.'); }
	catch (e) { showResult(res, 'bad', esc(e.message)); }
});
if ($('#orShareBtn')) $('#orShareBtn').addEventListener('click', async () => {
	const key = $('#orTrusteeKey').value.trim();
	const res = $('#orResult');
	if (!key) return showResult(res, 'bad', 'Paste your trustee private key.');
	try {
		const r = await withBusy(() => api('/api/recovery-share', { path: ownerRecoveryTarget, key }));
		if (!r.share) { $('#orShareOutRow').hidden = true; return showResult(res, 'bad', 'This key holds no recovery share for this vault.'); }
		$('#orShareOut').value = r.share; $('#orShareOutRow').hidden = false;
		showResult(res, 'ok', 'Your share is shown below — send it to whoever is recovering owner access.');
	} catch (e) { showResult(res, 'bad', esc(e.message)); }
});
if ($('#orRecoverBtn')) $('#orRecoverBtn').addEventListener('click', async () => {
	const shares = $('#orShares').value.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
	const newPassword = $('#orNewPw').value;
	const res = $('#orResult');
	if (shares.length < 2) return showResult(res, 'bad', 'Paste at least the required number of trustee shares, one per line.');
	if (!newPassword) return showResult(res, 'bad', 'Choose a new owner password to regain access with.');
	try { await withBusy(() => api('/api/owner-recover', { path: ownerRecoveryTarget, shares, newPassword })); showResult(res, 'ok', 'Owner access recovered — open the vault with your new owner password.'); refresh(); }
	catch (e) { showResult(res, 'bad', esc(e.message)); }
});

// ---- Critical-event notifications ----
// Config is loaded and saved through the dedicated, local-only /api/notify-* routes (the webhook URL is sensitive, so
// it is never exposed over a network bind). The event list is rendered from the server's catalog so a new event type
// appears automatically.
let notifyCatalog = [];
async function openNotify() {
	const res = $('#notifyResult'); if (res) res.hidden = true;
	try {
		const c = await api('/api/notify-get', {});
		notifyCatalog = c.catalog || [];
		$('#notifyEnabled').checked = !!c.enabled;
		$('#notifyWebhook').value = c.webhookUrl || '';
		$('#notifyGeneric').checked = !!c.generic;
		$('#notifyEvents').innerHTML = notifyCatalog.map(ev => {
			const on = (c.events && (ev.type in c.events)) ? !!c.events[ev.type] : !!ev.defaultOn;
			return '<label class="checkfield"><input type="checkbox" class="notify-ev" data-ev="' + esc(ev.type) + '"' + (on ? ' checked' : '') + '><span>' + esc(ev.title) + '</span></label>';
		}).join('');
...
function notifyFormValues() {
	const events = {};
	document.querySelectorAll('#notifyEvents .notify-ev').forEach(cb => { events[cb.dataset.ev] = cb.checked; });
	return { enabled: $('#notifyEnabled').checked, webhookUrl: $('#notifyWebhook').value.trim(), generic: $('#notifyGeneric').checked, events };
}
if ($('#notifySave')) $('#notifySave').addEventListener('click', async () => {
	const res = $('#notifyResult');
	try { await withBusy(() => api('/api/notify-set', notifyFormValues())); showResult(res, 'ok', 'Saved.'); }
	catch (e) { showResult(res, 'bad', esc(e.message)); }
});
if ($('#notifyTest')) $('#notifyTest').addEventListener('click', async () => {
	const res = $('#notifyResult');
	const url = $('#notifyWebhook').value.trim();
	if (!url) return showResult(res, 'bad', 'Add a webhook URL first.');
	showResult(res, '', 'Sending a test…');
	try { await withBusy(() => api('/api/notify-test', { webhookUrl: url })); showResult(res, 'ok', 'Test sent — check that it arrived at your endpoint.'); }
	catch (e) { showResult(res, 'bad', esc(e.message)); }
});
...
async function loadImportBrowse(p) {
	...
	for (const f of (data.folders || [])) {
		...
		rows.push(`<div class="pick-row"><input type="checkbox" class="pick-box" aria-label="Select ${esc(f.name)}" data-path="${esc(f.path)}" data-name="${esc(f.name)}"${on}><button type="button" class="pick-open" data-open="${esc(f.path)}"><span class="pick-folder">${esc(f.name)}</span><span class="pick-hint">open</span></button></div>`);
	}
...
// A daily schedule's hour/minute are stored in UTC on the backend, so it fires at the same absolute moment on every
// machine (uniform for sync and mirror, and stable if a vault moves timezones). The UI captures and shows LOCAL time
// and converts at these edges, using today's offset so it tracks the current daylight-saving state.
function localHmToUtc(hour, minute) { const d = new Date(); d.setHours(Number(hour) || 0, Number(minute) || 0, 0, 0); return { hour: d.getUTCHours(), minute: d.getUTCMinutes() }; }
function utcHmToLocal(hour, minute) { const d = new Date(); d.setUTCHours(Number(hour) || 0, Number(minute) || 0, 0, 0); return { hour: d.getHours(), minute: d.getMinutes() }; }
// This machine's IANA zone (e.g. "America/New_York"), stored with a daily schedule so the backend fires it at the same
// LOCAL wall-clock time every day — it does not drift by an hour across a daylight-saving change. '' if unavailable.
function schedTz() { try { return Intl.DateTimeFormat().resolvedOptions().timeZone || ''; } catch (_) { return ''; } }
// The local time to show for a stored daily schedule: prefer the intended local time saved with a zone-aware schedule,
// and fall back to converting the legacy UTC hour/minute for a schedule saved before zones were recorded.
function scheduleLocalHm(sch) { return (sch && Number.isFinite(sch.localHour)) ? { hour: sch.localHour, minute: sch.localMinute } : utcHmToLocal(sch.hour, sch.minute); }
...
function openBackup(path, name) {
	...
	if (sch.mode === 'daily') { const lt = scheduleLocalHm(sch); $('#backupTime').value = pad2(lt.hour) + ':' + pad2(lt.minute); } else { $('#backupTime').value = '02:00'; }
	$('#backupTz').textContent = (sch.mode === 'daily' && sch.tz) ? sch.tz : tzLabel();
	backupFreqChanged();
...
$('#backupSave').addEventListener('click', async () => {
	...
	if (f === 'off') schedule = { mode: 'off' };
		...
		const dest = backupDestValue();
		if (!dest) return toast('Choose a destination first', true);
		if (f === 'daily') { const [h, m] = ($('#backupTime').value || '02:00').split(':').map(Number); const ut = localHmToUtc(h, m); schedule = { mode: 'daily', hour: ut.hour, minute: ut.minute, localHour: h, localMinute: m, tz: schedTz(), dest }; }
...
// Saved peers, shown at the top of the peer dialog so they can be tested, edited, or removed (not only added).
// Mirrors the cloud-storage manage list; reads the peers already in the poll state and calls the peer routes.
function renderPeerList() {
	const el = $('#peerList'); if (!el) return;
	const peers = (lastState && lastState.peers) || [];
	if (!peers.length) { el.innerHTML = '<p class="hint">No peers saved yet.</p>'; return; }
	el.innerHTML = peers.map(p => '<div class="row-between">'
		+ '<span>' + esc(p.label || p.url) + ' <span class="muted">(' + esc(p.user || 'vd') + ')</span></span>'
		+ '<span><button type="button" class="ghost-btn" data-peer-test="' + esc(p.id) + '">Test</button> '
		+ '<button type="button" class="ghost-btn" data-peer-edit="' + esc(p.id) + '">Edit</button> '
		+ '<button type="button" class="ghost-btn danger" data-peer-remove="' + esc(p.id) + '">Remove</button></span></div>').join('');
	el.querySelectorAll('[data-peer-test]').forEach(b => b.addEventListener('click', async () => {
		try { const t = await withBusy(() => api('/api/peer-test', { id: b.getAttribute('data-peer-test') })); peerMsg(t.ok ? 'ok' : 'bad', t.ok ? 'Connected — the peer is reachable and the login works.' : 'Could not connect: ' + esc(t.error || 'unknown') + '.'); }
		catch (e) { peerMsg('bad', esc(e.message)); }
	}));
	el.querySelectorAll('[data-peer-edit]').forEach(b => b.addEventListener('click', () => openPeer(b.getAttribute('data-peer-edit'), peerOnSaved)));
	el.querySelectorAll('[data-peer-remove]').forEach(b => b.addEventListener('click', async () => {
		const okd = await uiConfirm({ title: 'Remove peer', message: 'Remove this peer? Any mirror pointed at it will stop until you add it again — nothing at the peer itself is deleted.', confirmLabel: 'Remove', danger: true });
		if (!okd) return;
		try { await withBusy(() => api('/api/peer-remove', { id: b.getAttribute('data-peer-remove') })); await refresh(); renderPeerList(); }
		catch (e) { peerMsg('bad', esc(e.message)); }
	}));
}
...
function serveShowConn(info) {
	...
	$('#serveSecNote').textContent = info.bind === 'relay'
		...
		: info.bind === 'wan'
			? (info.secure ? (info.wan ? '🔒 Served over an encrypted connection — your router opened a port, so the other device can reach it directly from anywhere. The port is released when you stop.'
					: '🔒 Served over an encrypted connection on your local network — your router would not open a port, so it stays reachable on this network only.')
				: '⚠ Serving over plain HTTP: no TLS certificate could be made. The vault contents are encrypted either way; try serving again.')
			: info.bind === 'lan'
				? (info.secure ? '🔒 Served over an encrypted connection on your local network — the other device pins this node’s certificate and finds it automatically, with no server involved.'
					: '⚠ Serving over plain HTTP on your network: no TLS certificate could be made. The vault contents are encrypted either way; try serving again.')
				: '';
	$('#serveConn').hidden = !info.url;
	$('#serveBindRow').hidden = true;
	// Hide the mode-specific option rows too: with the "Reachable on" selector now gone, leaving the onion (single-hop,
	// stable, bridge) or relay (host/token) rows on screen would strand context-less controls above the connection panel.
	for (const id of ['#serveRelayRow', '#serveSingleHopRow', '#serveStableRow', '#serveBridgeRow']) { const el = $(id); if (el) el.hidden = true; }
...
async function openServe(path, name) {
	...
	if (v.serving && v.serving.serving) {
		...
		$('#serveStop').hidden = true; $('#serveGo').hidden = false; $('#serveBind').value = 'lan';
		$('#serveRelayHost').value = ''; $('#serveRelayToken').value = ''; $('#serveRelayToken').placeholder = '';
		if ($('#serveRelayCode')) $('#serveRelayCode').value = '';
		if ($('#serveSingleHop')) $('#serveSingleHop').checked = false;
		if ($('#serveStable')) $('#serveStable').checked = false;
		serveBindChanged();
		// Offer onion mode only when a Tor is actually reachable, so the option never appears as a dead end.
		try { const t = await api('/api/tor-available', {}); if ($('#serveOnionOption')) $('#serveOnionOption').hidden = !(t && t.available); } catch (_) { if ($('#serveOnionOption')) $('#serveOnionOption').hidden = true; }
		// Reflect the current global Tor bridge setting, and build the options from the modes the bundle actually offers
		// (from pt_config) so a future bundle's transports appear automatically instead of a hardcoded list drifting. A
		// custom line, set from the CLI, leaves the select at its neutral position without disturbing the saved setting.
		try {
			const tb = await api('/api/tor-bridges', {});
			const sel = $('#serveBridge');
			if (sel) {
				const labels = { snowflake: 'Snowflake (good general choice)', obfs4: 'obfs4', meek: 'meek' };
				const modes = (tb && Array.isArray(tb.builtIn) && tb.builtIn.length) ? tb.builtIn : ['snowflake', 'obfs4', 'meek'];
				sel.textContent = '';
				const off = document.createElement('option'); off.value = 'off'; off.textContent = 'No bridge — connect to Tor directly'; sel.appendChild(off);
				for (const m of modes) { const o = document.createElement('option'); o.value = m; o.textContent = 'Use a bridge — ' + (labels[m] || m); sel.appendChild(o); }
				sel.value = (tb && modes.indexOf(tb.mode) >= 0) ? tb.mode : 'off';
			}
		} catch (_) {}
...
function serveBindChanged() { $('#serveRelayRow').hidden = $('#serveBind').value !== 'relay'; const onion = $('#serveBind').value === 'onion'; if ($('#serveSingleHopRow')) $('#serveSingleHopRow').hidden = !onion; if ($('#serveStableRow')) $('#serveStableRow').hidden = !onion; if ($('#serveBridgeRow')) $('#serveBridgeRow').hidden = !onion; }
// The Tor bridge is a GLOBAL setting (it applies to every Tor connection, not just this serve), so it is loaded from and
// saved to the server as soon as it changes — not bundled into the serve request. Built-in modes only here; a custom
// bridge line is set from the CLI (vdisk tor-bridge custom ...).
if ($('#serveBridge')) $('#serveBridge').addEventListener('change', guarded(async () => {
	try { const r = await api('/api/tor-bridges', { set: true, mode: $('#serveBridge').value }); if (r && r.mode) $('#serveBridge').value = (r.mode === 'custom' ? 'off' : r.mode); serveMsg('ok', r.mode === 'off' ? 'Connecting to Tor directly.' : 'Bridge set to ' + r.mode + '.'); }
	catch (e) { serveMsg('bad', esc(e.message)); }
}));
$('#serveBind').addEventListener('change', serveBindChanged);
// A relay INVITE code (from `vdisk relay --host …`) bundles the hub address and token — paste it once to fill both
// fields below. Decoded in the browser (base64 JSON, UTF-8-safe via TextDecoder), so nothing is sent until you start
// the node; the existing serve flow then uses the filled host + token. atob/TextDecoder/Uint8Array are cross-browser.
function decodeRelayCode(code) {
	try {
		const bytes = Uint8Array.from(atob(String(code || '').trim()), (c) => c.charCodeAt(0));
		const o = JSON.parse(new TextDecoder().decode(bytes));
		if (o && o.t === 'relay' && o.host && o.token) return { host: String(o.host), token: String(o.token) };
	} catch (_) {}
	return null;
}
if ($('#serveRelayCode')) $('#serveRelayCode').addEventListener('input', () => {
	const rc = decodeRelayCode($('#serveRelayCode').value);
	if (rc) { $('#serveRelayHost').value = rc.host; $('#serveRelayToken').value = rc.token; }
});
$('#serveGo').addEventListener('click', guarded(async (ev) => {
	...
	try {
		...
		if (bind === 'onion' && $('#serveSingleHop') && $('#serveSingleHop').checked) body.singleHop = true;
		if (bind === 'onion' && $('#serveStable') && $('#serveStable').checked) body.stableOnion = true;
...
// Saved SFTP servers, shown at the top of the SFTP dialog so they can be tested, edited, or removed after they
// are first added. Mirrors the cloud-storage manage list; reads the destinations already in the poll state.
function renderSftpList() {
	const el = $('#sftpList'); if (!el) return;
	const dests = (lastState && lastState.sftpDests) || [];
	if (!dests.length) { el.innerHTML = '<p class="hint">No SFTP servers saved yet.</p>'; return; }
	el.innerHTML = dests.map(d => '<div class="row-between">'
		+ '<span>' + esc(d.label || d.host) + ' <span class="muted">(' + esc(d.user || '') + '@' + esc(d.host) + ')</span></span>'
		+ '<span><button type="button" class="ghost-btn" data-sftp-test="' + esc(d.id) + '">Test</button> '
		+ '<button type="button" class="ghost-btn" data-sftp-edit="' + esc(d.id) + '">Edit</button> '
		+ '<button type="button" class="ghost-btn danger" data-sftp-remove="' + esc(d.id) + '">Remove</button></span></div>').join('');
	el.querySelectorAll('[data-sftp-test]').forEach(b => b.addEventListener('click', async () => {
		try { const t = await withBusy(() => api('/api/sftp-test', { id: b.getAttribute('data-sftp-test') })); showResult($('#sftpResult'), t.ok ? 'ok' : 'bad', t.ok ? 'Connected — the server is reachable and the login works.' : 'Could not connect: ' + esc(t.error || 'unknown') + '.'); }
		catch (e) { showResult($('#sftpResult'), 'bad', esc(e.message)); }
	}));
	el.querySelectorAll('[data-sftp-edit]').forEach(b => b.addEventListener('click', () => openSftp(b.getAttribute('data-sftp-edit'), sftpOnSaved)));
	el.querySelectorAll('[data-sftp-remove]').forEach(b => b.addEventListener('click', async () => {
		const okd = await uiConfirm({ title: 'Remove SFTP server', message: 'Remove this SFTP server? Any backup pointed at it will stop until you add it again — nothing on the server itself is deleted.', confirmLabel: 'Remove', danger: true });
		if (!okd) return;
		try { await withBusy(() => api('/api/sftp-dest-remove', { id: b.getAttribute('data-sftp-remove') })); await refresh(); renderSftpList(); }
		catch (e) { showResult($('#sftpResult'), 'bad', esc(e.message)); }
	}));
}
...
// Read text in scans and images (OCR): the language file downloads as soon as this is turned on, and once a vault has a
// content index, new scans and images are then read automatically each time it is opened.
const ocrEl = $('#ocrToggle');
if (ocrEl) ocrEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { ocr: on }); toast(on ? 'Text recognition on — new scans and images are read automatically when you open a vault' : 'Text recognition off'); }
	catch (e) { toast(e.message, true); ev.target.checked = !on; }
});
// Hide folder names: convert an existing named vault to the opaque format on its next unlock.
const aanEl = $('#autoAnonymize');
if (aanEl) aanEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { autoAnonymize: on }); toast(on ? 'Existing vaults will hide their folder name the next time you unlock them' : 'Folder names are left as they are — new vaults are still hidden'); }
	catch (e) { ev.target.checked = !on; toast(e.message, true); }
});
// Privacy screen: blur vault names and paths until deliberately revealed. Apply it to the cards at once (don't wait for
// the next poll), and forget any reveals when turning it off.
const psEl = $('#privacyScreen');
if (psEl) psEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { privacyScreen: on }); if (lastState && lastState.settings) lastState.settings.privacyScreen = on; if (!on) revealedPaths.clear(); renderVaults(true); toast(on ? 'Privacy screen on — names and paths stay blurred until you reveal them' : 'Privacy screen off'); }
	catch (e) { toast(e.message, true); }
});
// Re-hide any revealed cards when the window loses focus or the tab is hidden, so stepping away never leaves a name or
// path exposed. It only touches the DOM when something is actually revealed under the privacy screen.
function reHidePrivacy() { if (privacyScreen && revealedPaths.size) { revealedPaths.clear(); renderVaults(); } }
window.addEventListener('blur', reHidePrivacy);
document.addEventListener('visibilitychange', () => { if (document.hidden) reHidePrivacy(); });
...
const kdlEl = $('#keepDiagnosticLog');
if (kdlEl) kdlEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { keepDiagnosticLog: on }); toast(on ? 'Diagnostic logs are on — a daily log file is kept for troubleshooting' : 'Diagnostic logs off — no activity is written to disk'); }
	catch (e) { toast(e.message, true); }
});
...
function renderUpdateStatus(st, err) {
	...
	if (st.updateAvailable) {
		...
		if (st.releasesUrl && /^https?:\/\//i.test(st.releasesUrl)) {
			...
			// In the desktop app the WebView cannot open a window, so a plain link is a dead click. Open the releases
			// page through the backend in the user's real browser instead. A normal browser keeps the plain link.
			if (st.desktop) a.addEventListener('click', (ev) => { ev.preventDefault(); api('/api/open-releases').catch((e) => toast((e && e.message) || 'Could not open the releases page.', true)); });
			out.appendChild(a);
...
// Peer connection preference: Automatic / Relay only / Direct only.
const p2pModeEl = $('#p2pMode');
if (p2pModeEl) p2pModeEl.addEventListener('change', async (ev) => {
	const v = ev.target.value;
	try { const r = await api('/api/settings', { p2pMode: v }); const set = (r.settings && r.settings.p2pMode) || 'auto'; ev.target.value = set; toast(set === 'relay' ? 'Peers reached through the relay only' : set === 'direct' ? 'Peers reached directly only (no relay fallback)' : 'Peers reached directly when possible, relay otherwise'); }
	catch (e) { toast(e.message, true); }
});

// File-version retention: how many earlier snapshots to keep, plus two optional caps (age, total size). Each posts
// the one field it owns, echoes the server-confirmed value back into the control, and reports the outcome plainly.
const vkEl = $('#versionsKeep');
if (vkEl) vkEl.addEventListener('change', async (ev) => {
	const n = Number(ev.target.value);
	try { const r = await api('/api/settings', { versionsKeep: n }); const set = (r.settings && r.settings.versionsKeep); ev.target.value = String(set); toast(set ? 'Keeping the ' + set + ' most recent versions of a changed file' : 'Past versions off — a changed file keeps no earlier snapshots'); }
	catch (e) { toast(e.message, true); }
});
const vmaEl = $('#versionsMaxAgeDays');
if (vmaEl) vmaEl.addEventListener('change', async (ev) => {
	const n = Number(ev.target.value);
	try { const r = await api('/api/settings', { versionsMaxAgeDays: n }); const set = (r.settings && r.settings.versionsMaxAgeDays); ev.target.value = String(set); toast(set ? 'Also dropping versions older than ' + set + ' days' : 'No age limit on past versions'); }
	catch (e) { toast(e.message, true); }
});
const vmsEl = $('#versionsMaxSizeMB');
if (vmsEl) vmsEl.addEventListener('change', async (ev) => {
	const n = Number(ev.target.value);
	try { const r = await api('/api/settings', { versionsMaxSizeMB: n }); const set = (r.settings && r.settings.versionsMaxSizeMB); ev.target.value = String(set); toast(set ? 'Capping version history at ' + (set >= 1000 ? (set / 1000) + ' GB' : set + ' MB') : 'No size limit on past versions'); }
	catch (e) { toast(e.message, true); }
});

// Auto-restart a WEDGED (hung, not crashed) drive. Off by default: a wedged drive otherwise stays locked and
// secured until recovered by hand. Nothing in the vault is lost either way.
const wrEl = $('#wedgeRestart');
if (wrEl) wrEl.addEventListener('change', async (ev) => {
	const on = ev.target.checked;
	try { await api('/api/settings', { wedgeRestart: on }); toast(on ? 'A wedged drive will be stopped and restarted automatically' : 'Wedged-drive restart off — a hung drive stays locked until you recover it'); }
	catch (e) { ev.target.checked = !on; toast(e.message, true); }
});
...
// Enter in the restore field must trigger Restore. Its button is type="button", and the form's only submit button
// lives in the (hidden) enable panel, so without this an implicit submit would misfire the enable path into a hidden
// result element. preventDefault stops that; the keyboard restore path now matches the enable path.
if ($('#travelRestorePassword')) $('#travelRestorePassword').addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); $('#travelRestoreBtn').click(); } });

// ---- Emergency / inheritance access (dead-man's switch) ----
async function openEmergency(preselectVaultPath) {
	emPreselectVault = preselectVaultPath || null; // applied once by renderEmergencyStatus, so it survives the first-time enroll
	...
	// Start each open from a clean, valid state so a stale (now past) date or unsent trustees never linger.
	if ($('#emArmMode')) $('#emArmMode').value = 'inactivity';
	if ($('#emArmDate')) $('#emArmDate').value = '';
	if ($('#emArmDateEcho')) $('#emArmDateEcho').hidden = true;
	if ($('#emBsTrustees')) $('#emBsTrustees').value = ''; if ($('#emBsK')) $('#emBsK').value = '2'; if ($('#emBsResult')) $('#emBsResult').hidden = true;
	...
	if (s.enrolled) renderEmergencyStatus(s); // applies emPreselectVault; when NOT enrolled, the enroll step runs first and its handler renders (and pre-selects) afterward
	$('#emergencyDialog').showModal();
}
let emContacts = [];
let emPreselectVault = null; // a vault to pre-select in the "Route a vault" picker, carried from a vault's More menu through the first-time enroll
function renderEmergencyStatus(s) {
	...
	if (list) {
		const fmtLocal = (iso) => { try { return new Date(iso).toLocaleString(); } catch (_) { return ''; } };
		list.innerHTML = emContacts.length ? emContacts.map(c => {
			const vaults = (s.armed || []).filter(a => a.contactId === c.id).map(a => esc(a.name) + (a.mode === 'date' && a.opensAt ? ' <span class="fine">(unlocks ' + esc(fmtLocal(a.opensAt)) + ')</span>' : '')).join(', ') || 'no vaults yet';
	...
	const sel = $('#emArmVault'); const prev = sel.value; const vaults = ((lastState && lastState.vaults) || []).filter(v => v.valid && !v.mounted);
	sel.innerHTML = vaults.length ? vaults.map(v => '<option value="' + esc(v.path) + '">' + esc(v.name || v.path) + '</option>').join('') : '<option value="">No closed vaults to protect</option>';
	// Keep the picker on the right vault across a refresh. A vault carried from its More menu (emPreselectVault) wins —
	// including after the first-time enroll — otherwise the current selection is preserved, so clicking Check in, Add,
	// or Set backstop never silently snaps the picker to a different vault (which could route the wrong one).
	const has = (val) => val && Array.from(sel.options).some(o => o.value === val);
	const want = has(emPreselectVault) ? emPreselectVault : (has(prev) ? prev : '');
	if (want) sel.value = want;
	emPreselectVault = null; // consumed
	...
	if (typeof emUpdateArmMode === 'function') emUpdateArmMode(); // set the unlock-date field's initial visibility/default
	const bsEl = $('#emBackstopStatus');
	if (bsEl) bsEl.textContent = s.backstop
		? ('Backstop on: any ' + s.backstop.k + ' of ' + s.backstop.n + ' trustees can recover access if the time-beacon is ever unreachable.')
		: 'Backstop off (optional).';
}
...
onDialogSubmit('#emergencyForm', async () => {
	if (!$('#emergencyEnroll').hidden) {
		...
		try {
			...
			// The button that had focus (the now-hidden enroll "Set up") is gone, so move focus into the revealed manage
			// view rather than letting it fall to <body> — a keyboard or screen-reader user keeps their place in the dialog.
			try { const f = $('#emArmVault'); if (f) f.focus(); } catch (_) {}
	...
	// Manage view: send an implicit submit (Enter in a field) to the primary "Route this vault" action — but ONLY when
	// the focus is in one of the arm fields, so pressing Enter inside the recovery-backstop or add-beneficiary
	// sub-sections never fires the arm by surprise. Without this branch, Enter in the manage view did nothing at all.
	if (!$('#emergencyManage').hidden) {
		const armFields = ['emArmVault', 'emArmContact', 'emArmMode', 'emArmDate', 'emArmPassword'];
		const active = document.activeElement;
		if (active && armFields.includes(active.id) && $('#emArmBtn')) $('#emArmBtn').click();
	}
});
// A local Date -> the value a <input type="datetime-local"> expects (local wall-clock, no zone). Reuses pad2.
...
// Show the release-date field only for the "date" mode, and default it to a month out with a floor of now (so a past
// moment can't be picked). The input is captured in the browser's LOCAL time and converted to an absolute instant on
// submit; the echo confirms the resolved moment so there is never an off-by-a-timezone surprise.
function emUpdateArmMode() {
	const mode = ($('#emArmMode') && $('#emArmMode').value) || 'inactivity';
	const field = $('#emArmDateField'), input = $('#emArmDate'), echo = $('#emArmDateEcho');
	if (!field || !input) return;
	field.hidden = mode !== 'date';
	if (mode === 'date') {
		input.min = toLocalDatetimeValue(new Date(Date.now() + 60000));
		if (!input.value) input.value = toLocalDatetimeValue(new Date(Date.now() + 30 * 86400000));
		emUpdateArmDateEcho();
	} else if (echo) { echo.hidden = true; }
}
function emUpdateArmDateEcho() {
	const input = $('#emArmDate'), echo = $('#emArmDateEcho'); if (!input || !echo) return;
	const t = input.value ? new Date(input.value).getTime() : NaN;
	if (!Number.isFinite(t)) { echo.hidden = true; return; }
	echo.hidden = false;
	echo.className = 'fine' + (t <= Date.now() ? ' bad-text' : '');
	echo.textContent = t <= Date.now() ? 'That moment is in the past — pick a future date.' : 'Unlocks ' + new Date(t).toLocaleString() + ' (your time). It cannot be opened before then, by anyone.';
}
if ($('#emArmMode')) $('#emArmMode').addEventListener('change', emUpdateArmMode);
if ($('#emArmDate')) $('#emArmDate').addEventListener('input', emUpdateArmDateEcho);
if ($('#emArmBtn')) $('#emArmBtn').addEventListener('click', guarded(async () => {
	const path = $('#emArmVault').value, password = $('#emArmPassword').value;
	const contactId = ($('#emArmContact') && $('#emArmContact').value) || '';
	if (!path) return showResult($('#emManageResult'), 'bad', 'There is no closed vault to protect. Close a vault first.');
	...
	// A scheduled-unlock date is captured in local time, validated as a real future moment, and sent as an absolute
	// instant (ms). The server validates it again before turning it into a beacon round, so a bad value cannot slip
	// through from any client.
	const mode = ($('#emArmMode') && $('#emArmMode').value) || 'inactivity';
	const extra = {};
	if (mode === 'date') {
		const v = $('#emArmDate').value;
		const dateMs = v ? new Date(v).getTime() : NaN;
		if (!Number.isFinite(dateMs)) return showResult($('#emManageResult'), 'bad', 'Choose a valid unlock date and time.');
		if (dateMs <= Date.now()) return showResult($('#emManageResult'), 'bad', 'The unlock date must be in the future.');
		extra.mode = 'date'; extra.dateMs = dateMs;
	}
	try {
		const r = await withBusy(() => api('/api/emergency-arm', { path, password, contactId, ...extra }));
		$('#emArmPassword').value = '';
		const s = await api('/api/emergency-status', {}); renderEmergencyStatus(s);
		const when = r.mode === 'date' ? ' It unlocks ' + new Date(r.opensAt).toLocaleString() + ', and cannot be opened before then.' : '';
		showResult($('#emManageResult'), 'ok', 'Routed. Its read access is sealed to ' + esc(r.contactLabel) + '.' + esc(when));
	}
...
if ($('#emAddBtn')) $('#emAddBtn').addEventListener('click', guarded(async () => {
...
// Optional recovery backstop: parse "Label = public key" lines into trustees, and set/clear a k-of-n quorum.
function parseTrusteeLines(text) {
	return String(text || '').split(/\r?\n/).map(l => l.trim()).filter(Boolean).map((l, i) => {
		const eq = l.indexOf('=');
		return eq >= 0 ? { label: l.slice(0, eq).trim() || ('Trustee ' + (i + 1)), pub: l.slice(eq + 1).trim() } : { label: 'Trustee ' + (i + 1), pub: l };
	}).filter(t => t.pub);
}
if ($('#emBsSetBtn')) $('#emBsSetBtn').addEventListener('click', async () => {
	const trustees = parseTrusteeLines($('#emBsTrustees').value);
	const k = Number($('#emBsK').value) || 0;
	if (trustees.length < 2) return showResult($('#emBsResult'), 'bad', 'Enter at least two trustees, one per line, as "Label = public key".');
	if (!(k >= 2 && k <= trustees.length)) return showResult($('#emBsResult'), 'bad', 'The number needed must be between 2 and ' + trustees.length + '.');
	try { const r = await withBusy(() => api('/api/emergency-backstop-set', { trustees, k })); $('#emBsTrustees').value = ''; const s = await api('/api/emergency-status', {}); renderEmergencyStatus(s); showResult($('#emBsResult'), 'ok', 'Backstop set: any ' + esc(String(r.k)) + ' of ' + esc(String(r.n)) + ' trustees can recover access. Re-route a vault to apply it.'); }
	catch (e) { showResult($('#emBsResult'), 'bad', esc(e.message)); }
});
if ($('#emBsOffBtn')) $('#emBsOffBtn').addEventListener('click', async () => {
	if (!(await uiConfirm({ title: 'Turn off recovery backstop', message: 'Turn off the recovery backstop? Vaults already routed keep the backstop they were sealed with — re-route them to drop it.', confirmLabel: 'Turn off', danger: true }))) return;
	try { await api('/api/emergency-backstop-clear', {}); const s = await api('/api/emergency-status', {}); renderEmergencyStatus(s); showResult($('#emBsResult'), 'ok', 'Backstop turned off.'); }
	catch (e) { showResult($('#emBsResult'), 'bad', esc(e.message)); }
});
...
// is to slam the vault shut fast. It closes anything on screen (dialogs and menus) so a shoulder-surfer sees nothing,
// wipes clipboard and on-screen residue (a just-copied password, revealed names), then unmounts every vault (each
// flushed first; a vault with an open file is left mounted rather than risking data loss, exactly like Lock all).
// Cross-platform (metaKey on macOS, ctrlKey elsewhere) and layout-independent (ev.code). It never fires while typing a
// plain key, since it requires the modifier combo.
async function panicLock() {
	...
	// Amnesic: also wipe on-screen and clipboard residue, so a panic leaves nothing decrypted behind either. Clearing the
	// clipboard matters most — a password or a secure-note field copied a moment ago would otherwise still be pasteable.
	// Re-blur any revealed cards so names and paths are hidden again. Both are best-effort (a browser may refuse a
	// clipboard write) and never block the lock, which is the urgent part.
	try { if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText('').catch(() => {}); } catch (_) {}
	try { lastCopied = null; } catch (_) {}
	try { reHidePrivacy(); } catch (_) {}
...
// Undercover disguise: Ctrl/Cmd + Shift + H drops a full-screen innocuous cover (a dim idle clock, no branding) over the
// whole app so a passer-by sees nothing, and locks every vault first via the same panic path — so the disguise is not
// just cosmetic: the keys are already out of memory before the cover is even visible. The same combo toggles it back off,
// which reveals only the locked app. The browser tab title is swapped to a neutral word while it is up and restored after.
// Cross-platform (metaKey on macOS, ctrlKey elsewhere) and layout-independent (ev.code). Best-effort and never throws.
let undercoverTimer = null;
let undercoverPrevTitle = null;
function ucPad(n) { return (n < 10 ? '0' : '') + n; }
function undercoverTick() {
	const el = $('#ucClock'); if (!el) return;
	const d = new Date();
	el.textContent = ucPad(d.getHours()) + ':' + ucPad(d.getMinutes());
}
function undercoverOn() {
	if (undercoverActive) return;
	undercoverActive = true;
	// Lock and wipe first (same amnesic panic path), so nothing decrypted survives behind the cover. Fire-and-forget:
	// showing the cover must not wait on the network, and the toast it would raise is suppressed while undercover is up.
	try { panicLock(); } catch (_) {}
	try { hideToast(); } catch (_) {}
	const el = $('#undercover');
	if (el) { el.hidden = false; el.setAttribute('aria-hidden', 'false'); }
	undercoverTick();
	try { clearInterval(undercoverTimer); } catch (_) {}
	undercoverTimer = setInterval(undercoverTick, 15000); // HH:MM only, so a slow tick is plenty and stays off the hot path
	try { undercoverPrevTitle = document.title; document.title = 'Untitled'; } catch (_) {}
}
function undercoverOff() {
	if (!undercoverActive) return;
	undercoverActive = false;
	try { clearInterval(undercoverTimer); } catch (_) {} undercoverTimer = null;
	const el = $('#undercover');
	if (el) { el.hidden = true; el.setAttribute('aria-hidden', 'true'); }
	try { if (undercoverPrevTitle != null) document.title = undercoverPrevTitle; } catch (_) {}
	undercoverPrevTitle = null;
}
function undercoverToggle() { if (undercoverActive) undercoverOff(); else undercoverOn(); }
document.addEventListener('keydown', (ev) => {
	if ((ev.metaKey || ev.ctrlKey) && ev.shiftKey && (ev.code === 'KeyH' || (ev.key || '').toLowerCase() === 'h')) { ev.preventDefault(); undercoverToggle(); }
});
...
// ---- Run a relay hub (start/stop the hub on this machine; show a one-paste invite code + QR) ----
// The invite code is built HERE in the browser from the host you type + the running hub's token, so it updates live
// and matches the CLI's makeRelayCode byte-for-byte (same shape). Base64 is UTF-8-safe and cross-browser.
let relayHubToken = ''; // the running hub's token (from the server)
let relayHubPort = 7443; // the running hub's control port (from the server)
function encodeRelayCode(host, token) {
	const json = JSON.stringify({ v: 1, t: 'relay', host: String(host || ''), token: String(token || '') });
	try { return btoa(unescape(encodeURIComponent(json))); } catch (_) { return ''; }
}
function relayHubMsg(kind, text) { const m = $('#relayHubMsg'); if (!m) return; if (!text) { m.hidden = true; m.textContent = ''; return; } m.hidden = false; m.className = 'tamper-result ' + (kind || ''); m.setAttribute('aria-live', 'polite'); m.textContent = text; } // textContent (not innerHTML) — the message is set safely regardless of source
function refreshRelayInvite() {
	const host = (($('#relayHubHost') && $('#relayHubHost').value) || '').trim();
	const hasHost = !!host && !!relayHubToken;
	if ($('#relayHubInviteBlock')) $('#relayHubInviteBlock').hidden = !hasHost;
	if ($('#relayHubNoHost')) $('#relayHubNoHost').hidden = hasHost;
	if (!hasHost) return;
	const hostPort = host.includes(':') ? host : host + ':' + relayHubPort;
	const code = encodeRelayCode(hostPort, relayHubToken);
	if ($('#relayHubCode')) $('#relayHubCode').value = code;
	if ($('#relayHubQr')) renderQr($('#relayHubQr'), code);
}
function renderRelayHub(info) {
	const running = !!(info && info.running);
	if ($('#relayHubIdle')) $('#relayHubIdle').hidden = running;
	if ($('#relayHubRunning')) $('#relayHubRunning').hidden = !running;
	if (running) {
		relayHubToken = info.token || '';
		relayHubPort = info.controlPort || 7443;
		if ($('#relayHubPort')) $('#relayHubPort').textContent = String(info.controlPort || '');
		if ($('#relayHubRange')) $('#relayHubRange').textContent = (info.portRange || []).join('–');
		if ($('#relayHubToken')) $('#relayHubToken').textContent = info.token || '';
		refreshRelayInvite();
	} else { relayHubToken = ''; }
}
async function openRelayHub() {
	relayHubMsg('', ''); renderRelayHub({ running: false });
	$('#relayHubDialog').showModal();
	try { renderRelayHub(await api('/api/relay-hub-status', {})); } catch (e) { relayHubMsg('bad', e.message); }
}
if ($('#relayHubBtn')) $('#relayHubBtn').addEventListener('click', openRelayHub);
if ($('#relayHubHost')) $('#relayHubHost').addEventListener('input', refreshRelayInvite);
if ($('#relayHubStart')) $('#relayHubStart').addEventListener('click', guarded(async () => {
	relayHubMsg('', 'Starting the relay hub…');
	try { renderRelayHub(await api('/api/relay-hub-start', {})); relayHubMsg('ok', 'The hub is running. Open its ports in this machine’s firewall, then share the invite code.'); }
	catch (e) { relayHubMsg('bad', e.message); }
}));
if ($('#relayHubStop')) $('#relayHubStop').addEventListener('click', guarded(async () => {
	try { renderRelayHub(await api('/api/relay-hub-stop', {})); relayHubMsg('', ''); } catch (e) { relayHubMsg('bad', e.message); }
}));
if ($('#relayHubCopy')) $('#relayHubCopy').addEventListener('click', async () => {
	const code = $('#relayHubCode') && $('#relayHubCode').value; if (!code) return;
	try { await navigator.clipboard.writeText(code); toast('Copied'); } catch (_) { try { $('#relayHubCode').select(); } catch (_2) {} }
});

// ---- Storage node: offer this machine as encrypted off-site storage for other people ----
function storeNodeMsg(kind, text) { const m = $('#storeNodeMsg'); if (!m) return; if (!text) { m.hidden = true; m.textContent = ''; return; } m.hidden = false; m.className = 'tamper-result ' + (kind || ''); m.setAttribute('aria-live', 'polite'); m.textContent = text; }
function renderStoreNode(info) {
	const running = !!(info && info.serving);
	if ($('#storeNodeIdle')) $('#storeNodeIdle').hidden = running;
	if ($('#storeNodeRunning')) $('#storeNodeRunning').hidden = !running;
	if (running) {
		const bindMap = { lan: 'This network', wan: 'This network + the internet', relay: 'A relay hub' };
		if ($('#storeNodeBindLabel')) $('#storeNodeBindLabel').textContent = (bindMap[info.bind] || info.bind || '—') + (info.secure ? ' (encrypted)' : '');
		if ($('#storeNodeUsage')) $('#storeNodeUsage').textContent = fmtBytes(info.bytes || 0) + ' across ' + (info.slots || 0) + ' ' + (info.slots === 1 ? 'person' : 'people');
	}
	if ($('#storeNodeQuota') && document.activeElement !== $('#storeNodeQuota')) $('#storeNodeQuota').value = String((info && info.quotaMB) || 0);
}
function renderStoreSlots(slots) {
	const el = $('#storeNodeSlots'); if (!el) return;
	if (!slots || !slots.length) { el.innerHTML = '<div class="muted">No one is using your storage yet — invite someone above.</div>'; return; }
	el.innerHTML = slots.map((s) => '<div class="dev-row"><div class="dev-main"><div class="dev-label">' + esc(s.label || 'Unnamed') + '</div><div class="dev-sub">' + esc(fmtBytes(s.bytes || 0)) + (s.createdAt ? ' · since ' + esc(new Date(s.createdAt).toLocaleDateString()) : '') + '</div></div><button type="button" class="ghost-btn danger" data-store-remove="' + esc(s.slot) + '" aria-label="Remove ' + esc(s.label || 'this person') + '">Remove</button></div>').join('');
}
async function refreshStoreSlots() { try { renderStoreSlots((await api('/api/store-slots', {})).slots); } catch (_) {} }
async function openStoreNode() {
	storeNodeMsg('', ''); if ($('#storeNodeInviteBlock')) $('#storeNodeInviteBlock').hidden = true;
	renderStoreNode({ serving: false });
	$('#storeNodeDialog').showModal();
	try { renderStoreNode(await api('/api/store-status', {})); await refreshStoreSlots(); } catch (e) { storeNodeMsg('bad', e.message); }
}
if ($('#storeNodeBtn')) $('#storeNodeBtn').addEventListener('click', openStoreNode);
if ($('#storeNodeStart')) $('#storeNodeStart').addEventListener('click', guarded(async () => {
	storeNodeMsg('', 'Starting the storage node…');
	try { renderStoreNode(await api('/api/store-serve-start', { bind: $('#storeNodeBind').value })); await refreshStoreSlots(); storeNodeMsg('ok', 'Running. Invite someone to get their one-paste backup code.'); }
	catch (e) { storeNodeMsg('bad', e.message); }
}));
if ($('#storeNodeStop')) $('#storeNodeStop').addEventListener('click', guarded(async () => {
	try { renderStoreNode(await api('/api/store-serve-stop', {})); if ($('#storeNodeInviteBlock')) $('#storeNodeInviteBlock').hidden = true; storeNodeMsg('', ''); } catch (e) { storeNodeMsg('bad', e.message); }
}));
if ($('#storeNodeInviteBtn')) $('#storeNodeInviteBtn').addEventListener('click', guarded(async () => {
	try {
		const r = await api('/api/store-invite', { label: ($('#storeNodeInviteLabel').value || '').trim() });
		$('#storeNodeInviteCode').value = r.code; $('#storeNodeInviteBlock').hidden = false; $('#storeNodeInviteLabel').value = '';
		await refreshStoreSlots(); renderStoreNode(await api('/api/store-status', {}));
	} catch (e) { storeNodeMsg('bad', e.message); }
}));
if ($('#storeNodeInviteCopy')) $('#storeNodeInviteCopy').addEventListener('click', async () => {
	const code = $('#storeNodeInviteCode') && $('#storeNodeInviteCode').value; if (!code) return;
	try { await navigator.clipboard.writeText(code); toast('Copied'); } catch (_) { try { $('#storeNodeInviteCode').select(); } catch (_2) {} }
});
if ($('#storeNodeQuotaSave')) $('#storeNodeQuotaSave').addEventListener('click', guarded(async () => {
	try { const r = await api('/api/store-quota', { quotaMB: Number($('#storeNodeQuota').value) || 0 }); toast(r.quotaMB ? 'Limit set to ' + r.quotaMB + ' MB' : 'No size limit'); renderStoreNode(await api('/api/store-status', {})); } catch (e) { storeNodeMsg('bad', e.message); }
}));
if ($('#storeNodeSlots')) $('#storeNodeSlots').addEventListener('click', async (e) => {
	const btn = e.target.closest('[data-store-remove]'); if (!btn) return;
	if (!(await uiConfirm({ title: 'Remove this person’s storage', message: 'Delete this person’s encrypted backup from your machine? They will need a new invite to back up here again. (You can never read it either way.)', confirmLabel: 'Remove', danger: true }))) return;
	try { await api('/api/store-slot-remove', { slot: btn.dataset.storeRemove }); await refreshStoreSlots(); renderStoreNode(await api('/api/store-status', {})); } catch (err) { storeNodeMsg('bad', err.message); }
});
...
// Open the (collapsible) create panel and focus its first field. Shared by the header "New vault" button and the
// first-run empty-state "Create your first vault" button, so both entry points behave identically.
function focusCreate() {
	const cp = $('#createPanel'); if (cp) cp.open = true; // the create panel collapses once you have vaults, so open it first
	const input = $('#createPath');
	if (input) { input.scrollIntoView({ behavior: scrollBehavior(), block: 'center' }); input.focus({ preventScroll: true }); }
}
if ($('#newVaultBtn')) $('#newVaultBtn').addEventListener('click', focusCreate);
if ($('#emptyCreateBtn')) $('#emptyCreateBtn').addEventListener('click', focusCreate);

// "Remove all unreachable": forget every vault that can't be read right now, in one confirmed action. It reuses the
// same per-vault /api/remove-vault call the single-card Remove uses, looped over the current unreachable set — so it
// forgets entries only and never deletes files. The list is recomputed from the latest state at click time (never a
// stale closure), removals run sequentially so one failure doesn't abandon the rest, and the list refreshes once at
// the end.
if ($('#removeUnreachableBtn')) $('#removeUnreachableBtn').addEventListener('click', async () => {
	const stale = ((lastState && lastState.vaults) || []).filter(v => !v.valid);
	if (!stale.length) return;
	const okd = await uiConfirm({
		title: 'Remove all unreachable',
		message: 'Remove all ' + stale.length + ' unreachable vaults from this list? This only forgets them here — your encrypted files are NOT deleted, and you can add any of them back later by its folder.',
	...
	await withBusy(async () => {
		for (const v of stale) {
			try { await api('/api/remove-vault', { path: v.path }); removed++; }
			catch (_) { failed++; }
		}
	});
	toast(failed ? ('Removed ' + removed + ', ' + failed + ' could not be removed') : ('Removed ' + removed + ' from the list'), !!failed);
	refresh();
});
...
initTheme();
initDashboardPanel(); // remember the overview's collapsed state and wire its tile actions
initDevices();        // wire the Devices & connections dialog (Check all, per-row test/remove)
initFleet();          // wire the Fleet dialog (add node, per-row remove)
initColorPickers();   // close a vault's color-tag popover on an outside click
...
nameDialogs();
// Give every labeled field a clean accessible NAME and, where present, a separate DESCRIPTION. In this UI the visible
