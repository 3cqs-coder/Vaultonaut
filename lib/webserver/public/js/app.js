...
async function openRecoveryKit(addKey) {
	...
	try {
		...
		// The token is SINGLE-USE, so exactly ONE thing may load it. Point the window opened above at the kit URL if we
		// got one (a real browser); otherwise show the in-app viewer, whose iframe is then the sole loader. We must NOT
		// try a second window.open here as a middle step — in a WebView that fires a request that consumes the token,
		// leaving the iframe to load the already-spent URL (a blank/expired view). Report success only when it displayed.
		let inApp = false;
		if (win && !win.closed) { win.location = url; win.focus(); }
		else { showKitInApp(url, addKey); inApp = true; }
		keysShow('ok', addKey
