...
onDialogSubmit('#searchForm', async () => {
	...
	try {
		if (searchScope() === 'names') {
			...
			results.innerHTML = matches.map(m => '<div class="search-hit search-hit-open" dir="auto" data-rel="' + esc(m) + '" title="Open ' + esc(m) + '">' + esc(m) + '</div>').join('');
			...
			results.innerHTML = res.results.map(m => '<div class="search-hit search-hit-open" dir="auto" data-rel="' + esc(m.path) + '" title="Open ' + esc(m.path) + '">' + esc(m.path) + '</div>').join('');
