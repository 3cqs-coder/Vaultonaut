...
onDialogSubmit('#tamperForm', async () => {
	...
	try {
		...
		tamperShow('bad', lead + tamper +
			fileList('Modified', r.modified) + fileList('Removed', r.removed) + fileList('Added', r.added) + fileList('Unrecognized — added from outside the vault', r.foreign) + fileList('Damaged — present but their content cannot be read', r.damaged) + fingerprintLine(r) + identityBlock(r) + sync);
	} catch (e) { tamperShow('bad', esc(e.message)); }
});
