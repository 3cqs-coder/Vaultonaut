...
function recoveryBadge(v) {
	const r = v.recovery;
	if (!r) return '';
	if (r.refreshing) return ` <span class="badge updating" title="Rebuilding recovery data after your changes">updating ${r.refreshPercent != null ? r.refreshPercent + '%' : '…'}</span>`;
	if (r.protected) {
