...
function renderEmergencyStatus(s) {
	...
	if (list) {
		list.innerHTML = emContacts.length ? emContacts.map(c => {
			...
			return '<div class="em-contact"><span class="nm"><strong>' + esc(c.label) + '</strong> — ' + vaults + '</span> <button type="button" class="link-btn danger" data-act="em-remove-contact" data-id="' + esc(c.id) + '" data-label="' + esc(c.label) + '">Remove</button></div>';
