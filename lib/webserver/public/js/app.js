...
async function mobileOpenHere() {
	...
	// In the desktop app the WebView cannot open a window, so the backend already opened the viewer in the user's real
	// browser (r.opened). Leave the app window in place rather than navigating it away to the viewer.
	if (r.opened) { if (tab && !tab.closed) tab.close(); toast('Opened the viewer in your browser.'); $('#mobileDialog').close(); return; }
