...
async function openKeys(path, name) {
	...
	// Full key rotation is not yet available for cloud vaults (it would re-encrypt and re-upload the whole store), so
	// hide the rotate action for a cloud vault and explain why, rather than offering a button that only errors.
	const keysCloud = !!(vaultsByPath[path] || {}).cloud;
	if ($('#keysRotate')) $('#keysRotate').hidden = keysCloud;
