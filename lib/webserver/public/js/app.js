...
// The path whose content index we have already brought up to date in THIS search-dialog session. An "inside files"
// search first refreshes the index once (incrementally — only files added or changed since the last index are read,
// with OCR when it is on), so a file added while the vault was open is findable without pressing "Update index". We do
// it once per dialog open, not on every keystroke, so refining a query does not re-walk the vault each time.
let contentIndexFreshFor = null;
...
function openSearch(path, name) {
	searchTarget = path;
	contentIndexFreshFor = null; // a fresh dialog session: the content index is refreshed on the first inside-files search
...
// Bring the content index up to date once per dialog session before an inside-files search, so a file added since the
// last index is searchable without a manual "Update index". The refresh is incremental (only new/changed files are
// read) and runs in the index worker thread; it streams the same progress bar as the manual button. It is best-effort:
// a failure (a read-only mount, a wedged mount) is swallowed and the search proceeds against whatever index exists, so
// the refresh can never block or break a search. A pressed "Update index" marks the index fresh too (below), so the
// two never double-index in one session.
async function ensureContentIndexFresh() {
	if (contentIndexFreshFor === searchTarget) return;
	$('#searchProgress').hidden = false; searchProgress({ indeterminate: true, label: 'Updating index' });
	try { await withBusy(() => apiStream('/api/content-reindex', { path: searchTarget }, searchProgress)); contentIndexFreshFor = searchTarget; }
...
$('#searchIndexBtn').addEventListener('click', guarded(async (ev) => {
	...
	try {
		...
		contentIndexFreshFor = searchTarget; // just indexed — a following search this session need not refresh again
