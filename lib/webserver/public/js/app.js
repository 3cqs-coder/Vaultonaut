...
function noteEditor(id, title, body, meta) {
	notesCurrentId = id || null;
	$('#noteTitle').value = title || '';
	$('#noteBody').value = body || '';
	$('#noteDelete').hidden = !id;
	// Show created/updated for a saved note. The timestamps come from the note's own (encrypted) metadata, so they are
	// only known for a note that was opened; a fresh, unsaved note shows nothing.
	const m = $('#noteMeta');
	if (m) {
		const fmt = (t) => { try { return new Date(t).toLocaleString(); } catch (_) { return ''; } };
		const parts = [];
		if (meta && meta.createdAt) parts.push('Created ' + fmt(meta.createdAt));
		if (meta && meta.updatedAt) parts.push('Updated ' + fmt(meta.updatedAt));
...
document.addEventListener('click', async (e) => {
	...
	if (!item) return;
	try { const { note } = await api('/api/note-get', { path: notesTarget, id: item.dataset.noteId }); noteEditor(note.id, note.title, note.body, { createdAt: note.createdAt, updatedAt: note.updatedAt }); $('#notesResult').hidden = true; await loadNotesList(); }
...
onDialogSubmit('#notesForm', async () => {
	...
	try {
		...
		noteEditor(r.id, title, body, { createdAt: r.createdAt, updatedAt: r.updatedAt }); // reflect the fresh created/updated times
		notesShow('ok', 'Saved.');
		await loadNotesList();
	} catch (err) { notesShow('bad', esc(err.message)); }
});
