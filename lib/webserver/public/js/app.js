...
function renderNotesList() {
	const el = $('#notesList');
	const q = (($('#notesFilter') && $('#notesFilter').value) || '').trim().toLowerCase();
	const icon = (t) => VS ? VS.iconFor(t) : '🔒';
	const shown = q ? notesAll.filter(n => (n.title || '').toLowerCase().indexOf(q) >= 0 || (n.type || '').toLowerCase().indexOf(q) >= 0) : notesAll;
	if (!notesAll.length) { el.innerHTML = '<div class="muted">No items yet. Start a new one below and Save.</div>'; return; }
	el.innerHTML = shown.length
		? shown.map(n => `<button type="button" class="note-item${n.id === notesCurrentId ? ' active' : ''}" data-note-id="${esc(n.id)}"><span class="note-ic" aria-hidden="true">${icon(n.type)}</span> <span class="note-nm">${esc(n.title || '(untitled)')}</span></button>`).join('')
...
function renderHealthPanel(r) {
	...
	if (!clean) {
		html += '<ul class="health-items">' + r.items.map(it => {
			...
			return `<li><button type="button" class="note-item health-open" data-note-id="${esc(it.id)}"><span class="note-nm">${esc(it.title || '(untitled)')}</span></button> ${tags}</li>`;
		}).join('') + '</ul>';
	}
...
function buildFieldRow(field) {
	...
	kindSel.innerHTML = kinds.map(k => `<option value="${k}"${k === f.kind ? ' selected' : ''}>${esc(KIND_LABELS[k] || k)}</option>`).join('');
	...
	function paintValue(kind, value) {
		...
		if (info.totp) { const t = document.createElement('div'); t.className = 'sf-totp'; valWrap.appendChild(t); wireTotp(row, () => input.value, t); }
	}
...
function wireTotp(row, getValue, box) {
	clearRowTotp(row);
	async function tick() {
		if (!VS) { box.textContent = ''; return; }
		const cfg = VS.parseTotp(getValue());
		if (!cfg.secret) { box.textContent = ''; return; }
		try { const r = await VS.totpCode(cfg); box.innerHTML = ''; const code = document.createElement('span'); code.className = 'totp-code'; code.textContent = r.code.replace(/(\d{3})(\d+)/, '$1 $2'); const ring = document.createElement('span'); ring.className = 'totp-secs'; ring.textContent = r.secondsRemaining + 's'; const cp = mkBtn('Copy', 'Copy code', () => copySecretValue(r.code)); box.appendChild(code); box.appendChild(ring); box.appendChild(cp); }
		catch (_) { box.textContent = ''; } // unusable secret or no secure context — show nothing rather than an error
	}
...
(function wireNotesIdle() {
	...
	if ($('#notesFilter')) $('#notesFilter').addEventListener('input', renderNotesList);
...
if ($('#sendCreate')) $('#sendCreate').addEventListener('click', async () => {
	try {
