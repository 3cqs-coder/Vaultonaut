...
async function openEmergency(preselectVaultPath) {
	...
	if (s.enrolled) {
		renderEmergencyStatus(s);
		// Opened from a specific vault's menu: pre-select it in the "Route a vault" picker so setup is one step. (The
		// menu item is disabled while a vault is mounted or for a cloud vault, so a routable vault is in the list here.)
		if (preselectVaultPath) { const sel = $('#emArmVault'); if (sel && Array.from(sel.options).some(o => o.value === preselectVaultPath)) sel.value = preselectVaultPath; }
	}
