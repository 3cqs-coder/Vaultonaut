...
onDialogSubmit('#searchForm', async () => {
	...
	try {
		if (searchScope() === 'names') {
			...
			results.innerHTML = matches.map(m => '<div class="search-hit' + (mounted ? ' search-hit-open' : '') + '" dir="auto"' + (mounted ? ' data-rel="' + esc(m) + '"' : '') + ' title="' + (mounted ? 'Open ' : '') + esc(m) + '">' + esc(m) + '</div>').join('');
...
// Open a search result in its default app (an image in the previewer, a PDF in the reader). The file lives inside the
// mounted vault, so it is opened by its path relative to the mount; the server confines that path to the vault and only
// opens on the machine running the app. A name-only hit on an unmounted vault carries no data-rel, so it stays inert.
$('#searchResults').addEventListener('click', async (e) => {
	const hit = e.target.closest('.search-hit-open'); if (!hit) return;
	const rel = hit.dataset.rel; if (rel == null) return;
	try { const r = await api('/api/open-match', { path: searchTarget, rel }); if (r && r.ok === false) toast(r.error || 'Could not open that file.', true); }
...
async function mobileOpenHere() {
	...
	// In the desktop app the single WebView has no second tab, so the backend asks us to show the viewer IN the app
	// window itself (r.inApp): close any stray blank tab the gesture trick opened, then navigate this window to the
	// viewer. Its Back control returns to the vault list. A plain browser instead opens the viewer in its own tab.
