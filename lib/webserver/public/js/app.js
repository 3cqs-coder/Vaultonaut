...
function notesResetIdle() {
	...
	notesIdleTimer = setTimeout(async () => {
		try {
			...
			if (title || body) {
				try { const r = await api('/api/note-save', { path: notesTarget, id: notesCurrentId, title, body }); notesCurrentId = r.id; }
				catch (_) { toast('A note in progress could not be auto-saved', true); return; } // save failed — leave the dialog open so the user can retry rather than lose the text
			}
			...
			toast(title || body ? 'Notes closed after inactivity — your note was saved' : 'Notes closed after inactivity');
		} catch (_) {}
	}, NOTES_IDLE_MS);
}
...
// Recovery Kit: a printable one-page kit. It can carry a one-time recovery key, so it is served ONCE from the backend
// over a single-use token URL (never returned in the API JSON, never written to disk) and rendered under its own
// kit CSP. "Recovery Kit" always includes a spare recovery key and so needs the current password; "Identity-only kit"
// records just the vault's identity and fingerprint, changes nothing, and needs no password. One token URL serves
// both a real browser window AND — where a WebView cannot open one (the desktop app, or a pop-up blocker) — the
// in-app viewer, so the feature works standalone, in every browser, and in the desktop app from one code path.
async function openRecoveryKit(addKey) {
	...
	try {
		...
		const url = location.origin + r.viewUrl; // an absolute, same-origin single-use token URL (absolute so an about:blank window resolves it)
		// Point the window (opened above) at the kit URL; if the WebView could not open one, fall back to the in-app
		// viewer. Report success only when it actually displayed, so a shown-once key is never silently swallowed.
		let inApp = false;
		if (win && !win.closed) { win.location = url; win.focus(); }
		else if (!openInNewTab(url)) { showKitInApp(url, addKey); inApp = true; }
		keysShow('ok', addKey
			? (inApp ? 'Recovery Kit shown below — it includes a spare recovery key, so print it or save the PDF now, then store it somewhere safe and private.' : 'Recovery Kit opened in a separate window — it includes a spare recovery key, so print it or save the PDF, then store it somewhere safe and private.')
			: (inApp ? 'Identity‑only Recovery Kit shown below — it records the vault’s identity and fingerprint but no key, so it changes nothing and needs no password.' : 'Identity‑only Recovery Kit opened in a separate window — it records the vault’s identity and fingerprint but no key, so it changes nothing and needs no password.'));
		if (addKey) await loadKeys();
	} catch (e) { if (win && !win.closed) win.close(); keysShow('bad', esc(e.message)); }
}
// Try to open a same-origin URL in a new browser tab/window; returns true on success. Returns false where a WebView
// blocks it (the desktop app), so the caller can fall back to the in-app viewer.
function openInNewTab(url) { try { return !!window.open(url, '_blank', 'noopener'); } catch (_) { return false; } }
// Show the Recovery Kit inside the app, used when a separate window is unavailable (the desktop app, or a pop-up
// blocker). The kit loads from its single-use token URL into an iframe, so it renders under the endpoint's own CSP
// (a srcdoc/blob frame would inherit the app's strict CSP and lose the kit's inline styles). Print uses the iframe's
// OWN print so only the kit prints. Closing clears the frame so a one-time recovery key never lingers in the page.
function showKitInApp(url, addKey) {
	const dlg = $('#kitDialog'), frame = $('#kitFrame'), note = $('#kitNote');
	if (!dlg || !frame) return;
	if (note) note.textContent = addKey
		? 'This kit includes a spare recovery key. Print it or save it as a PDF now, then store it somewhere safe and private. It is shown only here — closing this clears it.'
		: 'This kit records the vault’s identity and fingerprint only — no key. Print it or save it as a PDF for your records.';
	frame.src = url;
	if (typeof dlg.showModal === 'function') dlg.showModal(); else dlg.setAttribute('open', '');
}
function closeKitDialog() {
	const dlg = $('#kitDialog'), frame = $('#kitFrame');
	if (frame) frame.src = 'about:blank'; // drop the kit (and any one-time key) from the page as soon as it is closed
	if (dlg) { if (typeof dlg.close === 'function') dlg.close(); else dlg.removeAttribute('open'); }
}
if ($('#kitPrint')) $('#kitPrint').addEventListener('click', () => {
	const frame = $('#kitFrame');
	// Print just the kit via the iframe's own window. Where a WebView does not support printing, this is a no-op and
	// the kit stays readable on screen — never an error.
	try { if (frame && frame.contentWindow) { frame.contentWindow.focus(); frame.contentWindow.print(); } } catch (_) {}
});
if ($('#kitClose')) $('#kitClose').addEventListener('click', closeKitDialog);
if ($('#kitDialog')) $('#kitDialog').addEventListener('cancel', (ev) => { ev.preventDefault(); closeKitDialog(); }); // Escape closes and clears the frame too
