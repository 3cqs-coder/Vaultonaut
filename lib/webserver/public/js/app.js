...
function renderEmergencyStatus(s) {
	...
	// Apply a vault carried from its More menu ONCE (then clear it), so opening from a vault pre-selects it — including
	// after the first-time enroll — without overriding a manual choice on a later refresh.
	if (emPreselectVault) { if (Array.from(sel.options).some(o => o.value === emPreselectVault)) sel.value = emPreselectVault; emPreselectVault = null; }
