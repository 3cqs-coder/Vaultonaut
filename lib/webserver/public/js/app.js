...
// Read text in scans and images (OCR): the language file downloads on the next manual index update.
const ocrEl = $('#ocrToggle');
if (ocrEl) ocrEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { ocr: on }); toast(on ? 'Text recognition on — use "Update index" on a vault to search inside its scans and images' : 'Text recognition off'); }
