...
if ($('#anonymizeBtn')) $('#anonymizeBtn').addEventListener('click', guarded(async () => {
	...
	const okd = await uiConfirm({ title: 'Hide folder name', message: 'Rename this vault’s folder to an opaque id? Your files, keys, and password are unchanged — only the name on disk changes, and the name you set stays visible here.', confirmLabel: 'Hide it', danger: true });
	if (!okd) return;
	try {
