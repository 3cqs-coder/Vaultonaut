...
// the list and wipe an in-progress button state (or invite a double action).
let busy = false;
async function withBusy(fn) { busy = true; try { return await fn(); } finally { busy = false; } }
...
function hideToast() { const t = $('#toast'); if (!t) return; clearTimeout(toast._t); try { t.hidePopover(); } catch (_) {} t.hidden = true; }
...
// a future single-quoted one from becoming an injection point.
...
function showResult(el, kind, html) { el.className = 'tamper-result ' + (kind || ''); el.setAttribute('aria-live', 'polite'); el.hidden = false; el.innerHTML = html; } // live region + content set while visible, so a screen reader announces the result
...
function vaultCard(v) {
	...
	const badge = !v.valid ? '<span class="badge invalid">invalid</span>'
	...
	if (v.valid) {
		...
		more.push(moreBtn(v, 'mobile', 'View files', { needsLocal: true }));
...
// The "storage used" line under a vault's path. Offered ONLY while the vault is mounted, and it measures the
// mounted (decrypted) contents — never the encrypted folder on disk. That keeps decoys deniable: an unmounted vault
// never shows a footprint, and a decoy unlock reports the decoy's size. On-demand: a full walk, so it never runs on
...
// the HTML actually changed, and never while a "More" menu is open.
function renderVaults() {
	...
	if (!list.querySelector('details.more[open]')) {
...
async function refresh() {
	if (busy || refreshing) return; // skip while an action is in flight, or while a previous (slow) poll is still outstanding — so a late response can't overwrite a newer render
	refreshing = true;
	try {
...
async function forceUnmount(target, name, { message, success = 'Recovered', btn } = {}) {
	...
	try { if (btn) btn.disabled = true; await withBusy(() => api('/api/unmount', { target, force: true })); toast(success); refresh(); return 'ok'; }
...
function openMount(path, name) {
	...
	if (kfBtn) kfBtn.hidden = !hasKeyfile;
...
async function doMount(password, over = {}) {
	...
	try {
		...
		$('#mountDialog').close();
		const note = r.mode === 'off' ? ' (streaming — no cache)' : (r.inRam ? ' (buffer in memory)' : '');
...
async function loadMembers() {
	const r = await api('/api/members', { path: membersTarget });
	if (!r.team) {
	...
	const rows = r.members.map(m => {
		...
		return '<li><span class="nm">' + esc(m.label || 'Member') + '</span> <span class="badge">' + (m.owner ? 'owner' : (m.role === 'write' ? 'read-write' : 'read-only')) + '</span> ' + idLine +
...
async function loadNotesList() {
	...
	el.innerHTML = notes.length
		? notes.map(n => `<button type="button" class="note-item${n.id === notesCurrentId ? ' active' : ''}" data-note-id="${esc(n.id)}">${esc(n.title || '(untitled)')}</button>`).join('')
		: '<div class="muted">No notes yet. Write one below and Save.</div>';
}
function noteEditor(id, title, body) {
	notesCurrentId = id || null;
...
function notesResetIdle() {
	...
	notesIdleTimer = setTimeout(() => { try { noteEditor(null, '', ''); dlg.close(); toast('Notes closed after inactivity'); } catch (_) {} }, NOTES_IDLE_MS);
}
...
(function wireNotesIdle() {
	...
	dlg.addEventListener('close', () => { if (notesIdleTimer) { clearTimeout(notesIdleTimer); notesIdleTimer = null; } noteEditor(null, '', ''); });
})();
...
onDialogSubmit('#searchForm', async () => {
	...
	try {
		if (searchScope() === 'names') {
			...
			results.innerHTML = matches.map(m => '<div class="search-hit" title="' + esc(m) + '">' + esc(m) + '</div>').join('');
			...
			results.innerHTML = res.results.map(m => '<div class="search-hit" title="' + esc(m.path) + '">' + esc(m.path) + '</div>').join('');
...
document.addEventListener('click', async (e) => {
	const item = e.target.closest('#notesList .note-item');
	if (!item) return;
	try { const { note } = await api('/api/note-get', { path: notesTarget, id: item.dataset.noteId }); noteEditor(note.id, note.title, note.body); $('#notesResult').hidden = true; await loadNotesList(); }
	catch (err) { notesShow('bad', esc(err.message)); }
});
$('#noteNew').addEventListener('click', () => { noteEditor(null, '', ''); loadNotesList().catch(() => {}); $('#noteTitle').focus(); });
onDialogSubmit('#notesForm', async () => {
	const title = $('#noteTitle').value, body = $('#noteBody').value;
	if (!title && !body) return toast('Write a title or some text first', true);
	try {
		const r = await withBusy(() => api('/api/note-save', { path: notesTarget, id: notesCurrentId, title, body }));
...
$('#noteDelete').addEventListener('click', async () => {
	...
	const okd = await uiConfirm({ title: 'Delete note', message: 'Permanently delete this secure note? This cannot be undone.', confirmLabel: 'Delete', danger: true });
	if (!okd) return;
...
async function loadKeys() {
	const { slots, level, recoveryReminder } = await api('/api/keys', { path: keysTarget });
...
// Recovery Kit: a printable one-page kit opened in its own window (it can carry a recovery key, so it never
// renders inside the app). The choice is now explicit, not inferred from whether a field happens to be filled:
// "Recovery Kit" always includes a spare recovery key and so requires the current password to authorize it;
// "Identity-only kit" records just the vault's identity and fingerprint, changes nothing, and needs no password.
// The window is opened synchronously inside the click so a pop-up blocker never swallows it.
async function openRecoveryKit(addKey) {
	const win = window.open('', '_blank'); // opened synchronously on the click so a pop-up blocker never swallows it
	let password = '';
	if (addKey) {
		...
		if (!password) { if (win && !win.closed) win.close(); return; }
	}
	try {
		const r = await withBusy(() => api('/api/recovery-kit', { path: keysTarget, password, addKey }));
		// Render the kit into its own window. It may carry a one-time recovery key, so it is never written into the
		// app's own document. If the synchronous window could not be created (some embedded views return null), fall
		// back to a same-origin blob URL. Only report success when it actually displayed — never claim "opened" when
		// nothing was shown, which would hide a shown-once key.
		let shown = false;
		if (win && !win.closed) { win.document.open(); win.document.write(r.html); win.document.close(); win.focus(); shown = true; }
		else {
			try {
				const url = URL.createObjectURL(new Blob([r.html], { type: 'text/html' }));
				const alt = window.open(url, '_blank', 'noopener');
				if (alt) { shown = true; setTimeout(() => URL.revokeObjectURL(url), 60000); } else { URL.revokeObjectURL(url); }
			} catch (_) {}
		}
		if (!shown) { keysShow('bad', 'The Recovery Kit could not be opened in a separate window here. Open the interface in a web browser and try again, or create the kit from the command line.'); return; }
		keysShow('ok', addKey
			? 'Recovery Kit opened in a separate window — it includes a spare recovery key, so print it or save the PDF, then store it somewhere safe and private.'
			: 'Identity‑only Recovery Kit opened in a separate window — it records the vault’s identity and fingerprint but no key, so it changes nothing and needs no password.');
		if (addKey) await loadKeys();
	} catch (e) { if (win && !win.closed) win.close(); keysShow('bad', esc(e.message)); }
}
...
async function loadShares() {
	let rep; try { rep = await api('/api/shares', { path: keysTarget }); } catch (_) { return; }
...
async function loadShareKeys() {
	...
	let data; try { data = await api('/api/keys', { path: shareTarget }); } catch (e) { box.innerHTML = '<div class="muted">' + esc(e.message) + '</div>'; return; }
...
onDialogSubmit('#tamperForm', async () => {
	...
	try {
		...
		// When only a tamper note is present (the baseline record was removed or forged, or foreign files
		// were found in the store) lead with that rather than "Changes detected since <date>", which would
		// read as an ordinary edit.
		const lead = (noFileDiff && r.tamper && r.tamper.length)
			? 'The vault’s integrity check found a problem:'
			: 'Changes detected since ' + esc(r.snapshotAt) + ':';
		tamperShow('bad', lead + tamper +
			fileList('Modified', r.modified) + fileList('Removed', r.removed) + fileList('Added', r.added) + fileList('Unrecognized — added from outside the vault', r.foreign) + fingerprintLine(r) + identityBlock(r) + sync);
	} catch (e) { tamperShow('bad', esc(e.message)); }
});
...
// Run a check-and-repair. allowUnverified is the explicit "repair anyway" override, offered only after the
// server has refused an unverified (possibly-forged) recovery signature — so the default stays fail-closed.
async function runHeal(allowUnverified) {
	try {
		...
		const r = await withBusy(() => apiStream('/api/heal', { path: protectTarget, allowUnverified: !!allowUnverified }, protectProgress));
		protectProgress(null);
		const v = r.verify || {};
		...
		if (r.refused === 'authenticity') {
			protectShow('bad', '<strong>Repair refused — the recovery data may have been tampered with.</strong> Its authenticity signature did not verify. Rebuild the recovery data from a trusted read-write session (unlock the vault, then “Update self-healing”), or remove its recovery folder and rebuild. <button type="button" id="healAnyway" class="link-btn danger">Repair anyway</button>');
		...
		const tail = h.unrecoverableStripes ? ' ' + h.unrecoverableStripes + ' area(s) could not be repaired — restore from a backup.' : (h.writeErrors ? '' : ' Open the vault to confirm.');
		protectShow((h.unrecoverableStripes || h.writeErrors) ? 'bad' : 'ok', 'Repaired ' + h.repairedData + ' data block' + (h.repairedData === 1 ? '' : 's') + (h.repairedParity ? ' and ' + h.repairedParity + ' recovery block' + (h.repairedParity === 1 ? '' : 's') : '') + sizeNote + '.' + filesNote + writeNote + changedNote + tail + authNote + unverifiedNote);
	} catch (e) { protectProgress(null); protectShow('bad', esc(e.message)); }
	finally { protectBusy(false); }
}
...
async function loadBrowse(p) {
	...
	const folderHtml = data.folders.map(f => `<button type="button" class="browse-item${f.isVault ? ' vault' : ''}" data-path="${esc(f.path)}" data-vault="${f.isVault ? 1 : 0}"><span class="bi-name">${esc(f.name)}</span>${f.isVault ? '<span class="vault-tag">vault</span>' : ''}</button>`).join('');
	...
	const fileHtml = wantFiles.map(f => `<button type="button" class="browse-item file" data-file="${esc(f.path)}"><span class="bi-name">${esc(f.name)}</span><span class="vault-tag file">file</span></button>`).join('');
...
$('#sftpTest').addEventListener('click', async () => {
	...
	try {
		...
		else { res.className = 'tamper-result bad'; res.textContent = 'Could not connect: ' + esc(t.error || 'check the host, login, and that the folder exists'); }
	} catch (e) { res.className = 'tamper-result bad'; res.textContent = esc(e.message); }
});
...
if ($('#signinKeyAdd')) $('#signinKeyAdd').addEventListener('click', async () => {
	...
	try {
		...
		const pw = await uiConfirm({ title: 'Confirm it\'s you', message: 'Enter your web password to add ' + esc(label) + ' as a passwordless sign-in.', confirmLabel: 'Add', requirePassword: true, passwordLabel: 'Web password' });
...
function renderEmergencyStatus(s) {
	const armed = (s.armed || []).map(a => esc(a.name)).join(', ') || 'none yet';
	...
	showResult($('#emStatus'), s.phase === 'released' || s.phase === 'grace' ? 'bad' : 'ok',
		'Contact: <strong>' + esc(s.contactLabel) + '</strong>. Check in every ' + s.inactivityDays + ' days (+' + s.graceDays + ' day grace). Status: ' + phase + '.<br>Protected vaults: ' + armed + '.');
	// Fill the vault picker with valid, unmounted vaults.
...
if ($('#emArmBtn')) $('#emArmBtn').addEventListener('click', async () => {
	...
	try { await withBusy(() => api('/api/emergency-arm', { path, password })); $('#emArmPassword').value = ''; const s = await api('/api/emergency-status', {}); renderEmergencyStatus(s); showResult($('#emManageResult'), 'ok', 'Protected. Its read access is sealed to your contact.'); }
...
if (!(window.CSS && CSS.supports && CSS.supports('-webkit-text-security', 'disc'))) {
	document.querySelectorAll('input.mask').forEach((i) => { i.type = 'password'; });
}
...
function attachStrength(inputId, meterId) {
	...
	const bar = meter.querySelector('.strength-bar i'), text = meter.querySelector('.strength-text');
	const update = () => {
		const v = input.value;
		...
		text.textContent = r.label + (r.hint ? ' — ' + r.hint : '');
	};
...
function stepProgress(barSel, fillSel, textSel) {
	return (p) => {
		...
		if (!p) { bar.hidden = true; bar.classList.remove('indeterminate'); return; }
		bar.hidden = false;
		if (p.indeterminate) { // a phase with no measurable percent (pack/unpack) — animate instead of showing a stuck number
			bar.classList.add('indeterminate'); $(fillSel).style.width = ''; $(textSel).textContent = (p.label || 'Working') + '…';
		} else {
			bar.classList.remove('indeterminate'); $(fillSel).style.width = (p.percent || 0) + '%'; $(textSel).textContent = (p.label ? p.label + ' — ' : '') + (p.percent || 0) + '%';
		}
	};
}
