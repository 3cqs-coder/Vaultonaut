...
if (psEl) psEl.addEventListener('change', async (ev) => {
	const on = !!ev.target.checked;
	try { await api('/api/settings', { privacyScreen: on }); if (lastState && lastState.settings) lastState.settings.privacyScreen = on; if (!on) revealedPaths.clear(); renderVaults(); toast(on ? 'Privacy screen on — names and paths stay blurred until you reveal them' : 'Privacy screen off'); }
	catch (e) { toast(e.message, true); }
});
