...
(function (root) {
	...
	// Strip characters that can hide or REORDER rendered text — C0/C1 controls, the Arabic letter mark, the LTR/RTL
	// marks, the bidi overrides/embeddings, and the isolates — before any stored value reaches the screen. Values are
	// placed with textContent (never innerHTML), so there is no markup-injection risk here; the risk is a right-to-left
	// override or an invisible control embedded in a value making the DISPLAYED text read differently from what it is —
	// most dangerously in a URL label, where a right-to-left override in the text could visually masquerade as a different address than
	// the link actually opens. Reuse the shared sanitizer's exact character class when the page has loaded safe-text.js,
	// else a byte-identical local copy so this shared module is self-sufficient wherever it runs.
	var DISPLAY_STRIP = (root.VaultSafe && root.VaultSafe.UNSAFE_DISPLAY) || new RegExp("[\\u0000-\\u0008\\u000B\\u000C\\u000E-\\u001F\\u007F-\\u009F\\u061C\\u200E\\u200F\\u202A-\\u202E\\u2066-\\u2069]", "g");
	function displaySafe(s) { return String(s == null ? '' : s).replace(DISPLAY_STRIP, '�'); }
	...
	function copyToClipboard(text, opts) {
		...
		return navigator.clipboard.writeText(String(text == null ? '' : text)).then(function () {
			// Keep only ONE pending auto-clear: a new copy cancels the previous copy's timer. Without this the timers
			// stack, and an earlier secret's timer would wipe a value copied AFTER it (clearing it early, less than the
			// intended window after that later copy). The clear now always fires clearMs after the most recent copy.
			if (clipboardClearTimer) { clearTimeout(clipboardClearTimer); clipboardClearTimer = null; }
			if (clearMs > 0) clipboardClearTimer = setTimeout(function () { clipboardClearTimer = null; try { navigator.clipboard.writeText('').catch(function () {}); } catch (_) {} }, clearMs);
			return true;
		});
	}
	...
	function injectStyleOnce() {
		...
		var css = '.vs-field{border:1px solid var(--border,#e2e6ef);border-radius:10px;padding:8px 10px;margin:0 0 8px;background:var(--panel,#fff)}'
			...
			+ '.vs-val.vs-masked{letter-spacing:.1em}.vs-val a{color:var(--accent-ink,#1e56d6)}'
			+ '.vs-tools{display:flex;gap:4px;flex:0 0 auto}'
			+ '.vs-btn{min-height:40px;padding:8px 14px;background:var(--panel-2,#f0f2f8);color:var(--text,#1a2233);border:1px solid var(--border,#e2e6ef);border-radius:999px;font-size:13px;cursor:pointer}'
	...
	function renderItem(container, item, opts) {
		...
		(item && item.fields || []).forEach(function (f) {
			...
			var lab = document.createElement('div'); lab.className = 'vs-label'; lab.textContent = displaySafe(f.label || (f.kind || 'Field')); wrap.appendChild(lab);
			if (info.totp) {
				var box = document.createElement('div'); box.className = 'vs-totp'; wrap.appendChild(box);
				var cfg0 = parseTotp(f.value || '');
				if (cfg0.secret) {
					// Build the code, seconds, and Copy nodes ONCE and only UPDATE their text each tick — never rebuild
					// the DOM — so a keyboard or screen-reader user can actually focus and use the Copy button (a
					// per-second rebuild would throw focus away every tick). The Copy handler reads the latest code.
					var cur = '';
					var c = document.createElement('span'); c.className = 'vs-code';
					var s = document.createElement('span'); s.className = 'vs-secs';
					var cp = btn('Copy', function () { copy(cur); });
					box.appendChild(c); box.appendChild(s); box.appendChild(cp);
					var tick = function () { totpCode(parseTotp(f.value || '')).then(function (r) { cur = r.code; c.textContent = r.code.replace(/(\d{3})(\d+)/, '$1 $2'); s.textContent = r.secondsRemaining + 's'; }, function () {}); };
			...
			var shown = false;
			function paint() { if (secret && !shown) { val.textContent = '••••••••'; } else if (info.link && f.value && /^https?:\/\//i.test(f.value)) { val.innerHTML = ''; var a = document.createElement('a'); a.href = f.value; a.target = '_blank'; a.rel = 'noopener noreferrer'; a.textContent = displaySafe(f.value); val.appendChild(a); } else { val.textContent = displaySafe(f.value || ''); } }
		...
		if (item && item.note) { var lab2 = document.createElement('div'); lab2.className = 'vs-label'; lab2.textContent = 'Note'; container.appendChild(lab2); var pre = document.createElement('div'); pre.className = 'vs-note'; pre.setAttribute('dir', 'auto'); pre.textContent = displaySafe(item.note); container.appendChild(pre); }
	...
	root.VaultSecret = {
		...
		copyToClipboard: copyToClipboard, aesGcmOpen: aesGcmOpen, b64ToBytes: b64ToBytes, renderItem: renderItem, displaySafe: displaySafe,
