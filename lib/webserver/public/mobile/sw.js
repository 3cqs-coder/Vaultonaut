...
importScripts('vendor/nacl.min.js', 'crypt-reader.js'); // the range decryptor (nacl secretbox) for media streaming
var R = self.CryptReader;
...
// Every file the page loads at startup MUST be listed here, or the FIRST offline load right after a version bump
// (when activate has deleted the old cache and only this SHELL is precached) will miss it and fail. safe-text.js is
// loaded before app.js and defines window.VaultSafe, which app.js uses immediately — omitting it left the viewer blank
// offline until one online load re-populated the runtime cache.
var SHELL = ['./', 'index.html', 'theme-boot.js', 'app.css', 'app.js', 'crypt-reader.js', 'shared/safe-text.js', 'shared/secret-schema.js', 'vendor/nacl.min.js', 'vendor/scrypt.js', 'vendor/aes-js.js', 'icon.svg', 'apple-touch-icon.png', 'manifest.webmanifest', 'config'];
...
self.addEventListener('install', function (e) {
	// Do NOT swallow an addAll failure. addAll is atomic, so if any shell asset fails to fetch, the new versioned
	// cache would be left EMPTY; swallowing the error let install succeed and skipWaiting fire anyway, and the activate
	// handler below then deletes the previous version's WORKING cache — breaking offline until the network returns.
	// Letting install reject instead keeps the old service worker and its populated cache in place until a later,
	// fully successful install; skipWaiting runs only once the whole shell is cached.
...
self.addEventListener('fetch', function (e) {
	...
	// App shell: NETWORK-FIRST, with the precache as an offline fallback. Cache-first served stale code until a second
	// load, so a fix to the viewer only appeared on the next open — a real, repeated papercut. Network-first means the
	// viewer picks up a code change on the very next open whenever the server is reachable (instant on loopback, a
	// paired local network otherwise), while a genuinely offline viewer still falls back to the cached shell. A non-ok
	// response (a transient error) also prefers a good cached copy.
	e.respondWith(
		fetch(req).then(function (res) {
			if (res && res.ok && res.type === 'basic') { var copy = res.clone(); caches.open(CACHE).then(function (c) { return c.put(req, copy); }).catch(function () {}); return res; }
			return caches.match(req).then(function (hit) { return hit || res; });
		}).catch(function () { return caches.match(req).then(function (hit) { return hit || Promise.reject(new Error('offline and not cached')); }); })
	);
});
