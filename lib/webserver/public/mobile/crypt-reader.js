/* crypt-reader.js — a faithful, DECRYPTION-ONLY reimplementation of the engine's "crypt" file format,
 ...
 *
 * Why this exists: the vault uses the engine's standard crypt format, and a web-scoped read capability already carries
 ...
 *
 * Format facts it implements (all from the engine crypt format):
 *   - Key derivation: scrypt(N=16384, r=8, p=1) over the UTF-8 bytes of the read key and the salt string,
 *     80 bytes out, split dataKey[0:32] / nameKey[32:64] / nameTweak[64:80]. An empty salt uses the engine's
 *     fixed default salt.
...
(function (root, factory) {
	if (typeof module === 'object' && module.exports) {
		...
		root.CryptReader = factory(root.nacl, root.scrypt, root.aesjs);
	}
})(typeof self !== 'undefined' ? self : this, function (nacl, scrypt, aesjs) {
	'use strict';

	// ---- constants (verbatim from the engine crypt format) ----
	...
	var AES_BLOCK = 16;
	// The engine's default salt, used when a vault configures no salt (no password2).
