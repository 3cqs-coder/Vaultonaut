...
(function () {
	...
	function wireNoteTotp(value, box) {
		...
		function tick() {
			var cfg = VS.parseTotp(value); if (!cfg.secret) { box.textContent = ''; return; }
			VS.totpCode(cfg).then(function (r) { box.innerHTML = ''; var code = document.createElement('span'); code.className = 'totp-code'; code.textContent = r.code.replace(/(\d{3})(\d+)/, '$1 $2'); var secs = document.createElement('span'); secs.className = 'totp-secs'; secs.textContent = r.secondsRemaining + 's'; box.appendChild(code); box.appendChild(secs); box.appendChild(mkNoteBtn('Copy', function () { copyValue(r.code); })); }, function () { box.textContent = ''; });
		}
