...
(function () {
	...
	function paintNotes(ul) {
		...
		if (!notesMeta.length) { var li = document.createElement('li'); li.className = 'fine'; li.style.cursor = 'default'; li.textContent = 'No notes in this vault.'; ul.appendChild(li); return; }
		notesMeta.forEach(function (n) { ul.appendChild(row('🗒️', n.title || '(untitled)', n.updatedAt ? whenShort(n.updatedAt) : '', function () { openNote(n); })); });
	}
	// Decrypt only each note's small METADATA (title + timestamps) for the list — never the body — so a vault with
	// very many notes lists without loading every body into memory. Sequential so peak memory is one note at a time.
	function loadNotesMeta() {
		...
		function step() {
			...
			return getCipher(f.enc)
				.then(function (cipher) { var j = JSON.parse(new TextDecoder().decode(R.decryptContent(keys.dataKey, cipher))); return openNoteMeta(j).then(function (meta) { out.push({ id: j.id, enc: f.enc, title: meta.title, createdAt: meta.createdAt, updatedAt: meta.updatedAt }); }); })
				.catch(function (e) { if (e && e.expired) throw e; /* skip an unreadable or locked note, keep listing the rest */ })
				.then(step);
		}
	...
	function openNote(n) {
		...
		getCipher(n.enc)
			.then(function (cipher) { var j = JSON.parse(new TextDecoder().decode(R.decryptContent(keys.dataKey, cipher))); return Promise.all([openNoteMeta(j), j.b ? openSealed(j.b) : Promise.resolve('')]).then(function (r) { renderNote(n.title, r[0], r[1]); }); })
			.catch(function (e) { if (e && e.expired) return; setViewerBody('<p class="pad">Could not open this note.<br><span class="fine">' + escapeHtml(e && e.message || '') + '</span></p>'); });
	}
	function renderNote(title, meta, body) {
		...
		var pre = document.createElement('pre'); pre.setAttribute('dir', 'auto'); pre.textContent = body || ''; wrap.appendChild(pre);
	...
	function openNoteMeta(j) {
		if (!j || !j.t) return Promise.resolve({ title: '', createdAt: null, updatedAt: null });
		return openSealed(j.t).then(function (s) { var o = {}; try { o = JSON.parse(s) || {}; } catch (_) {} return { title: o.title != null ? String(o.title) : '', createdAt: o.createdAt || null, updatedAt: o.updatedAt || null }; });
	}
