...
(function () {
	...
	var pendingOpen = null; // a decrypted file path to open once the list loads (a deep-link from a desktop search result)
	var deepLinked = false;  // true once a deep-linked search result is showing, so its Back returns to the search, not the file list
	...
	function idbCount(store) { return idb().then(function (d) { return new Promise(function (res) { var t = d.transaction(store).objectStore(store).count(); t.onsuccess = function () { res(t.result || 0); }; t.onerror = function () { res(0); }; }); }).catch(function () { return 0; }); }
	...
	// Honest, state-aware trace indicator. `stored` is true only when an encrypted copy actually exists on this device
	// (the user chose Save offline). The wording is deliberately scoped to what THIS app writes — it does not claim the
	// phone's own operating system keeps nothing (previews, swap, etc. are outside any app's control).
	function setTrace(stored) {
		var t = el('traceTag'); if (!t) return;
		t.hidden = false;
		if (stored) {
			t.className = 'tracetag stored';
			t.textContent = 'Encrypted copy on this device';
			t.title = 'An encrypted copy is stored on this device for offline use. It cannot be read without your key, which is held only in memory — and Lock erases it.';
		} else {
			t.className = 'tracetag notrace';
			t.textContent = 'No-trace session';
			t.title = 'Files are decrypted only in memory. Nothing is saved to this device by the viewer, so closing or locking leaves nothing behind here.';
		}
	}
	// Strip invisible/bidi/control characters from decrypted, attacker-influenceable text (file and folder names, typed
	// field labels and values, URL labels, the note body, the viewer title) before it is shown — the SAME neutralization
	// the desktop viewer applies through VaultSecret.displaySafe. Values still reach the DOM via textContent (never
	// innerHTML), so there is no markup-injection risk; the risk is a right-to-left override or an isolate making the
	// DISPLAYED text read differently from what it is — most dangerously a URL label spoofing a different address.
	function safeText(s) {
		var f = window.VaultSecret && window.VaultSecret.displaySafe;
		if (f) return f(s);
		var re = window.VaultSafe && window.VaultSafe.UNSAFE_DISPLAY; // fallback if the schema module is unavailable
		return re ? String(s == null ? '' : s).replace(re, '�') : String(s == null ? '' : s);
	}
	...
	function afterConnect() {
		...
		setStatus('Connected');
		// Reflect whether any encrypted copy is already on this device (a prior Save offline), so the badge is truthful
		// from the first paint rather than assuming no-trace. Best-effort; defaults to no-trace if the count is unknown.
		idbCount('cipher').then(function (n) { setTrace(n > 0); });
		return loadList();
	}
	...
	function loadList() {
		...
		return authFetch(session.list).then(function (r) {
			...
			maybeOpenPending(); // a deep-linked search result: jump to that file's folder and open it, now that names are known
		}).catch(function (e) {
	...
	// A normalization- and whitespace-tolerant key for matching a file name across two independent decryptions (the
	// desktop search index and this viewer). Real file names — especially macOS screenshots — carry exotic spaces such
	// as the narrow no-break space (U+202F), and a name can arrive NFC on one side and NFD on the other; matching on the
	// raw string then silently fails. Folding Unicode to NFC and every run of any whitespace to one plain space makes
	// the match robust, while staying specific enough that two genuinely different files never collide.
	// JavaScript's \s already covers every exotic space a name can carry (the narrow no-break space U+202F, NBSP, the
	// ideographic space, and the rest), so folding runs of \s to one plain space needs no hand-listed character class.
	function matchKey(s) { try { return String(s).normalize('NFC').replace(/\s+/g, ' '); } catch (e) { return String(s); } }
	// A deep-linked search result: once the (decrypted) names are known, jump to the target file's folder and open it in
	// the same in-app viewer a tap would. Uses the tolerant key above so an exotic-space or normalization difference
	// never defeats the match. If the file is genuinely gone (renamed or removed since the search), say so once and
	// leave the list showing rather than opening the wrong thing. Runs once; the pending target is always cleared.
	function maybeOpenPending() {
		if (!pendingOpen) return;
		var target = matchKey(pendingOpen); pendingOpen = null;
		var hit = null;
		for (var i = 0; i < allFiles.length; i++) { if (matchKey(allFiles[i].path) === target) { hit = allFiles[i]; break; } }
		if (!hit) { toast('That file is no longer in the vault.', 3200, true); return; }
		var segs = hit.path.split('/'); var nm = segs.pop(); cwd = segs; render();
		deepLinked = true; // came straight from a search result — its Back should return to the search, not this file list
		openFile(hit, nm);
	}
	// Return to the app that opened this viewer, which re-opens the search modal with its results (a deep-linked "quick
	// look" came from there). If this viewer is its own tab (opened by the desktop app's browser), close it so focus
	// returns to the tab holding the search; otherwise it is the single desktop-app window, so navigate it back to the
	// app, which restores the search from the note it left in sessionStorage.
	...
	function render() {
		...
		cwd.forEach(function (seg, i) { cr.appendChild(document.createTextNode('  /  ')); var a = document.createElement('a'); a.href = '#'; a.setAttribute('dir', 'auto'); a.textContent = safeText(seg); a.onclick = function (e) { e.preventDefault(); cwd = cwd.slice(0, i + 1); render(); }; cr.appendChild(a); });
	...
	function row(icon, name, size, onclick) {
		...
		var n = document.createElement('span'); n.className = 'nm'; n.setAttribute('dir', 'auto'); n.textContent = safeText(name); // dir=auto so a non-Latin (e.g. RTL) file or folder name picks its own base direction
		...
		getCipher(n.enc)
			.then(function (cipher) {
				...
				return Promise.all([openNoteMeta(j), contentP]).then(function (r) {
					...
					// Fail CLOSED on a note written by a newer app version than this viewer understands, rather than
					// misreading it as the current format. The desktop reader refuses `> NOTE_VERSION` the same way;
					// NOTE_VERSION (Vault.js) is the authority and a drift-guard test pins this literal against it.
					if (Number(j.v) > 3) { setViewerBody('<p class="pad">This item was made by a newer version of the app. Update to view it.</p>'); return; }
	...
	function renderNote(meta, content) {
		...
		if (content.note) { var lab = document.createElement('div'); lab.className = 'nf-label'; lab.textContent = 'Note'; wrap.appendChild(lab); var pre = document.createElement('pre'); pre.setAttribute('dir', 'auto'); pre.textContent = safeText(content.note); wrap.appendChild(pre); }
	...
	function renderField(f) {
		...
		var lab = document.createElement('div'); lab.className = 'nf-label'; lab.textContent = safeText(f.label || (VS ? (f.kind || 'Field') : 'Field')); wrap.appendChild(lab);
		...
		var shown = false;
		function paint() { if (secret && !shown) { val.textContent = '••••••••'; } else if (info.link && f.value) { val.innerHTML = ''; var a = document.createElement('a'); var safe = /^https?:\/\//i.test(f.value); if (safe) { a.href = f.value; a.target = '_blank'; a.rel = 'noopener noreferrer'; } a.textContent = safeText(f.value); val.appendChild(a); } else { val.textContent = safeText(f.value || ''); } }
		paint();
	...
	// PDF rendering with a bundled pdf.js. A browser's own PDF viewer does not render a PDF inside a sub-frame in a
	// WebView (the desktop app shows a blank frame), so the viewer draws each page to a canvas itself, which looks the
	// same in the desktop app's WebView and in every browser. pdf.js loads only when a PDF is first opened, so the viewer
	// never pays for it otherwise, and it decodes only OUR bytes — it runs neither a PDF's embedded JavaScript nor any
	// network fetch of its own.
	var pdfjsLib = null;
	function loadPdfjs() {
		if (pdfjsLib) return Promise.resolve(pdfjsLib);
		// A dynamic import of the same-origin ES module (script-src 'self'); its worker is the same-origin module worker.
		// The specifier is resolved to an absolute URL against the page base — a bare relative path like "vendor/…" is
		// treated as a module specifier and rejected, so this must not be a plain relative string.
		return import(new URL('vendor/pdfjs/pdf.min.mjs', document.baseURI).href).then(function (m) {
			var lib = m && (m.getDocument ? m : m.default) || m;
			try { lib.GlobalWorkerOptions.workerSrc = new URL('vendor/pdfjs/pdf.worker.min.mjs', document.baseURI).href; } catch (e) {}
			pdfjsLib = lib; return lib;
		});
	}
	function renderPdf(body, name, bytes) {
		var myGen = viewerGen; // if the viewer is closed or another file opens while pdf.js loads, abandon this render
		body.innerHTML = '<div class="pdfdoc"><div class="spin">Rendering…</div></div>';
		var doc = body.querySelector('.pdfdoc');
		loadPdfjs().then(function (lib) {
			// pdf.js detaches the buffer it is handed, so give it a copy — the caller keeps `bytes` for its own cleanup.
			// isEvalSupported:false and no network options keep it decoding only these bytes, entirely offline.
			return lib.getDocument({ data: bytes.slice(0), isEvalSupported: false, disableAutoFetch: true, disableStream: true }).promise;
		}).then(function (pdf) {
			// The viewer moved on (closed, or a different file opened) while the document was loading — tear the just-built
			// document down at once rather than rendering into a detached body and leaking its worker.
			if (myGen !== viewerGen) { try { pdf.destroy(); } catch (e) {} return; }
			doc.innerHTML = '';
			var scale = Math.min(2, (window.devicePixelRatio || 1) * 1.3); // crisp on the screen without exploding memory
			// Lazily render pages as they scroll near view and free those that scroll far away, so a long document never
			// holds every page's pixels at once. Where IntersectionObserver is missing, render every page up front.
			var io = ('IntersectionObserver' in window) ? new IntersectionObserver(function (entries) {
				entries.forEach(function (en) { if (en.isIntersecting) renderPage(en.target); else clearPage(en.target); });
			}, { root: doc, rootMargin: '1200px 0px' }) : null;
			// Closing the viewer (or opening another file) must release the pdf.js worker and stop observing, or every PDF
			// opened would leave an idle worker and its pages pinned in memory. Registered so closeViewer/showViewer drain it.
			viewerTeardowns.push(function () { try { if (io) io.disconnect(); } catch (e) {} try { pdf.destroy(); } catch (e) {} });
			function renderPage(ph) {
				if (ph.getAttribute('data-rendered')) return;
				ph.setAttribute('data-rendered', '1');
				pdf.getPage(+ph.getAttribute('data-page')).then(function (page) {
					...
					// Swallow a cancellation (from pdf.destroy() on close) so it never surfaces as an unhandled rejection.
					page.render({ canvasContext: canvas.getContext('2d'), viewport: vp }).promise.catch(function () {});
				}).catch(function () { ph.removeAttribute('data-rendered'); });
			}
			function clearPage(ph) { if (ph.getAttribute('data-rendered')) { ph.removeAttribute('data-rendered'); ph.innerHTML = ''; } }
			pdf.getPage(1).then(function (p1) {
				var vp1 = p1.getViewport({ scale: 1 }); var ar = vp1.width + ' / ' + vp1.height;
				for (var i = 1; i <= pdf.numPages; i++) {
					var ph = document.createElement('div'); ph.className = 'pdfpage'; ph.setAttribute('data-page', String(i));
					try { ph.style.aspectRatio = ar; } catch (e) {} // reserve each page's height so lazy loading detects scroll
					doc.appendChild(ph);
					if (io) io.observe(ph); else renderPage(ph);
				}
			});
		}).catch(function (err) {
			doc.innerHTML = '<p class="pad">Could not display this PDF.<br><span class="fine">' + escapeHtml(err && err.message || '') + '</span></p>';
		});
	}
	...
	// Neutralize invisible bidi-formatting and control characters (each -> U+FFFD) as well as escaping HTML, so a file
	// name carrying a right-to-left override or similar cannot SPOOF how it reads in the viewer's list. Strong RTL letters
	// are untouched, so a genuine Arabic/Hebrew name still renders; only the override/embedding/isolate format characters
	// and non-whitespace C0/C1 controls are removed. Mirrors esc() in the desktop client.
	// HTML-escape from the shared sanitizer (shared/safe-text.js), loaded before this script — one copy for every
	// client, so the desktop and mobile bundles can never drift to different escaping.
	var escapeHtml = window.VaultSafe.escapeHtml;
	...
	// Teardowns to run when the current viewer content is replaced or the viewer closes — a general hook a heavy view
	// (the PDF renderer's worker and observer) registers into so it is released deterministically, not left to the GC.
	// `viewerGen` bumps on every open and close, so an async view that finishes loading after the viewer moved on can
	// tell it is stale and dispose of itself instead of attaching to a body that is no longer its own.
	var viewerTeardowns = [];
	var viewerGen = 0;
	function runViewerTeardowns() { var t = viewerTeardowns; viewerTeardowns = []; t.forEach(function (fn) { try { fn(); } catch (e) {} }); }
	...
	function showViewer(name, html) {
		viewerGen++; runViewerTeardowns(); // release whatever the previous open left behind before showing the next file
		lastFocusBeforeViewer = document.activeElement;
		el('viewerName').textContent = safeText(name); setViewerBody(html); el('viewer').hidden = false;
	...
	function closeViewer() {
		...
		// A deep-linked search result: closing it returns to the search rather than dropping into this vault's file list,
		// which the viewer never showed and the user did not ask for.
		if (deepLinked && wasOpen) { deepLinked = false; viewerGen++; runViewerTeardowns(); returnToSearch(); return; }
		clearNoteTotp(); // stop any live 2FA refresh so a closed item leaves no ticking timer
		viewerGen++; runViewerTeardowns(); // release the PDF worker / observer (and any future heavy view) on close
	...
	function saveOffline() {
		...
		function step() {
			if (i >= everything.length) { el('offlineBtn').disabled = false; if (okc > 0) setTrace(true); toast('Saved ' + okc + ' files for offline use.'); return; }
			var f = everything[i++];
	...
	function forget(silent) {
		...
		var tt = el('traceTag'); if (tt) tt.hidden = true; // Lock erased the offline copy and ended the session — reset the badge
		el('codeInput').value = '';
	...
	function boot() {
		...
		document.addEventListener('visibilitychange', function () { if (!document.hidden && session) { postSessionToSW(); authFetch(session.list).catch(function () {}); } }); // re-prime the worker too: it may have been evicted while backgrounded, dropping its in-memory stream key, which would otherwise break media streaming until a re-pair
		...
		window.addEventListener('pagehide', function () {
			...
			// Drop the streaming key from the service worker the moment the tab goes away — for a phone session too, not
			// only a local one. It is RAM-only either way, but this stops it from lingering in the worker until the
			// browser happens to evict the idle worker, tightening the key's lifetime to the tab's.
			try { if (navigator.serviceWorker && navigator.serviceWorker.controller) navigator.serviceWorker.controller.postMessage({ type: 'session-clear' }); } catch (e) {}
			if (session && session.local) stopServerSession(true); // a local in-app session also ends its SERVER session when its tab closes
		});

		// A pairing code carried in the URL fragment (from a scanned QR, or the desktop "Open in this browser"):
		// auto-connect, then scrub it. An optional "&f=<url-encoded decrypted path>" after the code is a deep-link from a
		// desktop search result — remember it and open that one file once the list loads. It rides in the fragment (never
		// the query), so the path is never sent to the server or written to any log, and replaceState scrubs it from the
		// address bar immediately, exactly like the code.
		var hash = (location.hash || '').replace(/^#/, '').trim();
		var m = /^([0-9A-Za-z]{5}-[0-9A-Za-z]{5})(?:&(.*))?$/.exec(hash);
		if (m) {
			history.replaceState(null, '', location.pathname);
			if (m[2]) { try { var f = new URLSearchParams(m[2]).get('f'); if (f) pendingOpen = f; } catch (e) {} }
			el('codeInput').value = m[1].toUpperCase();
