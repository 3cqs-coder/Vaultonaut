...
(function () {
	...
	function renderPlain(name, bytes) {
		...
		if (/^(png|jpg|jpeg|gif|webp|bmp|svg)$/.test(e)) {
			...
			// Sandbox the PDF frame as defense-in-depth: the blob is forced to application/pdf and rendered by the
			// browser's own viewer. The sandbox needs BOTH allow-same-origin and allow-scripts, and needs them together:
			// a Chromium PDF plugin renders with neither, but Firefox renders PDFs with pdf.js, which is JavaScript and
			// shows only a blank page without allow-scripts; and allow-scripts WITHOUT allow-same-origin puts the frame in
			// an opaque origin, which then blocks the parent-minted blob: URL as cross-origin. The framed content is a PDF
			// drawn by the browser's viewer, not our HTML, and pdf.js does not run a PDF's own embedded JavaScript, so
			// the pair renders in every browser without giving a vault file a way to run code in the page.
			var u = blobUrl(bytes, 'application/pdf'); body.innerHTML = '<iframe title="' + escapeHtml(name) + '" sandbox="allow-scripts allow-same-origin" style="width:100%;height:100%;border:0;background:#fff" src="' + u + '"></iframe>';
	...
	function closeViewer() {
		...
		if (deepLinked && wasOpen) { deepLinked = false; returnToSearch(); return; }
