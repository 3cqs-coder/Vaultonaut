...
(function () {
	...
	function renderPlain(name, bytes) {
		...
		if (/^(png|jpg|jpeg|gif|webp|bmp|svg)$/.test(e)) {
			...
			// sandbox the PDF frame as defense-in-depth: the blob is forced to application/pdf (rendered by the browser's
			// own viewer, which does not run PDF JavaScript here), but a minimal sandbox guards against a future browser
			// behavior change without breaking the built-in viewer. allow-same-origin lets the blob: URL load.
			var u = blobUrl(bytes, 'application/pdf'); body.innerHTML = '<iframe title="' + escapeHtml(name) + '" sandbox="allow-same-origin" style="width:100%;height:100%;border:0;background:#fff" src="' + u + '"></iframe>';
