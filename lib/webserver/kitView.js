...
// window — a few minutes is enough to print it and to re-print or re-save if the first attempt is canceled — after
...
function createKitView({ ttlMs = 5 * 60 * 1000, now = () => Date.now() } = {}) {
	...
	function handler(req, res) {
		...
		res.setHeader('Content-Security-Policy', "default-src 'none'; style-src 'unsafe-inline'; img-src data:; font-src data:; frame-ancestors 'none'; base-uri 'none'");
