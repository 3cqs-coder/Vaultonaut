...
function buildRouter({ mobileDir, isLoopbackAddr, exposed } = {}) {
	...
	mobile.use((req, res, next) => {
		...
		res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; frame-src blob:; font-src 'self'; connect-src 'self'; worker-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'");
	...
	mobile.get('/manifest.webmanifest', (req, res) => {
		...
		res.json({
			...
			background_color: '#0b1220', theme_color: '#0b1220', orientation: 'portrait-primary',
			icons: [
	...
	mobile.get(/^\/files\/([0-9a-f]+)\/(.+)$/, async (req, res) => {
		...
		if (!s) return res.status(401).type('text').send('Unauthorized');
		let target;
		try { target = await Mobile.resolveFile(s, decodeURIComponent(req.params[1])); }
		catch (_) { return res.status(404).type('text').send('Not found'); }
		res.setHeader('Content-Type', 'application/octet-stream');
		res.sendFile(target.abs, { acceptRanges: true, cacheControl: false, headers: { 'Cache-Control': 'no-store' } }, (err) => { if (err && !res.headersSent) res.status(404).type('text').send('Not found'); });
	});
	...
	if (mobileDir) {
		...
		mobile.get(/^\/(?:$|[^.]*$)/, (req, res) => res.sendFile(path.join(mobileDir, 'index.html'))); // SPA fallback
	}
	return mobile;
}
