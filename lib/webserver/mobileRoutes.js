...
function buildRouter({ mobileDir, isLoopbackAddr, exposed } = {}) {
	...
	mobile.get(/^\/files\/([0-9a-f]+)\/(.+)$/, async (req, res) => {
		...
		res.sendFile(target.abs, { acceptRanges: true, cacheControl: false, headers: { 'Cache-Control': 'no-store' } }, (err) => { if (err) { if (!res.headersSent) res.status(404).type('text').send('Not found'); Common.warn('viewer: could not send a resolved file — ' + (err && err.message)); } });
	});
