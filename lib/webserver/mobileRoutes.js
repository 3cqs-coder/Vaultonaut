...
function buildRouter({ mobileDir, isLoopbackAddr, exposed } = {}) {
	...
	if (mobileDir) {
		...
		mobile.get('/sw.js', (req, res) => { if (swBody == null) return res.status(404).end(); res.type('application/javascript'); res.setHeader('Cache-Control', 'no-store'); res.send('self.__SHELL_VERSION=' + JSON.stringify(swVersion) + ';\n' + swBody); });
