...
function buildRouter({ mobileDir, sharedDir, isLoopbackAddr, exposed } = {}) {
	...
	mobile.use((req, res, next) => {
		...
		// blob URLs and workers (to render decrypted bytes in memory), and WebAssembly — so it sets its own
		// strict-but-wider policy here, overriding the global headers already set upstream.
		// PDFs render with a bundled pdf.js (a same-origin ES module plus a same-origin module worker), which draws each
		// page to a canvas so the desktop app's WebView and every browser display it the same way — a browser's own PDF
		// viewer does not render a PDF inside a sub-frame in a WebView. script-src 'self' and worker-src 'self' cover the
		// module and its worker; 'wasm-unsafe-eval' lets pdf.js instantiate its own bundled image and color-management
		// WebAssembly. It only ever runs OUR vendored pdf.js code — pdf.js does not execute a PDF's embedded scripts or
		// supply attacker WebAssembly — so the surface stays limited to code we ship and sign. The viewer renders no
		// sub-frames at all (a PDF is a canvas now), so frame-src is 'none'. There is no in-app camera use (pairing uses
		// the phone's native camera to open the URL), so camera is not granted.
		res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; frame-src 'none'; font-src 'self'; connect-src 'self'; worker-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'");
	...
	mobile.post('/pair', (req, res) => {
		...
		pairFails.clear(ip);
		res.json({ ok: true, cap: r.cap, token: r.bearer, name: r.name, base: r.base, list: r.list, sessionId: r.sessionId, local: !!r.local, notesKey: r.notesKey || null, notesDir: r.notesDir || null });
	});
	...
	// Assets shared byte-for-byte with the desktop UI (the typed-secret schema), served from the one shared
	// folder so the viewer and the editor can never drift. Reachable at /m/shared/… under this router.
	if (sharedDir) mobile.use('/shared', express.static(sharedDir));
	...
	if (mobileDir) {
		...
		// A content hash of the shipped shell, folded into the cache version below. The app version alone busts the
		// cache only on a version bump, so a release that edits a shell asset WITHOUT bumping the version would ship a
		// byte-identical worker and an installed PWA would keep serving the stale (possibly security-fixed) shell. The
		// browser re-installs the worker whenever its served bytes change, and folding this fingerprint in makes those
		// bytes change whenever ANY shell file does. Hash the whole mobile directory (index, scripts, styles, icons,
		// vendored primitives) plus the shared typed-secret schema the viewer caches. Computed once at startup and
		// stable for identical files, so an ordinary restart never forces a needless re-install.
		const shellFingerprint = (() => {
			try {
				const fs = require('fs'); const crypto = require('crypto');
				const h = crypto.createHash('sha256');
				const walk = (dir) => { for (const e of fs.readdirSync(dir, { withFileTypes: true }).sort((a, b) => a.name < b.name ? -1 : 1)) { const p = path.join(dir, e.name); if (e.isDirectory()) walk(p); else if (e.isFile()) { h.update(e.name); h.update(fs.readFileSync(p)); } } };
				walk(mobileDir);
				// Hash the WHOLE shared directory, not just one file: the viewer precaches several shared modules (the
				// typed-secret schema AND the safe-text escaper), and a change to any of them must bust the installed
				// cache. Reusing walk() means a shared file added to the shell later is covered automatically.
				if (sharedDir) { try { walk(sharedDir); } catch (_) {} }
				return h.digest('hex').slice(0, 12);
			} catch (_) { return null; }
		})();
		// Fold the BRAND into the version too: `config` and `manifest.webmanifest` are precached by the worker but are
		// route outputs derived from Brand.name (not files under mobileDir), so the file fingerprint alone would not bust
		// the cache if the product were renamed. A short brand tag ensures a rebrand re-installs the worker.
		const brandTag = (() => { try { return require('crypto').createHash('sha256').update(String(Brand.name)).digest('hex').slice(0, 8); } catch (_) { return ''; } })();
		const swVersion = (() => { try { const v = 'v-' + String(require('../../package.json').version || 'dev'); const base = shellFingerprint ? v + '-' + shellFingerprint : v; return brandTag ? base + '-' + brandTag : base; } catch (_) { return shellFingerprint ? 'dev-' + shellFingerprint : 'dev'; } })();
