...
function buildRouter({ mobileDir, sharedDir, isLoopbackAddr, exposed } = {}) {
	...
	if (mobileDir) {
		...
		const swVersion = (() => { try { const v = 'v-' + String(require('../../package.json').version || 'dev'); return shellFingerprint ? v + '-' + shellFingerprint : v; } catch (_) { return shellFingerprint ? 'dev-' + shellFingerprint : 'dev'; } })();
