...
async function startServing(vaultPath, { bind, useRelay } = {}) {
	...
	const p = (async () => {
		...
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, cand: handle.candidates || [], bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (bind || '127.0.0.1'))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	const code = (e.bind === 'relay' || e.bind === 'lan' || e.bind === 'wan') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca, cand: e.cand }) : null;
...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/peer', handle(async (req) => {
		...
		if (peer.code) { const parsed = Vault.parsePeerCode(peer.code); if (!parsed) throw new Error('That connection code is not valid — copy it again from the other machine.'); return await Vault.savePeer({ label: peer.label, url: parsed.url, user: parsed.user, password: parsed.password, ca: parsed.ca, cand: parsed.cand }); }
