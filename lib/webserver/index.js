...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/tor-bridges', handle(async (req) => {
		...
		if (b.set) {
			...
			const saved = await Vault.setTorBridges({ mode: b.mode, lines: b.lines });
			try { require('../Onion').stopManagedTor(); } catch (_) {} // drop the running managed Tor so the next use starts with the new bridge config
			return saved;
		}
