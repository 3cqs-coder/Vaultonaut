...
async function startServing(vaultPath, { bind, useRelay, readOnly } = {}) {
	const key = path.resolve(vaultPath);
	...
	const p = (async () => {
		...
		const handle = await Vault.serveVault(vaultPath, { bind: (relay || isLan || isWan || isOnion) ? undefined : bind, lan: isLan, wan: isWan, onion: isOnion, relay, readOnly: readOnly || undefined, onEvent });
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, onion: handle.onion || null, onionAuth: handle.onionAuth || null, cand: handle.candidates || [], sig: handle.sig || null, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (isOnion ? 'onion' : (bind || '127.0.0.1')))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	return { serving: true, failed: !!e.failed, url: e.url, user: e.user, pass: e.pass, bind: e.bind, secure: e.secure, wan: !!e.wan, onion: e.onion || null, code, startedAt: e.startedAt };
}
