...
async function startServing(vaultPath, { bind, useRelay } = {}) {
	...
	const p = (async () => {
		...
		// "lan" is a direct, same-network serve: it binds every interface, serves over TLS with a LAN cert, and
		// announces itself so an authorized peer discovers it automatically — no relay, no port forwarding.
		const isLan = !relay && bind === 'lan';
		const handle = await Vault.serveVault(vaultPath, { bind: (relay || isLan) ? undefined : bind, lan: isLan, relay, onEvent });
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, cand: handle.candidates || [], bind: relay ? 'relay' : (isLan ? 'lan' : (bind || '127.0.0.1')), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	const code = (e.bind === 'relay' || e.bind === 'lan') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca, cand: e.cand }) : null;
