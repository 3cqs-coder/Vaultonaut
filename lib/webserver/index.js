...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/mobile-start', handle(async (req) => {
		...
		// In the packaged desktop app the WebView cannot open a second window, so "open in this browser" would replace
		// the whole app with the viewer. When running as the desktop app on this computer, open the viewer in the user's
		// real browser instead — the same this-computer-only, loopback action as printing the Recovery Kit — and report
		// it so the client leaves the app window in place. Gated on `!exposed` so a remote caller can never make the host
		// spawn a browser; when exposed this simply omits `opened` and the client opens the viewer in its own browser.
		if (local && Common.isDesktopApp() && !exposed) {
			try { if (Common.openExternal('http://127.0.0.1:' + port + '/m/#' + s.code)) out.opened = true; } catch (_) {}
		}
		return out;
	}));
