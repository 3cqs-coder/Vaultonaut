...
async function startServing(vaultPath, { bind, useRelay, readOnly } = {}) {
	...
	const p = (async () => {
		...
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, onion: handle.onion || null, cand: handle.candidates || [], sig: handle.sig || null, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (isOnion ? 'onion' : (bind || '127.0.0.1')))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	const code = (e.bind === 'relay' || e.bind === 'lan' || e.bind === 'wan' || e.bind === 'onion') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca, cand: e.cand, sig: e.sig || undefined }) : null;
