...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/emergency-arm', handle(async (req) => { const { path: p, password, contactId, mode, dateMs } = req.body || {}; return await Vault.emergencyArm(p, { password, contactId, mode, dateMs }); }));
