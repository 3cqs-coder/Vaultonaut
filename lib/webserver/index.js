...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	if (readyToken) {
		...
		// only AND gated on the same per-launch token (sent in a header, not the URL), so ONLY the shell that
		// started this backend can read the count — no other local process can poll how many vaults are open. Like
		// /__ready it is pre-login and inert for the headless service (installed only when a token was passed).
		app.get('/__mounted', async (req, res) => {
			...
			// answer, and fail-safe to 0 only on error (the shell asks anyway when it gets no clear count).
			try { mounted = (await Common.withTimeout(Vault.listMounts(), 3000).catch(() => [])).length; } catch (_) { mounted = 0; }
			res.type('application/json').end(JSON.stringify({ mounted }));
		});
	}
