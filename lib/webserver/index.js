...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp } = {}) {
	...
	app.post(/^\/s\/redeem\/(.+)$/, sendCsp, (req, res) => {
		...
		if (r.gone) return res.status(410).json({ gone: true });
		if (r.badPassword) return res.status(401).json({ badPassword: true });
	...
	app.post('/api/send-create', handle(async (req) => {
		...
		const pwHash = linkPassword ? Common.sha256(String(linkPassword)).toString('hex') : null;
		const rec = sendStore.create({ ciphertext: sealed.ciphertext, meta: sealed.meta, ttlMs: (Number(ttlMinutes) > 0 ? Number(ttlMinutes) : 7 * 24 * 60) * 60 * 1000, maxViews: Number(maxViews) || 1, pwHash });
