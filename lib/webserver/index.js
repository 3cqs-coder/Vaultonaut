...
// The one-time, in-memory holder + endpoint for a just-generated Recovery Kit (see kitView.js). Never written to disk.
...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp } = {}) {
	...
	// ---- Recovery Kit view -----------------------------------------------------------------------------
	// Serve a just-generated Recovery Kit ONCE, over a single-use token rather than the login session, so it renders
	...
	// in memory only and never written to disk; the token authorizes exactly one read, then the entry is dropped.
	// Registered BEFORE the login gate — the one-time token IS the capability, so it works even from a browser window
	// that has no session cookie, and a rebinding or unauthorized caller has no token to present.
	...
	// in memory and served exactly once at /kit-view over a single-use token. That keeps the key off disk AND out of
	// the JSON/network response, and lets the kit render under its own CSP in a browser window or the in-app viewer.
