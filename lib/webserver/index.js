...
function schedulePostUnmount(vaultPath) {
	...
	(async () => {
		try {
			...
			// Log the OUTCOME on every unmount — one concise line — so "why didn't self-heal update?" is always
			// answerable from the log rather than a silent no-op. A real rebuild, or the reason it did not run.
			try {
				...
				else Common.log('Self-heal: no rebuild for ' + key + ' — ' + ((rr && rr.reason) || 'unknown') + (rr && rr.missing ? ' (' + rr.missing + ' file(s) missing)' : '') + '.');
			} catch (_) {}
		}
...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/unmount', handle(async (req) => {
		...
		// its state untrusted).
		if (r && r.ok && !r.forced && !r.redirected && r.vault) schedulePostUnmount(r.vault); // !r.redirected: never run the real-vault follow-up after a decoy unmount
		else if (r && r.ok) { try { Common.log('Self-heal: post-unmount refresh NOT scheduled for ' + (r.vault || '(no vault path)') + ' — forced=' + !!r.forced + ' redirected=' + !!r.redirected + ' vault=' + (r.vault ? 'yes' : 'null') + '.'); } catch (_) {} } // make a skipped follow-up visible instead of silent
		return r;
	}));
