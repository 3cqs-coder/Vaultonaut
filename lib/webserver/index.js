...
async function startServing(vaultPath, { bind, useRelay, readOnly } = {}) {
	...
	const p = (async () => {
		...
		const handle = await Vault.serveVault(vaultPath, { bind: (relay || isLan || isWan) ? undefined : bind, lan: isLan, wan: isWan, relay, readOnly: readOnly || undefined, onEvent });
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, cand: handle.candidates || [], sig: handle.sig || null, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (bind || '127.0.0.1'))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	const code = (e.bind === 'relay' || e.bind === 'lan' || e.bind === 'wan') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca, cand: e.cand, sig: e.sig || undefined }) : null;
	return { serving: true, failed: !!e.failed, url: e.url, user: e.user, pass: e.pass, bind: e.bind, secure: e.secure, wan: !!e.wan, code, startedAt: e.startedAt };
}
