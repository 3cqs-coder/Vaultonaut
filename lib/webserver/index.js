...
function schedulePostUnmount(vaultPath) {
	...
	(async () => {
		try {
			...
			// Log the OUTCOME, not just failures — so "why didn't self-heal update?" is always answerable from the log
			// rather than a silent no-op. Stay quiet for the two normal, expected cases (nothing to do); report a real
			// rebuild and every actionable skip (needs an unlock to re-sign, beyond the size limit, a loss to repair, etc.).
			try {
				...
				else if (rr && rr.reason && rr.reason !== 'up-to-date' && rr.reason !== 'not-protected') Common.log('Self-heal recovery data was NOT updated for ' + key + ': ' + rr.reason + (rr.missing ? ' (' + rr.missing + ' file(s) missing)' : '') + '.');
			} catch (_) {}
		}
