...
async function computeVaultState(dir) {
	...
	try { manifest = await Vault.readManifestForPoll(dir); } catch (_) { ok = false; } // stat-keyed: a single stat when unchanged, not a full read+parse every poll
	let recovery = { protected: false };
	try { recovery = await Vault.recoveryStatus(dir); } catch (_) {} // reads only the small index; stat-cached
...
async function watchTick() {
	...
	try {
		...
		if (wokeFromSleep && mounts.length && (await Common.withTimeout(Vault.getSettings(), 5000).catch(() => ({}))).lockOnSleep) {
			try { const r = await Vault.lockAll(); afterLock(r.vaults); if (r.locked) Common.log('Resumed from sleep — locked ' + r.locked + ' open vault' + (r.locked === 1 ? '' : 's') + ' (lock-on-sleep).'); } // afterLock so the sleep path runs the same lease/viewer teardown and mirror/recovery refresh as every other lock
			catch (_) {}
		}
		...
		const { changes } = await Watchdog.refresh(mounts);
		let sawDead = false;
...
async function startServing(vaultPath, { bind, useRelay } = {}) {
	...
	const onEvent = (ev) => {
		try {
			...
			if (ev.type === 'gaveup') { const e = servedVaults.get(key); if (e) e.failed = true; Common.warn('Stopped serving "' + label + '": the node could not be kept running after repeated restarts. Start it again once the cause is fixed.'); }
...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp } = {}) {
	...
	// Backstop: lock any vaults left mounted by a previous service instance that was
	// hard-killed (so even its guardian could not run), before we start serving.
	// Only vaults whose owning instance is gone are touched.
	try { await Vault.unmountOrphans({ wipeCache: true }); } catch (_) {}
	// Launch the external guardian: if THIS process is hard-killed (SIGKILL, a
	// segfault, no chance to run any handler), the guardian notices and locks the
	// vaults. This is what makes "even on a crash it unmounts" true. Supervised: the
	// health tick re-launches it if it ever dies (see ensureGuardian / watchTick).
	guardianOwner = owner;
	ensureGuardian();
	...
	app.get('/js/strength.js', (req, res) => res.type('application/javascript').sendFile(path.join(__dirname, '..', 'passwordStrength.js')));
	...
	const sendBrandIcon = (req, res) => res.type('image/svg+xml').sendFile(path.join(ROOT, 'public', 'mobile', 'icon.svg'));
	...
	app.get('/readme.md', (req, res) => {
		...
		res.sendFile(path.join(ROOT, '..', '..', 'docs', 'README.md'), (err) => { if (err && !res.headersSent) res.status(404).type('text').send('Help is unavailable.'); });
	});
	...
	app.post('/api/unmount', handle(async (req) => {
		const { target, wipeCache, force } = req.body || {};
		const r = await Vault.unmount(target, { wipeCache: wipeCache !== false, force: !!force });
	...
	app.post('/api/shards-inspect', handle(async (req) => await Vault.inspectShards(await collectShards((req.body || {}).folders))));
	...
	app.post('/api/verify-backup', handle(async (req) => { const { path: p, dest } = req.body || {}; return await Vault.verifyBackup(p, { dest }); }));
	...
	app.post('/api/reveal', handle(async (req) => {
		...
		const roots = [...(await Vault.listKnownVaults().catch(() => [])), ...(await Vault.listMounts().catch(() => [])).map(m => m && m.mountpoint).filter(Boolean)].map(p => path.resolve(p));
	...
	app.post('/api/browse', handle(async (req) => await browseDir((req.body || {}).path, { includeFiles: !!(req.body || {}).includeFiles })));
