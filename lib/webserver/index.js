...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	// Open a search result: a file identified by its path RELATIVE to a mounted vault, opened in its default app. Same
	// this-computer-only rule as reveal (the opener runs on the machine hosting the app), and the relative path is
	// confined to the vault's mount server-side, so a network caller can never open an arbitrary file.
	app.post('/api/open-match', handle(async (req) => {
		if (exposed) throw new Error('Opening a file works only on the computer running the app, not over a network connection.');
		const b = req.body || {};
		return await Vault.revealInVault(String(b.path || ''), String(b.rel || ''));
	}));
