...
const crypto = require('crypto'); // owner-identity token for the desktop attach handshake (proof of possession)
...
const PerfHistory = require('../PerfHistory');
...
const ApiV1 = require('./apiV1'); // the curated, versioned public API surface (shared with the drift test and docs)
const ROOT = path.join(__dirname);

// The short-lived, in-memory holder + endpoint for a just-generated Recovery Kit (see kitView.js). Never written to disk.
const { createKitView } = require('./kitView');
const kitView = createKitView();
const { createSendStore } = require('./sendLinks');
...
function updateStatusFrom(last) {
	...
	return { current, latest: last.latest, at: last.at || null, updateAvailable: UpdateCheck.compareVersions(last.latest, current) > 0, ahead: UpdateCheck.isAhead(current, last.latest), releasesUrl: UpdateCheck.RELEASES_URL, desktop: Common.isDesktopApp() };
}
function publicSettings(s) {
	...
	return {
		...
		keepDiagnosticLog: !!s.keepDiagnosticLog, // persistent diagnostic log files; OFF by default so no vault-activity trail is written to disk
		privacyScreen: !!s.privacyScreen, // blur vault names and paths in the UI until revealed; OFF by default (a display-only convenience for screen sharing)
		...
		wedgeRestart: !!s.wedgeRestart, // auto-restart a WEDGED (hung, not crashed) service; OFF by default (a wedged drive stays locked and secured, not restarted)
		autoAnonymize: s.autoAnonymize !== false, // convert an existing named vault to the opaque-folder format on its next unlock; ON by default (new vaults are always opaque)
		ocr: !!(s.ocr && s.ocr.enabled), // recognize text in scanned PDFs and images so they are searchable; OFF by default (recognition is heavy and downloads a one-time language file when first enabled)
		bwlimit: s.bwlimit || '',
		// How a peer node is reached: 'auto' (direct when possible, silent relay fallback — the default), 'relay' (never
		// try a direct hop), or 'direct' (require a direct hop, no relay fallback).
		p2pMode: (s.p2pMode === 'relay' || s.p2pMode === 'direct') ? s.p2pMode : 'auto',
	};
}
...
async function buildState() {
	...
	const vaults = await Common.mapLimit(known, 12, async (dir) => {
		...
		// The display name is the vault's encrypted title once THIS machine has decrypted it (from the local name
		// cache, seeded at unlock/create), else the folder name. The description comes from the same local cache. Both
		// are the vault's own label, not contents metadata, so showing them for a locked vault is intended and safe.
		const cachedMeta = manifest ? Vault.cachedVaultName(manifest, settings) : null;
		return {
			path: dir,
			pathDisplay: Common.tildePath(dir),       // home collapsed to ~ for display only; `path` stays real for actions
			name: Vault.vaultTitle(dir, cachedMeta && cachedMeta.title),
			description: (cachedMeta && cachedMeta.description) || '', // local per-machine cache; empty until this machine unlocks the vault
			valid,
			checking: pending,                        // first read not back yet (e.g. a slow/wedged backing drive)
			createdAt: manifest ? manifest.createdAt : null,
			mounted: !!m,
			mountpoint,
			mountpointDisplay: mountpoint ? Common.tildePath(mountpoint) : null, // home collapsed to ~ for display only
			...
			color: (settings.vaultColors || {})[abs] || null,     // a per-vault color tag (trust / sensitivity signal)
			consentOnUse: !!((settings.consentOnUse || {})[abs]), // require explicit approval on each unlock (recorded in the ledger) — the vault's OWN flag; a decoy is never listed here, so this never reveals a pairing
	...
	// `disk` is the cached per-volume free-space snapshot captured by the periodic disk_space self-check (OFF this
	// hot path), so the dashboard's disk gauge reads a value here instead of re-stat-ing the disks on every poll.
	// `perf` is the in-memory sample ring for the Performance tile's sparklines (also non-blocking, read from memory).
	// `fleet` is the cached fleet-node snapshot (background-polled, read from memory here — no network I/O on a page poll).
	return { doctor, vaults, mounts, settings: publicSettings(settings), sftpDests, cloudRemotes, peers, auth: { enabled: uiAuth.enabled }, selfCheck: SelfCheck.last(), disk: SelfCheck.diskSnapshot(), perf: PerfHistory.series(), fleet: Fleet.snapshot(), vaultsDir: Vault.vaultsDir(), defaultRoot: Vault.defaultMountRoot(), home: os.homedir() };
}
...
// Push a notification when the periodic self-check WORSENS (ok -> warn/error, or warn -> error), so a problem that
// appears while no one is looking reaches the user through the webhook they configured. Only a worsening fires, so a
// steady warning does not re-alert every five minutes; the level is remembered so a return to healthy re-arms it. The
// payload carries only counts and the severity — never a finding's message (which can contain a path). Reuses the same
// Notify path the backup-failed and check-in reminders use, and never throws on this hot tick.
let lastSelfCheckAlertLevel = 'ok';
async function maybeAlertSelfCheck(findings) {
	try {
		const Metrics = require('../Metrics');
		const v = Metrics.healthVerdict(findings);
		const rank = { ok: 0, warn: 1, error: 2 };
		if (rank[v.status] > rank[lastSelfCheckAlertLevel]) {
			const n = v.error || v.warn;
			const Notify = require('../Notify');
			await Notify.emit({ type: 'system-check', message: 'The system check found ' + n + ' ' + (v.status === 'error' ? 'issue' : 'warning') + (n === 1 ? '' : 's') + ' that need' + (n === 1 ? 's' : '') + ' attention. Open the app to see the details.' }, await Vault.getSettings());
		}
		lastSelfCheckAlertLevel = v.status;
	} catch (_) {}
}
function ensureGuardian() {
	...
	try {
		...
		// Clear the "guardian down" self-check flag on ANY confirmed (re)launch, not only when THIS process had warned.
		// guardianWarned resets per process, so a healthy new process must still clear a stale flag a PRIOR process left
		// after a transient launch failure — otherwise the self-check shows a false "crash-safety net is down" alarm for
		// up to 24h even though the guardian is running. This only runs on an actual (re)launch (the alive check returns
		// above), so it is not a per-tick write.
		Vault.setSettings({ guardianDownAt: null }).catch(() => {});
		if (guardianWarned) { Common.log('Crash-safety guardian re-launched.'); guardianWarned = false; }
	} catch (e) {
...
async function watchTick() {
	...
	try {
		...
		if (wokeFromSleep && mounts.length && !sleepLockRunning && (await Common.withTimeout(Vault.getSettings(), 5000).catch(() => ({}))).lockOnSleep) {
			...
			Vault.lockAll()
				.then(r => { afterLock(r.vaults, r.redirected); if (r.locked) Common.log('Resumed from sleep — locked ' + r.locked + ' open vault' + (r.locked === 1 ? '' : 's') + ' (lock-on-sleep).'); })
				.catch(() => {})
		...
		if (Date.now() - lastSelfCheckAt > SELFCHECK_REFRESH_MS) { lastSelfCheckAt = Date.now(); SelfCheck.run({ label: 'periodic' }).then(maybeAlertSelfCheck).catch(() => {}); } // keep the self-check findings fresh (disk space, reconnected drives) without a restart, and alert on a worsening
		...
		if (s.autoLockMinutes > 0 && !autoLockRunning) {
			...
			Vault.autoLockTick(mounts, s.autoLockMinutes * 60 * 1000)
				.then(r => {
					...
					afterLock(r.vaults, r.redirected);
				})
				.catch(() => {})
		...
		// Same reasoning for the other in-memory holders of sensitive material: expired Send links (which hold item
		// ciphertext) and expired Recovery Kit views (which can hold a one-time recovery key) are otherwise pruned only
		// on the next create/stash, so sweep them on the tick too — nothing sensitive lingers on a quiet service.
		try { sendStore.sweep(); } catch (_) {}
		try { kitView.sweep(); } catch (_) {}
...
async function listVolumes() {
	...
	if (process.platform === 'win32') {
		for (const L of 'ABCDEFGHIJKLMNOPQRSTUVWXYZ') candidates.push({ p: L + ':\\', name: L + ':', kind: 'drive' }); // include A:/B: too (network shares are sometimes mapped there); absent letters fail the bounded stat
	} else {
...
function schedulePostUnmount(vaultPath) {
	...
	(async () => {
		try {
			const rr = await Vault.refreshRecoveryIfStale(vaultPath, { onProgress: (p) => { if (p) refreshingRecovery.set(key, { percent: p.percent, label: p.label }); } });
			// Log only MEANINGFUL outcomes — a real rebuild, or a skip that needs the user's attention (the recovery data
			// could not be brought up to date). Stay silent on the routine no-ops that fire on ordinary mount/unmount
			// cycles (up-to-date, not-protected, metadata-only, busy), so a long-running service does not spam its log.
			try {
				if (rr && rr.refreshed) Common.log('Self-heal: recovery data refreshed for ' + key + ' (tier ' + rr.tier + ', ' + rr.dataBlocks + ' blocks' + (rr.signed ? ', signed' : '') + ').');
				else if (rr && rr.reason && !['up-to-date', 'not-protected', 'metadata-only', 'busy'].includes(rr.reason)) Common.warn('Self-heal: recovery data could NOT be updated for ' + key + ' — ' + rr.reason + (rr.missing ? ' (' + rr.missing + ' file(s) missing)' : '') + '. Run "protect" to update it.');
			} catch (_) {}
		}
...
function afterLock(vaults, redirected) {
	// A DECOY unmount presents the REAL vault path, so the recovery/mirror follow-up would touch the real vault on
	// disk and sync it to its remote purely because the decoy was opened — a deniability leak. Skip that follow-up for
	// a redirected vault (its own decoy baseline was refreshed in-process during teardown). Stopping the lease
	// heartbeat and any viewer session for it is a safe no-op (a decoy mount starts neither), so those still run.
	const skipFollowUp = new Set(redirected || []);
	for (const v of (vaults || [])) { stopLeaseHeartbeat(v); Mobile.stopByVaultPath(v); if (!skipFollowUp.has(v)) schedulePostUnmount(v); }
}
...
// A single machine-wide relay hub the user can run from the app (the CLI `vdisk relay` is the headless equivalent).
// One module-level instance; starting again while it runs returns the running one rather than binding the ports twice.
// The invite code is built by the browser from the host + this token, so it updates live as the host is edited.
let relayHub = null; // { hub, controlPort, portRange:[lo,hi], token, startedAt }
function relayHubInfo() {
	if (!relayHub) return { running: false };
	return { running: true, controlPort: relayHub.controlPort, portRange: relayHub.portRange, token: relayHub.token, startedAt: relayHub.startedAt };
}
const startingServe = new Map(); // vaultPath -> in-flight start promise (coalesces concurrent serve-start)
async function startServing(vaultPath, { bind, useRelay, readOnly, singleHop, stableOnion } = {}) {
	const key = path.resolve(vaultPath);
	...
	const p = (async () => {
		...
		// "lan" is a direct, same-network serve (binds all interfaces, TLS, self-announcing). "wan" is the same plus a
		// best-effort router port map (NAT-PMP/PCP), so it is also reachable over the internet when the router allows.
		const isLan = !relay && bind === 'lan';
		const isWan = !relay && bind === 'wan';
		// "onion" publishes an ephemeral onion through a running Tor and serves on loopback behind it — an anonymizing
		// overlay reach with no exposed IP or forwarded port. Mutually exclusive with lan/wan/relay.
		const isOnion = !relay && !isLan && !isWan && bind === 'onion';
		const handle = await Vault.serveVault(vaultPath, { bind: (relay || isLan || isWan || isOnion) ? undefined : bind, lan: isLan, wan: isWan, onion: isOnion, singleHop: isOnion && !!singleHop, stableOnion: isOnion && !!stableOnion, relay, readOnly: readOnly || undefined, onEvent });
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, onion: handle.onion || null, onionAuth: handle.onionAuth || null, singleHop: !!handle.singleHop, stableOnion: !!handle.stableOnion, cand: handle.candidates || [], sig: handle.sig || null, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (isOnion ? 'onion' : (bind || '127.0.0.1')))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	// For a relay OR a LAN serve, include a one-paste connection code (address + login + pinned cert) for the other
	// machine. The peer that pastes it discovers this node directly on the LAN afterwards (matched by that cert).
	const code = (e.bind === 'relay' || e.bind === 'lan' || e.bind === 'wan' || e.bind === 'onion') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca, cand: e.cand, sig: e.sig || undefined, oca: e.onionAuth || undefined }) : null;
	return { serving: true, failed: !!e.failed, url: e.url, user: e.user, pass: e.pass, bind: e.bind, secure: e.secure, wan: !!e.wan, onion: e.onion || null, singleHop: !!e.singleHop, stableOnion: !!e.stableOnion, code, startedAt: e.startedAt };
}
...
// The storage-node role: this machine offers space for OTHER people's vaults (zero-knowledge — only ciphertext lands
// here, in per-client slots, and nothing is ever adopted). One module-level serve, like the relay hub; starting again
// while it runs returns the running one. Enabled from the machine itself (loopback), and torn down on shutdown/lock/travel.
let storeServe = null; // { handle, bind, startedAt }
async function startStoreServing({ bind, useRelay } = {}) {
	if (storeServe) return storeNodeStatus();
	const relay = useRelay ? await Vault.relayForServe() : null;
	if (useRelay && !relay) throw new Error('Set up a relay first — enter the hub address and token — so people can reach your storage node from anywhere.');
	const isLan = !relay && bind === 'lan';
	const isWan = !relay && bind === 'wan';
	const handle = await Vault.serveStore({ bind: (relay || isLan || isWan) ? undefined : bind, lan: isLan, wan: isWan, relay });
	storeServe = { handle: handle, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (bind || '127.0.0.1'))), startedAt: new Date().toISOString() };
	return storeNodeStatus();
}
async function stopStoreServing() { if (storeServe) { const e = storeServe; storeServe = null; try { await e.handle.stop(); } catch (_) {} } return { serving: false }; }
async function storeNodeStatus() {
	const usage = await Vault.storeUsage().catch(() => ({ slots: 0, bytes: 0 }));
	const quotaMb = Number((await Vault.getSettings().catch(() => ({}))).storeQuotaMB) || 0;
	const base = { slots: usage.slots, bytes: usage.bytes, quotaMB: quotaMb };
	if (!storeServe) return { serving: false, ...base };
	return { serving: true, bind: storeServe.bind, secure: !!storeServe.handle.secure, startedAt: storeServe.startedAt, ...base };
}

async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	// A per-process owner-identity token, for the desktop app's "attach to the already-running instance" handshake.
	// The running loopback service writes this into its owner-only pidfile (below) and proves possession of it at
	// /__owner-check WITHOUT ever echoing it. Before showing an already-running instance in its trusted window, the
	// desktop app confirms the server actually answering the port holds this exact secret — so a stale pidfile whose
	// process id has been reused, or any other process that merely holds the port, can never be shown as the vault.
	// Loopback only (an exposed bind writes no pidfile and needs no attach handshake); high-entropy and fresh each
	// start, so it is never a long-lived on-disk secret.
	const ownerToken = exposed ? null : crypto.randomBytes(32).toString('hex');
	...
	// Setting up who inherits a vault, and where a recovery backstop sends its shares, is standing configuration that
	// steers a future read of the vault's contents. Unlike arming (which requires the vault's own password), these
	// setup routes take no vault credential, so a network caller holding only the web password could quietly redirect
	// the inheritance to a beneficiary or backstop they control, or silently drop the owner's grants. The owner does
	// this from the app on this computer, so refuse it on an exposed bind. Checking in and reading status stay open, so
	// the owner can still prove liveness and see the state from a phone on the same network.
	const refuseEmergencyConfigIfExposed = () => { if (exposed) throw new Error('Emergency access can only be set up from the app on this computer, not over a network connection.'); };
	...
	// Watch stdin as a cross-platform dead-man's switch whenever we run as a SUPERVISED CHILD — the packaged desktop
	// shell (which also writes "quit" for a clean stop) OR any other parent that holds our stdin open and set
	// VDISK_SUPERVISED (a test harness, a future supervisor). If that parent dies by ANY means (a clean quit, a crash, a
	// SIGKILL, an interrupted test run), the OS closes our stdin and we drain, lock, and exit — so a supervised service
	// can never be orphaned, on every platform and without a PID poll or a Linux-only parent-death signal. A headless
	// service under launchd/systemd sets no flag and is unaffected; a loopback CLI run no-ops on a terminal stdin.
	if (Common.isDesktopApp() || Common.isSupervisedChild()) ProcRegistry.watchStdinForQuit();
	PerfHistory.start(); // begin sampling event-loop lag and heap use for the Performance tile (unref'd, off the hot path)
	// Push a notification when a fleet node's status changes for real (the hysteresis is inside Fleet, so a single blip
	// never alerts). The message carries only the node's own label and its new status — never a vault's contents. Reuses
	// the same Notify path as the other alerts and never throws on the poll loop.
	Fleet.setAlertHook((transitions) => {
		Vault.getSettings().then((settings) => {
			const Notify = require('../Notify');
			for (const t of transitions) {
				const label = t.label || 'A node';
				const msg = t.to === 'online' ? (label + ' is back online.')
					: t.to === 'unreachable' ? (label + ' is unreachable.')
					: t.to === 'degraded' ? (label + ' reports a problem — open the app to see its health.')
					: (label + ' status changed to ' + t.to + '.');
				Notify.emit({ type: 'fleet-node', label: label, message: msg }, settings).catch(() => {});
			}
		}).catch(() => {});
	});
	Fleet.start();       // begin polling any enrolled fleet nodes in the background (unref'd, cached; page polls read the cache)
	ProcRegistry.onShutdown(async () => { try { PerfHistory.stop(); } catch (_) {} try { Fleet.stop(); } catch (_) {} });
	...
	ProcRegistry.onShutdown(async () => {
		...
		// read stale.)
		const phaseHeartbeat = setInterval(() => { Phase.setPhase('stopping').catch(() => {}); }, 1000);
		...
		await stopStoreServing();
		try { require('../Onion').stopManagedTor(); } catch (_) {} // stop our managed Tor helper too (also reaped by the process registry)
	...
	// Also reap FULLY-UNTRACKED orphan mounts under our own mount root — live mounts a prior instance left with no state
	// row (an unmount that dropped the row without releasing the mount, or a crash mid-teardown), which the tracked sweep
	// above cannot see. Without this they stack (the next mount picks "Vault (2)") and degrade the host. Safe here: this
	// runs before we mount anything, so any live mount under our root belongs to a prior instance. Background, like above.
	Vault.unmountUntrackedOrphans({ wipeCache: true }).catch(() => {});
	...
	// Desktop readiness probe (pre-auth, loopback only). The packaged desktop shell passes a per-launch random token
	// and, before it navigates the trusted window to this interface, confirms this endpoint echoes that token — proof
	// the responding server is the backend the shell just started, not some other process that happened to hold the
	// loopback port first (which the shell would otherwise show in the trusted window). The token is not a credential:
	// it grants nothing and is fresh per launch, so echoing it only proves identity. Served ONLY on loopback (keyed on
	// the real peer address, which cannot be forged) and ONLY when a token was provided, so it is inert for the
	// headless service and never reachable on a network bind.
	// Owner-identity proof (pre-auth, loopback only). The desktop app, before it SHOWS an already-running instance in
	// its trusted window, must confirm the server answering the port is genuinely this user's Vaultonaut and not some
	// other local process that holds the port (a squatter, or a stale pidfile whose process id was reused). The app
	// reads the expected token from the owner-only pidfile and sends it here; this endpoint proves possession by
	// comparing in constant time and returning only yes/no — it NEVER echoes the token, unlike /__ready (whose token
	// is not a secret). Present only on a loopback bind (an exposed bind has no ownerToken and writes no pidfile).
	if (ownerToken) {
		const ownerTokenBuf = Buffer.from(ownerToken);
		app.get('/__owner-check', (req, res) => {
			if (!isLoopbackAddr((req.socket && req.socket.remoteAddress) || '')) { res.status(404).end(); return; }
			const got = Buffer.from(String(req.headers[Common.OWNER_TOKEN_HEADER] || ''));
			// Length-check first so timingSafeEqual never throws; the token length is fixed, so this leaks nothing.
			const ok = got.length === ownerTokenBuf.length && crypto.timingSafeEqual(got, ownerTokenBuf);
			if (ok) res.type('application/json').end(JSON.stringify({ ok: true })); else res.status(404).end();
		});
	}
	if (readyToken) {
		app.get('/__ready', (req, res) => {
			if (!isLoopbackAddr((req.socket && req.socket.remoteAddress) || '')) { res.status(404).end(); return; }
			res.type('text/plain').end(String(readyToken));
		});
		// How many vaults are currently unlocked (mounted), for the desktop shell's quit confirmation: closing the
		// window locks and unmounts every open vault, so the shell asks first when at least one is open. Loopback
		// only AND gated on the same per-launch token (sent in a header, not the URL). The token is not a strong
		// secret here — /__ready hands it to any loopback caller by design — so this gate stops a casual reader, not
		// a determined local one; the value it protects is only a mount count, which a local caller could anyway
		// infer from the operating system's own mount table. Like /__ready it is pre-login and inert for the
		// headless service (installed only when a token was passed).
		app.get('/__mounted', async (req, res) => {
			if (!isLoopbackAddr((req.socket && req.socket.remoteAddress) || '')) { res.status(404).end(); return; }
			if (String(req.headers['x-vaultonaut-token'] || '') !== String(readyToken)) { res.status(404).end(); return; }
			// Reuse the same active-mounts source the interface uses, bounded so a wedged mount can never delay the
			// answer. Crucially, a timeout or error must NOT report zero: zero is a definite count the shell would act
			// on by closing WITHOUT confirming, which would silently tear down open vaults on a slow state read. So on
			// any failure send no numeric count, which the shell reads as "unknown" and confirms to be safe.
			try {
				const list = await Common.withTimeout(Vault.listMounts(), 3000);
				res.type('application/json').end(JSON.stringify({ mounted: list.length }));
			} catch (_) {
				res.type('application/json').end(JSON.stringify({ mounted: null }));
			}
		});
	}

	// ---- Monitoring endpoints: /metrics (text exposition) and /health (JSON) ----
	// Both are registered BEFORE the login gate so a scraper gets a clean 401 rather than a redirect to /login, and so a
	// custom CSRF header is not required (a plain metrics scrape cannot send one). Access rule, reusing the tools we
	// already have: a LOOPBACK caller is allowed (local scraping needs no token), with the same Host check the /api guard
	// uses to block DNS-rebinding; a REMOTE caller must present a valid API bearer token (the same tokens `vdisk
	// api-token` mints). They expose ONLY counts, ratios, and status — never a vault name, path, size, or contents — so a
	// monitoring stack learns the install's health, never what is in it. Everything read here is a cached, non-blocking
	// snapshot plus a few small settings reads; nothing heavy runs on a scrape.
	async function monitorAllowed(req, res) {
		const peer = (req.socket && req.socket.remoteAddress) || '';
		if (isLoopbackAddr(peer)) {
			const host = Common.splitHostPort(req.headers.host || '').host;
			if (!isLoopbackAddr(host) && host !== bindAddr) { res.status(403).type('text/plain').end('forbidden'); return false; }
			return true;
		}
		const bearer = /^Bearer\s+(.+)$/i.exec(req.get('Authorization') || '');
		if (bearer && await Vault.verifyApiToken(bearer[1].trim())) return true;
		res.status(401).type('text/plain').end('unauthorized');
		return false;
	}
	async function collectMetrics() {
		const health = Watchdog.snapshot() || {}; // cached per-mount health: 'healthy' | 'unresponsive' | 'dead'
		let mounted = 0, unresponsive = 0;
		for (const k of Object.keys(health)) { mounted++; if (health[k] !== 'healthy') unresponsive++; }
		const sc = SelfCheck.last() || { findings: [] };
		const v = Metrics.healthVerdict(sc.findings);
		const disks = SelfCheck.diskSnapshot() || [];
		let tight = null;
		for (const d of disks) { if (d && typeof d.pctFree === 'number' && (!tight || d.pctFree < tight.pctFree)) tight = d; }
		// Small, cached-or-cheap reads for the counts. Each is guarded so a slow read reports 0, never fails the scrape.
		const safe = async (p, dflt) => { try { return await p; } catch (_) { return dflt; } };
		const [known, peers, remotes, sftp, settings] = await Promise.all([
			safe(Vault.listKnownVaults(), []), safe(Vault.listPeers(), []), safe(Vault.listCloudRemotes(), []), safe(Vault.listSftpDests(), []), safe(Vault.getSettings(), {}),
		]);
		const scheds = (settings && settings.backupSchedules) || {};
		let scheduled = 0, failedBk = 0;
		for (const key of Object.keys(scheds)) { const s = scheds[key] || {}; if (!s.mode || s.mode === 'off') continue; scheduled++; if (typeof s.lastResult === 'string' && s.lastResult.indexOf('error') === 0) failedBk++; }
		let heap = {}; try { heap = v8.getHeapStatistics(); } catch (_) {}
		return {
			version: ReleaseIntegrity.appVersion() || '', up: 1, uptimeSeconds: Math.round(process.uptime()),
			rssBytes: (process.memoryUsage() || {}).rss || 0, heapUsedBytes: heap.used_heap_size || 0, heapLimitBytes: heap.heap_size_limit || 0,
			vaults: { total: known.length, mounted: mounted, unresponsive: unresponsive },
			selfcheck: { warn: v.warn, error: v.error, status: v.status },
			disk: tight ? { freeBytes: tight.freeBytes, freeRatio: tight.pctFree / 100 } : {},
			serving: servedVaults.size, peers: peers.length, remotes: remotes.length + sftp.length,
			backups: { scheduled: scheduled, failed: failedBk },
		};
	}
	app.get('/metrics', async (req, res) => {
		if (!(await monitorAllowed(req, res))) return;
		try { res.type('text/plain; version=0.0.4').end(Metrics.renderMetrics(await collectMetrics())); }
		catch (e) { res.status(500).type('text/plain').end('# error collecting metrics\n'); }
	});
	app.get('/health', async (req, res) => {
		if (!(await monitorAllowed(req, res))) return;
		const sc = SelfCheck.last() || { findings: [], at: null };
		const v = Metrics.healthVerdict(sc.findings);
		// 200 for ok/warn, 503 for error, so a plain HTTP health check (a container health check, an uptime monitor, an
		// orchestration liveness probe) treats a hard problem as unhealthy while a benign warning stays up. The JSON carries
		// the precise status. Findings are reported by check name and level only — never their (possibly path-bearing)
		// messages — to keep the endpoint free of identifying detail.
		res.status(v.status === 'error' ? 503 : 200).type('application/json').end(JSON.stringify({
			status: v.status, version: ReleaseIntegrity.appVersion() || null, at: sc.at || null,
			checks: sc.total || 0, warnings: v.warn, errors: v.error,
			findings: (sc.findings || []).map((f) => ({ check: f.check, level: f.level })),
		}));
	});
	...
	// Assets shared byte-for-byte between the desktop UI and the /m viewer (e.g. the typed-secret schema), served
	// from one folder to both origins so the two clients can never drift. The /m router mounts the same folder.
	...
	// ---- Recovery Kit view -----------------------------------------------------------------------------
	// Serve a just-generated Recovery Kit over a short-lived random token rather than the login session, so it renders
	// under its OWN kit-appropriate CSP. The kit is a self-contained page with inline styles, which the app's strict
	// style-src would strip inside an in-app frame (a srcdoc/blob frame inherits the embedder's CSP; a document loaded
	// from a real URL uses its own response CSP). This one endpoint therefore serves BOTH a normal browser window and
	// the desktop app's in-app viewer (a WebView cannot open a separate window), from one code path. The kit is held
	// in memory only and never written to disk; the token is viewable for a few minutes (long enough to print or
	// re-save more than once), then its TTL elapses and the entry is dropped. Registered BEFORE the login gate — the
	// short-lived token IS the capability, so it works even from a browser window that has no session cookie, and a
	// rebinding or unauthorized caller has no token to present.
	// Printing a Recovery Kit is a loopback/desktop feature — the kit can show a one-time recovery key. On a
	// network-exposed bind, do not serve it at all (answer 404 so the endpoint is not even confirmed), so a remote
	// caller holding a leaked token can never render it. On loopback the token is the capability, as above.
	app.get('/kit-view', (req, res, next) => { if (exposed) return res.status(404).end(); return kitView.handler(req, res, next); });
	...
	app.use('/m', MobileRoutes.buildRouter({ mobileDir: path.join(ROOT, 'public', 'mobile'), sharedDir: path.join(ROOT, 'public', 'shared'), isLoopbackAddr, exposed }));

	// ---- Send links (/s) ------------------------------------------------------------------------------
	// A recipient opens /s/<id>#<key> to view ONE item shared with them. The link IS the capability, so these
	// routes sit BEFORE the login gate; the server only ever holds CIPHERTEXT (the key is in the fragment and
	// never reaches here), and each open is counted so a "view once" link opens once. The static page decrypts
	// and renders entirely in the recipient's browser. Its own CSP allows the shared schema script + Web Crypto.
	const sendDir = path.join(ROOT, 'public', 'send');
	const sendCsp = (req, res, next) => {
	...
	app.get('/s/config', sendCsp, (req, res) => res.json({ name: Brand.name }));
	app.get(/^\/s\/info\/(.+)$/, sendCsp, (req, res) => { const i = sendStore.info(req.params[0]); if (!i) return res.json({ gone: true }); res.json({ needsPassword: i.needsPassword, remaining: i.remaining, expiresAt: i.expiresAt }); });
	// Per-IP online-guessing throttle for a protected Send link, mirroring the mobile /pair gate: a wrong link
	// password backs the caller off, so obtaining the URL never allows guessing the password at network speed. The
	// link ALSO self-destructs after a small number of wrong guesses (see sendLinks), so the two bounds compound.
	const sendFails = Common.failureBackoff();
	app.post(/^\/s\/redeem\/(.+)$/, sendCsp, (req, res) => {
		const ip = (req.socket && req.socket.remoteAddress) || 'unknown';
		const nowMs = Date.now();
		// The throttle guards an EXPOSED bind (a remote recipient guessing a link password). Exempt loopback like the
		// mobile /pair gate: on the default loopback-only bind every redeemer is 127.0.0.1, so a per-IP gate would be
		// one global counter that a local owner fumbling their own link password could trip. The per-link 10-guess
		// self-destruct in the store still bounds a local link. The socket peer (not a spoofable header) is the key.
		const local = isLoopbackAddr && isLoopbackAddr(ip);
		if (!local && sendFails.blocked(ip, nowMs)) return res.status(429).json({ error: 'Too many attempts — wait a moment and try again.' });
		const r = sendStore.redeem(req.params[0], (req.body || {}).password);
		if (r.gone) return res.status(410).json({ gone: true }); // do NOT clear the backoff here — a self-destructed link must not hand the guesser a fresh budget for the next link
		if (r.badPassword) { if (!local) sendFails.fail(ip, nowMs); return res.status(401).json({ badPassword: true }); }
		if (!local) sendFails.clear(ip); // a correct open clears the backoff for this caller
		res.json({ ok: true, ciphertext: r.ciphertext, meta: r.meta, remaining: r.remaining });
	});
	app.use('/s', sendCsp, express.static(sendDir, { index: 'index.html' }));
	app.get(/^\/s(?:\/[^.]*)?$/, sendCsp, (req, res) => res.sendFile(path.join(sendDir, 'index.html'), { dotfiles: 'allow' })); // /s and /s/<id> serve the recipient shell
	...
	// gets a 401 the browser turns into a redirect.
	{
		...
		app.post('/login', async (req, res) => {
			...
			if (!ok) {
				// A cross-site page can auto-submit this form — it is intentionally exempt from the /api custom-header gate
				// so a normal browser login works. On the LOOPBACK default that forged POST shares the user's own
				// 127.0.0.1, so counting its failure toward the per-IP lockout would let any open web page lock the user out
				// of their own UI; a genuine browser sets Sec-Fetch-Site and a page cannot forge it, so a cross-site value
				// is not counted there. That exemption is safe on loopback (no remote brute force reaches 127.0.0.1) but
				// would be a hole on an EXPOSED bind, which is exactly the online-guessing threat model: a non-browser
				// attacker could simply send `Sec-Fetch-Site: cross-site` on every guess to dodge the lockout forever. So
				// when exposed, every failed attempt counts regardless of the header; the exemption applies to loopback only.
				const sfs = String(req.get('sec-fetch-site') || '').toLowerCase();
				const crossSiteBrowser = !!(sfs && sfs !== 'same-origin' && sfs !== 'none');
				if (exposed || !crossSiteBrowser) loginFails.fail(ip, now);
		...
		app.use(async (req, res, next) => {
			const a = await currentUiAuth();
			if (!a.enabled) {
				// No password set. On the loopback default this is the intended single-user experience — wave requests
				// through. But when the interface is EXPOSED to the network, serving with no login would be a hole, and
				// the only way to reach this state at runtime is clearing the password while an exposed server runs
				// (startup refuses to bind exposed without one). Fail CLOSED there rather than trust that the password
				// was never cleared: refuse every request until a password is set again (done from the loopback CLI).
				if (exposed) {
					if (req.path.startsWith('/api')) return res.status(503).json({ ok: false, error: 'This networked interface has no web password set. Set one to use it again.' });
					return res.status(503).type('text/plain').send('This networked interface has no web password set. Set a web password (for example, "' + Brand.cli + ' web-password set" on the host) to use it again.');
				}
				return next(); // loopback with no password -> no login required (the safe single-user default)
			}
			...
			// A headless / integration client can authenticate with a bearer API token instead of the browser session
			// cookie (see "vdisk api-token"); a valid token is accepted here just like a signed-in session.
			const bearer = /^Bearer\s+(.+)$/i.exec(req.get('Authorization') || '');
			if (bearer && await Vault.verifyApiToken(bearer[1].trim())) return next();
	...
	app.use('/api', (req, res, next) => {
		...
		if (req.get(Common.OWNER_CSRF_HEADER) !== Common.OWNER_CSRF_VALUE) return res.status(403).json({ ok: false, error: 'forbidden' });
		next();
	});

	// ---- Published, versioned API (/api/v1) ----
	// A third-party tool builds against /api/v1/<name>, a CURATED, STABLE subset (see lib/webserver/apiV1.js and the
	// README's "Local API" section) that serves the SAME handlers as the internal /api/<name> — the bundled UI's contract.
	// One implementation, no duplication: a v1 request is rewritten to the internal path here, AFTER the auth/CSRF guard
	// above and BEFORE the routes below, so it inherits the same authentication and the same handler. Additive within a
	// version (new endpoints/fields only); a breaking change would be a v2. The middleware is a cheap string check on
	// every request and does no I/O, so it never blocks the loop.
	const V1_ROUTES = new Set(ApiV1.V1);
	app.use((req, res, next) => {
		if (!req.path.startsWith('/api/v1/')) return next();
		const name = req.path.slice('/api/v1/'.length);
		if (!V1_ROUTES.has(name)) return res.status(404).json({ ok: false, error: 'Unknown or unversioned API endpoint — see the Local API section of the README for the /api/v1 surface.' });
		const q = req.url.indexOf('?'); // preserve any query string across the rewrite
		req.url = '/api/' + name + (q >= 0 ? req.url.slice(q) : '');
		next();
	});
	// Capability discovery: the running app version and the API versions this build serves, so a client can check what
	// it can call before relying on a v1 endpoint. Behind the same header guard as the rest of the API.
	app.get('/api/version', (req, res) => res.json({ ok: true, name: Brand.name, version: ReleaseIntegrity.appVersion(), apiVersions: ApiV1.VERSIONS, endpoints: [...V1_ROUTES].sort() }));
	...
	app.post('/api/create', handle(async (req) => {
		...
		// New vaults from the interface are OPAQUE: the folder on disk is named by the vault's own id (revealing
		// nothing), and the name the user typed is stored ENCRYPTED and cached locally, so the list shows it at once.
		// The typed basename becomes that encrypted name.
		const res = await Vault.create(p, { password, level, cloud: cloud && cloud.remoteId ? cloud : undefined, worm: worm && worm.retainDays ? worm : undefined, opaque: true }); // level = KDF cost; cloud stores on a remote backend; worm = tamper-proof Object Lock
	...
	app.post('/api/cloud-connect', stream(async (req, onProgress) => {
		...
		const res = await Vault.cloudAuthorize(type, { clientId, clientSecret, onUrl: (url) => {
			// The consent URL is always the engine's OWN loopback server (http(s)://127.0.0.1:<port>/auth?…), so opening
			// it is safe and same-machine. In the desktop app the WebView cannot open a real browser window with
			// window.open, so open it HERE with the same backend opener the Recovery Kit uses; the standalone/web case
			// leaves it to the client, which opens the streamed URL itself. `opened` tells the client which happened so
			// it shows "finish in your browser" rather than a dead link that would navigate the app window away.
			let opened = false;
			if (Common.isDesktopApp()) { try { opened = Common.openExternal(url); } catch (_) {} }
			try { onProgress({ url, opened, label: 'Waiting for you to authorize in the browser' }); } catch (_) {}
		} });
	...
	app.post('/api/import', handle(async (req) => {
		...
		return await Vault.importFolder(p, { password, level, sourceDir, opaque: true }); // new interface vaults are opaque, like /api/create — the typed name is kept as the encrypted title
	}));
	...
	app.post('/api/mount', handle(async (req) => {
		...
		// A mount point or cache folder is a caller-named host path (the mount point, and the location of the plaintext
		// VFS scratch cache). These are CLI-parity pass-throughs the browser never sends, so refuse them over a network
		// bind rather than let a remote request steer this computer's mount point or plaintext cache onto an arbitrary
		// host path. The volume label and allow-other flag are not host paths and stay allowed.
		if (exposed && (mountpoint || cacheDir)) throw new Error('Choosing a mount point or cache folder works only from the app on the computer running it, not over a network connection.');
		...
		if (rememberPrefs !== false) await Vault.recordMountPrefs(r.vault || p, { readOnly, workingDisk, streaming, fuseBackend }); // key on the RESOLVED path (r.vault): a first unlock may have converted a named vault to its opaque path, so `p` would write the preference under a folder that no longer exists
	...
	app.post('/api/unmount', handle(async (req) => {
		...
		// its state untrusted).
		if (r && r.ok && !r.forced && !r.redirected && r.vault) schedulePostUnmount(r.vault); // !r.redirected: never run the real-vault follow-up after a decoy unmount (would leak that a decoy exists)
		// Strip ONLY the sensitive field, keeping everything else. `redirected` is true only for a decoy unmount, so
		// echoing it would reveal that a decoy (hence a hidden real vault) exists, defeating deniability; it is consumed
		// server-side just above and the client never needs it. A BLACKLIST (not a whitelist) is essential here: the
		// deferred-drain path returns { ok:false, busy, flushing, error } so the client can show "still saving — wait a
		// moment" and NOT offer a force-unmount; a whitelist would drop those and make the UI offer a force that discards
		// in-progress writes. `redirected` only ever appears on the happy-path return, never on the busy/flushing returns.
		if (!r) return r;
		const { redirected, ...safe } = r; void redirected;
		return safe;
	}));
	...
	app.post('/api/web-password-set', handle(async (req, res) => {
		const body = req.body || {};
		// Defense in depth: when a web password is ALREADY set, require the current one to change it, so a momentarily
		// hijacked or left-open exposed session cannot lock the owner out by silently resetting the password. The
		// first-time set (no password yet — the only flow the interface uses this route for) needs no current password.
		const cur = await Vault.getUiAuth();
		if (cur.enabled && !(await UiAuth.verifyPassword(String(body.currentPassword || ''), cur.password))) {
			throw new Error('Enter your current web password to change it.');
		}
		await Vault.setUiPassword(body.password);
	...
	app.post('/api/lock-all', handle(async () => { stopAllLeaseHeartbeats(); Mobile.stopAll(); await stopAllServing(); await stopStoreServing(); require('../Onion').stopManagedTor(); const r = await Vault.lockAll({ owner }); afterLock(r.vaults, r.redirected); return r; })); // stopAllServing: "Lock all" cuts ALL access, so end any active serve too, matching travel mode — a served vault must not keep serving after a lock-everything action
	...
	app.post('/api/settings', handle(async (req) => {
		const body = req.body || {};
		// Coerce a numeric setting to a non-negative integer, treating a NON-FINITE input (a crafted "Infinity"/"1e999")
		// or NaN as 0 rather than storing it. `Number(x) || 0` alone passed Infinity through (Math.floor(Infinity) is
		// Infinity), which would persist a non-finite setting; these are all "0 = off, else N" knobs, so 0 is the safe floor.
		const nonNegInt = (v) => { const n = Math.floor(Number(v)); return Number.isFinite(n) && n > 0 ? n : 0; };
		if (body.autoLockMinutes != null) await Vault.setSettings({ autoLockMinutes: nonNegInt(body.autoLockMinutes) });
		...
		if (body.keepDiagnosticLog != null) { await Vault.setSettings({ keepDiagnosticLog: !!body.keepDiagnosticLog }); Common.setDiagnosticLogging(!!body.keepDiagnosticLog); } // persistent diagnostic log files; OFF by default (no on-disk vault-activity trail); applied live so it starts/stops writing at once
		if (body.privacyScreen != null) await Vault.setSettings({ privacyScreen: !!body.privacyScreen }); // blur vault names and paths in the UI until revealed; OFF by default (display-only, for screen sharing)
		if (body.versionsKeep != null) await Vault.setSettings({ versionsKeep: nonNegInt(body.versionsKeep) }); // file version history: 0 = off, else keep N snapshots
		if (body.versionsMaxAgeDays != null) await Vault.setSettings({ versionsMaxAgeDays: nonNegInt(body.versionsMaxAgeDays) }); // optional cap: drop snapshots older than N days (0 = off)
		if (body.versionsMaxSizeMB != null) await Vault.setSettings({ versionsMaxSizeMB: nonNegInt(body.versionsMaxSizeMB) }); // optional cap: trim oldest until .versions fits N MB (0 = off)
		if (body.wedgeRestart != null) await Vault.setSettings({ wedgeRestart: !!body.wedgeRestart }); // auto-restart a WEDGED (hung) service; OFF by default — a wedged drive otherwise stays locked and secured, never restarted
		if (body.autoAnonymize != null) await Vault.setSettings({ autoAnonymize: !!body.autoAnonymize }); // convert an existing named vault to the opaque format on its next unlock; ON by default
		if (body.ocr != null) { await Vault.setSettings({ ocr: { enabled: !!body.ocr, lang: 'eng', engine: 'tesseract' } }); if (body.ocr) Vault.ocrPrefetch().catch(() => {}); } // turn OCR of scanned PDFs/images on or off; on enable, fetch the language file now (detached) so the first index never waits on a download
		...
		if (body.p2pMode != null) { const m = String(body.p2pMode); await Vault.setSettings({ p2pMode: (m === 'relay' || m === 'direct') ? m : 'auto' }); } // how a peer is reached: Auto / Relay only / Direct only
		const s = await Vault.getSettings();
		return { settings: publicSettings(s) };
	}));

	// Critical-event notifications. Config is LOCAL-ONLY: the webhook URL can carry a token, and letting a network
	// caller read it or repoint it would leak it or hijack where a person's alerts go — so these refuse on an exposed
	// bind, like the other sensitive setup actions. Reads/writes the additive settings.notify section (an older client
	// simply ignores it).
	app.post('/api/notify-get', handle(async () => {
		if (exposed) throw new Error('Notification settings are available only from the app on the computer running it.');
		const Notify = require('../Notify');
		const n = (await Vault.getSettings()).notify || {};
		return {
			enabled: !!n.enabled,
			webhookUrl: typeof n.webhookUrl === 'string' ? n.webhookUrl : '',
			generic: !!n.generic,
			events: (n.events && typeof n.events === 'object') ? n.events : {},
			catalog: Object.entries(Notify.EVENTS).filter(([k]) => k !== 'test').map(([type, v]) => ({ type, title: v.title, defaultOn: v.defaultOn })),
		};
	}));
	app.post('/api/notify-set', handle(async (req) => {
		if (exposed) throw new Error('Notifications can only be set up from the app on the computer running it, not over a network connection.');
		const Notify = require('../Notify');
		const b = req.body || {};
		const url = typeof b.webhookUrl === 'string' ? b.webhookUrl.trim() : '';
		if (url && !/^https?:\/\//i.test(url)) throw new Error('A webhook URL must start with http:// or https://.');
		const events = {};
		if (b.events && typeof b.events === 'object') for (const k of Object.keys(Notify.EVENTS)) if (k in b.events) events[k] = !!b.events[k];
		await Vault.setSettings({ notify: { enabled: !!b.enabled, webhookUrl: url, generic: !!b.generic, events } });
		return { ok: true };
	}));
	app.post('/api/notify-test', handle(async (req) => {
		if (exposed) throw new Error('Sending a test notification is available only from the app on the computer running it.');
		const Notify = require('../Notify');
		// Test the URL provided in the request (so the user can test BEFORE saving), falling back to the saved one.
		const url = (req.body && typeof req.body.webhookUrl === 'string' && req.body.webhookUrl.trim()) || Notify.configFrom(await Vault.getSettings()).webhookUrl;
		if (!url) throw new Error('Add a webhook URL first.');
		if (!/^https?:\/\//i.test(url)) throw new Error('A webhook URL must start with http:// or https://.');
		const r = await Notify.emit({ type: 'test', message: 'This is a test notification from ' + Brand.name + '. If you can see this, your webhook is working.' }, { notify: { enabled: true, webhookUrl: url, events: { test: true } } });
		if (!r.delivered) throw new Error('The test could not be delivered: ' + (r.error || 'unknown reason') + '.'); // r.error is already a safe, generic reason (status or error class) that never contains the webhook host/token
		return { ok: true };
	}));
	...
	// Open the releases page in the user's real browser. The desktop WebView cannot open a window, so its "Releases"
	// link would otherwise be a dead click; the web client opens the link itself and never needs this. The URL is the
	// fixed, backend-owned releases location (never a client-supplied value), and this is a this-computer-only action,
	// so a remote caller is refused — the same rule the Recovery Kit print path follows.
	app.post('/api/open-releases', handle(async () => {
		refuseHostFsIfExposed('Opening the releases page');
		// Only the desktop app needs the backend to open the real browser; a normal browser opens the link itself. Gate
		// on isDesktopApp() so the backend enforces the same contract as the other host-browser routes, rather than
		// relying on the client to only call this in the desktop app.
		if (!Common.isDesktopApp()) return { opened: false };
		const opened = Common.openExternal(UpdateCheck.RELEASES_URL);
		if (!opened) throw new Error('Could not open your browser. The releases page is at ' + UpdateCheck.RELEASES_URL + '.');
		return { opened: true };
	}));
	...
	// Owner recovery (team vaults): split owner access across n trustees so any k of them can restore a lost owner.
	// Each route reuses the same Vault functions the CLI uses, so the GUI and CLI behave identically. The credential
	// gates each operation — setup needs the owner password, a trustee's share needs that trustee's private key, and
	// reconstruction needs a quorum of shares plus a new password (which the shares self-verify against the vault) —
	// so no extra exposed-bind gate is added beyond the credential itself, matching the other key/recovery routes.
	app.post('/api/owner-recovery-setup', handle(async (req) => {
		const { path: p, password, trustees, k } = req.body || {};
		return await Vault.setupOwnerRecovery(p, { password, trustees, k: Number(k) });
	}));
	app.post('/api/recovery-share', handle(async (req) => {
		const { path: p, key } = req.body || {};
		const share = await Vault.getRecoveryShare(p, key);
		return { share: share || null }; // null when this key holds no share for that vault (the client shows a clear "not a trustee" message)
	}));
	app.post('/api/owner-recover', handle(async (req) => {
		const { path: p, shares, newPassword, label } = req.body || {};
		return await Vault.recoverOwner(p, { shares, newPassword, label });
	}));
	...
	app.post('/api/mobile-start', handle(async (req) => {
		...
		// useMountedKey reuses the key an already-mounted vault holds (no password re-prompt); this-computer-only, so it
		// is refused on a network bind and only when no password was given (see mobilePrepare for the full rationale).
		const useMountedKey = !!local && !password && !exposed;
		const s = await Mobile.start(p, { password, local: !!local, useMountedKey });
		const out = { sessionId: s.sessionId, code: s.code, name: s.name, expiresAt: s.expiresAt, path: '/m/' };
		// In the packaged desktop app the single WebView has no second window to open into. Show the low-residue viewer
		// IN the app window itself rather than spawning the user's external browser (which takes them out of the app);
		// the viewer's Back control (session.local) returns to the vault list. Signal the client to navigate its own
		// WebView. Gated on the desktop app and a loopback bind, so a remote caller can never repoint this; a plain
		// browser (or an exposed bind) omits the flag and opens the viewer in its own new tab as before.
		if (local && Common.isDesktopApp() && !exposed) out.inApp = true;
		return out;
	}));
	...
	app.post('/api/emergency-enroll', handle(async (req) => { refuseEmergencyConfigIfExposed(); const { contactPubKey, contactLabel, inactivityDays, graceDays } = req.body || {}; return await Vault.emergencyEnroll({ contactPubKey, contactLabel, inactivityDays, graceDays }); }));
	app.post('/api/emergency-add-contact', handle(async (req) => { refuseEmergencyConfigIfExposed(); const { label, pubKey } = req.body || {}; return await Vault.emergencyAddContact({ label, pubKey }); }));
	app.post('/api/emergency-remove-contact', handle(async (req) => { refuseEmergencyConfigIfExposed(); return await Vault.emergencyRemoveContact((req.body || {}).contactId); }));
	app.post('/api/emergency-arm', handle(async (req) => {
		const { path: p, password, contactId, mode, dateMs } = req.body || {};
		// Validate a scheduled-unlock date here too, not only in the browser: a direct API call must never create a
		// past-dated (already-openable) grant. The library accepts any instant (its tests seal to a past round); the
		// user-facing edges — this route and the CLI — are what enforce "a real future moment."
		if ((mode === 'date' || mode === 'both') && !(Number.isFinite(Number(dateMs)) && Number(dateMs) > Date.now())) throw new Error('The unlock date must be a valid time in the future.');
		return await Vault.emergencyArm(p, { password, contactId, mode, dateMs });
	}));
	app.post('/api/emergency-backstop-set', handle(async (req) => { refuseEmergencyConfigIfExposed(); const { trustees, k } = req.body || {}; return await Vault.emergencySetBackstop({ trustees, k }); }));
	app.post('/api/emergency-backstop-clear', handle(async () => { refuseEmergencyConfigIfExposed(); return await Vault.emergencyClearBackstop(); }));
	app.post('/api/emergency-checkin', handle(async () => await Vault.emergencyCheckIn()));
	app.post('/api/emergency-disarm', handle(async () => { refuseEmergencyConfigIfExposed(); return await Vault.emergencyDisarm(); }));
	...
	app.post('/api/travel-start', handle(async (req) => { stopAllLeaseHeartbeats(); Mobile.stopAll(); await stopAllServing(); await stopStoreServing(); require('../Onion').stopManagedTor(); return await Vault.travelEnable({ travelPassword: (req.body || {}).password, owner }); }));
	...
	app.post('/api/heal', stream(async (req, onProgress) => {
		...
		const allowUnverified = !!(req.body || {}).allowUnverified; // the user's explicit "repair anyway" choice (unverified signature)
		const force = !!(req.body || {}).force; // the user's explicit "repair the same-size changes anyway" choice
		...
		// preserveInPlaceEdits: never silently revert a same-size change (indistinguishable from a same-length edit
		// without the key) — leave it and report it; the caller can pass force to repair it as genuine bit-rot.
		const heal = await Vault.heal(p, { allowUnverified, force, preserveInPlaceEdits: true, onProgress });
		return { verify, heal };
	}));
	...
	app.post('/api/serve-start', handle(async (req) => {
		const { path: p, bind, relayHost, relayToken, password, singleHop, stableOnion } = req.body || {};
		...
		return await startServing(p, { bind, useRelay, singleHop: !!singleHop, stableOnion: !!stableOnion }); // singleHop/stableOnion only take effect for an onion serve (see startServing)
	}));
	app.post('/api/serve-stop', handle(async (req) => await stopServing((req.body || {}).path)));
	// Can onion serve mode be offered? Yes when a Tor is already running OR this platform can download one automatically
	// (a first onion use then fetches a verified Tor once, like the storage engine). Hidden only on an unsupported
	// platform with no running Tor. `downloads` tells the UI a first use may fetch Tor.
	app.post('/api/tor-available', handle(async () => { const running = await require('../Onion').torAvailable(); const canDownload = !!require('../TorSetup').torSuffix(); return { available: running || canDownload, downloads: !running && canDownload }; }));
	// Read or set the GLOBAL Tor bridge setting (for reaching Tor from a network that blocks it). A read returns the
	// current mode and the built-in modes this install can offer (from the bundle's pt_config). Setting it restarts any
	// managed Tor so the new bridge config takes effect at once; a config-changing route, so refuse it on an exposed bind.
	app.post('/api/tor-bridges', handle(async (req) => {
		const b = req.body || {};
		if (b.set) {
			refuseOutboundIfExposed('a Tor bridge'); // a bridge line is an outbound target the operator sets from this machine
			// The new bridge config takes effect the next time a managed Tor starts with nothing published — the managed
			// Tor is NOT killed here, so changing the bridge can never drop a live onion serve out from under it.
			return await Vault.setTorBridges({ mode: b.mode, lines: b.lines });
		}
		let modes = []; try { const cfg = await require('../TorSetup').readPtConfig(); modes = cfg ? Object.keys(cfg.bridges || {}) : []; } catch (_) {}
		return { ...(await Vault.getTorBridges()), builtIn: modes };
	}));
	// Run a relay hub on THIS machine (the effortless, no-CLI path). Guarded to the local machine — a relay binds public
	// ports, so it is set up from the machine itself, never over a network-exposed UI. The token is generated strong;
	// the browser builds the one-paste invite code from the host + this token. Non-blocking: runHub returns as soon as
	// the listeners are bound. Starting while already running just returns the running hub.
	app.post('/api/relay-hub-start', handle(async () => {
		refuseOutboundIfExposed('a relay hub');
		if (relayHub) return relayHubInfo();
		const Relay = require('../Relay');
		const token = Relay.nodeId() + Relay.nodeId(); // 20 hex chars — comfortably above the minimum, and never shown on any command line
		const controlPort = Common.DEFAULT_RELAY_PORT;
		const portRange = [...Common.DEFAULT_RELAY_DATA_PORT_RANGE];
		const hub = Relay.runHub({ controlPort, token, portRange, onLog: () => {} });
		relayHub = { hub, controlPort, portRange, token, startedAt: Date.now() };
		return relayHubInfo();
	}));
	app.post('/api/relay-hub-stop', handle(async () => { if (relayHub) { try { relayHub.hub.close(); } catch (_) {} relayHub = null; } return { running: false }; }));

	// ---- Storage-node role: offer this machine as zero-knowledge off-site storage for other people's vaults ----
	// Enabling the role, minting an invite, and managing slots are loopback-only setup actions (like the relay hub) —
	// they bind listeners / mint credentials, so they must be done from the machine itself, never steered over the
	// network. Status and slot listing are plain reads. Only ciphertext ever lands here and nothing is ever adopted.
	app.post('/api/store-serve-start', handle(async (req) => {
		refuseOutboundIfExposed('a storage node');
		const b = req.body || {};
		if (b.relayHost) { refuseOutboundIfExposed('a relay host'); await Vault.setRelay({ host: b.relayHost, token: b.relayToken }); }
		return await startStoreServing({ bind: b.bind, useRelay: b.bind === 'relay' });
	}));
	app.post('/api/store-serve-stop', handle(async () => await stopStoreServing()));
	app.post('/api/store-status', handle(async () => await storeNodeStatus()));
	app.post('/api/store-invite', handle(async (req) => {
		refuseOutboundIfExposed('a storage node');
		if (!storeServe) throw new Error('Start the storage node first, then invite someone.');
		return await Vault.storeInvite(storeServe.handle, (req.body || {}).label);
	}));
	app.post('/api/store-slots', handle(async () => ({ slots: await Vault.listStoreSlots() })));
	app.post('/api/store-slot-remove', handle(async (req) => await Vault.removeStoreSlot((req.body || {}).slot)));
	app.post('/api/store-quota', handle(async (req) => { const mb = Math.max(0, Math.floor(Number((req.body || {}).quotaMB) || 0)); await Vault.setSettings({ storeQuotaMB: mb }); return { quotaMB: mb }; }));
	app.post('/api/relay-hub-status', handle(async () => {
		// The status carries the hub token (the invite-code secret). The hub can only be started or stopped from
		// loopback (relay-hub-start/stop are refuseOutbound-gated), so a network caller has no need for the raw token
		// here; withhold it on an exposed bind (report only that one is set) so an authenticated remote caller cannot
		// read the secret and hand out relay access. Loopback is unaffected.
		const info = relayHubInfo();
		if (exposed && info.token != null) { delete info.token; info.hasToken = true; }
		return info;
	}));
	app.post('/api/serve-info', handle(async (req) => {
		// Returns the served vault's login, password, and one-paste connection code. On an exposed bind require the
		// vault's own password (as serve-start does) so a caller holding only the web-login session cannot read the
		// credentials of a vault the owner is serving. Loopback keeps its passwordless single-user behavior.
		const b = req.body || {};
		await requireVaultReadIfExposed(b.path || '', b.password);
		return servingInfo(servedVaults.get(path.resolve(b.path || '')));
	}));
	// Receive (pull) a vault from another node by its connect code — the destination side of "move a vault to another
	// node". The code is minted by the source node's read-only serve; this node copies the ENCRYPTED store across,
	// verifies it is complete, and adopts it into the default vault home. Only ciphertext moves: the same password
	// opens the vault here, since the master key derives from the password plus the salt in the copied manifest.
	// refuseOutboundIfExposed: a receive makes an OUTBOUND connection to the source, which a network-exposed instance
	// must never be coerced into (mirrors serve-start's outbound guards).
	app.post('/api/receive-vault', handle(async (req) => {
		refuseOutboundIfExposed('receiving a vault');
		const b = req.body || {};
		if (!b.code) throw new Error('A connect code is required to receive a vault.');
		// The vault lands in the default vault home; receiveVault sanitizes the name, defaults it, and keeps it unique.
		return await Vault.receiveVaultFromCode(b.code, { toRoot: Vault.vaultsDir(), name: b.name });
	}));
	// Serve a vault read-only for a fleet MOVE and return a one-paste connect code the destination node pulls from. The
	// vault password authorizes it (requireVaultReadIfExposed) exactly as serve-start does — publishing a vault's
	// ciphertext must prove you can open it first — so a caller holding only node control cannot exfiltrate ciphertext
	// without the password. Read-only and same-network (LAN, TLS-pinned): only encrypted, opaque data is ever served.
	app.post('/api/serve-for-move', handle(async (req) => {
		const b = req.body || {};
		await requireVaultReadIfExposed(b.path || '', b.password);
		const info = await startServing(b.path, { bind: 'lan', readOnly: true });
		if (!info || !info.code) throw new Error('Could not start serving the vault for the move — the two machines must be on the same network for a move.');
	...
	// Build a printable Recovery Kit (identity + fingerprint + restore steps, and — unless addKey is false — a freshly
	// generated recovery key). The kit HTML (which may hold the one-time key) is NOT returned in this JSON; it is held
	// in memory and served at /kit-view over a short-lived token (viewable for a few minutes so it can be printed or
	// re-saved). That keeps the key off disk AND out of the JSON/network response, and lets the kit render under its
	// own CSP in a browser window or the in-app viewer.
	app.post('/api/recovery-kit', handle(async (req) => {
		const { path: p, password, addKey } = req.body || {};
		const r = await Vault.recoveryKit(p, { password, addKey: addKey !== false });
		// The kit's VALUES go to the client so it can show the kit natively IN the app window (reliable in every
		// WebView, and it keeps the feel uniform). The full PRINTABLE kit is held in memory and served only at
		// /kit-view, so printing (the one thing a WebView cannot do in place) can open in a real browser on demand.
		const token = kitView.stash(r.html);
		return { token, addKey: addKey !== false, identity: r.identity, fingerprint: r.fingerprint, recoveryKey: r.recoveryKey || null, desktop: Common.isDesktopApp() };
	}));
	// Open the printable kit in the user's REAL default browser — the only reliable way to print or save it as a PDF
	// from the desktop app, whose WebView has no working print (a known Tauri/WKWebView/WebKitGTK limitation). The
	// client sends the token it already holds; we only ever open OUR OWN live /kit-view URL for a token that is still
	// stashed, so this can never be turned into opening an arbitrary address. On the web this route is unused (the
	// browser opens the URL itself); it is safe there regardless because it opens only a validated same-machine URL.
	app.post('/api/recovery-kit-open', handle(async (req) => {
		// Opening the host's real browser is a this-computer-only action, like reveal and start-at-login: a network
		// caller must never make the machine running the service spawn a browser. It is only meaningful for the
		// loopback desktop app anyway (the web client opens the URL itself).
		refuseHostFsIfExposed('Printing the Recovery Kit');
		const token = String((req.body && req.body.token) || '');
		if (!kitView.has(token)) throw new Error('This Recovery Kit is no longer available to print. Create it again, then print it.');
		// The web client opens /kit-view itself; only the desktop WebView needs the backend to open the browser. Gate on
		// isDesktopApp() so this cannot spawn a host browser in standalone (matches open-releases and mobile-start).
		if (!Common.isDesktopApp()) return { opened: false };
		const opened = Common.openExternal('http://127.0.0.1:' + port + '/kit-view?token=' + encodeURIComponent(token));
		if (!opened) throw new Error('Could not open your browser to print the kit. You can note the recovery key shown here instead.');
		return { opened: true };
	}));
	...
	app.post('/api/consent-on-use', handle(async (req) => { const { path: p, on } = req.body || {}; return await Vault.setConsentOnUse(p, on !== false); }));
	app.post('/api/vault-color', handle(async (req) => { const { path: p, color } = req.body || {}; return await Vault.setVaultColor(p, color); }));
	...
	app.post('/api/content-reindex', stream(async (req, onProgress) => { const p = (req.body || {}).path; return await oncePerVault(reindexingNow, p, 'content reindex', () => Vault.contentReindex(p, { onProgress, allowOcr: true })); }));

	// A vault's encrypted metadata (display title + description). Sealed under the vault key, so both routes require
	// the vault's own password — that credential IS the gate (a locked vault reveals nothing, and a network caller can
	// read or change it only if it already holds the password), so no extra exposed-bind guard is needed. Setting
	// requires the read-write password; a read-only holder can view but not rename.
	app.post('/api/vault-meta-get', handle(async (req) => { const { path: p, password } = req.body || {}; return await Vault.getVaultMeta(p, password); }));
	app.post('/api/vault-meta-set', handle(async (req) => { const { path: p, password, fields } = req.body || {}; return await Vault.setVaultMeta(p, { password, fields }); }));
	// Hide an existing vault's folder name by renaming its folder to its opaque id. Renames only the vault's OWN folder
	// (to its own id — never a caller-named arbitrary path), requires the vault's read-write password, and the vault
	// must be unmounted. The returned `vault` is the new opaque path the client should switch to.
	app.post('/api/vault-anonymize', handle(async (req) => { refuseHostFsIfExposed('Hiding a vault folder name'); const { path: p, password } = req.body || {}; return await Vault.anonymizeVault(p, { password }); })); // renames a folder on THIS machine's disk — a host-filesystem mutation, refused over a network bind like the other host-FS routes
	...
	app.post('/api/note-save', handle(async (req) => { const { path: p, id, title, type, fields, note, body } = req.body || {}; return await Vault.noteSave(p, { id, title, type, fields, note, body }); }));
	app.post('/api/note-delete', handle(async (req) => { const { path: p, id } = req.body || {}; return await Vault.noteDelete(p, id); }));
	// Import logins/notes from another password manager's export. The file is uploaded as RAW bytes
	// (application/octet-stream, so the global 256 kB JSON parser skips it and a .1pux's binary ZIP survives intact);
	// the small metadata rides in the query string. This route's own raw parser caps the upload size. CSRF-safe: it
	// sits behind the same X-Vdisk header guard as every other API call. notesImport parses off the event loop
	// (async zlib) and writes each item through the normal noteSave path, so the vault must be open.
	app.post('/api/notes-import', express.raw({ type: () => true, limit: '48mb' }), handle(async (req) => {
		const q = req.query || {};
		const data = Buffer.isBuffer(req.body) ? req.body : Buffer.alloc(0);
		return await Vault.notesImport(String(q.path || ''), { text: data, format: q.format, filename: q.filename, dryRun: q.dry === '1', skipDuplicates: q.dedupe === '1' });
	}));
	// Password-health report over the open vault's logins. The breach check is opt-in (it makes an outbound request
	// to a fixed breach-corpus host, sending only a 20-bit hash prefix — see PwnedCheck); everything else is local.
	app.post('/api/notes-health', handle(async (req) => await Vault.notesHealth((req.body || {}).path, { breach: !!(req.body || {}).breach })));
	// Create a "Send" link for one item: seal it under a fresh key (returned once, for the link fragment — never
	// stored), and stash only the ciphertext with the chosen expiry, view limit, and optional link password. The
	// client composes the shareable URL as <origin>/s/<id>#<key>.
	app.post('/api/send-create', handle(async (req) => {
		const { path: p, id, ttlMinutes, maxViews, linkPassword } = req.body || {};
		// Pass the passphrase into the seal so it is folded into the decryption key (client-side zero-knowledge), AND
		// keep it on the record below for the server-side release gate + wrong-guess throttle — belt and suspenders.
		const sealed = await Vault.sealItemForSend(p, { id, password: linkPassword || undefined });
		const rec = sendStore.create({ ciphertext: sealed.ciphertext, meta: sealed.meta, ttlMs: (Number(ttlMinutes) > 0 ? Number(ttlMinutes) : 7 * 24 * 60) * 60 * 1000, maxViews: Number(maxViews) || 1, password: linkPassword || null, vaultPath: sealed.vault });
		return { id: rec.id, key: sealed.key, expiresAt: rec.expiresAt, maxViews: rec.maxViews };
	}));
	// Cancel a Send link before it expires or is opened (a mis-sent link). In-memory, so it also vanishes on restart.
	app.post('/api/send-revoke', handle(async (req) => ({ revoked: sendStore.revoke((req.body || {}).id) })));
	...
	app.post('/api/repair-schedule', handle(async (req) => {
		const b = req.body || {};
		return await Vault.saveRepairSchedule({ folders: b.folders, label: b.label, mode: b.mode, intervalHours: b.intervalHours, hour: b.hour, minute: b.minute, tz: b.tz, localHour: b.localHour, localMinute: b.localMinute });
	}));
	...
	app.post('/api/backup', handle(async (req) => {
		...
		// A local-folder destination is an arbitrary host write (it creates directories and copies the vault's key slots
		// to a caller-named path), so it is loopback-only like the other host-fs actions. An "sftp:<id>" destination is
		// an owner-configured remote target (added under the outbound guard) and stays allowed over a network bind.
		if (!(typeof dest === 'string' && dest.startsWith('sftp:'))) refuseHostFsIfExposed('Backing up to a folder on this computer');
	...
	app.post('/api/pack', stream(async (req, onProgress) => {
		...
		refuseHostFsIfExposed('Packing a vault into a portable file'); // writes a portable copy (with key slots) to a caller-named host folder, and may overwrite an existing file there — loopback-only, like unpack
	...
	app.post('/api/peer', handle(async (req) => {
		...
		if (peer.code) { const parsed = Vault.parsePeerCode(peer.code); if (!parsed) throw new Error('That connection code is not valid — copy it again from the other machine.'); return await Vault.savePeer({ label: peer.label, url: parsed.url, user: parsed.user, password: parsed.password, ca: parsed.ca, cand: parsed.cand, sig: parsed.sig }); }
	...
	// ---- Fleet: manage other Vaultonaut nodes from this hub ----
	// Enrolling a node makes this computer reach out to the node's address, so it is gated like the other outbound
	// routes (refused over a network-exposed connection to avoid an attacker steering an outbound request). The hub
	// stores only the node's address and a scoped monitoring token, never a vault key, and only ever reads the node's
	// /health (counts and status). verifyAndAdd reaches the node once before saving, so a typo'd address never enrolls.
	app.post('/api/fleet-add', handle(async (req) => { refuseOutboundIfExposed('a fleet node'); const b = req.body || {}; return await Fleet.verifyAndAdd({ label: b.label, url: b.url, token: b.token }); }));
	app.post('/api/fleet-remove', handle(async (req) => await Fleet.removeNode((req.body || {}).id)));
	// Trigger a SAFE action on a node (lock it, or refresh its self-check). Outbound to the node, so gated like the
	// other outbound routes; the action set is a fixed allowlist server-side (Fleet.FLEET_ACTIONS) — never destructive.
	app.post('/api/fleet-action', handle(async (req) => { refuseOutboundIfExposed('a fleet action'); const b = req.body || {}; return await Fleet.nodeAction(b.id, b.action); }));
	// Move a vault from one node to another. The hub connects out to BOTH nodes, so it is outbound-gated. It is fail-safe:
	// the source is retired only after the destination reports a VERIFIED receipt, and its data stays on disk regardless.
	// The vault password authorizes the source serve and is not stored; only encrypted data ever moves, node to node.
	app.post('/api/fleet-move', handle(async (req) => { refuseOutboundIfExposed('a fleet move'); const b = req.body || {}; return await Fleet.moveVault({ sourceId: b.sourceId, destId: b.destId, vaultPath: b.vaultPath, password: b.password, name: b.name }); }));
	// Push one of THIS machine's own vaults to a fleet node — the common one-click move. This machine serves the vault
	// read-only, the destination pulls and verifies it, and only then does this machine retire its copy (forget it; the
	// data stays on disk). Outbound-gated (the hub connects out to the destination). Fail-safe: a failed receive leaves
	// the source untouched, and the temporary serve is always stopped afterward.
	app.post('/api/fleet-push', handle(async (req) => {
		refuseOutboundIfExposed('a fleet move');
		const b = req.body || {};
		if (!b.destId) throw new Error('A destination node is required.');
		if (!b.path) throw new Error('A vault to move is required.');
		await requireVaultReadIfExposed(b.path, b.password); // authorize publishing this vault's ciphertext
		const info = await startServing(b.path, { bind: 'lan', readOnly: true });
		if (!info || !info.code) throw new Error('Could not start serving the vault for the move — the two machines must be on the same network.');
		try {
			const r = await Fleet.pushVaultToNode(b.destId, info.code, b.name);
			if (!r.ok) return { ok: false, stage: 'receive', error: r.error };
			// Verified receipt — retire the local source. A retire failure does not fail the move (the vault is safely on
			// the destination); it is reported so the interface can tell the user to remove the source copy manually.
			let retired = false, retireError = null;
			try { const rm = await Vault.removeKnownVault(b.path); if (rm && rm.path) readVaultState.evict(rm.path); retired = true; SelfCheck.run({ label: 'after fleet push', fresh: true }).catch(() => {}); }
			catch (e) { retireError = e && e.message || 'could not retire the source'; }
			return { ok: true, vault: r.vault, verdict: r.verdict, retired: retired, retireError: retireError };
		} finally {
			await stopServing(b.path).catch(() => {});
		}
	}));
	...
	// Restore a vault from a backup copy: copy the (already-encrypted) backup vault folder to a chosen local folder,
	// then add it to the known list so it appears immediately. Local-only: it writes a vault to an arbitrary path on
	// THIS machine, so like the other filesystem-writing setup actions it is refused over an exposed network bind. No
	// password is needed — the restored copy still opens only with its own password. Reuses Vault.restore (the same
	// code the CLI uses), so the GUI and CLI restore identically.
	app.post('/api/restore', handle(async (req) => {
		refuseHostFsIfExposed('Restoring a backup'); // writes a vault to a chosen local path, like import/reconstruct/unpack — refuse over a network bind
		const { source, dest, overwrite } = req.body || {};
		if (!source || !dest) throw new Error('Choose both the backup vault folder and where to restore it.');
		const r = await Vault.restore(source, dest, { overwrite: !!overwrite });
		await Vault.addKnownVault(r.vault).catch(() => {}); // surface the restored vault in the list; a failure here is non-fatal (the copy already succeeded)
		SelfCheck.run({ label: 'after restore', fresh: true }).catch(() => {});
		return r;
	}));
	...
	app.post('/api/secure-remove', handle(async (req) => {
		...
		// Match what the UI actually shows: the friendly title (the decrypted name from the local cache) for an opaque
		// vault, else the folder name. Accept the opaque folder name too as a fallback, so a machine whose cache lacks
		// the title (where the list falls back to showing the folder id) can still confirm by typing that id.
		const shown = await Vault.friendlyName(abs);
		const folder = Vault.displayName(abs);
		const typed = String(confirmName == null ? '' : confirmName).trim();
		if (typed !== shown && typed !== folder) throw new Error('The typed name did not match "' + shown + '", so nothing was deleted.');
		...
		try { sendStore.revokeByVault(abs, Common.samePath); } catch (_) {} // a crypto-erased vault must leave no live Send link that could still hand out one of its items
	...
	app.use((err, req, res, next) => {
		...
		// A malformed or oversized body is EXPECTED client input, not a server fault, and this parser runs before the
		// auth gate — so on a network-exposed instance a scanner could drive endless console/log noise by spraying junk
		// bodies. Answer those with a 400 and no log line; only a genuine server fault (a real 500) is worth a warning.
		if (!(bad || tooBig)) { try { Common.warn('[web] request error: ' + ((err && err.message) || err)); } catch (_) {} }
	...
	return new Promise((resolve, reject) => {
		...
		server.listen(port, bindAddr, () => {
			...
			if (!exposed && runDirHardened) { Common.writeJsonAtomic(servicePidFile, { pid: process.pid, at: new Date().toISOString(), url: 'http://127.0.0.1:' + port, token: ownerToken }).catch(() => {}); }
			if (exposed) Common.log('Exposed to the network with a self-signed certificate — expect a browser trust prompt the first time.');
			// Run one housekeeping sweep at boot BEFORE the startup self-check, then fire the check. sweep() finishes or
			// rolls back any crash-interrupted key rotation (resumeRekey) for every vault — its own stated intent is that
			// this happens "at service start," but until now it ran only on the 15-minute timer, so the stuck_rekey check
			// could fire before the automatic resume had a chance and show a false "unfinished rotation" alarm at boot.
			// Chaining the check after the sweep resolves gives the resume its chance first. Both are fire-and-forget and
			// warn-only (after listen), so they never delay startup or disturb anything — they just record findings.
			Vault.sweep().catch(() => {}).finally(() => { SelfCheck.run({ label: 'startup' }).catch(() => {}); });
