...
async function startServing(vaultPath, { bind, useRelay, readOnly, singleHop } = {}) {
	const key = path.resolve(vaultPath);
	...
	const p = (async () => {
		...
		const handle = await Vault.serveVault(vaultPath, { bind: (relay || isLan || isWan || isOnion) ? undefined : bind, lan: isLan, wan: isWan, onion: isOnion, singleHop: isOnion && !!singleHop, relay, readOnly: readOnly || undefined, onEvent });
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, wan: !!handle.wan, onion: handle.onion || null, onionAuth: handle.onionAuth || null, singleHop: !!handle.singleHop, cand: handle.candidates || [], sig: handle.sig || null, bind: relay ? 'relay' : (isLan ? 'lan' : (isWan ? 'wan' : (isOnion ? 'onion' : (bind || '127.0.0.1')))), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	return { serving: true, failed: !!e.failed, url: e.url, user: e.user, pass: e.pass, bind: e.bind, secure: e.secure, wan: !!e.wan, onion: e.onion || null, singleHop: !!e.singleHop, code, startedAt: e.startedAt };
}
...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/serve-start', handle(async (req) => {
		const { path: p, bind, relayHost, relayToken, password, singleHop } = req.body || {};
		...
		return await startServing(p, { bind, useRelay, singleHop: !!singleHop }); // singleHop only takes effect for an onion serve (see startServing)
	}));
