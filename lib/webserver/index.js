...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/travel-start', handle(async (req) => { stopAllLeaseHeartbeats(); Mobile.stopAll(); await stopAllServing(); await stopStoreServing(); return await Vault.travelEnable({ travelPassword: (req.body || {}).password, owner }); }));
	...
	// Is a Tor reachable, so onion serve mode can be offered? A fast best-effort probe; onion mode stays hidden otherwise.
	app.post('/api/tor-available', handle(async () => ({ available: await require('../Onion').torAvailable() })));
