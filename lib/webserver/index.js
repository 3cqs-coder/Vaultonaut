...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/mobile-start', handle(async (req) => {
		...
		// A local viewer of a vault that is already MOUNTED on this machine may reuse the key the open session holds, so
		// the user is not asked for the password again. This is strictly this-computer-only: refused on a network bind
		// (a remote caller must always supply the password), and only when no password was given.
