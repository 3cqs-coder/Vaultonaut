	...
	// gets a 401 the browser turns into a redirect.
	{
		...
		app.post('/login', async (req, res) => {
			...
			if (!ok) {
				...
				// so a normal browser login works. On loopback that forged POST shares the user's own 127.0.0.1, so
				// counting its failure toward the per-IP lockout would let any open web page lock the user out of their own
				// UI. The browser sets Sec-Fetch-Site and a page cannot forge it, so only count a failure that is
				// same-origin — or that carries no such header at all (an older browser or the CLI), where the brute-force
				// lockout must still apply. A cross-site value is simply not counted; the password check still ran.
				const sfs = String(req.get('sec-fetch-site') || '').toLowerCase();
				if (!(sfs && sfs !== 'same-origin' && sfs !== 'none')) loginFails.fail(ip, now);
