...
async function startServing(vaultPath, { bind, useRelay } = {}) {
	...
	const p = (async () => {
		...
		const entry = { handle, url: handle.url, user: handle.user, pass: handle.pass, ca: handle.ca || null, secure: !!handle.secure, bind: relay ? 'relay' : (isLan ? 'lan' : (bind || '127.0.0.1')), startedAt: new Date().toISOString() };
...
function servingInfo(e) {
	...
	const code = (e.bind === 'relay' || e.bind === 'lan') ? Vault.makePeerCode({ url: e.url, user: e.user, pass: e.pass, ca: e.ca }) : null;
