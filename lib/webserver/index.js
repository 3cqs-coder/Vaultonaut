...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp } = {}) {
	...
	app.post(/^\/s\/redeem\/(.+)$/, sendCsp, (req, res) => {
		...
		if (sendFails.blocked(ip, nowMs)) return res.status(429).json({ error: 'Too many attempts — wait a moment and try again.' });
		const r = sendStore.redeem(req.params[0], (req.body || {}).password);
		if (r.gone) { sendFails.clear(ip); return res.status(410).json({ gone: true }); }
		if (r.badPassword) { sendFails.fail(ip, nowMs); return res.status(401).json({ badPassword: true }); }
		sendFails.clear(ip);
	...
	app.post('/api/send-create', handle(async (req) => {
		...
		const rec = sendStore.create({ ciphertext: sealed.ciphertext, meta: sealed.meta, ttlMs: (Number(ttlMinutes) > 0 ? Number(ttlMinutes) : 7 * 24 * 60) * 60 * 1000, maxViews: Number(maxViews) || 1, password: linkPassword || null });
