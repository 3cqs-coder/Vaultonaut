...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/emergency-add-contact', handle(async (req) => { const { label, pubKey } = req.body || {}; return await Vault.emergencyAddContact({ label, pubKey }); }));
	app.post('/api/emergency-remove-contact', handle(async (req) => await Vault.emergencyRemoveContact((req.body || {}).contactId)));
	...
	app.post('/api/emergency-backstop-set', handle(async (req) => { const { trustees, k } = req.body || {}; return await Vault.emergencySetBackstop({ trustees, k }); }));
	app.post('/api/emergency-backstop-clear', handle(async () => await Vault.emergencyClearBackstop()));
