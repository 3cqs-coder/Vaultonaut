...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	// gets a 401 the browser turns into a redirect.
	{
		...
		app.use(async (req, res, next) => {
			...
			// A headless / integration client can authenticate to the API with a bearer API token instead of the browser
			// session cookie (see "vdisk api-token"). Only the API accepts it; page requests still use the login form.
			if (req.path.startsWith('/api')) {
				const m = /^Bearer\s+(.+)$/i.exec(req.get('Authorization') || '');
				if (m && await Vault.verifyApiToken(m[1].trim())) return next();
				return res.status(401).json({ ok: false, error: 'Please sign in again.', login: true });
			}
