...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/settings', handle(async (req) => {
		...
		if (body.ocr != null) await Vault.setSettings({ ocr: { enabled: !!body.ocr, lang: 'eng', engine: 'tesseract' } }); // turn OCR of scanned PDFs/images on or off; the language file is fetched on the next reindex when enabled
