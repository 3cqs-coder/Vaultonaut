...
async function start(port = Common.DEFAULT_UI_PORT, { bind, allowIp, denyIp, readyToken } = {}) {
	...
	app.post('/api/unmount', handle(async (req) => {
		...
		// Return a CURATED subset — never the raw result. `redirected` is true only for a decoy unmount, so echoing it to
		// the client would reveal that a decoy (hence a hidden real vault) exists, defeating the deniability the feature
		// exists for. It is consumed server-side just above; the client never needs it. (Mirrors the mount handler, which
		// likewise returns a curated object.) `vault` here is presentVault (the real path stored in state), safe to return.
		return r ? { ok: r.ok, mountpoint: r.mountpoint, vault: r.vault, forced: r.forced, stuck: r.stuck, recovered: r.recovered } : r;
	}));
