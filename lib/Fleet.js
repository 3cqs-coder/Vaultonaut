...
function postAction(nodeUrl, token, endpoint, body, postOpts) {
	return new Promise((resolve) => {
		let u; try { u = new URL(endpoint, nodeUrl.replace(/\/*$/, '/')); } catch (e) { return resolve({ ok: false, error: 'bad url' }); }
		...
		// Most actions are quick (the default cap keeps one stuck node from stalling the hub). A few — a vault receive —
		// copy a whole store, so the caller passes a longer timeout for those; the node itself still bounds the work.
		const opts = { method: 'POST', headers: headers, timeout: (postOpts && postOpts.timeoutMs) || POLL_TIMEOUT_MS };
...
// A receive copies the whole encrypted store, which can take a while on a large vault, so the hub waits longer for that
// one step than for a quick control action; the receiving node still bounds the copy with its own hard timeout.
const MOVE_RECEIVE_TIMEOUT_MS = 2 * 60 * 60 * 1000;

// Move a vault from one fleet node to another, coordinated by the hub. This crosses the deliberately non-destructive
// FLEET_ACTIONS boundary, so it is a dedicated, auditable flow rather than a generic allow-listed action. It is
// FAIL-SAFE and keys never touch the hub:
//   1. the SOURCE serves the vault read-only and mints a connect code (authorized by the vault password, which only
//      proves the caller can open the vault — it is not stored, and only ciphertext is ever served);
//   2. the DESTINATION pulls that ciphertext, verifies it is COMPLETE, and adopts it (the same password opens it there,
//      since the key derives from the password + the copied salt — nothing secret crosses the wire);
//   3. ONLY after a verified receipt, the source RETIRES its copy (forgets it from its list; the data stays on disk, so
//      a failure never loses anything — the user deletes the source folder once satisfied).
// If the receive fails, the source is left exactly as it was. The temporary serve is always stopped afterward. `deps.post`
// is a test seam (defaults to the real authenticated POST). Non-blocking: every step is an async request with a timeout.
async function moveVault({ sourceId, destId, vaultPath, password, name } = {}, deps = {}) {
	const post = deps.post || postAction;
	const list = await load();
	const source = list.find((n) => n.id === sourceId);
	const dest = list.find((n) => n.id === destId);
	if (!source) throw Object.assign(new Error('Unknown source node.'), { code: 'NO_NODE' });
	if (!dest) throw Object.assign(new Error('Unknown destination node.'), { code: 'NO_NODE' });
	if (source.id === dest.id) throw Object.assign(new Error('The source and destination must be different nodes.'), { code: 'SAME_NODE' });
	if (!vaultPath) throw Object.assign(new Error('A vault to move is required.'), { code: 'NO_VAULT' });
	const errText = (r) => (r && (r.error || (r.json && r.json.error) || (r.status ? 'HTTP ' + r.status : null))) || 'the node did not respond';
	// 1. Source serves the vault read-only and returns a one-paste connect code.
	const served = await post(source.url, source.token, '/api/serve-for-move', { path: vaultPath, password });
	if (!served.ok || !served.json || !served.json.code) return { ok: false, stage: 'serve', error: errText(served) };
	try {
		// 2. Destination pulls, verifies completeness, and adopts. Long timeout: this copies the whole store.
		const received = await post(dest.url, dest.token, '/api/receive-vault', { code: served.json.code, name }, { timeoutMs: MOVE_RECEIVE_TIMEOUT_MS });
		if (!received.ok || !received.json || !received.json.vault) return { ok: false, stage: 'receive', error: errText(received) };
		// 3. Only now retire the source copy — never before a verified receipt.
		const retired = await post(source.url, source.token, '/api/remove-vault', { path: vaultPath });
		return { ok: true, vault: received.json.vault, verdict: received.json.verdict || 'RECEIVED', retired: !!retired.ok, retireError: retired.ok ? null : errText(retired) };
	} finally {
		try { await post(source.url, source.token, '/api/serve-stop', { path: vaultPath }); } catch (_) {}
	}
}

// Push a vault THIS machine already serves (by its just-minted connect code) to a destination node, and report whether
// the destination received and verified it. Used by the common "move one of my own vaults to a node" flow, where the
// source is local: the webserver serves the vault read-only and retires it locally, and this only handles the one
// remote step (the destination pull). Never throws; `deps.post` is a test seam. Non-blocking (a long-timeout request).
async function pushVaultToNode(destId, code, name, deps = {}) {
	const post = deps.post || postAction;
	const dest = (await load()).find((n) => n.id === destId);
	if (!dest) throw Object.assign(new Error('Unknown destination node.'), { code: 'NO_NODE' });
	if (!code) throw Object.assign(new Error('A connect code is required.'), { code: 'NO_CODE' });
	const r = await post(dest.url, dest.token, '/api/receive-vault', { code, name }, { timeoutMs: MOVE_RECEIVE_TIMEOUT_MS });
	const okReceipt = !!(r.ok && r.json && r.json.vault);
	return { ok: okReceipt, vault: okReceipt ? r.json.vault : null, verdict: (r.json && r.json.verdict) || null, error: okReceipt ? null : (r.error || (r.json && r.json.error) || (r.status ? 'HTTP ' + r.status : 'the node did not respond')) };
}
...
module.exports = {
	...
	setAlertHook, detectTransitions, effectiveStatus, tick, nodeAction, moveVault, pushVaultToNode, FLEET_ACTIONS,
