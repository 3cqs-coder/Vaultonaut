...
function waitListening(host, port, proc, timeoutMs = 8000) {
	...
	// A server bound to a WILDCARD address (0.0.0.0 / :: — used for a LAN or WAN serve) also accepts on loopback, so
	// probe loopback there. Connecting TO 0.0.0.0 maps to loopback on macOS and Linux but Windows rejects it with
	// WSAEADDRNOTAVAIL, which would make every readiness probe fail and tear down an engine that is actually listening.
	const probeHost = (host === '0.0.0.0' || host === '::' || !host) ? '127.0.0.1' : host;
	return new Promise((resolve, reject) => {
		let done = false;
		let onExit, onError;
		const cleanup = () => { proc.removeListener('exit', onExit); proc.removeListener('error', onError); };
		const finish = (fn, val) => { if (done) return; done = true; cleanup(); fn(val); };
		// Listen for BOTH 'exit' and 'error'. A process that starts and then quits emits 'exit'; a process that never
		// launches at all (e.g. the engine binary was removed mid-run → ENOENT) emits 'error' and NO 'exit'. Without the
		// 'error' handler that failure would not reject here until the full connect-retry deadline elapsed, and with a
		// misleading "did not start listening in time" message instead of the real launch error.
		onExit = (code) => finish(reject, new Error('The server stopped before it was ready (exit ' + code + ').'));
		onError = (err) => finish(reject, err instanceof Error ? err : new Error('The server failed to start: ' + String(err)));
		proc.once('exit', onExit);
		proc.once('error', onError);
		const tryOnce = () => {
			if (done) return;
			const sock = net.connect({ host: probeHost, port }, () => { sock.destroy(); finish(resolve); });
			sock.once('error', () => { sock.destroy(); if (done) return; if (Date.now() > deadline) { finish(reject, new Error('The server did not start listening in time.')); } else { const t = setTimeout(tryOnce, 150); if (t.unref) t.unref(); } }); // unref: a pending retry must not keep the event loop alive after the caller has moved on
		};
		tryOnce();
	});
}
...
async function serveWebdav(bin, dir, { host = '127.0.0.1', port, user, pass, readOnly = false, onLine, cert, key } = {}) {
	// Argument-injection guard: `dir` is a bare positional argv token (the ciphertext folder to serve). A value that
	// began with "-" would be read by the engine as a flag rather than a path. A served folder is always a real path,
	// so a leading "-" is never legitimate — reject it rather than hand the engine an attacker-shaped flag.
	if (dir != null && String(dir).startsWith('-')) throw new Error('Refusing to serve: the folder path may not start with "-" (it would be read as a command-line option).');
	...
	if (onLine) {
		...
		// No line consumer: still DRAIN both pipes. This is a long-lived, often relay/network-reachable serve, and the
		// engine logs to stderr (auth failures from port scans, notices). With the pipes created (stdio 'pipe') but
		// unread, the OS pipe buffer fills after ~64 KiB and the engine blocks on its next write to that fd — the
		// classic unconsumed-pipe deadlock, which the supervisor would not catch because the process stays alive.
		// Resuming discards the bytes so the engine never blocks; error handlers keep a pipe reset from ever surfacing
		// as an uncaughtException in a non-resilient one-shot caller.
		if (proc.stdout) { proc.stdout.resume(); proc.stdout.on('error', () => {}); }
		if (proc.stderr) { proc.stderr.resume(); proc.stderr.on('error', () => {}); }
	}
